...
- An option carried on a prototype, or defined non-enumerably, is seen by the check that refuses unknown options. It read own enumerable names only, so `Object.create({ password: "pw" })` and a `password` defined with `enumerable: false` both answered `opts.password` while showing the check nothing -- and `pki.key.export` returned the private key in the clear, which is the case the refusal exists to prevent, reached by a different object shape. Inherited non-enumerable members are still ignored: `constructor` and the methods class syntax defines are machinery, not options, and an instance of a caller's own class remains a valid options bag.
