...
- The key is derived as RFC 2875 section 3 states it: SHA-1 over the authority certificate's subject name, the agreed secret, and its issuer name, in that order. Section 4.3 settles which certificate those names come from and what to do when one is absent, so a certificate whose subject or issuer name is empty contributes its alternative name instead, and one carrying neither is refused rather than keyed from nothing.
