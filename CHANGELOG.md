...
- The same rule holds for verificationMaterial, whose certificate and x509CertificateChain are a oneof of the same kind. Both arms set drew the leaf certificate from one and the chain it is validated through from the other, so the certificate whose key was checked and the certificates it was checked through came from different statements about the same signer.
