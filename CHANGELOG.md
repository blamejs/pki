...
- pki.path.validate refuses a name constraint whose base it cannot read, instead of treating it as one that matches nothing. An rfc822Name, dNSName, or uniformResourceIdentifier base outside the form its tag names, such as one carrying an empty label ("..example.com"), a label edged with a hyphen ("-bad.example.com"), or a second trailing dot (".example.com.."), now fails the certificate with path/name-constraint-unsupported. Before this, such a base matched no name, so an excluded subtree built on one excluded nothing: a sub-CA required to carry an exclusion could hold one that inspection renders in full and validation never applies. The permitted direction already failed closed. A caller's base has always been refused at the door by pki.trust.anchor and by opts.initialPermittedSubtrees and opts.initialExcludedSubtrees; this is the same rule reaching a base that a certificate carries in its own nameConstraints extension, where there is no door. A dNSName base that is a bare dot still names the root of the namespace and still matches every name.
