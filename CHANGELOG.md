...
- A Full PKI Response must be authenticated before it is interpreted (RFC 5272 sec. 3.2.1.3.4), and the opt-out is named rather than silent. Pass certs and the CMS signature is verified; pass allowUnverified: true and the verdict reports signatureVerified: false. Doing neither is refused, so no caller ends up assuming a check that did not happen. A carrier with no signer at all is refused outright. pki.est.fullcmc threads this through as responderCerts / allowUnverifiedResponse, on both the success and the rejection paths.
