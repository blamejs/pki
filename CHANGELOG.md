...
- The catalogue's freshness is enforced rather than reported. A BLOB whose sequence number does not exceed the one the caller already holds is refused as a rollback, and requireRollbackCheck makes supplying that number mandatory so the check cannot be skipped by forgetting the option. A BLOB past its nextUpdate is refused as stale. Both fail closed and both are opt-outable only by the caller.
