...
- opts.authorizedBySharedSecret states the exemption section 4.1.6 gives an entity that protected its request with a shared secret, which MAY authorize the authority by that secret instead of validating its certificate. It applies only to a container opened with opts.password, it refuses opts.trustAnchors so that one call names one authorization rule, and the result reports trusted: false.
...
- pki.path.validate refuses a name constraint whose base no name could ever match, instead of treating it as one that matches nothing. An rfc822Name, dNSName, or uniformResourceIdentifier base carrying an empty label, written as a doubled dot ("..example.com", "a..b.example.com"), a second trailing dot (".example.com.."), or a base of dots alone (".."), now fails the certificate with path/name-constraint-unsupported. Before this, such a base matched no name, so an excluded subtree built on one excluded nothing: a sub-CA required to carry an exclusion could hold one that inspection renders in full and validation never applies. The permitted direction already failed closed. This is the comparison's own question, which is narrower than the well-formed host name a caller is held to at the door, so a base that is not a host name and still names a namespace the comparison can place, such as a label carrying an underscore, a label edged with a hyphen, or a label longer than 63 characters, keeps being enforced exactly as before. A dNSName base that is a bare dot still names the root of the namespace and still matches every name.
...
- pki.cmp.session accepts opts.authorizedBySharedSecret alongside opts.trustAnchors. On a session those anchors do two jobs, authorizing the key generation authority and path-validating the issued certificate, and the exemption replaces only the first, so a caller states both to authorize the authority by the shared secret and still chain the certificate it was issued. The pair previously reached the door of pki.cmp.openKeyPackage, which knows the first job alone and refuses it, and the refusal landed after the authority had generated and delivered the key, with the session's one transaction slot already spent.
- pki.cmp.build sets cmp2021 on an ir / cr / kur that asks for central key generation. RFC 9480 section 2.20 needs the version when a message exchange uses EnvelopedData, "for the request being sent or for the expected response", and RFC 9483 section 4.1.6 requires it in the header "already for the initial request message". A request carrying a real public key still goes out as cmp2000, unless its proof of possession is the agreeMAC or encryptedKey method, which has always carried the later version.
