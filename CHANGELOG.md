...
- The authority certificate is read in both encodings a finite-field key is written in. An X.509 Diffie-Hellman certificate names dhpublicnumber and carries X9.42 DomainParameters (RFC 3279 section 2.3.3), where the runtime classifies only the PKCS#3 form, so the key is rewritten into the form it reads. DomainParameters also state q, the order of the subgroup the public value must lie in, which the PKCS#3 form cannot carry: it is checked before the parameter stating it is dropped, and a value outside that subgroup is refused rather than agreed with, since the secret it produces would expose the requester's private exponent modulo the small order it actually has. The stated q is held against p and g rather than believed, and the cofactor is held against p and q where it is given. p and q must both be prime, p because every statement the parameters make is about the group modulo a prime, and q because that is what makes the subgroup test mean anything, and q must also be large enough to hide an exponent, since a prime q makes the order of the public value exactly q and three is prime: p and q both prime and q equal to three admits a public value of order three. The floor is the smallest subgroup order NIST SP 800-56A states for a finite-field group. a composite q bounds the public value's order to a divisor of q rather than to q, and a certificate stating q = p-1 would otherwise satisfy every structural test, since g raised to p-1 is 1 for the whole group. The cofactor and the validation parameters are each optional and independent, so a fourth field is read by its tag: validation parameters are accepted with no cofactor before them, and are held to the seed BIT STRING and pgenCounter INTEGER they consist of. The public key BIT STRING must hold whole octets, since the codec admits a nonzero unused-bit count whose padding bits are zero and the conversion carries that BIT STRING through unchanged, which would re-emit a second spelling of one key rather than normalize it. Every container is read for the tag it carries rather than for having children, since a context-specific constructed value in place of the DomainParameters SEQUENCE would otherwise be rewritten into a PKCS#3 key that imports while the request still carried the original the recipient's own reader refuses. The parameters are sized before anything is computed with them, since every one of these tests raises one operand to the power of another and proves a modulus prime, while the DER reader admits an integer far wider than any Diffie-Hellman group. The bound is 4096 bits, which covers every MODP group a certificate carries; an authority certificate exceeding it is refused on its size rather than by computing with it. The same group tests apply to both encodings: the PKCS#3 form needs no rewrite, so it is checked in its own right rather than arriving unexamined because it happened not to need converting. It states no subgroup order, so there is nothing to test the public value against and the group itself must be one whose only small orders the range already excludes: a safe prime, whose element orders are one, two, r and 2r alone. A modulus that is merely prime is not enough, since p-1 may carry small factors and an order-three public value then sits inside the range. Whichever way the encoding states the order, that order carries the same floor, since being a safe prime bounds the order to (p-1)/2 and seven is a safe prime whose own subgroup has order three.
