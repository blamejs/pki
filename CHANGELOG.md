...
- Path validation appends to every one of its own lists the same way. The RFC 5280 state machine reads its answer back out of those lists, so the name forms a name-constraint check compares, the CRLs a revocation decision scans, the policy-tree nodes that produce the user-constrained set, and the candidate and anchor lists a build searches are all written where an indexed setter cannot reach them.
