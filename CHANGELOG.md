...
- The exponent the requester agrees with must be one the authority cannot work out. RFC 2631 section 2.2 states X9.42's own requirement, that the private key lie in the interval [2, q-2], and the exponents outside it agree a secret that follows from the authority's certificate alone: zero agrees one, one agrees the authority's public value itself, and q-1 agrees the inverse of that value. A MAC keyed from any of them proves possession of nothing, since whoever holds the certificate can compute the same key. The exponent is read off the public value the key states rather than out of the key, so the test needs no copy of private material and no stated order.
