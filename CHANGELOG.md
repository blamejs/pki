...
- `pki.cms.countersign` adds the countersigner's digest algorithm to the SignedData `digestAlgorithms` set. RFC 5652 section 5.1 has that set list the digests every signer employs so a consumer can process the message in one pass, and section 11.4 makes a countersignature a SignerInfo. The set is covered by no signature in the message, so every signature a countersigned message already carried still verifies.
