...
function _normalizeAlg(algorithm, who) {
  ...
  var out = guard.identifier.ownOptions(alg);
...
var HASH_NODE = intrinsic.assign(intrinsic.create(null), {
  "SHA-1":   "sha1",
  "SHA-224": "sha224",
  "SHA-256": "sha256",
  "SHA-384": "sha384",
...
var XOF_NODE = intrinsic.assign(intrinsic.create(null), {
...
var HMAC_BLOCK_BITS = intrinsic.assign(intrinsic.create(null), {
...
var ML_KEM_INNER = intrinsic.create(null);
...
var PKCS8_INNER_VALIDATORS = intrinsic.create(null);
...
function _preValidateSpki(spki, name) {
  if (name !== "ECDSA" && name !== "ECDH") return;
  var params;
  try {
    var alg = asn1.decode(spki).children[0];
    params = alg.children.length > 1 ? alg.children[1] : null;
  } catch (e) { throw new WebCryptoError("webcrypto/data", "importKey spki: the SubjectPublicKeyInfo is not well-formed", e); }
  if (params && params.tagClass === "universal" && !params.constructed && params.tagNumber === asn1.TAGS.OBJECT_IDENTIFIER) return;
  throw new WebCryptoError("webcrypto/data",
    "importKey spki: an " + name + " key must name its curve as a namedCurve OBJECT IDENTIFIER, " +
    "and this SubjectPublicKeyInfo carries " + (params ? "explicit domain parameters" : "none") + " (RFC 5480 sec. 2.1.1)");
}
...
/** @internal Each inner set is asked whether it holds the key's own type, so it answers from its own
 * entries alone: a type naming an Object.prototype member would otherwise resolve to a truthy
 * inherited value and pass a key the algorithm does not accept. */
function _typeSet(names) {
  var t = intrinsic.create(null);
  _forEach(names, function (n) { t[n] = 1; });
  return t;
}
var IMPORT_KEY_TYPE = intrinsic.assign(intrinsic.create(null), {
  "RSASSA-PKCS1-V1_5": _typeSet(["rsa"]), "RSA-PSS": _typeSet(["rsa", "rsa-pss"]), "RSA-OAEP": _typeSet(["rsa"]),
  "ECDSA": _typeSet(["ec"]), "ECDH": _typeSet(["ec"]),
  "ED25519": _typeSet(["ed25519"]), "ED448": _typeSet(["ed448"]),
  "X25519": _typeSet(["x25519"]), "X448": _typeSet(["x448"]),
});
[ML_DSA_NODE, ML_KEM_NODE, SLH_DSA_NODE].forEach(function (m) {
  _forEach(_keys(m), function (webName) { IMPORT_KEY_TYPE[webName] = _typeSet([m[webName]]); });
});
...
function _curveFromKey(ko) {
  try {
    ...
    var curves = guard.identifier.optionNames(CURVE_NODE);
    for (var k = 0; k < curves.length; k++) { if (jwk.crv === curves[k]) return curves[k]; }
  }
  catch (_e) { }
  return undefined;
}
...
// @internal
/** @internal `want` names the WebCrypto export format, defaulting to the DER one the key's type
 * calls for. A key from another implementation can be reached three ways, and which of them works
 * is a property of the key rather than of the format asked for, so every format is exported
 * through the same three; a format that went straight to this engine's `exportKey` would refuse a
 * key this engine did not make while the others returned it. A JWK is an object rather than
 * bytes, so only the byte formats are wrapped. */
function exportAnyKey(key, E, code, want) {
  var format = want || _EXPORT_FORMAT[key && key.type];
  if (!format) throw E(code, "a CryptoKey with a private, public, or secret type is required");
  if (want === "jwk" && !_EXPORT_FORMAT[key && key.type]) {
    throw E(code, "a CryptoKey with a private, public, or secret type is required");
  }
  var asJwk = format === "jwk";
  function wrap(b) { return asJwk ? b : _bufferFrom(b); }
