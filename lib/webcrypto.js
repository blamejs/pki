...
// Every runtime operation this engine decides with, captured at load. Each is an ordinary writable
// property of a global, so reading one at the moment it runs hands the decision to whoever last
// wrote it: a replaced concat assembles ciphertext out of bytes of its own choosing, a replaced
// array test lets a usages list past the permission check, and a replaced own-membership test
// reads a private JWK as public and mints a public key that still announces it can sign. See
...
var _arrJoin = intrinsic.join;
var _arrConcat = intrinsic.concat;
var _arraySlice = intrinsic.arraySlice;
var _push = intrinsic.uncurry(Array.prototype.push);
// The conversions, which are globals and so ordinary writable properties of globalThis. `String`
// renders the algorithm name every dispatch table below is keyed on, and `Number` is the narrowing
// an RSA public exponent passes through on its way to the key generator.
...
// The string and byte operations an algorithm name is matched with, and a key, a ciphertext and a
// shared secret are measured and sliced with.
...
var _abSlice = intrinsic.uncurry(ArrayBuffer.prototype.slice);
// The settle half of the promise chains that export and adopt a foreign CryptoKey.
...
function _toArrayBuffer(buf) {
  return _abSlice(buf.buffer, buf.byteOffset, buf.byteOffset + buf.byteLength);
}
...
// allow:guard-reads-runtime-live -- module-init, the parameter-set table is folded once at load
...
["RSASSA-PKCS1-V1_5", "RSA-PSS", "ECDSA", "ED25519", "ED448", "HMAC"]
  // allow:guard-reads-runtime-live -- module-init, the recognized-name set is built once at load
...
function CryptoKey(type, extractable, algorithm, usages, handle) {
  ...
  // The copy is taken with the captured array slice, so the list a permission check later reads is
  // not one a replaced `slice` chose. That slice is generic, and on a value that is not an array it
  // answers with an empty list rather than refusing -- an empty usages list reads as a key permitted
  // for nothing, which is a verdict about a key nobody described. A supplied usages that is not an
  // array is refused instead.
  if (usages && !_isArray(usages)) throw new TypeError("CryptoKey: usages must be an array of key usages");
...
function _requireUsage(key, usage) {
  ...
  if (_arrIndexOf(key.usages, usage) === -1) {
    throw new WebCryptoError("webcrypto/invalid-access", "key is not permitted for '" + usage + "' (usages: " + _arrJoin(key.usages, ",") + ")");
  }
}
...
SubtleCrypto.prototype.generateKey = async function generateKey(algorithm, extractable, keyUsages) {
  ...
  // The asymmetric arm splits this list with the captured array filter, which is generic: on a value
  // that is not an array it yields an empty list, so a key pair would be minted permitted for nothing
  // rather than the request being refused. The same rule the CryptoKey constructor holds, applied at
  // the door the split happens behind.
  if (!_isArray(usages)) throw new TypeError("generateKey: keyUsages must be an array of key usages");
  var name = alg.name;
  ...
  if (name === "AES-GCM") {
    ...
    return _toArrayBuffer(_runCipher(function () {
      return _withSecretBytes(key, function (kb) {
        ...
        var ct = _bufferConcat([cipher.update(buf), cipher.final()]);
  ...
  if (name === "AES-CBC") {
    ...
    return _toArrayBuffer(_runCipher(function () {
      return _withSecretBytes(key, function (kb) {
        ...
        return _bufferConcat([c2.update(buf), c2.final()]);
      });
    }, "encrypt"));
  }
  if (name === "AES-CTR") {
    ...
    return _toArrayBuffer(_runCipher(function () {
      return _withSecretBytes(key, function (kb) {
        ...
        return _bufferConcat([c3.update(buf), c3.final()]);
      });
    }, "encrypt"));
  }
  ...
  if (name === "AES-CBC") {
    ...
    return _toArrayBuffer(_runCipher(function () {
      return _withSecretBytes(key, function (kb) {
        ...
        return _bufferConcat([d2.update(buf), d2.final()]);
      });
    }, "decrypt"));
  }
  if (name === "AES-CTR") {
    ...
    return _toArrayBuffer(_runCipher(function () {
      return _withSecretBytes(key, function (kb) {
        ...
        return _bufferConcat([d3.update(buf), d3.final()]);
      });
    }, "decrypt"));
  }
...
// allow:guard-reads-runtime-live -- module-init, the cap is read from the environment once at load
...
async function _deriveBitsRaw(alg, key, length) {
  ...
  if (name === "HKDF") {
    ...
    return _withSecretBytes(key, function (ikm) {
      ...
      return derived instanceof ArrayBuffer ? derived : _toArrayBuffer(_bufferFrom(derived));
    });
  }
  if (name === "PBKDF2") {
  ...
  if (alg.name === "AES-KW") {
    ...
    try {
      return _withSecretBytes(wrappingKey, function (wkBytes) {
        ...
        return _toArrayBuffer(_bufferConcat([c.update(bytes), c.final()]));
      });
  ...
  if (alg.name === "AES-KW") {
    ...
    try {
      bytes = _withSecretBytes(unwrappingKey, function (kwBytes) {
        ...
        return _bufferConcat([d.update(wrapped), d.final()]);
      });
...
function _cloneWithUsage(key, usage) {
  var k = new CryptoKey(key.type, key.extractable, key.algorithm, _arrConcat(key.usages, [usage]), key._handle);
  return k;
}
...
// allow:guard-reads-runtime-live -- module-init, the validator table is built once at load
...
[ML_DSA_NODE, ML_KEM_NODE, SLH_DSA_NODE].forEach(function (m) {
  // allow:guard-reads-runtime-live -- module-init, the key-type table is built once at load
...
Crypto.prototype.getRandomValues = function getRandomValues(typedArray) {
  if (!_isView(typedArray) || typedArray instanceof Float32Array || typedArray instanceof Float64Array || typedArray instanceof DataView) {
...
// allow:guard-reads-runtime-live -- module-init, the format table is built once at load
...
function exportAnyKey(key, E, code) {
  ...
  if (key instanceof CryptoKey) {
    return _promiseResolve(_Promise)
...
function adoptKey(key, importParams, usages, E, code) {
  if (key instanceof CryptoKey) return _promiseResolve(_Promise, key);
  ...
  for (var i = 0; i < usages.length; i++) {
    if (_arrIndexOf(key.usages, usages[i]) === -1) {
      throw E(code, "the CryptoKey is not permitted for '" + usages[i] + "' (usages: " + _arrJoin(key.usages, ",") + ")");
    }
  }
  return _promiseResolve(_Promise)
