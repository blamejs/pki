...
// The node Cipheriv instance methods, captured at load for the same reason as the hash methods above:
// encryptStream feeds plaintext chunks through the cipher across `await` points, and a co-resident that
// replaced Cipheriv.prototype.update while the stream was suspended between chunks could drop or alter a
// chunk, so the emitted ciphertext would not be the encryption of the plaintext the caller streamed.
// Calling the captured update/final keeps the ciphertext bound to the bytes the stream actually yielded.
...
// decryptStream drives these across the caller's iteration steps, so it captures them for the same
// reason: a co-resident that replaced Decipheriv.prototype.update between two chunk reads could steer
// the recovered plaintext. The captured pair keeps the yielded chunks bound to the ciphertext.
...
SubtleCrypto.prototype.digestStream = async function digestStream(algorithms, source) {
  ...
  // Follow the async-iteration protocol exactly as `for await` does: read the source's
  // Symbol.asyncIterator ONCE, call it to obtain the iterator, then capture THAT iterator's `next` with
  // a single read and drive it manually. Driving `source` by its own `next` would diverge from
  // `for await` whenever Symbol.asyncIterator returns a distinct iterator, and an inherited `next` (say
  // from a polluted prototype) would misclassify an ordinary iterable; going through the method avoids
  // both. Native `for await` caches `next` after one read, so capturing each method once and reusing it
  // -- rather than re-reading to drive the loop -- also consumes a one-shot or stateful accessor a
  // direct `for await` accepts.
  // Build (and thereby validate) the hashes BEFORE acquiring the iterator: an unsupported algorithm
  // must reject before the iterator factory runs, so a factory that opens a file or socket is never
  // invoked -- and never leaked outside the cleanup guard -- on a rejection, keeping the lazy-stream
  // guarantee that a pre-hash rejection acquires no resource.
  // A plain index loop, not `_map`: Array.prototype.map SKIPS holes, so a sparse list such as
  // `[, "SHA-256"]` would leave a hole in `hashes` (later read as `undefined` and updated as a raw
  // TypeError) or, if fully sparse, yield sparse output. Indexing every slot instead makes a hole read
  // as `undefined`, which `_normalizeAlg` rejects with a typed webcrypto/syntax before any chunk is read.
  var hashes = [];
  ...
  try {
    // The `next` retrieval and its validation are INSIDE the guard: the factory above may have opened a
    // resource, so a non-callable next (or a throwing next accessor) must still reach the cleanup below
    // that calls the iterator's return(), rather than leak on a malformed iterator protocol.
    ...
    // Best-effort completion, as `for await` performs on an early exit or throw: let the source release.
    // The retrieval of `return` is INSIDE the guard too, so a throwing `return` accessor cannot replace
    // the original error -- the malformed-chunk verdict is what the caller must see, not a cleanup fault.
    try {
      var ret = iterator == null ? undefined : iterator.return;
      if (typeof ret === "function") await _apply(ret, iterator, _NO_ARGS);
    } catch (_r) { /* cleanup is best-effort */ }
    throw e;
  }
  return _map(hashes, function (h) { return _toArrayBuffer(_hashDigest(h)); });
};

// encryptStream(spec, source) -> { ciphertext, tag }. Runs the plaintext chunks of an async iterable
// through the content cipher incrementally, assembling the ciphertext -- its definite length is needed
// for the EnvelopedData encryptedContent OCTET STRING, and the plaintext is never held whole (the memory
// win of streaming encrypt). Mirrors digestStream's captured-method + acquire-iterator-once discipline so
// a chunk-boundary prototype swap cannot steer the ciphertext, and drives the iterator's return() on any
// exit. spec: { algNode, key, iv, aad (optional), authTagLength (optional, AEAD -> a tag is returned) }.
...
// decryptStream(spec, ct) -> an async iterable of plaintext chunks. Unlike encryptStream the whole
// ciphertext is already in memory (a parsed message states its definite length), so this streams the
// OUTPUT: the plaintext is yielded a chunk at a time and never held whole (the memory win of streaming
// decrypt). The decipher is created EAGERLY here -- node copies the key into the cipher at creation, so
// a caller that clears spec.key after this returns (as the CMS decrypt path does once the content is
// open) does not disturb the running decipher. The chunked drive uses captured update/final + subarray,
// so a prototype swap between the caller's iteration steps cannot steer the recovered bytes. A CBC
// final() rejects a wrong key / bad padding exactly as the buffered path does; the caller translates
// that failure into its own error domain. spec: { algNode, key, iv }.
SubtleCrypto.prototype.decryptStream = function decryptStream(spec, ct) {
  ...
  // Snapshot the ciphertext into a private copy NOW: the drive slices it lazily as the caller iterates,
  // so a view aliasing the caller's buffer would let a reuse of that buffer after this returns change the
  // recovered plaintext and its padding verdict. The copy is read; the caller's buffer is theirs to reuse.
