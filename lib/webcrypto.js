...
SubtleCrypto.prototype.digestStream = async function digestStream(algorithms, source) {
  ...
  // Acquire the iterator and capture its `next` with a SINGLE read each, then drive it manually. A
  // source already an async iterator (its own `next`) is driven directly; otherwise its
  // Symbol.asyncIterator method is read once and the iterator acquired from it, then that iterator's
  // `next` is read once. Native `for await` caches `next` after one read, so reading `next` (or a
  // one-shot Symbol.asyncIterator accessor) a second time to drive the loop would refuse a source a
  // direct `for await` accepts; capturing each method once matches for-await and avoids that.
  var iterator, nextMethod = source == null ? undefined : source.next;
  if (typeof nextMethod === "function") {
    ...
    iterator = _apply(acquire, source, _NO_ARGS);
    nextMethod = iterator == null ? undefined : iterator.next;
    if (typeof nextMethod !== "function") {
      throw new WebCryptoError("webcrypto/syntax", "digestStream: source must be an async iterable of byte chunks");
    }
  }
