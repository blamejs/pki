  ...
  if (name === "PBKDF2") {
    ...
    return _withSecretBytes(key, function (pw) {
      return _pbkdf2Async(pw, _toBuf(alg.salt, "PBKDF2 salt"), alg.iterations, length / 8, _hashNode(alg.hash, "PBKDF2"))
        .then(function (out) { return _toArrayBuffer(out); });
    });
  }
  if (name === "X963KDF") {
    ...
    return _withSecretBytes(key, function (z) {
      return _toArrayBuffer(_x963Kdf(_hashNode(alg.hash, "X963KDF"), z, _toBuf(alg.info || Buffer.alloc(0), "X963KDF SharedInfo"), length / 8));
    });
  }
