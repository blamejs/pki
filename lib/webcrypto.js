...
SubtleCrypto.prototype.digestStream = async function digestStream(algorithms, source) {
  ...
  // Acquire the iterator with AT MOST ONE read of the source's Symbol.asyncIterator. A source that is
  // already an async iterator (it exposes `next`, e.g. an iterator a door acquired once and threaded
  // through) is driven directly, reading no protocol property. Otherwise the method is read a single
  // time and the iterator acquired from it; reading it again to drive the loop (as a bare `for await`
  // on `source` would) fires a one-shot or stateful accessor twice and can refuse a stream a direct
  // `for await` accepts.
  var iterator;
  if (source != null && typeof source.next === "function") {
    iterator = source;
  } else {
    ...
    iterator = _apply(acquire, source, []);
  }
  ...
  // Hand `for await` the acquired iterator so the source's Symbol.asyncIterator is never read again,
  // while for-await still performs the protocol's completion (calling the iterator's `return` on an
  // early exit or a throw).
  var reader = {};
  reader[_asyncIterator] = function () { return iterator; };
