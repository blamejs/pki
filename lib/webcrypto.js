...
SubtleCrypto.prototype.sign = async function sign(algorithm, key, data) {
  ...
  if (ML_DSA_NODE[name]) {
    ...
    var mlKey = (alg.context != null) ? { key: key._handle, context: _toBuf(alg.context, "sign context") } : key._handle;
    return _toArrayBuffer(nodeCrypto.sign(null, buf, mlKey));
  }
...
SubtleCrypto.prototype.verify = async function verify(algorithm, key, signature, data) {
  ...
  if (ML_DSA_NODE[name]) {
    ...
    var mlKey = (alg.context != null) ? { key: key._handle, context: _toBuf(alg.context, "verify context") } : key._handle;
    return nodeCrypto.verify(null, buf, mlKey, sig);
  }
  if (name === "ED25519" || name === "ED448" || SLH_DSA_NODE[name]) {
