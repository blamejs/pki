...
function _toBuf(data, who) {
  ...
  if (isAb || ArrayBuffer.isView(data)) {
    try {
      // A detached backing ArrayBuffer (a transferred / structuredClone'd
      // ArrayBuffer or view) surfaces as a typed webcrypto/data error here, not
      // a raw TypeError from Buffer.from deeper in a sign / verify / digest path.
