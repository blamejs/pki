...
// Parse one domain URI entry (RFC 7616 sec. 3.3) into { origin, path }: an abs_path ("/a") is relative to the
// responding server -> { origin: null (any same-origin request), path }; an ABSOLUTE URI carries its own
// authority -> { origin: "scheme://authority" (lowercased), path } and matches ONLY a request to that SAME
// origin, since an entry to a different host names a protection space on a different server; a relative /
// unparseable entry -> null (never matches).
function _domainEntry(d) {
  d = String(d == null ? "" : d);
  if (d.charAt(0) === "/") return { origin: null, path: d };
  ...
  try { var u = new URL(d); return { origin: u.origin.toLowerCase(), path: u.pathname || "/" }; }
  catch (_e) { return null; }
}

// inProtectionSpace(challenge, requestOrigin, requestPath) -> is the request within the challenge's protection
// space? An absent / empty domain means "all URIs on the responding server" (RFC 7616 sec. 3.3), so any
// same-origin request matches; otherwise the request must match a listed domain URI -- for an absolute entry
// the SAME origin AND a path-segment prefix, for an abs_path entry the prefix alone (sec. 3.5). Used to scope
// where a cached Digest answer may be preemptively reused, so it is not sent to a resource outside the space.
function inProtectionSpace(challenge, requestOrigin, requestPath) {
  ...
  for (var i = 0; i < domain.length; i++) {
    ...
    if (path === e.path || path.indexOf(pref) === 0) return true;
  }
  return false;
}
