...
function _validateDigest(paramText, E, code) {
  ...
  // domain (RFC 7616 sec. 3.3): a quoted, space-separated list of URIs defining the protection space. Surfaced
  // as an array of the listed URIs, or null when omitted / empty -- which per the RFC means "all URIs on the
  // responding server". A caller uses it to scope where the answer may be preemptively reused (sec. 3.5).
  var domain = (p.domain && p.domain.quoted && p.domain.value.replace(/^\s+|\s+$/g, "") !== "")
...
// The abs_path of a domain URI entry (RFC 7616 sec. 3.3): an abs_path ("/a") as-is, an absolute URI's path
// component, or null for a relative / unparseable entry (which cannot match a same-origin request path).
function _domainPath(d) {
  d = String(d == null ? "" : d);
  if (d.charAt(0) === "/") return d;
  var m = /^[a-zA-Z][a-zA-Z0-9+.-]*:\/\/[^/]*(\/[^\s]*)?$/.exec(d);
  return m ? (m[1] || "/") : null;
}

// inProtectionSpace(challenge, requestPath) -> is the request path within the challenge's protection space?
// An absent / empty domain means "all URIs on the responding server" (RFC 7616 sec. 3.3), so any same-origin
// path matches; otherwise the path must have a listed domain URI as a path-segment prefix (sec. 3.5). Used to
// scope where a cached Digest answer may be preemptively reused, so it is not sent to a resource outside the
// space (a different realm) that never requested it.
function inProtectionSpace(challenge, requestPath) {
  var domain = challenge && challenge.domain;
  if (!domain || !domain.length) return true;
  var path = String(requestPath == null ? "" : requestPath);
  for (var i = 0; i < domain.length; i++) {
    var dp = _domainPath(domain[i]);
    if (dp === null) continue;
    var pref = dp.charAt(dp.length - 1) === "/" ? dp : dp + "/";
    if (path === dp || path.indexOf(pref) === 0) return true;
  }
  return false;
}
