...
// replaceable properties to answer one thing, and here it decides whether an auth-param repeats --
// answering false lets the second copy of a nonce overwrite the first.
...
var pkix = require("./schema-pkix");   // the shared JS-\s code-point predicate (pkix.isJsWhitespace)
...
//
// Null prototype, because the gate on an unknown algorithm is `if (!ALGS[name])`. With an ordinary
// object literal that question is asked of the prototype chain too, so an algorithm named for any
// member of Object.prototype answers with a truthy value the table never carried: the
// unsupported-algorithm refusal is skipped and the descriptor's own fields read as undefined.
...
// Quote a value: wrap in double quotes, backslash-escaping any " or \ (matching /(["\\])/g -> "\\$1").
function _qstr(v) {
  var s = String(v), out = "";
...
// Unquote a well-formed quoted-string (only ever called after _closedQuote): drop the surrounding
// quotes and each escaping backslash (matching s.slice(1,-1).replace(/\\(.)/g, "$1")).
function _unq(s) {
  var out = "", last = 1, n = s.length;                       // skip the opening quote at 0
  for (var i = 1; i < n - 1; i++) { if (_charCodeAt(s, i) === 92) { out += _strSlice(s, last, i); last = i + 1; i += 1; } }
  return out + _strSlice(s, last, n - 1);                     // trailing content, excluding the closing quote
}
...
// The leading token when `pre` is "token WS more" (/^(\S+)\s+(\S[\s\S]*)$/) -- a scheme name before the
// first auth-param -- else null. \s is the shared JS-\s predicate (RFC 7230 OWS is a subset).
function _matchTokenRest(pre) {
  var n = pre.length, p = 0;
  while (p < n && !pkix.isJsWhitespace(_charCodeAt(pre, p))) p += 1;              // ^(\S+)
  if (p === 0) return null;
  var tokEnd = p;
  if (p >= n || !pkix.isJsWhitespace(_charCodeAt(pre, p))) return null;          // \s+
  while (p < n && pkix.isJsWhitespace(_charCodeAt(pre, p))) p += 1;
  if (p >= n || pkix.isJsWhitespace(_charCodeAt(pre, p))) return null;           // (\S[\s\S]*)$ -- at least one non-\s
  return _strSlice(pre, 0, tokEnd);
}
// The index past a leading "OWS token OWS" (/^\s*\S+\s+/), or 0 when the string does not begin that way.
// The caller slices from this index (the slice that replaces the removed .replace).
function _stripTokenIndex(seg) {
  var n = seg.length, p = 0;
  while (p < n && pkix.isJsWhitespace(_charCodeAt(seg, p))) p += 1;              // ^\s*
  var ts = p;
  while (p < n && !pkix.isJsWhitespace(_charCodeAt(seg, p))) p += 1;             // \S+
  if (p === ts) return 0;
  var ws = p;
  while (p < n && pkix.isJsWhitespace(_charCodeAt(seg, p))) p += 1;              // \s+ (required)
  return p === ws ? 0 : p;
}
// String.prototype.split(/\s+/): the pieces between \s runs (a leading/trailing run yields a "" element).
...
function _splitChallenges(s) {
  ...
  for (var i = 0; i < segs.length; i++) {
    ...
    else if (scheme !== null) { cur = { scheme: scheme, paramText: _strSlice(seg, _stripTokenIndex(seg)) }; out.push(cur); }   // "scheme firstKey=..."
