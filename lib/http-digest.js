...
function _qstr(v) { return "\"" + _String(v).replace(/(["\\])/g, "\\$1") + "\""; }
...
function _commaSplitOutsideQuotes(s) {
  ...
  for (var i = 0; i < s.length; i++) {
    ...
    if (c === "," && !inQ) { _push(out, buf); buf = ""; continue; }
    buf += c;
  }
  _push(out, buf);
  return out;
...
function _splitChallenges(s) {
  ...
  for (var i = 0; i < segs.length; i++) {
    ...
    if (eq < 0) { cur = { scheme: seg, paramText: "" }; _push(out, cur); }        // a bare scheme token (e.g. "Basic")
    else if (ws) { cur = { scheme: ws[1], paramText: seg.replace(/^\s*\S+\s+/, "") }; _push(out, cur); }   // "scheme firstKey=..."
...
function _parseParams(paramText, E, code) {
  var segs = _commaSplitOutsideQuotes(paramText), map = _create(null);
  for (var i = 0; i < segs.length; i++) {
    var seg = segs[i].replace(/^\s+|\s+$/g, "");
    if (seg === "") continue;
    var eq = _firstEqOutsideQuotes(seg);
    if (eq < 0) throw E(code, "malformed Digest auth-param (no '='): " + _stringify(seg));
    ...
    if (Object.prototype.hasOwnProperty.call(map, key)) throw E(code, "repeated Digest auth-param " + _stringify(key));
...
function _validateDigest(paramText, E, code) {
  ...
  if (p.charset && (p.charset.quoted || _String(p.charset.value).toUpperCase() !== "UTF-8")) throw E(code, "the Digest charset must be the unquoted token UTF-8 (RFC 7616 sec. 3.3)");
  ...
  if (p.stale) {
    var sv = _String(p.stale.value).toLowerCase();
  ...
  if (p.userhash) {
    var uhv = _String(p.userhash.value).toLowerCase();
  ...
  return {
    ...
    stale: !!(p.stale && _String(p.stale.value).toLowerCase() === "true"),
    userhash: !!(p.userhash && _String(p.userhash.value).toLowerCase() === "true"),
...
function _octets(s, charset) {
  var v = s == null ? "" : _String(s);
  return (charset && _String(charset).toUpperCase() === "UTF-8") ? _bufferFrom(v, "utf8").toString("latin1") : v;
}
...
function _pctEncodeUtf8(s) {
  var bytes = _bufferFrom(_String(s == null ? "" : s), "utf8");
  var out = "";
...
function answer(challenge, params, E) {
  ...
  var cnonce = _String((params.rng || function () { return crypto.randomBytes(18).toString("base64"); })());
  ...
  var isUtf8 = _String(challenge.charset || "").toUpperCase() === "UTF-8";
  var pUser = params.username == null ? "" : _String(params.username);
  var pPass = params.password == null ? "" : _String(params.password);
  ...
  var ncNum = (typeof params.nc === "number" && params.nc >= 1) ? _floor(params.nc) : 1;
  ...
  _push(parts, "realm=" + _qstr(realm), "nonce=" + _qstr(nonce), "uri=" + _qstr(params.uri), "algorithm=" + challenge.algorithm);
  if (useQop) { _push(parts, "qop=" + useQop); _push(parts, "nc=" + nc); }
  ...
  if (useQop || alg.sess) { _push(parts, "cnonce=" + _qstr(cnonce)); }
  _push(parts, "response=" + _qstr(response));
  if (challenge.opaque != null) _push(parts, "opaque=" + _qstr(challenge.opaque));
  if (challenge.userhash) _push(parts, "userhash=true");
...
function inProtectionSpace(challenge, requestOrigin, requestPathAndSearch) {
  ...
  var origin = _String(requestOrigin == null ? "" : requestOrigin).toLowerCase();
  var full = _String(requestPathAndSearch == null ? "" : requestPathAndSearch);
