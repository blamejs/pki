...
function parseChallenge(www, E, code, policy) {
  ...
  for (var i = 0; i < challenges.length; i++) {
    ...
    var stale = preferStale && !!parsed.stale;   // a retryability tie-breaker, only in the rejection context
    // Usability dominates; then (in a rejection context) a retryable stale offer; then the stronger algorithm.
    if (best === null ||
        (usable && !bestUsable) ||
        (usable === bestUsable && stale && !bestStale) ||
        (usable === bestUsable && stale === bestStale && rank > bestRank)) {
      best = parsed; bestUsable = usable; bestStale = stale; bestRank = rank;
    }
  }
  if (best) return best;
