...
function _unq(s) {
  if (s.length < 2 || s.charAt(s.length - 1) !== "\"") return s.slice(1);   // best-effort; the required-quoted validation is what rejects a malformed value
...
// parseChallenge(www, E, code) -> the most-secure structurally-valid Digest challenge object,
//   OR null (no Digest challenge present),  OR throw E(code) (a Digest challenge present but none valid).
function parseChallenge(www, E, code) {
  ...
  var best = null, bestRank = -1, sawDigest = false;
  for (var i = 0; i < challenges.length; i++) {
    ...
    if (rank > bestRank) { best = parsed; bestRank = rank; }
  }
  if (best) return best;
...
function answer(challenge, params, E) {
  ...
  if (!alg) throw E(codes.unsupportedAlgorithm, "unsupported Digest algorithm " + challenge.algorithm + " (supported: SHA-512-256, SHA-256, MD5)");
  if (alg.hash === "md5" && !pol.allowMD5) throw E(codes.weakAlgorithm, "MD5 Digest refused by default (RFC 7616 sec. 3.4 discourages it); set opts.auth.allowMD5 for legacy interop");
  var useQop = challenge.qop.indexOf("auth") !== -1 ? "auth" : (challenge.qop.indexOf("auth-int") !== -1 ? "auth-int" : null);
  if (challenge.qop.length === 0) {
    if (!pol.allowLegacyQop) throw E(codes.noQop, "a no-qop (RFC 2069) Digest challenge is refused by default; set opts.auth.allowLegacyQop for legacy interop");
  } else if (useQop === null) {
    throw E(codes.badChallenge, "the Digest qop offered no member this client supports (auth / auth-int)");
  }
  ...
  var nc = "00000001";   // a (nonce, nc) pair is used exactly once -- a fresh nonce restarts at 1 (RFC 7616 sec. 3.4)
  var response = useQop
  ...
  if (useQop) { parts.push("qop=" + useQop); parts.push("nc=" + nc); parts.push("cnonce=" + _qstr(cnonce)); }
