...
function answer(challenge, params, E) {
  ...
  // The realm goes through the SAME charset-aware octet path as user / pass (RFC 7616 sec. 4): under
  // charset=UTF-8 a non-ASCII realm must contribute its UTF-8 octets to A1 / userhash (and the echoed field),
  // not its Latin-1 code points, or the digest disagrees with the server and authentication always fails.
  var realm = _octets(challenge.realm, challenge.charset), nonce = challenge.nonce;
...
// Parse one domain URI entry (RFC 7616 sec. 3.3) into { origin, path, search }: an abs_path ("/a", "/a?x=1")
// is relative to the responding server -> origin null (any same-origin request); an ABSOLUTE URI carries its
// own authority -> origin "scheme://authority" (lowercased, default port normalized) and matches ONLY a request
// to that SAME origin, since an entry to a different host names a protection space on a different server. The
// query is kept separate so it can NARROW the space. A relative / unparseable absolute entry -> null (never matches).
function _domainEntry(d) {
  d = String(d == null ? "" : d);
  if (d.charAt(0) === "/") {
    var q = d.indexOf("?");
    return q === -1 ? { origin: null, path: d, search: "" } : { origin: null, path: d.slice(0, q), search: d.slice(q) };
  }
  ...
  try { var u = new URL(d); return { origin: u.origin.toLowerCase(), path: u.pathname || "/", search: u.search || "" }; }
...
// any same-origin request matches; otherwise the request must have a listed domain URI as a prefix (sec. 3.5) --
// for an absolute entry the SAME origin too. The comparison keeps the QUERY: a domain entry carrying a query
// (/enroll?tenant=a) covers only requests whose query EXTENDS it, so it does not widen to /enroll?tenant=b; a
// query-less entry covers its whole path (any query). Used to scope where a cached answer may be reused.
function inProtectionSpace(challenge, requestOrigin, requestPathAndSearch) {
  ...
  var qi = full.indexOf("?");
  var rPath = qi === -1 ? full : full.slice(0, qi);
  var rSearch = qi === -1 ? "" : full.slice(qi);
  for (var i = 0; i < domain.length; i++) {
    ...
    var pref = e.path.charAt(e.path.length - 1) === "/" ? e.path : e.path + "/";
    if (!(rPath === e.path || rPath.indexOf(pref) === 0)) continue;
    if (e.search && rSearch.indexOf(e.search) !== 0) continue;   // a query on the entry NARROWS the space
    return true;
  }
  return false;
}
