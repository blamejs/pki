...
function _validateDigest(paramText, E, code) {
  ...
  // stale (RFC 7616 sec. 3.3) is an unquoted true/false token. A QUOTED stale is malformed and rejected (thus
  // skipped in selection) rather than trusted as a retryability flag that could shadow a conforming offer.
  if (p.stale && p.stale.quoted) throw E(code, "the Digest stale directive must be an unquoted token (RFC 7616 sec. 3.3)");
