...
var COMPOSITE_PREFIX = _bufferFrom("CompositeAlgorithmSignatures2025", "ascii");
var MLDSA_COMPONENT = {
...
function _comp(name, mldsa, ph, label, trad) {
  ...
  COMPOSITE_ALGS[oid.byName(name)] = {
    ...
    ph: ph, phCms: PH_CMS[ph], label: _bufferFrom(label, "ascii"), trad: trad,
  };
}
...
function compositeMprime(d, phBuf, ctx) {
  ctx = ctx || _bufferAlloc(0);
  return _bufferConcat([COMPOSITE_PREFIX, d.label, _bufferFrom([ctx.length]), ctx, _bufferFrom(phBuf)]);
}
...
function compositeSign(d, keys, message, ctx) {
  return subtle.digest({ name: d.ph }, message).then(function (phBuf) {
    ...
    return Promise.all([mldsaP, tradP]).then(function (s) { return _bufferConcat([_bufferFrom(s[0]), _bufferFrom(s[1])]); });
  });
}
function _signTradComponent(trad, pkcs8, mprime) {
  if (trad.ec) {
    return subtle.importKey("pkcs8", pkcs8, { name: "ECDSA", namedCurve: trad.ec }, false, ["sign"])
      ...
      .then(function (raw) { var r = _bufferFrom(raw); return validator.sig.rawToEcdsaDer(r, r.length / 2); });
  }
  if (trad.eddsa) {
    return subtle.importKey("pkcs8", pkcs8, { name: trad.eddsa }, false, ["sign"])
      .then(function (k) { return subtle.sign({ name: trad.eddsa }, k, mprime); }).then(function (s) { return _bufferFrom(s); });
  }
  ...
  return subtle.importKey("pkcs8", pkcs8, imp, false, ["sign"]).then(function (k) { return subtle.sign(alg, k, mprime); }).then(function (s) { return _bufferFrom(s); });
}
