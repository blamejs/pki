...
var _verdictOf = require("./guard-verdict").of;
...
/** @internal Every verdict this returns carries its own `then`: resolving a promise reads that property
 * off the value, so an inherited accessor would run with the verdict as its receiver and could hand the
 * caller a different object than the one the comparison produced (see guard-verdict). */
function compositeVerify(spkiBytes, sigBytes, message, d, E, unsupported, badSig) {
  if (d.trad.unsupported) {
    return Promise.resolve(_verdictOf({ ok: false, code: unsupported,
  ...
  try {
    ...
    rawKey = bs.bytes;
  } catch (e) { return Promise.resolve(_verdictOf({ ok: false, code: _codeOf(e, badSig), error: e })); }
  if (rawKey.length <= d.mldsaPk || sigBytes.length <= d.mldsaSig) {
    return Promise.resolve(_verdictOf({ ok: false, code: badSig,
  ...
  return subtle.digest({ name: d.ph }, message).then(function (phBuf) {
    ...
    return Promise.all([mldsaP, tradP]).then(function (r) { return _verdictOf({ ok: r[0] === true && r[1] === true }); });
  }).catch(function (e) { return _verdictOf({ ok: false, code: _codeOf(e, badSig), error: e }); });
}
