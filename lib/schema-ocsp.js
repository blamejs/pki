...
function _readArchiveCutoff(ext) {
  var node;
...
function _validateOcspExtensions(list) {
  ...
  for (var i = 0; i < list.length; i++) {
    var ext = list[i];
    if (ext.oid === OID_OCSP_ARCHIVE_CUTOFF) { ext.archiveCutoff = _readArchiveCutoff(ext); continue; }
    if (ext.oid === OID_OCSP_CRL) { ext.crlReferences = _readCrlId(ext); continue; }
