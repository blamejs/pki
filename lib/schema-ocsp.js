...
// A leaf surfacing the whole node's raw DER (a certs [0] SEQUENCE OF Certificate
// element, kept raw rather than recursively parsed).
function rawBytes() { return schema.decode(function (n) { return n.bytes; }); }

// requestorName [1] EXPLICIT GeneralName — GeneralName is a CHOICE of context tags
// [0]..[8] (RFC 5280 §4.2.1.6). We keep the value RAW (no pkix.generalName factory
// yet), but still validate its outer tag is a real GeneralName alternative so a
// non-GeneralName inner (e.g. [1] EXPLICIT INTEGER) fails closed rather than passing
// the signed-request requestorName guard.
...
var CERTS = schema.seqOf(rawBytes(), { assert: "sequence", code: "ocsp/bad-certs", what: "certs" });
...
var RESPONSE_DATA = schema.seq([
  ...
  schema.field("responses", schema.seqOf(SINGLE_RESPONSE, { assert: "sequence", code: "ocsp/bad-responses", what: "responses" })),
  schema.trailing([
...
var BASIC_OCSP_RESPONSE = schema.seq([
  ...
  build: function (m) {
    ...
    return {
      ...
      signature: m.fields.signature.value.bytes,
