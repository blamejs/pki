// SPDX-License-Identifier: Apache-2.0
// Copyright (c) blamejs contributors
"use strict";
/**
 * @module     pki.schema.ocsp
 * @nav        Schema
 * @title      OCSP
 * @order      150
 * @slug       ocsp
 *
 * @intro
 *   OCSP request and response handling per RFC 6960 (sec. 4.1 OCSPRequest, sec. 4.2
 *   OCSPResponse). Two entry points, `parseRequest` and `parseResponse`, turn a
 *   DER or PEM message into a structured object. A response is a two-stage
 *   OID-dispatch: `OCSPResponse` carries an ENUMERATED `responseStatus` and an
 *   OPTIONAL `responseBytes`; when the `responseType` is `id-pkix-ocsp-basic` its
 *   `response` OCTET STRING content is a fresh DER `BasicOCSPResponse` that is
 *   decoded and walked, so the per-certificate statuses surface structurally.
 *
 *   OCSP is a signed protocol: the bytes an external verifier must hash are surfaced
 *   raw and never re-serialized. `tbsRequestBytes` and `tbsResponseDataBytes` are the
 *   exact on-wire `tbsRequest` / `ResponseData` TLVs; each `CertID` surfaces its
 *   `issuerNameHash` / `issuerKeyHash` as raw octets, and the responder's `byKey`
 *   hash and the raw `signature` bytes are left for the caller to verify. Embedded
 *   certificates are surfaced as raw DER, so an unknown alternative never fails the
 *   parse. The nonce extension's value syntax is validated (OCTET STRING of 1..128
 *   octets, RFC 9654 sec. 2.1) with the decoded bytes surfaced as `nonce` alongside the
 *   raw value; other extension values stay opaque. DER-only, fail-closed.
 *
 * @card
 *   Parse DER / PEM OCSP requests and responses (RFC 6960) into structured,
 *   validated fields: per-certificate status, responder identity, raw tbs bytes for
 *   external verification, certificates kept raw, fail-closed.
 */

var asn1 = require("./asn1-der");
var schema = require("./schema-engine");
var pkix = require("./schema-pkix");
var oid = require("./oid");
var constants = require("./constants");
var frameworkError = require("./framework-error");
var intrinsic = require("./guard-intrinsic");
var _hasOwn = intrinsic.hasOwn;
var _String = intrinsic.String;

var OcspError = frameworkError.OcspError;
var PemError = frameworkError.PemError;

var NS = pkix.makeNS("ocsp", OcspError, oid);

var ALGORITHM_IDENTIFIER = pkix.algorithmIdentifier(NS);
var NAME = pkix.name(NS);
var EXTENSIONS = pkix.extensions(NS);

var VERSION = pkix.versionReader(NS, {});

var OID_OCSP_BASIC = oid.byName("ocspBasic");

var OID_OCSP_NONCE = oid.byName("ocspNonce");
var OID_OCSP_ARCHIVE_CUTOFF = oid.byName("ocspArchiveCutoff");
var OID_OCSP_CRL = oid.byName("ocspCrl");

var STATUS_NAMES = constants.NAMES.OCSP_STATUS;

var CRL_REASONS = pkix.CRL_REASON_NAMES;


var GENERALIZED_TIME = pkix.generalizedTime(NS, { code: "ocsp/bad-time", message: "OCSP time must be a GeneralizedTime (RFC 6960)" });

var OCSP_RESPONSE_STATUS = schema.decode(function (n, ctx) {
  var v = asn1.read.enumerated(n);
  var key = v.toString();
  if (!_hasOwn(STATUS_NAMES, key)) throw ctx.E("ocsp/bad-response-status", "undefined OCSPResponseStatus " + key + " (RFC 6960 sec. 4.2.1)");
  return { code: Number(v), name: STATUS_NAMES[key] };
});

var CRL_REASON = schema.decode(function (n, ctx) {
  var v = asn1.read.enumerated(n);
  var key = v.toString();
  if (!_hasOwn(CRL_REASONS, key)) throw ctx.E("ocsp/bad-revocation-reason", "undefined CRLReason " + key + " (RFC 5280 sec. 5.3.1)");
  return CRL_REASONS[key];
});

function certificateBytes() {
  return pkix.rawNonEmptySequence(NS, { code: "ocsp/bad-certs", message: "each certs element must be a non-empty Certificate SEQUENCE (RFC 6960 sec. 4.1.1/sec. 4.2.1)" });
}

/** @internal The requestorName is read with its value decoded, so a responder compares a
 *  directoryName to the signer certificate's subject (RFC 6960 sec. 4.1.2) off the parse. */
var GENERAL_NAME_RAW = pkix.generalName(NS, { code: "ocsp/bad-requestor-name", decodeValue: true });

function _rawSignature(field) {
  var sig = field.value;
  if (sig.unusedBits !== 0) throw NS.E("ocsp/bad-signature", "an OCSP signature BIT STRING must be octet-aligned (0 unused bits)");
  return sig.bytes;
}

var CERTS = schema.seqOf(certificateBytes(), { assert: "sequence", code: "ocsp/bad-certs", what: "certs", max: constants.LIMITS.OCSP_MAX_CERTS, maxCode: "ocsp/too-many-certs" });

/** @internal RFC 6960 sec. 4.4.4: ArchiveCutoff ::= GeneralizedTime, carried as a singleExtension. */
function _readArchiveCutoff(ext, ctx) {
  var node;
  try { node = pkix.decodeNested(ext.value, ctx, "an archiveCutoff extension value"); }
  catch (e) { throw NS.E("ocsp/bad-archive-cutoff", "the archiveCutoff extension value did not decode: " + ((e && e.message) || _String(e)), e); }
  if (!(node.tagClass === "universal" && node.tagNumber === asn1.TAGS.GENERALIZED_TIME)) {
    throw NS.E("ocsp/bad-archive-cutoff", "the archiveCutoff extension value must be a GeneralizedTime (RFC 6960 sec. 4.4.4)");
  }
  try { return asn1.read.time(node); }
  catch (e) { throw NS.E("ocsp/bad-archive-cutoff", "the archiveCutoff GeneralizedTime is malformed", e); }
}

/** @internal RFC 6960 sec. 4.4.2: CrlID ::= SEQUENCE { crlUrl [0] EXPLICIT IA5String OPTIONAL,
 *  crlNum [1] EXPLICIT INTEGER OPTIONAL, crlTime [2] EXPLICIT GeneralizedTime OPTIONAL }. Every
 *  field is optional and each may appear at most once, in tag order. */
function _readCrlId(ext, ctx) {
  var C = "ocsp/bad-crl-id";
  var node;
  try { node = pkix.decodeNested(ext.value, ctx, "a crlID extension value"); }
  catch (e) { throw NS.E(C, "the CRL references extension value did not decode: " + ((e && e.message) || _String(e)), e); }
  if (!(node.tagClass === "universal" && node.tagNumber === asn1.TAGS.SEQUENCE && node.children)) {
    throw NS.E(C, "CrlID must be a SEQUENCE (RFC 6960 sec. 4.4.2)");
  }
  var out = { crlUrl: null, crlNum: null, crlTime: null }, last = -1;
  for (var i = 0; i < node.children.length; i++) {
    var k = node.children[i];
    if (k.tagClass !== "context" || !k.constructed || !k.children || k.children.length !== 1) {
      throw NS.E(C, "each CrlID member must be one EXPLICIT [0], [1] or [2] tag (RFC 6960 sec. 4.4.2)");
    }
    if (k.tagNumber <= last) throw NS.E(C, "CrlID members must appear at most once, in tag order");
    last = k.tagNumber;
    var inner = k.children[0];
    try {
      if (k.tagNumber === 0) {
        if (inner.tagClass !== "universal" || inner.tagNumber !== asn1.TAGS.IA5_STRING) throw NS.E(C, "crlUrl must be an IA5String");
        out.crlUrl = asn1.read.string(inner);
      } else if (k.tagNumber === 1) {
        if (inner.tagClass !== "universal" || inner.tagNumber !== asn1.TAGS.INTEGER) throw NS.E(C, "crlNum must be an INTEGER");
        out.crlNum = asn1.read.integer(inner);
        if (out.crlNum < 0n) throw NS.E(C, "crlNum must be non-negative (it is a cRLNumber, RFC 5280 sec. 5.2.3)");
      } else if (k.tagNumber === 2) {
        if (inner.tagClass !== "universal" || inner.tagNumber !== asn1.TAGS.GENERALIZED_TIME) throw NS.E(C, "crlTime must be a GeneralizedTime");
        out.crlTime = asn1.read.time(inner);
      } else {
        throw NS.E(C, "CrlID has no member tagged [" + k.tagNumber + "] (RFC 6960 sec. 4.4.2)");
      }
    } catch (e) {
      if (e && e.code === C) throw e;
      throw NS.E(C, "CrlID member [" + k.tagNumber + "] is malformed", e);
    }
  }
  return out;
}

/** @internal The extensions this profile reads are decoded onto their entry beside the raw value,
 *  so a consumer gets the value by name and a malformed one is refused here rather than passed
 *  through as opaque bytes. The nonce is RFC 9654; the other two are RFC 6960 sec. 4.4. */
function _validateOcspExtensions(list, ctx) {
  if (!list) return list;
  for (var i = 0; i < list.length; i++) {
    var ext = list[i];
    if (ext.oid === OID_OCSP_ARCHIVE_CUTOFF) { ext.archiveCutoff = _readArchiveCutoff(ext, ctx); continue; }
    if (ext.oid === OID_OCSP_CRL) { ext.crlReferences = _readCrlId(ext, ctx); continue; }
    if (ext.oid !== OID_OCSP_NONCE) continue;
    var node;
    try { node = pkix.decodeNested(ext.value, ctx, "a nonce extension value"); }
    catch (e) { throw NS.E("ocsp/bad-nonce", "the nonce extension value did not decode: " + ((e && e.message) || String(e)), e); }
    if (!(node.tagClass === "universal" && node.tagNumber === asn1.TAGS.OCTET_STRING && node.content)) {
      throw NS.E("ocsp/bad-nonce", "the nonce extension value must be an OCTET STRING (RFC 9654 sec. 2.1)");
    }
    if (node.content.length < 1 || node.content.length > 128) {
      throw NS.E("ocsp/bad-nonce", "the nonce must be 1..128 octets (RFC 9654 sec. 2.1)");
    }
    ext.nonce = node.content;
  }
  return list;
}


var CERT_ID = schema.seq([
  schema.field("hashAlgorithm", ALGORITHM_IDENTIFIER),
  schema.field("issuerNameHash", schema.octetString()),
  schema.field("issuerKeyHash", schema.octetString()),
  schema.field("serialNumber", schema.integerLeaf()),
], {
  assert: "sequence", arity: { exact: 4 }, code: "ocsp/bad-cert-id", what: "CertID",
  /** @internal RFC 6960 sec. 4.1.1: issuerNameHash and issuerKeyHash are the hashes of the issuer's
   *  name and key "using the hash algorithm specified in hashAlgorithm", so each is that digest's
   *  length; a hash of any other length names no certificate. A digest this build does not know is
   *  read as given, since its length is not known here. */
  build: function (m) {
    var hashAlgorithm = m.fields.hashAlgorithm.value.result;
    var want = constants.NAMES.DIGEST_OCTETS[hashAlgorithm.name];
    if (want !== undefined) {
      if (m.fields.issuerNameHash.value.length !== want) throw NS.E("ocsp/bad-cert-id", "CertID issuerNameHash is " + m.fields.issuerNameHash.value.length + " octets, not the " + want + " a " + hashAlgorithm.name + " digest is (RFC 6960 sec. 4.1.1)");
      if (m.fields.issuerKeyHash.value.length !== want) throw NS.E("ocsp/bad-cert-id", "CertID issuerKeyHash is " + m.fields.issuerKeyHash.value.length + " octets, not the " + want + " a " + hashAlgorithm.name + " digest is (RFC 6960 sec. 4.1.1)");
    }
    return {
      hashAlgorithm: hashAlgorithm,
      issuerNameHash: m.fields.issuerNameHash.value,
      issuerKeyHash: m.fields.issuerKeyHash.value,
      serialNumber: m.fields.serialNumber.value,
      serialNumberHex: m.fields.serialNumber.node.content.toString("hex"),
    };
  },
});


var REVOKED_INFO = schema.seq([
  schema.field("revocationTime", GENERALIZED_TIME),
  schema.trailing([
    { tag: 0, name: "revocationReason", schema: CRL_REASON, explicit: true, emptyCode: "ocsp/bad-revoked-info" },
  ], { minTag: 0, maxTag: 0, unexpectedCode: "ocsp/bad-revoked-info", orderCode: "ocsp/bad-revoked-info" }),
], {
  assert: "constructed", code: "ocsp/bad-revoked-info", what: "RevokedInfo",
  build: function (m) {
    return {
      revocationTime: m.fields.revocationTime.value,
      revocationReason: m.fields.revocationReason.present ? m.fields.revocationReason.value : null,
    };
  },
});

var CERT_STATUS = schema.choice([
  { when: { tagClass: "context", tagNumber: 0 }, schema: schema.implicitNull(0) },
  { when: { tagClass: "context", tagNumber: 1 }, schema: REVOKED_INFO },
  { when: { tagClass: "context", tagNumber: 2 }, schema: schema.implicitNull(2) },
], { code: "ocsp/bad-cert-status", what: "CertStatus" });

function _shapeCertStatus(field) {
  var t = field.node.tagNumber;
  if (t === 0) return { type: "good" };
  if (t === 2) return { type: "unknown" };
  var ri = field.value.result;
  return { type: "revoked", revocationTime: ri.revocationTime, revocationReason: ri.revocationReason };
}

var SINGLE_RESPONSE = schema.seq([
  schema.field("certID", CERT_ID),
  schema.field("certStatus", CERT_STATUS),
  schema.field("thisUpdate", GENERALIZED_TIME),
  schema.trailing([
    { tag: 0, name: "nextUpdate", schema: GENERALIZED_TIME, explicit: true, emptyCode: "ocsp/bad-single-response" },
    { tag: 1, name: "singleExtensions", schema: EXTENSIONS, explicit: true, emptyCode: "ocsp/bad-single-response" },
  ], { minTag: 0, maxTag: 1, unexpectedCode: "ocsp/bad-single-response", orderCode: "ocsp/bad-single-response" }),
], {
  assert: "sequence", code: "ocsp/bad-single-response", what: "SingleResponse",
  build: function (m, ctx) {
    return {
      certID: m.fields.certID.value.result,
      certStatus: _shapeCertStatus(m.fields.certStatus),
      thisUpdate: m.fields.thisUpdate.value,
      nextUpdate: m.fields.nextUpdate.present ? m.fields.nextUpdate.value : null,
      singleExtensions: m.fields.singleExtensions.present ? _validateOcspExtensions(m.fields.singleExtensions.value.result, ctx) : null,
    };
  },
});

var RESPONDER_ID = schema.choice([
  { when: { tagClass: "context", tagNumber: 1 }, schema: schema.explicit(1, NAME, { code: "ocsp/bad-responder-id" }) },
  { when: { tagClass: "context", tagNumber: 2 }, schema: schema.explicit(2, schema.octetString(), { code: "ocsp/bad-responder-id" }) },
], { code: "ocsp/bad-responder-id", what: "ResponderID" });

function _shapeResponderID(field) {
  if (field.node.tagNumber === 1) return { byName: field.value.result };
  var keyHash = field.value;
  if (keyHash.length !== 20) throw NS.E("ocsp/bad-responder-id", "ResponderID byKey (KeyHash) must be a 20-byte SHA-1 hash (RFC 6960 sec. 4.2.1)");
  return { byKey: keyHash };
}

var RESPONSE_DATA = schema.seq([
  schema.optional("version", VERSION, { tag: 0, explicit: true, default: 1, emptyCode: "ocsp/bad-version" }),
  schema.field("responderID", RESPONDER_ID),
  schema.field("producedAt", GENERALIZED_TIME),
  schema.field("responses", schema.seqOf(SINGLE_RESPONSE, { assert: "sequence", min: 1, code: "ocsp/bad-responses", what: "responses" })),
  schema.trailing([
    { tag: 1, name: "responseExtensions", schema: EXTENSIONS, explicit: true, emptyCode: "ocsp/bad-response-data" },
  ], { minTag: 1, maxTag: 1, unexpectedCode: "ocsp/bad-response-data", orderCode: "ocsp/bad-response-data" }),
], {
  assert: "sequence", code: "ocsp/bad-response-data", what: "ResponseData",
  build: function (m, ctx) {
    return {
      version: m.fields.version.value,
      responderID: _shapeResponderID(m.fields.responderID),
      producedAt: m.fields.producedAt.value,
      responses: m.fields.responses.value.items.map(function (it) { return it.value.result; }),
      responseExtensions: m.fields.responseExtensions.present ? _validateOcspExtensions(m.fields.responseExtensions.value.result, ctx) : null,
    };
  },
});

var BASIC_OCSP_RESPONSE = schema.seq([
  schema.field("tbsResponseData", RESPONSE_DATA),
  schema.field("signatureAlgorithm", ALGORITHM_IDENTIFIER),
  schema.field("signature", schema.bitString()),
  schema.trailing([
    { tag: 0, name: "certs", schema: CERTS, explicit: true, emptyCode: "ocsp/bad-basic-response" },
  ], { minTag: 0, maxTag: 0, unexpectedCode: "ocsp/bad-basic-response", orderCode: "ocsp/bad-basic-response" }),
], {
  assert: "sequence", code: "ocsp/bad-basic-response", what: "BasicOCSPResponse",
  build: function (m) {
    var tbs = m.fields.tbsResponseData.value.result;
    return {
      tbsResponseDataBytes: m.fields.tbsResponseData.node.bytes,
      version: tbs.version,
      responderID: tbs.responderID,
      producedAt: tbs.producedAt,
      responses: tbs.responses,
      responseExtensions: tbs.responseExtensions,
      signatureAlgorithm: m.fields.signatureAlgorithm.value.result,
      signature: _rawSignature(m.fields.signature),
      certs: m.fields.certs.present ? m.fields.certs.value.items.map(function (it) { return it.value; }) : [],
      certsPresent: m.fields.certs.present,
    };
  },
});

var RESPONSE_BYTES = schema.seq([
  schema.field("responseType", schema.oidLeaf()),
  schema.field("response", schema.octetString()),
], {
  assert: "sequence", arity: { exact: 2 }, code: "ocsp/bad-response-bytes", what: "ResponseBytes",
  build: function (m, ctx) {
    var responseType = m.fields.responseType.value;
    var raw = m.fields.response.value;
    if (responseType !== OID_OCSP_BASIC) {
      throw NS.E("ocsp/unsupported-response-type", (ctx.oid.name(responseType) || responseType) + " is not id-pkix-ocsp-basic (RFC 6960 sec. 4.2.1)");
    }
    return {
      responseType: responseType,
      responseTypeName: ctx.oid.name(responseType) || null,
      response: raw,
      basicResponse: schema.embeddedDer(BASIC_OCSP_RESPONSE, raw, ctx,
        { code: "ocsp/bad-der", what: "BasicOCSPResponse" }).result,
    };
  },
});

var OCSP_RESPONSE = schema.seq([
  schema.field("responseStatus", OCSP_RESPONSE_STATUS),
  schema.optional("responseBytes", RESPONSE_BYTES, { tag: 0, explicit: true, emptyCode: "ocsp/bad-ocsp-response" }),
], {
  assert: "sequence", arity: { min: 1 }, code: "ocsp/bad-ocsp-response", what: "OCSPResponse",
  build: function (m) {
    var status = m.fields.responseStatus.value;
    var present = m.fields.responseBytes.present;
    if (status.code === 0 && !present) throw NS.E("ocsp/bad-response-bytes", "a successful OCSPResponse must carry responseBytes (RFC 6960 sec. 4.2.1)");
    if (status.code !== 0 && present) throw NS.E("ocsp/bad-response-bytes", "a non-successful OCSPResponse must not carry responseBytes (RFC 6960 sec. 4.2.1)");
    var rb = present ? m.fields.responseBytes.value.result : null;
    return {
      responseStatus: status,
      responseBytes: rb ? { responseType: rb.responseType, responseTypeName: rb.responseTypeName, response: rb.response } : null,
      basicResponse: rb ? rb.basicResponse : null,
    };
  },
});


var REQUEST = schema.seq([
  schema.field("reqCert", CERT_ID),
  schema.trailing([
    { tag: 0, name: "singleRequestExtensions", schema: EXTENSIONS, explicit: true, emptyCode: "ocsp/bad-request" },
  ], { minTag: 0, maxTag: 0, unexpectedCode: "ocsp/bad-request", orderCode: "ocsp/bad-request" }),
], {
  assert: "sequence", code: "ocsp/bad-request", what: "Request",
  build: function (m, ctx) {
    return {
      certID: m.fields.reqCert.value.result,
      singleRequestExtensions: m.fields.singleRequestExtensions.present ? _validateOcspExtensions(m.fields.singleRequestExtensions.value.result, ctx) : null,
    };
  },
});

var SIGNATURE = schema.seq([
  schema.field("signatureAlgorithm", ALGORITHM_IDENTIFIER),
  schema.field("signature", schema.bitString()),
  schema.trailing([
    { tag: 0, name: "certs", schema: CERTS, explicit: true, emptyCode: "ocsp/bad-signature" },
  ], { minTag: 0, maxTag: 0, unexpectedCode: "ocsp/bad-signature", orderCode: "ocsp/bad-signature" }),
], {
  assert: "sequence", code: "ocsp/bad-signature", what: "Signature",
  build: function (m) {
    return {
      signatureAlgorithm: m.fields.signatureAlgorithm.value.result,
      signature: _rawSignature(m.fields.signature),
      certs: m.fields.certs.present ? m.fields.certs.value.items.map(function (it) { return it.value; }) : [],
    };
  },
});

var TBS_REQUEST = schema.seq([
  schema.optional("version", VERSION, { tag: 0, explicit: true, default: 1, emptyCode: "ocsp/bad-version" }),
  schema.optional("requestorName", GENERAL_NAME_RAW, { tag: 1, explicit: true, emptyCode: "ocsp/bad-requestor-name" }),
  schema.field("requestList", schema.seqOf(REQUEST, { assert: "sequence", min: 1, code: "ocsp/bad-request-list", what: "requestList" })),
  schema.trailing([
    { tag: 2, name: "requestExtensions", schema: EXTENSIONS, explicit: true, emptyCode: "ocsp/bad-tbs-request" },
  ], { minTag: 2, maxTag: 2, unexpectedCode: "ocsp/bad-tbs-request", orderCode: "ocsp/bad-tbs-request" }),
], {
  assert: "sequence", code: "ocsp/bad-tbs-request", what: "TBSRequest",
  build: function (m, ctx) {
    var rn = m.fields.requestorName;
    return {
      version: m.fields.version.value,
      requestorName: rn.present ? { bytes: rn.value.bytes, tagClass: rn.value.tagClass, tagNumber: rn.value.tagNumber, value: rn.value.value } : null,
      requestList: m.fields.requestList.value.items.map(function (it) { return it.value.result; }),
      requestExtensions: m.fields.requestExtensions.present ? _validateOcspExtensions(m.fields.requestExtensions.value.result, ctx) : null,
    };
  },
});

var OCSP_REQUEST = schema.seq([
  schema.field("tbsRequest", TBS_REQUEST),
  schema.optional("optionalSignature", SIGNATURE, { tag: 0, explicit: true, emptyCode: "ocsp/not-a-request" }),
], {
  assert: "sequence", arity: { min: 1 }, code: "ocsp/not-a-request", what: "OCSPRequest",
  build: function (m) {
    var tbs = m.fields.tbsRequest.value.result;
    var optionalSignature = m.fields.optionalSignature.present ? m.fields.optionalSignature.value.result : null;
    if (optionalSignature && tbs.requestorName === null) {
      throw NS.E("ocsp/missing-requestor-name", "a signed OCSPRequest must specify requestorName (RFC 6960 sec. 4.1.2)");
    }
    return {
      tbsRequestBytes: m.fields.tbsRequest.node.bytes,
      version: tbs.version,
      requestorName: tbs.requestorName,
      requestList: tbs.requestList,
      requestExtensions: tbs.requestExtensions,
      optionalSignature: optionalSignature,
    };
  },
});

/**
 * @primitive  pki.schema.ocsp.parseRequest
 * @signature  pki.schema.ocsp.parseRequest(input, caps?) -> ocspRequest
 * @since      0.1.11
 * @status     stable
 * @spec       RFC 6960
 * @related    pki.schema.ocsp.parseResponse, pki.schema.parse
 *
 * Parse a DER `Buffer` or a PEM (`OCSP REQUEST`) string into a structured
 * OCSPRequest: `{ tbsRequestBytes, version, requestorName, requestList,
 * requestExtensions, optionalSignature }`. `tbsRequestBytes` is the exact on-wire
 * `tbsRequest` TLV for external signature verification; each `requestList` entry
 * carries its `CertID` (with the two issuer hashes raw). A malformed structure
 * throws a typed `OcspError` (`ocsp/*`) and a leaf-level codec fault surfaces as
 * `asn1/*`.
 *
 * @example
 *   var ca = await pki.key.generate("Ed25519");
 *   var caKey = await pki.key.export(ca.privateKey);
 *   var caDer = await pki.x509.sign({ subject: "Example CA", subjectPublicKey: await pki.key.export(ca.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z"),
 *     extensions: { basicConstraints: { cA: true }, keyUsage: ["keyCertSign"], subjectKeyIdentifier: true } }, { key: caKey });
 *   var leaf = await pki.key.generate("Ed25519");
 *   var leafDer = await pki.x509.sign({ subject: "leaf.example", subjectPublicKey: await pki.key.export(leaf.publicKey),
 *     serialNumber: 0x1332n, notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z") },
 *     { cert: caDer, key: caKey });
 *   var der = await pki.ocsp.buildRequest({ cert: leafDer, issuer: caDer }, {});
 *   var req = pki.schema.ocsp.parseRequest(der);
 *   req.requestList[0].certID.serialNumberHex;   // -> "1332"
 * @opts
 *   - `maxBytes` / `maxDepth` / `maxItems` (number) -- decode caps for this parse. Each
 *     defaults to the matching `pki.C.LIMITS` figure and may only be set lower; a value
 *     above it, or an option outside this set, is refused. A parse that exceeds one is
 *     refused with the `/too-large` code of this format.
 */
var parseRequest = pkix.makeParser({ pemLabel: "OCSP REQUEST", PemError: PemError, ErrorClass: OcspError, prefix: "ocsp", what: "OCSP request", topSchema: OCSP_REQUEST, ns: NS });

/**
 * @primitive  pki.schema.ocsp.parseResponse
 * @signature  pki.schema.ocsp.parseResponse(input, caps?) -> ocspResponse
 * @since      0.1.11
 * @status     stable
 * @spec       RFC 6960
 * @related    pki.schema.ocsp.parseRequest, pki.schema.parse
 *
 * Parse a DER `Buffer` or a PEM (`OCSP RESPONSE`) string into a structured
 * OCSPResponse: `{ responseStatus, responseBytes, basicResponse }`. For a successful
 * basic response, `basicResponse` carries `{ tbsResponseDataBytes, responderID,
 * producedAt, responses, signatureAlgorithm, signature, certs, certsPresent }`; each
 * `responses[i].certStatus` is `{ type: "good" | "revoked" | "unknown" }`. `certs` is the
 * embedded certificate list, empty when the field is absent, and `certsPresent` says
 * whether the response carried the field at all. A
 * non-successful status carries no `responseBytes`. A malformed structure throws a
 * typed `OcspError` (`ocsp/*`); an unsupported `responseType` throws
 * `ocsp/unsupported-response-type`.
 *
 * @example
 *   var ca = await pki.key.generate("Ed25519");
 *   var caKey = await pki.key.export(ca.privateKey);
 *   var caDer = await pki.x509.sign({ subject: "Example CA", subjectPublicKey: await pki.key.export(ca.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z"),
 *     extensions: { basicConstraints: { cA: true }, keyUsage: ["keyCertSign"], subjectKeyIdentifier: true } }, { key: caKey });
 *   var leaf = await pki.key.generate("Ed25519");
 *   var leafDer = await pki.x509.sign({ subject: "leaf.example", subjectPublicKey: await pki.key.export(leaf.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z") },
 *     { cert: caDer, key: caKey });
 *   var der = await pki.ocsp.sign({ responderID: "byName", responses: [{ cert: leafDer, issuer: caDer, status: "good" }] },
 *     { cert: caDer, key: caKey });
 *   var res = pki.schema.ocsp.parseResponse(der);
 *   res.responseStatus.name;                       // -> "successful"
 *   res.basicResponse.responses[0].certStatus.type; // -> "good" | "revoked" | "unknown"
 * @opts
 *   - `maxBytes` / `maxDepth` / `maxItems` (number) -- decode caps for this parse. Each
 *     defaults to the matching `pki.C.LIMITS` figure and may only be set lower; a value
 *     above it, or an option outside this set, is refused. A parse that exceeds one is
 *     refused with the `/too-large` code of this format.
 */
var parseResponse = pkix.makeRecordingParser({ pemLabel: "OCSP RESPONSE", PemError: PemError, ErrorClass: OcspError, prefix: "ocsp", what: "OCSP response", topSchema: OCSP_RESPONSE, ns: NS }, "ocspResponse");

/**
 * @primitive  pki.schema.ocsp.pemDecode
 * @signature  pki.schema.ocsp.pemDecode(text, label?) -> Buffer
 * @since      0.1.11
 * @status     stable
 * @spec       RFC 7468, RFC 6960
 * @related    pki.schema.ocsp.parseResponse
 *
 * Extract the DER bytes from a PEM OCSP block (default label `OCSP RESPONSE`; pass
 * `"OCSP REQUEST"` for a request). Throws `PemError` on a missing / mismatched
 * envelope or a non-base64 body.
 *
 * @example
 *   var ca = await pki.key.generate("Ed25519");
 *   var caKey = await pki.key.export(ca.privateKey);
 *   var caDer = await pki.x509.sign({ subject: "Example CA", subjectPublicKey: await pki.key.export(ca.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z"),
 *     extensions: { basicConstraints: { cA: true }, keyUsage: ["keyCertSign"], subjectKeyIdentifier: true } }, { key: caKey });
 *   var leaf = await pki.key.generate("Ed25519");
 *   var leafDer = await pki.x509.sign({ subject: "leaf.example", subjectPublicKey: await pki.key.export(leaf.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z") }, { cert: caDer, key: caKey });
 *   var reqDer = await pki.ocsp.buildRequest({ cert: leafDer, issuer: caDer }, {});
 *   var pemText = pki.schema.ocsp.pemEncode(reqDer, "OCSP REQUEST");
 *   var der = pki.schema.ocsp.pemDecode(pemText, "OCSP REQUEST");
 */
function pemDecode(text, label) { return pkix.pemDecode(text, label || "OCSP RESPONSE", PemError); }

/**
 * @primitive  pki.schema.ocsp.pemEncode
 * @signature  pki.schema.ocsp.pemEncode(der, label?) -> string
 * @since      0.1.23
 * @status     stable
 * @spec       RFC 7468, RFC 6960
 * @related    pki.schema.ocsp.pemDecode
 *
 * Wrap DER bytes in a PEM envelope (default label `OCSP RESPONSE`, the same
 * default `pemDecode` reads; pass `"OCSP REQUEST"` for a request). Throws
 * `PemError` on a malformed label.
 *
 * @example
 *   var ca = await pki.key.generate("Ed25519");
 *   var caKey = await pki.key.export(ca.privateKey);
 *   var caDer = await pki.x509.sign({ subject: "Example CA", subjectPublicKey: await pki.key.export(ca.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z"),
 *     extensions: { basicConstraints: { cA: true }, keyUsage: ["keyCertSign"], subjectKeyIdentifier: true } }, { key: caKey });
 *   var leaf = await pki.key.generate("Ed25519");
 *   var leafDer = await pki.x509.sign({ subject: "leaf.example", subjectPublicKey: await pki.key.export(leaf.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z") }, { cert: caDer, key: caKey });
 *   var der = await pki.ocsp.sign({ responderID: "byName", responses: [{ cert: leafDer, issuer: caDer, status: "good" }] },
 *     { cert: caDer, key: caKey });
 *   var pem = pki.schema.ocsp.pemEncode(der);
 */
function pemEncode(der, label) { return pkix.pemEncode(der, label || "OCSP RESPONSE", PemError); }

function matchesResponse(root) {
  var T = asn1.TAGS;
  var k = pkix.rootSequenceChildren(root, 1, 2);
  if (!k) return false;
  if (!schema.isUniversal(k[0], T.ENUMERATED)) return false;
  if (k.length === 2 && !(schema.isContext(k[1], 0) && k[1].children)) return false;
  return true;
}

function matchesRequest(root) {
  var T = asn1.TAGS;
  var k = pkix.rootSequenceChildren(root, 1, 2);
  if (!k) return false;
  if (!(schema.isUniversal(k[0], T.SEQUENCE) && k[0].children)) return false;
  if (k.length === 2 && !(schema.isContext(k[1], 0) && k[1].children)) return false;
  return true;
}

module.exports = {
  parseRequest: parseRequest,
  parseResponse: parseResponse,
  pemDecode: pemDecode,
  pemEncode: pemEncode,
  matchesRequest: matchesRequest,
  matchesResponse: matchesResponse,
  /** @internal Reads a list of pre-encoded Extension DER the way this parser reads a SingleResponse's
   *  singleExtensions, shared with pki.ocsp.sign so an entry the signer emits is one the parser
   *  reads. Throws the parser's own ocsp/* code for a malformed entry. */
  readSingleExtensionsDer: function (derList) {
    var node;
    // allow:bare-decode reached from the signer, not from a parse, so no parse context carries caps here.
    try { node = asn1.decode(asn1.build.sequence(intrinsic.map(derList, function (d) { return asn1.build.raw(d); }))); }
    catch (e) { throw NS.E("ocsp/bad-single-response", "a pre-encoded singleExtension is not DER", e); }
    return _validateOcspExtensions(schema.walk(EXTENSIONS, node, NS).result);
  },
  extensionPlacement: extensionPlacement,
};

/** @internal Where each OCSP extension belongs, keyed by dotted OID through the registry the caller
 *  passes, with the clause that places it: the three request-only extensions (RFC 6960 sec. 4.4.3,
 *  4.4.6, 4.4.7) and the two that a response carries in responseExtensions alone (the extended
 *  revoked definition, sec. 4.4.8, and the nonce, RFC 9654 sec. 2.1). The signer refuses one in
 *  singleExtensions and the linter grades it there from this one table. */
var REQUEST_ONLY_EXTENSION_ROWS = [["ocspResponse", "RFC 6960 4.4.3"], ["ocspServiceLocator", "RFC 6960 4.4.6"], ["ocspPrefSigAlgs", "RFC 6960 4.4.7"]];
var RESPONSE_ONLY_EXTENSION_ROWS = [["ocspExtendedRevoke", "RFC 6960 4.4.8"], ["ocspNonce", "RFC 9654 2.1"]];
function extensionPlacement(oidRegistry) {
  var out = { requestOnly: intrinsic.create(null), responseOnly: intrinsic.create(null) };
  intrinsic.forEach(REQUEST_ONLY_EXTENSION_ROWS, function (r) { out.requestOnly[oidRegistry.byName(r[0])] = r[1]; });
  intrinsic.forEach(RESPONSE_ONLY_EXTENSION_ROWS, function (r) { out.responseOnly[oidRegistry.byName(r[0])] = r[1]; });
  return out;
}
