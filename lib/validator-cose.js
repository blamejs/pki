...
function credentialKey(node, E, code, unsupportedCode) {
  ...
  if (kty === 2n) {
    ...
    if (key.n[0] === 0) throw bad("an RSA COSE_Key modulus (-1) must be minimally encoded, with no leading zero byte");
    if (key.n.length < RSA_MIN_MODULUS_BYTES) {
      throw bad("an RSA COSE_Key modulus (-1) is " + (key.n.length * 8) + " bits, below the " +
        (RSA_MIN_MODULUS_BYTES * 8) + "-bit minimum");
    }
