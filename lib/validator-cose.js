...
function credentialKey(node, E, code, unsupportedCode) {
  ...
  var key = { kty: _Number(kty), alg: _Number(algv) };
  if (kty === 2n) {
    key.crv = ii(-1) != null ? _Number(ii(-1)) : null;
    ...
    key.crv = ii(-1) != null ? _Number(ii(-1)) : null; key.x = ib(-2);
    ...
    throw bad("unsupported COSE_Key kty " + _Number(kty));
  }
...
function assertKeyMaterial(key, E, code, unsupportedCode) {
  ...
  var MAX = _BigInt(Number.MAX_SAFE_INTEGER);
  function _isInt(v) {
    if (typeof v === "bigint") return v <= MAX && v >= -MAX;
    return typeof v === "number" && _isSafeInteger(v);
  }
  ...
  key.kty = _Number(key.kty);
  key.alg = _Number(key.alg);
  if (key.crv !== undefined && key.crv !== null) key.crv = _Number(key.crv);
  var kty = _BigInt(key.kty);
  if (kty === 2n) {
    var el2 = EC2_CRV_LEN[key.crv];
    if (!_isBuffer(key.x) || !_isBuffer(key.y)) throw bad("an EC2 COSE_Key must carry crv (-1), x (-2), and y (-3)");
    ...
    if (!okp2 || !_isBuffer(key.x) || key.x.length !== okp2.len) throw bad("an OKP COSE_Key must be Ed25519 (crv 6) or Ed448 (crv 7) with a matching-length x (-2)");
  } else if (kty === 3n) {
    if (!_isBuffer(key.n) || !key.n.length || !_isBuffer(key.e) || !key.e.length) throw bad("an RSA COSE_Key must carry n (-1) and e (-2)");
    if (key.n[0] === 0) throw bad("an RSA COSE_Key modulus (-1) must be minimally encoded, with no leading zero byte (RFC 8230 sec. 4)");
    if (key.e[0] === 0) throw bad("an RSA COSE_Key exponent (-2) must be minimally encoded, with no leading zero byte (RFC 8230 sec. 4)");
    ...
    throw bad("unsupported COSE_Key kty " + _Number(key.kty));
  }
  // PROFILE: the declared alg must match the key type (and, for EC2, the curve).
  var prof = ALG_PROFILE[_String(key.alg)];
  ...
  if (_Number(key.kty) === 1 && !edwardsPoint.validate(key.x, key.crv)) throw bad("the OKP credential public key is not a valid, full-order Edwards point");
  return key;
}
...
function toSpki(key, E, code) {
  ...
  if (key.kty === 2 && key.x && key.y) {
    ...
    return b.sequence([
      ...
      b.bitString(_bufferConcat([_bufferFrom([0x04]), key.x, key.y])),
    ]);
  }
  if (key.kty === 3 && key.n && key.n.length && key.e && key.e.length) {
    ...
    return bb.sequence([
      ...
      bb.bitString(bb.sequence([bb.integer(_BigInt("0x" + key.n.toString("hex"))), bb.integer(_BigInt("0x" + key.e.toString("hex")))])),
    ]);
  }
