...
function assertKeyMaterial(key, E, code, unsupportedCode) {
  ...
  // kty and alg are read straight off a caller-supplied object here, so their TYPE is checked
  // before anything converts them. BigInt() throws a raw TypeError on a Symbol and on undefined,
  // and this boundary's contract is that a malformed stored key is the module's own typed error.
  // The VALUE as well as the type: BigInt() rejects a fractional or non-finite number with a raw
  // RangeError, so "it is a number" is not the check -- "it is an integer" is.
  function _isInt(v) { return typeof v === "bigint" || (typeof v === "number" && Number.isSafeInteger(v)); }
