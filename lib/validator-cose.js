...
//   - ON-CURVE: the public key point MUST be valid for its curve. The SPKI is imported
//     via node:crypto so OpenSSL validates the EC/Edwards point (an off-curve or
//     identity point, or an invalid Edwards encoding, fails to parse) -- a correct
//     in-tree on-curve gate with no hand-rolled modular arithmetic to get wrong.
...
function credentialKey(node, E, code) {
  ...
  // ON-CURVE: import the SPKI so OpenSSL validates the EC/Edwards point on its curve. An
  // off-curve x/y, the identity point, or an invalid Edwards encoding fails to parse here.
