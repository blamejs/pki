...
var SIG_SCHEME = {
  rsaEncryption: { kind: "rsa" },
  rsassaPss: { kind: "rsapss" },
  ecPublicKey: { kind: "ec" },
  Ed25519: { kind: "eddsa", name: "Ed25519" },
  Ed448: { kind: "eddsa", name: "Ed448" },
  sha1WithRSAEncryption: { kind: "rsa", hash: "SHA-1" },
  sha256WithRSAEncryption: { kind: "rsa", hash: "SHA-256" },
  sha384WithRSAEncryption: { kind: "rsa", hash: "SHA-384" },
  sha512WithRSAEncryption: { kind: "rsa", hash: "SHA-512" },
  ecdsaWithSHA1: { kind: "ec", hash: "SHA-1" },
  ecdsaWithSHA256: { kind: "ec", hash: "SHA-256" },
  ecdsaWithSHA384: { kind: "ec", hash: "SHA-384" },
  ecdsaWithSHA512: { kind: "ec", hash: "SHA-512" },
};
...
// Locate the signer certificate a SignerInfo identifies, among the SignedData's embedded
// certificates plus any the caller supplies. issuerAndSerialNumber matches on the canonical
// DN + serial; subjectKeyIdentifier matches the cert's SKI extension.
function _findSignerCert(sid, parsedCerts) {
  for (var i = 0; i < parsedCerts.length; i++) {
    var c = parsedCerts[i];
    if (sid.subjectKeyIdentifier != null) {
      if (c.ski && c.ski.equals(_toBuf(sid.subjectKeyIdentifier, "sid.subjectKeyIdentifier"))) return c;
    } else if (sid.issuer && sid.serialNumberHex != null) {
      if (c.cert.serialNumberHex === sid.serialNumberHex && guard.name.dnEqual(c.cert.issuer, sid.issuer, CmsError, "cms/bad-name")) return c;
    }
  }
  return null;
}
...
function _certSki(cert) {
  ...
  if (!ext) return null;
  try { return asn1.read.octetString(asn1.decode(ext.value)); } catch (_e) { return null; }
}
...
function _resolvePss(paramsBytes) {
  ...
  var n;
  try { n = asn1.decode(paramsBytes); } catch (_e) { return null; }
  ...
  var mgfOid;
  try { mgfOid = asn1.read.oid(mgfNode.children[0]); } catch (_e4) { return null; }
  if (mgfOid !== OID_MGF1 || PSS_HASH[_hashAlgOid(mgfNode.children[1])] !== hash) return null;
  if (trailer !== 1n) return null;
  // saltLength: an absent field is the SHA-1 DEFAULT 20, which never matches a SHA-2 profile;
  // require the stated length to equal the hash length. Compared as BigInt (asn1.read.integer
  // yields a BigInt) so an oversized value is rejected without a lossy Number narrowing.
  var want = PSS_SALT[hash];
  if (saltLen === null || saltLen !== BigInt(want)) return null;
  return { hash: hash, saltLength: want };
}
...
function _verifySignature(scheme, hashName, sigBytes, spki, signedBytes, curveOid, pssSalt) {
  ...
  // EdDSA -- the WebCrypto name follows the signing key's SPKI OID (Ed25519 / Ed448).
...
function _certCurveOid(cert) {
  ...
  try { return asn1.read.oid(asn1.decode(p)); } catch (_e) { return null; }
}
...
function _verifyOne(si, content, eContentType, parsedCerts) {
  ...
  if ((scheme.kind !== "eddsa" && !sigHash) || (si.signedAttrs && !digestHash)) return Promise.resolve({ ok: false, code: "cms/unsupported-algorithm", sid: si.sid, message: "unsupported digest algorithm " + JSON.stringify(si.digestAlgorithm.name) });
  var signer = _findSignerCert(si.sid, parsedCerts);
  if (!signer) return Promise.resolve({ ok: false, code: "cms/signer-cert-not-found", sid: si.sid, message: "no certificate matches this SignerInfo's signer identifier" });
  var spki = signer.cert.subjectPublicKeyInfo.bytes;
  var sigBytes = _toBuf(si.signature, "the SignerInfo signature");

  return Promise.resolve().then(function () {
    if (!si.signedAttrs) return content;   // no signed attributes: sign over the content directly
    // With signed attributes: the content-type attribute MUST be present and equal the
    // eContentType (RFC 5652 sec. 5.3), the message-digest attribute MUST equal the digest of
    // the content, and the signature is over the DER re-encoding of the SignedAttributes with
    // the implicit [0] tag replaced by a universal SET OF (RFC 5652 sec. 5.4).
    var ctAttr = si.signedAttrs.filter(function (a) { return a.type === OID_CONTENT_TYPE; });
    ...
    var ctOid;
    try { ctOid = asn1.read.oid(asn1.decode(ctAttr[0].values[0])); } catch (e) { throw _err("cms/bad-signed-attrs", "the content-type attribute value is not an OBJECT IDENTIFIER", e); }
    ...
    var mdAttr = si.signedAttrs.filter(function (a) { return a.type === OID_MESSAGE_DIGEST; })[0];
    ...
    var declared;
    try { declared = asn1.read.octetString(asn1.decode(mdAttr.values[0])); } catch (e) { throw _err("cms/bad-signed-attrs", "the message-digest attribute value is not an OCTET STRING", e); }
    ...
    if (signedBytes && signedBytes.mismatch) return { ok: false, code: signedBytes.mismatch.code, sid: si.sid, cert: signer.der, message: signedBytes.mismatch.message };
    return _verifySignature(scheme, sigHash, sigBytes, spki, signedBytes, _certCurveOid(signer.cert), pss ? pss.saltLength : 0)
      .then(function (ok) { return { ok: ok === true, sid: si.sid, cert: signer.der }; },
        function (e) { throw _err("cms/verify-error", "the SignerInfo signature could not be evaluated", e); });
  });
}
...
function _addCert(out, der) {
  var buf;
  try { buf = _toBuf(der, "a certificate"); } catch (_e) { return; }
  var cert;
  try { cert = x509.parse(buf); } catch (_e2) { return; }
