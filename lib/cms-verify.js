...
var WEAK_DIGEST_OIDS = _digestOidTable([["sha1", "SHA-1"]]);
var UNVERIFIABLE_DIGEST_OIDS = _digestOidTable([["md5", "MD5"], ["md4", "MD4"], ["md2", "MD2"]]);
var WEAK_SIGNATURE_HASHES = intrinsic.assign(intrinsic.create(null), { "SHA-1": 1 });
...
async function _verifyCountersignatures(si, parsedCerts, allowWeakDigests) {
  ...
  var out = [];
  for (var _cp = 0; _cp < values.length; _cp++) guard.list.append(out, await _verifyOneCountersig(values[_cp], targetSig, parsedCerts, allowWeakDigests));
  return out;
}

async function _verifyOneCountersig(vDer, targetSig, parsedCerts, allowWeakDigests) {
  var csSi;
  ...
  var verdict = await _verifyOne(csSi, targetSig, null, parsedCerts, targetSig, undefined, allowWeakDigests);
  var nested = await _verifyCountersignatures(csSi, parsedCerts, allowWeakDigests);
...
async function _verify(input, opts) {
  ...
  for (var _sp = 0; _sp < parsed.signerInfos.length; _sp++) {
    ...
    var counters = await _verifyCountersignatures(siOne, parsedCerts, allowWeakDigests);
...
// @internal
module.exports = { verify: verify, sign: sign, countersign: countersign, encrypt: encrypt, authenticate: authenticate, decrypt: decrypt, compress: compress, decompress: decompress, digest: cmsDigest.digest, verifyDigest: cmsDigest.verifyDigest, certsOnly: certsOnly, parseCertsOnly: parseCertsOnly, isCertsOnly: isCertsOnly, setEngine: setEngine };
