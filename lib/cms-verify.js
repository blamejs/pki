...
function _applyTrust(res, parsedCerts, cfg) {
  ...
  anchors.forEach(function (a) {
    _engine.toAnchor(a);
    if (cfg.checkPurpose != null) _engine.assertAnchorConstraints(a, cfg.checkPurpose);
  });
  // The INSTANT is checked here for the same reason the anchors are, and in the same place: a
  // message whose signers all failed never reaches a chain walk, so leaving the time to be
  // validated there would accept an unusable one in silence and report it as `trusted: false`.
  // Whether a caller's configuration is usable cannot depend on whether the message was good.
  if (cfg.time !== undefined) guard.time.assertValid(cfg.time, _err, "cms/bad-input", "opts.time");
  // The key-purpose options, through the SAME resolver the walk uses. Every piece of the trust
  // configuration is now checked in this one place, before any signer is looked at: anchors, the
  // instant, and these. Validating them only where they are consumed made a caller's mistake
  // depend on the message -- a SignedData whose signers all failed never reaches a chain walk, so
  // a malformed purpose would be accepted in silence and reported as `trusted: false`.
  _engine.resolvePurposeOpts({ requiredEku: cfg.requiredEku, checkPurpose: cfg.checkPurpose });
