...
function _verifyComposite(si, comp, content, eContentType, parsedCerts) {
  ...
  // sec. 3.4 digest parameters: id-sha256/id-sha512 carry no parameters; accept absent OR NULL
  // (RFC 5754 sec. 2), reject a present non-NULL fail-closed (it is outside the signed preimage).
  var dp = si.digestAlgorithm.parameters;
  if (dp !== null && dp !== undefined && !_isDerNull(dp)) {
    return Promise.resolve({ ok: false, code: "cms/unsupported-algorithm", sid: si.sid, message: "the " + si.digestAlgorithm.name + " digest algorithm parameters must be absent or DER NULL (RFC 5754 sec. 2)" });
  }
