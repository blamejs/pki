...
var WEAK_DIGEST_OIDS = (function () {
  var t = intrinsic.create(null);
  var named = [["sha1", "SHA-1"], ["md5", "MD5"], ["md4", "MD4"], ["md2", "MD2"]];
...
async function _verify(input, opts) {
  ...
  for (var _sp = 0; _sp < parsed.signerInfos.length; _sp++) {
    ...
    var verdict = await _verifyOne(siOne, content, eContentType, parsedCerts, null, digestByName, opts.allowWeakDigests === true);
    var counters = await _verifyCountersignatures(siOne, parsedCerts, opts.allowWeakDigests === true);
