...
async function _verifyOneCountersig(vDer, targetSig, parsedCerts) {
  ...
  return _row({
    ok: verdict.ok, sid: verdict.sid, cert: verdict.cert,
    digestAlgorithm: csSi.digestAlgorithm.name,
    code: guard.verdict.carries(verdict, "code") ? verdict.code : undefined,
    unsignedAttrs: _surfaceUnsignedAttrs(csSi),
    countersignatures: nested,
  });
}
...
async function _verify(input, opts) {
  ...
  for (var _sp = 0; _sp < parsed.signerInfos.length; _sp++) {
    ...
    intrinsic.push(signers, guard.verdict.of(verdict, {
