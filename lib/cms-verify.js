...
function _snapshotTrust(opts) {
  ...
  return {
    ...
    // The DEFAULT validation instant is resolved HERE, at snapshot time -- before the streamed content
    // is consumed -- not later in _applyTrust: the stream's iterator could otherwise replace the writable
    // global Date to return an instant inside an expired or not-yet-valid signer certificate's lifetime,
    // reporting it trusted. A caller Date is cloned; undefined becomes now, captured now.
    time: guard.time.isDate(opts.time) ? new Date(guard.time.instantOf(opts.time)) : (opts.time === undefined ? new Date() : opts.time),
