...
async function _verifyCountersignatures(si, parsedCerts) {
  ...
  // Await each (started concurrently), not Promise.all: its internal element handling calls the
  // caller-replaceable Promise.prototype.then (see _verify).
  var csPromises = intrinsic.map(values, function (vDer) { return _verifyOneCountersig(vDer, targetSig, parsedCerts); });
  var out = [];
  for (var _cp = 0; _cp < csPromises.length; _cp++) intrinsic.push(out, await csPromises[_cp]);
  return out;
}
...
async function _verify(input, opts) {
  ...
  // The per-signer verifications START concurrently (intrinsic.map invokes each async callback), then
  // are collected by awaiting each. NOT Promise.all: its spec-internal element handling calls the
  // caller-replaceable Promise.prototype.then, so a streamed content that replaced it could fabricate a
  // resolved `{ ok: true }` for every signer. await dispatches through no such method.
  var signerPromises = intrinsic.map(parsed.signerInfos, async function (si) {
    ...
    var verdict = await _verifyOne(si, content, eContentType, parsedCerts, null, digestByName);
    verdict.countersignatures = await _verifyCountersignatures(si, parsedCerts);
  ...
  var signers = [];
  for (var _sp = 0; _sp < signerPromises.length; _sp++) intrinsic.push(signers, await signerPromises[_sp]);
