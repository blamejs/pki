...
// An empty map is still the streaming marker, so those per-signer verdicts are produced. `streamIterator`
// is the async iterator already acquired at the door (null when not streaming, buffered per-signer).
function _resolveContentDigests(parsed, streamIterator) {
  if (streamIterator == null) return Promise.resolve(null);
  var names = [], seen = {};
  ...
  return subtle.digestStream(intrinsic.map(names, function (n) { return DIGEST_HASH[n]; }), streamIterator).then(function (ds) {
    var byName = {};
...
function _verify(input, opts) {
  ...
  if (content == null) {
    ...
    // A streamed (async-iterable) detached content is classified by ACQUIRING its async iterator once
    // (asyncIteratorOf reads Symbol.asyncIterator a single time); the iterator is hashed once,
    // incrementally, below, after the per-signer digest algorithms are known, so a single-use stream
    // serves every signer in one pass. It is not snapshotted: an async iterator has no bytes to copy,
    // and it is consumed exactly once. A byte source (or any non-iterable) yields null and is buffered.
    streamIterator = guard.bytes.asyncIteratorOf(opts.content);
  ...
  return _resolveContentDigests(parsed, streamIterator).then(function (digestByName) {
