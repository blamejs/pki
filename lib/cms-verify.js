...
function _computeDigest(name, content) {
  return subtle.digest(DIGEST_HASH[name], content).then(function (d) { return _bufferFrom(d); });
}
...
function _findSignerCerts(sid, parsedCerts) {
  ...
  for (var i = 0; i < parsedCerts.length; i++) {
    ...
    if (sid.subjectKeyIdentifier != null) {
      if (c.ski && c.ski.equals(_toBuf(sid.subjectKeyIdentifier, "sid.subjectKeyIdentifier"))) _push(out, c);
    } else if (sid.issuer && sid.serialNumberHex != null) {
      if (c.cert.serialNumberHex === sid.serialNumberHex && guard.name.dnEqual(c.cert.issuer.rdns, sid.issuer.rdns, _err, "cms/bad-name", "the signer certificate issuer")) _push(out, c);
    }
  }
  return out;
}
...
function _certCurveOid(cert) {
  ...
  if (cert.subjectPublicKeyInfo.algorithm.oid !== oid.byName("ecPublicKey") || !_isBuffer(p)) return null;
  try {
...
function _verifyOne(si, content, eContentType, parsedCerts, csTarget) {
  ...
  if (!scheme) return Promise.resolve({ ok: false, code: "cms/unsupported-algorithm", sid: si.sid, message: "unsupported signature algorithm " + _stringify(si.signatureAlgorithm.name) });
  ...
  if ((scheme.kind !== "eddsa" && scheme.kind !== "mldsa" && scheme.kind !== "slhdsa" && !sigHash) || (si.signedAttrsBytes && !_supportedDigest(si.digestAlgorithm.name))) return Promise.resolve({ ok: false, code: "cms/unsupported-algorithm", sid: si.sid, message: "unsupported digest algorithm " + _stringify(si.digestAlgorithm.name) });
...
function _computeCountersigBytes(si, csTarget) {
  return Promise.resolve().then(function () {
    ...
    var reTagged = _bufferFrom(si.signedAttrsBytes);
...
function _verifyComposite(si, comp, content, eContentType, parsedCerts, csTarget) {
  ...
  if (si.digestAlgorithm.name !== comp.phCms) {
    return Promise.resolve({ ok: false, code: "cms/unsupported-algorithm", sid: si.sid, message: "the SignerInfo digestAlgorithm " + _stringify(si.digestAlgorithm.name) + " is not the composite " + comp.name + " pre-hash " + _stringify(comp.phCms) + " (draft-ietf-lamps-cms-composite-sigs sec. 3.4)" });
  }
  var signers = _findSignerCerts(si.sid, parsedCerts);
  if (!signers.length) return Promise.resolve({ ok: false, code: "cms/signer-cert-not-found", sid: si.sid, message: "no certificate matches this SignerInfo's signer identifier" });
  var sigBytes = _toBuf(si.signature, "the SignerInfo signature");
...
function _verifyCountersignatures(si, parsedCerts) {
  ...
  (si.unsignedAttrs || []).forEach(function (a) {
    ...
    a.values.forEach(function (v) { _push(values, v && v.bytes ? v.bytes : v); });
  });
...
function _verifyOneCountersig(vDer, targetSig, parsedCerts) {
  ...
  try { csSi = cms.walkCountersignature(asn1.decode(_isBuffer(vDer) ? vDer : _bufferFrom(vDer))); }
...
function _verify(input, opts) {
  ...
  if (!_isArray(parsed.signerInfos)) throw _err("cms/bad-input", "input is not a CMS SignedData");
  ...
  if (content == null) {
    ...
    if (_bufferCompare(_toBuf(opts.content, "opts.content"), _bufferFrom(content)) !== 0) {
...
function _cloneAnchorValue(v, depth) {
  if (_isBuffer(v) || _isView(v) || v instanceof ArrayBuffer) {
  ...
  if (_isArray(v)) return v.map(function (e) { return _cloneAnchorValue(e, depth + 1); });
  var out = {}, k;
  for (k in v) {
...
function _snapshotTrust(opts) {
  ...
  var anchors = null;
  if (raw != null) {
    anchors = (_isArray(raw) ? raw : [raw]).map(function (a) {
  ...
  return {
    ...
    requiredEku: _isArray(opts.requiredEku) ? opts.requiredEku.slice() : opts.requiredEku,
    ...
    return p.then(function () {
      ...
      return _engine.build(s.cert, buildOpts)
        .then(function (r) { s.trusted = !!(r && r.valid); }, function (e) {
          ...
          if (e && e.code && /bad-input|bad-anchor|bad-time/.test(_String(e.code))) throw e;
          s.trusted = false;
        });
    });
...
function _addCert(out, der) {
  ...
  _push(out, { cert: cert, der: buf, ski: _certSki(cert) });
}
