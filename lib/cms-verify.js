...
// The AUTHENTICATED attributes, surfaced per signer so a caller reads a signed attribute -- a content
// type, a signing time, a protocol's own transaction state (SCEP's messageType / nonces) -- BOUND to
// the verified signature rather than re-parsed from the untrusted message, where an attacker could
// present attributes the signature never covered. The signature is over the SignedAttributes these
// decode from, so read them only where the signer's `ok` is true.
...
async function _verify(input, opts) {
  ...
  for (var _sp = 0; _sp < parsed.signerInfos.length; _sp++) {
    ...
    // The authenticated attributes themselves, bound to this signer's verdict (read only when ok).
    verdict.signedAttributes = _surfaceSignedAttrs(siOne);
    // The SignerInfo digestAlgorithm on the primary signer node (the countersignature node already
    // carries it), so a caller reading a signer's verdict learns the digest without re-parsing.
    verdict.digestAlgorithm = siOne.digestAlgorithm.name;
  ...
  var res = {
    ...
    // The attached encapsulated content (null for a detached signature). A signer's message-digest
    // attribute binds these exact bytes, so a `valid` verdict lets a caller consume the authenticated
    // eContent -- SCEP's pkcsPKIEnvelope -- without a second parse of the untrusted message.
...
// A certs-only reader/recognizer input: a DER Buffer (snapshotted at the door so a later mutation
// cannot swap the bytes the verdict describes) or a PEM CMS/PKCS7 block. An undecodable input is a
// typed cms/bad-input, never a raw throw.
...
function parseCertsOnly(input, opts) {
  ...
  // maxCerts bounds the parse of an untrusted bundle, so the bound itself is validated at the door: a
  // negative, fractional, NaN, or non-numeric cap that slice would silently treat as no cap (or, for
  // -1, as "all but the last") is refused rather than forwarded (guard.limits.cap). Absent or null
  // leaves the count uncapped.
...
function isCertsOnly(input) {
  // A certs-only message is a SignedData with an id-data content, absent eContent, and no signer.
  // digestAlgorithms is deliberately NOT constrained: RFC 5652 sec. 5.1 permits a non-empty set even
  // with no signer, so a valid but non-canonical bundle (an EST or AIA response) is still recognized.
  var der = _certsOnlyInputDer(input, "pki.cms.isCertsOnly");
  var r;
  try {
    r = cms.parse(der);
  } catch (e) {
    // A well-formed CMS ContentInfo whose content type this toolkit does not model (a bare id-data,
    // a DigestedData) is definitively not a certs-only SignedData, so the predicate reports false
    // rather than making a caller catch an exception for a valid non-SignedData CMS. Truly
    // undecodable bytes (not a ContentInfo at all) still throw.
