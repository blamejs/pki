...
// unused in that mode.
function _verifyOne(si, content, eContentType, parsedCerts, csTarget, digestByName) {
  ...
  if (scheme.kind === "mldsa" && si.signedAttrsBytes && !_mldsaDigestSuitable(scheme.name, si.digestAlgorithm.name)) return Promise.resolve({ ok: false, code: "cms/unsupported-algorithm", sid: si.sid, message: "the " + si.digestAlgorithm.name + " message digest is below the security strength of " + scheme.name + " (RFC 9882 sec. 3.3)" });
  ...
  return (csTarget != null ? _computeCountersigBytes(si, csTarget) : _computeSignedBytes(si, content, eContentType, digestByName)).then(function (signedBytes) {
...
function _computeSignedBytes(si, content, eContentType, digestByName) {
  return Promise.resolve().then(function () {
    ...
    // Over a buffered content the digest is computed here; over a streamed content it was computed
    // once, incrementally, before any signer was verified, and looked up by algorithm.
    var dP = digestByName ? Promise.resolve(digestByName[si.digestAlgorithm.name]) : _computeDigest(si.digestAlgorithm.name, content);
    return dP.then(function (d) {
...
// downgrade).
function _verifyComposite(si, comp, content, eContentType, parsedCerts, csTarget, digestByName) {
...
// distinct SignerInfo digest algorithm, returning { <digestName>: Buffer }. Every signer MUST carry
// signed attributes -- a signer that signs the content directly binds the whole content as the
// signature preimage, which a stream cannot hold, so it is refused BEFORE the stream is consumed.
// An unsupported digest is NOT refused here: _verifyOne returns the per-signer cms/unsupported-algorithm
// verdict just as the buffered path does, and a signer with an unsupported digest never reads a
// precomputed one, so choosing a streamed content does not change the verdict of a mixed message.
// Returns null (no precomputed digests, digest per-signer over the buffered content) when not streaming.
function _resolveContentDigests(parsed, streaming, streamSource) {
  if (!streaming) return Promise.resolve(null);
  var names = [], seen = {};
  intrinsic.forEach(parsed.signerInfos, function (si) {
    if (!si.signedAttrsBytes) {
      throw _err("cms/streamed-content-unverifiable", "a SignerInfo that signs the content directly (no signed " +
        "attributes) cannot be verified against a streamed content, whose bytes are not retained; pass opts.content as a Buffer");
    }
    var dn = si.digestAlgorithm.name;
    if (_supportedDigest(dn) && !seen[dn]) { seen[dn] = 1; intrinsic.push(names, dn); }
  });
  // Every signer had an unsupported digest: nothing to hash, and each is verdicted per-signer.
  if (!names.length) return Promise.resolve({});
  return subtle.digestStream(intrinsic.map(names, function (n) { return DIGEST_HASH[n]; }), streamSource).then(function (ds) {
    var byName = {};
    intrinsic.forEach(names, function (n, i) { byName[n] = intrinsic.bufferFrom(ds[i]); });
    return byName;
  }, function (e) {
    // A non-byte chunk surfaces from the engine as webcrypto/data; report it in this verb's domain,
    // exactly as a buffered content's bad bytes are reported, rather than leaking the engine code.
    if (e && e.code === "webcrypto/data") throw _err("cms/bad-input", "a streamed content chunk is not a byte source", e);
    throw e;
  });
}
...
function _verify(input, opts) {
  ...
  if (content == null) {
    ...
    // A streamed (async-iterable) detached content stays null here and is hashed once, incrementally,
    // below -- after the per-signer digest algorithms are known, so a single-use stream serves every
    // signer in one pass. It is not snapshotted: an async iterator has no bytes to copy, and it is
    // consumed exactly once.
    if (guard.bytes.isAsyncIterable(opts.content)) {
      streaming = true;
    } else {
  ...
  return _resolveContentDigests(parsed, streaming, opts.content).then(function (digestByName) {
  return Promise.all(parsed.signerInfos.map(function (si) {
    ...
    return _verifyOne(si, content, eContentType, parsedCerts, null, digestByName).then(function (verdict) {
    ...
    var res = {
      // Signature soundness is decided by comparison for the same reason the trust verdict below
      // is: `every` replaced after load reports the whole set sound without reading one signer.
