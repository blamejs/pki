...
function _wrapConstructed(schema, parts) {
  ...
  if (mode === "sequence") return asn1.build.sequence(parts);
  _encFail("cannot encode a seq/repeat with assert " + JSON.stringify(mode) + " and no implicitTag (its tag is external)");
  return undefined;
}
