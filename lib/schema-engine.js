...
var RESOURCE_REFUSALS = Object.assign(Object.create(null), {
...
function isResourceRefusal(e) {
  return !!(e && typeof e.code === "string" && RESOURCE_REFUSALS[e.code] === 1);
}

/** @internal Carries a parse's caps and budget onto another namespace. A format whose structure
 *  is carried inside another format walks it in its OWN namespace, so a refusal names the domain
 *  that owns the fault and keeps its own error class; the caps and the budget are the only things
 *  that cross the boundary. Returns `ns` unchanged when there is nothing to carry. */
function withResources(ns, source) {
  if (!source || (source.caps == null && source.budget == null)) return ns;
  var out = Object.create(null);
  var keys = Object.keys(ns);
  for (var i = 0; i < keys.length; i++) out[keys[i]] = ns[keys[i]];
  out.caps = source.caps;
  out.budget = source.budget;
  return out;
}

/** @internal The decode a parse performs on a value it has already read out of its input: an
 *  embedded OCTET STRING payload, an encapsulated protocol body. `ctx` is the walk context the
 *  door built, so the caps a caller named at the door hold at every level rather than at the
 *  root alone, and a resource refusal down here trips the parse budget before the original
 *  error is re-thrown. The error itself is re-thrown unchanged, because a format's existing
 *  code for a malformed embedded value is one callers pin.
 *
 *  `opts.ber` is read once into a local and carries the relaxed acceptance a format whose door
 *  is BER-tolerant already applies at the root. The decode options are built fresh per call. */
function decodeNested(bytes, ctx, label, opts) {
  var caps = ctx ? ctx.caps : null;
  var ber = !!(opts && opts.ber === true);
  var decodeOpts = Object.create(null);
  if (caps) {
    decodeOpts.maxBytes = caps.maxBytes;
    decodeOpts.maxDepth = caps.maxDepth;
    decodeOpts.maxItems = caps.maxItems;
  }
  if (ber) decodeOpts.ber = true;
  try {
    // allow:bare-decode this IS the primitive every other nested decode routes through.
    return asn1.decode(bytes, decodeOpts);
  } catch (e) {
    if (isResourceRefusal(e) && ctx && ctx.budget) {
      try {
        ctx.budget.trip(label || "an embedded value");
      } catch (_b) {
        /* allow:swallow-unverified the latch is the point; the original error is re-thrown below */
      }
    }
    throw e;
  }
...
// allow:bare-decode a round trip through the codec on an integer this module just encoded.
...
function _walkSeq(schema, node, ctx) {
  ...
  /** @internal Prototype-less, so writing a field creates an own property instead of dispatching
   * through an accessor of the same name on Object.prototype. Every format parser's record is
   * built here, so a getter-only slot planted under any field name a schema declares would
   * otherwise throw out of an ordinary parse, and a setter would swallow the decoded value. The
   * absent-member test below reads it the same way. */
  var fields = intrinsic.create(null);
  var idx = 0;
...
function _encodeRepeat(schema, items, ctx) {
  ...
  if (schema.unique) {
    ...
    parts.forEach(function (tlv) {
      // allow:bare-decode a round trip through the codec on a TLV this encoder just produced.
...
function embeddedDer(schema, bytes, ctx, opts) {
  ...
  var node;
  try {
    node = decodeNested(bytes, ctx, opts.what || "embedded DER", { ber: !!opts.ber });
  } catch (e) {
...
module.exports = {
  ...
  walk: walk, encode: encode, embeddedDer: embeddedDer, decodeNested: decodeNested,
  isResourceRefusal: isResourceRefusal, withResources: withResources,
  assertMinimalNamedBits: assertMinimalNamedBits,
