...
function _assertShape(schema, node, ctx) {
  ...
  if (mode === "sequence") {
    ...
    _fail(ctx, schema.code, "unknown assert mode " + _stringify(mode));
  }
...
function implicitBitString(tag) { return { kind: "leaf", read: function (n) { var b = asn1.read.bitStringImplicit(n, tag); return { unusedBits: b.unusedBits, bytes: b.bytes }; }, write: function (v) { return asn1.build.contextPrimitive(tag, _bufferConcat([_bufferFrom([v.unusedBits]), v.bytes])); } }; }
...
function implicitNull(tag) { return { kind: "leaf", read: function (n) { return asn1.read.nullImplicit(n, tag); }, write: function () { return asn1.build.contextPrimitive(tag, _bufferAlloc(0)); } }; }
...
function implicitBoolean(tag) { return { kind: "leaf", read: function (n) { return asn1.read.booleanImplicit(n, tag); }, write: function (v) { return asn1.build.contextPrimitive(tag, _bufferFrom([v ? 0xff : 0x00])); } }; }
...
function setOfUnique(item, keyFn, opts) {
  return setOf(item, _assign({ unique: keyFn }, opts || {}));
}
...
function walk(schema, node, ctx) {
  switch (schema.kind) {
    ...
    default: _fail(ctx, (ctx.prefix || "schema") + "/bad-schema", "unknown schema kind " + _stringify(schema.kind));
  }
  return undefined;
}
...
function _walkRepeat(schema, node, ctx) {
  ...
  if (schema.derSetOrder) {
    for (var s = 1; s < kids.length; s++) {
      if (_bufferCompare(kids[s - 1].bytes, kids[s].bytes) > 0) {
...
function _equalsDefault(value, def) {
  if (_isBuffer(value) && _isBuffer(def)) return value.equals(def);
  return value === def;
}
...
function _walkSeq(schema, node, ctx) {
  ...
  for (var f = 0; f < schema.fields.length; f++) {
    ...
    if (fld.fkind === "required") {
      if (idx >= kids.length) _fail(ctx, schema.code, "missing required field " + _stringify(fld.name));
      var child = kids[idx++];
      ...
      if (next && fld.match(next)) {
        ...
        if (fld.defaultCode != null && fld.hasDefault && _equalsDefault(val, fld.def)) {
          _fail(ctx, fld.defaultCode, "field " + _stringify(fld.name) + " explicitly encodes its DEFAULT value (X.690 sec. 11.5)");
        }
      ...
      _fail(ctx, (ctx.prefix || "schema") + "/bad-schema", "unknown field kind " + _stringify(fld && fld.fkind));
    }
  }
...
function encode(schema, value, ctx) {
  switch (schema.kind) {
    ...
    case "any":      return _isBuffer(value) ? value : value.bytes;
    case "decode":
    ...
    default:         _encFail("cannot encode schema kind " + _stringify(schema.kind));
  }
  return undefined;
}

function _wrapConstructed(schema, parts) {
  if (schema.implicitTag != null) return asn1.build.contextConstructed(schema.implicitTag, _bufferConcat(parts));
  var mode = schema.assert || "sequence";
  ...
  _encFail("cannot encode a seq/repeat with assert " + _stringify(mode));
  return undefined;
}
...
function _encodeSeq(schema, value, ctx) {
  ...
  for (var f = 0; f < schema.fields.length; f++) {
    var fld = schema.fields[f];
    if (fld.fkind === "required") {
      _push(parts, encode(fld.schema, value[fld.name], ctx));
    } else if (fld.fkind === "optional") {
      if (_present(value[fld.name])) {
        ...
        if (!isDefault) _push(parts, fld.explicit ? asn1.build.explicit(fld.tag, enc) : enc);
      }
    } else if (fld.fkind === "trailing") {
      ...
      for (var i = 0; i < members.length; i++) {
        ...
        if (_present(value[mm.name])) {
          ...
          _push(parts, mm.explicit ? asn1.build.explicit(mm.tag, menc) : menc);
        }
      }
    } else {
      ...
      _encFail("unknown field kind " + _stringify(fld && fld.fkind));
    }
  }
...
function _encodeRepeat(schema, items, ctx) {
  if (!_isArray(items)) _encFail("a repeat value must be an array");
...
function _encodeChoice(schema, value, ctx) {
  ...
  if (!value || !_isInteger(value.arm) || value.arm < 0 || value.arm >= schema.alts.length) {
