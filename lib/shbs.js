...
function u8(x) { return _bufferFrom([x & 0xff]); }
function u16(x) { return _bufferFrom([(x >> 8) & 0xff, x & 0xff]); }
function u32(x) { return _bufferFrom([(x >>> 24) & 0xff, (x >>> 16) & 0xff, (x >>> 8) & 0xff, x & 0xff]); }
...
function _ilog2(x) { var r = 0; while (x > 1) { x = _floor(x / 2); r += 1; } return r; }
...
function _deriveWinternitz(n, w) {
  var u = _ceil((8 * n) / w);
  var v = _ceil((_ilog2((Math.pow(2, w) - 1) * u) + 1) / w);
...
function _cksm(Q, set) {
  var w = set.w, sum = 0, u = _ceil((8 * set.n) / w);
...
function _lmotsKc(otsSet, I, q, C, y, message) {
  ...
  var Qc = _bufferConcat([Q, _cksm(Q, otsSet)]);
