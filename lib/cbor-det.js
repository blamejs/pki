...
function _cbItem(x, who) {
  ...
  // A nested item MUST be exactly one well-formed deterministic-CBOR item (decode rejects a malformed head,
  // an indefinite length, a stray break, or trailing bytes), so a container never embeds bytes the strict
  // decoder would then reject -- the encoder never produces what its own decoder cannot read.
  try { decode(x); } catch (e) { throw new CborError("cbor/bad-item", who + ": not a single well-formed deterministic-CBOR item", e); }
  return x;
}
...
var build = {
  ...
  byteString: function (buf) { var b = _asBuffer(buf, "build.byteString"); return Buffer.concat([_head(2, b.length, "build.byteString"), b]); },
  textString: function (s) { var b = Buffer.from(String(s), "utf8"); return Buffer.concat([_head(3, b.length, "build.textString"), b]); },
  array: function (items) {
    ...
    return Buffer.concat([_head(4, items.length, "build.array")].concat(parts));
  },
  map: function (pairs) {
    ...
    enc.sort(function (a, b) { return Buffer.compare(a[0], b[0]); });                                    // RFC 8949 sec. 4.2.1 bytewise map-key order
    ...
    return Buffer.concat(flat);
  },
  tag: function (tagNum, inner) { return Buffer.concat([_head(6, tagNum, "build.tag"), _cbItem(inner, "build.tag content")]); },
  ...
  time: function (v) { var secs = v instanceof Date ? BigInt(Math.floor(v.getTime() / 1000)) : (typeof v === "bigint" ? v : BigInt(v)); return build.tag(1, build.int(secs)); },
