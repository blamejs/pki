...
function _decodeItem(buf, start, limit, depth, maxD, rules) {
  ...
  if (mt === 0 || mt === 1) {
    ...
    for (var i = 0n; i < argument; i++) {
      var el = _decodeItem(buf, cp, limit, depth + 1, maxD, rules);
    ...
    for (var j = 0n; j < argument; j++) {
      var k = _decodeItem(buf, mp, limit, depth + 1, maxD, rules);
      mp = k.end;
      var v = _decodeItem(buf, mp, limit, depth + 1, maxD, rules);
      mp = v.end;
    ...
    var inner = _decodeItem(buf, contentStart, limit, depth + 1, maxD, rules);
    children = [inner.node];
    end = inner.end;
  } else {
 ...
 * `allowTrailing`) leftover bytes after the top-level item. Size and depth are
 * bounded before the walk. A node carries `majorType`, the `argument`
...
function decode(input, opts) {
  ...
  var r = _decodeItem(buf, 0, buf.length, 0, maxDepth, rules);
