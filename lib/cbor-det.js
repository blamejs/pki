 ...
 * @fullname   Deterministic CBOR encoding and decoding (RFC 8949)
 * @order      35
 * @slug       cbor
 *
 * @intro
...
var _utf8 = new TextDecoder("utf-8", { fatal: true, ignoreBOM: true });
...
/** @internal A text builder encodes the caller's text. Turning something else into text encodes a
 *  value the caller never named: an object arrives as its default string form and a list as its
 *  members joined with a comma. */
function _asText(s, who) {
  if (typeof s !== "string") {
    throw new CborError("cbor/bad-argument", who + ": the value must be a string, got " + (s === null ? "null" : "a " + typeof s));
  }
  return s;
}
...
var build = Object.assign(Object.create(null), {
  ...
  textString: function (s) { var str = _asText(s, "build.textString"); _assertWellFormedText(str, "build.textString"); var b = Buffer.from(str, "utf8"); return _capString(Buffer.concat([_head(3, b.length, "build.textString"), b]), "build.textString"); },
  ...
  time: function (v) {
    ...
    var secs = guard.time.isDate(v) ? BigInt(Math.floor(ms / 1000)) : (typeof v === "bigint" ? v : BigInt(v));
