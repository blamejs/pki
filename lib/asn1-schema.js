...
function optional(name, schema, opts) {
  opts = opts || {};
  return { fkind: "optional", name: name, schema: schema, tag: opts.tag,
...
function _walkExplicit(schema, node, ctx) {
  if (node.tagClass !== "context" || node.tagNumber !== schema.tag || !node.children || !node.children.length) {
    _fail(ctx, schema.emptyCode || schema.code, "expected an EXPLICIT [" + schema.tag + "] wrapper");
  }
  return walk(schema.schema, node.children[0], ctx);
}
...
function _walkSeq(schema, node, ctx) {
  ...
  for (var f = 0; f < schema.fields.length; f++) {
    ...
    if (fld.fkind === "required") {
      ...
      if (next && next.tagClass === "context" && next.tagNumber === fld.tag) {
        idx++;
        var inner = next;
        if (fld.explicit) {
          if (!next.children || !next.children.length) _fail(ctx, fld.emptyCode || schema.code, "EXPLICIT [" + fld.tag + "] must wrap a value");
...
function _consumeTrailing(fld, kids, start, fields, ctx) {
  ...
  var last = fld.minTag != null ? fld.minTag - 1 : 0;
  for (var i = start; i < kids.length; i++) {
    ...
    var inner = t;
    if (member.explicit) {
      if (!t.children || !t.children.length) _fail(ctx, member.emptyCode, "EXPLICIT [" + t.tagNumber + "] must wrap a value");
      inner = t.children[0];
    }
