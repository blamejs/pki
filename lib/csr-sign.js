 ...
 * @signature pki.csr.verify(request) -> Promise<boolean>
 * @since 0.5.13
 * @status stable
 ...
 *   var heldTheKey = await pki.csr.verify(req);   // true / false
 */
function verify(request) { return guard.async.deferred(function () { return _verify(request); }); }
function _verify(request) { return csrVerify.verifyCsrSignature(_coerceCsr(request)); }
