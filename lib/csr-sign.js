...
function _implicitSetOf0(members) {
  return b.contextConstructed(0, _bufferConcat(members.slice().sort(Buffer.compare)));
}
...
function _sign(spec, key, opts) {
  opts = opts || {};
  if (!spec || typeof spec !== "object" || _isBuffer(spec)) throw _err("csr/bad-input", "the certification-request spec must be an object");
  ...
  function addAttr(attrType, valueTlv) {
    ...
    _push(attrs, b.sequence([b.oid(attrType), b.set([valueTlv])]));   // Attribute ::= SEQUENCE { type, SET OF value }
  }
  ...
  if (spec.attributes != null) {
    if (!_isArray(spec.attributes)) throw _err("csr/bad-input", "spec.attributes must be an array of pre-encoded Attribute DER");
 ...
 * @signature pki.csr.verify(request) -> Promise<{ verified, subject, subjectPublicKeyInfo, attributes, certificationRequestInfoBytes }>
 * @since 0.5.13
 * @status stable
...
function _verify(request) {
  // The fields come off the structure the door re-derived from the recorded bytes, never off the
  // argument. A caller who normalizes a parse result in place -- rewriting subject or attributes
  // before verifying -- still holds their edited object afterwards, and a bare boolean would let
  // them issue from it while the verdict was about something else. Handing back the verified
  // structure means the fields a CA acts on are the fields the signature covers.
