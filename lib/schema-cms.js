...
function _checkContentBindingAttrs(attrs, mode) {
  var ct = 0, md = 0, seen = _create(null);
  for (var i = 0; i < attrs.length; i++) {
    var a = attrs[i];
...
function _ascendingDer(nodes) {
  for (var i = 1; i < nodes.length; i++) {
    if (_bufferCompare(nodes[i - 1].bytes, nodes[i].bytes) > 0) return false;
  }
  return true;
}
...
function looksLikeSignedAttributes(bytes) {
  ...
  var sawContentType = false, sawMessageDigest = false, seenTypes = _create(null);
...
function _validateAeadParams(alg, macLen) {
  ...
  if (node.children.length === 2) {
    ...
    icvLen = _Number(iv); icvEncoded = true;
  }
...
function parseCertsOnly(der, E, prefix, maxCerts) {
  var r;
  try { r = parse(der); }
  catch (e) { throw E(prefix + "/bad-response", "a certs-only response did not decode as CMS: " + ((e && e.message) || _String(e)), e); }
