...
function _checkSignedAttrs(attrs) {
  ...
  for (var i = 0; i < attrs.length; i++) {
    ...
    if (a.type === OID_CONTENT_TYPE) {
      ...
      // ContentType ::= OBJECT IDENTIFIER (RFC 5652 §11.1) — validate the value's syntax.
      var ctNode = asn1.decode(a.values[0]);
      if (ctNode.tagClass !== "universal" || ctNode.tagNumber !== asn1.TAGS.OBJECT_IDENTIFIER) {
        throw NS.E("cms/bad-content-type-attr", "the content-type signed attribute value must be an OBJECT IDENTIFIER");
      }
      ...
      // MessageDigest ::= OCTET STRING (RFC 5652 §11.2) — validate the value's syntax.
      var mdNode = asn1.decode(a.values[0]);
      if (mdNode.tagClass !== "universal" || mdNode.tagNumber !== asn1.TAGS.OCTET_STRING) {
        throw NS.E("cms/bad-message-digest-attr", "the message-digest signed attribute value must be an OCTET STRING");
      }
    }
  }
