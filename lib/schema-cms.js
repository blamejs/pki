...
// `requireCert` (default true) sets the profile floor: the RFC 5272 sec. 4.1 Simple PKI Response the
// EST client and path builder read REQUIRES at least one certificate, so they pass nothing and get the
// strict floor. The general RFC 8551 sec. 3.8 / RFC 5652 sec. 5.1 certificate-management message
// permits "certificates and/or CRLs", so pki.cms.parseCertsOnly passes `requireCert: false` and a
// CRL-only message is accepted; a message with neither is still refused.
function parseCertsOnly(der, E, prefix, maxCerts, requireCert) {
  ...
  // digestAlgorithms is NOT required to be empty: RFC 5652 sec. 5.1 permits any number of elements
  // (including algorithms no SignerInfo uses), so a valid EST /cacerts or AIA caIssuers bundle may
  // carry a non-empty set. The builder emits an empty one (the canonical form), but the reader
  // accepts a non-empty one rather than rejecting valid CMS.
  var allCerts = r.certificates;   // parse always surfaces certificates and crls as arrays (empty when the field is absent)
  var allCrls = r.crls;
  if (needCert) {
  ...
  // Cap BEFORE the per-element parse: bound parse work by count, not only by the decoded byte size. The
  // cap is on the COMBINED number of certificates and CRLs, so a bundle carrying both cannot force up to
  // 2*maxCerts parses: certificates are counted first and CRLs draw only from the remaining budget.
  ...
  var crlCap = maxCerts != null ? (maxCerts - certs.length) : null;   // the budget certificates left for CRLs
