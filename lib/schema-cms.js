...
// RecipientKeyIdentifier ::= SEQUENCE { subjectKeyIdentifier OCTET STRING, date
//   GeneralizedTime OPTIONAL, other OtherKeyAttribute OPTIONAL } (RFC 5652 §6.2.2),
//   reached as rKeyId [0] IMPLICIT (a SEQUENCE — constructed, unlike ktri's [0] leaf).
var RECIPIENT_KEY_IDENTIFIER = schema.seq([
  schema.field("subjectKeyIdentifier", schema.octetString()),
  ...
  assert: "constructed", code: "cms/bad-recipient-key-identifier", what: "RecipientKeyIdentifier",
  build: function (m) {
    return {
      subjectKeyIdentifier: m.fields.subjectKeyIdentifier.value,
      date: m.fields.date.present ? m.fields.date.value : null,
    };
  },
});
...
var KEK_IDENTIFIER = schema.seq([
  schema.field("keyIdentifier", schema.octetString()),
  schema.optional("date", schema.time(NS), { whenUniversal: [T.GENERALIZED_TIME] }),
  schema.optional("other", schema.any(), { whenUniversal: [T.SEQUENCE] }),
], {
  assert: "sequence", code: "cms/bad-kek-identifier", what: "KEKIdentifier",
  build: function (m) { return { keyIdentifier: m.fields.keyIdentifier.value, date: m.fields.date.present ? m.fields.date.value : null }; },
});
