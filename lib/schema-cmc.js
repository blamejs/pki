...
function parseEncryptedPop(node) {
  ...
  return {
    request: parseTaggedRequest(node.children[0]),
    ...
    witness: asn1.read.octetString(node.children[4]),
  };
}
...
function parseDecryptedPop(node) {
  ...
  var thePOP;
  try { thePOP = asn1.read.octetString(node.children[2]); }
  catch (e) { throw E("cmc/bad-pop-challenge", "a DecryptedPOP thePOP is an OCTET STRING (RFC 5272 sec. 6.7)", e); }
  return {
...
/** @internal How many elements each RecipientInfo alternative carries at minimum (RFC 5652 sec. 6.2):
 * key transport is version, rid, algorithm and key; key agreement is version, originator, algorithm and
 * the encrypted keys; key encryption is version, identifier, algorithm and key; password is version,
 * algorithm and key; and other is its type and value. */
var RECIPIENT_INFO_MIN = [0, 4, 4, 3, 2];
var KTRI_MIN_ELEMENTS = 4;

/** @internal A RecipientInfo is the key-transport SEQUENCE or one of the four tagged alternatives.
 * Counting its elements is as far as this goes: the count it feeds is the number of recipients to try,
 * and reading INSIDE one is what sec. 6.7's NULL issuer rules out, so no field is decoded here. */
function _isRecipientInfoShape(node) {
  if (!node.constructed) return false;
  if (schema.isUniversal(node, TAGS.SEQUENCE)) return node.children.length >= KTRI_MIN_ELEMENTS;
  if (node.tagClass !== "context" || node.tagNumber < 1 || node.tagNumber > 4) return false;
  return node.children.length >= RECIPIENT_INFO_MIN[node.tagNumber];
}
...
function _assertEnvelopedChallenge(node) {
  ...
  /** @internal Checked by its shape rather than by running the CMS parser, because sec. 6.7 overrides
   * one of that parser's rules: an issuerAndSerialNumber recipient here carries a NULL issuer, which is
   * not a Name and which the generic decoder refuses. The wrapper and the EnvelopedData's own required
   * elements are still read, so a field naming the right content type but carrying something else is
   * refused; nothing inside a RecipientInfo is inspected, which is what leaves the NULL issuer alone. */
  var content = kids[1];
  if (content.tagClass !== "context" || content.tagNumber !== 0 || !content.constructed ||
  ...
  var env = content.children[0];
  var envKids = pkix.rootSequenceChildren(env, 1, null);
  if (!envKids || !schema.isUniversal(envKids[0], TAGS.INTEGER)) {
    throw E("cmc/bad-pop-challenge",
      "an EnvelopedData is a SEQUENCE leading with its version INTEGER (RFC 5652 sec. 6.1)");
  }
  /** @internal Read positionally, in the order sec. 6.1 gives: the version, an optional [0]
   * originatorInfo, the recipientInfos SET, then the encryptedContentInfo SEQUENCE. Finding a SET
   * somewhere in the SEQUENCE would not establish any of that. Each recipient is required to be a
   * constructed node and nothing more, which is what leaves the NULL issuer sec. 6.7 mandates alone. */
  var at = 1;
  if (at < envKids.length && envKids[at].tagClass === "context" && envKids[at].tagNumber === 0) at++;
  var recipientInfos = at < envKids.length ? envKids[at] : null;
  if (!recipientInfos || !schema.isUniversal(recipientInfos, TAGS.SET) || !recipientInfos.children.length ||
      !recipientInfos.children.every(_isRecipientInfoShape)) {
    throw E("cmc/bad-pop-challenge",
      "an EnvelopedData names at least one recipient in its recipientInfos SET, each of them one of the " +
      "RecipientInfo alternatives (RFC 5652 sec. 6.1)");
  }
  var eci = envKids[at + 1];
  if (!schema.isUniversal(eci, TAGS.SEQUENCE) || eci.children.length < 2 ||
      !schema.isUniversal(eci.children[0], TAGS.OBJECT_IDENTIFIER) ||
      !schema.isUniversal(eci.children[1], TAGS.SEQUENCE)) {
    throw E("cmc/bad-pop-challenge",
      "an EnvelopedData carries an encryptedContentInfo naming its content type and content encryption " +
      "algorithm after its recipients (RFC 5652 sec. 6.1)");
  }
  var recipients = recipientInfos.children.length;
...
function _popAlgorithm(node, what) {
  ...
  return {
    oid: asn1.read.oid(node.children[0]),
    name: oid.name(asn1.read.oid(node.children[0])) || null,
