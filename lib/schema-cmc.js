 ...
 */
function parseOtherStatusInfo(node) {
  if (node.tagClass === "universal" && node.tagNumber === TAGS.INTEGER) {
    var v = asn1.read.integer(node);
...
var TAGGED_CONTENT_INFO = schema.seq([
  ...
  build: function (m) {
    return { bodyPartID: m.fields.bodyPartID.value, contentInfoBytes: m.fields.contentInfo.value.bytes };
  },
});
