...
var TAGGED_CONTENT_INFO = schema.seq([
  ...
  build: function (m) {
    ...
    var lead = kids[0];
    if (!(ci.tagClass === "universal" && ci.tagNumber === TAGS.SEQUENCE && ci.constructed) ||
        !lead || lead.tagClass !== "universal" || lead.tagNumber !== TAGS.OBJECT_IDENTIFIER) {
    ...
    if (kids.length > 2 || (kids.length === 2 &&
        !(kids[1].tagClass === "context" && kids[1].tagNumber === 0 && kids[1].constructed))) {
      throw E("cmc/bad-cms-sequence",
        "a CMS ContentInfo is { contentType, [0] EXPLICIT content OPTIONAL }; this carries " +
        (kids.length > 2 ? kids.length + " fields" : "a second field that is not the [0] content") +
        " (RFC 5652 sec. 3)");
    }
