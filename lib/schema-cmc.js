 ...
 * (RFC 5652 sec. 6.2). Recognizing the alternative is as far as this goes: the count it feeds is the
 * number of recipients to try, and reading inside one is what sec. 6.7's NULL issuer rules out. */
function _isRecipientInfoShape(node) {
  if (!node.constructed) return false;
  if (schema.isUniversal(node, TAGS.SEQUENCE)) return true;
  return node.tagClass === "context" && node.tagNumber >= 1 && node.tagNumber <= 4;
}
