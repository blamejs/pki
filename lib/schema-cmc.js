...
function _collectStatuses(controls) {
  ...
  for (var i = 0; i < controls.length; i++) {
    ...
    _push(out, parseStatusInfo(c.values[0], version, c.bodyPartID));
  }
  return out;
}
...
function _finishBody(body, where) {
  ...
  var ids = [];
  function collect(list) { for (var j = 0; j < list.length; j++) _push(ids, list[j].bodyPartID); }
...
function parsePkiData(input) {
  var root = asn1.decode(_isBuffer(input) ? input : _bufferFrom(input));
...
function parse(input) {
  ...
  var parsedCms = (input && typeof input === "object" && !_isView(input) &&
