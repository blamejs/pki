...
var OID_ENCRYPTED_POP = O("id-cmc-encryptedPOP");
...
/** @internal The two halves of the challenge-response exchange travel in opposite directions: the
 * authority sends the challenge in its response, and the client answers in its next request (RFC 5272
 * sec. 6.7). A message carrying the other one is not the message that section describes. */
CONTROL_PLACEMENT[OID_ENCRYPTED_POP] = "pkiResponse";
CONTROL_PLACEMENT[OID_DECRYPTED_POP] = "pkiData";
...
function parseStatusInfo(valueDer, version, bodyPartID, ctx) {
  var root = pkix.decodeNested(valueDer, ctx, "a CMCStatusInfo value");
...
/** @internal The challenge-response proof of possession a server sends when the key being certified
 * cannot sign for itself (RFC 5272 sec. 6.7). The `cms` field is surfaced as the bytes it arrived as,
 * because it is an EnvelopedData a caller decrypts with its own key material rather than something
 * this parser opens. */
/** @internal The request a challenge quotes is one the client answers for, so it is held to the
 * definition of the arm it uses rather than to its outer tag alone: a proof computed over a request
 * that is not one could never be checked against anything. This is asked of a CHALLENGED request only.
 * A request body carries what its sender sent, and judging it is the enrollment's business, not the
 * decoder's. */
function _assertAnswerableRequest(request, ctx) {
  if (request.arm !== "tcr" && request.arm !== "crm") return;
  /** @internal A CertReqMessages of one, which is the shape the CRMF parser takes; built outside the
   * catch below so only the parse it wraps can be what fails. */
  var payload = request.arm === "tcr" ? request.certificationRequestBytes
    : asn1.build.sequence([asn1.build.raw(request.certReqMsgBytes)]);
  try {
    var caps = ctx ? ctx.caps : undefined;
    if (request.arm === "tcr") { csr.parse(payload, caps); } else { crmf.parse(payload, caps); }
  } catch (e) {
    throw E("cmc/bad-pop-challenge",
      "the request an EncryptedPOP quotes is not a " +
      (request.arm === "tcr" ? "PKCS#10 CertificationRequest" : "CRMF CertReqMsg") +
      ", so a proof over it could not be checked: " + ((e && e.message) || "malformed") +
      " (RFC 5272 sec. 6.7)", e);
  }
}

function parseEncryptedPop(node, ctx) {
  if (node.tagClass !== "universal" || node.tagNumber !== TAGS.SEQUENCE || !node.constructed ||
      node.children.length !== 5) {
    throw E("cmc/bad-pop-challenge",
      "an EncryptedPOP is SEQUENCE { request, cms, thePOPAlgID, witnessAlgID, witness } (RFC 5272 sec. 6.7)");
  }
  var envelope = _assertEnvelopedChallenge(node.children[1], ctx);
  var request;
  try { request = parseTaggedRequest(node.children[0]); }
  catch (e) {
    throw E("cmc/bad-pop-challenge",
      "an EncryptedPOP quotes the request it challenges as a TaggedRequest: " +
      ((e && e.message) || "malformed") + " (RFC 5272 sec. 6.7)", e);
  }
  _assertAnswerableRequest(request, ctx);
  return {
    request: request,
    /** @internal The TaggedRequest exactly as it arrived. A proof of possession is a MAC over these
     * bytes, and the challenge carries them so neither side has to re-encode the request to agree on
     * what was covered (RFC 5272 sec. 6.7). */
    requestBytes: node.children[0].bytes,
    cms: envelope.bytes,
    cmsRecipientCount: envelope.recipientCount,
    thePOPAlgID: _popAlgorithm(node.children[2], "thePOPAlgID"),
    witnessAlgID: _popAlgorithm(node.children[3], "witnessAlgID"),
    witness: _popOctetString(node.children[4], "witness"),
  };
}

/** @internal A leaf of one of the POP controls. A malformed one is a malformed challenge, so it carries
 * this control's code rather than the codec's. */
function _popOctetString(node, what) {
  try { return asn1.read.octetString(node); }
  catch (e) {
    throw E("cmc/bad-pop-challenge", "a POP " + what + " is an OCTET STRING (RFC 5272 sec. 6.7)", e);
  }
}

function parseDecryptedPop(node) {
  if (node.tagClass !== "universal" || node.tagNumber !== TAGS.SEQUENCE || !node.constructed ||
      node.children.length !== 3) {
    throw E("cmc/bad-pop-challenge",
      "a DecryptedPOP is SEQUENCE { bodyPartID, thePOPAlgID, thePOP } (RFC 5272 sec. 6.7)");
  }
  var bodyPartID;
  try { bodyPartID = readBodyPartIdentity(node.children[0]); }
  catch (e) {
    throw E("cmc/bad-pop-challenge",
      "a DecryptedPOP names the certification request it proves with a BodyPartID: " +
      ((e && e.message) || "malformed") + " (RFC 5272 sec. 6.7)", e);
  }
  return {
    bodyPartID: bodyPartID,
    thePOPAlgID: _popAlgorithm(node.children[1], "thePOPAlgID"),
    thePOP: _popOctetString(node.children[2], "thePOP"),
  };
}

/** @internal The proof value travels in an EnvelopedData (RFC 5272 sec. 6.7), and the bytes are handed
 * on unopened because opening them needs key material this parser does not hold. Unopened is not
 * unchecked: a field that is not a ContentInfo carrying an EnvelopedData cannot be a challenge, and
 * saying so here is what keeps a control that could never be answered from reading as one. */
function _assertEnvelopedChallenge(node, ctx) {
  var kids = pkix.rootSequenceChildren(node, 2, 2);
  if (!kids) {
    throw E("cmc/bad-pop-challenge",
      "an EncryptedPOP carries the proof value in a CMS ContentInfo, which is a SEQUENCE of a content " +
      "type and its content (RFC 5272 sec. 6.7)");
  }
  var contentType;
  try { contentType = asn1.read.oid(kids[0]); }
  catch (e) {
    throw E("cmc/bad-pop-challenge",
      "an EncryptedPOP's ContentInfo names its content type with an OBJECT IDENTIFIER (RFC 5272 sec. 6.7)", e);
  }
  if (contentType !== O("envelopedData")) {
    throw E("cmc/bad-pop-challenge",
      "an EncryptedPOP carries the proof value in an EnvelopedData, got " +
      (oid.name(contentType) || contentType) + " (RFC 5272 sec. 6.7)");
  }
  var content = kids[1];
  if (content.tagClass !== "context" || content.tagNumber !== 0 || !content.constructed ||
      content.children.length !== 1) {
    throw E("cmc/bad-pop-challenge",
      "an EncryptedPOP's ContentInfo carries its content in [0] EXPLICIT (RFC 5652 sec. 3)");
  }
  /** @internal Validated by the CMS EnvelopedData schema itself rather than by a shape check written
   * here, so every field is held to one definition and the count reported below is the count of
   * recipients that same definition accepted. Sec. 6.7 also allows a recipient identified by an
   * issuerAndSerialNumber whose issuer is encoded as NULL, which is not the Name that schema requires;
   * a challenge addressed only that way is refused here, and `pki.cms.decrypt` could not open it
   * either, so it is refused where the reason can be named rather than after a failed answer. */
  var enveloped;
  try { enveloped = cms.walkEnvelopedData(content.children[0], ctx); }
  catch (e) {
    throw E("cmc/bad-pop-challenge",
      "an EncryptedPOP's ContentInfo does not carry an EnvelopedData a recipient could open: " +
      ((e && e.message) || "malformed") + " (RFC 5652 sec. 6.1)", e);
  }
  var recipients = (enveloped.recipientInfos || []).length;
  if (recipients < 1) {
    throw E("cmc/bad-pop-challenge",
      "an EnvelopedData names at least one recipient in its recipientInfos SET (RFC 5652 sec. 6.1)");
  }
  return { bytes: node.bytes, recipientCount: recipients };
}

/** @internal Both fields are AlgorithmIdentifiers, and both are read the same way: the OID names the
 * algorithm and the parameters travel with it, so a caller answering a challenge copies what it was
 * sent rather than reconstructing it. */
function _popAlgorithm(node, what) {
  if (node.tagClass !== "universal" || node.tagNumber !== TAGS.SEQUENCE || !node.constructed ||
      node.children.length < 1 || node.children.length > 2) {
    throw E("cmc/bad-pop-challenge", "a POP " + what + " is an AlgorithmIdentifier (RFC 5272 sec. 6.7)");
  }
  var algOid;
  try { algOid = asn1.read.oid(node.children[0]); }
  catch (e) {
    throw E("cmc/bad-pop-challenge",
      "a POP " + what + " names its algorithm with an OBJECT IDENTIFIER (RFC 5272 sec. 6.7)", e);
  }
  return {
    oid: algOid,
    name: oid.name(algOid) || null,
    parameters: node.children.length === 2 ? node.children[1].bytes : null,
    bytes: node.bytes,
  };
}

/** @internal One AttributeValue, which is what a control carrying a single structure holds. */
function _onlyValue(c, what, ctx) {
  if (c.values.length !== 1) {
    throw E("cmc/bad-pop-challenge",
      "a " + what + " control carries exactly one AttributeValue, got " + c.values.length);
  }
  return pkix.decodeNested(c.values[0], ctx, what + " control value");
}

function _decodePopChallenges(controls, ctx) {
  for (var i = 0; i < controls.length; i++) {
    var c = controls[i];
    if (c.attrType === OID_ENCRYPTED_POP) {
      c.encryptedPOP = parseEncryptedPop(_onlyValue(c, "an Encrypted POP", ctx), ctx);
    } else if (c.attrType === OID_DECRYPTED_POP) {
      c.decryptedPOP = parseDecryptedPop(_onlyValue(c, "a Decrypted POP", ctx));
    }
  }
}

function _collectStatuses(controls, ctx) {
  var out = [];
  for (var i = 0; i < controls.length; i++) {
    var c = controls[i];
    ...
    out.push(parseStatusInfo(c.values[0], version, c.bodyPartID, ctx));
  }
  return out;
}

function _finishBody(body, where, ctx) {
  for (var i = 0; i < body.controls.length; i++) assertControlPlacement(body.controls[i], where);
  _decodePopChallenges(body.controls, ctx);
  body.statuses = _collectStatuses(body.controls, ctx);
 ...
 * @signature  pki.schema.cmc.parsePkiData(der, caps?) -> parsed
 * @since      0.4.16
 * @status     stable
 ...
 */
function parsePkiData(input, callerOpts) {
  return pkix.runParseWith(input, _BODY_PARSE_OPTS, callerOpts, function (root, ns) {
    var body = schema.walk(PKI_DATA, root, ns).result;
    body.bytes = root.bytes;
    return _finishBody(body, "pkiData", ns);
  });
}

/**
 * @primitive  pki.schema.cmc.parsePkiResponse
 * @signature  pki.schema.cmc.parsePkiResponse(der, caps?) -> parsed
 * @since      0.4.16
 * @status     stable
 ...
 *     refused with the `/too-large` code of this format.
 */
function parsePkiResponse(input, callerOpts) {
  return pkix.runParseWith(input, _BODY_PARSE_OPTS, callerOpts, function (root, ns) {
    var body = schema.walk(PKI_RESPONSE, root, ns).result;
    body.bytes = root.bytes;
    return _finishBody(body, "pkiResponse", ns);
  });
}

/** @internal The door options for the two body parsers, which read a bare DER body rather than
 *  a CMS carrier, so they take no PEM label. `rawDecodeErrors` keeps the codec's own code for a
 *  malformed body, which callers already pin. */
var _CARRIER_PARSE_OPTS = pkix.parseOpts({ ErrorClass: CmcError, prefix: "cmc", what: "the CMC message" });

var _BODY_PARSE_OPTS = pkix.parseOpts({
  ns: NS, ErrorClass: CmcError, PemError: CmcError, prefix: "cmc",
  what: "the CMC body", pemLabel: null, rawDecodeErrors: true,
});

/**
 * @primitive  pki.schema.cmc.parse
 * @signature  pki.schema.cmc.parse(input, caps?) -> parsed
 * @since      0.4.16
 * @status     stable
...
function parse(input, callerOpts) {
  /** @internal Before the branch, because the already-parsed arm reaches no decode and would
   *  otherwise hold a caller to the option contract only when they passed bytes. The snapshot
   *  is what travels onward: handing the caller's own object to the CMS parser would read each
   *  cap a second time, and a value read twice need not answer the same way twice. */
  var caps = pkix.parseCaps(callerOpts, _CARRIER_PARSE_OPTS);
  var alreadyParsed = input && typeof input === "object" && !ArrayBuffer.isView(input) &&
    !guard.bytes.isByteSource(input) && input.encapContentInfo != null;
  var parsedCms;
  if (alreadyParsed) { parsedCms = input; }
  else {
    /** @internal Bytes that are not a CMS message at all are a CMC message this cannot read, and every
     * other format parser answers for its own input in its own domain. A resource refusal is the one
     * exception: it is not a statement about the bytes, so it keeps its own code. */
    try { parsedCms = cms.parse(input, caps); }
    catch (e) {
      if (e && typeof e.code === "string" && e.code.indexOf("/too-large") !== -1) {
        throw E("cmc/too-large", "the CMC message exceeds the decode budget for this parse: " + (e.message || ""), e);
      }
      throw E("cmc/bad-der",
        "a CMC message is carried in a CMS SignedData or AuthenticatedData, and these bytes are not " +
        "one: " + ((e && e.message) || "malformed") + " (RFC 5272 sec. 3.2 / 4.2)", e);
    }
  }
  ...
  var body = eContentType === OID_PKI_DATA
    ? parsePkiData(encap.eContent, caps)
    : parsePkiResponse(encap.eContent, caps);
  body.cms = parsedCms;
...
module.exports = {
  parseEncryptedPop: parseEncryptedPop,
  parseTaggedRequest: parseTaggedRequest,
  parse: parse,
