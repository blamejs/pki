...
function _assertEnvelopedChallenge(node) {
  ...
  var enveloped;
  try { enveloped = cms.walkEnvelopedData(content.children[0]); }
  catch (e) {
    throw E("cmc/bad-pop-challenge",
...
function _onlyValue(c, what) {
  if (c.values.length !== 1) {
    throw E("cmc/bad-pop-challenge",
      "a " + what + " control carries exactly one AttributeValue, got " + c.values.length);
  }
  return asn1.decode(c.values[0]);
}

function _decodePopChallenges(controls) {
  for (var i = 0; i < controls.length; i++) {
    var c = controls[i];
    if (c.attrType === OID_ENCRYPTED_POP) {
      c.encryptedPOP = parseEncryptedPop(_onlyValue(c, "an Encrypted POP"));
    } else if (c.attrType === OID_DECRYPTED_POP) {
      c.decryptedPOP = parseDecryptedPop(_onlyValue(c, "a Decrypted POP"));
    }
  }
}
...
function _finishBody(body, where) {
  ...
  _decodePopChallenges(body.controls);
  ...
  if (alreadyParsed) { parsedCms = input; }
    ...
    try { parsedCms = cms.parse(input, caps); }
      ...
      if (e && typeof e.code === "string" && e.code.indexOf("/too-large") !== -1) {
        throw E("cmc/too-large", "the CMC message exceeds the decode budget for this parse: " + ((e && e.message) || ""), e);
      }
      throw E("cmc/bad-der",
