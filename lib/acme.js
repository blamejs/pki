...
function _clientUrl(urlStr) {
  ...
  // (`/..`, `/%2e%2e/`) -- by comparing the raw path in `s` to the WHATWG-parsed pathname the transport
  // sends as the request target; a mismatch means the signed JWS `url` would keep a path the request never
  // used. (The authority may still normalize -- a default :443 port, an uppercase host -- as that does not
  // change the path; an encoded char like `%41` or a dot inside a segment name round-trips unchanged.)
