 ...
 * and again where the value is used. A falsy one is the same fault: reading it as "none supplied"
 * would fall back to the default client and reach the CA over the network for a caller that believed
 * it had injected one. */
function _assertTransport(transport) {
  if (transport != null && typeof transport !== "function") {
    throw E("acme/bad-input", "opts.transport must be a function (request) => Promise<{ status, headers, body }>, got " + guard.text.showValue(transport));
  }
}
