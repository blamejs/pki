...
// RFC 3339 date-time (a strict-enough subset: date "T" time with a zone).
var RFC3339_RE = /^\d{4}-\d{2}-\d{2}[Tt]\d{2}:\d{2}:\d{2}(\.\d+)?([Zz]|[+-]\d{2}:\d{2})$/;
function _isRfc3339(v) { return _isString(v) && RFC3339_RE.test(v); }
// A URL string: an absolute http(s) URI (RFC 3986). ACME URLs are server-provided.
function _isUrl(v) { return _isString(v) && /^https?:\/\/[^\s]+$/.test(v); }
...
var SPECS = {
  ...
  authorization: [
    ...
    { name: "challenges", type: "array", required: true, minItems: 1, elemType: "object" },
...
var TRANSITIONS = {
  ...
  order:         { pending: ["ready", "invalid"], ready: ["processing", "invalid"], processing: ["valid", "invalid"] },
};
...
function validateProblem(obj) {
  ...
  if (!_isString(obj.type)) throw E("acme/bad-problem", "a problem document must carry a string type");
  if (Object.prototype.hasOwnProperty.call(obj, "subproblems")) {
    ...
    for (var i = 0; i < obj.subproblems.length; i++) {
      ...
      if (!_isObject(sub) || !_isString(sub.type)) throw E("acme/bad-problem", "each subproblem must be a problem document");
      if (Object.prototype.hasOwnProperty.call(sub, "identifier")) _validateIdentifier(sub.identifier);
    }
  }
  return obj;
}
...
async function verifyTlsAlpn01(certDer, token, accountJwk, identifier) {
  ...
  if (identifier.type === "dns") {
    ...
    if (String(entry.value).toLowerCase() !== String(identifier.value).toLowerCase()) throw E("acme/bad-tlsalpn", "the SAN dNSName does not match the identifier");
    ...
    var wantIp = _canonicalIpText(identifier.value);
    if (sanIp === null || wantIp === null || sanIp !== wantIp) throw E("acme/bad-tlsalpn", "the iPAddress SAN does not match the ip identifier (RFC 8738 sec. 6)");
  } else {
 ...
 */
function postAsGet(o) { return _signOuter(o, undefined); }
...
function _assertContacts(contacts) {
  ...
  contacts.forEach(function (c) {
    if (!_isString(c)) throw E("acme/bad-contact", "each contact must be a URL string");
    if (c.indexOf("mailto:") === 0) {
...
function newAccount(o) {
  ...
  if (o.termsOfServiceAgreed !== undefined) payload.termsOfServiceAgreed = !!o.termsOfServiceAgreed;
  if (o.onlyReturnExisting !== undefined) payload.onlyReturnExisting = !!o.onlyReturnExisting;
...
function _subjectCommonName(subject) {
  if (!subject || !Array.isArray(subject.rdns)) return null;
  for (var i = 0; i < subject.rdns.length; i++) {
    var atvs = subject.rdns[i];
    for (var j = 0; j < atvs.length; j++) {
      if (atvs[j].name === "commonName" || atvs[j].type === OID_CN) return _isString(atvs[j].value) ? atvs[j].value : null;
    }
  }
  return null;
}

// The RFC 8738 canonical text of an ip identifier VALUE (dotted-decimal IPv4 with
// no leading zeros, or the RFC 5952 IPv6 form), or null if it is not a valid IP.
// Used to compare a requested ip identifier against an iPAddress SAN's octets.
function _canonicalIpText(value) {
  if (!_isString(value)) return null;
  if (/^\d{1,3}(\.\d{1,3}){3}$/.test(value)) {
    var p = value.split("."), out = [];
    for (var i = 0; i < 4; i++) {
      if (!/^\d+$/.test(p[i])) return null;
      var oct = parseInt(p[i], 10);
      if (oct > 255) return null;
      out.push(String(oct));
    }
    return out.join(".");
  }
  if (value.indexOf(":") !== -1) return _canonicalizeIpv6(value.toLowerCase());
  return null;
}
...
function _csrIdentifierSet(parsedCsr) {
  var set = {};
  var cn = _subjectCommonName(parsedCsr.subject);
  if (cn) set["dns:" + cn.toLowerCase()] = true;
  var extReq = (parsedCsr.attributes || []).filter(function (a) { return a.type === oid.byName("extensionRequest"); })[0];
  if (extReq && Array.isArray(extReq.extensions)) {
    var san = extReq.extensions.filter(function (e) { return e.oid === OID_SAN; })[0];
    if (san) {
      var dec = _extDecoders.byOid[OID_SAN](san.value, _extCtx);
 ...
 */
function deactivate(o) { return _signOuter({ key: o.key, alg: o.alg, nonce: o.nonce, url: o.url, kid: o.kid }, { status: "deactivated" }); }
 ...
 * is the base64url of the DER certificate and optional `reason` is an RFC 5280
 * CRLReason (0..10). Signed EITHER by the account key (`kid` mode) OR by the
...
function revokeCert(o) {
  ...
  if (o.reason !== undefined) {
    if (typeof o.reason !== "number" || !isFinite(o.reason) || Math.floor(o.reason) !== o.reason || o.reason < 0 || o.reason > 10) {
      throw E("acme/bad-revocation-reason", "reason must be an RFC 5280 CRLReason integer 0..10");
    }
