...
function _clientUrl(urlStr) {
  ...
  // Reject an input the URL parser had to REPAIR (a missing `//`, surrounding whitespace, a backslash, a
  // dropped default port): the transport re-parses the request URL, so a non-canonical string would sign a
  // JWS `url` that diverges from the URL actually connected to. A canonical URL is returned VERBATIM (so a
  // non-default port the server chose is preserved for the sec. 6.4/7.3 match).
  if (url.href !== s) throw E("acme/bad-url", "the ACME URL is not canonical (the parser would repair it): " + JSON.stringify(s));
  return s;
}
// Resolve a server `Location` (which MAY be a relative URI-reference -- valid HTTP, common behind a proxy)
// against the request URL, then https-gate the resolved ABSOLUTE URL (its canonical href is used as the
// account/order URL). A caller URL uses _clientUrl directly; only a server Location is resolved this way.
...
function client(directoryUrl, opts) {
  ...
  function _tlsFor(url) {
    var t = budgets.tls;
    if ((t.cert != null || t.key != null) && new URL(url).origin !== trustedOrigin) {
      t = Object.assign({}, t); delete t.cert; delete t.key;
    }
    return t;
  }
  ...
  function _keyChange(o) {
    ...
    return _resource("keyChange").then(function (url) {
      return _post(url, keyChange, { account: account, oldKey: accountJwk, newKey: o.newKey, newJwk: o.newJwk, newAlg: o.newAlg }, "kid").then(function (res) {
        // Validate the account object the server returns (RFC 8555 sec. 7.3.5) BEFORE rotating the session
        // key: a misrouted / empty / non-account 200 must leave the client on the WORKING key, not
        // desynchronize it (server on the old key, client on the new one, every later request failing).
        var acct = validate("account", _json(res));
