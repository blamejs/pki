...
function _validAbsoluteUri(t) {
  if (!/^[A-Za-z][A-Za-z0-9+.-]*:/.test(t) || LINK_URI_BADPCT.test(t) || !_bracketsOnlyInAuthority(t) || _authorityMultiAt(t) || _multiFragment(t)) return false;
...
// encoded "%23" is a literal char, not counted. Applies to a fetched target/anchor AND an ext-rel-type alike.
...
// "@" host); a second literal "@" is invalid RFC 3986, which WHATWG ACCEPTS while rewriting the first "@". Applies
// to a fetched target/anchor AND an ext-rel-type identifier alike.
...
function _linkUriInvalid(uri) {
  ...
  return LINK_URI_BADCHAR.test(uri) || LINK_URI_BADPCT.test(uri) || _hasEncodedDotSegment(uri) || !_bracketsOnlyInAuthority(uri) || _hasEmptyAuthority(uri) || _authorityMultiAt(uri) || _multiFragment(uri);
}
