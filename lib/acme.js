...
function _resolveLocation(loc, base) {
  ...
  // the raw advertised query (RFC 8555 sec. 6.4). Compare the raw query to the resolved search and reject a repair
  // -- _clientUrl makes this check for an absolute URL, which a relative reference would otherwise bypass.
  var qi = loc.indexOf("?");
  if (qi !== -1 && loc.slice(qi).split("#")[0] !== u.search) throw E("acme/bad-url", "a relative Link/Location query is not canonical (resolution re-encoded it): " + JSON.stringify(loc));
...
// Is `t` a valid RFC 3986 absolute URI? Require a scheme + valid pct-escapes, then defer the STRUCTURE to WHATWG,
// which correctly rejects an empty IP-literal ("http://[]"), a bad port ("http://h:bad"), and an unterminated
// bracket ("http://["). WHATWG lacks IPvFuture, which RFC 3986 allows, so a valid IPvFuture authority is swapped
// for a placeholder IPv6 host before the parse (the IPvFuture form itself is validated by the pattern that matches
// it). WHATWG also caps the port at 16 bits, but RFC 3986 port = *DIGIT (no range limit for an IDENTIFIER never
// connected to), so an all-digit port is swapped for ":1" before the parse -- a non-numeric port (":bad") is left
// intact and still rejected. (This relaxation is for the ext-rel-type only; a fetched target keeps WHATWG's port
// range, since it is actually connected to over TCP.) `URL.canParse` never throws, so no swallow.
function _validAbsoluteUri(t) {
  if (!/^[A-Za-z][A-Za-z0-9+.-]*:/.test(t) || LINK_URI_BADPCT.test(t) || _uriStructurallyInvalid(t)) return false;
  // WHATWG also coerces an all-numeric dotted host into an IPv4 literal and REJECTS an out-of-range one
  // ("1.2.3.4.5", "999.999.999.999"), but RFC 3986 reg-name = *(unreserved / pct-encoded / sub-delims) accepts it
  // (its characters are already validated). Swap a NON-IP-literal host for a placeholder reg-name before the parse
  // so canParse still validates the IP-literal host (an invalid IPv6 / empty "[]" still fails) and the port, but
  // does not impose its IPv4 coercion on a reg-name. (An IP-literal "[...]" host is left intact.)
  var probe = t
    .replace(/^([A-Za-z][A-Za-z0-9+.-]*:\/\/(?:[^/?#@[\]]*@)?)\[[vV][0-9A-Fa-f]+\.[A-Za-z0-9._~!$&'()*+,;=:-]+\]/, "$1[::1]")
    .replace(/^([A-Za-z][A-Za-z0-9+.-]*:\/\/(?:[^/?#@]*@)?)[^/?#:@[\]]+/, "$1x")
    .replace(/^([A-Za-z][A-Za-z0-9+.-]*:\/\/(?:[^/?#@]*@)?(?:\[[^\]]*\]|[^/?#:]*)):[0-9]+(?=[/?#]|$)/, "$1:1");
  return URL.canParse(probe);
}
...
function _hasEmptyAuthority(uri) {
  ...
  var after = uri.charAt(m[0].length);
...
function _bracketsOnlyInAuthority(uri) {
  ...
  return rest.indexOf("[") === -1 && rest.indexOf("]") === -1;
}
...
// while rewriting): brackets only in an authority IP-literal, a non-empty authority, at most one "@" in the
// authority, at most one "#", no colon in a relative-path first segment. SHARED by the fetched-target/anchor check
// (_linkUriInvalid) AND the ext-rel-type check (_validAbsoluteUri) so the two can NEVER diverge -- a new structural
// rule added here binds both at once.
function _uriStructurallyInvalid(uri) {
  return !_bracketsOnlyInAuthority(uri) || _hasEmptyAuthority(uri) || _authorityMultiAt(uri) || _multiFragment(uri) || _relFirstSegHasColon(uri);
}
...
function _parseLinkValue(raw) {
  ...
  var params = _splitLinkParams(rest), rel = "", relSeen = false, anchor = null;
  for (var i = 0; i < params.length; i++) {
    // The FIRST occurrence of rel / anchor wins; later ones are ignored (RFC 8288 sec. 3.3) -- take the first
    ...
    else if (params[i].name === "anchor" && anchor === null) {
      // anchor is a URI-VALUED param (RFC 8288 sec. 3.2). A VALUELESS anchor (no `=`) is malformed; an explicit
      // value -- even an empty quoted one (`anchor=""`, the empty URI-reference that resolves to the context) --
      // is valid, so it is recorded (a valueless generic extension param, in contrast, is permitted elsewhere).
  ...
  for (var fi = 0; fi < fields.length; fi++) {
    ...
    for (var vi = 0; vi < values.length; vi++) {
      ...
      if (lv.anchor != null) {
        ...
        var ctx;
        try { ctx = new URL(lv.anchor, base).href; } catch (_ae) { continue; }
        if (ctx !== baseHref) continue;
      }
      var resolved;
...
function client(directoryUrl, opts) {
  ...
  function _newAuthz(identifier) {
    ...
    return _resource("newAuthz").then(function (url) {
      return _post(url, newAuthz, { identifier: canon }, "kid", undefined, [201]).then(function (res) {
        var loc = res.headers["location"];
  ...
  function _renewalInfo(certDer, clockFn, retryAfterCapSeconds) {
    ...
    if (clk() > x509.parse(certDer).validity.notAfter.getTime()) throw E("acme/certificate-expired", "the certificate is already past its notAfter; a client MUST NOT check RenewalInfo after it has expired (RFC 9773 sec. 4.3)");
