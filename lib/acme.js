...
function _validAbsoluteUri(t) {
  if (!/^[A-Za-z][A-Za-z0-9+.-]*:/.test(t) || LINK_URI_BADPCT.test(t) || !_bracketsOnlyInAuthority(t)) return false;
  // An authority carries at most one "@" (userinfo "@" host); a second literal "@" is invalid RFC 3986, which
  // WHATWG would ACCEPT while silently repairing (percent-encoding the first "@"). Reject it before the parse so
  // canParse's leniency cannot pass a rewritten URI. (The check is on the raw authority, before any IPv* probe.)
  var auth = /^[A-Za-z][A-Za-z0-9+.-]*:\/\/([^/?#]*)/.exec(t);
  if (auth && auth[1].split("@").length > 2) return false;
  var probe = t.replace(/^([A-Za-z][A-Za-z0-9+.-]*:\/\/)\[[vV][0-9A-Fa-f]+\.[A-Za-z0-9._~!$&'()*+,;=:-]+\]/, "$1[::1]");
  return URL.canParse(probe);
}
// Are any brackets in `uri` confined to a single authority IP-literal (`scheme://[...]`)? RFC 3986 permits `[`/`]`
// ONLY there; anywhere else -- a path ("/cert/[alt]") or an opaque scheme part ("urn:[") -- they are invalid, yet
// WHATWG's lenient parse accepts them (a certificate URL never uses one). No bracket at all is trivially fine.
// A URI-reference that declares an authority ("//", optionally after a scheme) must have a NON-EMPTY authority.
// A "//" immediately followed by "/", "?", "#", or end is an empty authority WHATWG silently REPAIRS by promoting
// the next path segment to the host ("///cert/alt" -> host "cert"; "////acme.example/p" -> host "acme.example",
// which then passes a same-origin gate although the raw reference was malformed). Reject it before resolution.
...
function _bracketsOnlyInAuthority(uri) {
  ...
  // The scheme is optional: a network-path reference ("//[2001:db8::1]/p", RFC 3986 sec. 4.2) has an authority
  // IP-literal with no scheme, and is as valid a same-origin alternate as a full absolute URI.
  var m = /^(?:[A-Za-z][A-Za-z0-9+.-]*:)?\/\/\[[^[\]]*\]/.exec(uri);
  if (!m) return false;
...
function _linkUriInvalid(uri) {
  ...
  return LINK_URI_BADCHAR.test(uri) || LINK_URI_BADPCT.test(uri) || _hasEncodedDotSegment(uri) || !_bracketsOnlyInAuthority(uri) || _hasEmptyAuthority(uri);
}
