...
// Every operation this module decides with, taken at load. Read at call time each of these is an
// ordinary writable property a caller can replace once this module has loaded, and each answers a
// question the module then acts on: whether a value is a list, whether two byte strings are equal,
// where a URL's authority ends, whether a table really holds a key. See guard-intrinsic.
...
// The settle operations need the constructor as their receiver, so it is captured alongside them.
var _Promise = Promise;
var _resolve = intrinsic.uncurry(intrinsic.promiseResolve);
var _reject = intrinsic.uncurry(intrinsic.promiseReject);
...
var _replace = intrinsic.uncurry(String.prototype.replace);
...
var _trim = intrinsic.uncurry(String.prototype.trim);
...
function _isObject(v) { return v && typeof v === "object" && !_isArray(v); }
...
function _defaultRandom() { return _bufferFrom(webcrypto.getRandomValues(new _Uint8Array(6))).readUIntBE(0, 6) / _RANDOM_DENOM; }
...
function _checkType(kind, field, value, obj) {
  switch (field.type) {
    ...
    case "array":
      if (!_isArray(value)) return "must be an array";
...
function _codeSlug(kind) { return _toLowerCase(_replace(kind, /([A-Z])/g, "-$1")); }
...
function _validate(kind, obj) {
  ...
  for (var f = 0; f < spec.length; f++) {
    ...
    if (!present) {
      if (required) throw E("acme/missing-field", "an ACME " + kind + " is missing the required field " + _stringify(field.name));
      continue;
    }
    if (field.enum && _indexOf(field.enum, obj[field.name]) === -1) {
      throw E("acme/bad-status", "the " + kind + " " + field.name + " " + _stringify(obj[field.name]) + " is not a recognized value");
    }
    var err = _checkType(kind, field, obj[field.name], obj);
    if (err) throw E("acme/bad-" + _codeSlug(kind), "the " + kind + " field " + _stringify(field.name) + " " + err);
  }
  return obj;
}
...
function _validateIdentifier(id) {
  ...
  if (type === "dns") {
    if (_strIndexOf(value, "*.") === 0) throw E("acme/bad-identifier", "a wildcard *. value is not permitted in an authorization identifier (RFC 8555 sec. 7.1.4)");
...
function _assertDnsName(name) {
  ...
  var labels = _split(name, ".");
  for (var i = 0; i < labels.length; i++) {
    var l = labels[i];
    if (l.length > 63) throw E("acme/bad-identifier", "a dns label must be 1..63 characters (RFC 1035 sec. 2.3.4): " + _stringify(l));
    if (!/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/.test(l)) throw E("acme/bad-identifier", "a dns label must be lowercase LDH ASCII (A-label): " + _stringify(l));
    if (_strIndexOf(l, "xn--") === 0 && !/^xn--[a-z0-9]+(-[a-z0-9]+)*$/.test(l)) throw E("acme/bad-identifier", "a malformed xn-- A-label: " + _stringify(l));
  }
}
...
function _assertIpAddress(value) {
  ...
  if (/^\d{1,3}(\.\d{1,3}){3}$/.test(value)) {
    ...
    for (var i = 0; i < 4; i++) {
      if (!/^(0|[1-9]\d{0,2})$/.test(parts[i]) || _parseInt(parts[i], 10) > 255) throw E("acme/bad-identifier", "an ip identifier IPv4 octet is out of range or non-canonical: " + _stringify(value));
    }
    return;
  }
  if (_strIndexOf(value, ":") !== -1) {
    if (value !== _toLowerCase(value)) throw E("acme/bad-identifier", "an IPv6 ip identifier must be lowercase (RFC 5952)");
    var canon = _canonicalizeIpv6(value);
    if (canon === null || canon !== value) throw E("acme/bad-identifier", "an ip identifier must be the RFC 5952 canonical IPv6 form: " + _stringify(value));
    return;
  }
...
function _canonicalizeIpv6(value) {
  var groups;
  if (_strIndexOf(value, "::") !== -1) {
    if (_strIndexOf(value, "::") !== _strLastIndexOf(value, "::")) return null;   // only one ::
    var halves = _split(value, "::");
    var left = halves[0] ? halves[0].split(":") : [];
    var right = halves[1] ? halves[1].split(":") : [];
    var fill = 8 - left.length - right.length;
    if (fill < 1) return null;
    groups = _concat(_concat(left, new Array(fill).fill("0")), right);
  } else {
    groups = _split(value, ":");
  }
  ...
  for (var i = 0; i < 8; i++) {
    ...
    _push(nums, _parseInt(groups[i], 16));
  }
  var hex = _map(nums, function (n) { return _numToString(n, 16); });
  ...
  var head = _join(_arraySlice(hex, 0, bestStart), ":");
  var tail = _join(_arraySlice(hex, bestStart + bestLen), ":");
  return head + "::" + tail;
}
...
function assertTransition(kind, from, to) {
  ...
  if (!table) throw E("acme/bad-input", "unknown resource kind " + _stringify(kind));
  if (from === to) return;
  ...
  if (!allowed || _indexOf(allowed, to) === -1) throw E("acme/bad-transition", "illegal " + kind + " transition " + _stringify(from) + " -> " + _stringify(to) + " (RFC 8555 sec. 7.1.6)");
}
...
function validateProblem(obj) {
  ...
  if (!_isString(obj.type) || _strIndexOf(obj.type, ERROR_NAMESPACE) !== 0) throw E("acme/bad-problem", "an ACME problem type must be in the " + ERROR_NAMESPACE + " namespace (RFC 8555 sec. 6.7)");
  if (_hasOwn(obj, "subproblems")) {
    if (!_isArray(obj.subproblems)) throw E("acme/bad-problem", "subproblems must be an array");
    for (var i = 0; i < obj.subproblems.length; i++) {
      var sub = obj.subproblems[i];
      if (!_isObject(sub) || !_isString(sub.type) || _strIndexOf(sub.type, ERROR_NAMESPACE) !== 0) throw E("acme/bad-problem", "each subproblem must be an ACME problem document in the error namespace");
...
function validate(kind, obj) {
  ...
  if (!SPECS[kind]) throw E("acme/bad-input", "unknown ACME object kind " + _stringify(kind));
...
function identify(obj) {
  ...
  if (_isString(obj.type) && _strIndexOf(obj.type, ERROR_NAMESPACE) === 0) return "problem";
...
function _assertToken(token) {
  if (!_isString(token) || !_reTest(TOKEN_RE, token)) throw E("acme/bad-token", "a challenge token must be >= " + constants.LIMITS.ACME_TOKEN_MIN_CHARS + " base64url characters with no padding (RFC 8555 sec. 8)");
}

async function _sha256(bytes) { return _bufferFrom(await subtle.digest("SHA-256", bytes)); }
...
async function dns01(token, accountJwk, domain) {
  ...
  var base = _strIndexOf(domain, "*.") === 0 ? _strSlice(domain, 2) : domain;
  _assertDnsName(base);
  var ka = await keyAuthorization(token, accountJwk);
  return { name: "_acme-challenge." + base, value: jose.base64url.encode(await _sha256(_bufferFrom(ka, "ascii"))) };
}
...
async function tlsAlpn01Extension(token, accountJwk) {
  var ka = await keyAuthorization(token, accountJwk);
  var digest = await _sha256(_bufferFrom(ka, "ascii"));            // 32 bytes
...
async function verifyTlsAlpn01(certDer, token, accountJwk, identifier) {
  ...
  var acmeExt = _filter(exts, function (e) { return e.oid === OID_ACME_IDENTIFIER; })[0];
  ...
  var ka = await keyAuthorization(token, accountJwk);
  var expected = await _sha256(_bufferFrom(ka, "ascii"));
  if (!_bufEquals(auth, expected)) throw E("acme/bad-tlsalpn", "the acmeIdentifier digest does not match the key authorization");
  // SAN: exactly one entry, of the identifier's type, equal to its value.
  var sanExt = _filter(exts, function (e) { return e.oid === OID_SAN; })[0];
  ...
  if (identifier.type === "dns") {
    ...
    if (_toLowerCase(_String(entry.value)) !== identifier.value) throw E("acme/bad-tlsalpn", "the SAN dNSName does not match the identifier");
    ...
    throw E("acme/bad-tlsalpn", "unsupported tls-alpn-01 identifier type " + _stringify(identifier.type));
  }
}
...
function _payloadBuf(obj) {
  if (obj === undefined) return _bufferAlloc(0);                 // POST-as-GET
  return _bufferFrom(_stringify(obj), "utf8");
}
...
function _assertContacts(contacts) {
  if (!_isArray(contacts)) throw E("acme/bad-contact", "contact must be an array of URL strings");
  _forEach(contacts, function (c) {
    ...
    if (_toLowerCase(_strSlice(c, 0, "mailto:".length)) === "mailto:") {
      var addr = _strSlice(c, "mailto:".length);
      if (_strIndexOf(addr, "?") !== -1) throw E("acme/bad-contact", "a mailto contact must not carry header fields (RFC 8555 sec. 7.3 / RFC 6068)");
      if (_strIndexOf(addr, ",") !== -1) throw E("acme/bad-contact", "a mailto contact must be a single addr-spec, not a comma list (RFC 8555 sec. 7.3)");
...
async function externalAccountBinding(o) {
  ...
  var alg = o.alg || "HS256";
  if (!_HMAC_HASH[alg]) throw E("acme/bad-input", "an EAB inner JWS must use an HS* MAC algorithm (sec. 7.3.4), not " + _stringify(alg));
  var key = o.macKey;
  if (_isBuffer(key)) {
...
function _validateOrderIdentifier(id) {
  ...
  if (type === "dns") {
    ...
    if (_strIndexOf(v, "*.") === 0) {
      v = _strSlice(v, 2);
      if (_strIndexOf(v, "*") !== -1) throw E("acme/bad-identifier", "a wildcard order identifier permits exactly one leading *. label (RFC 8555 sec. 7.1.3)");
    } else if (_strIndexOf(v, "*") !== -1) {
      throw E("acme/bad-identifier", "a wildcard must be a single leading *. label (RFC 8555 sec. 7.1.3)");
    }
    _assertDnsName(v);
  } else if (type === "ip") {
    if (_strIndexOf(value, "*") !== -1) throw E("acme/bad-identifier", "an ip identifier has no wildcard form (RFC 8738)");
    _assertIpAddress(value);
  }
...
function newOrder(o) {
  return _promised(function () {
  if (!_isObject(o) || !_isArray(o.identifiers) || o.identifiers.length === 0) throw E("acme/bad-order", "newOrder requires a non-empty identifiers array (RFC 8555 sec. 7.4)");
  ...
  var payload = { identifiers: _map(o.identifiers, _validateOrderIdentifier) };
...
async function _jwkToSpki(jwk) {
  ...
  else throw E("acme/bad-key", "unsupported account key type " + _stringify(jwk && jwk.kty));
  var key;
  ...
  return _bufferFrom(await subtle.exportKey("spki", key));
}
...
function _subjectCommonNames(subject) {
  var names = [];
  if (!subject || !_isArray(subject.rdns)) return names;
...
function _csrIdentifierSet(parsedCsr) {
  var set = {};
  _forEach(_subjectCommonNames(parsedCsr.subject), function (cn) { set["dns:" + _toLowerCase(cn)] = true; });
  ...
  (parsedCsr.attributes || []).forEach(function (a) {
    if (a.type !== oid.byName("extensionRequest") || !_isArray(a.extensions)) return;
    _forEach(a.extensions, function (e) {
      ...
      (dec.names || []).forEach(function (n) {
        if (n.tagNumber === 2) set["dns:" + _toLowerCase(_String(n.value))] = true;
...
function _orderIdentifierSet(identifiers) {
  ...
  _forEach(identifiers, function (id) {
    if (id.type === "dns") set["dns:" + _toLowerCase(id.value)] = true;
...
function _assertCsrIdentifiers(parsedCsr, identifiers) {
  ...
  var haveKeys = _keys(have), wantKeys = _keys(want);
  var mismatch = haveKeys.length !== wantKeys.length ||
    _some(wantKeys, function (k) { return !have[k]; }) ||
    _some(haveKeys, function (k) { return !want[k]; });
  if (mismatch) {
    throw E("acme/csr-identifier-mismatch", "the finalize CSR identifier set " + _stringify(_sort(haveKeys)) +
      " does not equal the order identifiers " + _stringify(_sort(wantKeys)) + " (RFC 8555 sec. 7.4)");
  }
}
...
async function finalize(o) {
  if (!_isObject(o) || !_isBuffer(o.csr)) throw E("acme/bad-input", "finalize requires a DER CSR Buffer (opts.csr)");
  ...
  if (o.accountJwk !== undefined) {
    ...
    if (parsed.subjectPublicKeyInfo && _isBuffer(parsed.subjectPublicKeyInfo.bytes) &&
        _bufEquals(parsed.subjectPublicKeyInfo.bytes, accountSpki)) {
  ...
  if (o.identifiers !== undefined) {
    if (!_isArray(o.identifiers) || o.identifiers.length === 0) throw E("acme/bad-input", "finalize identifiers must be the non-empty order identifier array");
    _forEach(o.identifiers, _validateOrderIdentifier);   // caller-supplied -> validate before comparing (no raw TypeError)
...
function revokeCert(o) {
  return _promised(function () {
  if (!_isObject(o) || !_isBuffer(o.certificate)) throw E("acme/bad-input", "revokeCert requires a DER certificate Buffer (opts.certificate)");
  ...
  if (o.reason !== undefined) {
    if (typeof o.reason !== "number" || !_isFinite(o.reason) || _floor(o.reason) !== o.reason || _indexOf(CRL_REASONS, o.reason) === -1) {
...
function ariCertId(certDer) {
  if (!_isBuffer(certDer)) throw E("acme/bad-input", "ariCertId requires a DER certificate Buffer");
  var cert = x509.parse(certDer);
  ...
  if (!aki || !_isBuffer(aki.keyIdentifier)) throw E("acme/bad-certid", "the authorityKeyIdentifier has no keyIdentifier field (RFC 9773 sec. 4.1)");
  var serialBytes = _bufferFrom(cert.serialNumberHex, "hex");   // DER INTEGER content; sign-pad preserved
...
function _clientUrl(urlStr) {
  ...
  if (((/^[A-Za-z][A-Za-z0-9+.-]*:\/\/([^/?#]*)/.exec(s) || [])[1] || "").indexOf("@") !== -1) throw E("acme/bad-url", "an ACME URL must not contain userinfo: " + _stringify(s));
  ...
  if (/[\s\\]/.test(s)) throw E("acme/bad-url", "an ACME URL must not contain whitespace or a backslash: " + _stringify(s));
  ...
  if (_uriStructurallyInvalid(s)) throw E("acme/bad-url", "an ACME URL authority is not canonical (the transport would repair it): " + _stringify(s));
  ...
  if (_auth) {
    ...
    if (_rawHost.toLowerCase() !== url.hostname) throw E("acme/bad-url", "an ACME URL host is not canonical (the transport would rewrite it): " + _stringify(s));
  }
  ...
  var rawPath = _split(_replace(s, /^[a-z]+:\/\/[^/?#]*/i, ""), /[?#]/)[0];
  if (rawPath !== url.pathname) throw E("acme/bad-url", "an ACME URL path is not canonical (the transport would normalize it): " + _stringify(s));
  ...
  var qi = _strIndexOf(s, "?");
  var rawQuery = qi === -1 ? "" : _split(_strSlice(s, qi), "#")[0];
  if (rawQuery !== url.search) throw E("acme/bad-url", "an ACME URL query is not canonical (the transport would normalize it): " + _stringify(s));
  ...
  if (_strIndexOf(s, "#") !== -1) throw E("acme/bad-url", "an ACME URL must not contain a fragment: " + _stringify(s));
...
function _resolveLocation(loc, base) {
  ...
  catch (e) { throw E("acme/bad-url", "the Location header did not resolve to a valid URL: " + _stringify(loc), e); }
  ...
  if (_queryRepaired(loc, u)) throw E("acme/bad-url", "a relative Link/Location query is not canonical (resolution re-encoded it): " + _stringify(loc));
...
function _queryRepaired(rawRef, resolvedUrl) {
  var qi = _strIndexOf(rawRef, "?");
  return qi !== -1 && _split(_strSlice(rawRef, qi), "#")[0] !== resolvedUrl.search;
}
...
function _splitPemChain(text) {
  ...
  while ((m = _reExec(re, text)) !== null) {
    ...
    if (_trim(_strSlice(text, lastEnd, m.index)) !== "") throw E("acme/bad-certificate-chain", "the certificate chain contained non-whitespace text outside a PEM block (RFC 8555 sec. 7.4.2)");
    var der;
  ...
  if (_trim(_strSlice(text, lastEnd)) !== "") throw E("acme/bad-certificate-chain", "the certificate chain contained trailing non-whitespace text outside a PEM block (RFC 8555 sec. 7.4.2)");
...
function _trimOWS(s) { return _replace(_replace(s, /^[ \t]+/, ""), /[ \t]+$/, ""); }
...
function _validRelType(t) {
  ...
  if (_reTest(LINK_URI_BADCHAR, t)) return false;
...
function _validAbsoluteUri(t) {
  return /^[A-Za-z][A-Za-z0-9+.-]*:/.test(t) && !_reTest(LINK_URI_BADPCT, t) && !_uriStructurallyInvalid(t);
}
...
function _multiFragment(uri) { return _split(uri, "#").length > 2; }
...
function _hasEmptyAuthority(uri) {
  ...
  if (_charAt(host, 0) !== "[") { var ci = _strIndexOf(host, ":"); if (ci !== -1) host = _strSlice(host, 0, ci); }
  return host === "";
}
...
function _bracketsOnlyInAuthority(uri) {
  if (_strIndexOf(uri, "[") === -1 && _strIndexOf(uri, "]") === -1) return true;
  ...
  var m = /^(?:[A-Za-z][A-Za-z0-9+.-]*:)?\/\/(?:[^/?#@[\]]*@)?\[[^[\]]*\]/.exec(uri);
  if (!m) return false;
  var rest = _strSlice(uri, m[0].length);
  if (_strIndexOf(rest, "[") !== -1 || _strIndexOf(rest, "]") !== -1) return false;   // no bracket past the authority
...
function _relFirstSegHasColon(uri) {
  if (/^[A-Za-z][A-Za-z0-9+.-]*:/.test(uri) || _strIndexOf(uri, "//") === 0) return false;
  return _strIndexOf(_split(uri, /[/?#]/)[0], ":") !== -1;
}
...
function _dedupKey(href) {
  return _replace(href, /%[0-9A-Fa-f]{2}/g, function (m) {
    var c = _fromCharCode(_parseInt(_strSlice(m, 1), 16));
    return /[A-Za-z0-9._~-]/.test(c) ? c : "%" + _strSlice(m, 1).toUpperCase();
  });
}
...
function _hasEncodedDotSegment(uri) {
  var segs = _split(_split(uri, /[?#]/)[0], "/");
  for (var _si = 0; _si < segs.length; _si++) {
    if (!_reTest(/%2e/i, segs[_si])) continue;
...
function _linkUriInvalid(uri) {
  ...
  return _reTest(LINK_URI_BADCHAR, uri) || _reTest(LINK_URI_BADPCT, uri) || _hasEncodedDotSegment(uri) || _uriStructurallyInvalid(uri);
}
...
function _hasCtlOctet(s) {
  for (var _ci = 0; _ci < s.length; _ci++) { var _cc = _charCodeAt(s, _ci); if (_cc === 0x7F || (_cc < 0x20 && _cc !== 0x09)) return true; }
  return false;
}
...
function _splitLinkParams(rest) {
  ...
  for (var i = 0; i < rest.length; i++) {
    var ch = _charAt(rest, i);
    if (inQuote) { if (ch === "\\") { i++; continue; } if (ch === "\"") inQuote = false; continue; }
    if (ch === "\"") inQuote = true;
    else if (ch === ";") { _push(parts, _strSlice(rest, start, i)); start = i + 1; }
  }
  _push(parts, _strSlice(rest, start));
  var out = [];
  for (var j = 0; j < parts.length; j++) {
    ...
    if (p === "") { if (j === 0) continue; throw E("acme/bad-link", "an empty Link parameter (a ';' with no parameter) is malformed (RFC 8288): " + _stringify(rest)); }
    var eq = _strIndexOf(p, "=");
    var name = _toLowerCase(eq === -1 ? p : _trimOWS(_strSlice(p, 0, eq)));
    ...
    if (!_reTest(LINK_TOKEN, name)) throw E("acme/bad-link", "a Link parameter name must be a token (RFC 8288 / RFC 7230): " + _stringify(p));
    ...
    if (eq === -1) { _push(out, { name: name, value: "", hasValue: false }); continue; }
    var v = _trimOWS(_strSlice(p, eq + 1));
    if (v.length >= 2 && _charAt(v, 0) === "\"" && _charAt(v, v.length - 1) === "\"") {
      var inner = _strSlice(v, 1, -1);   // a quoted-string may hold a space-separated list (RFC 8288)
      if (!_reTest(LINK_QUOTED_INTERIOR, inner)) throw E("acme/bad-link", "a quoted Link parameter value is malformed (an unescaped quote or dangling backslash, RFC 7230): " + _stringify(p));
      v = _replace(inner, /\\(.)/g, "$1");
    } else if (!_reTest(LINK_TOKEN, v)) {
      ...
      throw E("acme/bad-link", "an unquoted Link parameter value must be a non-empty token (RFC 8288 / RFC 7230): " + _stringify(p));
    }
    _push(out, { name: name, value: v, hasValue: true });
  }
  return out;
}
...
function _parseLinkValue(raw) {
  var s = _trimOWS(raw);
  if (s === "") return null;
  var gt = _strIndexOf(s, ">");
  if (_charAt(s, 0) !== "<" || gt === -1) throw E("acme/bad-link", "a Link value must be <URI-Reference> with parameters (RFC 8288): " + _stringify(raw));
  ...
  var rest = _replace(_strSlice(s, gt + 1), /^[ \t]+/, "");
  if (rest !== "" && _charAt(rest, 0) !== ";") throw E("acme/bad-link", "a Link value's parameters must be introduced by ';' (RFC 8288): " + _stringify(raw));
  var uri = _strSlice(s, 1, gt);
  ...
  if (_linkUriInvalid(uri)) throw E("acme/bad-link", "a Link URI-Reference contains a character, percent-escape, or encoded dot-segment not permitted by RFC 3986: " + _stringify(raw));
  ...
  for (var i = 0; i < params.length; i++) {
    ...
    if (params[i].name === "rel" && !relSeen) { rel = params[i].value; relSeen = true; }
      ...
      if (anchorSeen) throw E("acme/bad-link", "a Link value must not carry more than one anchor parameter (RFC 8288 sec. 3.2, ambiguous context): " + _stringify(raw));
      anchorSeen = true;
      if (!params[i].hasValue) throw E("acme/bad-link", "a Link anchor parameter requires a URI value (RFC 8288 sec. 3.2): " + _stringify(raw));
...
function _parseLinkAlternates(headers, base) {
  var raw = null;
  for (var k in headers) { if (_hasOwn(headers, k) && _toLowerCase(k) === "link") { raw = headers[k]; break; } }
  if (raw == null) return [];
  var baseUrl = new URL(base), baseOrigin = baseUrl.origin, baseHref = baseUrl.href;   // the download URL
  var fields = _isArray(raw) ? raw : [raw], out = [], seen = _create(null), totalBytes = 0;
  ...
  for (var fi = 0; fi < fields.length; fi++) {
    ...
    for (var vi = 0; vi < values.length; vi++) {
      ...
      if (lv.rel !== "" && (_charAt(lv.rel, 0) === " " || _charAt(lv.rel, lv.rel.length - 1) === " ")) throw E("acme/bad-link", "a Link rel value has a leading or trailing space (RFC 8288 sec. 3.3): " + _stringify(lv.rel));
      ...
      var relToks = lv.rel === "" ? [] : _split(lv.rel, " ");
      for (var ri = 0; ri < relToks.length; ri++) { if (relToks[ri] !== "" && !_validRelType(relToks[ri])) throw E("acme/bad-link", "a Link rel value contains a token that is not a valid relation-type (RFC 8288 sec. 3.3): " + _stringify(lv.rel)); }
      ...
      try { resolved = _resolveLocation(lv.uri, base); }   // resolves a relative ref + enforces the https gate
        ...
        if (e && e.code === "acme/insecure-url") throw E("acme/bad-link", "a rel=\"alternate\" Link target is not an https URL: " + _stringify(lv.uri), e);
        continue;
      }
      ...
      if (parsed.origin !== baseOrigin) throw E("acme/bad-link", "a rel=\"alternate\" Link target is not on the certificate download's origin (SSRF guard): " + _stringify(lv.uri));
      ...
      if (!seen[key]) { seen[key] = true; _push(out, resolved); }
    }
  }
  return out;
}
...
function client(directoryUrl, opts) {
  ...
  if (!transport) {
    var t0 = opts.tls || {};
    var hasAnchors = t0.anchors !== undefined && t0.anchors !== null && !(_isArray(t0.anchors) && t0.anchors.length === 0);
  ...
  function _bodyText(res) { return _isBuffer(res.body) ? _bufToString(res.body, "utf8") : _String(res.body || ""); }
  ...
  function _jsonInput(res) { return _isBuffer(res.body) ? res.body : _bufferFrom(_String(res.body == null ? "" : res.body), "utf8"); }
  ...
  function _isProblem(res) { return _reTest(/application\/problem\+json/i, _String(res.headers["content-type"] || "")); }
  ...
  function _json(res) {
    ...
    catch (e) { throw E("acme/bad-response", "the response body was not valid JSON: " + ((e && e.message) || _String(e)), e); }
  }
  ...
  function _sendFollowing(method, url, headers, body, redirects) {
    var reqHeaders = _assign({ "user-agent": CLIENT_USER_AGENT }, headers || {});
    ...
    return transport({ method: method, url: url, headers: reqHeaders, body: body, tls: _tlsFor(url), timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      res = res || {};
      var h = {};
      _forEach(_keys(res.headers || {}), function (k) { h[_toLowerCase(k)] = res.headers[k]; });
      ...
      var blen = _isBuffer(res.body) ? guard.bytes.lengthOf(res.body) : _byteLength(_String(res.body == null ? "" : res.body), "utf8");
  ...
  function _getDirectory() {
    if (dirCache) return _resolve(_Promise, dirCache);
  ...
  function _takeNonce() {
    if (nonces.length) return _resolve(_Promise, nonces.pop());
  ...
  function _postJws(url, makeJws, accept, okStatuses) {
    ...
    function once() {
      return _takeNonce().then(makeJws).then(function (jws) {
        return _send("POST", url, { "content-type": "application/jose+json", "accept": accept || "application/json" }, _stringify(jws));
      }).then(function (res) {
        ...
        if (_indexOf(ok, res.status) === -1) throw E("acme/unexpected-status", "an ACME POST returned an unexpected status " + res.status + " (expected " + _join(ok, "/") + ", RFC 8555)");
        return res;
      });
    }
    return once();
  ...
  function _post(url, builder, extra, mode, accept, okStatuses) {
    return _postJws(url, function (nonce) {
      ...
      var base = _assign({}, extra || {}, { key: accountKey, alg: alg, nonce: nonce, url: url });
  ...
  function _newAuthz(identifier) {
    return _resolve(_Promise).then(function () {
    ...
    return _resource("newAuthz").then(function (url) {
      ...
      return _post(url, newAuthz, { identifier: canon }, "kid", undefined, [200, 201]).then(function (res) {
        ...
        if (authz.status !== "pending" && authz.status !== "valid") throw E("acme/unexpected-authorization-status", "newAuthz returned an authorization that is neither pending nor already valid (status " + _stringify(authz.status) + "); RFC 8555 sec. 7.4.1");
  ...
  function _finalizeBody(order, o) {
    ...
    if (!_isArray(order.identifiers)) throw E("acme/bad-input", "finalize requires the order's identifiers to enforce the RFC 8555 sec. 7.4 CSR-set match");
  ...
  function _poll(kind, url, budget) {
    ...
    function step() {
      ...
      return _postAsGet(url).then(function (res) {
        ...
        var delaySec = 1;
        if (typeof ra === "string" && _trim(ra) !== "") { delaySec = retryAfter.parse(ra, { now: clock(), E: E, code: "acme/bad-retry-after" }).retryAfterSeconds; if (delaySec == null) delaySec = 1; }
        waited += delaySec;
        ...
        return _resolve(_Promise, sleep(delaySec * constants.TIME.seconds(1))).then(step);
      });
    }
    return step();
  }
  ...
  function _readCertChain(res) {
    ...
    var ctToken = _toLowerCase(_trim(_split(_String(res.headers["content-type"] || ""), ";")[0]));
    if (ctToken !== "application/pem-certificate-chain") throw E("acme/bad-certificate-chain", "the certificate download returned an unexpected media type " + _stringify(ctToken) + " (expected application/pem-certificate-chain, RFC 8555 sec. 7.4.2)");
    var chain = _splitPemChain(_bodyText(res));
    return { certificate: chain[0], chain: _arraySlice(chain, 1), certificates: chain };
  }
  ...
  function _downloadCertificate(url, opts) {
    ...
    return _resolve(_Promise).then(function () {
      ...
      return _post(dlUrl, postAsGet, null, "kid", "application/pem-certificate-chain").then(function (res) {
        ...
        return _resolve(_Promise, select(primary)).then(function (primaryMatch) {
          ...
          var leaf = primary.certificate, toFetch = _arraySlice(alternates, 0, maxAlt), overBudget = alternates.length > maxAlt, idx = 0;
          function next() {
            ...
            return _fetchCertChain(toFetch[idx++]).then(function (cand) {
              if (!_bufEquals(cand.certificate, leaf)) throw E("acme/bad-alternate", "an alternate chain did not start with the same end-entity certificate (RFC 8555 sec. 7.4.2)");
              return _resolve(_Promise, select(cand)).then(function (m) { return m ? result(cand) : next(); });
            });
          }
          return next();
        });
  ...
  function _revokeCert(o) {
    ...
    if (!_isBuffer(o.certificate)) return _reject(_Promise, E("acme/bad-input", "revokeCert requires a DER certificate Buffer (opts.certificate)"));
    ...
    if (certKeyMode && !(o.certKey != null && _isObject(o.certJwk) && _isString(o.certAlg))) return _reject(_Promise, E("acme/bad-input", "revokeCert certificate-key mode requires certKey, certJwk, AND certAlg together, or none (RFC 8555 sec. 7.6)"));
    ...
    return _resource("revokeCert").then(function (url) {
      if (certKeyMode) {
        ...
        return _postJws(url, function (nonce) {
          return revokeCert(_assign({}, extra, { key: o.certKey, alg: o.certAlg, nonce: nonce, url: url, jwk: o.certJwk }));
  ...
  function _renewalInfoBody(certDer, clockFn, retryAfterCapSeconds) {
    if (!_isBuffer(certDer)) throw E("acme/bad-input", "renewalInfo requires a DER certificate Buffer");
    ...
    function clk() { var t = _rawClk(); if (typeof t !== "number" || !_isFinite(t)) throw E("acme/bad-input", "the renewalInfo clock returned a non-finite value"); return t; }
    ...
    return _resource("renewalInfo").then(function (base) {
      ...
      var u = new URL(base);
      u.pathname = _replace(u.pathname, /\/+$/, "") + "/" + certId;
  ...
  function _renewalWindow(certDer, o) {
    o = o || {};
    return _resolve(_Promise).then(function () {
      if (!_isBuffer(certDer)) throw E("acme/bad-input", "renewalWindow requires a DER certificate Buffer");
      ...
      function clk() { var t = _clk(); if (typeof t !== "number" || !_isFinite(t)) throw E("acme/bad-input", "the renewalWindow clock returned a non-finite value"); return t; }
      ...
      return _renewalInfo(certDer, clk, RENEWAL_RETRY_MAX_SECONDS).then(function (ri) {
        ...
        var effEnd = _min(endMs, notAfterMs), effStart = _min(startMs, effEnd);
        ...
        if (pw && pw.start === w.start && pw.end === w.end && _isString(o.previous.selectedTime)) {
          ...
          if (!_isFinite(prevMs)) throw E("acme/bad-input", "renewalWindow opts.previous.selectedTime must be an RFC 3339 date-time");
          selectedMs = _max(effStart, _min(effEnd, prevMs));
        } else {
        ...
        var retryAfterSeconds = ri.retryAfterSeconds == null ? RENEWAL_RETRY_DEFAULT_SECONDS :
          _max(RENEWAL_RETRY_MIN_SECONDS, _min(RENEWAL_RETRY_MAX_SECONDS, ri.retryAfterSeconds));
        return {
          suggestedWindow: w,
          selectedTime: new _Date(selectedMs).toISOString(),
