...
function _splitPemChain(text) {
  ...
  while ((m = re.exec(text)) !== null) {
    try { out.push(x509.pemDecode(m[0], "CERTIFICATE")); }
    catch (e) { throw E("acme/bad-certificate-chain", "a downloaded certificate did not decode", e); }
  }
 ...
 * typed `acme/server-problem`; a poll surfaces Retry-After (bounded) and is capped. Returns a client
 ...
 *   - `maxPolls` / `maxTotalWait` / `sleep` -- poll-loop budgets + an injectable sleeper; `now` -- receipt time for a Retry-After HTTP-date.
 * @example
...
function client(directoryUrl, opts) {
  ...
  var sleep = typeof opts.sleep === "function" ? opts.sleep : function (ms) { return new Promise(function (r) { setTimeout(r, ms); }); };
  ...
  function _serverProblem(res) {
    var prob = null;
    try { prob = jose.parseJson(_bodyText(res)); if (_isObject(prob)) validateProblem(prob); }
  ...
  function _json(res) {
    try { return jose.parseJson(_bodyText(res)); }
  ...
  // response (validated base64url, RFC 8555 sec. 6.5.1). No redirect/HTTP-auth loop: ACME URLs are absolute.
  // Every request carries the RFC 8555 sec. 6.1 REQUIRED User-Agent (caller-overridable).
  function _send(method, url, headers, body) {
    var reqHeaders = Object.assign({ "user-agent": CLIENT_USER_AGENT }, headers || {});
    return transport({ method: method, url: url, headers: reqHeaders, body: body, tls: budgets.tls, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      res = res || {};
      var h = {};
  ...
  function _takeNonce() {
    ...
    return _resource("newNonce").then(function (nn) { return _send("HEAD", nn, {}, null); }).then(function () {
  ...
  function _postJws(url, makeJws, accept) {
    var attempt = 0;
    function once() {
      return _takeNonce().then(makeJws).then(function (jws) {
        ...
        if (res.status >= 300) {
          if (_isProblem(res)) {
            var prob = null;
            try { prob = jose.parseJson(_bodyText(res)); }
  ...
  function _post(url, builder, extra, mode, accept) {
    return _postJws(url, function (nonce) {
      var base = Object.assign({ key: accountKey, alg: alg, nonce: nonce, url: url }, extra || {});
  ...
  function _newAccount(payloadOpts) {
    return _resource("newAccount").then(function (url) {
      return _post(url, newAccount, payloadOpts || {}, "jwk").then(function (res) {
  ...
  function _finalize(order, o) {
    ...
    // Default the identifier set to the order's own authoritative identifiers (the client holds the
    // validated order), so the sec. 7.4 CSR-vs-order check ALWAYS runs -- never skipped when the caller
    // omits identifiers. A caller may still narrow via o.identifiers.
    return _post(_clientUrl(order.finalize), finalize, { csr: o.csr, identifiers: (o.identifiers || order.identifiers), accountJwk: accountJwk }, "kid").then(function (res) { return validate("order", _json(res)); });
  }
  ...
  function _poll(kind, url, budget) {
    budget = budget || {};
    var capPolls = budget.maxPolls != null ? budget.maxPolls : maxPolls;
    var capWait = budget.maxTotalWait != null ? budget.maxTotalWait : maxTotalWait;
    var terminal = kind === "order" ? { valid: true, invalid: true } : { valid: true, invalid: true, deactivated: true, expired: true, revoked: true };
    url = _clientUrl(url);
    ...
    function step() {
      ...
      return _postAsGet(url).then(function (res) {
        ...
        var delaySec = 1;
        if (typeof ra === "string" && ra.trim() !== "") { delaySec = retryAfter.parse(ra, { now: now, E: E, code: "acme/bad-retry-after" }).retryAfterSeconds; if (delaySec == null) delaySec = 1; }
        waited += delaySec;
  ...
  function _revokeCert(o) {
    ...
    return _resource("revokeCert").then(function (url) {
      if (o.certKey && _isObject(o.certJwk)) {
        ...
        return _postJws(url, function (nonce) {
          return revokeCert(Object.assign({ key: o.certKey, alg: o.certAlg || alg, nonce: nonce, url: url, jwk: o.certJwk }, extra));
  ...
  function _keyChange(o) {
    ...
    return _resource("keyChange").then(function (url) {
      return _post(url, keyChange, { account: account, oldKey: accountJwk, newKey: o.newKey, newJwk: o.newJwk, newAlg: o.newAlg }, "kid").then(function (res) {
        // rotate the session's account key on success (subsequent requests sign with the new key).
        accountKey = o.newKey; accountJwk = o.newJwk; alg = o.newAlg;
        return { account: (_isProblem(res) ? null : (res.status < 300 ? _tryJson(res) : null)), url: account };
      });
    });
  }
  function _tryJson(res) {
    try { return jose.parseJson(_bodyText(res)); }
  ...
  function _renewalInfo(certDer) {
    ...
    return _resource("renewalInfo").then(function (base) {
      var url = _clientUrl(base.replace(/\/+$/, "") + "/" + certId);
      return _send("GET", url, { accept: "application/json" }, null).then(function (res) {
        ...
        if (typeof ra === "string" && ra.trim() !== "") retryAfterSeconds = retryAfter.parse(ra, { now: now, E: E, code: "acme/bad-retry-after" }).retryAfterSeconds;
