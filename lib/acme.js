...
function _csrIdentifierSet(parsedCsr) {
  ...
  // The subject common name only where the request asserts no alternative name, the same rule the
  // issued-certificate set applies -- the two halves of the exchange read a name the same way or the
...
function _addCommonName(set, cn, what) {
  ...
  if (looksIp) {
    // Only the canonical form is an ip identifier. A near-miss spelling is not one, and is left out
    // rather than normalized into one -- the rule the ip identifier validator already applies.
    try { _assertIpAddress(cn); } catch (_e) { return; }
    set["ip:" + cn] = true;
    return;
  }
  ...
  // A common name that is not a dns name is not an identifier this comparison can express: an
  // organizational common name is ordinary, and RFC 6125 has deprecated reading a common name as a
  // name at all. Leaving it out can only make the certificate's set SMALLER, and set equality with the
  // order then fails rather than passes, so this cannot turn a mismatch into an acceptance the way
  // dropping an ORDER identifier could. The names a certificate actually asserts are its
  // subjectAltName entries, and every one of those is accounted for.
  ...
  if (intrinsic.stringIndexOf(base, "*") !== -1) return;
  try { _assertDnsName(base); } catch (_e2) { return; }
  set["dns:" + folded] = true;
}
