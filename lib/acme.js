...
function _validAbsoluteUri(t) {
  if (!/^[A-Za-z][A-Za-z0-9+.-]*:/.test(t) || LINK_URI_BADPCT.test(t) || !_bracketsOnlyInAuthority(t) || _authorityMultiAt(t)) return false;
...
function _linkUriInvalid(uri) {
  ...
  return LINK_URI_BADCHAR.test(uri) || LINK_URI_BADPCT.test(uri) || _hasEncodedDotSegment(uri) || !_bracketsOnlyInAuthority(uri) || _hasEmptyAuthority(uri) || _authorityMultiAt(uri);
}
