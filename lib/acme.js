...
function _parseLinkValue(raw) {
  ...
  return { uri: s.slice(1, gt), rel: rel };
}
...
function _parseLinkAlternates(headers, base) {
  ...
  for (var fi = 0; fi < fields.length; fi++) {
    ...
    // parse work (and the collected alternate set) cannot amplify regardless of the header's shape.
  ...
  function _downloadCertificate(url, opts) {
    ...
    return Promise.resolve().then(function () {
      ...
      return _post(dlUrl, postAsGet, null, "kid", "application/pem-certificate-chain").then(function (res) {
        ...
        // The Link header is UNTRUSTED. When the caller is selecting, a malformed header is fatal (the fetch
        // plan is built from it); when the caller only wants the primary, a malformed header is ignored so a
        // broken or hostile Link can never deny a valid primary download.
        var alternates;
        try { alternates = _parseLinkAlternates(res.headers, dlUrl); }
        catch (e) { if (select) throw e; alternates = []; }
  ...
  function _renewalWindow(certDer, o) {
    ...
    return Promise.resolve().then(function () {
      ...
      return _renewalInfo(certDer, clk, RENEWAL_RETRY_MAX_SECONDS).then(function (ri) {
        ...
        // RFC 9773 sec. 4.2: select a uniform-random instant across the window to spread renewals; if it is
        // already in the past (the window opened before now), the caller should renew immediately.
        var selectedMs = Math.round(startMs + draw * (endMs - startMs));
        ...
        return {
          ...
          // instant, so the pre-fetch `now` (used above only for the expiry gate) could be stale here.
          renewNow: selectedMs <= clk(),
