...
function _parseLinkAlternates(headers, base) {
  ...
  for (var fi = 0; fi < fields.length; fi++) {
    ...
    for (var vi = 0; vi < values.length; vi++) {
      ...
      // Dedup by the NORMALIZED href (WHATWG lowercases the host and drops a default :443), so two equivalent
      // spellings ("acme.example" vs "ACME.EXAMPLE:443") collapse to one fetch (CWE-770 amplification) -- while
      // the VERBATIM resolved string is what is fetched + signed (RFC 8555 sec. 6.4 exact-URL match).
      if (!seen[parsed.href]) { seen[parsed.href] = true; out.push(resolved); }
    }
  }
  return out;
}
