...
function _validRelType(t) {
  ...
  // ext-rel-type: a scheme prefix, canonical RFC 3986 characters, AND a URL that actually PARSES (so a token
  // with only-valid characters but a broken structure -- e.g. "http://[" -- is rejected, not accepted).
  if (!/^[A-Za-z][A-Za-z0-9+.-]*:/.test(t) || _linkUriInvalid(t)) return false;
  try { new URL(t); return true; } catch (_e) { return false; }
}
