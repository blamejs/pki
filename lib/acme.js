...
function _clientUrl(urlStr) {
  ...
  // (RFC 8555 sec. 6.4); refuse it rather than sign a credential-bearing URL that steers nothing.
  if (url.username !== "" || url.password !== "") throw E("acme/bad-url", "an ACME URL must not contain userinfo: " + JSON.stringify(s));
...
function _bracketsOnlyInAuthority(uri) {
  ...
  // After the IP-literal HOST "]", only a port (":"), a path/query/fragment delimiter, or end may follow -- an "@"
  // or other authority text ("[::1]@host") means the literal sits in an invalid position (userinfo cannot be
  // bracketed, and a host cannot be followed by more authority), which WHATWG would re-parse to a different host.
  return rest === "" || rest.charAt(0) === ":" || rest.charAt(0) === "/" || rest.charAt(0) === "?" || rest.charAt(0) === "#";
}
