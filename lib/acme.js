...
var SPECS = {
  ...
  account: [
    ...
    { name: "contact", type: "array", elemType: "url" },
  ...
  order: [
    ...
    { name: "identifiers", type: "array", required: true, minItems: 1, elemType: "identifier" },
...
function _validate(kind, obj) {
  if (!_isObject(obj)) throw E("acme/bad-" + kind, "an ACME " + kind + " must be a JSON object");
  var spec = SPECS[kind];
  for (var f = 0; f < spec.length; f++) {
    ...
    if (err) throw E("acme/bad-" + kind, "the " + kind + " field " + JSON.stringify(field.name) + " " + err);
  }
  return obj;
}
...
function assertTransition(kind, from, to) {
  ...
  if (from === to) return;
  if (to === "invalid" && kind !== "authorization") return; // any state -> invalid (order/challenge)
