...
function _clientUrl(urlStr) {
  ...
  // Reject dot-segments in the path (`/.` or `/..`): the transport normalizes them away when it builds the
  // request target, so the signed JWS `url` would keep a path the request never used. (The authority may
  // still normalize -- a default :443 port, an uppercase host -- because that does not change the path.)
  var rawPath = s.replace(/^[a-z]+:\/\/[^/?#]*/i, "").split(/[?#]/)[0];
  if (/(?:^|\/)\.\.?(?:\/|$)/.test(rawPath)) throw E("acme/bad-url", "an ACME URL path must not contain dot-segments: " + JSON.stringify(s));
