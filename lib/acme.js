  ...
  function _keyChange(o) {
    ...
    return _resource("keyChange").then(function (url) {
      return _post(url, keyChange, { account: account, oldKey: accountJwk, newKey: o.newKey, newJwk: o.newJwk, newAlg: o.newAlg }, "kid").then(function (res) {
        // RFC 8555 sec. 7.3.5: the 200 status IS the success (the server has rotated the key). An account
        // object body is OPTIONAL -- validate it when present so a non-empty NON-account body still fails
        // closed BEFORE rotating, but a bodyless 200 is a valid rollover, so rotate rather than throwing
        // and desynchronizing from the server (which would then hold the new key while the client keeps
        // the old one).
        var acct = (_bodyText(res).trim() !== "") ? validate("account", _json(res)) : null;
