...
function _assertContacts(contacts) {
  ...
  // An own-index loop, NOT forEach: forEach SKIPS a hole in a sparse array (`new Array(1)`), so the hole
  // would never be validated and JSON serialization would sign `{"contact":[null]}`. Every index is
  // read through an own-property check, so a sparse hole -- or a nullish entry -- is a typed
  // acme/bad-contact rather than a silently-signed null.
...
async function finalize(o) {
  ...
  if (o.identifiers !== undefined) {
    ...
    // An own-index loop, NOT forEach: forEach skips a hole in a sparse array, leaving it unvalidated.
    for (var ii = 0; ii < o.identifiers.length; ii++) {
      ...
      _validateOrderIdentifier(o.identifiers[ii]);   // caller-supplied -> validate before comparing (no raw TypeError)
    }
...
function _parseLinkRelation(headers, base, relName, opts) {
  ...
  if (raw == null) return [];
  var baseUrl = new URL(base), baseOrigin = baseUrl.origin, baseHref = baseUrl.href;   // the request URL
  ...
  // The base URL is not a relation OF itself -- a certificate advertised as its own rel="alternate" (RFC 8555
  // sec. 7.4.2), or an orders page advertised as its own rel="next" -- so it is excluded from the followable
  // set below and never causes a redundant re-fetch. It is still COUNTED toward the singleton test, so a
  // self-link plus a distinct target is the two distinct targets RFC 8288 sec. 3.3 makes ambiguous, rather
  // than one target with the self-link silently dropped.
  for (var fi = 0; fi < fields.length; fi++) {
    ...
    for (var vi = 0; vi < values.length; vi++) {
      ...
      try { resolved = _resolveLocation(lv.uri, base); }   // resolves a relative ref + enforces the https gate
        ...
        // A non-https target is a security-relevant anomaly -> fail the whole header closed. A target that is a
        ...
        // (RFC 8555 sec. 6.4), so it is UNUSABLE. For a MULTI-target relation (alternate) it is skipped and the
        // others are kept (sec. 7.4.2 permits several), rather than discarding every valid alternate over one
        // non-canonical spelling. For a SINGLETON relation (next) there is only the one target, so skipping it
        // would silently END pagination and omit orders -- an unusable singleton fails closed instead.
      ...
      if (seen[key]) continue;                  // an equivalent spelling of this target already counted
      seen[key] = true;
      distinctCount += 1;                        // every distinct matched target, INCLUDING a self-link
      if (key !== baseKey) out.push(resolved);   // the base is not followable, but it still counted above
    }
  }
...
// Validate + snapshot opts.resignKeys (config-time, fail closed). Absent -> null (the feature is OFF and
// a badSignatureAlgorithm surfaces to the caller unchanged). Present -> a non-empty array of { alg, key }
// where alg is a JWS alg string and key is a CryptoKey; each pair is read ONCE into a frozen copy so a
// caller getter cannot rewrite it between this check and the re-sign, and the caller's order is preserved.
function _resignKeysOf(raw) {
  ...
  for (var i = 0; i < raw.length; i++) {
    ...
    // Read each field ONCE into the frozen snapshot: a caller getter cannot then answer the validation
    // and the re-sign with different values (the alg checked is the alg signed, the key checked is signed).
    var a = e.alg, k = e.key;
    ...
    out[i] = intrinsic.freeze({ alg: a, key: k });   // index-write, not a live-prototype .push
  }
  return intrinsic.freeze(out);
}
// Pick the re-sign entry to use for a badSignatureAlgorithm: the FIRST caller-listed entry whose alg the
// server advertised as supported. Caller order selects and the server's array only filters, so a bogus
// badSignatureAlgorithm cannot steer the client to the server's weakest alg (RFC 8555 sec. 10). Returns
// null when no caller entry matches (the badSignatureAlgorithm then surfaces, fail closed).
...
// A copy of a public JWK with its optional `alg` member set to the alg actually being signed. A
// WebCrypto-exported RSA JWK carries the key's native alg (e.g. RS256), so embedding it unchanged under a
// re-sign alg (PS256) would advertise a JWK whose declared alg contradicts the JWS header, which a strict
// ACME server may reject. Own members are copied through captured intrinsics (no live prototype read).
...
function client(directoryUrl, opts) {
  ...
  // Optional badSignatureAlgorithm re-sign material (RFC 8555 sec. 6.2). Each entry pairs an alternative
  // JWS alg with a CryptoKey for the SAME account key material under that alg (an RSA public JWK is
  // padding-agnostic, so the registered account key still verifies a re-signed request). Snapshotted here
  // so a caller getter cannot rewrite the pairs between validation and use, and ordered by CALLER
  // preference: on a badSignatureAlgorithm the caller's order selects, never the server's (sec. 10).
  ...
  function _bodyText(res) {
    ...
    // A BufferSource (Uint8Array, ArrayBuffer) from an injected transport: read its raw bytes rather than
    // String()-coercing it into its "123,45,..." toString.
  ...
  function _jsonInput(res) {
    ...
    // A BufferSource (Uint8Array, ArrayBuffer) from an injected transport: normalize its raw bytes so
    // jose.parseJson runs its fatal UTF-8 validation on the wire body, not on its "123,45,..." toString.
      ...
      return _takeNonce().then(function (nonce) { return makeJws(nonce, override); }).then(function (jws) {
        ...
        if (res.status >= 300 || _isProblem(res)) {
          if (_isProblem(res)) {
            ...
            // A badNonce retry keeps the CURRENT signing override (a re-sign is not undone by a following
            // badNonce), so it re-signs with the same alg the request already switched to.
            if (prob && typeof prob.type === "string" && _endsWith(prob.type, ":badNonce") && attempt < maxNonceRetries) { attempt++; return once(override); }
            // A badSignatureAlgorithm (RFC 8555 sec. 6.2): if the caller supplied resignKeys and one of its
            // algs is in the server's advertised `algorithms`, re-sign ONCE with that caller-preferred key.
            // The caller's order selects (never the server's), so a bogus badSignatureAlgorithm cannot force
            // a downgrade (sec. 10); no match, or no resignKeys, falls through to the fail-closed throw.
  ...
  function _post(url, builder, extra, mode, accept, okStatuses) {
    return _postJws(url, function (nonce, override) {
      ...
      // desynchronize the protected header from the actual request (or reuse a nonce). On a badSignatureAlgorithm
      // re-sign `override` carries the caller's chosen { alg, key } (the SAME account material under another alg);
      // the embedded jwk stays accountJwk, which still verifies it (an RSA public JWK is padding-agnostic).
      var base = Object.assign({}, extra || {}, { key: override ? override.key : accountKey, alg: override ? override.alg : alg, nonce: nonce, url: url });
      // On a re-sign the embedded jwk's declared alg must follow the alg being signed, not the account
      // key's native one, so the published JWK and the JWS header agree (jwk mode / newAccount).
  ...
  function _updateAccount(o) {
    return _promised(function () {
      ...
      // RFC 8555 sec. 7.3.2 says the server MUST IGNORE a termsOfServiceAgreed update to the account, so
      // sending it is a silently-dropped no-op; this client refuses it at the door instead. termsOfServiceAgreed
      // is set at account creation (sec. 7.3.1 newAccount), and a CA whose terms have changed answers with a
      // 403 userActionRequired problem (sec. 7.3.3) that directs a HUMAN to the terms URL -- it is not re-agreed
      // through an account update.
    ...
    return _promised(function () {
      ...
      function fetchPage(pageUrl) {
        ...
        return _postAsGet(pageUrl).then(function (res) {
          ...
          // The loop guard runs BEFORE the cap decision: a next to an already-visited page names nothing new,
          // so the list is COMPLETE (not truncated). Only a next to an unseen page that the cap forbids is a
          // truncation -- a cycle landing exactly on the cap must not be reported as one.
  ...
  function _keyChangeBody(o) {
    ...
    return _resource("keyChange").then(function (url) {
      return _post(url, keyChange, { account: account, oldKey: accountJwk, newKey: o.newKey, newJwk: o.newJwk, newAlg: o.newAlg }, "kid").then(function (res) {
        ...
        // The resignKeys alternatives are CryptoKeys for the OLD account key material, so they no longer
        // match the rotated account key. Drop them: re-signing with a stale-key alternative under the new
        // kid would be rejected by the CA. With them cleared, a later badSignatureAlgorithm surfaces (the
        // conformant default) rather than retrying with obsolete key material.
        resignKeys = null;
        var acct = null;
