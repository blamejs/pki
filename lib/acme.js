...
function client(directoryUrl, opts) {
  ...
  function _sendFollowing(method, url, headers, body, redirects) {
    ...
    return transport({ method: method, url: url, headers: reqHeaders, body: body, tls: budgets.tls, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      ...
      if ((method === "GET" || method === "HEAD") && res.status >= 300 && res.status < 400) {
        ...
        return _sendFollowing(method, _clientUrl(h["location"]), headers, body, redirects + 1);
      }
  ...
  function _revokeCert(o) {
    ...
    // Certificate-key mode requires BOTH certKey and certJwk: an incomplete pair must NOT silently fall
    // through to the account-key path, which would authorize the revocation with the wrong credential.
    if ((o.certKey != null) !== !!_isObject(o.certJwk)) return Promise.reject(E("acme/bad-input", "revokeCert certificate-key mode requires BOTH certKey and certJwk, or neither (RFC 8555 sec. 7.6)"));
