...
// it). `URL.canParse` never throws, so no swallow.
function _validAbsoluteUri(t) {
  if (!/^[A-Za-z][A-Za-z0-9+.-]*:/.test(t) || LINK_URI_BADPCT.test(t)) return false;
  var probe = t.replace(/^([A-Za-z][A-Za-z0-9+.-]*:\/\/)\[v[0-9A-Fa-f]+\.[A-Za-z0-9._~!$&'()*+,;=:-]+\]/, "$1[::1]");
...
function _linkUriInvalid(uri) {
  if (LINK_URI_BADCHAR.test(uri) || LINK_URI_BADPCT.test(uri) || _hasEncodedDotSegment(uri)) return true;
  // A bracket is valid ONLY in an authority IP-literal (`scheme://[...]`); in a path (a relative target such as
  // "/cert/[alt]") it is invalid and WHATWG would repair it by percent-encoding. Permit exactly one authority
  // IP-literal (a same-origin IPv6/IPvFuture cert URL is legal) and no bracket anywhere else.
  if (uri.indexOf("[") !== -1 || uri.indexOf("]") !== -1) {
    var m = /^[A-Za-z][A-Za-z0-9+.-]*:\/\/\[[^[\]]*\]/.exec(uri);
    if (!m) return true;
    var rest = uri.slice(m[0].length);
    if (rest.indexOf("[") !== -1 || rest.indexOf("]") !== -1) return true;
  }
  return false;
}
...
function _parseLinkValue(raw) {
  ...
  for (var i = 0; i < params.length; i++) {
    ...
    // even when its value is empty, so `rel="";rel=alternate` uses the empty first value (not the alternate).
  ...
  return { uri: uri, rel: rel, anchor: anchor };
}
