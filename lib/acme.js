...
// A percent-encoded dot (`%2e`): unreserved chars are never pct-encoded canonically (RFC 3986 sec. 2.3), and
// URL resolution DECODES `%2e` to `.` for dot-segment removal -- so an encoded `..` silently becomes a path
// traversal, changing the resolved target. Reject the encoded form (a literal `.` segment resolves normally).
var LINK_URI_ENCODED_DOT = /%2e/i;
...
function _parseLinkValue(raw) {
  ...
  if (LINK_URI_BADCHAR.test(uri) || LINK_URI_BADPCT.test(uri) || LINK_URI_ENCODED_DOT.test(uri)) throw E("acme/bad-link", "a Link URI-Reference contains a character, percent-escape, or encoded dot-segment not permitted by RFC 3986: " + JSON.stringify(raw));
