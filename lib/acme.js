...
function assertTransition(kind, from, to) {
  ...
  if (from === to) return;
  var allowed = _hasOwn(table, from) ? table[from] : null;
 ...
 * `revokeCert`, `deactivateAccount` / `deactivateAuthorization`, `keyChange`, `renewalInfo`, `renewalWindow`, `scheduleRenewal`.
 *
 * @opts
...
function client(directoryUrl, opts) {
  ...
  function _scheduleRenewal(certDer, o) {
    return _resolve(Promise).then(function () {
      ...
      var startMs = _clk(), checks = 0, tempBackoffMs = tempBaseMs, previous = null, cert = certDer;
      ...
      function loop() {
        if (o.shouldStop && o.shouldStop() === true) return { reason: "stopped" };
        ...
        return _renewalWindow(cert, { random: o.random, clock: clock, previous: previous == null ? undefined : previous }).then(function (decision) {
          ...
          if (decision.renewNow) {
            if (o.renew) {
              return _resolve(Promise, o.renew(decision)).then(function (next) {
                ...
                cert = next; previous = null; return loop();
              });
            }
          ...
          if (code === "acme/bad-input" || code === "acme/resource-unavailable" || (typeof code === "string" && intrinsic.stringIndexOf(code, "x509/") === 0)) throw err;
