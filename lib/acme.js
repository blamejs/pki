...
function _splitLinkParams(rest) {
  ...
  for (var j = 0; j < parts.length; j++) {
    ...
    if (v.length >= 2 && v.charAt(0) === "\"" && v.charAt(v.length - 1) === "\"") v = v.slice(1, -1).replace(/\\(.)/g, "$1");
...
function _parseLinkValue(raw) {
  ...
  var params = _splitLinkParams(s.slice(gt + 1)), rel = "";
...
function _parseLinkAlternates(headers, base) {
  ...
  var fields = Array.isArray(raw) ? raw : [raw], out = [], seen = Object.create(null);
  for (var fi = 0; fi < fields.length; fi++) {
    var field = String(fields[fi]);
    if (field.length > LINK_HEADER_MAX_BYTES) throw E("acme/bad-link", "a Link header exceeds the " + LINK_HEADER_MAX_BYTES + "-byte cap (RFC 8288)");
...
function client(directoryUrl, opts) {
  ...
  function _newAuthz(identifier) {
    ...
    return _resource("newAuthz").then(function (url) {
      return _post(url, newAuthz, { identifier: identifier }, "kid", undefined, [201]).then(function (res) {
        ...
        // for a different identifier is not silently accepted.
        if (!authz.identifier || authz.identifier.type !== identifier.type || authz.identifier.value !== identifier.value) throw E("acme/identifier-mismatch", "the returned authorization identifier does not match the requested one (RFC 8555 sec. 7.4.1)");
  ...
  function _renewalInfo(certDer, clockFn) {
    ...
    return _resource("renewalInfo").then(function (base) {
      ...
      return _send("GET", url, { accept: "application/json" }, null).then(function (res) {
        ...
        if (typeof ra === "string" && ra.trim() !== "") retryAfterSeconds = retryAfter.parse(ra, { now: clk(), E: E, code: "acme/bad-retry-after" }).retryAfterSeconds;
  ...
  function _renewalWindow(certDer, o) {
    ...
    return Promise.resolve().then(function () {
      ...
      return _renewalInfo(certDer, clk).then(function (ri) {
