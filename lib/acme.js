...
function _addCommonName(set, cn, what) {
  ...
  if (!looksIp) { set["dns:" + _lower(cn)] = true; return; }
  try { _assertIpAddress(cn); }
  catch (e) {
    throw E("acme/unsupported-identifier-type",
      what + " carries a common name that reads as an IP address but is not its canonical form (" +
      _stringify(cn) + "), so it maps to no order identifier", e);
  }
  set["ip:" + cn] = true;
}

function _addSanName(set, n, what) {
  if (n.tagNumber === 2) { set["dns:" + _lower(_StringI(n.value))] = true; return; }
  if (n.tagNumber === 7) {
...
function _orderIdentifierSet(identifiers) {
  ...
  _forEach(identifiers, function (id) {
    if (id.type === "dns") set["dns:" + _lower(id.value)] = true;
...
// so the three are comparable. Same relation on both sides of the exchange: the subject CN counted as
// dns (every CN RDN, not the first), plus every dNSName / iPAddress SAN entry. A certificate may carry
// several SAN extensions in a non-conformant encoding, so every one is aggregated rather than the
// first -- taking one would let a second smuggle a name past the comparison.
