...
function _parseLinkValue(raw) {
  ...
  var params = _splitLinkParams(rest), rel = "", anchor = null;
  for (var i = 0; i < params.length; i++) {
    if (params[i].name === "rel" && rel === "") rel = params[i].value;
    else if (params[i].name === "anchor" && anchor === null) {
      // anchor is a URI-VALUED param (RFC 8288 sec. 3.2) -- a generic extension param may be valueless, but a
      // valueless/empty anchor is malformed (an empty value would resolve to the context and spoof a match).
      if (params[i].value === "") throw E("acme/bad-link", "a Link anchor parameter requires a URI value (RFC 8288 sec. 3.2): " + JSON.stringify(raw));
...
function _parseLinkAlternates(headers, base) {
  ...
  for (var fi = 0; fi < fields.length; fi++) {
    ...
    for (var vi = 0; vi < values.length; vi++) {
      ...
      if (lv === null) continue;
      // The rel value is a SINGLE-space-separated relation-type list (RFC 8288 sec. 3.3): no leading, trailing,
      // or repeated space. An empty token from split(" ") means one of those -- malformed, fail closed (so
      // "alternate " is not split-and-matched as containing "alternate").
      if (lv.rel !== "" && lv.rel.split(" ").indexOf("") !== -1) throw E("acme/bad-link", "a Link rel value is not a well-formed space-separated relation-type list (RFC 8288 sec. 3.3): " + JSON.stringify(lv.rel));
