...
function client(directoryUrl, opts) {
  ...
  function _send(method, url, headers, body) { return _sendFollowing(method, url, headers, body, 0, budgets.tls); }
  function _sendFollowing(method, url, headers, body, redirects, tls) {
    var reqHeaders = Object.assign({ "user-agent": CLIENT_USER_AGENT }, headers || {});
    return transport({ method: method, url: url, headers: reqHeaders, body: body, tls: tls, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      res = res || {};
      var h = {};
      ...
      if ((method === "GET" || method === "HEAD") && res.status >= 300 && res.status < 400) {
        ...
        // Strip the mTLS client credential when the redirect crosses to a different origin, so an open
        // redirect on the trusted origin cannot disclose the client certificate / key to another host.
        var nextTls = tls;
        if ((tls.cert != null || tls.key != null) && new URL(next).origin !== new URL(url).origin) {
          nextTls = Object.assign({}, tls); delete nextTls.cert; delete nextTls.key;
        }
        return _sendFollowing(method, _clientUrl(next), headers, body, redirects + 1, nextTls);
      }
  ...
  function _postJws(url, makeJws, accept) {
    ...
    function once() {
      return _takeNonce().then(makeJws).then(function (jws) {
        ...
        // ACME uses only 200 / 201 for a successful POST (RFC 8555). Rejecting any other 2xx keeps a verb
        // from acting on an incomplete result -- e.g. keyChange rotating the session key on a 202 Accepted.
        if (res.status !== 200 && res.status !== 201) throw E("acme/unexpected-status", "an ACME POST returned an unexpected success status " + res.status + " (only 200 and 201 are valid, RFC 8555)");
        return res;
      });
    }
    return once();
  ...
  function _downloadCertificate(url) {
    return _post(_clientUrl(url), postAsGet, null, "kid", "application/pem-certificate-chain").then(function (res) {
      ...
      // happens to contain parseable PEM.
      if (!/application\/pem-certificate-chain/i.test(String(res.headers["content-type"] || ""))) throw E("acme/bad-certificate-chain", "the certificate download returned an unexpected media type (expected application/pem-certificate-chain, RFC 8555 sec. 7.4.2)");
  ...
  function _revokeCert(o) {
    ...
    return _resource("revokeCert").then(function (url) {
      if (certKeyMode) {
        ...
        return _postJws(url, function (nonce) {
          return revokeCert(Object.assign({ key: o.certKey, alg: o.certAlg, nonce: nonce, url: url, jwk: o.certJwk }, extra));
