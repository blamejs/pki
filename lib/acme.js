...
function _addCommonName(set, cn, what) {
  ...
  // An address in a common name is not an ip identity, whatever its spelling. Address matching reads
  // the iPAddress subjectAltName and never falls back to the common name, and CABF TLS BR 7.1.4.2.2
  // puts an address in that SAN, so a certificate naming the ordered address only in its common name
  // cannot authenticate it -- reporting the identifier set as bound would say it could.
  if (looksIp) throw _unmappableName(what, cn, null);
