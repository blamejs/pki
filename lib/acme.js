...
function _splitLinkParams(rest) {
  ...
  for (var j = 0; j < parts.length; j++) {
    ...
    // A parameter carries a value: `parameter-name "=" parameter-value` (RFC 9110 sec. 5.6.6). A valueless
    // token (no `=`) is ambiguous and carries no meaning for the relations consumed here -- fail closed.
    if (eq === -1) throw E("acme/bad-link", "a Link parameter must have a value (name=value, RFC 9110 sec. 5.6.6): " + JSON.stringify(p));
...
function _parseLinkValue(raw) {
  ...
  if (LINK_URI_BADCHAR.test(uri) || LINK_URI_BADPCT.test(uri) || _hasEncodedDotSegment(uri)) throw E("acme/bad-link", "a Link URI-Reference contains a character, percent-escape, or encoded dot-segment not permitted by RFC 3986: " + JSON.stringify(raw));
