...
// "http:///relations.example" -> host "relations.example"). Reject it before resolution.
...
// authority, at most one "#". SHARED by the fetched-target/anchor check (_linkUriInvalid) AND the ext-rel-type
// check (_validAbsoluteUri) so the two can NEVER diverge -- a new structural rule added here binds both at once.
function _uriStructurallyInvalid(uri) {
  return !_bracketsOnlyInAuthority(uri) || _hasEmptyAuthority(uri) || _authorityMultiAt(uri) || _multiFragment(uri);
}
