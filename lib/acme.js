...
function _validRelType(t) {
  ...
  // ext-rel-type: an absolute URI validated by RFC 3986 GRAMMAR, INDEPENDENT of fetch-target normalization -- a
  // relation type is an identifier, never resolved, so a percent-encoded dot (%2e) is a legal char (not a dot-
  // segment) and an IPvFuture host is valid, both of which the fetch-target rules / WHATWG would wrongly reject.
  // Require a scheme, only RFC 3986 characters, valid pct-escapes, and a balanced IP-literal (a stray unmatched
  // "[" -- e.g. "http://[" -- is invalid).
  if (!/^[A-Za-z][A-Za-z0-9+.-]*:/.test(t)) return false;
  if (LINK_URI_BADCHAR.test(t) || LINK_URI_BADPCT.test(t)) return false;
...
function _linkUriInvalid(uri) {
  // A FETCH target/anchor is resolved, so it also rejects a bracket: `[`/`]` are valid only in an authority
  // IP-literal, never in a path (a certificate URL never uses one), and WHATWG would repair a stray bracket by
  // percent-encoding it. (The ext-rel-type check keeps brackets, for a valid IPvFuture/IPv6 relation host.)
  return LINK_URI_BADCHAR.test(uri) || LINK_URI_BADPCT.test(uri) || _hasEncodedDotSegment(uri) || uri.indexOf("[") !== -1 || uri.indexOf("]") !== -1;
}
