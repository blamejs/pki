...
var _isXnLabel = guard.name.isXnLabel;
...
var STATUS = intrinsic.assign(intrinsic.create(null), {
...
var _canonicalizeIpv6 = ipUtils.canonicalizeIpv6;
...
/** @internal The decoders take the value and nothing else: the error class and the OID registry
 * they raise and resolve through come from the namespace the table was built with, above. Passing
 * a second argument here suggested a per-call context that the table has no parameter for and
 * silently dropped; where a decode genuinely needs resources, the way to supply them is to build
 * the table with `schema.withResources`, as `schema-attrcert.js` does. */
...
async function verifyTlsAlpn01(certDer, token, accountJwk, identifier) {
  ...
  var san = _extDecoders.byOid[OID_SAN](sanExt.value);
...
function _outerHeader(o) {
  var h = intrinsic.assign(intrinsic.create(null), { alg: o.alg, nonce: o.nonce, url: o.url });
...
function _ipBytesToText(buf) {
  return _isBuffer(buf) ? ipUtils.textFromOctets(buf) : null;
}
...
function _csrIdentifierSet(parsedCsr) {
  ...
  _forEach(parsedCsr.attributes || [], function (a) {
    ...
    _forEach(a.extensions, function (e) {
      ...
      var dec = _extDecoders.byOid[OID_SAN](e.value);
...
function ariCertId(certDer) {
  ...
  var aki = _extDecoders.byOid[OID_AKI](akiExt.value);
...
function _splitPemChain(text) {
  var ders;
  try { ders = pkix.pemDecodeAll(text, "CERTIFICATE", frameworkError.PemError); }
  catch (e) {
    throw E("acme/bad-certificate-chain",
      "the certificate download is not a chain of PEM CERTIFICATE blocks (RFC 8555 sec. 7.4.2): " + e.message, e);
  }
  for (var i = 0; i < ders.length; i++) {
    try { x509.parse(ders[i]); }
    catch (e2) { throw E("acme/bad-certificate-chain", "a downloaded certificate did not decode as a valid X.509 certificate (RFC 8555 sec. 7.4.2)", e2); }
  }
  return ders;
}
...
function _parseLinkRelation(headers, base, relName, opts) {
  ...
  for (var hi = 0; hi < hNames.length; hi++) {
    if (hNames[hi].toLowerCase() === "link") { raw = headers[hNames[hi]]; break; }
  }
  if (raw == null) return [];
 ...
 */
var _CLIENT_OPTS = intrinsic.assign(intrinsic.create(null), {
...
var _METHOD_OPTS = intrinsic.assign(intrinsic.create(null), {
...
function _assertTransport(opts) {
  return guard.identifier.assertCallableOption(opts, "transport", E, "acme/bad-input", "opts.transport",
...
function client(directoryUrl, opts) {
  ...
  var transport = checkedTransport;
  if (!transport) {
    var t0 = opts.tls || {};
  ...
  function _sendFollowing(method, url, headers, body, redirects) {
    ...
    return guard.async.deferred(function () {
      return guard.async.invoked(transport, [{ method: method, url: url, headers: reqHeaders, body: body, tls: _tlsFor(url), proxy: budgets.proxy, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }], E, "acme/bad-input", "opts.transport");
    }).then(function (res) {
      res = res || {};
  ...
  function _post(url, builder, extra, mode, accept, okStatuses) {
    return _postJws(url, function (nonce, override) {
      var base = guard.identifier.ownOptions(extra);
      base.key = override ? override.key : accountKey;
      base.alg = override ? override.alg : alg;
      base.nonce = nonce;
      base.url = url;
      ...
      return _resource("revokeCert").then(function (url) {
        if (certKeyMode) {
          return _postJws(url, function (nonce) {
            var rb = guard.identifier.ownOptions(extra);
            rb.key = o.certKey; rb.alg = o.certAlg; rb.nonce = nonce; rb.url = url; rb.jwk = o.certJwk;
            return revokeCert(rb);
