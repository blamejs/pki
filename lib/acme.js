 ...
 */
function newAuthz(o) {
  if (!_isObject(o) || !_isObject(o.identifier)) throw E("acme/bad-order", "newAuthz requires a single identifier object (RFC 8555 sec. 7.4.1)");
  _validateIdentifier(o.identifier);
  return _signOuter({ key: o.key, alg: o.alg, nonce: o.nonce, url: o.url, kid: o.kid }, { identifier: o.identifier });
}
...
function _splitLinkParams(rest) {
  ...
  for (var j = 0; j < parts.length; j++) {
    ...
    if (p === "") continue;
    var eq = p.indexOf("=");
    if (eq === -1) { out.push({ name: p.toLowerCase(), value: "" }); continue; }
    var v = p.slice(eq + 1).trim();
    if (v.length >= 2 && v.charAt(0) === "\"" && v.charAt(v.length - 1) === "\"") {
      v = v.slice(1, -1).replace(/\\(.)/g, "$1");   // a quoted-string may hold a space-separated list (RFC 8288)
    } else if (v !== "" && !/^[A-Za-z0-9!#$%&'*+.^_`|~-]+$/.test(v)) {
      // An UNQUOTED value is a single RFC 7230 token -- no whitespace or specials. Reject a malformed one
      // (e.g. rel=alternate garbage) rather than split-and-match it as if it were a quoted relation list.
      throw E("acme/bad-link", "an unquoted Link parameter value must be a single token (RFC 8288 / RFC 7230): " + JSON.stringify(p));
    }
    out.push({ name: p.slice(0, eq).trim().toLowerCase(), value: v });
  }
  return out;
}
...
function _relHasAlternate(rel) {
  var toks = String(rel).trim().split(/[ \t]+/);
...
function _parseLinkValue(raw) {
  ...
  // A URI-Reference carries no raw whitespace or backslash (RFC 3986); WHATWG URL parsing would silently
  // REPAIR those (trim/rewrite) when the target is later resolved, masking a malformed / off-path value --
  // reject it up front, the same canonicality gate the client applies to every server-provided URL.
  if (/[\s\\]/.test(uri)) throw E("acme/bad-link", "a Link URI-Reference must not contain whitespace or a backslash (RFC 3986): " + JSON.stringify(raw));
  var params = _splitLinkParams(rest), rel = "";
  for (var i = 0; i < params.length; i++) { if (params[i].name === "rel") { rel = params[i].value; break; } }
  return { uri: uri, rel: rel };
}
...
function _parseLinkAlternates(headers, base) {
  ...
  if (raw == null) return [];
  var baseOrigin = new URL(base).origin;   // base is the already-https-gated download URL
  ...
  for (var fi = 0; fi < fields.length; fi++) {
    ...
    for (var vi = 0; vi < values.length; vi++) {
      ...
      if (lv === null || !_relHasAlternate(lv.rel)) continue;
      var resolved;
      try { resolved = _resolveLocation(lv.uri, base); }   // resolves a relative ref + enforces the https gate
      catch (e) { throw E("acme/bad-link", "a rel=\"alternate\" Link target is not a usable https URL: " + JSON.stringify(lv.uri), e); }
      ...
      if (new URL(resolved).origin !== baseOrigin) throw E("acme/bad-link", "a rel=\"alternate\" Link target is not on the certificate download's origin (SSRF guard): " + JSON.stringify(lv.uri));
      if (!seen[resolved]) { seen[resolved] = true; out.push(resolved); }
    }
  }
  return out;
}
 ...
 *   - `renewalWindow(certDer, { random, clock, replaced })` -- the RFC 9773 ARI renewal decision: composes `renewalInfo`, selects a uniform-random instant in the suggested window -> { suggestedWindow, selectedTime, renewNow, retryAfterSeconds, explanationURL }.
 * @example
...
function client(directoryUrl, opts) {
  ...
  function _newAuthz(identifier) {
    ...
    // Reject a wildcard / bad AUTHORIZATION identifier BEFORE any network (sec. 7.4.1); the builder re-checks.
    _validateIdentifier(identifier);
    return _resource("newAuthz").then(function (url) {
      return _post(url, newAuthz, { identifier: identifier }, "kid", undefined, [201]).then(function (res) {
        var loc = res.headers["location"];
        ...
        if (!authz.identifier || authz.identifier.type !== identifier.type || authz.identifier.value !== identifier.value || authz.wildcard === true) throw E("acme/identifier-mismatch", "the returned authorization does not match the requested identifier (RFC 8555 sec. 7.4.1)");
        // A pre-authorization CREATES an authorization, which begins in the "pending" state (sec. 7.1.4/7.1.6):
        // the caller then fulfils a challenge. A non-pending status -- a terminal "invalid"/"deactivated"/
        // "expired"/"revoked", or even a "valid" with no challenge to answer -- is not the fresh, fulfillable
        // authorization the pre-auth flow expects, so it fails closed rather than being handed back unusable.
        if (authz.status !== "pending") throw E("acme/unexpected-authorization-status", "newAuthz returned a non-pending authorization (status " + JSON.stringify(authz.status) + "); a pre-authorization must be pending (RFC 8555 sec. 7.1.4)");
  ...
  function _downloadCertificate(url, opts) {
    ...
    return Promise.resolve().then(function () {
      ...
      return _post(dlUrl, postAsGet, null, "kid", "application/pem-certificate-chain").then(function (res) {
        ...
        // (CWE-770 fetch amplification). The predicate runs per candidate; the first truthy wins. An
        // alternate whose end-entity certificate differs from the primary's violates sec. 7.4.2 ("starting
        // with the same end-entity certificate") -- fail closed rather than accept a substituted leaf. A
        // predicate throw propagates unswallowed. Exhausting the budget with alternates left unfetched is
        // acme/too-many-alternates; fetching every alternate with no match is acme/no-matching-chain.
        if (select(primary)) return result(primary);
        ...
        function next() {
          ...
          return _fetchCertChain(toFetch[idx++]).then(function (cand) {
            ...
            return select(cand) ? result(cand) : next();
          });
        }
        return next();
      });
  ...
  function _renewalInfo(certDer, clockFn, retryAfterCapSeconds) {
    ...
    var clk = typeof clockFn === "function" ? clockFn : clock;   // renewalWindow may pass a per-call clock
  ...
  function _renewalWindow(certDer, o) {
    ...
    return Promise.resolve().then(function () {
      ...
      // "should I renew as of time T?" deterministically without reconfiguring the client.
      var clk = typeof o.clock === "function" ? o.clock : clock;
      ...
      var now = clk();
      // allow:nan-date-comparison-unguarded -- notAfter is a codec-parsed cert date (asn1 readTime rejects a
      // NaN instant) and the window start/end below are rfc3339-validated by validateRenewalInfo; neither is NaN.
      ...
      return _renewalInfo(certDer, clk, RENEWAL_RETRY_MAX_SECONDS).then(function (ri) {
        ...
        var draw = o.random ? Number(o.random()) : _defaultRandom();
        if (!(draw >= 0 && draw <= 1)) throw E("acme/bad-input", "renewalWindow opts.random must return a number in [0, 1]");
        // RFC 9773 sec. 4.2: select a uniform-random instant across the (validity-bounded) window to spread
        // renewals; if it is already in the past, the caller should renew immediately.
        var selectedMs = Math.round(effStart + draw * (effEnd - effStart));
        ...
        return {
          ...
          // renew now when the ENTIRE suggested window opens after notAfter -- there is no valid renewal
          // instant left in it, so waiting for the (clamped) time would leave the cert to expire unrenewed.
          renewNow: selectedMs <= clk() || startMs > notAfterMs,
