...
function _requireOneDerTlv(content) {
  ...
  if (node.tagClass === "universal") {
    if (node.tagNumber === 0) throw _err("c509/bad-extensions", "a ~oid-form subjectDirectoryAttributes value must not be the reserved end-of-contents encoding (tag 0)");
    var reader = node.constructed ? undefined : _SDA_VALUE_READERS[node.tagNumber];
    if (reader) { try { reader(node); } catch (e) { throw _err("c509/bad-extensions", "a ~oid-form subjectDirectoryAttributes value is not a valid DER element for its type", e); } }
  }
  return content;
}
