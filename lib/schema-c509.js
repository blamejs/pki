...
function _name509(node, isSubject) {
  ...
  for (var i = 0; i + 1 < kids.length; i += 2) {
    ...
    var rdn = { type: tname, value: vv, printable: ti < 0, eui64: v.eui64 };
    // The (type, sign) pair DECLARES an X.509 string type, so the text must be valid for it -- a printableString
    // sign over a non-PrintableString character, or a non-ASCII emailAddress (an IA5String-only attribute). Run
    // the SAME builder the reconstruction uses, so a Name this parse accepts is one it can always rebuild, and a
    // type-2 certificate (which never reconstructs) is held to the identical rule. The builder's asn1/* fault is
    // re-thrown in this module's domain rather than leaking onto the parse surface.
    try { _reconAttrValue(rdn); }
    catch (e) { throw _err("c509/bad-name", "the " + tname + " value is not valid for the string type its attribute integer declares", e); }
    rdns.push(rdn);
...
function _reconAttrValue(rdn) {
  ...
  if (rdn.type === "emailAddress") return b.ia5(String(rdn.value));
