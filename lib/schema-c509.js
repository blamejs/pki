...
function parse(input) {
  ...
  var _f0 = f[0].bytes, _f9 = f[9].bytes;
  result._fieldBytes = input.subarray(_f0.byteOffset - input.byteOffset, _f9.byteOffset - input.byteOffset + _f9.length);
...
function _encodeC509Array(r) {
  ...
  if (Buffer.isBuffer(r._fieldBytes)) {
    return Buffer.concat([Buffer.from([0x8b]), r._fieldBytes, cbor.build.byteString(r.signatureValue)]);   // array(11) head + fields 0..9 + signatureValue
  }
