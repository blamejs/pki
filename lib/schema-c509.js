 ...
 * carry. It is resolved authoritatively (never a magnitude guess): from a self-signed certificate's own
 * subject curve, or from the RFC 5480 standard digest<->curve pairing the signature algorithm implies. A
 * non-self-issued certificate signed with a non-standard digest/curve pairing (its r/s wider than the
 * digest's standard curve) fails closed -- supply the issuer curve via `opts.issuerCurve`.
 *
 * @opts
...
// a smallest-fit guess: (1) an explicit opts.issuerCurve; (2) the subject curve when the certificate is
// self-issued (a self-signed certificate is signed by its own key); (3) the RFC 5480 standard digest<->curve
// pairing the signature algorithm implies. A non-self-issued signature whose r/s magnitudes exceed the
// digest's standard curve is a non-standard digest/curve pairing that cannot be inferred -- it fails closed
// and directs the caller to opts.issuerCurve.
function _resolveIssuerSigCurve(c, subjectCurveNode, opts) {
  ...
  } else if (Buffer.compare(c.issuer.bytes, c.subject.bytes) === 0) {
    curve = NODE_TO_WEBCRYPTO[subjectCurveNode];
    if (!curve) throw _err("c509/non-invertible", "unsupported self-signed issuer curve " + subjectCurveNode);
  } else {
...
_derToType3 = function (input, opts) {
  ...
  var sigCurve = _resolveIssuerSigCurve(c, curve, opts);   // the ISSUER signing curve, resolved authoritatively (opts / self-issued / digest pairing)
