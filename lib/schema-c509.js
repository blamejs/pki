...
function _requireOneDerTlv(content) {
  ...
  // asn1.decode is strict DER: it rejects a truncated TLV AND trailing bytes past the first TLV
  // (asn1/trailing-bytes), so a byte string holding more than one -- or a malformed -- TLV fails here, and
  // only exactly one well-formed AttributeValue TLV survives to be spliced into the values SET.
  try { asn1.decode(content); } catch (e) { throw _err("c509/bad-extensions", "a ~oid-form subjectDirectoryAttributes value must be exactly one well-formed DER AttributeValue TLV (no trailing data)", e); }
  return content;
}
  ...
  for (var i = 0; i + 1 < kids.length; i += 2) {
    ...
    if (typeNode.majorType === 0 || typeNode.majorType === 1) {          // int form: a sec. 8.6 registry alias, text values
      ...
      for (var vi = 0; vi < valuesNode.children.length; vi++) {
        ...
        // _reconAttrValue -> b.printable / b.utf8 throws an Asn1Error for a value outside its string type's
        ...
        catch (e) { if (e instanceof C509Error) throw e; throw _err("c509/bad-extensions", "a subjectDirectoryAttributes " + tname + " value is not valid for its string type", e); }
      }
