...
// asn1.decode validates TLV FRAMING (it rejects a truncated TLV + trailing bytes past the first, asn1/trailing-
// bytes) but not per-type CONTENT -- it frames an empty INTEGER, a reserved end-of-contents tag 0, and an
// out-of-alphabet string. AttributeValue is ANY, so a decoded UNIVERSAL PRIMITIVE is re-validated through its
// strict content reader below, and reserved tag 0 is rejected, so the reconstruct splices nothing a strict DER
// decoder would reject. A constructed or non-universal element (a legitimately-typed ANY with no single reader)
// passes on its framing.
var _SDA_VALUE_READERS = (function () {
...
// Fully strict-validate a decoded DER element (an ANY AttributeValue at ANY depth). asn1.decode checks framing
// but not per-type CONTENT, DER SET ordering (X.690 sec. 11.6), or nested validity: reject the reserved EOC tag
// 0; run a universal primitive through its strict content reader; recurse into a constructed element's children
// and require a universal SET's members be Buffer.compare-ascending -- the EXACT order asn1.build.set emits, so a
// value the encoder could not produce is rejected. A non-universal element (a legitimate context/application-
// tagged ANY with no single reader) passes on its own framing, but its constructed children are still recursed.
function _strictDerElement(node) {
  if (node.tagClass === "universal" && node.tagNumber === 0) throw _err("c509/bad-extensions", "a subjectDirectoryAttributes value must not use the reserved end-of-contents encoding (tag 0)");
  if (node.constructed) {
    var kids = node.children;   // asn1.decode always sets a (possibly empty) children array on a constructed node
    for (var i = 0; i < kids.length; i++) _strictDerElement(kids[i]);
    if (node.tagClass === "universal" && node.tagNumber === asn1.TAGS.SET) {
      for (var j = 1; j < kids.length; j++) {
        if (Buffer.compare(kids[j - 1].bytes, kids[j].bytes) > 0) throw _err("c509/bad-extensions", "a subjectDirectoryAttributes SET value must be in DER ascending order (X.690 sec. 11.6)");
      }
    }
    return;
  }
  if (node.tagClass === "universal") {
    var reader = _SDA_VALUE_READERS[node.tagNumber];
    if (reader) { try { reader(node); } catch (e) { throw _err("c509/bad-extensions", "a subjectDirectoryAttributes value is not a valid DER element for its type", e); } }
  }
}
// A ~oid AttributeValue must be exactly ONE non-empty, well-formed AND strictly-valid DER element (framing via
// asn1.decode + no trailing data; content / SET order / nesting via _strictDerElement) before it is spliced.
function _requireOneDerTlv(content) {
  if (content.length === 0) throw _err("c509/bad-extensions", "a ~oid-form subjectDirectoryAttributes value must be a non-empty DER AttributeValue TLV");
  var node;
  try { node = asn1.decode(content); } catch (e) { throw _err("c509/bad-extensions", "a ~oid-form subjectDirectoryAttributes value must be exactly one well-formed DER AttributeValue TLV (no trailing data)", e); }
  _strictDerElement(node);
  return content;
}
...
function _sdaToDer(node) {
  ...
  for (var i = 0; i + 1 < kids.length; i += 2) {
    ...
    if (typeNode.majorType === 0 || typeNode.majorType === 1) {          // int form: a sec. 8.6 registry alias, text values
      ...
      attrs.push(b.sequence([b.oid(oid.byName(tname)), b.set(vals)]));
      ...
      for (var vj = 0; vj < valuesNode.children.length; vj++) {
        ...
        vals.push(b.raw(_requireOneDerTlv(vb.content)));
      }
      attrs.push(b.sequence([b.oid(dotted), b.set(vals)]));
    } else {
...
function _sdaTryIntForm(tint, valueNodes) {
...
function _sdaFromDer(node) {
  ...
  for (var ai = 0; ai < node.children.length; ai++) {
    ...
    var intForm = _sdaTryIntForm(tint, setNode.children);
    if (intForm != null) {
