...
function _encodeC509Array(r) {
  // A NATIVE (type-2) certificate's signature covers its raw CBOR fields (elements 0..9), so re-emit them
  // VERBATIM from the parsed signedData -- re-deriving from the decoded values could differ on a
  // canonical-equivalent form (a byte-string attribute value, a registry alias) and silently invalidate
  // the signature. A type-3 certificate is signed over the reconstructed DER, so its fields re-derive
  // safely (the round trip is validated on the DER, not the CBOR).
  if (r.certificateType === 2 && Buffer.isBuffer(r.signedData)) {
    return Buffer.concat([Buffer.from([0x8b]), r.signedData, cbor.build.byteString(r.signatureValue)]);   // array(11) head + fields 0..9 + signatureValue
  }
  ...
  return cbor.build.array([
    ...
    cbor.build.uint(BigInt(Math.floor(r.validity.notBefore.getTime() / 1000))),
    r.validity.notAfter === null ? cbor.build.nullValue() : cbor.build.uint(BigInt(Math.floor(r.validity.notAfter.getTime() / 1000))),
 ...
 * @signature  pki.schema.c509.encode(input) -> Buffer
 * @since      0.3.4
 * @status     experimental
...
// The ISSUER signature's fixed-width r||s is sized by the ISSUER's signing curve, which is neither the
// SUBJECT's public-key curve (a CA may sign a subject on a different curve) nor implied by the signature
// digest (a P-384 key may sign with SHA-256). Infer it from the DER signature's own r / s integer widths:
// the smallest supported field size that holds both. A magnitude byte length is the DER INTEGER content
// past any sign octet.
...
function _issuerSigCurve(derSig) {
  var n;
  ...
  var w = Math.max(_magBytes(n.children[0]), _magBytes(n.children[1]));
  if (w <= 32) return "P-256";
  if (w <= 48) return "P-384";
  if (w <= 66) return "P-521";
  throw _err("c509/non-invertible", "the ECDSA issuer signature width " + w + " does not match a supported curve");
}
...
_derToType3 = function (input) {
  ...
  var sigCurve = _issuerSigCurve(c.signatureValue.bytes);   // the ISSUER signing curve, inferred from the signature's own r/s widths
...
function encode(input) {
  ...
  var encoded = _encodeC509Array(_derToType3(input));
