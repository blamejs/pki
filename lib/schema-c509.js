...
function _specialText(node) {
  ...
  if (node.majorType === 6 && Number(node.argument) === 48) {
    return { eui64: node.children && node.children[0] ? node.children[0].content : Buffer.alloc(0) };
  }
...
function _reconstructDer(r, sigNode) {
  var sigAlgSeq = b.sequence([b.oid(r.signatureAlgorithm.oid)]);   // ecdsaWith* omits parameters
  var tbs = b.sequence([
