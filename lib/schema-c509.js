...
function _specialText(node) {
  ...
  if (node.majorType === 6 && Number(node.argument) === 48) {
    // A tag-48 MAC address (RFC 9542) MUST wrap a CBOR byte string; anything else is malformed.
    ...
    return { eui64: node.children[0].content };
  }
...
function _reconSpki(spkAlg, keyBytes, rsaKey) {
  if (spkAlg.name === "ecPublicKey") {
    ...
    if (head === 0x04 || head === 0x02 || head === 0x03) point = keyBytes;   // uncompressed / SEC1 kept
...
function _reconKeyUsageBits(value) {
  if (value <= 0) throw _err("c509/non-invertible", "a keyUsage value must set at least one bit");
...
function _reconAlgId(alg) {
  ...
  if (alg.parameters && alg.parameters.length) fields.push(b.raw(alg.parameters));
  return b.sequence(fields);
}
...
function _reconstructDer(r, sigNode) {
  ...
  // A non-empty even length is exactly rawToEcdsaDer's precondition (raw.length === 2*coordLen), so this
  // check is the typed guard -- an empty or odd r||s surfaces c509/bad-signature instead of a raw throw.
  if (r.signatureValue.length === 0 || r.signatureValue.length % 2 !== 0) {
    throw _err("c509/bad-signature", "the type-3 ECDSA issuerSignatureValue must be a non-empty fixed-width r||s");
  }
  var sigValue = validator.sig.rawToEcdsaDer(r.signatureValue, r.signatureValue.length / 2);
