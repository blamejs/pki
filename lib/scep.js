...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function step(current) {
    ...
    if (opts.proxy != null) req.proxy = opts.proxy;
...
function _snapshotProxy(p) {
  ...
  if (p.tls != null && typeof p.tls === "object" && !Array.isArray(p.tls)) {
    ...
    if (Array.isArray(p.tls.anchors)) out.tls.anchors = p.tls.anchors.slice();
  }
  return out;
}
...
var _TRANSPORT_KEYS = { transport: 1, timeout: 1, maxResponseBytes: 1, maxRedirects: 1, tls: 1, allowCrossOriginRedirect: 1, proxy: 1 };
