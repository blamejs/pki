...
function _invert(table) {
  var o = Object.create(null);
  var names = guard.identifier.optionNames(table);
  for (var i = 0; i < names.length; i++) o[table[names[i]]] = names[i];
  return o;
}
...
function _transactionId(v) {
  ...
  if (!asn1.isPrintableString(v)) throw _err("scep/bad-input", "transactionId is a PrintableString (RFC 8894 sec. 3.2.1.1): letters, digits, space and ' ( ) + , - . / : = ? only");
  return v;
}
...
function _header(headers, name) {
  ...
  var hNames = guard.identifier.optionNames(headers);
  for (var hi = 0; hi < hNames.length; hi++) {
    if (_foldLower(String(hNames[hi])) === name) { found = headers[hNames[hi]]; seen += 1; }
  }
...
/** @internal Refused at every network verb's door and again where the value is used. The checked
 * value is RETURNED and it is that one the caller uses: reading the option again would ask a second
 * time, and an accessor can answer differently on each read. */
function _assertTransport(opts) {
  return guard.identifier.assertCallableOption(opts, "transport", _err, "scep/bad-input", "opts.transport",
...
function _client(method, url, body, headers, opts, checked) {
  var resolved = checked !== undefined ? checked : _assertTransport(opts);
  var transport = resolved || httpTransport.https({ E: _err, errPrefix: "scep" });
  var budgets = {
...
async function getCACaps(baseUrl, opts) {
  ...
  var checkedTransport = _assertTransport(opts);
  var profile = { expectSCEPStandard: !!opts.expectSCEPStandard, requireStrongProfile: !!opts.requireStrongProfile };
  var res = await _client("GET", _scepUrl(baseUrl, "GetCACaps"), null, {}, opts, checkedTransport);
  if (res.status === 200) {
    var caps = parseCapabilities(_requireOk(res, [CT_PLAIN], "GetCACaps"));
    _assertStrongProfile(caps, profile);
    return caps;
  }
  /** @internal Built the way parseCapabilities builds one, so a CA that advertises nothing yields a
   * capabilities map of the same shape as one that advertises something. */
  if (res.status === 204 || res.status === 400 || res.status === 404 || res.status === 405 || res.status === 501) {
    var none = Object.create(null);
    _assertStrongProfile(none, profile);
    return none;
  }
...
async function getCACert(baseUrl, opts) {
  ...
  var res = await _client("GET", _scepUrl(baseUrl, "GetCACert"), null, {}, opts, checkedTransport);
...
async function getNextCACert(baseUrl, opts) {
  ...
  var res = await _client("GET", url, null, {}, opts, checkedTransport);
...
async function _pkiOperation(messageType, baseUrl, opts, verbName) {
  ...
  /** @internal Held from the door and forwarded below. The options this verb reads on the way there
   * are the caller's, and reading the transport again afterwards would let one of them change what
   * comes back, so the value that was checked is the value that is used. */
  var checkedTransport = _assertTransport(opts);
  ...
  var clientOpts = {
    tls: opts.tls != null ? guard.identifier.ownOptions(opts.tls) : opts.tls,
  ...
  /** @internal The value held from the door, carried only when the caller supplied one, so an
   * omitted transport does not arrive downstream as a key that is present and undefined. */
  if (checkedTransport !== undefined) clientOpts.transport = checkedTransport;
...
async function _queryOperation(messageType, baseUrl, opts, verbName) {
  ...
  /** @internal Held from the door and forwarded below, so a later read cannot answer differently. */
  var checkedTransport = _assertTransport(opts);
  if (opts.caCert == null) throw _err("scep/bad-input", "a caCert (the SCEP CA certificate DER) is required");
  if (opts.signer == null || typeof opts.signer !== "object") throw _err("scep/bad-input", "a signer { cert, key } is required");
  if (opts.recipientKey == null || typeof opts.recipientKey !== "object") throw _err("scep/bad-input", "a recipientKey { cert, key } is required to decrypt the CA response");
  ...
  var clientOpts = { tls: opts.tls != null ? guard.identifier.ownOptions(opts.tls) : opts.tls, maxRedirects: opts.maxRedirects, allowCrossOriginRedirect: opts.allowCrossOriginRedirect, timeout: opts.timeout, maxResponseBytes: opts.maxResponseBytes, proxy: _snapshotProxy(opts.proxy) };
  if (checkedTransport !== undefined) clientOpts.transport = checkedTransport;
  var senderNonce = nodeCrypto.randomBytes(16);
  var transactionId = opts.transactionId != null ? opts.transactionId : _defaultTransactionId();
  var httpMethod = _pkiMethod(opts);
...
function _withKeys(extra) {
  var o = Object.create(null);
  var base = guard.identifier.optionNames(_TRANSPORT_KEYS);
  for (var bi = 0; bi < base.length; bi++) o[base[bi]] = 1;
  var more = guard.identifier.optionNames(extra);
  for (var mi = 0; mi < more.length; mi++) o[more[mi]] = 1;
  return o;
}
