...
var CT_NEXT_CA_CERT = "application/x-x509-next-ca-cert"; // GetNextCACert, the rollover certificate(s), sec. 4.7.1
...
async function getNextCACert(baseUrl, opts) {
  ...
  // The current CA certificate authenticates the response (RFC 8894 sec. 4.7.1). Require it and snapshot
  // its bytes at this synchronous door, before the GET await, so a caller cannot switch the authenticating
  // certificate out from under the exchange, and so an unauthenticated rollover certificate -- which would
  // become a future trust anchor -- is never returned.
  ...
  // The current CA must itself be authorized to sign, or its signature over the rollover certificate
  // authenticates nothing (RFC 8894 sec. 2.3); fail fast at the door before the network call.
  ...
  // Verify the outer SignedData, passing the current CA as a candidate so the response need not embed the
  // signer. cms.verify reports a per-signer verdict rather than throwing on a bad signature, so an invalid
  // signature is refused here before any content is read. A detached outer (no content) has no rollover
  // certificate to carry and is refused.
  var v;
  ...
  // A GetNextCACert response is signed by the current CA and only the current CA (RFC 8894 sec. 4.7.1);
  // exactly one SignerInfo is expected, as with any authenticated SCEP message (sec. 3.1). Requiring one
  // signer binds the signature-verified check and the current-CA key pin to the SAME signer, so a second
  ...
  // Pin the one verified signer to the current CA key: only the current CA holds the current CA key, so the
  // verified signer carrying that key IS the current CA (RFC 8894 sec. 4.7.1).
  ...
  // The verified content is a degenerate certs-only SignedData carrying the next CA certificate(s);
  // parseCertsOnly fails closed on an empty CertificateSet (scep/no-certificates).
