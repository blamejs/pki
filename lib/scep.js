...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function step(current) {
    ...
    return guard.async.awaited(transport(req), _err, "scep/bad-input", "opts.transport").then(function (res) {
      res = res || {};
...
/** @internal A transport that is not callable is a config-time fault, so every network verb refuses
 * it at its door and the shared client refuses it again where the value is used. A falsy one is the
 * same fault: reading it as "none supplied" would fall back to the default client and open a real
 * connection for a caller that believed it had injected one. */
function _assertTransport(transport) {
  if (transport != null && typeof transport !== "function") {
    throw _err("scep/bad-input", "opts.transport must be a function (request) => Promise<{ status, headers, body }>, got " + guard.text.showValue(transport));
  }
}
...
async function _queryOperation(messageType, baseUrl, opts, verbName) {
  ...
  var clientOpts = { transport: opts.transport, tls: opts.tls != null ? Object.assign({}, opts.tls) : opts.tls, maxRedirects: opts.maxRedirects, allowCrossOriginRedirect: opts.allowCrossOriginRedirect, timeout: opts.timeout, maxResponseBytes: opts.maxResponseBytes, proxy: _snapshotProxy(opts.proxy) };
  var senderNonce = nodeCrypto.randomBytes(16);
  var transactionId = opts.transactionId != null ? opts.transactionId : _defaultTransactionId();
  var httpMethod = _pkiMethod(opts);
