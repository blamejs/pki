...
function _bodyBuf(body) {
  ...
  return Buffer.from(String(body == null ? "" : body), "utf8");
}
...
function _redirectTarget(current, location, method, opts) {
  ...
  if (resolved.origin !== current.origin && !safe && !opts.allowCrossOriginRedirect) throw _err("scep/cross-origin-redirect", "a cross-origin redirect on a " + method + " needs opts.allowCrossOriginRedirect");
  return resolved;
}
...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function _hopTls(current) {
    var t = _tlsForRequest(opts);
    if (current.origin !== initialOrigin && (t.cert != null || t.key != null || t.servername != null)) {
      t = Object.assign({}, t);
  ...
  function step(current) {
    ...
    return transport(req).then(function (res) {
      ...
      if (status >= 300 && status < 400) {
        ...
        redirects += 1;
        var target = _redirectTarget(current, _header(res.headers, "location"), curMethod, opts);
...
function _assertStrongProfile(caps, opts) {
  if (opts.expectSCEPStandard && !caps.SCEPStandard) throw _err("scep/weak-profile", "the CA did not advertise SCEPStandard (RFC 8894 sec. 3.5.2); the GetCACaps response is unauthenticated, so its absence is a downgrade signal (sec. 7.5)");
  if (opts.requireStrongProfile && (!caps.AES || !caps["SHA-256"] || !caps.POSTPKIOperation)) throw _err("scep/weak-profile", "the CA did not advertise the strong profile (AES + SHA-256 + POSTPKIOperation)");
}
...
async function getCACaps(baseUrl, opts) {
  ...
  if (res.status === 200) {
    ...
    _assertStrongProfile(caps, opts);
    return caps;
  }
  if (res.status === 204 || res.status === 404 || res.status === 400) { _assertStrongProfile({}, opts); return {}; }
...
async function getCACert(baseUrl, opts) {
  ...
  if (opts.expectedFingerprint != null) {
    var fp = _normalizeFingerprint(opts.expectedFingerprint, opts.fingerprintAlgorithm), matched = null, j;
...
// The default transaction ID: the SHA-256 of the request's SubjectPublicKeyInfo, hex-encoded. The RFC
// requires only uniqueness (sec. 3.2.1.1); keying it to the public key gives a stable ID across retries.
function _defaultTransactionId(csrSpkiBytes) {
  return nodeCrypto.createHash("sha256").update(csrSpkiBytes).digest("hex");
}
async function _pkiOperation(messageType, baseUrl, opts, verbName) {
  ...
  var transactionId = opts.transactionId != null ? opts.transactionId : _defaultTransactionId(spki.bytes);
  var reqMsg = await build({ messageType: messageType, messageData: csrView, recipient: opts.caCert, signer: opts.signer, transactionId: transactionId, senderNonce: senderNonce });
  var res = await _client("POST", _scepUrl(baseUrl, "PKIOperation"), reqMsg, { "content-type": CT_PKI_MESSAGE }, opts);
  var body = _requireOk(res, [CT_PKI_MESSAGE], "PKIOperation");
  var rep = await parse(body, { signerCert: opts.responderCert != null ? opts.responderCert : opts.caCert, expectedSenderNonce: senderNonce, recipientKey: opts.recipientKey });
  ...
  if (rep.pkiStatus === "FAILURE") throw _err("scep/enrollment-failed", "the CA refused the request with failInfo " + JSON.stringify(rep.failInfo) + (rep.failInfoText ? " (" + rep.failInfoText + ")" : ""));
  if (rep.pkiStatus === "PENDING") return { status: "PENDING", transactionId: rep.transactionId, senderNonce: senderNonce };
  var issued = findIssuedCert(rep.certificates || [], spki.bytes);
  ...
  return { status: "SUCCESS", certificate: issued, certificates: rep.certificates, transactionId: rep.transactionId };
}
 ...
 *   - `transactionId` -- a caller-unique PrintableString (defaults to the hex SHA-256 of the request public key).
 *   - `transport` / `timeout` / `maxResponseBytes` / `maxRedirects` / `tls` / `allowCrossOriginRedirect` -- transport bounds.
 * @example
