...
var _PARSE_SUPPORTED = { PKCSReq: 1, RenewalReq: 1, CertRep: 1, CertPoll: 1, GetCert: 1, GetCRL: 1 };
...
async function build(spec) {
  ...
  if (spec.messageType !== "PKCSReq" && spec.messageType !== "RenewalReq" && spec.messageType !== "CertPoll" && spec.messageType !== "GetCert" && spec.messageType !== "GetCRL") {
    throw _err("scep/bad-message-type", "pki.scep.build issues a request (PKCSReq / RenewalReq / CertPoll / GetCert / GetCRL); " + spec.messageType + " is not built in this release");
  }
...
var _BUILD_KEYS = { messageType: 1, messageData: 1, recipient: 1, requestSubject: 1, issuer: 1, certificate: 1, serialNumber: 1, signer: 1, transactionId: 1, senderNonce: 1 };
...
function _buildIssuerAndSubject(recipientCertDer, requestSubjectSource, issuerOverride) {
  ...
  if (!pkix.rootSequenceChildren(issuerNode, 0) || !pkix.rootSequenceChildren(subjNode, 0)) {
    throw _err("scep/bad-input", "the CertPoll issuer and requestSubject must each be a Name (a SEQUENCE) DER (RFC 8894 sec. 3.3.3)");
  }
...
function _serialToInteger(s) {
  ...
  if (typeof s === "bigint") n = s;
    ...
    try { n = BigInt("0x" + t); } catch (_e) { throw _err("scep/bad-input", "serialNumber must be a hex string, a non-negative safe integer, or a BigInt: " + JSON.stringify(s)); }
  } else {
    throw _err("scep/bad-input", "serialNumber must be a hex string, a non-negative safe integer, or a BigInt");
  }
  if (n < 0n) throw _err("scep/bad-input", "serialNumber must be non-negative (RFC 5280 sec. 4.1.2.2)");
  return n;
}
...
function _buildIssuerAndSerial(spec) {
  ...
  if (hasCert) {
    ...
    if (!pkix.rootSequenceChildren(node, 0)) throw _err("scep/bad-input", "issuer must be a Name (a SEQUENCE) DER (RFC 8894 sec. 3.3.4)");
  }
  return b.sequence([issuer, b.integer(serial)]);
}
...
async function parse(bytes, opts) {
  ...
  if (recipientKey != null && carriesEnvelope && v.eContent != null) {
    ...
    if (messageType === "CertRep" && out.pkiStatus === "SUCCESS") {
      ...
      if (!iasCh || !pkix.rootSequenceChildren(iasCh[0], 0) || !pkix.rootSequenceChildren(iasCh[1], 0)) {
        throw _err("scep/bad-request-payload", "a CertPoll messageData must be an IssuerAndSubject: a SEQUENCE of two Name values (RFC 8894 sec. 3.3.3)");
      }
      ...
      if (!isnCh || !pkix.rootSequenceChildren(isnCh[0], 0) || isnCh[1].tagClass !== "universal" || isnCh[1].tagNumber !== asn1.TAGS.INTEGER) {
        throw _err("scep/bad-request-payload", "a " + messageType + " messageData must be an IssuerAndSerialNumber: a SEQUENCE of a Name and an INTEGER (RFC 8894 sec. 3.3.4)");
      }
    }
  }
  return out;
...
async function _pollForIssuance(ctx) {
  ...
  for (;;) {
    ...
    res = await _client("POST", ctx.postUrl, certPoll, { "content-type": CT_PKI_MESSAGE }, ctx.clientOpts);
...
async function _pkiOperation(messageType, baseUrl, opts, verbName) {
  ...
  return _pollForIssuance({
    firstResponse: res, firstSenderNonce: senderNonce, postUrl: postUrl, clientOpts: clientOpts,
...
async function _queryOperation(messageType, baseUrl, opts, verbName) {
  ...
  var clientOpts = { transport: opts.transport, tls: opts.tls != null ? Object.assign({}, opts.tls) : opts.tls, maxRedirects: opts.maxRedirects, allowCrossOriginRedirect: opts.allowCrossOriginRedirect, timeout: opts.timeout, maxResponseBytes: opts.maxResponseBytes };
  var senderNonce = nodeCrypto.randomBytes(16);
  var transactionId = opts.transactionId != null ? opts.transactionId : _defaultTransactionId();
  ...
  var res = await _client("POST", _scepUrl(baseUrl, "PKIOperation"), reqMsg, { "content-type": CT_PKI_MESSAGE }, clientOpts);
  var body = _requireOk(res, [CT_PKI_MESSAGE], "PKIOperation");
  var rep = await parse(body, { signerCert: signerCert, expectedSenderNonce: senderNonce, recipientKey: recipientKey });
 ...
 *   - `issuer` / `serialNumber` -- the issuer Name DER and serial (a BigInt or hex string) naming the certificate, for a caller that did not retain it.
 ...
 *   - `issuer` / `serialNumber` -- the issuer Name DER and serial naming the CA, when the caller did not retain such a certificate.
 *   - `caCert` -- the SCEP CA certificate DER the query is encrypted to.
 *   - `signer` -- `{ cert, key }`, the outer SignedData signer.
...
var _ENROLL_KEYS = _withKeys({ csr: 1, caCert: 1, signer: 1, recipientKey: 1, responderCert: 1, issuerCert: 1, transactionId: 1, pollCount: 1, maxTotalWait: 1, sleep: 1, onRetryAfter: 1 });
var _QUERY_KEYS = _withKeys({ certificate: 1, issuer: 1, serialNumber: 1, caCert: 1, signer: 1, recipientKey: 1, responderCert: 1, transactionId: 1 });
