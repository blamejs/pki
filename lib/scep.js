...
async function getCACaps(baseUrl, opts) {
  ...
  // as "no capabilities advertised": 400 (an old CA that does not parse the query), 404 (no such operation),
  // and 501 Not Implemented. A transient server error (500, 502, 503) is surfaced instead of masked.
  if (res.status === 204 || res.status === 404 || res.status === 400 || res.status === 501) { _assertStrongProfile({}, opts); return {}; }
