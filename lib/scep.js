...
function _buildIssuerAndSubject(recipientCertDer, requestSubjectSource, issuerOverride) {
  ...
  _assertNameNode(issuerNode, "scep/bad-input", "the CertPoll issuer must be an X.509 Name DER (RFC 8894 sec. 3.3.3)");
...
function _assertNameNode(node, code, message) {
  try { schema.walk(pkix.name(_KU_NS), node, _KU_NS); }
  catch (e) {
...
function _buildIssuerAndSerial(spec) {
  ...
  if (hasCert) {
    ...
    _assertNameNode(node, "scep/bad-input", "issuer must be an X.509 Name DER (RFC 8894 sec. 3.3.4)");
  }
...
async function parse(bytes, opts) {
  ...
  if (recipientKey != null && carriesEnvelope && v.eContent != null) {
    ...
    if (messageType === "CertRep" && out.pkiStatus === "SUCCESS") {
      ...
      _assertNameNode(iasCh[0], "scep/bad-request-payload", "a CertPoll messageData issuer must be an X.509 Name (RFC 8894 sec. 3.3.3)");
      ...
      _assertNameNode(isnCh[0], "scep/bad-request-payload", "a " + messageType + " messageData issuer must be an X.509 Name (RFC 8894 sec. 3.3.4)");
      var isnSerial;
