...
// A 16-byte nonce (RFC 8894 sec. 3.2.1.5): a caller value validated to width, or a fresh random one.
function _nonce(v, label) {
  if (v == null) return nodeCrypto.randomBytes(16);
  var buf = guard.bytes.view(v, ScepError, "scep/bad-nonce", label);
...
// Whether some bytes are a CMS ContentInfo wrapping a structurally well-formed EnvelopedData -- the
// pkcsPKIEnvelope shape (RFC 8894 sec. 3.2). Delegated to the strict CMS parser rather than a shallow
// tag probe, so a stub with the right content-type OID but a malformed or absent EnvelopedData is a
// non-match. The envelope is still decrypted separately with the recipient key. Any decode fault is a
// non-match, never a throw.
function _isEnvelopedData(bytes) {
  if (bytes == null) return false;
  try { return schemaCms.parse(bytes).contentTypeName === "envelopedData"; }
 ...
 *   - `recipient` -- the CA (or RA) certificate DER the messageData is encrypted to.
...
async function build(spec) {
  ...
  var parsedCsr;
  try { parsedCsr = schemaCsr.parse(spec.messageData); }
  ...
  var pkcsPKIEnvelope;
  try {
    pkcsPKIEnvelope = await cmsEncrypt.encrypt(spec.messageData, [{ cert: spec.recipient }], { contentEncryptionAlgorithm: ENVELOPE_CIPHER });
  } catch (e) {
  ...
  try {
    return await cmsSign.sign(pkcsPKIEnvelope, { cert: spec.signer.cert, key: spec.signer.key }, { additionalSignedAttributes: attrs });
  } catch (e) {
 ...
 * recipientNonce, pkiStatus, failInfo, failInfoText, messageData, certificates }` (fields null when the
 * message does not carry them). For a decrypted SUCCESS CertRep, `certificates` holds the issued
 * certificate(s) validated out of the certs-only messageData, raw for the caller to path-validate. A malformed message, a missing mandatory attribute, an unknown enumerant, or a
...
async function parse(bytes, opts) {
  ...
  var v;
  try { v = await cmsVerify.verify(bytes); }
  catch (e) {
    if (e && e.isPkiError) throw _err("scep/bad-der", "the pkiMessage is not a verifiable CMS SignedData (RFC 8894 sec. 3): " + e.message, e);
    throw e;
  }
  ...
  if (opts.signerCert != null) {
    var expectedCert = guard.bytes.view(opts.signerCert, ScepError, "scep/bad-input", "opts.signerCert");
    if (!_sameKey(signer.cert, expectedCert)) {
  ...
  if (opts.expectedSenderNonce != null) {
    var expected = _nonce(opts.expectedSenderNonce, "expectedSenderNonce");
    if (out.recipientNonce == null || !guard.crypto.constantTimeEqual(out.recipientNonce, expected)) {
  ...
  if (carriesEnvelope) {
    // For a message that carries messageData, the pkcsPKIEnvelope is mandatory and MUST be an
    // EnvelopedData (RFC 8894 sec. 3.2): a request or a SUCCESS CertRep whose signed content is absent
    // or is not an EnvelopedData is malformed and refused here, before any recipient-key decryption, so
    // it is never accepted as a valid message with a null messageData.
    if (!_isEnvelopedData(v.eContent)) {
      throw _err("scep/missing-envelope", "the pkcsPKIEnvelope is mandatory for a " + messageType + " message but the signed content is absent or is not an EnvelopedData (RFC 8894 sec. 3.2)");
    }
  } else {
  ...
  if (opts.recipientKey != null && carriesEnvelope && v.eContent != null) {
    var dec;
    try { dec = await cmsDecrypt.decrypt(v.eContent, opts.recipientKey); }
    catch (e) {
    ...
    if (messageType === "CertRep" && out.pkiStatus === "SUCCESS") {
      out.certificates = schemaCms.parseCertsOnly(dec.content, _err, "scep").certificates;
