...
async function build(spec) {
  ...
  // Snapshot the caller's messageData once at the door, so proof-of-possession verification and
  // encryption both apply to the same bytes: a mutable Buffer changed during the awaited verify cannot
  // desync the verified request from the enveloped one.
  ...
  var pkcsPKIEnvelope;
  try {
    pkcsPKIEnvelope = await cmsEncrypt.encrypt(messageData, [{ cert: spec.recipient }], { contentEncryptionAlgorithm: ENVELOPE_CIPHER });
  } catch (e) {
...
async function parse(bytes, opts) {
  ...
  try { v = await cmsVerify.verify(bytes); }
    ...
    if (e && e.code === "cms/detached-content-required") {
      try { v = await cmsVerify.verify(bytes, { content: EMPTY_CONTENT }); }
      catch (e2) {
