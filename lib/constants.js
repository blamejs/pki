...
function _positive(n, who) {
  if (typeof n !== "number" || !_isFinite(n) || n < 0) {
    throw new ConstantsError(
      "constants/bad-scale",
      who + ": expected a finite number >= 0, got " + _String(n)
    );
  }
  return n;
}
...
function _scale(n, who, factor) {
  ...
  if (!_isSafeInteger(out)) {
    throw new ConstantsError(
      "constants/bad-scale",
      who + ": the result " + _String(out) + " is not a safe integer"
    );
  }
  return out;
}
...
_keys(NAMES).forEach(function (k) { _freeze(NAMES[k]); });
_freeze(NAMES);
