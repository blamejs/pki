 ...
 *   // -> 31536000000
 */
var TIME = Object.assign(Object.create(null), {
 ...
 *   // -> 16777216
 */
var BYTES = Object.assign(Object.create(null), {
...
var LIMITS = Object.assign(Object.create(null), {
  DER_MAX_BYTES: BYTES.mib(16),
  DER_MAX_DEPTH: 64,
  DER_MAX_ITEMS: 4000000,
  NAMED_SCAN_MAX_ELEMENTS: 16777216,
  MAX_DECODE_DEPTH_CEILING: 256,
  PEM_MAX_BYTES: BYTES.mib(16),
  PEM_MAX_BLOCKS: 512,
  PEM_MAX_DECODED_BYTES: BYTES.mib(16),
  DER_MAX_INTEGER_BYTES: BYTES.kib(16),
  DH_MAX_MODULUS_BYTES: 512,
  DH_MIN_SUBGROUP_BYTES: 20,
  DH_MIN_MODULUS_BYTES: 256,
  RSA_MIN_MODULUS_BITS: 2048,
  DH_PRIME_MEMO_ENTRIES: 64,
  OID_MAX_SUBIDENTIFIER_BYTES: 32,
  OID_MAX_SUBIDENTIFIERS: 128,
  ...
  SCT_MAX_COUNT: 256,
  SAN_MAX_ENTRIES: 256,
  IDENTITY_MAX_REFERENCES: 64,
  REVOCATION_MAX_FETCHES: 12,
  REVOCATION_MAX_PER_CERT: 6,
  REVOCATION_MAX_RESPONSE_BYTES: BYTES.mib(8),
  REVOCATION_TOTAL_DEADLINE_MS: TIME.seconds(20),
  REVOCATION_SOURCE_TIMEOUT_MS: TIME.seconds(5),
  REVOCATION_CACHE_MAX_ENTRIES: 256,
  REVOCATION_CACHE_MAX_BYTES: BYTES.mib(32),
  TLOG_MAX_COUNT: 32,
  CMS_MAX_KARI_ENCRYPTED_KEYS: 64,
  CMP_KEM_MAX_KEY_BYTES: 1024,
  CMP_KEM_MIN_KEY_BYTES: 20,
  MERKLE_MAX_PROOF_NODES: 65,
  PROXY_MAX_TLS_ANCHORS: 256,
  PATH_MAX_CERTS: 100,
  ...
  PKCS12_MAX_DH_PAIRS: 8,
  PKCS12_MAX_REDECODES: 64,
  PKCS12_MAX_BAG_DEPTH: 16,
  CMS_MAX_COUNTERSIGNATURE_DEPTH: 16,
  ...
  CMP_MIN_NONCE_BYTES: 16,
  CMP_TRANSACTION_ID_BYTES: 16,
...
var NAMES = Object.assign(Object.create(null), {
  DN_SHORT: {
  ...
  EXTENSION: {
    ...
    subjectDirectoryAttributes: "X509v3 Subject Directory Attributes", ocspNoCheck: "OCSP No Check",
  },
  KEY_USAGE: {
  ...
  /** @internal The TLS ExtensionType values RFC 7633 sec. 4 names for the TLS feature extension. A
   *  value outside this table is printed as its number. */
  TLS_FEATURE: { "5": "status_request", "17": "status_request_v2" },
  /** @internal The IANA Address Family Numbers RFC 3779 sec. 2.2.3.3 names. */
  ADDRESS_FAMILY: { "1": "IPv4", "2": "IPv6" },
  ...
  /** @internal The output length in octets of each fixed-length digest, by the OID registry's name
   * (FIPS 180-4, FIPS 202). A structure that carries "the hash of X under algorithm A" is held to
   * this length: an OCSP CertID's issuer hashes, a timestamp's messageImprint. The XOFs (SHAKE) have
   * no single length and are absent. */
  DIGEST_OCTETS: { sha1: 20, sha256: 32, sha384: 48, sha512: 64, "sha3-256": 32, "sha3-512": 64 },
});

/** @internal Each inner table is looked up by a name a certificate supplies, so it answers from
 * its own entries alone rather than from whatever Object.prototype carries under that name. */
Object.keys(NAMES).forEach(function (k) {
  NAMES[k] = Object.freeze(Object.assign(Object.create(null), NAMES[k]));
});
Object.freeze(NAMES);
