...
// Parse an HTTP-date to epoch ms, or NaN when its grammar OR calendar is invalid.
// The calendar check is essential: V8's Date.parse NORMALIZES an impossible date
// (Feb 31 -> Mar 2) instead of returning NaN, so the day/month/year extracted from
// the string must agree with the parsed UTC date.
function _httpDateMs(s) {
  if (!HTTP_DATE_FORMS.some(function (re) { return re.test(s); })) return NaN;
  var when = Date.parse(s);
  if (isNaN(when)) return NaN;
  var day, monName, year;
  var asc = /^\w{3} (\w{3}) ([ \d]\d) \d{2}:\d{2}:\d{2} (\d{4})$/.exec(s);   // asctime: month day year
  if (asc) { monName = asc[1]; day = parseInt(asc[2], 10); year = parseInt(asc[3], 10); }
  else {
  ...
  var d = new Date(when);
  if (mon < 0 || d.getUTCDate() !== day || d.getUTCMonth() !== mon || d.getUTCFullYear() !== year) return NaN;
  return when;
}
