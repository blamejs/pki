...
function _recipientMatches(recipientInfos, opts) {
  ...
  if (ias && Buffer.isBuffer(ias.issuer) && ias.serialNumber != null) {
    ...
    if (typeof sn === "bigint") want = sn;
    else if (typeof sn === "number" && Number.isSafeInteger(sn) && sn >= 0) want = BigInt(sn);
    else if (typeof sn === "string" && /^[0-9]+$/.test(sn)) want = BigInt(sn);
    else throw E("est/bad-input", "expectedRecipientIssuerSerial.serialNumber must be a bigint, a safe non-negative integer, or a decimal digit string");
    ...
    return transport({ method: method, url: url.href, headers: _headersFor(url), body: body, tls: _tlsFor(url), timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      ...
      if (status === 401) {
        ...
        if (scheme === "digest") {
          ...
          var ch = httpDigest.parseChallenge(www, E, "est/digest-bad-challenge");
          if (!ch) throw E("est/auth-required", "the server offered no usable Digest challenge: " + www);
          // A fresh nonce (stale=true) permits a BOUNDED re-answer without re-prompting (RFC 7616 sec. 3.3);
          // any other second 401 after a credentialed attempt terminates fail-closed.
          if (authTried) {
            // A stale=true re-challenge MUST carry a FRESH nonce (RFC 7616 sec. 3.3); a repeated nonce with
            // stale=true is a self-contradictory / hostile server -> terminate rather than re-answer the same nonce.
          ...
          // The Digest response is bound to THIS request-target (op path incl. label) + nonce; _headersFor
          // attaches it only on the initial origin (the same request step() re-sends), so the uri matches.
          authValue = httpDigest.answer(ch, {
            method: method, uri: url.pathname + url.search, username: _authUser(opts), password: _authPass(opts), body: body,
            policy: { allowMD5: !!(opts.auth && opts.auth.allowMD5), allowLegacyQop: !!(opts.auth && opts.auth.allowLegacyQop), codes: DIGEST_CODES },
          }, E);
          return step();
        }
...
function _serverkeygenEncryptionFromCsr(csrDer) {
  ...
  return { requestedEncryption: keyId !== null, expectedRecipientKeyId: keyId };
}
...
function _assertConfidentialCipher(res) {
  ...
  if (/NULL|ANON|EXPORT|\bEXP\b|\bA(EC)?DH\b/.test(name)) throw E("est/weak-cipher", "the serverkeygen channel negotiated a NULL / anonymous / EXPORT cipher (" + (c.standardName || c.name) + "), which cannot protect the delivered private key (RFC 7030 sec. 4.4)");
}

function _serverkeygenResult(res, opts, derived) {
  ...
  return parseServerKeygenResponse(res.body, res.headers["content-type"], {
 ...
 * the key so the issued certificate's public key is the generated one, not the throwaway CSR key.
...
function serverkeygen(baseUrl, csrInput, opts) {
  ...
  return Promise.resolve().then(function () {
    ...
    if (opts.expectedRecipientIssuerSerial != null) {
      ...
      if (!(typeof s === "bigint" || (typeof s === "number" && Number.isSafeInteger(s) && s >= 0) || (typeof s === "string" && /^[0-9]+$/.test(s)))) throw E("est/bad-input", "opts.expectedRecipientIssuerSerial.serialNumber must be a bigint, a safe non-negative integer, or a decimal digit string");
    }
