...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function _headersFor(u) {
    ...
    if (digestChallenge) {
      ...
      // resource's own challenge (a different Digest realm) drives a fresh authentication instead of receiving
      // an Authorization computed for the wrong realm.
      if (!httpDigest.inProtectionSpace(digestChallenge, u.pathname)) return headers;
      digestNc += 1;
    ...
    return transport({ method: method, url: url.href, headers: _headersFor(url), body: body, tls: _tlsFor(url), timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      ...
      if (status === 401) {
        ...
        if (scheme === "digest") {
          ...
          // A second 401 is interpreted by protection space (RFC 7616 sec. 3.3 / 3.5). A DIFFERENT realm is a
          // NEW protection space -- a same-origin resource with its own Digest realm, not a rejection of this
          // space's credential -- so it is answered as a fresh authentication, bounded (maxRedirects) so a
          // server cannot loop with an endless stream of new realms. A SAME-realm second 401 is a rejection
          // unless it is a stale=true re-challenge carrying a FRESH nonce (a repeated nonce with stale=true is a
          // self-contradictory / hostile server -> terminate rather than re-answer the same nonce).
          if (authTried && digestChallenge && ch.realm === digestChallenge.realm) {
            ...
            if (authSpaces >= budgets.maxRedirects) throw E("est/auth-required", "the server demanded Digest authentication for too many distinct protection spaces (RFC 7616 sec. 3.3)");
            authSpaces += 1;
            staleRetries = 0;   // a newly entered protection space restarts its own stale-retry budget
          } else { authTried = true; }
