...
function classifyResponse(status, headers, body, opts) {
  ...
  if (status === 200) {
    ...
    if (spec) {
      ...
      var smimeOk = !spec.smimeTypes || spec.smimeTypes.indexOf(pt.smimeType) !== -1;
 ...
 * before the exchange reads as `issued`. They are surfaced as `issuedCertificates` (in request
 * order, with `certificate` the first), distinct from `certificates`, which is the whole returned
 * bag including any chain. A bag that answers only some of the requests, or none, is a refusal
 * rather than a partial success. That arm reports `signatureVerified: false`: a certs-only body is
 ...
 *   - `responderCerts` -- certificates for CMC signer lookup; the carrier's signature MUST be verified
 *     (RFC 5272 sec. 3.2.1.3.4), so a `CMC-response` without either this or the opt-out below is refused.
 *   - `allowUnverifiedResponse` -- interpret a `CMC-response` without checking its signature. The
 *     verdict then reports `signatureVerified: false`; for an unauthenticated bootstrap only.
...
function fullcmc(baseUrl, request, opts) {
  ...
  return Promise.resolve().then(function () {
    ...
    var der = _cmcRequestDer(request);
    // Read the submitted request's own keys BEFORE sending: they are what a
    // certs-only answer has to be correlated against, and the request is the only
    // place they exist.
    var wanted = _requestedPublicKeys(der);
    ...
    return _client("fullcmc", "POST", baseUrl, body,
      ...
      .then(function (res) { return _fullcmcResult(res, opts, wanted); });
  });
}
...
function _requestedPublicKeys(der) {
  var out = [], body;
  try { body = cmcFmt.parse(der); } catch (_e) { return out; }
  (body.requests || []).forEach(function (r) {
    ...
    } catch (_e) { /* an arm this layer cannot read contributes no key to match against */ }
  });
  return out;
}
...
function _cmcSent(opts) {
  return {
    transactionId: opts.transactionId, senderNonce: opts.senderNonce, dataReturn: opts.dataReturn,
    certs: opts.responderCerts, allowUnverified: opts.allowUnverifiedResponse === true,
  };
}
...
function _cmcRequestDer(request) {
  if (Buffer.isBuffer(request)) return request;
...
// reported as itself.
function _fullcmcResult(res, opts, wanted) {
  ...
  try {
    ...
    return _tryDecodeCmcFault(res, opts).then(function (cmcErr) { throw cmcErr || httpFault; });
  }
  ...
  var smimeType = _partMediaType(_ciHeader(res.headers, "content-type")).smimeType;
  if (smimeType === "certs-only") {
    ...
    // EVERY requested key, not just one. A PKIData may carry several certification
    // requests, and a bag answering one of them leaves the rest unanswered -- so
    // reporting the exchange as issued off the first match would call a partial
    // enrolment a complete one.
    var issuedCerts = [];
    for (var wi = 0; wi < wanted.length; wi++) {
      var one = findIssuedCert(certs.certificates, wanted[wi]);
      if (one === null) {
        throw E("est/no-issued-cert",
          "the /fullcmc certs-only response answered " + issuedCerts.length + " of " + wanted.length +
          " certification request(s): it carried no certificate for a public key the request asked to have " +
          "certified, so it does not answer this request (RFC 5272 sec. 4.1)");
      }
      if (findIssuedCert(certs.certificates.filter(function (c) { return c !== one; }), wanted[wi])) {
        throw E("est/ambiguous-issued-cert",
          "the /fullcmc response carried more than one certificate matching a submitted request key; the issued certificate is ambiguous (RFC 5272 sec. 4.1)");
      }
      issuedCerts.push(one);
    }
    if (!issuedCerts.length) {
      // No key could be read from the request at all, so there is nothing a
      // returned certificate could be tied to. An unattributable bag is not an
      // issuance verdict, whatever it contains.
      throw E("est/no-issued-cert",
        "this /fullcmc request declared no certification request whose key a returned certificate could be matched against, so a certs-only response cannot be read as an issuance (RFC 5272 sec. 4.1)");
    }
  ...
  return cmcVerify.verify(der, _cmcSent(opts));
}
...
// going to parse.
function _tryDecodeCmcFault(res, opts) {
  return Promise.resolve().then(function () {
    ...
    return cmcVerify.verify(transferDecode(res.body), _cmcSent(opts)).then(function (verdict) {
