...
// Does WWW-Authenticate advertise a Basic challenge? Matched as an auth-scheme TOKEN (at the start
// or after a comma-separated challenge, followed by whitespace / a comma / end), never a substring --
// so `Digest realm="basic"` or `Bearer error="basic required"` is NOT taken as a Basic challenge.
function _hasBasicChallenge(www) {
  return /(?:^|,)\s*Basic(?=[\s,]|$)/i.test(String(www || ""));
}
...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function step() {
    return transport({ method: method, url: url.href, headers: headers, body: body, tls: budgets.tls, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      res = res || {};
      ...
      if (status >= 300 && status < 400) {
        ...
        url = _redirectTarget(url, (res.headers || {}).location, method, opts);
        // Never carry HTTP credentials across an origin boundary -- a redirect to a different
        // web origin (even a permitted cross-origin GET) drops the Authorization header so a
        // prior 401's Basic credentials are not leaked to another server.
        if (url.origin !== prevOrigin && headers.authorization) {
      ...
      if (status === 401) {
        ...
        var www = String((res.headers || {})["www-authenticate"] || "");
...
function _client(op, method, baseUrl, body, headers, opts) {
  ...
  if (!transport) {
    ...
    if (!hasAnchors && !t.useSystemStore) throw E("est/no-trust-anchors", "no explicit trust anchor and tls.useSystemStore not set -- refusing an unpinned server (RFC 7030 sec. 3.6)");
...
function _certsResult(op, res, opts, csrSpki) {
  ...
  var chain = parsed.certificates.filter(function (c) { return !c.equals(issued); });
