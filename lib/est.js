...
// The byte length of a transport response body for the size-cap and empty-body checks: a null / absent body
// is 0, a string its UTF-8 byte length, and any BufferSource (a Buffer / Uint8Array / ArrayBuffer / DataView
// from an injected transport) its byte count through the byte guard -- never a String()-coerced comma list.
// res.body is read ONCE so a lying getter on an injected response cannot change it between the arms.
