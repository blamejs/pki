...
function _optsWithProxy(opts) {
  if (opts.proxy === undefined || opts.proxy === null) return opts;
  var out = intrinsic.assign({}, opts);
  out.proxy = httpTransport.snapshotProxy(opts.proxy);
  return out;
}
...
function cacerts(baseUrl, opts) {
  opts = opts || {};
  try { opts = _optsWithProxy(opts); } catch (_pe) { return _rej(_pe); }
  return Promise.resolve().then(function () {
