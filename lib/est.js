...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function step() {
    return transport({ method: method, url: url.href, headers: headers, body: body, tls: _tlsFor(url), timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      res = res || {};
      ...
      if (status >= 300 && status < 400) {
        ...
        // permitted cross-origin GET) drops the HTTP Basic Authorization header; the ORIGIN-SPECIFIC TLS
        // identity (mTLS cert/key + pinned SNI) is scoped per hop by _tlsFor, so it is likewise absent
        // off-origin but restored on a hop back to the caller's configured origin.
        if (url.origin !== prevOrigin && headers.authorization) {
          headers = Object.assign({}, headers); delete headers.authorization;
        }
        redirects += 1;
        return step();
      }
