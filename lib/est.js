...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function step() {
    if (typeof body === "function" && budgets.onAttempt) budgets.onAttempt();
    return transport({ method: method, url: url.href, headers: _headersFor(url), body: body, tls: _tlsFor(url), proxy: budgets.proxy, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      res = res || {};
...
function _enrollBound(op, baseUrl, builder, opts, proxySnap) {
  ...
  return _client(op, "POST", baseUrl, boundBody, ENROLL_HEADERS, opts, proxySnap, function () { current = null; })
    .then(function (res) {
