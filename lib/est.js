...
function _client(op, method, baseUrl, body, headers, opts, proxySnap) {
  var base;
...
function _enrollBound(op, baseUrl, builder, opts, proxySnap) {
  ...
  return _client(op, "POST", baseUrl, boundBody, ENROLL_HEADERS, opts, proxySnap)
    .then(function (res) {
 ...
 *     { transport: function () { return Promise.resolve({ status: 202, headers: { "retry-after": "60" }, body: "" }); } });
 *   r.retry && r.retryAfterSeconds;   // 60
 *   // channel binding: a builder signs the CSR over the connection's own tls-unique
