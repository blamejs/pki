  ...
  function step() {
    return transport({ method: method, url: url.href, headers: headers, body: body, tls: tls, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      ...
      if (status >= 300 && status < 400) {
        ...
        // TLS identity: the mTLS client cert/key (a prior 401's password + the caller's certificate and
        // proof of private-key possession) and the pinned servername (SNI) / checkServerIdentity (the
        // RFC 6125 verifier) -- both authenticate the trusted host, so applying them to a different
        // redirected origin authenticates against the wrong server. Stripped even with no cert/key set.
        if (url.origin !== prevOrigin) {
          if (headers.authorization) { headers = Object.assign({}, headers); delete headers.authorization; }
          if (tls && (tls.cert || tls.key || tls.servername || tls.checkServerIdentity)) {
            tls = Object.assign({}, tls);
            delete tls.cert; delete tls.key; delete tls.servername; delete tls.checkServerIdentity;
          }
        }
        redirects += 1;
        return step();
