...
function buildEnrollAttributes(csrattrsParsed) {
  ...
  for (var j = 0; j < items.length; j++) {
    ...
    else if (it.kind === "attribute" && (it.oid === OID_EC_PUBLIC_KEY || it.oid === OID_RSA_ENCRYPTION)) plan.keyType = { type: it.name, curve: it.curve || null, keySize: it.keySize || null };
...
// (RFC 2985 sec. 5.4.2), decoded by the CSR parser, or null when absent.
function _csrRequestedExtensions(parsedCsr) {
  ...
  for (var i = 0; i < attrs.length; i++) {
    if (attrs[i].type === extReqOid && attrs[i].extensions) return attrs[i].extensions;
  }
  return null;
}

// A re-enroll CSR MUST reuse the old certificate's Subject (by DN) and
// SubjectAltName extension (byte-identically) (RFC 7030 sec. 4.2.2). Returns the
// old cert's subject + SAN for verbatim reuse in the new CSR; on a
// caller-supplied new CSR, compares both and throws est/reenroll-subject-mismatch
// or est/reenroll-san-mismatch on any divergence (a SAN present on one side and
