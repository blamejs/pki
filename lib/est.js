...
// ---- P7: the RFC 8951 sec. 3/3.1 transfer codec (CTE-header-blind) --------
...
// ---- P15: the multipart/mixed splitter (/serverkeygen) -------------------
...
// ---- P1-P5: the certs-only Simple PKI Response validator -----------------
...
// encrypted (a cleartext part -> est/expected-encrypted-key).
function parseServerKeygenResponse(body, contentType, opts) {
  ...
  if (encrypted) {
    // The EnvelopedData is surfaced structurally; the ciphertext stays opaque
    // and the RecipientInfo arms feed the P17 coherence check the caller makes.
    out.encryptedKey = cms.parse(transferDecode(keyPart.body));
  } else {
...
// ---- P21: the HTTP response classifier ----------------------------------
...
function paths(baseUrl, opts) {
  ...
  if (opts.label != null) {
    ...
    if (label === "" || label.indexOf("/") !== -1 || OPERATIONS.indexOf(label) !== -1) {
      throw E("est/bad-label", "an EST CA label must be non-empty, contain no '/', and not equal an operation name (RFC 7030 sec. 3.2.2)");
    }
    prefix += "/" + label;
  }
  var out = {};
...
// P19: the RFC 5929 tls-unique channel-binding bytes -> a challengePassword
...
// P18: the out-of-band key-encryption-key identifiers (OCTET STRING values).
...
// P18: SMIMECapabilities ::= SEQUENCE OF SMIMECapability { capabilityID OID,
...
// P14 + P10: derive the enroll-request attribute plan from a parsed CsrAttrs.
...
function buildEnrollAttributes(csrattrsParsed) {
  ...
  for (var j = 0; j < items.length; j++) {
    ...
    else if (it.kind === "attribute" && (it.curve || it.keySize)) plan.keyType = { curve: it.curve || null, keySize: it.keySize || null };
    else if (it.name === null) plan.unrecognized.push(it.oid);
  }
  return plan;
}

// P6: a re-enroll CSR MUST reuse the old certificate's Subject and SAN extension
// byte-identically (RFC 7030 sec. 4.2.2). Returns the old cert's raw subject +
// SAN bytes for verbatim reuse in the new CSR; on a caller-supplied new CSR,
// compares them and throws est/reenroll-subject-mismatch on any divergence.
function reenrollGuard(oldCertDer, newCsrDer) {
  ...
  if (newCsrDer === undefined) return { subjectDn: oldSubjectDn, subject: oldSubject };
  var parsedCsr = csr.parse(newCsrDer);
  if (parsedCsr.subject.dn !== oldSubjectDn) throw E("est/reenroll-subject-mismatch", "a re-enroll CSR subject must be identical to the certificate being renewed (RFC 7030 sec. 4.2.2)");
  return { subjectDn: oldSubjectDn };
}
