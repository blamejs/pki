...
async function _serverkeygenResult(res, opts, derived) {
  ...
  if (out.privateKeyDer) {
     ...
     * server could set to the certificate's beside private material that cannot use it. */
    var pair;
 ...
 * (`est/key-cert-mismatch` when it is not), so a mis-associated or unusable key is refused, never handed back.
