...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function _headersFor(u) {
    ...
    if (digestChallenge) {
      digestNc += 1;
      hh.authorization = httpDigest.answer(digestChallenge, { method: method, uri: u.pathname + u.search, username: _authUser(opts), password: _authPass(opts), body: body, nc: digestNc, policy: { allowMD5: !!(opts.auth && opts.auth.allowMD5), allowLegacyQop: !!(opts.auth && opts.auth.allowLegacyQop), codes: DIGEST_CODES } }, E);
    ...
    return transport({ method: method, url: url.href, headers: _headersFor(url), body: body, tls: _tlsFor(url), timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      ...
      if (status === 401) {
        ...
        if (scheme === "digest") {
          ...
          digestChallenge = ch;
          digestNc = 0;   // a fresh nonce (initial or a stale re-challenge) restarts the nonce-count at 1 on the next send
          return step();
        }
...
function _serverkeygenResult(res, opts, derived) {
  ...
  return parseServerKeygenResponse(res.body, _ciHeader(res.headers, "content-type"), {
