 ...
 * bag that answers only some of them, or none, is a refusal rather than a partial success. They are
...
function fullcmc(baseUrl, request, opts) {
  ...
  try {
    ...
    // tls, credentials, redirect policy -- and reading them a turn later would let
    // an options object reused between calls redirect a request already begun.
...
// Own enumerable properties, one level. Enough to stop a reused options object
// being re-pointed after the call; the values themselves (a transport function, a
// tls config) stay the caller's, which is what they are meant to be.
