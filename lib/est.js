...
/** @internal A transport that is not callable is a config-time fault, so it is refused at the door,
 * before any payload work, and again where the value is used. A falsy one is the same fault: reading
 * it as "none supplied" would fall back to the default client and open a real connection for a
 * caller that believed it had injected one. */
function _assertTransport(transport) {
  if (transport != null && typeof transport !== "function") {
    throw E("est/bad-input", "opts.transport must be a function (request) => Promise<{ status, headers, body }>, got " + guard.text.showValue(transport));
  }
}
...
function _knownOpts(opts, known, verb) {
  ...
  _assertTransport(opts.transport);
}
...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function step() {
    ...
    return guard.async.awaited(transport({ method: method, url: url.href, headers: _headersFor(url), body: hopBody, tls: _tlsFor(url), proxy: budgets.proxy, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }), E, "est/bad-input", "opts.transport").then(function (res) {
      res = res || {};
