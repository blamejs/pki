...
function fullcmc(baseUrl, request, opts) {
  ...
  try {
    ...
    /** @internal Carried across the copy below, which keeps own properties only. A defaults bag
     * holds its options on a prototype, so the copy would drop the transport the caller supplied
     * and send the request to the default client instead. */
