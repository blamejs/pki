...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function _headersFor(u) {
    ...
    hh.authorization = digestChallenge
      ? httpDigest.answer(digestChallenge, { method: method, uri: u.pathname + u.search, username: _authUser(opts), password: _authPass(opts), body: body, policy: { allowMD5: !!(opts.auth && opts.auth.allowMD5), allowLegacyQop: !!(opts.auth && opts.auth.allowLegacyQop), codes: DIGEST_CODES } }, E)
      : authValue;
    return hh;
  }
  function step() {
