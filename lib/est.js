...
function classifyResponse(status, headers, body, opts) {
  ...
  if (status === 202) {
    ...
    if (/^\d+$/.test(raStr)) {
      ...
      if (isNaN(when)) throw E("est/bad-retry-after", "a 202 Retry-After must be delay-seconds or an HTTP-date (RFC 7231 sec. 7.1.3), got " + JSON.stringify(raStr));
      out202.retryAfterDate = when;
      if (typeof opts.now === "number" && isFinite(opts.now)) out202.retryAfterSeconds = Math.max(0, Math.round((when - opts.now) / constants.TIME.seconds(1)));
    }
    return out202;
  }
