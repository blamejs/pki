  ...
  function step() {
    return transport({ method: method, url: url.href, headers: headers, body: body, tls: tls, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }).then(function (res) {
      ...
      if (status >= 300 && status < 400) {
        ...
        // permitted cross-origin GET) drops the HTTP Basic Authorization header AND every ORIGIN-SPECIFIC
        // TLS identity: the mTLS client cert/key (a prior 401's password + the caller's certificate and proof
        // of private-key possession) and the pinned servername (SNI, which selects the trusted host's cert).
        // The caller's checkServerIdentity is RETAINED: it is an ADDITIONAL tightening constraint (a
        // certificate / SPKI pin, or a stricter host check) that node re-evaluates against the REDIRECTED
        // host, so a pin keeps applying and a host-pinned check fails the wrong host closed -- dropping it
        // would let the redirected host be accepted under only the default hostname validation, silently
        // bypassing the caller's pin.
        if (url.origin !== prevOrigin) {
          if (headers.authorization) { headers = Object.assign({}, headers); delete headers.authorization; }
          if (tls && (tls.cert || tls.key || tls.servername)) {
            tls = Object.assign({}, tls);
            delete tls.cert; delete tls.key; delete tls.servername;
          }
        }
        redirects += 1;
        return step();
