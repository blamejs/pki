 ...
 *   refused, a 202 Retry-After SURFACED and never slept, HTTP Basic or Digest answered
 *   only after the transport authenticated the server, and the issued certificate chosen
...
var CLIENT_OPTS = intrinsic.assign(intrinsic.create(null), {
  transport: 1, tls: 1, label: 1, timeout: 1, maxResponseBytes: 1, maxRedirects: 1, now: 1,
  auth: 1, username: 1, password: 1, allowCrossOriginRedirect: 1, proxy: 1,
});
...
var CLASSIFY_OPTS = intrinsic.assign(intrinsic.create(null), { op: 1, now: 1 });
var PATHS_OPTS = intrinsic.assign(intrinsic.create(null), { label: 1 });
var PARSE_SERVERKEYGEN_OPTS = intrinsic.assign(intrinsic.create(null), {
...
/** @internal Refused at the door, before any payload work, and again where the value is used. The
 * checked value is RETURNED and it is that one the caller uses: reading the option again would ask
 * a second time, and an accessor can answer differently on each read, so a value that passed here
 * need not be the value that gets called. */
function _assertTransport(opts) {
  return guard.identifier.assertCallableOption(opts, "transport", E, "est/bad-input", "opts.transport", TRANSPORT_SHAPE);
}

/** @internal Settled first, as the sibling clients do: an options object whose reads change what it
 * carries is refused here rather than answering one way to the check and another to the use, which
 * is how a transport that passed could be replaced before it is called. */
function _knownOpts(opts, known, verb) {
  var settled = guard.identifier.optionsObject(opts, E, "est/bad-input", "pki.est." + verb + " options");
  guard.identifier.assertKnownKeys(settled, known, E, "est/bad-input", function (k) {
    return "unknown option " + JSON.stringify(k) + " for pki.est." + verb + " -- accepted: " +
      Object.keys(known).sort().join(", ");
  });
  _assertTransport(settled);
  return settled;
}

function _snapProxy(opts) { return httpTransport.snapshotProxy(opts.proxy); }
function _rej(e) { return Promise.reject(e); }
...
function parseServerKeygenResponse(body, contentType, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, PARSE_SERVERKEYGEN_OPTS, "parseServerKeygenResponse");
...
var CONTENT_TYPE_BY_OP = intrinsic.assign(intrinsic.create(null), {
...
var NOT_IMPLEMENTED_OPS = intrinsic.assign(intrinsic.create(null), { fullcmc: 1 });
...
function classifyResponse(status, headers, body, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, CLASSIFY_OPTS, "classifyResponse");
  var op = opts.op;
  ...
  var h = intrinsic.assign(intrinsic.create(null), {
...
function paths(baseUrl, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, PATHS_OPTS, "paths");
...
/**
 * @primitive  pki.est.challengePasswordFromTlsUnique
 * @signature  pki.est.challengePasswordFromTlsUnique(channelBinding) -> Buffer
 * @since      0.6.31
 * @originated 0.1.24
 * @status     stable
 * @spec       RFC 7030, RFC 5929, RFC 2985
 * @related    pki.csr.sign, pki.transport.https, pki.est.simpleenroll
 *
 * Encode RFC 5929 tls-unique channel-binding bytes as the RFC 7030 sec. 3.5
 * challengePassword, binding an enrollment request to the TLS session it is sent over.
 * The returned Buffer is a DER `Attribute` (`SEQUENCE { type OID, SET { PrintableString } }`)
 * carrying the base64 of the binding, for a caller assembling certification-request
 * attributes directly. Empty or non-byte input is `est/bad-input`; a base64 value longer
 * than 255 octets is `est/tls-unique-too-long`.
 *
 * To sign a channel-bound request with `pki.csr.sign`, pass the base64 string as
 * `spec.challengePassword` instead: that signer builds this same attribute itself and
 * REFUSES a pre-encoded challengePassword in `spec.attributes` (`csr/bad-input`), so the
 * two forms are alternatives, never combined. `pki.transport.https` supplies the binding
 * as `tls.tlsUnique`, and a request body given as a function receives it before the body
 * is written, which is the point at which the request can still be signed over it.
 *
 * @example
 *   // the binding comes from the connection; pki.transport surfaces it as tls.tlsUnique
 *   var binding = Buffer.from("9f86d081884c7d65", "hex");
 *   var attr = pki.est.challengePasswordFromTlsUnique(binding);
 *   attr[0] === 0x30;                                  // a DER Attribute SEQUENCE
 *   // to SIGN a channel-bound request, hand pki.csr.sign the same value as a string
 *   // via spec.challengePassword; it builds this identical attribute itself.
 *   var challengePassword = binding.toString("base64");
 */
...
var DIGEST_CODES = intrinsic.assign(intrinsic.create(null), { unsupportedAlgorithm: "est/digest-unsupported-algorithm", weakAlgorithm: "est/digest-weak-algorithm", noQop: "est/digest-no-qop", badChallenge: "est/digest-bad-challenge" });
...
function _drive(method, url, body, headers, opts, transport, budgets) {
  ...
  function _headersFor(u) {
    ...
    var hh = guard.identifier.ownOptions(headers);
    if (digestChallenge) {
  ...
  function step() {
    var hopBody = (typeof body === "function" && budgets.onAttempt) ? budgets.onAttempt() : body;
    return guard.async.invoked(transport, [{ method: method, url: url.href, headers: _headersFor(url), body: hopBody, tls: _tlsFor(url), proxy: budgets.proxy, timeout: budgets.timeout, maxResponseBytes: budgets.maxResponseBytes }], E, "est/bad-input", "opts.transport").then(function (res) {
      res = res || {};
      ...
      if (status >= 300 && status < 400) {
        ...
        if (status === 303 && method !== "GET" && method !== "HEAD") {
          method = "GET";
          body = null;
          if (headers["content-type"]) { headers = guard.identifier.ownOptions(headers); delete headers["content-type"]; }
        }
...
/** @internal `checked` is the transport the verb already took at its door. Taking it again here
 * would ask the caller's options a second time, and anything read in between could change the
 * answer, so a verb that holds one passes it and this reads the option only when none was held. */
function _client(op, method, baseUrl, body, headers, opts, proxySnap, onAttempt, checked) {
  var base;
  try { base = new URL(String(baseUrl)); }
  catch (e) { throw E("est/bad-url", "the EST base URL did not parse: " + guard.text.showValue(baseUrl), e); }
  ...
  var transport = checked !== undefined ? checked : _assertTransport(opts);
  if (!transport) {
    var t = opts.tls || {};
  ...
  var budgets = {
    ...
    onAttempt: typeof onAttempt === "function" ? onAttempt : null,
  };
  _authScheme(opts);
  return _drive(method, url, body, guard.identifier.ownOptions(headers), opts, transport, budgets);
}
 ...
 *
 * @opts
 *   - `transport` / `tls` / `label` / `timeout` / `maxResponseBytes` / `maxRedirects` / `proxy` / `now`, as in pki.est.cacerts.
...
function fullcmc(baseUrl, request, opts) {
  ...
  try {
    ...
    opts = _knownOpts(opts, FULLCMC_OPTS, "fullcmc");
    /** @internal Held from the door, before the request is read. What follows traverses the
     * caller's nested objects, and one of those reads must not be able to change which transport
     * the request goes out on, so the value that was checked is the value carried below. */
    var carriedTransport = opts.transport;
    var proxySnap;
    try { proxySnap = _snapProxy(opts); } catch (_pe) { throw E("est/bad-input", "the options object could not be read", _pe); }
    der = _cmcRequestDer(request);
    wanted = _requestedPublicKeys(der);
    sent = _cmcSent(opts, der);
    /** @internal The copy below keeps own properties only, and a defaults bag holds its options on
     * a prototype, so the transport is put back from the value held at the door. */
    opts = _shallowCopy(opts);
    if (carriedTransport !== undefined) opts.transport = carriedTransport;
    var body = transferEncode(der);
    return _client("fullcmc", "POST", baseUrl, body,
      { accept: "application/pkcs7-mime", "content-type": "application/pkcs7-mime; smime-type=CMC-request" }, opts, proxySnap, undefined, carriedTransport)
...
function _correlateIssued(bag, wanted, arm) {
  ...
  var byKey = intrinsic.create(null);
...
function _cmcSent(opts, der) {
  ...
  return {
    ...
    certs: Array.isArray(opts.responderCerts) ? guard.list.copyMap(opts.responderCerts, _copyBytes) : opts.responderCerts,
...
function _requestBinding(der) {
  var out = intrinsic.assign(intrinsic.create(null), { transactionId: undefined, senderNonce: undefined, dataReturn: undefined, bodyPartIDs: undefined,
...
function _copyRecipient(r) {
  ...
  var out = intrinsic.create(null);
  var rNames = guard.identifier.optionNames(r);
  for (var ri = 0; ri < rNames.length; ri++) out[rNames[ri]] = _copyBytes(r[rNames[ri]]);
  return out;
}

function _shallowCopy(o) {
  var out = guard.identifier.ownOptions(o);
  if (out.auth && typeof out.auth === "object") {
    out.auth = guard.identifier.ownOptions(out.auth);
  }
  return out;
}
...
function _isTypedError(e) {
  try { return !!(e && e.isPkiError === true); }
  catch (_re) { return false; }
}

function _causedBy(e, target) {
...
var ENROLL_HEADERS = intrinsic.assign(intrinsic.create(null), { accept: "application/pkcs7-mime", "content-type": "application/pkcs10" });

function _enrollBound(op, baseUrl, builder, opts, proxySnap) {
  /** @internal Held before anything else the caller supplied is read, and carried to the client:
   * the builder runs during the handshake, so a later read could answer with a different value. */
  var checkedTransport = opts.transport;
  if (_authScheme(opts) === "digest") {
    throw E("est/channel-binding-digest-unsupported",
      "Digest authentication cannot be combined with a channel-bound CSR builder: the Authorization header " +
      "is computed over the request body before the handshake, while the bound body exists only after it " +
      "(RFC 7030 sec. 3.5). Use Basic authentication or a pre-built CSR.");
  }
  var current = null;
  function hopBody() {
    var attempt = { spki: null, error: null, done: false };
    current = attempt;
    return function (channel) {
      return guard.async.deferred(function () { return builder(channel); })
        .then(null, function (e) {
          if (_isTypedError(e)) throw e;
          throw E("est/csr-builder-failed", "the CSR builder threw", e);
        })
        .then(function (built) {
          var der = _csrDer(built);
          if (op === "simplereenroll") reenrollGuard(opts.oldCert, der);
          attempt.spki = csr.parse(der).subjectPublicKeyInfo;
          attempt.done = true;
          return transferEncode(der);
        })
        .then(null, function (e) { attempt.error = e; attempt.done = true; throw e; });
    };
  }
  return _client(op, "POST", baseUrl, hopBody, ENROLL_HEADERS, opts, proxySnap, hopBody, checkedTransport)
    .then(function (res) {
      if (current && current.error) throw current.error;
      if (!current || !current.done) {
        throw E("est/csr-builder-failed",
          "the response was accepted without a completed channel-bound certification request, so no CSR " +
          "was sent on this attempt (RFC 7030 sec. 3.5); a transport must request the body and await it");
      }
      return _certsResult(op, res, opts, current.spki);
    },
    function (e) {
      var failed = current && current.error;
      throw _causedBy(e, failed) ? failed : e;
    });
}

function _enroll(op, baseUrl, csrInput, opts, proxySnap) {
  if (typeof csrInput === "function") return _enrollBound(op, baseUrl, csrInput, opts, proxySnap);
  var checkedTransport = opts.transport;
  var csrDer = _csrDer(csrInput);
  var spki = csr.parse(csrDer).subjectPublicKeyInfo;
  var body = transferEncode(csrDer);
  return _client(op, "POST", baseUrl, body, ENROLL_HEADERS, opts, proxySnap, undefined, checkedTransport)
 ...
 *
 * @opts
 *   - `transport`: an injected `transport(request) -> Promise<{status, headers, body, tls}>`; default pki.transport.https. It must return a promise of the response; anything else is `est/bad-input`.
...
function cacerts(baseUrl, opts) {
  opts = opts || {};
  var proxySnap; try { proxySnap = _snapProxy(opts); } catch (_pe) { return _rej(E("est/bad-input", "the options object could not be read", _pe)); }
  return Promise.resolve().then(function () {
    opts = _knownOpts(opts, CACERTS_OPTS, "cacerts");
    return _client("cacerts", "GET", baseUrl, null, { accept: "application/pkcs7-mime" }, opts, proxySnap, undefined, opts.transport);
  }).then(function (res) { return _certsResult("cacerts", res, opts, null); });
}

/**
 * @primitive  pki.est.simpleenroll
 * @signature  pki.est.simpleenroll(baseUrl, csr | builder, opts?) -> Promise<{ certificate, chain, certificates } | { retry, retryAfterSeconds }>
 * @since      0.3.16
 * @status     stable
 ...
 *
 * For RFC 7030 sec. 3.5 channel binding, pass a BUILDER `(tls) -> csr | Promise<csr>` in place of a
 * CSR. The transport runs it after the handshake with that connection's
 * `{ protocol, cipher, peerCertificate, tlsUnique }`, so the request is signed over the tls-unique of
 * the connection it is POSTed on, which is what links identity and proof-of-possession. Every new
 * connection that carries a request body re-runs the builder and the CSR is recreated for that session
 * (sec. 3.2.1), so a redirect or a Basic-auth retry re-signs; reuse one key pair across invocations,
 * since only the binding changes. A 303 is the exception, because it turns the follow-up into a GET
 * that carries no request body: the certification request stays the one posted, and bound, on the POST
 * connection, and the GET only collects the result.
 * On TLS 1.3 `tlsUnique` is `null`, as RFC 5929 defines no tls-unique there. A builder cannot be combined
 * with Digest authentication (`est/channel-binding-digest-unsupported`): that Authorization header is
 * computed over the body before the handshake the body must be bound to. A builder that throws fails the
 * request `est/csr-builder-failed` and a typed toolkit error it raises keeps its own code; either way no
 * CSR is sent. `simplereenroll` runs its identity check on each built CSR before that CSR is written.
 *
 * @opts
 *   - `transport` / `tls` / `label` / `timeout` / `maxResponseBytes` / `maxRedirects` / `proxy` / `now`, as in pki.est.cacerts.
 *   - `strict`: reject an enroll response that carries more than the single issued certificate.
 *   - `username` / `password` / `auth` -- HTTP credentials: top-level `username` / `password` mean Basic; `auth` (`{ scheme: "basic" | "digest", username, password, ... }`, as in pki.est.cacerts) selects Basic or Digest. Answered only after server authorization (empty username allowed).
 ...
 *     { transport: function () { return Promise.resolve({ status: 202, headers: { "retry-after": "60" }, body: "" }); } });
 *   r.retry && r.retryAfterSeconds;   // 60
 *   // channel binding: a builder signs the CSR over the connection's own tls-unique. A transport
 *   // stands in for the CA here, so it must request and await the body the way a real one does.
 *   var bound = await pki.est.simpleenroll("https://ca.example",
 *     async function (tls) {
 *       return pki.csr.sign({ subject: "device.example", subjectPublicKey: signerSpki,
 *         challengePassword: tls.tlsUnique.toString("base64") }, { key: signerKeyPkcs8 });
 *     },
 *     { transport: async function (request) {
...
function simpleenroll(baseUrl, csrInput, opts) {
  ...
  var proxySnap; try { proxySnap = _snapProxy(opts); } catch (_pe) { return _rej(E("est/bad-input", "the options object could not be read", _pe)); }
  return Promise.resolve().then(function () {
    opts = _knownOpts(opts, SIMPLEENROLL_OPTS, "simpleenroll");
    return _enroll("simpleenroll", baseUrl, csrInput, opts, proxySnap);
  });
}

/**
 * @primitive  pki.est.simplereenroll
 * @signature  pki.est.simplereenroll(baseUrl, csr | builder, opts?) -> Promise<{ certificate, chain, certificates } | { retry, retryAfterSeconds }>
 * @since      0.3.16
 * @status     stable
 * @spec       RFC 7030
 ...
 * is `est/bad-input`.
 *
 * Given a channel-binding BUILDER instead of a CSR (see `pki.est.simpleenroll`), the identity check
 * runs on each CSR the builder produces, inside the post-handshake callback and before that CSR is
 * written, so a mismatch still aborts the request without sending it. The connection is opened in that
 * case, since the builder needs the handshake's tls-unique; a redirect or auth retry rebuilds the CSR
 * and re-checks it.
 *
 * @opts
...
function simplereenroll(baseUrl, csrInput, opts) {
  ...
  var proxySnap; try { proxySnap = _snapProxy(opts); } catch (_pe) { return _rej(E("est/bad-input", "the options object could not be read", _pe)); }
  return Promise.resolve().then(function () {
    opts = _knownOpts(opts, SIMPLEREENROLL_OPTS, "simplereenroll");
    ...
    if (typeof csrInput !== "function") reenrollGuard(opts.oldCert, _csrDer(csrInput));
    return _enroll("simplereenroll", baseUrl, csrInput, opts, proxySnap);
  });
}
...
function _ciHeaderList(headers, name) {
  headers = headers || {};
  var lname = name.toLowerCase(), keys = guard.identifier.optionNames(headers), parts = [];
...
function _ciHeader(headers, name) {
  headers = headers || {};
  var lname = name.toLowerCase(), keys = guard.identifier.optionNames(headers), found = null, n = 0;
  for (var i = 0; i < keys.length; i++) {
    if (keys[i].toLowerCase() !== lname) continue;
    n += 1;
...
async function _serverkeygenResult(res, opts, derived) {
  ...
  if (out.privateKeyDer) {
    ...
    /** @internal The public half the key states selected the certificate; the pair is then proven
     * with the PRIVATE half, since a private structure states public components of its own that a
     * server could set to the certificate's beside private material that cannot use it. A
     * finite-field Diffie-Hellman PrivateKeyInfo carries the exponent alone, so the value derived
     * from it above IS the proof, at any width the runtime reads. */
    var keyAlg = key.algorithmOidAt(out.privateKeyDer, 1);
    if (keyAlg === null || !key.isDhOid(keyAlg)) {
      var pair;
      try { pair = await key.correspondsTo(out.privateKeyDer, spki); }
      catch (e2) { throw E("est/key-cert-mismatch", "the cleartext server-generated private key could not be exercised with the returned certificate's public key, so the two cannot be shown to be a pair (RFC 7030 sec. 4.4.2)", e2); }
      if (pair !== true) throw E("est/key-cert-mismatch", "the cleartext server-generated private key is not the private half of the returned certificate's public key (RFC 7030 sec. 4.4.2)");
    }
 ...
 * one), and the key is then proven the private half of that certificate's public key with the key itself
 * (`est/key-cert-mismatch` when it is not; a finite-field DH key, whose structure carries the exponent alone,
 * is proven by the value it derives), so a mis-associated or unusable key is refused, never handed back.
...
function serverkeygen(baseUrl, csrInput, opts) {
  ...
  var proxySnap; try { proxySnap = _snapProxy(opts); } catch (_pe) { return _rej(E("est/bad-input", "the options object could not be read", _pe)); }
  return Promise.resolve().then(function () {
    opts = _knownOpts(opts, SERVERKEYGEN_OPTS, "serverkeygen");
    var checkedTransport = opts.transport;
    var csrDer = _csrDer(csrInput);
    ...
    return _client("serverkeygen", "POST", baseUrl, transferEncode(csrDer), { accept: "multipart/mixed", "content-type": "application/pkcs10" }, opts, proxySnap, undefined, checkedTransport)
...
function csrattrs(baseUrl, opts) {
  ...
  var proxySnap; try { proxySnap = _snapProxy(opts); } catch (_pe) { return _rej(E("est/bad-input", "the options object could not be read", _pe)); }
  return Promise.resolve().then(function () {
    opts = _knownOpts(opts, CSRATTRS_OPTS, "csrattrs");
    return _client("csrattrs", "GET", baseUrl, null, { accept: "application/csrattrs" }, opts, proxySnap, undefined, opts.transport);
