...
function fullcmc(baseUrl, request, opts) {
  ...
  return Promise.resolve().then(function () {
    ...
    // Both halves of every later check are fixed HERE, before the round trip: the
    // request's own keys above, and the exchange state below. What they are
    // compared against arrives from the network; what they mean must not change
    // while it is in flight.
    var sent = _cmcSent(opts);
...
function _correlateIssued(bag, wanted, arm) {
  ...
  var remaining = (bag || []).slice();
  var out = [];
  for (var i = 0; i < wanted.length; i++) {
    var one = findIssuedCert(remaining, wanted[i]);
    if (one === null) {
      throw E("est/no-issued-cert",
        "the /fullcmc " + arm + " response answered " + out.length + " of " + wanted.length +
    ...
    remaining = remaining.filter(function (c) { return c !== one; });
    if (findIssuedCert(remaining, wanted[i])) {
...
function _cmcSent(opts) {
  return {
    ...
    senderNonce: _copyBytes(opts.senderNonce),
    dataReturn: _copyBytes(opts.dataReturn),
