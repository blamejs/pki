...
// The runtime operations a decode or a build decides with, captured at load. Read at call time
// each one is an ordinary writable property, so a caller who replaces it after this module loads
// answers the codec's questions for it: a replaced `toString` names a different OID than the bytes
// carry, a replaced `test` admits a character outside the PrintableString set, and a replaced
// `push` drops a child from a decoded SEQUENCE while every later check still passes.
var _bufToString = intrinsic.uncurry(Buffer.prototype.toString);
var _bufSubarray = intrinsic.uncurry(Buffer.prototype.subarray);
var _bufSlice = intrinsic.uncurry(Buffer.prototype.slice);
...
var _strCharAt = intrinsic.uncurry(String.prototype.charAt);
var _strCharCodeAt = intrinsic.uncurry(String.prototype.charCodeAt);
...
var TAGS = {};
// allow:guard-reads-runtime-live -- module-init, the tag table is derived before any caller runs
...
// allow:guard-reads-runtime-live -- module-init, the form whitelist is derived before any caller runs
...
function _decodeTLV(buf, start, limit, depth, maxDepth, ber, strDepth, ctr) {
  ...
  if (constructed) {
    ...
    content = _bufSubarray(buf, contentStart, contentEnd);
    end = contentEnd;
  }
  ...
  var node = {
    ...
    content:      content,
    children:     children,
    bytes:        _bufSubarray(buf, start, end),
  };
  ...
  if (ber && constructed && tagClassBits === CLASS_UNIVERSAL && tagNumber === TAGS.OCTET_STRING) {
    ...
    for (var s = 0; s < children.length; s++) {
      ...
      _push(segments, children[s].content);
    }
...
function readBoolean(node) {
  ...
  throw new Asn1Error("asn1/bad-boolean", "DER BOOLEAN must be 0x00 or 0xFF, got 0x" + _numToString(v, 16));
}
...
function _readIntegerLikeContent(node, typeName, who) {
  ...
  var mag = c.length ? intrinsic.BigInt("0x" + _bufToString(intrinsic.bufferFrom(c), "hex")) : 0n;
  return neg ? mag - (1n << intrinsic.BigInt(c.length * 8)) : mag;
}
...
function _readBitStringContent(node, who) {
  ...
  return { unusedBits: unusedBits, bytes: _bufSubarray(c, 1) };
}
...
function decodeOidContent(buf) {
  ...
  var out = [_bigIntToString(a1), _bigIntToString(a2)];
  for (var j = 1; j < arcs.length; j++) _push(out, _bigIntToString(arcs[j]));
  return intrinsic.join(out, ".");
}

function _decodeText(buf, encoding) {
  return _bufToString(buf, encoding);
}
...
function _decodeIa5(buf) {
  ...
  return _bufToString(buf, "latin1");
}
...
function _decodePrintable(buf) {
  var s = _bufToString(buf, "latin1");
  if (!_reTest(PRINTABLE_RE, s)) throw new Asn1Error("asn1/bad-printable-string", "PrintableString has characters outside the restricted set");
  return s;
}
...
function readNumericString(node) {
  ...
  var s = _bufToString(node.content, "latin1");
  if (!_reTest(NUMERIC_RE, s)) throw new Asn1Error("asn1/bad-numeric-string", "NumericString has characters outside the digits-and-space set");
  return s;
}
...
function _decodeUtf16be(buf) {
  ...
  var swapped = intrinsic.bufferFrom(buf);
  swapped.swap16();
  return _bufToString(swapped, "utf16le");
}
...
function readTime(node, opts) {
  ...
  if (node.tagNumber === TAGS.UTC_TIME) {
    m = _reExec(UTC_RE, s);
    if (!m) throw new Asn1Error("asn1/bad-utctime", "UTCTime must be YYMMDDHHMMSSZ, got " + intrinsic.stringify(s));
    year = intrinsic.parseInt(m[1], 10);
    // RFC 5280 sec. 4.1.2.5.1 -- YY < 50 => 20YY, else 19YY.
    year += (year < 50) ? 2000 : 1900;
  } else if (node.tagNumber === TAGS.GENERALIZED_TIME) {
    m = _reExec(GEN_RE, s);
    if (!m) {
      if (!(opts && opts.allowFractional)) throw new Asn1Error("asn1/bad-generalizedtime", "GeneralizedTime must be YYYYMMDDHHMMSSZ, got " + intrinsic.stringify(s));
      m = _reExec(GEN_FRAC_RE, s);
      if (!m) throw new Asn1Error("asn1/bad-generalizedtime", "GeneralizedTime must be YYYYMMDDHHMMSS[.fraction]Z, got " + intrinsic.stringify(s));
      ...
      // serial paths).
      if (_strCharAt(m[7], m[7].length - 1) === "0") throw new Asn1Error("asn1/bad-generalizedtime", "GeneralizedTime fraction must not have trailing zeros, got " + intrinsic.stringify(s));
    }
    year = intrinsic.parseInt(m[1], 10);
  } else {
  ...
  var month = intrinsic.parseInt(m[2], 10);
  var day   = intrinsic.parseInt(m[3], 10);
  var hour  = intrinsic.parseInt(m[4], 10);
  var min   = intrinsic.parseInt(m[5], 10);
  var sec   = intrinsic.parseInt(m[6], 10);
  ...
  var ms = (node.tagNumber === TAGS.GENERALIZED_TIME && m[7]) ? intrinsic.parseInt(_strSlice(m[7] + "000", 0, 3), 10) : 0;
  var d = new _Date(0);
  ...
  if (intrinsic.isNaN(guard.time.instantOf(d))) throw new Asn1Error("asn1/bad-time", "unparseable time " + intrinsic.stringify(s));
  ...
  if (d.getUTCFullYear() !== year || d.getUTCMonth() !== month - 1 || d.getUTCDate() !== day ||
    ...
    throw new Asn1Error("asn1/bad-time", "time component out of range " + intrinsic.stringify(s));
  }
  return d;
}
...
function encodeLength(n) {
  if (typeof n !== "number" || !intrinsic.isFinite(n) || n < 0 || intrinsic.floor(n) !== n) {
    throw new Asn1Error("asn1/bad-length", "length must be a non-negative integer");
  }
  if (n < 0x80) return intrinsic.bufferFrom([n]);
  var bytes = [];
  var v = n;
  while (v > 0) { bytes.unshift(v & 0xff); v = intrinsic.floor(v / 256); }
  if (bytes.length > 126) throw new Asn1Error("asn1/length-too-large", "length needs more than 126 octets");
  return intrinsic.bufferConcat([intrinsic.bufferFrom([0x80 | bytes.length]), intrinsic.bufferFrom(bytes)]);
}
...
function sequenceTlv(node) {
  ...
  var content = node.content != null ? node.content : intrinsic.bufferConcat(intrinsic.map(node.children || [], function (c) { return c.bytes; }));
  return intrinsic.bufferConcat([intrinsic.bufferFrom([TAGS.SEQUENCE | CONSTRUCTED_BIT]), encodeLength(content.length), content]);
}
...
function encodeIdentifier(classBits, constructed, tagNumber) {
  ...
  if (typeof tagNumber !== "number" || !intrinsic.isFinite(tagNumber) || tagNumber < 0 || intrinsic.floor(tagNumber) !== tagNumber) {
  ...
  var body = [];
  var v = tagNumber;
  do { body.unshift(v & 0x7f); v = intrinsic.floor(v / 128); } while (v > 0);
  ...
  return intrinsic.bufferConcat([intrinsic.bufferFrom([lead | 0x1f]), intrinsic.bufferFrom(body)]);
}
...
function encodeTLV(classBits, constructed, tagNumber, content) {
  ...
  var body = intrinsic.isBuffer(content) ? guard.bytes.view(content, Asn1Error, "asn1/not-buffer", "encodeTLV content") : intrinsic.bufferFrom(content || []);
  var id = encodeIdentifier(classBits, constructed, tagNumber);
  return intrinsic.bufferConcat([id, encodeLength(body.length), body]);
}
...
function intToDer(v) {
  if (typeof v === "number") {
    if (!intrinsic.isSafeInteger(v)) throw new Asn1Error("asn1/bad-integer", "unsafe integer; pass a BigInt");
    v = intrinsic.BigInt(v);
  }
  ...
  if (v > 0n) {
    var t = v;
    while (t > 0n) { bytes.unshift(intrinsic.Number(t & 0xffn)); t >>= 8n; }
    if (bytes[0] & 0x80) bytes.unshift(0x00);
  } else {
    var len = 1;
    while (v < -(1n << intrinsic.BigInt(8 * len - 1))) len += 1;
    var tc = (1n << intrinsic.BigInt(8 * len)) + v;
    for (var i = len - 1; i >= 0; i--) { bytes[i] = intrinsic.Number(tc & 0xffn); tc >>= 8n; }
  }
  return intrinsic.bufferFrom(bytes);
}
...
function encodeOidContent(dotted) {
  ...
  var arcs = intrinsic.map(_strSplit(dotted, "."), function (p) { return intrinsic.BigInt(p); });
  var a1 = arcs[0], a2 = arcs[1];
  var subids = intrinsic.concat([a1 * 40n + a2], intrinsic.arraySlice(arcs, 2));
  var out = [];
  for (var s = 0; s < subids.length; s++) {
    var body = [];
    var v = subids[s];
    do { body.unshift(intrinsic.Number(v & 0x7fn)); v >>= 7n; } while (v > 0n);
    ...
    for (var j = 0; j < body.length; j++) _push(out, body[j]);
  }
  return intrinsic.bufferFrom(out);
}
 ...
 */
var build = {
  boolean:  function (v) { return _universal(TAGS.BOOLEAN, false, intrinsic.bufferFrom([v ? 0xff : 0x00])); },
  ...
  nullValue: function () { return _universal(TAGS.NULL, false, intrinsic.bufferAlloc(0)); },
  ...
  bitString: function (buf, unusedBits) {
    ...
    return _universal(TAGS.BIT_STRING, false, intrinsic.bufferConcat([intrinsic.bufferFrom([u]), body]));
  },
  ...
  namedBitString: function (positions) {
    if (!intrinsic.isArray(positions)) throw new Asn1Error("asn1/bad-bit-string", "namedBitString requires an array of bit positions");
    var hi = -1, i, p;
    for (i = 0; i < positions.length; i++) {
      p = positions[i];
      if (typeof p !== "number" || !intrinsic.isFinite(p) || p < 0 || (p | 0) !== p) throw new Asn1Error("asn1/bad-bit-string", "a named-bit position must be a non-negative integer");
      if (p > hi) hi = p;
    }
    if (hi < 0) return build.bitString(intrinsic.bufferAlloc(0), 0);
    var buf = intrinsic.bufferAlloc((hi >> 3) + 1);
    for (i = 0; i < positions.length; i++) { p = positions[i]; buf[p >> 3] |= 0x80 >> (p & 7); }
    return build.bitString(buf, 7 - (hi & 7));
  },
  utf8:     function (s) { return _universal(TAGS.UTF8_STRING, false, intrinsic.bufferFrom(intrinsic.String(s), "utf8")); },
  ia5:      function (s) {
    ...
    s = intrinsic.String(s);
    for (var i = 0; i < s.length; i++) {
      if (_strCharCodeAt(s, i) > 0x7F) throw new Asn1Error("asn1/bad-ia5-string", "IA5String requires 7-bit ASCII");
    }
    return _universal(TAGS.IA5_STRING, false, intrinsic.bufferFrom(s, "latin1"));
  },
  printable: function (s) {
    s = intrinsic.String(s);
    if (!_reTest(PRINTABLE_RE, s)) throw new Asn1Error("asn1/bad-printable-string", "value has characters outside the PrintableString set");
    return _universal(TAGS.PRINTABLE_STRING, false, intrinsic.bufferFrom(s, "latin1"));
  },
  bmpString: function (s) {
    ...
    s = intrinsic.String(s);
    var out = intrinsic.bufferAlloc(s.length * 2);
    for (var i = 0; i < s.length; i++) {
      var u = _strCharCodeAt(s, i);
  ...
  utcTime:  function (date) { return _universal(TAGS.UTC_TIME, false, intrinsic.bufferFrom(_utcTimeString(date), "latin1")); },
  generalizedTime: function (date) { return _universal(TAGS.GENERALIZED_TIME, false, intrinsic.bufferFrom(_generalizedTimeString(date), "latin1")); },
  sequence: function (children) { return _universal(TAGS.SEQUENCE, true, intrinsic.bufferConcat(_asBufferArray(children, "build.sequence"))); },
  set:      function (children) {
    // DER (X.690 11.6) requires SET component encodings in ascending order.
    var arr = intrinsic.arraySlice(_asBufferArray(children, "build.set"));
    intrinsic.sort(arr, intrinsic.compare);
    return _universal(TAGS.SET, true, intrinsic.bufferConcat(arr));
  },
  setOf:    function (children) {
    var arr = intrinsic.arraySlice(_asBufferArray(children, "build.setOf"));
    intrinsic.sort(arr, intrinsic.compare);
    return _universal(TAGS.SET, true, intrinsic.bufferConcat(arr));
  },
  ...
  implicit: function (tagNumber, tlv) {
    ...
    var node = decode(buf);
    return encodeTLV(CLASS_CONTEXT, node.constructed, tagNumber, _bufSlice(buf, buf.length - node.length));
  },
...
function _intContent(v, who) {
  ...
  var content = intrinsic.isBuffer(v) ? v : intToDer(v);
...
function _asBufferArray(arr, who) {
  if (!intrinsic.isArray(arr)) throw new Asn1Error("asn1/bad-children", who + ": expected an array of TLV buffers");
  return intrinsic.map(arr, function (b) { return _asBuffer(b, who); });
}
