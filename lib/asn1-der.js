...
function encodeOidContent(dotted) {
  if (typeof dotted === "string") {
    ...
    for (var d = 0; d < dotted.length; d++) {
      if (_charCodeAt(dotted, d) !== 46) continue;
      subidCount += 1;
      if (subidCount > constants.LIMITS.OID_MAX_SUBIDENTIFIERS) {
        throw new OidError("oid/too-many-subidentifiers",
          "OID carries more than " + constants.LIMITS.OID_MAX_SUBIDENTIFIERS + " sub-identifiers");
      }
    }
  }
