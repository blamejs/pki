...
function readInteger(node) {
  ...
  // work — reject a hostile length prefix up front.
  if (c.length > constants.LIMITS.DER_MAX_INTEGER_BYTES) {
    throw new Asn1Error("asn1/integer-too-large",
      "INTEGER content " + c.length + " bytes exceeds cap " + constants.LIMITS.DER_MAX_INTEGER_BYTES);
  }
  if (c.length > 1) {
...
function readTime(node) {
  ...
  // Date.UTC silently rolls over out-of-range fields (Feb 30 -> Mar 2,
  // month 13 -> next Jan, hour 25 -> +1 day). Reject unless every
