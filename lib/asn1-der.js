...
function _assertWellFormedUtf16(s) {
  for (var i = 0; i < s.length; i++) {
    ...
    if (c >= 0xD800 && c <= 0xDBFF) {
      var n = (i + 1) < s.length ? _charCodeAt(s, i + 1) : -1;
      if (!(n >= 0xDC00 && n <= 0xDFFF)) throw new Asn1Error("asn1/bad-utf8-string", "UTF8String has an unpaired high surrogate");
      i++;
    } else if (c >= 0xDC00 && c <= 0xDFFF) {
      throw new Asn1Error("asn1/bad-utf8-string", "UTF8String has an unpaired low surrogate");
    }
  }
}
...
function _generalizedTimeString(date, fractional) {
  ...
  return yyyy +
    ...
    _fmtTwo(date.getUTCHours()) + _fmtTwo(date.getUTCMinutes()) + _fmtTwo(date.getUTCSeconds()) +
    (fractional ? _fractionString(date.getUTCMilliseconds()) : "") + "Z";
}
...
function _generalizedTimeOptions(opts) {
  ...
  if (opts.fractional !== undefined && typeof opts.fractional !== "boolean") throw new Asn1Error("asn1/bad-generalizedtime", "build.generalizedTime fractional must be a boolean");
  return opts.fractional === true;
}
...
var build = intrinsic.assign(intrinsic.create(null), {
  ...
  utf8:     function (s) { var v = String(s); _assertWellFormedUtf16(v); return _universal(TAGS.UTF8_STRING, false, Buffer.from(v, "utf8")); },
  ia5:      function (s) {
    s = String(s);
    for (var i = 0; i < s.length; i++) {
  ...
  set:      function (children) {
    ...
    return encodeConstructed(CLASS_UNIVERSAL, TAGS.SET, arr);
  },
