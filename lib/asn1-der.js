...
function readTime(node, opts) {
  ...
  if (node.tagNumber === TAGS.UTC_TIME) {
    ...
    if (!m) {
      ...
      // by the regex). ".5Z" is valid; ".50Z" and ".500Z" are non-canonical.
      ...
      // This reader surfaces a millisecond-precision Date, so a fraction finer than
      // 3 digits cannot be represented losslessly — reject it rather than silently
      // truncate (which would collapse distinct sub-millisecond encodings).
      if (m[7].length > 3) throw new Asn1Error("asn1/bad-generalizedtime", "GeneralizedTime fraction exceeds representable millisecond precision, got " + JSON.stringify(s));
    }
    year = parseInt(m[1], 10);
  } else {
