...
var build = {
  ...
  ia5:      function (s) {
    var body = Buffer.from(String(s), "latin1");
    for (var i = 0; i < body.length; i++) {
      if (body[i] > 0x7F) throw new Asn1Error("asn1/bad-ia5-string", "IA5String requires 7-bit ASCII");
    }
    return _universal(TAGS.IA5_STRING, false, body);
  },
...
function _intContent(v, who) {
  if (!Buffer.isBuffer(v)) return intToDer(v);
