...
// The list test at each builder door decides whether a caller's argument is refused. Read at the
// moment it runs, a replacement answering true admits a non-list and the door it guards is gone.
...
// Character-scanned so no regex match protocol -- a live, replaceable `exec` -- stands between the
// codec and the time field it slices out.
...
// The fixed DER time layout, scanned into its components, or null on any format violation:
// yearLen digits (2 for UTCTime YY, 4 for GeneralizedTime YYYY), then MM DD HH MM SS, then --
// when allowFrac -- the X.690 sec. 11.7 fractional-seconds profile (a "." decimal separator, never
// ",", and a non-empty fraction), then a terminating "Z" as the final character with nothing after.
// `ms` is the first three fraction digits right-zero-padded (matching (frac + "000").slice(0,3));
// `fracLastIsZero` carries the sec. 11.7.3 trailing-zero check the caller enforces.
function _scanTimeFields(s, yearLen, allowFrac) {
  ...
  if (allowFrac) {
    ...
    if (fi === 0) return null; // a non-empty fraction is required
    hasFrac = true;
    fracLastIsZero = _charCodeAt(s, pos - 1) === 48;
    ms = f[0] * 100 + f[1] * 10 + f[2];
  }
  if (pos !== s.length - 1 || _charCodeAt(s, pos) !== 90) return null; // trailing "Z", nothing after
...
function readTime(node, opts) {
  ...
  if (node.tagNumber === TAGS.UTC_TIME) {
    ...
    if (!t) {
      ...
      // by the scan). ".5Z" is valid; ".50Z" and ".500Z" are non-canonical. X.690
  ...
  // Coverage residual -- scan-validated fields are finite integers and Date rolls over (never NaN); out-of-range is caught by the component round-trip check below.
...
// PrintableString (X.680): A-Z a-z 0-9 space and ' ( ) + , - . / : = ? -- checked by character scan (no
// regex). NumericString: digits and space only.
function _isPrintableChar(c) {
  if ((c >= 65 && c <= 90) || (c >= 97 && c <= 122) || (c >= 48 && c <= 57)) return true;   // A-Z a-z 0-9
  switch (c) {
    case 32: case 39: case 40: case 41: case 43: case 44: case 45: case 46: case 47:        // space ' ( ) + , - . /
...
// Frozen at load so a builder method cannot be replaced afterward: every format module captures `build`
// and calls its methods to produce emitted DER, so a mutable `build.contextPrimitive` (or any builder)
// would let co-resident code steer the bytes a certificate carries. Freezing closes that once, toolkit-wide.
