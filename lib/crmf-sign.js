...
function _x942PrivToPkcs3(pkcs8Bytes) {
  ...
  var p, g;
  try {
    var domain = asn1.decode(alg.children[1].bytes);
    if (!_isUniversal(domain, 16) || !domain.children || domain.children.length < 3) return null;
...
function _assertEnclosedKeyMatches(privateKeyDer, templateSpki) {
  return Promise.resolve()
    ...
    .then(function (derivedSpki) {
       ...
       * comparison is about keys. The group itself is not measured here. This arm asks one question,
       * which RFC 4211 sec. 4.2 states: is the enclosed key the private half of the requested one.
       * Whether that key sits in a good group is the certifying authority's policy, and answering it
       * here would mean proving a modulus prime, which costs seconds and forces an upper bound on the
       * size, so a key too large to prove would become a key too large to archive. */
      var wantedX942 = _x942ToPkcs3(wanted);
      if (wantedX942 !== null) wanted = wantedX942.der;
       ...
       * and refuse a pair that matches. */
      var derivedX942 = _x942ToPkcs3(derived);
      if (derivedX942 !== null) derived = derivedX942.der;
