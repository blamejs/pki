...
function _buildAgreeMac(pop, outerTag, certReqDer, template) {
  ...
  var caCertDer = guard.bytes.snapshotSource(pkix.coerceToDer(pop.caCert, { pem: "CERTIFICATE" }), CrmfError, "crmf/bad-popo", "the authority certificate");
  var parsedCa;
  ...
  var caPub, eePriv;
  try { caPub = _createPublicKey({ key: parsedCa.subjectPublicKeyInfo.bytes, format: "der", type: "spki" }); }
  ...
  try {
    ...
    if (!template.spki || !intrinsic.bufferEquals(intrinsic.bufferFrom(eeSpki), intrinsic.bufferFrom(template.spki))) {
    ...
    if (eePriv.asymmetricKeyType !== "dh" || caPub.asymmetricKeyType !== "dh") {
      throw _err("crmf/bad-popo", "the static Diffie-Hellman proof of possession is defined over a finite-field group (RFC 2875 sec. 3), and this pair is " +
        guard.text.showValue(eePriv.asymmetricKeyType) + " against " + guard.text.showValue(caPub.asymmetricKeyType));
    }
    ...
    /** @internal K = SHA1(LeadingInfo | ZZ | TrailingInfo) (RFC 2875 sec. 3, step 3c). */
    k = _createHash("sha1").update(leading).update(zz).update(trailing).digest();
    ...
    var mac = _createHmac("sha1", k).update(certReqDer).digest();
    ...
    return b.explicit(outerTag, b.implicit(3, b.sequence([
      b.sequence([b.oid(O("id-dhPop-static-HMAC-SHA1"))]),
