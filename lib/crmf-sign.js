...
function _buildAgreeMac(pop, outerTag, certReqDer, template) {
  ...
  var caX942 = _x942ToPkcs3(parsedCa.subjectPublicKeyInfo.bytes);
  var caSpkiBytes;
  if (caX942 !== null) {
    _assertDhGroup(caX942.p, caX942.g, caX942.y, caX942.q, caX942.j, true);
    caSpkiBytes = caX942.der;
  } else {
    caSpkiBytes = parsedCa.subjectPublicKeyInfo.bytes;
    var pkcs3 = _pkcs3Params(caSpkiBytes);
    if (pkcs3 !== null) _assertDhGroup(pkcs3.p, pkcs3.g, pkcs3.y, null, null, true);
  }
  ...
  try {
    ...
    if (templateSpki) {
      var converted = _x942ToPkcs3(templateSpki);
      if (converted !== null) {
        _assertDhGroup(converted.p, converted.g, converted.y, converted.q, converted.j, false);
        templateSpki = converted.der;
      } else {
        var tPkcs3 = _pkcs3Params(templateSpki);
        if (tPkcs3 !== null) _assertDhGroup(tPkcs3.p, tPkcs3.g, tPkcs3.y, null, null, false);
      }
    }
...
function _assertDhGroup(p, g, y, q, j, forAgreement) {
  ...
  /** @internal A stated order is held against p and g before anything is proven prime. Both tests
   * below cost seconds on a large modulus and this one costs a modular exponentiation, so a set of
   * parameters that cannot be a group is turned away at the cheaper of the two. */
  if (q !== null && (q <= 1n || (p - 1n) % q !== 0n || _modPow(g, q, p) !== 1n)) {
  ...
  if (p < _DH_MIN_P) {
    throw _err("crmf/bad-popo", "the authority certificate's Diffie-Hellman modulus is under " +
...
/** @internal Whether a [3] arm opens as an ORAddress. It is a SEQUENCE whose first element is the
 * BuiltInStandardAttributes SEQUENCE (X.411), so an arm holding something that is not a sequence of
 * attributes is not an address. The attributes themselves are an X.400 structure this does not read,
 * which the derivation can afford because it hashes these bytes rather than interpreting them; what
 * it cannot afford is hashing an arm that names nobody, so the two empty forms are refused. */
function _isOrAddress(node) {
  var kids = node.children;
  if (!kids || kids.length === 0) return false;
  var builtIn = kids[0];
  return schema.isUniversal(builtIn, 16) && !!builtIn.children && builtIn.children.length > 0;
}
...
function _assertEnclosedKeyMatches(privateKeyDer, templateSpki) {
  return Promise.resolve()
    ...
    .then(function (derivedSpki) {
      ...
      var wantedX942 = _x942ToPkcs3(wanted);
      if (wantedX942 !== null) {
        _assertDhGroup(wantedX942.p, wantedX942.g, wantedX942.y, wantedX942.q, wantedX942.j, false);
        wanted = wantedX942.der;
      } else {
        var wPkcs3 = _pkcs3Params(wanted);
        if (wPkcs3 !== null) _assertDhGroup(wPkcs3.p, wPkcs3.g, wPkcs3.y, null, null, false);
      }
