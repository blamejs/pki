...
// A cross-certification request (CMP ccr, RFC 9810 sec. 5.3.11) may carry signingAlg [2] to name the
// algorithm the requester asks the responding CA to sign the cross-certificate with (App. D.6); it is
// refused in every other request, where signingAlg is CA-assigned and omitted (RFC 4211 sec. 5).
var CROSS_CERT_TEMPLATE_KEYS = { version: 1, subject: 1, publicKey: 1, validity: 1, extensions: 1, issuer: 1, signingAlg: 1 };
...
// signingAlg [2] IMPLICIT AlgorithmIdentifier: the algorithm a cross-certification requester asks the
// responding CA to sign the cross-certificate with (RFC 9810 App. D.6). The requester supplies a
// pre-encoded AlgorithmIdentifier DER, validated here as a SEQUENCE whose first element is an OBJECT
// IDENTIFIER before it is re-tagged [2] IMPLICIT.
function _encodeSigningAlg(v) {
  var der = _b.reqDer(v, "certTemplate.signingAlg (a pre-encoded AlgorithmIdentifier DER)");
  var n;
  try { n = asn1.decode(der); } catch (err) { throw _err("crmf/bad-input", "certTemplate.signingAlg is not valid DER", err); }
  // AlgorithmIdentifier is a SEQUENCE of exactly the algorithm OID and at most one parameters element; a
  // SEQUENCE with no children, or with a second element beyond parameters, is not one.
  if (n.tagNumber !== asn1.TAGS.SEQUENCE || n.tagClass !== "universal" || !n.children || n.children.length < 1 || n.children.length > 2) throw _err("crmf/bad-input", "certTemplate.signingAlg must be an AlgorithmIdentifier SEQUENCE { algorithm OID, parameters ANY OPTIONAL }");
  try { asn1.read.oid(n.children[0]); } catch (err) { throw _err("crmf/bad-input", "certTemplate.signingAlg algorithm is not an OBJECT IDENTIFIER", err); }
  return b.implicit(2, der);   // signingAlg [2] IMPLICIT
}

// CertTemplate ::= SEQUENCE { [0..9] all IMPLICIT OPTIONAL }. A request omits serialNumber [1] / issuerUID
// [7] / subjectUID [8] (CA-assigned or deprecated, RFC 4211 sec. 5), and the builder never emits them.
// signingAlg [2] is likewise omitted, except on the cross-certification profile (see _encodeSigningAlg).
...
function _encodeCertTemplate(tpl, opts) {
  ...
  // RFC 9810 sec. 5.3.9) carries serialNumber [1] to name the certificate to revoke, allowed only then; a
  // cross-certification template (CMP ccr, sec. 5.3.11) may carry signingAlg [2], allowed only then.
  var crossCert = !!(opts && opts.crossCert);
  var allowed = (opts && opts.revocation) ? REVOCATION_TEMPLATE_KEYS : crossCert ? CROSS_CERT_TEMPLATE_KEYS : KNOWN_TEMPLATE_KEYS;
  guard.identifier.assertKnownKeys(tpl, allowed, _err, "crmf/bad-input", "unknown certTemplate field ");
  var fields = [];
  if (tpl.version != null) {
    // RFC 4211 sec. 5 fixes a request template version at 2 (v3); the cross-certification profile (RFC 9810
    // App. D.6) also permits 0 (v1), with v3 strongly recommended.
    if (tpl.version !== 2 && !(crossCert && tpl.version === 0)) throw _err("crmf/bad-version", crossCert ? "certTemplate version MUST be 2 (v3) or 0 (v1) for a cross-certification request (RFC 9810 App. D.6)" : "certTemplate version MUST be 2 (v3) if supplied (RFC 4211 sec. 5)");
    fields.push(b.implicit(0, b.integer(tpl.version === 0 ? 0n : 2n)));         // version [0]
  }
  if (tpl.serialNumber != null) fields.push(b.implicit(1, _b.serialInteger(tpl.serialNumber)));   // serialNumber [1] (revocation only)
  if (tpl.signingAlg != null) intrinsic.push(fields, _encodeSigningAlg(tpl.signingAlg));   // signingAlg [2] (cross-certification only)
 ...
 *   - `crossCert` (boolean) -- apply the RFC 9810 App. D.6 cross-certification profile relaxations of RFC 4211
 *     sec. 5: permit `certTemplate.signingAlg` (a pre-encoded AlgorithmIdentifier DER naming the algorithm the
 *     responding CA is asked to sign the cross-certificate with) and a `certTemplate.version` of 0 (v1) as well
 *     as 2 (v3). Both are refused in an ordinary request. Set by `pki.cmp.build` for a `ccr` body.
 * @example
...
function _buildCertReqMsg(spec, key, opts) {
  ...
  var template = _encodeCertTemplate(spec.certTemplate, { crossCert: opts && opts.crossCert });
