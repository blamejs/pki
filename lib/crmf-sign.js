...
function _buildAgreeMac(pop, outerTag, certReqDer, template) {
  ...
  var caPub, eePriv;
  var caSpkiBytes = _x942ToPkcs3(parsedCa.subjectPublicKeyInfo.bytes) || parsedCa.subjectPublicKeyInfo.bytes;
...
function _x942ToPkcs3(spkiBytes) {
  ...
  try {
    domain = asn1.decode(alg.children[1].bytes);
    ...
    y = asn1.read.integer(asn1.decode(asn1.read.bitString(node.children[1]).bytes));
  } catch (e3) {
  ...
  /** @internal Sized before anything is computed with it. The DER reader admits an INTEGER far
   * larger than any Diffie-Hellman group, and every test below raises one of these to the power of
   * another, so an oversized operand is a request for arbitrary work on this thread rather than a
   * group that could be agreed with. The bound is the widest MODP group RFC 3526 defines. */
  if (p >= _DH_MAX || g >= _DH_MAX || q >= _DH_MAX) {
    throw _err("crmf/bad-popo", "the authority certificate's X9.42 domain parameters are larger than any Diffie-Hellman group this toolkit agrees over (" +
      constants.LIMITS.DH_MAX_MODULUS_BYTES + " bytes)");
  }
  if (p <= 2n || g <= 1n || g >= p - 1n || q <= 1n) {
    throw _err("crmf/bad-popo", "the authority certificate's X9.42 domain parameters are not a Diffie-Hellman group");
  }
  /** @internal q arrives in the certificate, so it is checked against p and g rather than believed.
   * A q the group can have divides p-1 and satisfies g^q = 1. */
  if ((p - 1n) % q !== 0n || _modPow(g, q, p) !== 1n) {
  ...
  /** @internal q must also be prime, which is what makes the subgroup test below mean anything:
   * y^q = 1 bounds the order of y to a DIVISOR of q, so a composite q admits a y of small order and
   * the agreed secret then exposes the requester's private exponent modulo that order. A certificate
   * stating q = p-1 passes every test above, since g^(p-1) = 1 holds for the whole group. With q
   * prime the only divisors are 1 and q, and y = 1 is already excluded, so the order is q. */
  ...
  /** @internal X9.42 defines p as an odd prime, and every statement above is about the group modulo a
   * prime: over a composite modulus the order of an element need not divide q at all, so the tests
   * would be measuring a structure the modulus does not have. */
  if (!_checkPrimeSync(_bigToBuf(p))) {
    throw _err("crmf/bad-popo", "the authority certificate's X9.42 modulus is not prime, so the group its domain parameters describe does not exist");
  }
  /** @internal The cofactor is multiplied by q, so it is sized first like the rest. A cofactor this
   * group could have is (p-1)/q and so smaller than p, which the same bound already covers. */
  if (j !== null && j >= _DH_MAX) {
    throw _err("crmf/bad-popo", "the authority certificate's X9.42 cofactor is larger than any Diffie-Hellman group this toolkit agrees over");
  }
  if (j !== null && j * q !== p - 1n) {
