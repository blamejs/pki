...
function _assertDhGroup(p, g, y, q, j, forAgreement, what) {
  ...
  if (q === null) {
    ...
    if (y !== null && _modPow(y, q, p) !== 1n) {
      throw _err("crmf/bad-popo", what + " Diffie-Hellman public value is not in the subgroup its own domain parameters state, so agreeing with it would expose the private exponent of whichever side agrees");
    }
  }
...
function _pkcs3Params(spkiBytes, what) {
  ...
  try {
    ...
    if (!_isUniversal(prm, 16) || !prm.children || prm.children.length < 2 || prm.children.length > 3) {
      throw _err("crmf/bad-popo", what + " PKCS#3 Diffie-Hellman parameters are not a DHParameter SEQUENCE");
    }
    ...
    if (prm.children.length === 3) {
      if (!_isUniversal(prm.children[2], 2)) {
        throw _err("crmf/bad-popo", what + " PKCS#3 privateValueLength is not an INTEGER");
      }
      ...
      if (pvl <= 0n) {
        throw _err("crmf/bad-popo", what + " PKCS#3 privateValueLength is not a positive length");
      }
      /** @internal The field states how many bits the private value has, and a private value is less
       * than the modulus, so a length above the modulus's own bit count describes no value the group
       * holds. The encoded size bounds the count before anything is shifted by it, because this
       * reader also serves an arm that reads a key without bounding its modulus and the field itself
       * is an INTEGER the codec admits at any width. */
      if (pvl > intrinsic.BigInt(prm.children[0].bytes.length) * 8n || (1n << (pvl - 1n)) > p) {
        throw _err("crmf/bad-popo", what + " PKCS#3 privateValueLength states a private value longer than its own modulus, which no value in the group is");
      }
    }
    ...
    return { p: p, g: asn1.read.integer(prm.children[1]), y: asn1.read.integer(asn1.decode(pub.bytes)) };
  } catch (e) {
    if (e instanceof CrmfError) throw e;
    throw _err("crmf/bad-popo", what + " PKCS#3 Diffie-Hellman key could not be read", e);
  }
}
