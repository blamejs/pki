...
var KNOWN_POP_KEYS = {
  type: 1, raVerified: 1, sender: 1, method: 1, subsequentMessage: 1,
  privateKey: 1, identifier: 1, recipients: 1, archive: 1, contentEncryptionAlgorithm: 1,
};
...
function _buildEncryptedKeyPop(pop, outerTag, template) {
  ...
  var encKeyWithIdDer = _encKeyWithID(privateKeyDer, pop.identifier);
  // This arm is the one place in the toolkit that deliberately handles a caller's private key as
  // plaintext, and it makes two copies of it: the snapshot taken at the door, and the EncKeyWithID
  // that carries it. Both are cleared once the envelope exists or the attempt fails, on every path.
  // The caller's own buffer is left untouched, since it was copied rather than borrowed.
...
function _buildProofOfPossession(pop, certReqDer, template, signingKey, opts) {
  ...
  if (pop != null) guard.identifier.assertKnownKeys(pop, KNOWN_POP_KEYS, _err, "crmf/bad-input", "unknown spec.pop field ");
