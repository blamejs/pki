...
function _x942ToPkcs3(spkiBytes) {
  ...
  if (!alg.children || alg.children.length !== 2) return null;
  var algOid;
  ...
  try {
    ...
    if (!domain.children || domain.children.length < 3 || domain.children.length > 5) return null;
    ...
    if (domain.children.length >= 4) {
      ...
      if (opt.tagNumber === 2) {
        ...
        if (domain.children.length === 5 && domain.children[4].tagNumber !== 16) {
          throw _err("crmf/bad-popo", "the authority certificate's X9.42 validationParms is not a SEQUENCE");
        }
      } else if (opt.tagNumber === 16) {
