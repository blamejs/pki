...
function _encodeCertTemplate(tpl, opts) {
  ...
  var allowed = (opts && opts.revocation) ? REVOCATION_TEMPLATE_KEYS : (opts && opts.crossCert) ? CROSS_CERT_TEMPLATE_KEYS : KNOWN_TEMPLATE_KEYS;
 ...
 *   - `crossCert` (boolean) -- permit `certTemplate.signingAlg` (a pre-encoded AlgorithmIdentifier DER), the
 *     RFC 9810 App. D.6 cross-certification profile field naming the algorithm the responding CA is asked to
 *     sign the cross-certificate with. Refused in an ordinary request, where signingAlg is CA-assigned
 *     (RFC 4211 sec. 5). Set by `pki.cmp.build` for a `ccr` body.
 * @example
