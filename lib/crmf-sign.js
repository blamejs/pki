...
function _namesSomebody(node) {
  ...
  /** @internal The derivation keys from these bytes, so it uses only an arm whose structure is
   * checked. x400Address [3] and ediPartyName [5] are read as non-empty constructed values and no
   * further, so a [5] holding a NULL is a name-shaped hole; a list offering nothing else is treated
   * as naming nobody rather than hashed. Every other arm is read down to its own contents. */
  for (var i = 0; i < node.children.length; i++) {
    var t = node.children[i].tagNumber;
