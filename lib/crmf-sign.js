...
function _buildAgreeMac(pop, outerTag, certReqDer, template) {
  ...
  if (caX942 !== null) {
    ...
    if (pkcs3 !== null) _assertDhGroup(pkcs3.p, pkcs3.g, pkcs3.y, null, null, true, "the authority certificate's");
  }
  ...
  try {
    ...
    if (templateSpki) {
      ...
      if (converted !== null) {
        ...
        var tPkcs3 = _pkcs3Params(templateSpki);
    ...
    _assertRequesterExponent(_pkcs3Params(eeSpki));
...
/** @internal Whether the requester's own public value states an exponent the authority could work
 * out. Takes the PKCS#3 parameters of the key that agrees, or null when the key is not a
 * finite-field one, in which case there is no exponent of this kind to test. The inverse of g is
 * computed as g^(p-2), which is the inverse when p is prime; the authority's group was proven prime
 * before this key was imported, and the two share it. */
function _assertRequesterExponent(parts) {
  if (parts === null) return;
  if (parts.y === 1n || parts.y === parts.g || parts.y === _modPow(parts.g, parts.p - 2n, parts.p)) {
    throw _err("crmf/bad-popo", "pop.key states a private exponent outside the interval [2, q-2] that X9.42 " +
      "requires (RFC 2631 sec. 2.2), so the secret it agrees with the authority follows from the authority's " +
      "certificate alone and the proof would demonstrate possession of nothing");
  }
}
 ...
 * grows gently with the modulus and every path that reads these parameters can afford them: the
 * archival arm reads a key without bounding its size, and the tests it leaves to the certifying
 * authority are the ones whose cost grows with it, a modular exponentiation and two primality proofs.
 * A cofactor states (p-1)/q, so a cofactor with no order beside it states nothing. */
function _assertDhParamsCoherent(p, g, q, j, what) {
  if (p <= 2n || g <= 1n || g >= p - 1n) {
    throw _err("crmf/bad-popo", what + " parameters are not a Diffie-Hellman group");
  }
  if (j !== null && (q === null || j * q !== p - 1n)) {
    throw _err("crmf/bad-popo", what + " X9.42 cofactor does not match its own p and q");
  }
  if (q !== null && (q <= 1n || (p - 1n) % q !== 0n)) {
    throw _err("crmf/bad-popo", what + " X9.42 domain parameters state a subgroup order its own p and g do not have");
  }
}

function _assertDhGroup(p, g, y, q, j, forAgreement, what) {
  if (p >= _DH_MAX || g >= _DH_MAX || (y !== null && y >= _DH_MAX) ||
      (q !== null && q >= _DH_MAX) || (j !== null && j >= _DH_MAX)) {
    throw _err("crmf/bad-popo", what + " parameters are larger than any Diffie-Hellman group this toolkit agrees over (" +
  ...
  if (q !== null && _modPow(g, q, p) !== 1n) {
    throw _err("crmf/bad-popo", what + " X9.42 domain parameters state a subgroup order its own p and g do not have");
  }
  if (!_isPrime(p)) {
    throw _err("crmf/bad-popo", what + " Diffie-Hellman modulus is not prime, so the group its parameters describe does not exist");
  }
  ...
  if (q === null) {
    ...
    if (!_isPrime(q)) {
      throw _err("crmf/bad-popo", what + " X9.42 domain parameters state a subgroup order that is not prime, so a public value in it need not have that order");
    }
    order = q;
    ...
    if (y !== null && _modPow(y, q, p) !== 1n) {
      throw _err("crmf/bad-popo", what + " Diffie-Hellman public value is not in the subgroup its own domain parameters state, so agreeing with it would expose the requester's private exponent");
    }
  }
...
function _pkcs3Params(spkiBytes) {
  ...
  try {
    ...
    /** @internal privateValueLength is the one optional field DHParameter carries, and it is an
     * INTEGER. A third field of any other type is not that field, and this reader is the only thing
     * that looks at these bytes before the request travels under a MAC covering them. */
    if (prm.children.length === 3) {
      if (!_isUniversal(prm.children[2], 2)) {
        throw _err("crmf/bad-popo", "the authority certificate's PKCS#3 privateValueLength is not an INTEGER");
      }
      /** @internal Read, not counted. An INTEGER carries at least one content octet, so a field
       * tagged as one and holding none is not a length, and this reader is the only thing that looks
       * at these bytes before the request travels under a MAC covering them. */
      if (asn1.read.integer(prm.children[2]) <= 0n) {
        throw _err("crmf/bad-popo", "the authority certificate's PKCS#3 privateValueLength is not a positive length");
      }
    }
...
function _assertValidationParms(node, what) {
  var kids = node.children;
  if (!_isUniversal(node, 16) || !kids || kids.length !== 2 ||
      !_isUniversal(kids[0], 3) || !_isUniversal(kids[1], 2)) {
    throw _err("crmf/bad-popo", what + " X9.42 validationParms is not a seed BIT STRING with a pgenCounter INTEGER");
  }
 ...
 * form this runtime classifies carries none of the last three, so every field is read HERE, before
 * the parameter that states it is dropped. Both halves of a finite-field key carry this structure and
 * both take that conversion, so both read it in this one place: a field read on one side and skipped
 * on the other is a key the runtime refuses in the encoding it was handed and this rewrites into one
 * it accepts. The algorithm named dhpublicnumber, so these bytes are DomainParameters or the key is
...
function _x942Domain(paramsNode, what) {
  ...
  if (!_isUniversal(domain, 16) || !domain.children ||
    ...
    throw _err("crmf/bad-popo", what + " X9.42 domain parameters are not a DomainParameters SEQUENCE of p, g and q with at most two optional fields after them");
  }
  var out = {
  ...
  if (domain.children.length >= 4) {
    ...
    if (_isUniversal(opt, 2)) {
      ...
      if (domain.children.length === 5) _assertValidationParms(domain.children[4], what);
    } else if (_isUniversal(opt, 16)) {
      if (domain.children.length === 5) {
        throw _err("crmf/bad-popo", what + " X9.42 domain parameters carry a field after validationParms");
      }
      _assertValidationParms(opt, what);
    } else {
      throw _err("crmf/bad-popo", what + " X9.42 domain parameters carry a fourth field that is neither a cofactor nor validationParms");
    }
  }
  return out;
}
...
var _DH_MAX = _pow2(8 * constants.LIMITS.DH_MAX_MODULUS_BYTES);
...
/** @internal Whether a value is prime, remembered across calls. Proving a 4096-bit modulus prime
 * costs seconds, and an enrollment run asks the same question of the same group every time: the
 * authority's parameters do not change between requests, and the requester's key must share them.
 * What is remembered is a fact about a number rather than anything a caller supplied, so a hit is
 * the same answer the test would give. The map is capped and cleared wholesale when it fills, which
 * bounds it without giving a caller a way to decide what stays. */
...
function _namesSomebody(node) {
  ...
  /** @internal The shared reader takes ediPartyName [5] as any non-empty constructed value, so a [5]
   * holding a NULL passes it while encoding no name at all. EDIPartyName is SEQUENCE { nameAssigner
   * [0] DirectoryString OPTIONAL, partyName [1] DirectoryString } (RFC 5280 sec. 4.2.1.6), so the
   * arms it holds are context-tagged and a universal value is not one of them. x400Address [3] is an
   * ORAddress, which this does not read, and is left to the shared reader: the derivation hashes
   * these bytes rather than interpreting them, so an arm it cannot check is still an arm both sides
   * hash alike. */
  /** @internal Walked arm by arm, because the reader answers for most of them and not for all. It
   * requires the IA5 arms [1] [2] [6] to carry characters, [7] to be an address of a real width and
   * [8] to be an OBJECT IDENTIFIER, and it walks [0] and [4] into their own shapes. What it does not
   * do is refuse the two empty forms below, and x400Address [3] it takes on its tag alone, which is
   * left as it is: the derivation hashes these bytes rather than reading them, so an arm neither
   * side reads is still an arm both sides hash alike. */
  for (var i = 0; i < node.children.length; i++) {
    ...
    if (arm.tagNumber === 5 && !_isEdiPartyName(arm)) return false;
  }
  return true;
}
...
/** @internal The universal tags a DirectoryString is written under (RFC 5280 sec. 4.1.2.4):
 * UTF8String, PrintableString, TeletexString, UniversalString and BMPString. */
var DIRECTORY_STRING_TAGS = [12, 19, 20, 28, 30];

/** @internal Whether a [5] arm holds an EDIPartyName: the optional nameAssigner [0] before the
 * required partyName [1], each carrying a DirectoryString. DirectoryString is a CHOICE, so its tag
 * is EXPLICIT however the module tags: the arm is constructed and wraps the string itself, and the
 * string has to be one, since a [1] wrapping a NULL is tagged like a name and holds none. */
function _isEdiPartyName(node) {
  var kids = node.children;
  if (!kids || kids.length < 1 || kids.length > 2) return false;
  for (var i = 0; i < kids.length; i++) {
    var arm = kids[i];
    if (!schema.isContextInRange(arm, 0, 1) || !arm.children || arm.children.length !== 1) return false;
    if (!schema.isUniversalOneOf(arm.children[0], DIRECTORY_STRING_TAGS)) return false;
    /** @internal Read as the string it is tagged as, not counted as one. DirectoryString is
     * SIZE (1..MAX), so an empty one names nobody, and a string whose bytes are not the encoding its
     * tag claims is not a name either. The codec's own reader answers both. */
    var text;
    try { text = asn1.read.string(arm.children[0]); }
    catch (_e) { return false; } // allow:swallow-unverified a string the codec cannot read names nobody, which the caller's refusal states
    if (text.length === 0) return false;
  }
  if (kids.length === 2) return kids[0].tagNumber === 0 && kids[1].tagNumber === 1;
  return kids[0].tagNumber === 1;
}
