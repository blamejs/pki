...
function _verifyOne(msg) {
  ...
  var base = {
    ...
    subject: t && t.subject,
    publicKey: t && t.publicKey && t.publicKey.bytes,
  };
 ...
 * @signature pki.crmf.verifyPop(messages) -> Promise<{ verified, messages: [{ verified, method, cryptographicallyVerified, certReqId, subject, publicKey, reason }] }>
 * @since 0.5.14
 * @status stable
 ...
 * need the CA's decryption key, so they return `verified: false` naming the arm. Each verdict
 * carries the `certReqId`, `subject` and `publicKey` it read, so a CA issues from what was checked.
 *
 * @example
 *   var pair = await pki.key.generate("Ed25519");
