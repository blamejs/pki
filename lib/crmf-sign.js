...
/** @internal Whether a [5] arm holds an EDIPartyName: one or two context-tagged DirectoryStrings,
 * the optional nameAssigner [0] before the required partyName [1]. */
function _isEdiPartyName(node) {
  var kids = node.children;
  if (!kids || kids.length < 1 || kids.length > 2) return false;
  for (var i = 0; i < kids.length; i++) {
    if (!kids[i] || kids[i].tagClass !== "context") return false;
  }
