...
// A valid mailbox has EXACTLY ONE "@" (RFC 5321 addr-spec); a missing or multiple "@" is a malformed address
// -> compare byte-exact rather than case-fold a guessed domain suffix (an "a@b@X" would otherwise fold "X" and
// bind two byte-distinct malformed mailboxes differing only in that suffix's case).
function _rfc822Equal(a, b) {
  var ai = a.indexOf("@"), bi = b.indexOf("@");
  if (ai < 0 || bi < 0 || ai !== a.lastIndexOf("@") || bi !== b.lastIndexOf("@")) return a === b;
