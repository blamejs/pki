...
function _fixCertList(v) {
  ...
  // Own property names, not Object.keys: an element defined non-enumerable is still an element, and
  // every ordinary array operation consumes it, so skipping it would silently drop an anchor. The
  // index test is the language's own -- the canonical decimal form of a uint32 BELOW 2^32-1, which
  // excludes both "length" and the one uint32 that is not an array index, so a property the caller
  // happened to name "4294967295" is not mistaken for a certificate.
  var out = [];
  Object.getOwnPropertyNames(v).forEach(function (k) {
    var i = k >>> 0;
    if (String(i) !== k || i === 0xFFFFFFFF) return;
    out.push(_fixByteish(v[k]));
  });
  return out;
}
