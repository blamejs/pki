...
// The raw GeneralName TLV bytes of a parsed certificate's subjectAltName (empty if none / undecodable).
...
function _sanGeneralNames(parsed) {
  ...
  for (var i = 0; i < exts.length; i++) {
    ...
    try { return (asn1.decode(exts[i].value).children || []).map(function (c) { return c.bytes; }); }
    catch (_e) { return []; }
  }
  return [];
}
...
function _senderBoundToCert(sender, parsed) {
  ...
  for (var i = 0; i < san.length; i++) if (san[i].equals(sender.bytes)) return true;
  return false;
}
...
async function _chainSigner(signer, m, opts, extra) {
  ...
  // path/bad-input from a malformed pool certificate.
  var pool = _certList(opts.intermediates).concat(extra);
