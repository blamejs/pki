...
async function _chainSigner(signer, m, opts, extra) {
  ...
  try {
    ...
    for (ci = 0; ci < res.path.length; ci++) {
      ...
      if (d) _append(chain, Buffer.from(d));
    }
...
// hand the plaintext to caller code and take back whatever it returned as the PBMAC1 key.
...
function _certList(v) {
  ...
  for (var i = 0, n = arr.length; i < n; i++) {
    var c = arr[i];
    _append(out, (Buffer.isBuffer(c) || c instanceof Uint8Array || typeof c === "string")
...
function _fixCertList(v, label) {
  if (v == null) return v;
  if (!Array.isArray(v)) return _fixByteish(v, label);
  ...
  // And the elements are held to the same rule as the option names, through the same primitive:
  // an accessor under an index is caller code exactly as an accessor under a name is, and a list is
  ...
  //
  // Residual, stated rather than implied: a Proxy whose target is an array answers Array.isArray
  // truthfully and runs caller code in its own traps, so the descriptor inspection below is itself
  // trapped. Refusing a proxy needs a check the language does not offer cheaply; what is closed
  // here is the ordinary accessor-backed array.
