...
function _coerce(message) {
  if (message && typeof message === "object" && !_isBuffer(message) && !(message instanceof Uint8Array) &&
  ...
  if (_isBuffer(message) || message instanceof Uint8Array) return cmp.parse(_bufferFrom(message));
...
function _reassemble(m) {
  ...
  if (m.extraCerts != null && m.extraCerts.length) {
    _append(kids,b.explicit(1, b.sequence(m.extraCerts.map(function (c) { return b.raw(c); }))));
  }
...
function _certDer(cert, what) {
  if (_isBuffer(cert) || cert instanceof Uint8Array) return guard.bytes.view(cert, CmpError, "cmp/bad-input", what);
...
function _mailboxSplit(s) {
  ...
  if (_charAt(s, 0) !== "\"" && (local.length === 0 || _charAt(local, 0) === "." || _charAt(local, local.length - 1) === "." || _strIndexOf(local, "..") !== -1 || !/^[A-Za-z0-9!#$%&'*+/=?^_`{|}~.-]+$/.test(local))) return -1;
  return sep;
}
...
function _normalizeUri(u) {
  ...
  if (_strIndexOf(authority, "@") !== authority.lastIndexOf("@")) return null;
  ...
  var hostOk = _charAt(host, 0) === "["
    ? (_charAt(host, host.length - 1) === "]" && ipUtils.expandIpv6Hex(host.slice(1, -1)) !== null)
...
function _bufEq(a, x) {
  ...
  if (!_isBuffer(x)) { if (x instanceof Uint8Array) x = _bufferFrom(x); else return false; }
...
function _capWork(iterationCount, salt, keyLength, prfHash, opts) {
  ...
  if (opts.maxIterations != null) {
    if (typeof opts.maxIterations !== "number" || !isFinite(opts.maxIterations) || opts.maxIterations < 1 || _floor(opts.maxIterations) !== opts.maxIterations) {
...
async function _chainSigner(signer, m, opts, extra) {
  ...
  try {
    ...
    for (ci = 0; ci < pool.length; ci++) {
      var pc = pool[ci];
      if (!_isBuffer(pc) && !(pc instanceof Uint8Array)) continue;
...
function _boundExtraCerts(extra) {
  ...
  for (var i = 0; i < extra.length && out.length < MAX_EXTRA_CERTS && i < MAX_EXTRA_SCAN; i++) {
    var c = extra[i];
    if (!_isBuffer(c) && !(c instanceof Uint8Array)) continue;
    var key = _bufferFrom(c).toString("base64");
...
function _certList(v) {
  ...
  for (var i = 0, n = arr.length; i < n; i++) {
    var c = arr[i];
    _append(out, (_isBuffer(c) || c instanceof Uint8Array || typeof c === "string")
...
function _nonEmptySecret(s) {
  ...
  return (_isBuffer(s) || s instanceof Uint8Array) && s.length > 0;
}
...
function _fixVerifyOptions(opts, made) {
  ...
  // Through the toolkit's own options door, which this verb used to skip in favor of its own two
  // checks. The door does the thing those could not: it REFUSES an options bag whose fields are
  // accessors, and refuses one that changes which fields it carries while they are read.
  //
