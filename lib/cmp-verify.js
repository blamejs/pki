...
async function _chainSigner(signer, m, opts, extra) {
  ...
  try {
    ...
    // res.path is the ordered certificates path.build actually assembled (signer + the intermediates it used),
    // NOT the raw extraCerts -- returning it lets a caller cache only the certs that established trust.
    return { trusted: true, reason: null, chain: res.path };
  } catch (e) {
 ...
 * verdict `signer.chain` is the validated certificate path (the signer plus the intermediates that chained
 * it to the anchor) -- the certificates actually used, never the unsigned `extraCerts` a peer can pad. A
