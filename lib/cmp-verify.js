...
async function _chainSigner(signer, m, opts, extra) {
  ...
  if (room > 0) {
    ...
    var merged = [];
    for (pi = 0; pi < pool.length; pi++) merged.push(pool[pi]);
    for (pi = 0; pi < extra.length && merged.length - pool.length < room; pi++) {
      pk = _certKey(extra[pi]);
      if (pk == null || !seen[pk]) merged.push(extra[pi]);
    }
    pool = merged;
  }
...
//
// Built with an explicit loop, never `arr.map`. This runs AFTER verification has suspended, and
// `Array.prototype.map` is global and replaceable, so a caller who swaps it during that window would
// otherwise have this call dispatch into their code -- and an implementation that recognized the
// unrelated-anchor list and answered with the real CA would make a verification started with only
// the unrelated anchor settle `trusted: true`. Copying the caller's array at the door does not close
// that on its own: the fresh array is still an Array, and the replacement is on the prototype.
function _certList(v) {
  ...
  for (var i = 0, n = arr.length; i < n; i++) {
    var c = arr[i];
    out.push((Buffer.isBuffer(c) || c instanceof Uint8Array || typeof c === "string")
...
function _fixCertList(v) {
  ...
  var out = [];
  for (var i = 0; i < n; i++) out.push(_fixByteish(v[i]));
  return out;
}
