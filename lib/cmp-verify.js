...
/** @internal The MAC protection identifiers RFC 9810 sec. 5.1.3 and RFC 9481 sec. 6 name that this
 * verifier does NOT compute, keyed by OID, each with the reason its verdict carries. A message
 * carrying one is MAC-protected under a flavor that is not verified here, and its verdict says so:
 * it is never the signature case, which sec. 5.1.3.3 leaves open-ended and which an OID absent from
 * this table falls through to. The two legacy identifiers sit outside the RFC 9481 profile. A bare
 * symmetric MAC keys the shared secret directly, with no derivation and no work factor, so one
 * captured message is an offline oracle against a low-entropy secret; PBMAC1 exists to close that,
 * and RFC 9481 sec. 6.2.2 forbids AES-GMAC beside PBKDF2 outright. */
var UNSUPPORTED_MAC_OIDS = _create(null);
function _unsupportedMacs(why, bySection) {
  intrinsic.forEach(intrinsic.keys(bySection), function (section) {
    intrinsic.forEach(bySection[section], function (name) {
      UNSUPPORTED_MAC_OIDS[oid.byName(name)] = { name: name, why: why + " (" + section + ")" };
    });
  });
}
_unsupportedMacs("is not in the RFC 9481 algorithm profile", {
  "RFC 9481 sec. 6.1.1": ["passwordBasedMac"],
  "RFC 9810 sec. 5.1.3.2": ["dhBasedMac"],
});
_unsupportedMacs("is a symmetric-key MAC applied without a key derivation", {
  "RFC 9481 sec. 6.2.1": ["hmacWithSHA1", "hmacWithSHA224", "hmacWithSHA256", "hmacWithSHA384", "hmacWithSHA512"],
  "RFC 9481 sec. 6.2.2": ["aes128-GMAC", "aes192-GMAC", "aes256-GMAC"],
  "RFC 9481 sec. 6.2.3": ["kmacWithSHAKE128", "kmacWithSHAKE256"],
});
...
var _certSki = pkiBuild.skiValueOf;
...
function _capWork(iterationCount, salt, keyLength, prfHash, opts) {
  ...
  /** @internal Read once: the shape check and the comparison must read the same answer, or an
   * option reached through an accessor passes as a number and then answers the comparison with
   * something that is not one, leaving the cap NaN and bounding nothing. */
  var wantedIter = opts.maxIterations;
  if (wantedIter != null) {
    if (typeof wantedIter !== "number" || !_isFinite(wantedIter) || wantedIter < 1 || _floor(wantedIter) !== wantedIter) {
      throw _err("cmp/bad-input", "opts.maxIterations must be a positive integer");
    }
    cap = _min(wantedIter, cap);
  }
...
async function _verify(message, opts) {
  ...
  var unsupportedMac = UNSUPPORTED_MAC_OIDS[protectionAlg.oid];
  if (unsupportedMac !== undefined) {
    return _fail(m, "mac", protectionAlg, "cmp/unsupported-algorithm", "the " + unsupportedMac.name + " protection algorithm " + unsupportedMac.why + "; this verifier computes PBMAC1, KEM-based MAC and signature protection");
  }
 ...
 * `id-KemBasedMac` the KEM path, and a signature `AlgorithmIdentifier` the signature path. An
 * unprotected message fails closed, and so does a MAC identifier this verifier does not compute
 * (`id-PasswordBasedMac`, `id-DHBasedMac`, and the bare symmetric MACs of RFC 9481 sec. 6.2: the SHA-2
 * HMACs, AES-GMAC and KMAC), each reported as `protectionType: "mac"` with `cmp/unsupported-algorithm`
 * rather than as a failed signature. On the
