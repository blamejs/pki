...
// ---- input coercion (mirror cms-verify): reuse a parsed object, else parse a DER Buffer / PEM string. ----
function _coerce(message) {
  if (message && typeof message === "object" && !Buffer.isBuffer(message) && !(message instanceof Uint8Array) &&
    ...
    return message;   // an already-parsed pki.schema.cmp.parse result -- crypto is driven from its raw slices
  }
...
async function _verifySignature(m, protectedPart, protectionAlg, protection, opts) {
  ...
  var hc = _headerChecks(m, opts);
  if (hc) return _verdict(m, "signature", protectionAlg, true, false, hc.code, hc.reason, signer);
...
async function _chainSigner(signer, m, opts) {
  ...
  var time = opts.time || m.header.messageTime || new Date();
 ...
 *   - `time` (Date) -- the validity instant for path validation (default the message `messageTime`, else now).
