...
function verify(message, opts) {
  ...
  return guard.async.deferred(function () {
    fixed = _fixVerifyOptions(opts);
    return _verify(message, fixed);
  }).finally(function () {
    // Only the byte form is ours to clear. A string secret is a JavaScript string: immutable, and
    // interned wherever the caller created it, so there is nothing here to wipe -- a caller who
    // needs the secret gone from memory passes it as bytes.
    var sec = fixed && fixed.sharedSecret;
    if (Buffer.isBuffer(sec) || sec instanceof Uint8Array) {
      guard.secret.zeroizeAll([sec], CmpError, "cmp/bad-input", "the copied PBMAC1 shared secret");
    }
  });
}
...
function _fixVerifyOptions(opts) {
  ...
  var f = {
    sharedSecret: _fixByteish(opts.sharedSecret, "sharedSecret"),
