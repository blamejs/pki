...
async function _verifySignature(m, protectedPart, protectionAlg, protection, opts) {
  ...
  // subject of the protection certificate (failInfo badMessageCheck). Without this binding a holder of ANY
  ...
  var sender = m.header.sender;
  var senderIsDirName = sender && sender.tagClass === "context" && sender.tagNumber === 4 && sender.value && Array.isArray(sender.value.rdns);
  if (!senderIsDirName || !guard.name.dnEqual(sender.value.rdns, signer.parsed.subject.rdns, NS.E, "cmp/sender-mismatch", "the header sender / signer subject")) {
    return _fail(m, "signature", protectionAlg, "cmp/sender-mismatch", "the header sender field does not match the signer certificate subject (RFC 9483 sec. 3.1)", signer);
  }
...
async function _chainSigner(signer, m, opts) {
  ...
  // extraCerts are UNSIGNED (not covered by PKIProtection) so an unauthenticated peer can rewrite them; dedup
  // by exact DER and cap the count before seeding the path-builder pool, so a flood of duplicate / junk certs
  // cannot exhaust the candidate-search budget and degrade an otherwise-valid signer to untrusted.
  var pool = _certList(opts.intermediates).concat(_boundExtraCerts(m.extraCerts));
...
// Deduplicate message-provided extraCerts by exact DER and cap the count -- they are unsigned pool material,
// so a hostile peer can pad them without breaking protection; MAX_EXTRA_CERTS is well above any real chain.
var MAX_EXTRA_CERTS = 32;
function _boundExtraCerts(extra) {
  if (!Array.isArray(extra) || !extra.length) return [];
  var out = [], seen = Object.create(null);
  for (var i = 0; i < extra.length && out.length < MAX_EXTRA_CERTS; i++) {
    var c = extra[i];
