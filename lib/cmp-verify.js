...
function _coerce(message) {
  ...
  if (_isBuffer(message) || util.types.isUint8Array(message)) return cmp.parse(_bufferFrom(message));   // allow:byte-source-narrow -- copy-before-parse only for the two forms _bufferFrom borrows; any other BufferSource routes to cmp.parse below, which snapshots it
  return cmp.parse(message);   // any other BufferSource or a PEM CMP string (cmp.parse snapshots a BufferSource; throws cmp/* on malformed input)
}
