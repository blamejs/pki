...
// throwing door: an identity that cannot be derived returns null, and a non-deduped entry is a
// redundant pool slot rather than a fault. What must not happen is deriving a key from a partial
// object, since two different certificates could then collapse onto the same one.
function _certKey(c) {
  var p = c;
