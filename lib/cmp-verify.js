...
function _coerce(message) {
  ...
  return cmp.parse(message);   // a PEM CMP string (immutable; base64-decodes into fresh bytes, so it needs no copy; throws cmp/* on malformed input)
}
...
// A certificate DER from any BufferSource (Buffer / typed array / DataView / ArrayBuffer,
// detached-view-safe) or a PEM string.
...
// Through the toolkit's one definition of that fold, so no two name comparisons can disagree
// about which characters case-fold.
var _lowerAsciiDomain = guard.name.lowerAscii;

// ---- URI / email grammar predicates by character scan (no regex) ----------------------------------
// The non-alphanumeric members of the RFC 5321 atext set and the RFC 3986 URI character set (the
// A-Za-z0-9 range is checked directly), so charset membership is an indexOf rather than a live regex
// match protocol.
...
// A non-empty RFC 5321 Dot-string atext run (matching /^[A-Za-z0-9!#$%&'*+/=?^_`{|}~.-]+$/).
...
// Every character an RFC 3986 URI character (matching /^[A-Za-z0-9._~:/?#@!$&'()*+,;=%[\]-]*$/; empty is valid).
...
// A "%" not introducing a valid %HH pct-encoding (matching /%(?![0-9A-Fa-f]{2})/): true when malformed.
...
// A scheme + rest split (matching /^([a-zA-Z][a-zA-Z0-9+.-]*):(.*)$/): a leading letter, then
// [a-zA-Z0-9+.-]*, then ":"; returns { scheme, rest } or null. The URI charset gate runs first, so `u`
// carries no CR/LF here and the [^\n] semantics of "." never apply.
...
// The index of the first "/", "?" or "#" (the authority terminator), or -1 (matching search(/[/?#]/)).
...
// A ":" followed by zero-or-more digits, whole string (matching /^:[0-9]*$/).
...
function _normalizeUri(u) {
  ...
  var m = _splitScheme(u);   // scheme ":" rest (RFC 3986 sec. 3.1)
  if (!m) return null;
  ...
  var cut = _firstAuthorityDelim(body);   // authority ends at the first "/", "?" or "#" (RFC 3986 sec. 3.2)
...
function _headerChecks(m, opts) {
  // RFC 9483 sec. 3.5: the pvno MUST be cmp2000(2) or cmp2021(3) (failInfo unsupportedVersion). The parser
  // accepts cmp1999(1) as valid RFC 9810 sec. 5.1.1 syntax; the lightweight profile refuses it on receipt.
  ...
  // RFC 9483 sec. 3.5: the transactionID MUST be present (failInfo badDataFormat).
  ...
  // RFC 9483 sec. 3.5: the senderNonce MUST be present and MUST contain at least 128 bits (failInfo badSenderNonce).
