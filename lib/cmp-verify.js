...
// RFC 9483 sec. 3.1 sender <-> protection-certificate binding. With a POPULATED subject DN, the sender is a
// [4] directoryName equal to it under the RFC 5280 sec. 7.1 canonical comparison. With an EMPTY subject (RFC
// 5280 sec. 4.1.2.6 requires a critical subjectAltName), the sender MUST instead byte-match a subjectAltName
// GeneralName entry -- so an empty-subject certificate is bound to its SAN identity, never accepted anonymously.
function _senderBoundToCert(sender, parsed) {
  ...
  if (subjectRdns.length > 0) {
    ...
    return isDirName && guard.name.dnEqual(sender.value.rdns, subjectRdns, NS.E, "cmp/sender-mismatch", "the header sender / signer subject");
  }
...
// GeneralName equality for the empty-subject SAN binding: a dNSName [2] is compared case-insensitively (RFC
// 5280 sec. 4.2.1.6 -- EE.EXAMPLE and ee.example are the same identity), every other type by exact DER (a
// directoryName re-encoding difference fails closed to a sender-mismatch rather than a false accept). `sender`
// and `sanNode` are both DECODED [n] GeneralName nodes (from schema-cmp / x509.parse), so no re-decode.
function _generalNameMatches(sender, sanNode) {
  ...
  if (sanNode.tagClass === "context" && sanNode.tagNumber === 2 && sanNode.content) {
...
async function _chainSigner(signer, m, opts, extra) {
  ...
  if (room > 0) pool = pool.concat(extra.slice(0, room));
