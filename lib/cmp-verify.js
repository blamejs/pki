...
function _mailboxSplit(s) {
  ...
  // The Domain that follows the separator is a dot-atom / address-literal: it must be non-empty and carry no
  // further "@". A quoted local-part correctly holds the FIRST "@" but the remainder must still be a valid
  // domain, so "a"@b@X (an extra separator in the domain) is malformed -> exact comparison.
  var domain = s.slice(sep + 1);
  return (domain.length === 0 || domain.indexOf("@") !== -1) ? -1 : sep;
}
...
function _normalizeUri(u) {
  ...
  // An authority with no host is malformed: RFC 5280 sec. 4.2.1.6 requires a URI SAN authority to carry an FQDN
  // or IP host. Return null so a hostless "scheme:///path" is compared exactly (its scheme is not case-folded
  // into a false match), since the shared GeneralName parser only enforces printable IA5, not a present host.
  if (host === "") return null;
...
function _resolveSignerCert(m, opts, extra) {
  ...
  function bind(der, p) {   // (der, already-parsed cert) -> the signer object, or null on a senderKID SKI mismatch
    if (senderKID != null) {
      var ski = _certSki(p);
  ...
  function accept(der) {   // an UNTRUSTED message candidate: a non-parseable / mismatched entry is skipped (null)
    var p;
