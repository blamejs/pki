...
function _fixCertList(v) {
  if (v == null) return v;
  return Array.isArray(v) ? v.map(_fixByteish) : _fixByteish(v);
}
function _fixVerifyOptions(opts) {
  ...
  // Every field is read from the caller's object EXACTLY ONCE, into a local, before anything is
  // validated or copied. A property can be an accessor, and an accessor is free to answer a
  // different value each time it is asked, so reading twice is the same defect this function exists
  // to close, moved inside it: the value that passed the check would not be the value that was used.
  var f = {};
  Object.keys(KNOWN_VERIFY_OPTS).forEach(function (k) { f[k] = opts[k]; });
  ...
  return {
    sharedSecret: _fixByteish(f.sharedSecret),
    signerCert: _fixByteish(f.signerCert),
    trustAnchors: _fixCertList(f.trustAnchors),
    intermediates: _fixCertList(f.intermediates),
    // A real Date is re-made at the same instant, so setTime on the caller's object cannot move the
    // evaluation point. Anything else passes through untouched, including a falsy-but-present value,
    // so the rejection it already earns downstream is the one it still gets.
    time: guard.time.isDate(f.time) ? new Date(guard.time.instantOf(f.time)) : f.time,
    transactionID: _fixByteish(f.transactionID),
    expectRecipNonce: _fixByteish(f.expectRecipNonce),
    revocationChecker: f.revocationChecker,
