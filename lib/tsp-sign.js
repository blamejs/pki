...
// is defensive. A malformed ESS value IS reachable (a signed-but-broken attribute) and IS covered;
// the reachable verdicts (unsupported hash, EKU shape, imprint/nonce/policy/binding mismatch,
// untrusted TSA) are each driven by a RED conformance vector.
...
function _checkCertBinding(signerInfo, tsaCertDer) {
  ...
  return Buffer.compare(actual, Buffer.from(essCert.certHash)) === 0 ? true : "tsp/cert-binding-mismatch";
}
...
// single-purpose + critical gate is layered on top -- the OCSP-delegate precedent for a format
// gate above the path checks. Returns true or a tsp/* code.
function _checkTimeStampingEku(tsaCertDer) {
  var cert;
  ...
  var purposes;
  try { purposes = asn1.decode(exts[0].value).children.map(function (c) { return asn1.read.oid(c); }); }
 ...
 *                       TSA cert (validity at genTime, requiredEku timeStamping, revocation). The
 *                       TSA certificate must be issued directly by the anchor; for a TSA under an
 *                       intermediate, pre-validate the chain with `pki.path.validate` and omit the
 *                       anchor here. Omit to verify signature + imprint + binding + EKU only and
...
async function verify(token, data, opts) {
  ...
  // M11 -- the RFC 3161 sec. 2.3 critical single-timeStamping EKU gate.
  var eku = _checkTimeStampingEku(tsaCertDer);
  if (eku !== true) return fail(eku);
  ...
  if (opts.trustAnchor) {
    var pathRes;
    try {
      pathRes = await pathValidate.validate([pkiX509.parse(tsaCertDer)], {
