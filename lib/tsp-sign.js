...
// Any ASCII digit 1-9 present (a non-zero fractional digit), by character scan -- no regex.
...
// The signer certificate DER (a DER BufferSource / PEM string) -- for the ESSCertIDv2 hash.
function _certDer(c) {
  if (c == null) throw _err("tsp/bad-input", "the TSA signer requires a certificate (cert)");
  if (guard.bytes.isByteSource(c)) {                                                // any BufferSource -> a byte view
    var buf = guard.bytes.source(c, TspError, "tsp/bad-input", "the TSA certificate");
    if (buf[0] === 0x30) return buf;                                                // DER as-is
    c = buf.toString("latin1");                                                     // else decode as PEM
  }
...
// The three caller-owned objects pki.tsp.sign reads. A dropped option here is a token that quietly
// lacks what was asked for -- `odering` for `ordering` emitted a token with the ordering flag unset,
// which is a claim about the timestamp's orderability that the caller believes they made.
// The TSA is gated too, and the reason it needs to be is not the one it looks like. Both fields it
// carries are REQUIRED, so a MISSPELLING of either is already refused for what it leaves missing --
// but that is only half the class. A name belonging on the OPTIONS, written on the TSA instead, is
// read by nothing here: `ordering` placed on the TSA emits a token the same size as one that never
// asked for it, with the flag unset, while the caller believes they set it. Requiring the argument
// to carry only what it reads is what closes that, so all three caller-owned arguments answer the
// same question -- is this a name I read? -- rather than two of them answering it.
...
// Accuracy ::= SEQUENCE { seconds INTEGER OPTIONAL, millis [0] OPTIONAL, micros [1] OPTIONAL }
// (RFC 3161 sec. 2.4.2). All three are OPTIONAL and encoded only when present, so a misspelling is
// missed by nothing: `milis` emitted an EMPTY Accuracy that parses back as zero seconds, millis and
// micros, and the token then understates its own precision -- a claim about the timestamp that the
 ...
 */
// The options pki.tsp.request encodes into the TimeStampReq. Every one is OPTIONAL in the
// structure, so a misspelled name is missed by nothing and the request goes out claiming less than
// was asked for. `nonce` is the one that costs most: misspelled, the request carries no nonce at
// all, and the caller matches a reply against a replay defense that was never asked for.
...
function request(messageImprint, opts) {
  ...
  var mi = messageImprint || {};
  // The same table pki.tsp.sign holds its imprint to. The request's own options live in the second
  // argument, so `nonce` written on the imprint reaches nothing and the request goes out without
  // the replay defense the caller believes they asked for.
  // The same table pki.tsp.sign holds its imprint to. The request's own options live in the second
  // argument, so `nonce` written on the imprint reaches nothing and the request goes out without
  // the replay defense the caller believes they asked for.
 ...
 */
// The options pki.tsp.response encodes into the TimeStampResp. `status` carries the verdict and
// defaults to granted(0), so a misspelled name does not lose a detail -- it INVERTS the answer: a
// TSA refusing a request emits a granted response instead, over a token it did mean to withhold.
...
// Does any extension carry the critical flag? The answer IS the unknown-critical-extension gate, so
// it is decided by comparison instead of by a `some`. See guard-list.
...
async function verify(token, data, opts) {
  ...
  // An unrecognized option is refused rather than ignored. The anchor option is `trustAnchors` (an
  // anchor tuple, or a non-empty array of them), the same spelling pki.cms.verify / pki.cmp.verify use.
  // A caller who mistyped it would otherwise get no anchoring and no error: the TSA certificate
  // unchained, `valid: true`, and nothing to notice it by. Naming it at the boundary is cheap.
  ...
  // The scan for a critical extension IS the gate, so it is a loop rather than a `some`: one
  // answering false makes the unrecognized critical extension invisible and the token is accepted.
  ...
  // key-param inheritance, requiredEku, optional revocation), only when trustAnchors is supplied.
