...
async function verify(token, data, opts) {
  ...
  function fail(code, reason) {
    return { valid: false, trusted: false, revocationChecked: false, anchorConstraints: null, code: code, reason: reason || null, genTime: tst.genTime, accuracy: tst.accuracy, serialNumber: tst.serialNumber, serialNumberHex: tst.serialNumberHex, policy: tst.policy, nonce: tst.nonce, tsa: tst.tsa, tstInfo: tst, signer: null };
  }
  ...
  var trusted = false;
  // With no trust anchor no path ran, so nothing about revocation was established: false, not
  // "not applicable". The field says what WAS checked, and here the answer is nothing.
  ...
  if (opts.trustAnchor) {
    ...
    trusted = true;
    // What the path established travels with the verdict. Reduced to a bare `trusted: true`, this
    // answered the same way whether the TSA was established un-revoked or revocation was never
    // consulted at all -- and revocation only runs when a revocationChecker is supplied, so the
    // second case is the DEFAULT. A caller archiving a timestamp verdict could not tell the two
    // apart later, which is the distinction the valid/trusted split exists to keep.
