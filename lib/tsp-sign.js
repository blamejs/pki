...
function _orderTsaChain(leaf, pool) {
  ...
  for (var depth = 0; depth < pool.length; depth++) {
    ...
    for (var i = 0; i < pool.length; i++) {
      // skip a cert that IS the working cert -- identified by issuer + serialNumber (RFC 5280 cert
      // identity), never the serial alone (two issuers may reuse a serial) -- so a self-issued cert
      // is not chained to itself.
      if (used[i] || (pool[i].serialNumberHex === current.serialNumberHex && guard.name.dnEqual(pool[i].issuer.rdns, current.issuer.rdns, TspError, "tsp/bad-tsa-certificate"))) continue;
