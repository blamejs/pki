...
var b = asn1.build;
// The runtime operations this module builds and verifies tokens with, bound at load. A replaced
// Array.prototype.push drops a field out of an encoded structure, and a replaced Date decides the
// genTime a token attests to.
...
function _rankRevocation(v) {
  var r = _REVOCATION_RANK[intrinsic.String(v)];
...
function _hashAlgId(name) {
  if (!NODE_DIGEST[name]) throw _err("tsp/unsupported-algorithm", "unsupported hash algorithm " + intrinsic.stringify(name));
...
function _certDer(c) {
  ...
  if (c instanceof Uint8Array && !intrinsic.isBuffer(c)) c = intrinsic.bufferFrom(c);          // a Uint8Array -> Buffer
  if (intrinsic.isBuffer(c)) { if (c[0] === 0x30) return c; c = _bufToString(c, "latin1"); }   // DER as-is, else decode as PEM
  ...
  return intrinsic.bufferFrom(m[1].replace(/[^A-Za-z0-9+/=]/g, ""), "base64");
}
...
function _sign(messageImprint, tsa, opts) {
  opts = opts || {};
  if (typeof opts !== "object" || intrinsic.isBuffer(opts)) throw _err("tsp/bad-input", "pki.tsp.sign options must be an object");
  ...
  if (!intrinsic.isBuffer(mi.hashedMessage) && !(mi.hashedMessage instanceof Uint8Array)) throw _err("tsp/bad-input", "messageImprint.hashedMessage must be a Buffer");
  ...
  if (!NODE_DIGEST[certHashAlg]) throw _err("tsp/unsupported-algorithm", "unsupported certHashAlgorithm " + intrinsic.stringify(certHashAlg));
  ...
  var imprint = b.sequence([_hashAlgId(mi.hashAlgorithm), b.octetString(intrinsic.bufferFrom(mi.hashedMessage))]);
  ...
  var genTime = guard.time.isDate(opts.genTime) ? opts.genTime : new _Date();
  ...
  if (opts.accuracy) _push(fields, _accuracy(opts.accuracy));
  if (opts.ordering === true) _push(fields, b.boolean(true));
  if (opts.nonce != null) _push(fields, b.integer(guard.range.authoredInteger(opts.nonce, _err, "tsp/bad-input", "nonce")));
...
function _accuracy(a) {
  var f = [];
  if (a.seconds != null) {
    var s = intrinsic.Number(a.seconds);
    if (!intrinsic.isInteger(s) || s < 0 || s > 0x7fffffff) throw _err("tsp/bad-input", "Accuracy seconds must be a non-negative integer");
    _push(f, b.integer(intrinsic.BigInt(s)));
  }
  if (a.millis != null) _push(f, b.contextPrimitive(0, _subMilliBytes(a.millis, "millis")));
  if (a.micros != null) _push(f, b.contextPrimitive(1, _subMilliBytes(a.micros, "micros")));
  return b.sequence(f);
}
...
function _subMilliBytes(n, label) {
  ...
  if (!intrinsic.isInteger(v) || v < 1 || v > 999) throw _err("tsp/bad-input", "Accuracy " + label + " must be an integer in 1..999 (RFC 3161 sec. 2.4.2)");
  return b.integer(intrinsic.BigInt(v)).subarray(2);               // strip the universal INTEGER tag+length
}
...
function _failInfoBits(names) {
  if (!intrinsic.isArray(names)) throw _err("tsp/bad-input", "failInfo must be an array of PKIFailureInfo names");
  var idxs = intrinsic.map(names, function (n) {
    var i = FAILINFO_BIT[n];
    if (i == null) throw _err("tsp/bad-input", "unknown PKIFailureInfo name " + intrinsic.stringify(n) + " (RFC 3161 sec. 2.4.2)");
    return i;
  });
...
function _tokenDer(t) {
  if (intrinsic.isBuffer(t) && t[0] === 0x30) return t;
  if (t instanceof Uint8Array && !intrinsic.isBuffer(t)) { var u = intrinsic.bufferFrom(t); if (u[0] === 0x30) return u; }
...
function request(messageImprint, opts) {
  opts = opts || {};
  if (typeof opts !== "object" || intrinsic.isBuffer(opts)) throw _err("tsp/bad-input", "pki.tsp.request options must be an object");
  var mi = messageImprint || {};
  _assertImprint(mi);
  var imprint = b.sequence([_hashAlgId(mi.hashAlgorithm), b.octetString(intrinsic.bufferFrom(mi.hashedMessage))]);
  ...
  if (opts.reqPolicy != null) _push(fields, _policy(opts.reqPolicy));
  if (opts.nonce != null) _push(fields, b.integer(guard.range.authoredInteger(opts.nonce, _err, "tsp/bad-input", "nonce")));
  if (opts.certReq != null && typeof opts.certReq !== "boolean") throw _err("tsp/bad-input", "certReq must be a boolean");
  if (opts.certReq === true) _push(fields, b.boolean(true));
  if (opts.extensions != null) {
    if (!intrinsic.isArray(opts.extensions) || !intrinsic.every(opts.extensions, function (e) { return intrinsic.isBuffer(e) || e instanceof Uint8Array; })) throw _err("tsp/bad-input", "extensions must be an array of encoded Extension DER buffers");
    ...
    var seenExt = {};
    var encoded = intrinsic.map(opts.extensions, function (e, i) {
      var der = intrinsic.bufferFrom(e);
    ...
    if (encoded.length) _push(fields, b.contextConstructed(0, intrinsic.bufferConcat(encoded)));
  }
...
function response(token, opts) {
  opts = opts || {};
  if (typeof opts !== "object" || intrinsic.isBuffer(opts)) throw _err("tsp/bad-input", "pki.tsp.response options must be an object");
  var status = opts.status == null ? 0 : intrinsic.Number(opts.status);
  if (!intrinsic.isInteger(status) || status < 0 || status > 5) throw _err("tsp/bad-input", "PKIStatus must be an integer in 0..5 (RFC 3161 sec. 2.4.2)");
  ...
  var siFields = [b.integer(intrinsic.BigInt(status))];
  if (opts.statusString != null) {
    var texts = intrinsic.isArray(opts.statusString) ? opts.statusString : [opts.statusString];
    ...
    _push(siFields, b.sequence(intrinsic.map(texts, function (t) { return b.utf8(intrinsic.String(t)); })));
  }
  if (opts.failInfo != null) {
    ...
    _push(siFields, _failInfoBits(opts.failInfo));
  }
  var respFields = [b.sequence(siFields)];
  if (token != null) _push(respFields, b.raw(_tokenDer(token)));
...
function _imprintMatches(mi, data) {
  ...
  var actual;
  if (intrinsic.isBuffer(data) || data instanceof Uint8Array) {
    actual = nodeCrypto.createHash(nodeAlg).update(intrinsic.bufferFrom(data)).digest();
  } else if (data && typeof data === "object" && (intrinsic.isBuffer(data.hashedMessage) || data.hashedMessage instanceof Uint8Array)) {
    ...
    actual = intrinsic.bufferFrom(data.hashedMessage);
  } else {
    throw _err("tsp/bad-input", "pki.tsp.verify data must be a Buffer or a { hashAlgorithm, hashedMessage } imprint");
  }
  return intrinsic.compare(actual, intrinsic.bufferFrom(mi.hashedMessage)) === 0 ? true : "tsp/imprint-mismatch";
}
...
function _checkCertBinding(signerInfo, tsaCertDer) {
  ...
  if (intrinsic.compare(actual, intrinsic.bufferFrom(essCert.certHash)) !== 0) return "tsp/cert-binding-mismatch";
...
function _checkTsaCertUsage(tsaCertDer) {
  ...
  try { purposes = intrinsic.map(ekuNode.children, function (c) { return asn1.read.oid(c); }); }
  catch (_e) { return "tsp/bad-eku"; }
...
function _buildTsaChains(leaf, pool) {
  ...
  function dfs(current, acc, used) {
    ...
    _push(chains, intrinsic.arraySlice(acc));   // acc is leaf..current; the anchor may validate the path at this length
  ...
  dfs(leaf, [leaf], []);
  return intrinsic.map(chains, function (c) { return intrinsic.arraySlice(c).reverse(); }).sort(function (a, b) { return b.length - a.length; });
}
...
async function verify(token, data, opts) {
  opts = opts || {};
  if (typeof opts !== "object" || intrinsic.isBuffer(opts)) throw _err("tsp/bad-input", "pki.tsp.verify options must be an object");
  ...
  if (opts.certs != null && (!intrinsic.isArray(opts.certs) || !intrinsic.every(opts.certs, function (c) { return intrinsic.isBuffer(c) || c instanceof Uint8Array; }))) {
  ...
  if (tst.extensions && intrinsic.some(tst.extensions, function (e) { return e.critical; })) {
  ...
  if (opts.trustAnchor) {
    ...
    var ceilT = (tst.genTimeFraction != null && /[1-9]/.test(_strSlice(tst.genTimeFraction, 3))) ? new _Date(guard.time.instantOf(floorT) + 1) : floorT;
    try {
      ...
      (opts.certs || []).forEach(function (c) { _push(pool, pkiX509.parse(c)); });
    ...
    } catch (e) { return fail("tsp/untrusted-tsa", (e && e.message) || intrinsic.String(e)); }
