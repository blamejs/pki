...
/** @internal The shared certificate-extension decoders, so an extension this module reads is read
 * under the rules every other consumer reads it under rather than under a second set written here. */
var _EXT_DECODERS = pkix.certExtensionDecoders(_NS).byOid;
var b = asn1.build;
...
var HASH_LEN = constants.NAMES.DIGEST_OCTETS;

var _REVOCATION_RANK = intrinsic.assign(intrinsic.create(null), { "false": 0, "undetermined": 1, "waived": 2, "determined": 3 });
...
function _hashAlgId(name) {
  if (!NODE_DIGEST[guard.text.keyOf(name)]) throw _err("tsp/unsupported-algorithm", "unsupported hash algorithm " + guard.text.showValue(name));
...
function _certDer(c) {
  ...
  var der = pkix.pemDecodeLenient(c, "CERTIFICATE", _err, "tsp/bad-input");
 ...
 * token to the TSA certificate is attached automatically. The TSA certificate is held to
 * sec. 2.3 before anything is signed: one critical extendedKeyUsage naming id-kp-timeStamping
 * alone, and a keyUsage, when present, that allows signing; a certificate that fails is refused
 * with the code `pki.tsp.verify` would answer (`tsp/bad-eku`, `tsp/eku-not-critical`,
 * `tsp/eku-not-exclusive`, `tsp/bad-key-usage`).
 *
 ...
 * @opts  genTime       The trusted time (a `Date`). Default: now. Milliseconds are kept as the
 *                      sec. 2.4.2 fraction of a second, which `pki.schema.tsp.parseToken` reads back.
 * @opts  nonce         The request nonce to echo (a number or BigInt), for replay protection.
 * @opts  accuracy      The genTime +/- accuracy, as `{ seconds?, millis?, micros? }`, naming at
 *                      least one field; omit it to make no accuracy claim.
 ...
 * @opts  digestAlgorithm / pss  Passed through to `pki.cms.sign` (the signer digest algorithm, RSA-PSS padding).
 * @opts  additionalSignedAttributes  Extra signed attributes to carry on the token, as `[{ type, values }]` (forwarded to `pki.cms.sign`).
...
var _TSA_KEYS = intrinsic.assign(intrinsic.create(null), { cert: 1, key: 1 });
var _IMPRINT_KEYS = intrinsic.assign(intrinsic.create(null), { hashAlgorithm: 1, hashedMessage: 1 });
var _SIGN_OPTS = intrinsic.assign(intrinsic.create(null), {
...
function _sign(messageImprint, tsa, opts) {
  ...
  if (!mi.hashAlgorithm || !NODE_DIGEST[guard.text.keyOf(mi.hashAlgorithm)]) throw _err("tsp/unsupported-algorithm", "messageImprint.hashAlgorithm must be a supported hash name");
  ...
  /** @internal Sec. 2.3 binds the TSA: its certificate carries one critical extendedKeyUsage naming
   *  id-kp-timeStamping alone, and a keyUsage, when present, that allows signing. The verifier
   *  refuses a token under any other certificate, so the same check runs here, with the same
   *  codes, before a token no relying party would accept is signed. */
  var certFault = _checkTsaCertUsage(certDer);
  if (certFault !== true) throw _err(certFault, "the TSA certificate does not satisfy RFC 3161 sec. 2.3 (" + certFault + "): a relying party would refuse every token signed under it");
  var certHashAlg = opts.certHashAlgorithm || "sha256";
  if (!NODE_DIGEST[guard.text.keyOf(certHashAlg)]) throw _err("tsp/unsupported-algorithm", "unsupported certHashAlgorithm " + guard.text.showValue(certHashAlg));
  ...
  if (opts.genTime != null) guard.time.assertEncodable(opts.genTime, _err, "tsp/bad-input", "genTime");
  ...
  /** @internal Sec. 2.4.2 lets genTime carry a fraction of a second, which the parser reads, so a
   *  Date's milliseconds reach the token rather than being dropped at the encoder. */
  var fields = [b.integer(1n), _policy(opts.policy), imprint, b.integer(serial), b.generalizedTime(genTime, { fractional: true })];
  if (opts.accuracy) guard.list.append(fields,_accuracy(opts.accuracy));
  if (opts.ordering === true) guard.list.append(fields,b.boolean(true));
  if (opts.nonce != null) guard.list.append(fields,b.integer(guard.range.authoredInteger(opts.nonce, _err, "tsp/bad-input", "nonce")));
...
var _ACCURACY_KEYS = intrinsic.assign(intrinsic.create(null), { seconds: 1, millis: 1, micros: 1 });
...
function _accuracy(a) {
  ...
  var f = [];
  var sec = a.seconds, mil = a.millis, mic = a.micros;
  if (sec != null) {
    if (typeof sec !== "number" && typeof sec !== "bigint") throw _err("tsp/bad-input", "Accuracy seconds must be a non-negative integer, got " + guard.text.showValue(sec));
    var s = intrinsic.Number(sec);
    if (!Number.isInteger(s) || s < 0 || s > 0x7fffffff) throw _err("tsp/bad-input", "Accuracy seconds must be a non-negative integer");
    guard.list.append(f,b.integer(BigInt(s)));
  }
  if (mil != null) guard.list.append(f,b.contextPrimitive(0, _subMilliBytes(mil, "millis")));
  if (mic != null) guard.list.append(f,b.contextPrimitive(1, _subMilliBytes(mic, "micros")));
  /** @internal Sec. 2.4.2 reads an absent field as zero, so an Accuracy naming none asserts a zero
   *  deviation; a caller with no accuracy to state omits the option instead. */
  if (!f.length) throw _err("tsp/bad-input", "accuracy must name at least one of seconds, millis, micros; omit it to make no accuracy claim (RFC 3161 sec. 2.4.2)");
  return b.sequence(f);
}
function _subMilliBytes(n, label) {
  if (typeof n !== "number" && typeof n !== "bigint") throw _err("tsp/bad-input", "Accuracy " + label + " must be an integer in 1..999 (RFC 3161 sec. 2.4.2), got " + guard.text.showValue(n));
  var v = intrinsic.Number(n);
...
var FAILINFO_BIT = intrinsic.assign(intrinsic.create(null), { badAlg: 0, badRequest: 2, badDataFormat: 5, timeNotAvailable: 14, unacceptedPolicy: 15, unacceptedExtension: 16, addInfoNotAvailable: 17, systemFailure: 25 });
...
function _failInfoBits(names) {
  ...
  var idxs = names.map(function (n) {
    var i = FAILINFO_BIT[guard.text.keyOf(n)];
 ...
 */
var _REQUEST_OPTS = intrinsic.assign(intrinsic.create(null), { reqPolicy: 1, nonce: 1, certReq: 1, extensions: 1, pem: 1 });
...
function request(messageImprint, opts) {
  ...
  if (opts.reqPolicy != null) guard.list.append(fields,_policy(opts.reqPolicy));
  if (opts.nonce != null) guard.list.append(fields,b.integer(guard.range.authoredInteger(opts.nonce, _err, "tsp/bad-input", "nonce")));
  if (opts.certReq != null && typeof opts.certReq !== "boolean") throw _err("tsp/bad-input", "certReq must be a boolean");
  if (opts.certReq === true) guard.list.append(fields,b.boolean(true));
  if (opts.extensions != null) {
    ...
    var seenExt = intrinsic.create(null);
    ...
    if (encoded.length) guard.list.append(fields,b.contextConstructed(0, Buffer.concat(encoded)));
  }
 ...
 */
var _RESPONSE_OPTS = intrinsic.assign(intrinsic.create(null), { status: 1, statusString: 1, failInfo: 1, pem: 1 });
...
function response(token, opts) {
  ...
  var st = opts.status;
  if (st != null && typeof st !== "number" && typeof st !== "bigint") throw _err("tsp/bad-input", "PKIStatus must be an integer in 0..5 (RFC 3161 sec. 2.4.2), got " + guard.text.showValue(st));
  var status = st == null ? 0 : intrinsic.Number(st);
  ...
  if (opts.statusString != null) {
    ...
    guard.list.append(siFields,b.sequence(texts.map(function (t) { return b.utf8(String(t)); })));
  }
  if (opts.failInfo != null) {
    ...
    guard.list.append(siFields,_failInfoBits(opts.failInfo));
  }
  var respFields = [b.sequence(siFields)];
  if (token != null) guard.list.append(respFields, b.raw(_tokenDer(token)));
...
var _VERIFY_DIGEST_LEN = constants.NAMES.DIGEST_OCTETS;
...
function _checkTsaCertUsage(tsaCertDer) {
  ...
  /** @internal Read through the shared decoder, which is where the sec. 4.2.1.12 rules live: an
   * ExtKeyUsage carries at least one KeyPurposeId. Reading the SEQUENCE here instead called an
   * empty one well-formed and then reported it as the wrong PURPOSE, so the same class of fault
   * answered with a different code here than at every other consumer of this extension. */
  var purposes;
  try { purposes = _EXT_DECODERS[O("extKeyUsage")](exts[0].value); }
  catch (_e) { return "tsp/bad-eku"; }   // allow:swallow-unverified this verb answers with a code rather than throwing, and every malformed shape is the one code
...
function _tsaMatchesCert(tsaBytes, signerCert) {
  ...
  if (sanExt) {
    /** @internal Gated by the shared decoder before the comparison, so a SAN this toolkit refuses
     * everywhere else cannot be a source of identity here. The match itself stays a comparison of
     * the encoded GeneralName bytes, which is what "the token names this TSA" means: the name in
     * the token and the name in the certificate have to be the same name, not merely names that
     * decode alike. */
    var names;
    try { names = _EXT_DECODERS[O("subjectAltName")](sanExt.value).names || []; }
    catch (_e) { return false; }   // allow:swallow-unverified a SAN that does not decode names no TSA, which is what this verb answers
    for (var i = 0; i < names.length; i++) {
      if (names[i].bytes && names[i].bytes.equals(tsaBytes)) return true;
    }
  }
  return false;
}
...
function _buildTsaChains(leaf, pool) {
  ...
  function dfs(current, acc, used) {
    ...
    guard.list.append(chains, acc.slice());
    for (var i = 0; i < pool.length && chains.length < MAX_TSA_CHAINS; i++) {
      ...
      if (guard.name.dnEqual(pool[i].subject.rdns, current.issuer.rdns, _err, "tsp/bad-tsa-certificate", "the TSA certificate subject")) {
        used[i] = true; guard.list.append(acc, pool[i]);
        dfs(pool[i], acc, used);
 ...
 * serialNumberHex, policy, policyName, nonce, tsa, tstInfo, signer, code?, reason? }`. `valid` is true only
 ...
 */
var _VERIFY_OPTS = intrinsic.assign(intrinsic.create(null), { certs: 1, trustAnchors: 1, nonce: 1, reqPolicy: 1, revocationChecker: 1, allowWeakDigests: 1 });
...
async function verify(token, data, opts) {
  ...
  /** @internal Every option is taken once, here, and each pass below reads that copy. A token is
   * examined in several passes: the signature over it, then the chain from the signer to the
   * caller's anchors. An option left on the caller's object is read again by each, and an option
   * reached through an accessor answers each read separately, so the certificates that verified the
   * signature need not be the certificates the chain was built from. */
  opts = guard.identifier.snapshotOptions(opts, _VERIFY_OPTS);
  ...
  function fail(code, reason) {
    return guard.verdict.of({ valid: false, trusted: false, revocationChecked: revocationChecked, anchorConstraints: anchorConstraints, code: code, reason: reason || null, genTime: tst.genTime, accuracy: tst.accuracy, serialNumber: tst.serialNumber, serialNumberHex: tst.serialNumberHex, policy: tst.policy, policyName: tst.policyName, nonce: tst.nonce, tsa: tst.tsa, tstInfo: tst, signer: null });
  }
  var cmsRes = await cmsVerify.verify(tokenDer, { certs: opts.certs, allowWeakDigests: opts.allowWeakDigests === true });
  ...
  if (opts.trustAnchors) {
    ...
    try {
      ...
      (opts.certs || []).forEach(function (c) { guard.list.append(pool, pkiX509.parse(c)); });
