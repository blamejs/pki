...
function _accuracy(a) {
  var f = [];
  if (a.seconds != null) f.push(b.integer(BigInt(a.seconds)));
  if (a.millis != null) f.push(b.contextPrimitive(0, _intBytes(a.millis)));
  if (a.micros != null) f.push(b.contextPrimitive(1, _intBytes(a.micros)));
  return b.sequence(f);
}
// The minimal two's-complement content octets of a small non-negative integer (for the IMPLICIT
// [0]/[1] Accuracy fields, whose content is an INTEGER value without the universal tag).
function _intBytes(n) {
  var v = Number(n);
  if (!Number.isInteger(v) || v < 0 || v > 0x7fffffff) throw _err("tsp/bad-input", "an Accuracy field must be a small non-negative integer");
  var der = b.integer(BigInt(v));                       // 02 len <content>
  return der.subarray(2);                                // strip the universal INTEGER tag+length
}
