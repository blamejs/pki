...
function _checkCertBinding(signerInfo, tsaCertDer) {
  ...
  if (essCert.issuerSerial) {
    ...
    if (!gn || gn.tagNumber !== 4 || !gn.value || !guard.name.dnEqual(gn.value.rdns, signerCert.issuer.rdns, TspError, "tsp/cert-binding-mismatch")) {
...
function _tsaMatchesCert(tsaBytes, signerCert) {
  ...
  if (gn.tagNumber === 4 && gn.value && guard.name.dnEqual(gn.value.rdns, signerCert.subject.rdns, TspError, "tsp/tsa-mismatch")) return true;
...
function _buildTsaChains(leaf, pool) {
  ...
  function dfs(current, acc, used) {
    ...
    for (var i = 0; i < pool.length && chains.length < MAX_TSA_CHAINS; i++) {
      ...
      if (pool[i].serialNumberHex === current.serialNumberHex && guard.name.dnEqual(pool[i].issuer.rdns, current.issuer.rdns, TspError, "tsp/bad-tsa-certificate")) continue;
      if (guard.name.dnEqual(pool[i].subject.rdns, current.issuer.rdns, TspError, "tsp/bad-tsa-certificate")) {
...
async function verify(token, data, opts) {
  ...
  if (opts.trustAnchor) {
    var pathRes = null;
    // tst.genTime floors to millisecond precision. If genTime carries sub-millisecond digits the true
    // instant is LATER, so validate the path at the next millisecond (ceil): otherwise a token minted
    // just after the TSA certificate expired (notAfter 12:00:00Z, genTime 12:00:00.0001Z) would be
    // validated at the earlier floored instant and wrongly accepted. Certificate validity is
    // seconds-precise (RFC 5280 sec. 4.1.2.5), so a <=1ms ceil never over-rejects a valid token.
    var vtime = tst.genTime;
    if (tst.genTimeFraction != null && /[1-9]/.test(tst.genTimeFraction.slice(3))) {
      vtime = new Date(tst.genTime.getTime() + 1);
    }
    try {
      ...
      // happens here -- accept the TSA certificate if ANY enumerated chain validates to the anchor.
      ...
      for (var ci = 0; ci < chains.length && !(pathRes && pathRes.valid); ci++) {
        pathRes = await pathValidate.validate(chains[ci], {
          time: vtime, trustAnchor: opts.trustAnchor, requiredEku: ["timeStamping"], revocationChecker: opts.revocationChecker,
        });
      }
