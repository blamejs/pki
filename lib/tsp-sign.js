...
// The TSA is deliberately NOT gated, on a property of the argument rather than a judgment about
// it: both fields it carries are REQUIRED, so a misspelled name is already refused for what it
// leaves missing. `certificate` for `cert` raises "the TSA signer requires a certificate (cert)",
// and a misspelled key reaches no signer at all. No spelling of this argument yields a token that
// claims less than the caller asked for, which is the failure a door here would catch; callers
// also hand a key store's object through whole. The imprint and the options are the opposite
// case -- a dropped name there is missed by nothing, and the token is emitted claiming whatever
// survived, so both are gated. The same test decides any other argument: does omitting a field
// refuse the call, or change what the artifact claims?
