  ...
  if (opts.trustAnchor) {
    ...
    try {
      ...
      for (var ci = 0; ci < chains.length && !(pathRes && pathRes.valid); ci++) {
        ...
        // ACCUMULATED across candidates, never overwritten, because the two branches below answer
        // two different questions and a bare last-writer-wins would answer neither. Backtracking
        // tries several chains for the same TSA certificate, and a later candidate that fails
        // before its revocation step would otherwise erase what an earlier one established.
        if (_moreEstablished(pathRes.revocationChecked, revocationChecked)) {
