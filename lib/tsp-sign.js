...
// The TSA is deliberately NOT gated. It is a caller-owned CREDENTIAL BAG rather than a field spec:
// this toolkit's own signer helpers hand over `{ cert, key, keyObject, spki }` whole, and callers
// pass whatever their key store produced. Only `cert` and `key` are read, and a table listing the
// other two would name keys nothing reads -- which reopens the hole the door exists to close --
// while a table of just the two would refuse a shape the toolkit itself produces. The imprint and
// the options are different: both are field specs a caller writes out by hand, which is where a
// typo happens and where a dropped key changes what the token claims.
