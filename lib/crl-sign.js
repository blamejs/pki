...
/** @internal The builder is handed no decoder map: this module does not use the builder's
 * extension-building path, it walks the pre-encoded array itself below and validates each value
 * against this map there. */
...
/** @internal A pre-encoded extension is admitted on the shape of its Extension SEQUENCE, which says
 * nothing about whether extnValue decodes as the structure its extnID names. Where a decoder is
 * registered for that OID, the value is put through it, so a value that cannot be read is refused
 * here rather than signed and left for a relying party to find. The decoder's own typed code is
 * kept, since it names the field that failed. Called AFTER this module's own profile checks, so an
 * extension it already answers for keeps the code it has always raised and this only reaches the
 * ones nothing was checking. */
function _assertExtensionValueDecodes(node, extnId, what) {
  var dec = EXT_DECODERS[extnId];
  if (!dec) return;
  try { dec(asn1.read.octetString(node.children[node.children.length - 1])); }
  catch (err) {
    if (err instanceof CrlError) throw err;
    throw _err("crl/bad-input", "pre-encoded " + what + " " + (oid.name(extnId) || extnId) + " value is malformed", err);
  }
}
...
var KNOWN_CRL_EXT_KEYS = intrinsic.assign(intrinsic.create(null), { authorityKeyIdentifier: 1, issuerAltName: 1, issuingDistributionPoint: 1, deltaCRLIndicator: 1, freshestCRL: 1, authorityInfoAccess: 1 });
...
var KNOWN_SPEC_KEYS_ISSUER_NAMED = intrinsic.assign(intrinsic.create(null), {
...
var KNOWN_IDP_KEYS = intrinsic.assign(intrinsic.create(null), { fullName: 1, onlyContainsUserCerts: 1, onlyContainsCACerts: 1, onlyContainsAttributeCerts: 1, indirectCRL: 1 });

var KNOWN_REVOKED_KEYS = intrinsic.assign(intrinsic.create(null), {
...
var REQUIRED_ENTRY_CRITICALITY = intrinsic.create(null);
(function () {
  var shared = crlSchema.entryExtensionCriticality(oid);
  intrinsic.forEach(intrinsic.keys(shared), function (k) { REQUIRED_ENTRY_CRITICALITY[k] = shared[k].critical; });
})();
...
function _akiKeyId(val, ctx) {
  ...
  if (guard.bytes.isByteSource(val)) {
    var given = guard.bytes.snapshotSource(val, CrlError, "crl/bad-input", "the authorityKeyIdentifier keyIdentifier");
    /** @internal Sec. 5.2.1 names the CRL signer certificate's subject key identifier as the value,
     *  so a stated one is held to it when that certificate is at hand. */
    if (issuerSki && !_bufferEquals(given, issuerSki)) {
      throw _err("crl/bad-input", "the authorityKeyIdentifier keyIdentifier must equal the issuer certificate's subjectKeyIdentifier (RFC 5280 sec. 5.2.1); omit it to take that value");
    }
    return given;
  }
...
function _freshestValue(spec) {
  ...
  var dps = dense.every(function (e) { return e && typeof e === "object" && !Buffer.isBuffer(e) && guard.identifier.optionNames(e).length === 1 && e.fullName != null; })
...
function _buildCrlExtensions(spec, ctx) {
  var out = [], seen = intrinsic.create(null), isDelta = false;
  ...
  if (Array.isArray(ext)) {
    ...
    ext.forEach(function (e, i) {
      ...
      _assertExtensionValueDecodes(node, extnId, "extension");
  ...
  /** @internal Sec. 5.2.1: a conforming CRL issuer MUST use the key identifier method and MUST
   *  include the extension in every CRL, with no exception, so it is emitted unless named and
   *  declining it is refused. */
  var akiVal = ext.authorityKeyIdentifier == null ? true : ext.authorityKeyIdentifier;
  if (akiVal === false) throw _err("crl/bad-input", "a conforming CRL issuer MUST include the authorityKeyIdentifier in every CRL it issues, so authorityKeyIdentifier: false is refused (RFC 5280 sec. 5.2.1)");
  push("authorityKeyIdentifier", false, _extAki(_akiKeyId(akiVal, ctx)));
  /** @internal Sec. 5.2.2 takes the OID and syntax from sec. 4.2.1.7, so this is the certificate's
   *  own GeneralNames encoder, and states a SHOULD for non-critical, which is the form emitted. */
  if (ext.issuerAltName != null) push("issuerAltName", false, _b.encodeGeneralNames(_b.reqDenseArray(ext.issuerAltName, "issuerAltName")));
...
function _buildRevoked(entryList, isDelta) {
  ...
  var anyExt = false, seenSerials = intrinsic.create(null);
  var entries = _b.reqDenseArray(entryList, "revoked").map(function (e, idx) {
    ...
    var entryExts = [], seen = intrinsic.create(null);
    ...
    if (e.invalidityDate != null) {
      guard.time.assertEncodable(e.invalidityDate, _err, "crl/bad-input", "invalidityDate");
    ...
    if (e.extensions != null) {
      ...
      e.extensions.forEach(function (x, j) {
        ...
        _assertValidExtension(der, j, EXT_DECODERS);
        ...
        _assertExtensionValueDecodes(xnode, extnId, "entry extension");
...
function _sign(spec, issuer, opts) {
  ...
  if (spec.nextUpdate != null) {
    ...
    // allow:nan-date-comparison-unguarded -- both operands are guard.time.assertEncodable'd via _timeDer on the
 ...
 * and an optional `extensions` object (`authorityKeyIdentifier`, `issuerAltName`,
 * `issuingDistributionPoint`, `deltaCRLIndicator`, `freshestCRL`, `authorityInfoAccess`) or an array
 * of pre-encoded `Extension` DER. `issuerAltName` takes the same GeneralName list the certificate
 * extension takes, since RFC 5280 sec. 5.2.2 defines its OID and syntax by reference to sec. 4.2.1.7,
 * and is emitted non-critical as that section says a conforming CRL issuer should.
 ...
 *
 * The `authorityKeyIdentifier` is emitted whether or not the object form names it, because sec. 5.2.1
 * places it on every CRL a conforming issuer signs: its keyIdentifier is the issuer certificate's
 * subjectKeyIdentifier or, without one, the method (1) value of the issuer key. A stated value is held
 * to the issuer certificate's subjectKeyIdentifier when the issuer is given as a certificate carrying
 * one; `authorityKeyIdentifier: false` is refused. The pre-encoded array form emits exactly what it
 * is given. The version is derived from the field set (v2 when any CRL or entry extension is present,
 * else v1). The
...
function _verify(crl, issuer) {
  ...
  return crlVerify.verifyCrlSignature(parsed, resolved.spki).then(function (sigOk) {
    ...
    return guard.verdict.of(
      { valid: valid, issuerMaySign: issuerMaySign, signatureValid: signatureValid, issuer: parsed.issuer },
      valid ? null : {
        code: !issuerMaySign ? "crl/issuer-may-not-sign" : "crl/bad-signature",
        reason: !issuerMaySign
 ...
 */
var KNOWN_IS_REVOKED_KEYS = intrinsic.assign(intrinsic.create(null), { time: 1, historicalMode: 1 });
