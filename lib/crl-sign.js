    ...
    ext.forEach(function (e, i) {
      ...
      if (Object.prototype.hasOwnProperty.call(REQUIRED_CRITICALITY, extnId)) {
        ...
        if (extnId === O("cRLNumber")) arrCrlNum = _requireBoundedInt(inner, "cRLNumber");
          ...
          // A pre-encoded indirect-CRL IDP (an indirectCRL [4] member) produces a CRL crlChecker skips; reject it.
          if (extnId === O("issuingDistributionPoint")) (inner.children || []).forEach(function (c) { if (c.tagClass === "context" && c.tagNumber === 4) throw _err("crl/bad-idp", _INDIRECT_IDP_DEFERRED); });
        }
      }
