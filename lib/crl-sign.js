...
// replaceable properties to answer one thing, and either of them answering true admits a name the
// table never carried -- here, a reason code the CRL profile does not define.
...
// spec.issuer is the third place an issuer distinguished name can come from, and it is read only
// when neither issuer.cert nor issuer.name supplies one: both of those win outright. A spec naming
// one issuer beside a certificate for another was signed under the certificate's.
var KNOWN_SPEC_KEYS_ISSUER_NAMED = {
  thisUpdate: 1, nextUpdate: 1, revoked: 1, crlNumber: 1, extensions: 1,
};
// Two FORMS, exactly as x509-sign has: when `issuer.cert` is present the builder takes the DN and
// the public key from it and never reads `name` or `publicKey`, so a union signed under the
// certificate's identity while discarding the one the caller wrote out.
...
function _resolveReason(reason, isDelta) {
  ...
  if (typeof reason === "number") codeNum = reason;
    ...
    if (!_hasOwn(REASON_BY_NAME, reason)) throw _err("crl/bad-reason-code", "unknown CRLReason " + JSON.stringify(reason));
...
function _freshestValue(spec) {
  ...
  var dense = _b.reqDenseArray(spec, "freshestCRL");   // density before every()/map() skip a hole into b.sequence
...
function _sign(spec, issuer, opts) {
  ...
  // The issuer form is settled once, before either door, so the spec table matches the source the
  // distinguished name will actually come from.
...
function _verify(crl, issuer) {
  ...
  // A bare SPKI carries no certificate, so the two cert-only questions cannot be asked and only the
  // signature is checked (issuerMaySign is then reported true). Both checks are surfaced on the verdict:
  // a bare `true` used to hide whether the signer was ALLOWED to sign this CRL, so `if (verify(...))`
  // accepted a CRL minted under an end-entity certificate of the same CA as that CA's own.
