...
function _parseIssuerCert(cert) {
  return guard.parsed.accept(cert, "certificate", x509Schema.parse, _err, "crl/bad-input", "issuer.cert");
}
...
// A claimed-parsed CRL must be a COMPLETE one. Three properties were enough to be trusted as parser
// output, after which the scope guards read `(crlExtensions || [])` -- so omitting that property
// made a scope-restricted CRL answer as an unrestricted one, the guard unable to fire on a list
// that was not there.
function _coerceCrl(crl) {
  return guard.parsed.accept(crl, "crl", crlSchema.parse, _err, "crl/bad-input", "crl");
}
