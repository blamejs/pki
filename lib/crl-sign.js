...
function isRevoked(crl, serialNumber) {
  ...
  for (var i = 0; i < parsed.revokedCertificates.length; i++) {
    ...
    // Even on a direct CRL, an entry carrying certificateIssuer is about a different issuer's
    // certificate. It is not this serial's entry, so it neither matches nor hides a later one.
    if (_findExtOid(entry.crlEntryExtensions, "certificateIssuer")) continue;
    return entry;
  }
  return null;
}
