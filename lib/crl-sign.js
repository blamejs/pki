...
function _issuerMaySign(parsed, cert) {
  ...
  var ku = (cert.extensions || []).filter(function (e) { return e.oid === O("keyUsage"); })[0];
  if (!ku) return true;                              // absent keyUsage places no restriction (sec. 4.2.1.3)
  var bits;
  try { bits = asn1.read.bitString(asn1.decode(ku.value)); }
  catch (e) { throw _err("crl/bad-issuer", "the issuer certificate keyUsage extension is malformed", e); }
  // cRLSign is bit 6; the BIT STRING may be shorter than seven bits, in which case the bit is
  // absent and therefore not asserted.
  return !!(bits.bytes.length && (bits.bytes[0] & 0x02) !== 0);
}
 ...
 *
 * It does check SCOPE, because a serial number means something only within the set of certificates a
 * CRL speaks for. A DELTA CRL lists changes since a base, so a serial in it may be there to say the
 * certificate was RELEASED -- read alone, the entry meaning "no longer revoked" reads as "revoked" --
 * and an INDIRECT CRL carries entries for other issuers, whose serials are unrelated to yours.
 * Neither is answerable from the CRL alone, so both throw (`crl/delta-not-authoritative`,
 * `crl/indirect-not-supported`) rather than returning the opposite of the truth. Merge a delta with
 * its base through `pki.path.crlChecker`, which does that and honours the same scope rules. An entry
 * carrying `certificateIssuer` is about another issuer's certificate and never matches.
 *
 * @example
 *   var pair = await pki.key.generate("Ed25519");
 *   var signerKeyPkcs8 = await pki.key.export(pair.privateKey);
...
function isRevoked(crl, serialNumber) {
  ...
  if (idp) _assertNotIndirect(idp);
  for (var i = 0; i < parsed.revokedCertificates.length; i++) {
    var entry = parsed.revokedCertificates[i];
    if (entry.serialNumberHex !== hex) continue;
    // certificateIssuer (sec. 5.3.3) names the issuer an entry belongs to, and is meaningful ONLY on
    // an indirect CRL -- which was refused above. Reaching one here means the CRL declares itself
    // direct while carrying an entry that says otherwise, and the two readings disagree about whose
    // certificate this is. SKIPPING it would be the worse answer of the two: a matching entry
    // silently not matching reports the certificate as NOT revoked.
    if (_findExtOid(entry.crlEntryExtensions, "certificateIssuer")) {
      throw _err("crl/indirect-not-supported", "a revoked-certificate entry for this serial carries certificateIssuer, which names another issuer, on a CRL that does not declare itself indirect -- the entry and the CRL disagree about whose certificate this is, so no revocation status follows from it");
    }
    return entry;
  }
  return null;
...
// Refuse an indirect CRL -- and refuse one whose scope cannot be read at all, which is the same
// answer for the same reason: a scope that cannot be established is not a scope to answer from.
// indirectCRL is [4] IMPLICIT BOOLEAN inside the IDP SEQUENCE, and DER omits a FALSE default, so
// only an explicitly TRUE tag marks it. Written as a throwing assertion rather than a predicate so
// the malformed case cannot be mistaken for a verdict about the certificate.
...
function _assertNotIndirect(ext) {
  var node;
  try { node = asn1.decode(ext.value); }
  catch (e) { throw _err("crl/indirect-not-supported", "the issuingDistributionPoint is not decodable, so this CRL's scope cannot be established", e); }
  if (!node.children) throw _err("crl/indirect-not-supported", "the issuingDistributionPoint is not a SEQUENCE, so this CRL's scope cannot be established");
  for (var i = 0; i < node.children.length; i++) {
    var c = node.children[i];
    if (c.tagClass === "context" && c.tagNumber === 4 && c.content && c.content.length && c.content[0] !== 0x00) {
      throw _err("crl/indirect-not-supported", _INDIRECT_REFUSAL);
    }
  }
}
