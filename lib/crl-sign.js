...
var REASON_BY_NAME = {};
_keys(CRL_REASON).forEach(function (v) { REASON_BY_NAME[CRL_REASON[v]] = _Number(v); });
...
function _boundedInteger(v, label) {
  ...
  else if (typeof v === "number") { if (!_isSafeInteger(v)) throw _err("crl/bad-crl-number", label + " number must be a safe integer (pass a BigInt, hex string, or Buffer for a larger value)"); n = _BigInt(v); }
  else if (typeof v === "string") { try { n = _BigInt(v); } catch (e) { throw _err("crl/bad-crl-number", label + " string must be a decimal or 0x-hex integer", e); } }
  else if (_isBuffer(v)) { n = v.length ? _BigInt("0x" + v.toString("hex")) : 0n; }
...
function _resolveReason(reason, isDelta) {
  ...
  if (typeof reason === "number") codeNum = reason;
    ...
    if (!Object.prototype.hasOwnProperty.call(REASON_BY_NAME, reason)) throw _err("crl/bad-reason-code", "unknown CRLReason " + _stringify(reason));
    codeNum = REASON_BY_NAME[reason];
  } else throw _err("crl/bad-reason-code", "reason must be a CRLReason name or number");
  if (!Object.prototype.hasOwnProperty.call(CRL_REASON, _String(codeNum))) throw _err("crl/bad-reason-code", "undefined or reserved CRLReason " + codeNum + " (RFC 5280 sec. 5.3.1)");
...
function _akiKeyId(val, ctx) {
  if (_isBuffer(val)) return guard.bytes.snapshot(val, CrlError, "crl/bad-input", "the authorityKeyIdentifier keyIdentifier");
  if (val === true) {
    if (ctx.issuerCert) {
...
function _idpValue(idp) {
  if (!idp || typeof idp !== "object" || _isArray(idp) || _isBuffer(idp)) throw _err("crl/bad-idp", "issuingDistributionPoint must be an object");
  guard.identifier.assertKnownKeys(idp, KNOWN_IDP_KEYS, _err, "crl/bad-idp", function (k) {
    return "unknown issuingDistributionPoint field " + _stringify(k) + " (pass a pre-encoded Extension DER via the extensions array for an exotic field like onlySomeReasons)";
  });
  ...
  if (idp.fullName != null) {
    var entries = _isArray(idp.fullName) ? idp.fullName : [idp.fullName];
    ...
    _push(children, b.contextConstructed(0, b.contextConstructed(0, _bufferConcat(entries.map(_encodeGeneralName)))));
  }
  var scopeTrue = 0;
  if (idp.onlyContainsUserCerts === true) { _push(children, b.contextPrimitive(1, _bufferFrom([0xff]))); scopeTrue++; }
  if (idp.onlyContainsCACerts === true) { _push(children, b.contextPrimitive(2, _bufferFrom([0xff]))); scopeTrue++; }
...
function _freshestValue(spec) {
  if (!_isArray(spec) || !spec.length) throw _err("crl/bad-input", "freshestCRL must be a non-empty array of GeneralNames (or { fullName } distribution points)");
  var dps = spec.every(function (e) { return e && typeof e === "object" && !_isBuffer(e) && _keys(e).length === 1 && e.fullName != null; })
  ...
  return b.sequence(dps.map(function (fullName) {
    ...
    return b.sequence([b.contextConstructed(0, b.contextConstructed(0, _bufferConcat(entries.map(_encodeGeneralName))))]);
  }));
}
...
function _aiaValue(spec) {
  if (!_isArray(spec) || !spec.length) throw _err("crl/bad-input", "authorityInfoAccess must be a non-empty array of caIssuers GeneralNames");
...
function _buildCrlExtensions(spec, ctx) {
  ...
  function push(oidName, critical, valueDer) {
    ...
    seen[id] = true;
    _push(out, _ext(id, critical, valueDer));
  }
  ...
  guard.identifier.assertKnownKeys(ext, KNOWN_CRL_EXT_KEYS, _err, "crl/bad-input", function (k) {
    return "unknown CRL extension " + _stringify(k) + " in the extensions spec; pass a pre-encoded Extension DER via the array form";
  });
...
function _buildRevoked(entryList, isDelta) {
  if (!_isArray(entryList)) throw _err("crl/bad-input", "revoked must be an array of revoked-certificate entries");
  var anyExt = false, seenSerials = {};
  var entries = entryList.map(function (e, idx) {
    if (!e || typeof e !== "object" || _isBuffer(e)) throw _err("crl/bad-input", "each revoked entry must be an object");
    ...
    function pushE(oidName, critical, valueDer) {
      ...
      seen[id] = true;
      _push(entryExts, _ext(id, critical, valueDer));
    }
    if (e.reason != null) {
      var codeNum = _resolveReason(e.reason, isDelta);
      if (codeNum !== 0) pushE("reasonCode", false, b.enumerated(_BigInt(codeNum)));   // unspecified(0) omitted (sec. 5.3.1)
    }
    ...
    if (e.extensions != null) {
      if (!_isArray(e.extensions)) throw _err("crl/bad-input", "revoked entry [" + idx + "] extensions must be an array of pre-encoded Extension DER");
      e.extensions.forEach(function (x, j) {
        ...
        _push(entryExts, b.raw(der));
      });
    }
    if (entryExts.length) { anyExt = true; _push(children, b.sequence(entryExts)); }
    return b.sequence(children);
  });
...
function _sign(spec, issuer, opts) {
  opts = opts || {};
  if (!spec || typeof spec !== "object" || _isBuffer(spec)) throw _err("crl/bad-input", "the CRL spec must be an object");
  issuer = issuer || {};
  ...
  var tbsChildren = [];
  if (version === 2) _push(tbsChildren, b.integer(1n));   // bare INTEGER(1), NOT a [0] EXPLICIT tag like a certificate
  _push(tbsChildren, scheme.sigAlgId);                    // signature == signatureAlgorithm (sec. 5.1.1.2), single source
  _push(tbsChildren, issuerDer);
  _push(tbsChildren, thisU);
  if (nextU) _push(tbsChildren, nextU);
  if (revoked.entries.length) _push(tbsChildren, b.sequence(revoked.entries));   // omit the field when empty (sec. 5.1.2.6)
  if (crlExts.length) _push(tbsChildren, b.explicit(0, b.sequence(crlExts)));    // crlExtensions [0] EXPLICIT
...
function _resolveIssuer(issuer) {
  ...
  if (_isBuffer(issuer)) { _assertValidSpki(issuer, "issuer SPKI"); return { spki: issuer, cert: null }; }
...
function _serialHexOf(serial) {
  ...
  else if (typeof serial === "number") { if (!_isSafeInteger(serial)) throw _err("crl/bad-input", "serialNumber number must be a safe integer (pass a BigInt, hex string, or Buffer)"); v = _BigInt(serial); }
  else if (typeof serial === "string") { try { v = _BigInt(serial); } catch (e) { throw _err("crl/bad-input", "serialNumber string must be a decimal or 0x-hex integer", e); } }
  else if (_isBuffer(serial)) { v = serial.length ? _BigInt("0x" + serial.toString("hex")) : 0n; }
...
function isRevoked(crl, serialNumber, opts) {
  ...
  guard.identifier.assertKnownKeys(o, KNOWN_IS_REVOKED_KEYS, _err, "crl/bad-input", function (k) {
    return "pki.crl.isRevoked has an unknown option " + _stringify(k) +
