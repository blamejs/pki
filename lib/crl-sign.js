...
// The BigInt value of a pre-encoded Extension whose extnValue wraps an INTEGER (cRLNumber / baseCRLNumber),
// or null when it is not a decodable INTEGER -- best-effort, so the delta-ordering check on the escape hatch
// is applied only when the value is readable and never crashes on opaque caller bytes.
function _extIntValue(node) {
  try { return asn1.read.integer(asn1.decode(asn1.read.octetString(node.children[node.children.length - 1]))); }
...
function _buildCrlExtensions(spec, ctx) {
  ...
  if (Array.isArray(ext)) {
    ...
    ext.forEach(function (e, i) {
      ...
      seen[extnId] = true;
      // RFC 5280 sec. 5.2 -- a recognized CRL extension carries a fixed criticality; enforce it on the escape
      // hatch exactly as the object form does. assertValidExtension rejects an explicit critical=FALSE, so a
      // 3-child Extension is critical and a 2-child one is non-critical.
      ...
      if (extnId === O("cRLNumber")) arrCrlNum = _extIntValue(node);
      if (extnId === O("deltaCRLIndicator")) { isDelta = true; arrBase = _extIntValue(node); }
      out.push(b.raw(der));
    });
  ...
  var entries = entryList.map(function (e, idx) {
    ...
    if (e.extensions != null) {
      ...
      e.extensions.forEach(function (x, j) {
        ...
        seen[extnId] = true;
        // RFC 5280 sec. 5.3 -- a recognized entry extension carries a fixed criticality; enforce it on the escape hatch.
        if (Object.prototype.hasOwnProperty.call(REQUIRED_ENTRY_CRITICALITY, extnId) && (xnode.children.length === 3) !== REQUIRED_ENTRY_CRITICALITY[extnId]) {
          throw _err("crl/bad-input", "pre-encoded entry extension " + (oid.name(extnId) || extnId) + " has the wrong criticality (RFC 5280 sec. 5.3 requires it " + (REQUIRED_ENTRY_CRITICALITY[extnId] ? "critical" : "non-critical") + ")");
        }
