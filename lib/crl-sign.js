...
function _buildCrlExtensions(spec, ctx) {
  ...
  if (spec.crlNumber != null) push("cRLNumber", false, _boundedInteger(spec.crlNumber, "cRLNumber"));
  ...
  if (ext.deltaCRLIndicator != null) push("deltaCRLIndicator", true, _boundedInteger(ext.deltaCRLIndicator, "deltaCRLIndicator baseCRLNumber"));
 ...
 * @signature pki.crl.verify(crl, issuer, opts?) -> Promise<boolean>
 * @since 0.3.9
 * @status experimental
 ...
 */
function verify(crl, issuer, opts) { return Promise.resolve().then(function () { return _verify(crl, issuer, opts); }); }
