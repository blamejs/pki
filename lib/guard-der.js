...
// The byte comparison SET-OF ordering is decided by, taken at load. A replacement chooses which
// encodings this guard calls correctly ordered. See guard-intrinsic for the whole captured set.
var _compare = require("./guard-intrinsic").compare;
var _forEach = require("./guard-intrinsic").forEach;
// The emptiness gate below decides whether bytes reach the decoder at all, and on a byte view
// `length` is a replaceable accessor: one answering non-zero for an empty buffer sends nothing to
// asn1.decode as though it were an element.
...
function _readers() {
  ...
  var asn1 = _asn1();
  // Null-prototype: this table IS the allowlist of universal tags with a strict content reader, and
  // a tag it does not carry is refused. Built on a `{}`, an inherited numeric property supplies a
  // reader for a tag that was deliberately left out, and the refusal that is the point stops.
...
// Null-prototype: the rank decides canonical SET order, so an inherited property answering for an
// unrecognized tag class would rank it rather than leave it unranked, and a SET in no canonical
// order would compare as ordered.
...
function setOrderOk(kids) {
  // A null-prototype accumulator: on a `{}` a planted `Object.prototype["universal:16"]` answers
  // this membership test for a tag the SET never carried, which reports a duplicate that is not
  // there and selects the wrong canonical-order rule for the whole SET.
