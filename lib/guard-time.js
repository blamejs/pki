...
// @enforced-by behavioral -- calling an override rather than the intrinsic has no rename-proof
// code shape (`x.getTime()` is the ordinary spelling everywhere a Date is genuinely this
// toolkit's own); the RED vectors (a subclass whose getTime throws, and one that answers a
// different instant on each call) are the guard.
function instantOf(value) { return _dateGetTime.call(value); }
