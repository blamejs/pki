...
function _rfc850Year(yy, refMs) {
  if (typeof refMs !== "number" || !_isFinite(refMs)) return yy + (yy < 70 ? 2000 : 1900);
  var refYear = new Date(refMs).getUTCFullYear();
  var full = _floor(refYear / 100) * 100 + yy;
...
function httpDateMs(s, refMs) {
  ...
  if (asc) { monName = asc[1]; day = _parseInt(asc[2], 10); year = _parseInt(/(\d{4})$/.exec(s)[1], 10); }
  else {
    var dmy = /(\d{2})[ -](\w{3})[ -](\d{2,4})/.exec(s);                   // IMF / rfc850: day month year
    if (!dmy) return NaN;
    day = _parseInt(dmy[1], 10); monName = dmy[2]; year = _parseInt(dmy[3], 10);
  ...
  var hh = _parseInt(t[1], 10), mi = _parseInt(t[2], 10), ss = _parseInt(t[3], 10);
...
function parse(value, opts) {
  opts = opts || {};
  var raStr = _String(value).trim();
  var out = { retryAfterSeconds: null, retryAfterDate: null };
  if (/^\d+$/.test(raStr)) {
    var n = _parseInt(raStr, 10);
    ...
    if (!_isSafeInteger(n) || n > MAX_RETRY_AFTER_SECONDS) { if (opts.lenient) return out; throw _fail(opts, "the Retry-After delay is out of the supported range (0.." + MAX_RETRY_AFTER_SECONDS + " seconds)"); }
    out.retryAfterSeconds = n;
    return out;
  }
  var when = httpDateMs(raStr, opts.now);
  if (isNaN(when)) { if (opts.lenient) return out; throw _fail(opts, "a Retry-After must be delay-seconds or a valid HTTP-date (RFC 7231 sec. 7.1.1.1/7.1.3), got " + _stringify(raStr)); }
  out.retryAfterDate = when;
  if (typeof opts.now === "number" && _isFinite(opts.now)) {
    ...
    var d = _max(0, _ceil((when - opts.now) / constants.TIME.seconds(1)));
