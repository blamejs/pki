...
// cbcEncrypt(key, iv, plaintext, keyBits, E, code) -> the ciphertext.
// Takes the caller's typed error for the same reason cbcDecrypt does: the cipher runs through the
// shared guard, whose one throw has to keep the calling domain's own code.
...
// cbcDecrypt(key, iv, ct, keyBits, E, code) -> the recovered plaintext.
// Takes the caller's typed error the way rc2.cbcDecrypt does, so a refusal keeps its own domain.
function cbcDecrypt(key, iv, ct, keyBits, E, code) {
  var d = nodeCrypto.createDecipheriv("aes-" + keyBits + "-cbc", key, iv);
  // A wrong key fails inside final(), on the padding check, and update() has by then produced every
  // block but the last. Through the guard so that partial plaintext is cleared on both exits rather
  // than abandoned readable on exactly the path an attacker induces.
...
function pbes2Encrypt(pwBytes, plaintext, opts, E, prefix) {
  ...
  try {
    ...
    // guard-bytes, which constructs. `new` on a factory yields the object the factory returns, so
    // the currency is compatible -- and the throw is unreachable here regardless, because a freshly
