...
var PRF_NODE_BY_OID = Object.create(null); Object.keys(PRF_NODE_BY_NAME).forEach(function (n) { PRF_NODE_BY_OID[O(n)] = PRF_NODE_BY_NAME[n]; });

var CONTENT_KEYBITS = Object.create(null), CONTENT_MODE = Object.create(null);
...
function prfNodeByName(prf, E, prefix) { if (typeof prf !== "string" || !PRF_NODE_BY_NAME[prf]) throw E(prefix + "/bad-input", "unsupported prf " + guard.text.showValue(prf)); return PRF_NODE_BY_NAME[prf]; }
...
function parsePbkdf2Params(paramsDer, opts, E, prefix, strictPrf) {
  ...
  var wantedIter = opts ? opts.maxIterations : null;
  if (wantedIter != null) {
    if (typeof wantedIter !== "number" || !isFinite(wantedIter) || wantedIter < 1 || Math.floor(wantedIter) !== wantedIter) throw E(prefix + "/bad-input", "maxIterations must be a positive integer");
    cap = Math.min(wantedIter, cap);
  }
