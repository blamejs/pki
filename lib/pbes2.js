...
// Compute PBMAC1 = HMAC(PBKDF2(pwBytes, salt, iter, keyLength, prfHash), message). `prfHash` is a WebCrypto
// hash name ("SHA-256"). pwBytes is pre-formatted (the caller owns the encoding).
function pbmac1(pwBytes, salt, iterationCount, keyLength, prfHash, message) {
  ...
  return subtle.importKey("raw", pwBytes, { name: "PBKDF2" }, false, ["deriveBits"]).then(function (baseKey) {
    ...
    return subtle.importKey("raw", Buffer.from(bits), { name: "HMAC", hash: prfHash }, false, ["sign"]).then(function (hmacKey) {
