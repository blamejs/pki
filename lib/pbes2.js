...
var PRF_NODE_BY_OID = {}; _keys(PRF_NODE_BY_NAME).forEach(function (n) { PRF_NODE_BY_OID[O(n)] = PRF_NODE_BY_NAME[n]; });
...
function passwordBytesOwned(p, E, prefix) {
  if (_isBuffer(p)) return { bytes: p, owned: false };
  if (p instanceof Uint8Array) return { bytes: _bufferFrom(p), owned: true };
  if (typeof p === "string") return { bytes: _bufferFrom(p, "utf8"), owned: true };
...
function assertIterations(n, E, prefix) {
  if (typeof n !== "number" || !_isFinite(n) || n < 1 || _floor(n) !== n) throw E(prefix + "/bad-input", "iterations must be a positive integer");
...
function prfNodeByName(prf, E, prefix) { if (!PRF_NODE_BY_NAME[prf]) throw E(prefix + "/bad-input", "unsupported prf " + _stringify(prf)); return PRF_NODE_BY_NAME[prf]; }
...
function pbkdf2ParamsSeq(salt, iterations, prf) {
  var kids = [b.octetString(salt), b.integer(_BigInt(iterations))];
  if (prf !== "hmacWithSHA1") _push(kids, b.sequence([b.oid(O(prf)), b.nullValue()]));
...
function parsePbkdf2Params(paramsDer, opts, E, prefix, strictPrf) {
  var node = _isBuffer(paramsDer) ? asn1.decode(paramsDer) : paramsDer;
  ...
  if (opts && opts.maxIterations != null) {
    if (typeof opts.maxIterations !== "number" || !_isFinite(opts.maxIterations) || opts.maxIterations < 1 || _floor(opts.maxIterations) !== opts.maxIterations) throw E(prefix + "/bad-input", "maxIterations must be a positive integer");
...
function cbcEncrypt(key, iv, plaintext, keyBits) {
  ...
  return _bufferConcat([c.update(plaintext), c.final()]);
}
...
function pbmac1AlgId(desc) {
  var pbkdf2Params = [b.octetString(desc.salt), b.integer(_BigInt(desc.iterationCount)), b.integer(_BigInt(desc.keyLength))];
  if (desc.prfName !== "hmacWithSHA1") _push(pbkdf2Params, _hmacAlgId(desc.prfName));
...
function pbmac1(pwBytes, salt, iterationCount, keyLength, prfHash, macHash, message) {
  ...
  }).then(function (sig) { return _bufferFrom(sig); });
}
