...
// Buffer#toString, uncurried at load. Used to build dedup keys (a hex string) from bytes the toolkit
// owns; dispatched live, a replaced toString could make two distinct values key alike or one value key
// differently on two reads, so a dedup over untrusted-adjacent bytes would merge or split wrongly.
...
// The well-known async-iteration symbol, captured at load. `for await` uses this symbol at the
// language level regardless of the global `Symbol` object, so a classifier that reads
// `Symbol.asyncIterator` live would disagree with the loop the moment a co-resident replaced the
// writable `Symbol` global: a real async iterable would be looked up under the wrong key, judged not
// iterable, and refused where the engine would have consumed it. Reading the captured symbol keeps
// the door's answer and the loop's behavior the same.
