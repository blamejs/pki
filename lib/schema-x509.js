...
/**
 * @internal The tbsCertificate fields as a record, without the signature around them. RFC 5914
 * sec. 3's TrustAnchorChoice carries a bare TBSCertificate in its [1] arm, which is a certificate
 * with nothing to verify, and reading it with a second mapping is how the two would come to
 * disagree about what a field is called. The envelope's own checks stay with the envelope: a bare
 * TBSCertificate has no outer signatureAlgorithm to compare its own against.
 */
/** @internal A field written onto a record the caller receives, as an own data property. A plain
 *  assignment dispatches through an accessor an attacker planted on Object.prototype under the
 *  same name, which either throws out of a getter-only slot or swallows the parsed value. */
function _own(rec, name, value) {
  intrinsic.defineProperty(rec, name, { value: value, enumerable: true, configurable: true, writable: true });
}

function tbsRecord(tbs, ctx) {
...
var TBS_CERTIFICATE_ALONE = schema.decode(function (n, ctx) {
  return tbsRecord(schema.walk(CERTIFICATE_TBS, n, ctx), ctx);
});
...
var CERTIFICATE = pkix.signedEnvelope(NS, CERTIFICATE_TBS, {
  ...
  build: function (e, ctx) {
    ...
    /** @internal Defined, not assigned: the record is an object literal, so it inherits
     * Object.prototype, and a plain write under one of these names would dispatch through an
     * accessor planted there instead of creating the certificate's own field. */
    var out = tbsRecord(tbs, ctx);
    _own(out, "signatureAlgorithm", e.signatureAlgorithm);
    _own(out, "tbsBytes", e.tbsBytes);
    _own(out, "signatureValue", e.signatureValue);
    return out;
  },
});
...
module.exports = {
  ...
  /** @internal The TBSCertificate and Certificate schemas, for a format that embeds one rather than
   *  being one: RFC 5914 sec. 3's TrustAnchorChoice carries a Certificate in its untagged arm and a
   *  TBSCertificate in its [1], and reading them with a second definition is how the two drift.
   *  Not on the curated `pki.schema.x509` surface. */
  tbsCertificateSchema: TBS_CERTIFICATE_ALONE,
