...
function compressedCertificate(view, E, codes) {
  ...
  return { algorithm: algorithm, uncompressedLength: uncompressedLength, body: _bufferFrom(body) };
}
...
function _extensionRecords(view, E, codes) {
  ...
  while (!r.atEnd()) {
    ...
    seen[type] = true;
    _push(out, { type: type, data: _bufferFrom(data) });
  }
  return out;
}
...
function certificateMessage(view, E, codes, certificateType) {
  ...
  while (!list.atEnd()) {
    ...
    var entry = {
      extensions: _bufferFrom(extensions),
      extensionList: _extensionRecords(extensions, E, codes),
    };
    if (certificateType === "RawPublicKey") entry.spki = _bufferFrom(data);
    else entry.certData = _bufferFrom(data);
    _push(entries, entry);
  }
  return { certificateRequestContext: _bufferFrom(context), entries: entries };
}
