...
async function ocspCertIdMatches(certID, cert, issuerNameDer, issuerKeyBits) {
  ...
  var nameHash = await ocspDigest(hashName, issuerNameDer);
  if (!certID.issuerNameHash.equals(nameHash)) return false;
  var keyHash = await ocspDigest(hashName, issuerKeyBits);
  return certID.issuerKeyHash.equals(keyHash);
}
...
function ocspChecker(responses) {
  ...
  return {
    check: async function (cert, issuer, ctx) {
      var time = ctx.time;
      var historical = ctx.historicalMode === true;
      var issuerNameDer = cert.issuer.bytes;
      var issuerKeyBits;
      ...
      for (var k = 0; k < parsed.length; k++) {
        ...
        for (var s = 0; s < br.responses.length; s++) {
          ...
          if (!(await ocspCertIdMatches(sr.certID, cert, issuerNameDer, issuerKeyBits))) continue;   // not about this cert
