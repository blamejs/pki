...
function coerceCert(input) {
  ...
  // (a truthy tbsBytes) must actually carry the parsed X.509 fields the search
  // dereferences -- a bare { tbsBytes } object, or any partial shape, fails closed
  // here as a typed PathError rather than a raw TypeError deep in the walk.
  if (input && typeof input === "object" && !Buffer.isBuffer(input) && input.tbsBytes !== undefined) {
    // Require the NESTED fields the search + anchor conversion dereference (each
    // access guarded by its parent's presence), so a partial shape -- a missing
    // subject.bytes, a non-Buffer key -- fails closed here rather than throwing a
    // raw TypeError in identityKey / nameMatchSoft / toAnchor.
    if (!input.subject || !Array.isArray(input.subject.rdns) || !Buffer.isBuffer(input.subject.bytes) ||
      !input.issuer || !Array.isArray(input.issuer.rdns) ||
      !input.subjectPublicKeyInfo || !Buffer.isBuffer(input.subjectPublicKeyInfo.bytes) ||
      !input.subjectPublicKeyInfo.algorithm || typeof input.subjectPublicKeyInfo.algorithm.oid !== "string" ||
      !Array.isArray(input.extensions) || !input.extensions.every(_isExtensionEntry)) {
      throw E("path/bad-input", "build: an input has tbsBytes but is not a well-formed parsed certificate");
    }
    return input;
  }
...
function toAnchor(entry) {
  if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && entry.name && entry.publicKey && entry.algorithm) {
    if (!entry.name.rdns) throw E("path/bad-input", "build: a trustAnchor tuple's name must be a parsed distinguished name (with .rdns)");
    return entry;
  }
  var cert;
