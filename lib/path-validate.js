...
// Every operation this validator decides with, taken at load through the guard family's own home
// for that. Each one is an ordinary writable property until it is captured, and each one answers a
// question a verdict rests on: a replaced `Buffer.prototype.equals` makes a distribution point
// correspond to a certificate that never named it, a replaced `Array.prototype.some` reports no
// match over a populated candidate list, a replaced `String.prototype.toLowerCase` moves the
// boundary a name constraint is compared at, and a replaced `Object.prototype.hasOwnProperty` puts
// a build-only name into the option set forwarded to every internal validate. Capturing here
// freezes which function runs, whatever a caller installs afterwards. See guard-intrinsic.
...
// The array operations. A name-constraint scan, a policy-tree walk and the revocation reason-shard
// accumulation are each a loop expressed as one of these, so one that answers vacuously turns a
// rule keyed on what the loop found into a rule that applies to nothing.
...
// The string and pattern surface the RFC 5280 sec. 4.2.1.10 name-constraint comparisons are built
// out of: where a mailbox splits, which substring is the host, whether a host is a fully qualified
// domain name, how a URI splits into scheme and authority. A replacement anywhere in that chain
// decides which names a critical constraint reaches.
...
// The byte comparison and the byte-to-text step. The sec. 5.2.5 identical-encoding rule and the
// RFC 4158 sec. 2.4.2 loop key both rest on these, so one answering a constant makes every
// distribution point correspond and collapses every certificate on a branch to one identity.
var _equals = intrinsic.uncurry(Buffer.prototype.equals);
var _bufToString = intrinsic.uncurry(Buffer.prototype.toString);
// The settle operations, uncurried so the constructor is passed explicitly: `Promise.resolve` and
// `Promise.reject` each read `this` as the constructor they build from.
...
function pathCode(e, fallback) {
  return (e && typeof e.code === "string" && _strIndexOf(e.code, "path/") === 0) ? e.code : fallback;
}
...
// allow:guard-reads-runtime-live -- module-init, the set is sealed once at load before any caller runs
...
["sha2-128s", "sha2-128f", "sha2-192s", "sha2-192f", "sha2-256s", "sha2-256f",
 "shake-128s", "shake-128f", "shake-192s", "shake-192f", "shake-256s", "shake-256f"
  ...
  // allow:guard-reads-runtime-live -- module-init, the SLH-DSA registry rows are built once at load
...
function resolveRsaPss(paramsBytes) {
  ...
  var pssLastTag = -1;
  _forEach(n.children, function (f) {
...
function builtinVerify(state, cert) {
  ...
  } catch (e) { return _promiseResolve(_Promise, { ok: false, code: pathCode(e, "path/unsupported-algorithm"), error: e }); }
  ...
  if (!guard.crypto.isOctetAligned(cert.signatureValue)) return _promiseResolve(_Promise, { ok: false, code: "path/bad-signature" });
...
function requireCriticalExt(ext, name, checks) {
  if (ext && ext.critical !== true) {
    _push(checks, { name: name, ok: false, code: "path/extension-not-critical" });
...
function splitMailbox(addr) {
  var first = _strIndexOf(addr, "@");
  if (first === -1) return null;
  if (first !== _strLastIndexOf(addr, "@")) return "ambiguous";
...
function emailMatch(constraint, mailbox) {
  ...
  if (_strIndexOf(constraint, "@") !== -1) {
    ...
    return mb[0] === cb[0] && stripTrailingDot(_toLowerCase(mb[1])) === stripTrailingDot(_toLowerCase(cb[1]));
  }
  ...
  var host = stripTrailingDot(_toLowerCase(mb[1]));
  if (host === "") return "unsupported";
  var c = stripTrailingDot(_toLowerCase(constraint));
  if (_charAt(c, 0) === ".") return host.length > c.length && _strSlice(host, -c.length) === c;
  return host === c;
}
...
function hostConstraintMatch(constraint, host) {
  var c = stripTrailingDot(_toLowerCase(constraint)), h = stripTrailingDot(_toLowerCase(host));
  if (c === "") return true;
  if (_charAt(c, 0) === ".") return h.length > c.length && _strSlice(h, -c.length) === c;   // subdomain only
  return h === c || (h.length > c.length && _strSlice(h, -(c.length + 1)) === "." + c);   // host + subdomains
}
...
function isFqdnHost(host) {
  ...
  if (h === "" || _strIndexOf(h, ".") === -1) return false;  // empty or single-label (localhost)
  if (!_reTest(/^[a-z0-9.-]+$/i, h)) return false;           // scheme/path/port/IPv6 chars, "@", etc.
  if (_reTest(/^[0-9.]+$/, h)) return false;                 // IPv4 dotted-quad literal
  return true;
}
...
function uriMatch(constraint, uri) {
  ...
  if (!isFqdnHost(_charAt(c, 0) === "." ? _strSlice(c, 1) : c)) return "unsupported";
  if (_charAt(c, 0) === ".") return h.length > c.length && _strSlice(h, -c.length) === c;
  return h === c;
}

function uriHost(uri) {
  var m = _reExec(/^[a-z][a-z0-9+.-]*:\/\/([^/?#]*)/i, uri);
  if (!m) return null;
  var authority = m[1];
  ...
  var firstAt = _strIndexOf(authority, "@");
  if (firstAt !== _strLastIndexOf(authority, "@")) return null;
  if (firstAt !== -1) authority = _strSlice(authority, firstAt + 1);
  var host = _replace(authority, /:\d+$/, "");
...
function certNameForms(cert) {
  ...
  if (san) {
    ...
    _forEach(san.value.names, function (nm) {
      ...
      _push(forms, { tag: nm.tagNumber, value: nm.value === undefined ? null : nm.value });
    });
  }
  ...
  if (!hasRfc822San) {
    _forEach(cert.subject.rdns, function (rdn) {
      _forEach(rdn, function (atv) {
        if (atv.type === OID.emailAddress && typeof atv.value === "string") _push(forms, { tag: 1, value: atv.value });
      });
    });
  }
  if (cert.subject.rdns.length > 0) _push(forms, { tag: 4, value: cert.subject });
  return forms;
}
...
function checkNameConstraints(state, cert) {
  ...
  for (var f = 0; f < forms.length; f++) {
    ...
    for (var g = 0; g < state.permittedGenerations.length; g++) {
      var formSubtrees = _filter(state.permittedGenerations[g], function (s) { return s.tag === nf.tag; });
      if (!formSubtrees.length) continue;              // this generation does not constrain the form
      var permitted = false, unsupported = false;
      _forEach(formSubtrees, function (s) {
...
function absorbNameConstraints(state, decoded) {
  if (decoded.permittedSubtrees.length) {
    _push(state.permittedGenerations, _map(decoded.permittedSubtrees, function (st) {
      return { tag: st.base.tagNumber, base: st.base.value };
    }));
  }
  _forEach(decoded.excludedSubtrees, function (st) {
    _push(state.excludedSubtrees, { tag: st.base.tagNumber, base: st.base.value });
  });
}
...
function treeWithoutParent(node) {
  ...
  return {
    ...
    children: _map(node.children, treeWithoutParent),
  };
}
function leavesAt(tree, depth) {
  ...
  (function walk(node) {
    if (node.depth === depth) { _push(out, node); return; }
    _forEach(node.children, walk);
  })(tree);
  return out;
}
function pruneChildless(tree, depth) {
  ...
  for (var d = depth; d > 0; d--) {
    _forEach(leavesAt(tree, d), function (node) {
      if (node.children.length === 0 && node.parent) {
        var idx = _indexOf(node.parent.children, node);
...
function isSubtreeBaseValid(tag, base) {
  switch (tag) {
    ...
    case 7: return (_isBuffer(base) || base instanceof Uint8Array) &&
                   (base.length === 8 || base.length === 32);                      // iPAddress: address + mask
    case 4: return base !== null && typeof base === "object" && !!base.rdns && _isArray(base.rdns);   // directoryName
...
function checkedSubtreeSeeds(list, optName) {
  ...
  if (!_isArray(list)) throw E("path/bad-input", "validate: opts." + optName + " must be an array of { tag, base } subtree entries");
  return _map(list, function (st) {
    if (!st || typeof st !== "object" || !_isInteger(st.tag) || st.tag < 0 || st.tag > 8 || !isSubtreeBaseValid(st.tag, st.base)) {
...
function processPolicies(state, cert, i, checks) {
  ...
  catch (e) { _push(checks, { name: "policies", ok: false, code: "path/bad-policy" }); return { fatal: true, error: e }; }
  ...
  if (cp && state.validPolicyTree) {
    ...
    _forEach(policies, function (p) {
      if (p.policyIdentifier === OID.anyPolicy) { anyPolicyPresent = true; anyPolicyQualifiers = p.qualifiersBytes; return; }
      var matched = false;
      _forEach(leavesAt(state.validPolicyTree, depth), function (node) {
        if (_indexOf(node.expectedPolicySet, p.policyIdentifier) !== -1) {
    ...
    if (anyPolicyPresent && anyPolicyActive) {
      ...
      _forEach(leavesAt(state.validPolicyTree, depth), function (node) {
        _forEach(node.expectedPolicySet, function (ep) {
          var already = _some(node.children, function (ch) { return ch.validPolicy === ep; });
  ...
  if (!(state.explicitPolicy > 0 || state.validPolicyTree !== null)) {
    _push(checks, { name: "policy", ok: false, code: "path/policy-required" });
...
function ekuPurposeFails(cert, requiredEku, checks) {
  ...
  catch (e) { _push(checks, { name: "extendedKeyUsage", ok: false, code: pathCode(e, "path/bad-extension-value") }); return true; }
  if (!eku) return false;   // absent EKU: unrestricted (4.2.1.12 restricts only when present)
  var purposes = eku.value;
  var ok = _indexOf(purposes, OID.anyExtendedKeyUsage) !== -1 ||
    _every(requiredEku, function (p) { return _indexOf(purposes, p) !== -1; });
  _push(checks, { name: "extendedKeyUsage", ok: ok, code: ok ? undefined : "path/eku-not-permitted" });
  return !ok;
}
...
function prepareNext(state, cert, i, checks) {
  ...
  catch (e) { _push(checks, { name: "policyMappings", ok: false, code: "path/bad-policy" }); return { fatal: true, error: e }; }
  if (pm) {
    var badAny = _some(pm.value, function (m) { return m.issuerDomainPolicy === OID.anyPolicy || m.subjectDomainPolicy === OID.anyPolicy; });
    if (badAny) { _push(checks, { name: "policyMappings", ok: false, code: "path/bad-policy" }); return { fatal: true, error: E("path/bad-policy", "policyMappings must not map to or from anyPolicy (RFC 5280 6.1.4(a))") }; }
  ...
  catch (e) { _push(checks, { name: "nameConstraints", ok: false, code: pathCode(e, "path/bad-name-constraints") }); return { fatal: true, error: e }; }
  ...
  catch (e) { _push(checks, { name: "policyConstraints", ok: false, code: "path/bad-policy" }); return { fatal: true, error: e }; }
  ...
  catch (e) { _push(checks, { name: "inhibitAnyPolicy", ok: false, code: "path/bad-policy" }); return { fatal: true, error: e }; }
  ...
  catch (e) { _push(checks, { name: "basicConstraints", ok: false, code: "path/bad-basic-constraints" }); return { fatal: true, error: e }; }
  if (!bc || bc.value.cA !== true) {
    _push(checks, { name: "basicConstraints", ok: false, code: "path/not-a-ca" });
  ...
  // (l),(m) path length.
  if (!isSelfIssued) {
    if (state.maxPathLength <= 0) { _push(checks, { name: "pathLength", ok: false, code: "path/path-length-exceeded" }); return { fatal: true, error: E("path/path-length-exceeded", "certification path is longer than the CA path-length constraint allows") }; }
    state.maxPathLength--;
  }
  ...
  var ku;
  try { ku = decodeExt(cert, OID.keyUsage); }
  catch (e) { _push(checks, { name: "keyUsage", ok: false, code: "path/bad-key-usage" }); return { fatal: true, error: e }; }
  if (ku && ku.value.keyCertSign !== true) {
    _push(checks, { name: "keyUsage", ok: false, code: "path/missing-key-cert-sign" });
...
function applyPolicyMappings(state, mappings, i) {
  ...
  if (state.policyMapping > 0) {
    ...
    _forEach(mappings, function (m) { _push((mappedFrom[m.issuerDomainPolicy] = mappedFrom[m.issuerDomainPolicy] || []), m.subjectDomainPolicy); });
    var depthI = leavesAt(state.validPolicyTree, depth);
    var anyNodes = _filter(depthI, function (nd) { return nd.validPolicy === OID.anyPolicy; });
    _forEach(_keys(mappedFrom), function (idp) {
      var idpNodes = _filter(depthI, function (nd) { return nd.validPolicy === idp; });
      if (idpNodes.length) {
        _forEach(idpNodes, function (nd) { nd.expectedPolicySet = _arraySlice(mappedFrom[idp]); });
      } else {
        ...
        _forEach(anyNodes, function (anyNode) {
          if (anyNode.parent) addChild(state, anyNode.parent, idp, anyNode.qualifierSet[0] || null, _arraySlice(mappedFrom[idp]), []);
        });
      }
    });
  } else {
    ...
    var mappedSet = {};
    _forEach(mappings, function (m) { mappedSet[m.issuerDomainPolicy] = true; });
...
async function validate(path, opts) {
  ...
  if (!_isArray(path)) throw E("path/bad-input", "validate: path must be an array of certificates");
  ...
  var certs = _map(path, function (c, ci) { return coerceCert(c, "validate: path[" + ci + "]"); });
  var n = certs.length;
  ...
  if (opts.userInitialPolicySet !== undefined) {
    var uipsOk = _isArray(opts.userInitialPolicySet) && opts.userInitialPolicySet.length > 0 &&
      _every(opts.userInitialPolicySet, function (p) { return typeof p === "string" && p.length > 0; });
    ...
    _forEach(opts.userInitialPolicySet, function (p) {
      guard.identifier.assertCanonicalOid(p, E, "path/bad-input", "validate: opts.userInitialPolicySet entry " + _stringify(p));
    });
  }
  var seeds = {
  ...
  for (var idx = 0; idx < n; idx++) {
    ...
    _push(checks, { name: "signature", ok: sigRes.ok, code: sigRes.ok ? undefined : (sigRes.code || "path/bad-signature") });
    ...
    if (compositeSig.COMPOSITE_ALGS[cert.subjectPublicKeyInfo.algorithm.oid]) {
      ...
      _push(checks, { name: "compositeKeyUsage", ok: cku.ok, code: cku.ok ? undefined : cku.code });
    ...
    if (ML_KEM_OIDS[cert.subjectPublicKeyInfo.algorithm.oid]) {
      ...
      _push(checks, { name: "kemKeyUsage", ok: kku.ok, code: kku.ok ? undefined : kku.code });
    ...
    _push(checks, { name: "validity", ok: vOk, code: vCode });
    if (!vOk) failed = true;
    ...
    _push(checks, { name: "nameChaining", ok: chainOk === true, code: chainOk === true ? undefined : "path/name-chaining" });
    ...
    if (!(selfIssued(cert) && i !== n)) {
      ...
      _push(checks, { name: "nameConstraints", ok: ncRes.ok, code: ncRes.ok ? undefined : ncRes.code });
    ...
    if (revocationChecker) {
      ...
      if (rv && rv.status === "good") { _push(checks, { name: "revocation", ok: true, status: "good" }); }
      else if (rv && rv.status === "revoked") { _push(checks, { name: "revocation", ok: false, status: "revoked", code: "path/revoked" }); failed = true; }
      else if (rvError) {
        ...
        // and this is not.
        _push(checks, { name: "revocation", ok: false, status: "error", code: "path/revocation-checker-error", error: rvError });
        ...
        _push(checks, { name: "revocation", ok: true, status: rvStatus, waived: true });
        revocationWaived = true;
      } else {
        _push(checks, { name: "revocation", ok: false, status: rvStatus, code: "path/revocation-undetermined" });
        revocationUndetermined = true;
        failed = true;
      }
      revocationRan = true;
      ...
      _push(checks, { name: "revocation", ok: false, code: "path/revocation-undetermined" }); failed = true;
    }

    // 6.1.3(d-f) policies.
    var polRes = processPolicies(state, cert, i, checks);
    if (state._capHit) { _push(checks, { name: "policyTree", ok: false, code: "path/policy-tree-cap" }); failed = true; }
    ...
    if (cert.subject.rdns.length === 0) {
      ...
      if (!san || !san.critical) { _push(checks, { name: "emptySubject", ok: false, code: "path/empty-subject-no-critical-san" }); failed = true; }
    }
    ...
    if (i !== n) {
      ...
      catch (_e) { lpc = null; _push(checks, { name: "policyConstraints", ok: false, code: "path/bad-policy" }); failed = true; }
      ...
      catch (_e) { lpm = null; _push(checks, { name: "policyMappings", ok: false, code: "path/bad-policy" }); failed = true; }
      if (lpm && _some(lpm.value, function (m) { return m.issuerDomainPolicy === OID.anyPolicy || m.subjectDomainPolicy === OID.anyPolicy; })) {
        _push(checks, { name: "policyMappings", ok: false, code: "path/bad-policy" }); failed = true;
      }
      ...
      catch (e) { lnc = null; _push(checks, { name: "nameConstraints", ok: false, code: pathCode(e, "path/bad-name-constraints") }); failed = true; }
      if (requireCriticalExt(lnc, "nameConstraints", checks)) failed = true;
      var liap;
      try { liap = decodeExt(cert, OID.inhibitAnyPolicy); }
      catch (e) { liap = null; _push(checks, { name: "inhibitAnyPolicy", ok: false, code: pathCode(e, "path/bad-policy") }); failed = true; }
      ...
      if (distrustDate != null) {
        ...
        if (guard.time.instantOf(cert.validity.notBefore) > guard.time.instantOf(distrustDate)) {
          _push(checks, { name: "distrustAfter", ok: false, code: "path/distrusted-after" }); failed = true;
        }
      }
      if (checkPurpose && ta.purposes) {
        anchorPurposeApplied = true;
        if (ta.purposes[checkPurpose] !== true) {
          _push(checks, { name: "purposeTrust", ok: false, code: "path/purpose-not-trusted" }); failed = true;
        }
      }
    ...
    if (unk) { _push(checks, { name: "criticalExtensions", ok: false, code: "path/unrecognized-critical-extension" }); failed = true; }
    ...
    if (crit) { _push(checks, { name: "criticalExtensionValue", ok: false, code: crit }); failed = true; }

    _push(state.results, { index: idx, checks: checks });
  }
  ...
  if (!policyOk) {
    ...
    if (!_some(last.checks, function (c) { return c.code === "path/policy-required"; })) {
      _push(last.checks, { name: "policy", ok: false, code: "path/policy-required" });
    }
    failed = true;
  }
...
function userConstrainedPolicies(state, n) {
  ...
  if (hasAnyLeaf) {
    ...
    else _forEach(uips, function (p) { set[p] = true; });
  }
  return _keys(set);
}
    ...
    if (idpDpn.kind === "fullName" && cdpn.kind === "fullName") {
      for (var a = 0; a < idpDpn.names.length; a++) {
        for (var c = 0; c < cdpn.names.length; c++) {
          if (_equals(idpDpn.names[a], cdpn.names[c])) return dp;
        }
      }
    } else if (idpDpn.kind === "rdn" && cdpn.kind === "rdn") {
      if (_equals(idpDpn.bytes, cdpn.bytes)) return dp;
    }
  }
  return null;
}
...
function classifyCrls(parsed) {
  ...
  for (var i = 0; i < parsed.length; i++) {
    ...
    if (deltaRaw === null) { _push(completes, rec); continue; }
    try {
...
function crlChecker(crls, opts) {
  var parsed = _map(crls || [], function (c, ci) {
  ...
  return {
    check: async function (cert, issuer, ctx) {
      ...
      for (var ci = 0; ci < classified.completes.length; ci++) {
        ...
        var status;
        if (chosenDelta) {
          _push(consumedDeltas, chosenDelta);
      ...
      for (var dj = 0; dj < classified.deltas.length; dj++) {
        ...
        if (_indexOf(consumedDeltas, dRec) !== -1) continue;
      ...
      if (certStatus !== null) {
        var reasonName = constants.NAMES.CRL_REASON[_String(certStatus)] || "unspecified";
...
function _verifyWithSpki(sigAlg, rawSig, spkiBytes, tbsBytes) {
  ...
  } catch (_e) { return _promiseResolve(_Promise, false); }
...
function ocspChecker(responses) {
  ...
  var parsed = _map(responses || [], function (r) { return _ocspFromBytes(r); });
  return {
    check: async function (cert, issuer, ctx) {
      var time = ctx.time;
      var historical = ctx.historicalMode === true;
      ...
      function addNameCandidate(nm) {
        if (nm && nm.bytes && !_some(issuerNameCandidates, function (e) { return _equals(e, nm.bytes); })) _push(issuerNameCandidates, nm.bytes);
      }
...
function _ocspFromBytes(response) {
  return guard.parsed.fromTrustedSource(response, "ocspResponse", _OCSP_CLAIM, function (bytes) {
    if (!_isBuffer(bytes) && !(bytes instanceof Uint8Array) && typeof bytes !== "string") {
...
function _verifyOcspParsed(parsedResponse, cert, issuerCert, time, opts) {
  ...
  if (!guard.time.isDate(time) || _isNaN(guard.time.instantOf(time))) {
    return _promiseReject(_Promise, E("path/bad-input", "verifyOcspResponse: time must be a valid Date (the currency + responder-validity check date)"));
  }
  ...
  catch (e) { return _promiseReject(_Promise, e); }   // an embedded control byte in a DN -> path/name-chaining
  if (!boundName) {
    return _promiseResolve(_Promise, unbound("the supplied issuer certificate's subject does not match the target certificate's issuer"));
  }
  return builtinVerify({ workingPublicKey: issuerCert.subjectPublicKeyInfo.bytes }, cert).then(function (sig) {
    ...
    var issuerNameCandidates = [cert.issuer.bytes];
    function add(nm) { if (nm && nm.bytes && !_some(issuerNameCandidates, function (e) { return _equals(e, nm.bytes); })) _push(issuerNameCandidates, nm.bytes); }
...
function _hasPurposeScopedMetadata(ta) {
  ...
  return _some(["distrustAfter", "purposes"], function (k) {
    var m = ta[k];
    return !!m && typeof m === "object" && _keys(m).length > 0;
  });
}
...
function resolvePurposeOpts(opts) {
  ...
  if (opts.requiredEku !== undefined) {
    if (!_isArray(opts.requiredEku) || opts.requiredEku.length === 0) {
    ...
    requiredEku = _map(opts.requiredEku, function (p) {
      ...
      if (_reTest(/^[0-9]/, p)) return guard.identifier.assertCanonicalOid(p, E, "path/bad-input", "validate: opts.requiredEku entry " + _stringify(p));
      var dotted = oid.byName(p);
      if (typeof dotted !== "string") throw E("path/bad-input", "validate: opts.requiredEku entry " + _stringify(p) + " is not a registered OID name");
      return dotted;
    });
  }
  ...
  if (opts.checkPurpose !== undefined) {
    ...
    if (_reTest(/^[0-9]/, opts.checkPurpose)) {
      ...
      if (typeof oid.byName(opts.checkPurpose) !== "string") throw E("path/bad-input", "validate: opts.checkPurpose " + _stringify(opts.checkPurpose) + " is not a registered OID name");
...
function toAnchor(entry) {
  if (entry && typeof entry === "object" && !_isBuffer(entry) && entry.name && entry.publicKey && entry.algorithm) {
    ...
    if (!_isArray(entry.name.rdns) || !_isBuffer(entry.publicKey) || typeof entry.algorithm !== "string") {
...
function identityKey(cert) {
  var san = findExt(cert, OID.subjectAltName);
  return _bufToString(cert.subject.bytes, "hex") + "|" + (san ? _bufToString(san.value, "hex") : "") + "|" +
    _bufToString(cert.subjectPublicKeyInfo.bytes, "hex");
}
...
function certDerKey(cert) {
  return _bufToString(cert.tbsBytes, "base64") + "|" + (cert.signatureValue && cert.signatureValue.bytes ? _bufToString(cert.signatureValue.bytes, "base64") : "");
}
...
function scoreCandidate(cand, childAki, anchors, time) {
  ...
  if (childAki) {
    ...
    if (d && _isBuffer(d.value) && _equals(d.value, childAki)) score += 1000;   // sec. 3.5.12 KID match: heaviest
  }
...
function _pushCandidates(frame, scored, stack, counter) {
  _sort(scored, function (a, b) { return a.score - b.score; });   // ascending -> push lowest first so the highest is popped first
  var n = 0;
  for (var ci = 0; ci < scored.length; ci++) {
    ...
    _push(stack, { chain: _concat([cand], frame.chain), hop: frame.hop + 1, keys: childKeys });
  }
  return n;
}
...
function _isBlockedAiaHost(host) {
  if (_charAt(host, 0) === "[" && _charAt(host, host.length - 1) === "]") host = _strSlice(host, 1, -1);   // an IPv6 literal: URL.hostname keeps the [brackets]
...
function _aiaParseBody(body, contentType, maxCerts) {
  var certsFirst = _strIndexOf(_toLowerCase(_String(contentType || "")), "pkcs7") >= 0;   // HINT: order the attempts only
  var order = certsFirst ? ["certs", "cert"] : ["cert", "certs"];
  for (var i = 0; i < order.length; i++) {
    try {
      if (order[i] === "cert") { x509.parse(body); return [_bufferFrom(body)]; }
...
async function _aiaFetchOne(uri, aia) {
  ...
  var body = _isBuffer(res.body) ? res.body : _bufferFrom(res.body == null ? "" : _String(res.body), "latin1");
  ...
  var headers = {};
  _forEach(_keys(res.headers || {}), function (k) { headers[_toLowerCase(k)] = res.headers[k]; });
...
async function _fetchAiaIssuers(current, aia) {
  ...
  for (var i = 0; i < d.value.length; i++) {
    ...
    if (net.isIP(_replace(u.hostname, /^\[(.*)\]$/, "$1")) === 0 && !aia.transportGuardsAddresses) continue;
...
var _BUILD_OPTS = (function () {
  ...
  // allow:guard-reads-runtime-live -- module-init, the union is computed once at load inside this IIFE
...
async function build(leaf, opts) {
  ...
  if (!_isArray(poolInput)) throw E("path/bad-input", "build: opts.candidates must be an array of certificates");
  ...
  var pool;
  try { pool = _map(poolInput, coerceCert); }
  ...
  if (!_isArray(opts.trustAnchors) || opts.trustAnchors.length === 0) throw E("path/bad-input", "build: opts.trustAnchors must be a non-empty array of anchor tuples or root certificates");
  var anchors = _map(opts.trustAnchors, toAnchor);
  ...
  var maxDepth = guard.limits.cap(opts.maxDepth, "build: opts.maxDepth", _min(constants.LIMITS.PATH_BUILD_MAX_DEPTH, depthCeiling), { E: E, code: "path/bad-input", min: 0, max: depthCeiling });
  ...
  _forEach(_keys(_VALIDATE_OPTS), function (k) {
    if (_hasOwn(BUILD_ONLY_OPT, k)) return;
    if (k in opts) forwarded[k] = opts[k];
  });
  function validateOpts(anchor) {
    var vo = _create(null);   // same reason as `forwarded` above
    ...
    _forEach(_keys(forwarded), function (f) { vo[f] = forwarded[f]; });
  ...
  while (!success) {
    if (!stack.length) {
      ...
      if (!fb.fetched && aiaCtx && aiaCtx.fetches < aiaCtx.maxFetches) {
        ...
        for (var fj = 0; fj < fetched.length; fj++) {
          ...
          poolDerKeys.add(fdk);
          if (pool.length < poolCeiling) _push(pool, fetched[fj]);
        }
      }
      ...
      for (var pj = fb.poolMark; pj < pool.length; pj++) {
        if (nameMatchSoft(pool[pj].subject.rdns, fbCur.issuer.rdns)) {
          _push(fbScored, { cand: pool[pj], score: scoreCandidate(pool[pj], fbAki, anchors, opts.time) });
        }
      }
    ...
    for (var ai = 0; ai < anchors.length; ai++) {
      ...
      if (!doValidate) { success = { path: _arraySlice(frame.chain), trustAnchor: anchors[ai] }; break; }
      anyChainAssembled = true;
      var res = await validate(frame.chain, validateOpts(anchors[ai]));
      if (res.valid) { success = { valid: true, path: _arraySlice(frame.chain), trustAnchor: anchors[ai], result: res }; break; }
      if (!bestFail) bestFail = { path: _arraySlice(frame.chain), trustAnchor: anchors[ai], result: res };
    }
    ...
    for (var pi = 0; pi < pool.length; pi++) {
      if (nameMatchSoft(pool[pi].subject.rdns, current.issuer.rdns)) {
        _push(scored, { cand: pool[pi], score: scoreCandidate(pool[pi], childAki, anchors, opts.time) });
      }
    }
    ...
    if (aiaCtx && frame.hop < maxDepth && aiaCtx.fetches < aiaCtx.maxFetches) {
      ...
      _push(deferredAia, { chain: frame.chain, hop: frame.hop, keys: frame.keys, poolMark: pool.length, seq: deferSeq, fetched: false });
      deferSeq += 1;
    }
