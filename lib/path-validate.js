...
function toAnchor(entry, verb) {
  ...
  if (isTuple) {
    ...
    var algStr;
    try {
      var algIsObject = alg !== null && typeof alg === "object";
      algStr = typeof alg === "string" ? alg : (algIsObject && typeof alg.oid === "string" ? alg.oid : null);
    } catch (e) { throw E("path/bad-input", who + ": trustAnchor algorithm.oid accessor threw", e); }
    if (!(meta.name && intrinsic.isArray(meta.name.rdns)) || !Buffer.isBuffer(pubRaw) || algStr === null) {
    ...
    var flat = {};
    try {
      var names = intrinsic.getOwnPropertyNames(entry);
