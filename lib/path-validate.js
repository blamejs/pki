...
function assertAnchorConstraints(ta, checkPurpose) {
  var d = (checkPurpose && ta && ta.distrustAfter && intrinsic.hasOwn(ta.distrustAfter, checkPurpose))
    ? ta.distrustAfter[checkPurpose] : null;
...
function toAnchor(entry, verb) {
  ...
  if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && entry.name && entry.publicKey && entry.algorithm) {
    ...
    if (!intrinsic.isArray(entry.name.rdns) || !Buffer.isBuffer(entry.publicKey) || algStr === null) {
    ...
    var spkiKeyOid;
    try { spkiKeyOid = asn1.read.oid(asn1.decode(pk).children[0].children[0]); }
    catch (e) { throw E("path/bad-input", who + ": trustAnchor publicKey is not a decodable SubjectPublicKeyInfo", e); }
    if (spkiKeyOid !== algStr) {
      throw E("path/bad-input", who + ": trustAnchor algorithm " + algStr + " does not match its publicKey key algorithm " + spkiKeyOid);
    }
    // Return an anchor carrying the pinned publicKey and the normalized algorithm, inheriting every other
    // property (purposes, distrustAfter and any inherited/non-enumerable constraint metadata) from entry, so
    // initialize() binds the exact bytes the check read and an object { oid, parameters } form seeds a string
    // OID (an object never equals a cert's keyAlg.oid and would wrongly clear an EC child's inherited params).
    // Overrides are OWN data properties: a plain assignment would travel an inherited descriptor and throw
    // (strict) or silently no-op on a frozen / sealed / accessor-backed anchor.
    var flat = intrinsic.create(entry);
    intrinsic.defineProperty(flat, "publicKey", { value: pk, enumerable: true, configurable: true, writable: true });
    intrinsic.defineProperty(flat, "algorithm", { value: algStr, enumerable: true, configurable: true, writable: true });
    if (algIsObject && alg.parameters !== undefined) intrinsic.defineProperty(flat, "parameters", { value: alg.parameters, enumerable: true, configurable: true, writable: true });
    return flat;
  }
  var cert;
