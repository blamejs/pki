...
async function verifyOcspResponse(response, cert, issuerCert, time, opts) {
  ...
  /** @internal Defined, not assigned: an assignment runs an inherited setter, so a prototype
   * polluted while this verification was pending could swallow the write and answer its own getter. */
  var withValid = intrinsic.assign({}, v);
  intrinsic.defineProperty(withValid, "valid", { value: v.status === "good", enumerable: true, configurable: true, writable: true });
  return withValid;
}
