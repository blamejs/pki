...
function coerceCert(input) {
  ...
  if (input && typeof input === "object" && !Buffer.isBuffer(input) && input.tbsBytes !== undefined) {
    if (!input.subject || !input.issuer || !input.subjectPublicKeyInfo || !Array.isArray(input.extensions)) {
...
async function build(leaf, opts) {
  ...
  var maxDepth = guard.limits.cap(opts.maxDepth, "build: opts.maxDepth", Math.min(constants.LIMITS.PATH_BUILD_MAX_DEPTH, depthCeiling), { E: E, code: "path/bad-input", min: 1, max: depthCeiling });
