  ...
  if (paramsBytes) {
    ...
    }
    {
      n.children.forEach(function (f) {
        ...
        // a primitive/childless context field is malformed — fail closed rather
        // than skip it and verify under the defaults.
        ...
        if (f.tagNumber === 0) {
          ...
          // A negative saltLength must be rejected: the OpenSSL-backed verify
          // shim would read -1/-2/-3 as RSA_PSS_SALTLEN_DIGEST/AUTO/MAX, and
          // AUTO (-2) accepts a signature of ANY salt length — defeating the
          // salt-length binding this descriptor is meant to pin.
...
function crlEntryReason(entry) {
  ...
  for (var i = 0; i < exts.length; i++) {
    if (exts[i].name === "reasonCode" || exts[i].oid === OID_REASON_CODE) return exts[i].value;
  }
  return null;
}
