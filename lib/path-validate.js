...
// a total fetch budget (a SILENT cap -- stop fetching, never a throw that aborts a buildable path), a per-cert
// URL cap, a build-wide URL dedupe (on the NORMALIZED URL), a response size + certificate-count cap, no redirect
// following. Every fetch fault is a SKIP (the DFS continues over the pool). A fetched cert is UNTRUSTED pool
...
async function _fetchAiaIssuers(current, aia) {
  ...
  if (!d || !d.value) return [];
  var uris = [];
  for (var i = 0; i < d.value.length && uris.length < aia.maxPerCert; i++) {   // per-cert URL cap
    var ad = d.value[i];
    ...
    // (so whitespace / case / default-port variants collapse to one fetch, and the transport gets the string
    // that was actually validated, not the raw IA5String).
    var u;
    ...
    if (aia.fetchedUrls.has(u.href)) continue;            // already fetched build-wide -> dedupe on the normalized URL
    uris.push(u.href);
  }
  var out = [];
