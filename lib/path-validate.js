...
async function ctGateCheck(leaf, certs, anchor, ctLogList, ctPolicy, n) {
  ...
  try {
    verdict = await ct.verifySctList(entry, list, ctLogList,
      { minScts: ctPolicy.minScts, minOperators: ctPolicy.minOperators, certNotAfter: leaf.validity.notAfter });
  } catch (e) {
...
async function validate(path, opts) {
  ...
  if (opts.ctLogList != null) {
    guard.identifier.assertKnownKeys(guard.identifier.optionsObject(opts.ctPolicy, E, "path/bad-input", "validate: opts.ctPolicy"),
      { minScts: 1, minOperators: 1 }, E, "path/bad-input", "validate: opts.ctPolicy has an unknown key (the CT policy keys are minScts / minOperators): ");
  }
  ...
  if (opts.ctLogList != null) {
    var ctCheck = await ctGateCheck(certs[n - 1], certs, anchor, opts.ctLogList, opts.ctPolicy || {}, n);
    var target = null;
