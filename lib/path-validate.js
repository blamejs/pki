...
function coerceCert(input) {
  return (input && input.tbsBytes) ? input : x509.parse(input);
}
...
async function build(leaf, opts) {
  ...
  // The chain-length bound is capped at PATH_MAX_CERTS: a path deeper than the
  // validator's own per-path ceiling would be rejected on hand-off anyway, and
  // the explicit-stack search below carries no native-recursion stack risk.
  var maxDepth = guard.limits.cap(opts.maxDepth, "build: opts.maxDepth", constants.LIMITS.PATH_BUILD_MAX_DEPTH, { E: E, code: "path/bad-input", min: 1, max: constants.LIMITS.PATH_MAX_CERTS });
  ...
  while (stack.length && !success) {
    ...
    for (var ai = 0; ai < anchors.length && !success; ai++) {
