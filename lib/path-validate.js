...
// The trust-anchor contract's caller-supplied fields, carried onto the normalized anchor: the identity and
// constraint metadata path validation reads (name, purposes, distrustAfter) PLUS subjectDer, which the
// anchorFromCert output contract documents and the trust store's dedup consumes. publicKey / algorithm /
// parameters are NOT here -- they are derived from the validated SubjectPublicKeyInfo, never copied from the
// caller. Normalizing a tuple materializes each PRESENT field from the caller EXACTLY ONCE into plain data;
// a field outside this set is dropped. subjectDer is included so anchorFromCert(anchorFromCert(cert)) keeps
// the documented field -- re-normalizing a ready tuple is idempotent, not lossy. Keep this in step with the
// anchor contract: every anchor field a consumer reads -- here or in the trust store -- is one of these.
var ANCHOR_METADATA_FIELDS = ["name", "purposes", "distrustAfter", "subjectDer"];
...
function toAnchor(entry, verb) {
  ...
  if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && entry.name && entry.publicKey && entry.algorithm) {
    ...
    var algStr = typeof alg === "string" ? alg : (algIsObject && typeof alg.oid === "string" ? alg.oid : null);
    if (!(meta.name && intrinsic.isArray(meta.name.rdns)) || !Buffer.isBuffer(entry.publicKey) || algStr === null) {
    ...
    var pk = guard.bytes.snapshot(entry.publicKey, PathError, "path/bad-input", who + ": trustAnchor publicKey");
    ...
    // Return a SELF-CONTAINED anchor: a fresh plain object holding the anchor CONTRACT fields as OWN data
    // properties -- the pinned publicKey and the algorithm AND parameters DERIVED from the validated SPKI
    // (never a caller-declared field), plus the caller's identity/constraint metadata (name, purposes,
    // distrustAfter, subjectDer), each materialized to a plain value from the single `meta` read above so the
    // value that is shape-checked, bound, and stored is one read. Storing DATA -- not the caller's accessor --
    // is load-bearing: assertAnchorConstraints reads
    // distrustAfter several times (a truthy gate, a hasOwn gate, then the value), and _hasPurposeScopedMetadata
    // and the purpose block read purposes at more than one site, so an accessor-backed field left live on the
    // anchor could answer those reads inconsistently -- a restriction to the gates, {} to the value read --
    // and silently drop a distrust or purpose control (a leaf issued past the operator's distrust date would
    // then validate). Any field OUTSIDE this contract is dropped, never copied: validation never reads it, so
    // an unrelated caller getter is not evaluated and nothing the caller attached reaches validation as live
    // code. entry[field] reads through the prototype chain, so an anchor built with Object.create(baseAnchor)
    // keeps its metadata; defineProperty on a fresh plain object takes each value regardless of a frozen /
    // sealed / accessor-backed input, and no caller `__proto__` field is ever named here, so none can reach
    // Object.prototype's `__proto__` setter. Seeding the working key from a string OID keeps the sec. 6.1.4
    // key-algorithm comparison working (an object never equals a cert's keyAlg.oid); taking parameters from
    // the key's own AlgorithmIdentifier stops a declared value that disagrees with it (e.g. P-384 params on a
    // P-256 anchor) from being promoted into workingPublicKeyParameters and inherited by a child that omits
    // its own, which would validate a chain RFC 5280 inheritance from the real params rejects.
    var flat = {};
    for (mi = 0; mi < ANCHOR_METADATA_FIELDS.length; mi++) {
      mf = ANCHOR_METADATA_FIELDS[mi];
