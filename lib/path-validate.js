...
function _copyRdnsDeep(rdns) {
  ...
  var out = intrinsic.arraySlice(rdns, 0);
  for (var i = 0; i < out.length; i++) {
    if (!intrinsic.isArray(out[i])) continue;
    var rdnOut = intrinsic.arraySlice(out[i], 0);
    for (var j = 0; j < rdnOut.length; j++) {
      if (rdnOut[j] !== null && typeof rdnOut[j] === "object") rdnOut[j] = _copyAtv(rdnOut[j]);
    }
    out[i] = rdnOut;
  }
  return out;
...
function toAnchor(entry, verb) {
  ...
  try {
    ...
    if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && !guard.parsed.isRecorded(entry)) {
      ...
      // anchor built with Object.create(base) is still recognized), each exactly once. entry's own shape is
      // NEVER enumerated (no `in` / ownKeys / getOwnPropertyDescriptor) and the prototype chain is never
      // walked, so a Proxy anchor cannot use an enumeration trap to mutate state mid-normalization -- each
      // field is one plain `get` a hostile accessor answers at most once. (Buffer.isBuffer / isCert above
      // invoke getPrototypeOf a bounded number of times; the chain is never WALKED, so a hostile getPrototypeOf
      // cannot be driven into a loop.)
      ...
      // initialize()'s state.workingPublicKey both use the private copy. A non-tuple leaves pk null / isTuple
      // false and falls through to coerceCert below.
  ...
  if (isTuple) {
    ...
    try {
      ...
      nameHasRdns = intrinsic.isArray(nameOut.rdns);
