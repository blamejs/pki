...
function toAnchor(entry, verb) {
  ...
  if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && entry.name && entry.publicKey && entry.algorithm) {
    ...
    // Return an anchor carrying the pinned publicKey, and the algorithm AND parameters DERIVED from the
    // validated SPKI (never a caller-declared field), inheriting every other property (purposes,
    // distrustAfter, inherited/non-enumerable constraint metadata) from entry. initialize() binds the exact
    // bytes the check read; the working key seeds from a string OID (an object never equals a cert's
    // keyAlg.oid and would wrongly clear an EC child's inherited params); and taking parameters from the
    // key's own AlgorithmIdentifier stops a declared value that disagrees with it (e.g. P-384 params on a
    // P-256 anchor) from being promoted into workingPublicKeyParameters and inherited by a child that omits
    // its own, which would validate a chain RFC 5280 inheritance from the real params rejects. Overrides are
    // OWN data properties: a plain assignment would travel an inherited descriptor and throw (strict) or
    // silently no-op on a frozen / sealed / accessor-backed anchor.
    ...
    intrinsic.defineProperty(flat, "parameters", { value: spkiAlg.parameters == null ? undefined : spkiAlg.parameters, enumerable: true, configurable: true, writable: true });
    return flat;
  }
  var cert;
