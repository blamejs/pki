...
async function validate(path, opts) {
  ...
  for (var idx = 0; idx < n; idx++) {
    ...
    if (i !== n) {
      ...
      if (requiredEku) {
        var eku, ekuDecoded = false;
        try { eku = decodeExt(cert, OID.extKeyUsage); ekuDecoded = true; }
        catch (e) { checks.push({ name: "extendedKeyUsage", ok: false, code: pathCode(e, "path/bad-extension-value") }); failed = true; }
        if (ekuDecoded) {
          var ekuOk = true;
          if (eku) {
            var purposes = eku.value;
            var hasAny = purposes.indexOf(OID.anyExtendedKeyUsage) !== -1;
            ekuOk = hasAny || requiredEku.every(function (p) { return purposes.indexOf(p) !== -1; });
          }
          checks.push({ name: "extendedKeyUsage", ok: ekuOk, code: ekuOk ? undefined : "path/eku-not-permitted" });
          if (!ekuOk) failed = true;
        }
      }
