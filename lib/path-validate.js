...
async function build(leaf, opts) {
  ...
  /** @internal Checked whether or not AIA fetching is on, since supplying a transport that cannot be
   * called is a wiring fault at the moment it is written, and checking it only under fetchAia would
   * leave the mistake sitting until the day that option is turned on. The checked value is the one
   * used below: reading the option again would ask a second time, and an accessor can answer
