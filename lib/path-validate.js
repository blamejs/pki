...
// NO private / loopback / link-local IP-literal destination (an untrusted cert cannot drive an authenticated GET
// to an internal service / cloud metadata; a DNS-name destination is the operator's opts.transport policy), a
...
// SSRF guard: is a URL host a private / loopback / link-local / reserved IP LITERAL? An AIA URL comes from an
// UNTRUSTED certificate, so a literal address into RFC 1918 / loopback / the 169.254 cloud-metadata range must
// not be fetched (with enterprise TLS trust it would be an authenticated GET to an internal service). A DNS
// NAME is not judged here -- for hostname-based / enterprise destination policy the operator injects opts.transport
// (its own address filter). A malformed IP literal fails CLOSED (blocked).
function _isBlockedAiaHost(host) {
  ...
  var fam = net.isIP(host);   // 4 / 6 / 0; 0 = a DNS name (not judged here -- opts.transport is the operator's destination policy)
  if (fam === 4) {
    // net.isIP === 4 guarantees exactly four octets 0..255, so a/b are safe.
    var o = host.split("."), a = +o[0], b = +o[1];
    return a === 10 || a === 127 || a === 0 || a >= 224 ||        // private / loopback / this-network / multicast + reserved
      (a === 172 && b >= 16 && b <= 31) || (a === 192 && b === 168) ||   // RFC 1918
      (a === 169 && b === 254) || (a === 100 && b >= 64 && b <= 127);    // link-local (cloud metadata) + 100.64.0.0/10 CGNAT
  }
  if (fam === 6) {
    var l = host.toLowerCase();
    if (l === "::1" || l === "::" || l.indexOf("::ffff:") === 0) return true;   // loopback / unspecified / IPv4-mapped (may embed a private v4)
    var h = parseInt(l.split(":")[0], 16);                        // first hextet (a leading "::" is already handled above)
    return h >= 0xfc00 && h <= 0xfebf;                            // fc00::/7 ULA + fe80::/10 link-local + the reserved gap between them (fail-closed)
  }
  return false;
}
...
async function _aiaFetchOne(uri, aia) {
  var res = await aia.transport({ method: "GET", url: uri, tls: aia.tls, timeout: aia.timeout, maxResponseBytes: aia.maxResponseBytes });
  res = res || {};
...
async function _fetchAiaIssuers(current, aia) {
  ...
  for (var i = 0; i < d.value.length; i++) {
    ...
    uris.push(u.href);
    if (uris.length >= aia.maxPerCert) break;             // the per-cert cap counts DISTINCT urls
  }
  var out = [];
 ...
 * @opts  transport              The injectable transport seam (`fn(request) -> Promise<{ status, headers, body }>`); tests drive the fetch offline. With none, the default `pki.transport.https` is used, which fails closed unless `opts.tls` carries trust.
...
async function build(leaf, opts) {
  ...
  if (opts.fetchAia === true) {
    aiaCtx = {
      transport: opts.transport || httpTransport.https({ E: E, errPrefix: "path" }),
      tls: opts.tls || {},
      timeout: opts.aiaTimeout,               // forwarded; the transport applies its own default + cap
  ...
  while (stack.length && !success) {
    ...
    // An AIA-fallback frame: `current`'s LOCAL (pool) branches were all explored without success, so now --
    // and only now -- fetch its AIA caIssuers issuer(s) and expand (RFC 4158 sec. 7.2 "local before remote":
    // remote discovery is tried only after every local candidate dead-ends). A fetched cert joins the SHARED
    // pool (later branches reuse it, bounded by the ceiling) and is scored + accepted through the exact same
    // validate() gate as a static candidate -- it is never a trust anchor.
    if (frame.aiaFallback) {
      if (aiaCtx && aiaCtx.fetches < aiaCtx.maxFetches) {
        var fetched = await _fetchAiaIssuers(current, aiaCtx);
        var fAki = childAkiKeyId(current);
        var fscored = [];
        for (var fj = 0; fj < fetched.length; fj++) {
          if (pool.length < poolCeiling) pool.push(fetched[fj]);   // reuse across branches; the ceiling bounds growth
          if (nameMatchSoft(fetched[fj].subject.rdns, current.issuer.rdns)) {
            fscored.push({ cand: fetched[fj], score: scoreCandidate(fetched[fj], fAki, anchors, opts.time) });
          }
        }
        considered += _pushCandidates(frame, fscored, stack, counter);
      }
      continue;
    }
    ...
    // Opt-in AIA caIssuers fetch as a FALLBACK, explored only after the local pool branches (RFC 4158 sec. 7.2
    // "local before remote"): push a marker frame for `current` at the BOTTOM of this frame's children (before
    // the scored pool candidates below), so it is popped only once every local subtree has dead-ended. This is
    // lazy -- a build the pool can complete never pops it, so it never fetches; and a stale / wrong-key pool
    // cert that name-matches but fails validation no longer starves the fetch (the marker still waits below).
    // Fetch only when `current` is NOT anchor-adjacent: an issuer that matches a configured anchor is already
    // terminated by the anchor above (validate ran), so fetching past it is wasted work + wasted budget, and
    // burning the budget on such dead ends must never deny a valid sibling path.
    if (aiaCtx && !anchorAdjacent && frame.hop < maxDepth && aiaCtx.fetches < aiaCtx.maxFetches) {
      stack.push({ chain: frame.chain, hop: frame.hop, keys: frame.keys, aiaFallback: true });
    }
