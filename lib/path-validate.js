...
function classifyCrls(parsed) {
  ...
  for (var i = 0; i < parsed.length; i++) {
    ...
    var rec = {
      crl: theCrl,
      crlNumber: (typeof num === "bigint") ? num : null,
    ...
    try {
      ...
      if (typeof n === "bigint" && n >= 0n) { rec.baseCrlNumber = n; rec.mergeable = true; }
    } catch (_e) {
    ...
    check: async function (cert, issuer, ctx) {
      ...
      async function gateCrl(rec) {
        ...
        if (idpExtension) {
          ...
          idpMask = idp.onlySomeReasonsMask;                 // null when onlySomeReasons is absent
      ...
      for (var dj = 0; dj < classified.deltas.length; dj++) {
        ...
        if (!dRec.accounted) sawUnmergedDelta = true;
        var dStatus = scanCrl(dRec.crl);
        if (dStatus === null) continue;
        // A release on a SUPERSEDED delta (one that was mergeable but lost the
        // sec. 5.2.4 @3580 selection to a sibling with a later thisUpdate) does not
        // speak for its scope -- the selected delta does, and it was already
        // evaluated against the base. Only a delta that merged with nothing can
        // leave a release unaccounted for and thereby block a definitive verdict.
        if (dStatus === 8) { if (!dRec.accounted) releasedByUnmergedDelta = true; continue; }
