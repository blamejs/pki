...
function unrecognizedCriticalExtension(cert, isTarget) {
  for (var i = 0; i < cert.extensions.length; i++) {
    ...
    if (!guard.intrinsic.hasOwn(PROCESSED_EXTENSIONS, ext.oid)) return ext.oid;
    if (isTarget && guard.intrinsic.hasOwn(TARGET_UNPROCESSED_IF_CRITICAL, ext.oid)) return ext.oid;
  }
  return null;
}
  ...
  for (var idx = 0; idx < n; idx++) {
    ...
    if (i !== n) {
      ...
      if (checkPurpose && ta.purposes) {
        ...
        if (!guard.intrinsic.hasOwn(ta.purposes, checkPurpose) || ta.purposes[checkPurpose] !== true) {
...
function assertAnchorConstraints(ta, checkPurpose) {
  var d = (checkPurpose && ta && ta.distrustAfter && guard.intrinsic.hasOwn(ta.distrustAfter, checkPurpose))
...
function _pushCandidates(frame, scored, stack, counter) {
  ...
  for (var ci = 0; ci < scored.length; ci++) {
    ...
    if (guard.intrinsic.setHas(frame.keys, candKey)) continue;
    var childKeys = new Set(frame.keys);
    guard.intrinsic.setAdd(childKeys, candKey);
...
async function _fetchAiaIssuers(current, aia) {
  ...
  for (var i = 0; i < d.value.length; i++) {
    ...
    if (guard.intrinsic.setHas(aia.fetchedUrls, u.href) || guard.intrinsic.setHas(seenThisCert, u.href)) continue;   // dedupe (build-wide OR same-cert) on the normalized URL
    ...
    guard.intrinsic.setAdd(seenThisCert, u.href);
    uris.push(u.href);
  }
  var out = [];
  for (var k = 0; k < uris.length; k++) {
    if (guard.intrinsic.setHas(aia.fetchedUrls, uris[k])) continue;   // a same-cert normalization-equal duplicate
    ...
    guard.intrinsic.setAdd(aia.fetchedUrls, uris[k]);   // mark BEFORE the fetch -> fetched at most once, even on failure
    aia.fetches += 1;
    var certs;
...
async function build(leaf, opts) {
  ...
  for (var pdk = 0; pdk < pool.length; pdk++) guard.intrinsic.setAdd(poolDerKeys, certDerKey(pool[pdk]));
    ...
    if (!stack.length) {
      ...
      if (!fb.fetched && aiaCtx && aiaCtx.fetches < aiaCtx.maxFetches) {
        ...
        for (var fj = 0; fj < fetched.length; fj++) {
          ...
          if (guard.intrinsic.setHas(poolDerKeys, fdk)) continue;   // a byte-duplicate (mirror URL / repeating CMS) -> add once, never re-charge the budget/ceiling
          guard.intrinsic.setAdd(poolDerKeys, fdk);
