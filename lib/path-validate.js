...
function emailMatch(constraint, mailbox) {
  ...
  // The host is the addr-spec domain after "@". A conformant certificate
  // rfc822Name is a simple addr-spec with EXACTLY ONE "@" (RFC 5280 §4.2.1.6
  // deprecates the quoted local part, and an addr-spec domain never contains
  // "@"). A mailbox with zero or multiple "@" — e.g. a quoted local part like
  // "a@b"@example.com — is ambiguous: its domain cannot be determined reliably.
  // FAIL CLOSED ("unsupported") rather than guess a host that could slip the
  // constraint; the caller rejects the path (name-constraint-unsupported).
  var firstAt = m.indexOf("@");
  if (firstAt === -1 || firstAt !== m.lastIndexOf("@")) return "unsupported";
  var host = m.slice(firstAt + 1);
...
// Is `host` a fully qualified domain name (as a URI/dNSName host constraint
// requires, RFC 5280 §4.2.1.10)? A dotless single label (localhost), a dotted-
// quad IPv4 literal, or a bracketed/colon IPv6 literal is NOT a FQDN and cannot
// be matched against a domain-suffix constraint.
function isFqdnHost(host) {
  var h = stripTrailingDot(host);
  if (h === "" || h.indexOf(".") === -1) return false;       // empty or single-label (localhost)
  if (h.indexOf(":") !== -1 || h.charAt(0) === "[") return false; // IPv6 literal
...
// A URI with no extractable host authority, or a host that is not a fully
// qualified domain name (an IP literal or a dotless label such as localhost),
// cannot be evaluated against a host constraint — return "unsupported" so the
// caller fails closed rather than letting the name escape a critical constraint.
  ...
  return {
    check: async function (cert, issuer, ctx) {
      ...
      for (var k = 0; k < parsed.length; k++) {
        ...
        // CHANGES since a base CRL (RFC 5280 §5.2.4). Without base/delta
        // combination, a serial absent from the delta does NOT mean "good", so
        // the delta is unusable on its own — regardless of the critical bit
        // (the unhandled-critical check below only sees a critically-marked one).
        var isDelta = false;
        ...
        for (var r = 0; r < theCrl.revokedCertificates.length; r++) {
          ...
          // RFC 5280 §5.3: an entry takes effect as of the date it carries. For a
          // validation instant BEFORE that date — a historical check, e.g. a
          // timestamped signature made before a later revocation — the certificate
          // was not yet revoked, so this entry does not apply at `time`.
          if (entry.revocationDate instanceof Date && entry.revocationDate.getTime() > time.getTime()) continue;
