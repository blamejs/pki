...
function emailMatch(constraint, mailbox) {
  ...
  if (constraint.indexOf("@") !== -1) {
    // Full-mailbox constraint: exact local part + case-insensitive host.
    var cb = splitMailbox(constraint);
    if (cb === "ambiguous" || cb === null || mb === null) return "unsupported";
    return mb[0] === cb[0] && mb[1].toLowerCase() === cb[1].toLowerCase();
  }
  // Host/domain constraint: compare the mailbox host case-insensitively.
  if (mb === null) return "unsupported";          // no host -> cannot determine domain
  var host = mb[1].toLowerCase();
  if (host === "") return "unsupported";
  var c = constraint.toLowerCase();
  ...
  return {
    check: async function (cert, issuer, ctx) {
      ...
      for (var k = 0; k < parsed.length; k++) {
        ...
        // combination it cannot be processed on its own — regardless of the
        // critical bit (the unhandled-critical check below only sees a
        // critically-marked one). Crucially, its mere presence must BLOCK a
        // "good" result: a clean base plus a delta that lists the serial would
        // otherwise read "good" and accept a certificate the delta revokes.
        var isDelta = false;
        for (var dz = 0; dz < theCrl.crlExtensions.length; dz++) {
          if (theCrl.crlExtensions[dz].oid === OID_DELTA_CRL) { isDelta = true; break; }
        }
        if (isDelta) { sawDelta = true; continue; }
