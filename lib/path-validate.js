 ...
 */
// Every option pki.path.validate reads. The anchor option here is `trustAnchor`, SINGULAR --
// and pki.path.build, in this same module, spells it `trustAnchors` and takes a list. A caller
...
async function build(leaf, opts) {
  ...
  // Read the same surface the option check accepted. `Object.keys` here drops a recognized
  // option that arrived on a prototype or non-enumerably. `Object.create({ requireRevocation:
  // true })` passes the check and then reaches no internal validate, so a caller who demanded a
  // determined revocation result gets a valid path without one. The later copy in validateOpts
  // cannot recover what this one never took.
  guard.identifier.readableNames(opts, E, "path/bad-input", "pki.path.build options").forEach(function (k) {
    // Own membership, never `BUILD_ONLY_OPT[k]`. A table read by truthiness answers for every
    // name Object.prototype carries, so a runtime carrying `softFail` made the caller's own
    // `softFail: false` look build-only and dropped it here, and the forwarding pass below then
    // read the inherited `true` off `forwarded`. An explicit refusal to waive became a waiver.
    if (!Object.prototype.hasOwnProperty.call(BUILD_ONLY_OPT, k)) forwarded[k] = opts[k];
  });
  function validateOpts(anchor) {
    var vo = Object.create(null);   // same reason as `forwarded` above
    // The same surface the option check accepted. `Object.keys` reads a narrower one, so an
    // option carried on a prototype, such as `requireRevocation` on a shared defaults object,
    // passes the check as a recognized name and is then dropped here. Each internal validate
    // runs without it and can return a valid path the caller had asked to depend on a
    // determined revocation result. Forwarding less than was validated is worse than refusing
    // the bag.
    guard.identifier.readableNames(forwarded, E, "path/bad-input", "the forwarded options")
      .forEach(function (f) { vo[f] = forwarded[f]; });
