...
function isUniversalSeq(node) {
  return !!node && node.tagClass === "universal" && node.tagNumber === asn1.TAGS.SEQUENCE;
}

function _weakIssuerKey(state) {
  var bits;
  try { bits = _rsaModulusBits(state.workingPublicKey); }
...
function _rsaModulusBits(spkiBytes) {
  if (!spkiBytes || !spkiBytes.length) return null;
  var spki = asn1.decode(spkiBytes);
  if (!isUniversalSeq(spki) || !spki.children || spki.children.length !== 2) return null;
  var algId = spki.children[0];
  if (!isUniversalSeq(algId) || !algId.children || !algId.children.length) return null;
  var oidNode = algId.children[0];
  if (oidNode.tagClass !== "universal" || oidNode.tagNumber !== asn1.TAGS.OBJECT_IDENTIFIER) return null;
  var keyAlg = asn1.read.oid(oidNode);
  if (keyAlg !== OID_RSA_ENCRYPTION && keyAlg !== OID_RSA_PSS) return null;
  var keyBits = asn1.read.bitString(spki.children[1]);
  var inner = asn1.decode(keyBits.bytes);
  if (!isUniversalSeq(inner) || !inner.children || !inner.children.length) return null;
  var modulus = inner.children[0].content;
  var i = 0;
