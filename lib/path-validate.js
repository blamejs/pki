...
function checkedSubtreeSeeds(list, optName) {
  ...
  return list.map(function (st) {
    ...
    var base = (st.tag === 7 && guard.bytes.isByteSource(st.base))
      ...
      : st.base;
    if (st.tag === 4 && base !== null && typeof base === "object" && intrinsic.isArray(base.rdns)) {
      var dnCopy = {};
      intrinsic.defineProperty(dnCopy, "rdns", { value: _copyRdnsDeep(base.rdns), enumerable: true, configurable: true, writable: true });
      base = dnCopy;
    }
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    for (var k = 0; k < _NC_SUBTREE_KEYS.length; k++) {
      ...
      if (field === "base" && fv !== null && typeof fv === "object") {
        if (intrinsic.isArray(fv.rdns)) {
          var dnBase = {};
          intrinsic.defineProperty(dnBase, "rdns", { value: _copyRdnsDeep(fv.rdns), enumerable: true, configurable: true, writable: true });
          fv = dnBase;
