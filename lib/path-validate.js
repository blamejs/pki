...
function checkedSubtreeSeeds(list, optName) {
  ...
  return list.map(function (st) {
    ...
    var base = (st.tag === 7 && guard.bytes.isByteSource(st.base))
      ...
      : st.base;
    if (st.tag === 4 && base !== null && typeof base === "object") {
      var seedRdns = _capturedRdns(base, "validate: opts." + optName);
      if (seedRdns !== undefined) {
        var dnCopy = {};
        intrinsic.defineProperty(dnCopy, "rdns", { value: _copyRdnsDeep(seedRdns), enumerable: true, configurable: true, writable: true });
        base = dnCopy;
      }
    }
...
async function validate(path, opts) {
  ...
  if (anchorList.length > 1) {
    ...
    var seedPermOnce = checkedSubtreeSeeds(opts.initialPermittedSubtrees, "initialPermittedSubtrees");
    var seedExclOnce = checkedSubtreeSeeds(opts.initialExcludedSubtrees, "initialExcludedSubtrees");
    var singleOpts = function (a) {
      var keys = intrinsic.ownKeys(opts);
      var o = {};
      for (var ki = 0; ki < keys.length; ki++) { o[keys[ki]] = opts[keys[ki]]; }
      o.trustAnchors = a;
      if (opts.initialPermittedSubtrees != null) o.initialPermittedSubtrees = seedPermOnce;
      if (opts.initialExcludedSubtrees != null) o.initialExcludedSubtrees = seedExclOnce;
      return o;
    };
  ...
  var seeds = {
    permittedGenerations: [
      checkedSubtreeSeeds(opts.initialPermittedSubtrees, "initialPermittedSubtrees"),
      anchorNc.permitted,
    ],
    excluded: intrinsic.concat(checkedSubtreeSeeds(opts.initialExcludedSubtrees, "initialExcludedSubtrees"), anchorNc.excluded),
  };
...
function _capturedRdns(base, who) {
  ...
  return intrinsic.isArray(d.value) ? d.value : undefined;
}
var _NC_SUBTREE_KEYS = ["tag", "base"];
var _NC_LIST_KEYS = ["permitted", "excluded"];
function _copyAnchorSubtreeList(list, key, who) {
  if (list === undefined || list === null) return undefined;
  if (!intrinsic.isArray(list)) throw E("path/bad-input", who + ": a trustAnchor nameConstraints." + key + " must be an array of { tag, base } subtree entries");
  var copied = [];
  for (var i = 0; i < list.length; i++) {
    ...
    for (var k = 0; k < _NC_SUBTREE_KEYS.length; k++) {
      ...
      if (field === "base" && fv !== null && typeof fv === "object") {
        ...
        if (rdnsOwn !== undefined) {
          var dnBase = {};
          intrinsic.defineProperty(dnBase, "rdns", { value: _copyRdnsDeep(rdnsOwn), enumerable: true, configurable: true, writable: true });
          fv = dnBase;
    ...
    intrinsic.push(copied, out);
  }
  return copied;
}
function _copyAnchorNameConstraints(nc, who) {
  if (typeof nc !== "object" || nc === null || intrinsic.isArray(nc)) {
...
function toAnchor(entry, verb) {
  ...
  try {
    if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && !guard.parsed.isRecorded(entry)) {
      ...
      ncSnap = _captureConstraintMap(entry, "nameConstraints", E, "path/bad-input", who);
  ...
  if (isTuple) {
    ...
    if (ncSnap !== undefined) intrinsic.defineProperty(flat, "nameConstraints", { value: _copyAnchorNameConstraints(ncSnap, who), enumerable: true, configurable: true, writable: true });
...
async function build(leaf, opts) {
  ...
  /** @internal Every candidate is validated in its own pass after an await, so the caller's subtree
   * lists are read once here rather than again for each candidate. */
  if (opts.initialPermittedSubtrees != null) forwarded.initialPermittedSubtrees = checkedSubtreeSeeds(opts.initialPermittedSubtrees, "initialPermittedSubtrees");
  if (opts.initialExcludedSubtrees != null) forwarded.initialExcludedSubtrees = checkedSubtreeSeeds(opts.initialExcludedSubtrees, "initialExcludedSubtrees");
  ...
  if (doValidate && intrinsic.isArray(opts.userInitialPolicySet)) {
    ...
    for (var ui = 0; ui < opts.userInitialPolicySet.length; ui++) {
      ...
      if (!dupUip) intrinsic.push(uipsDistinct, uip);
    }
  ...
  if (direction === "reverse") {
    var rstack = [];
    for (var rsi = 0; rsi < anchors.length; rsi++) intrinsic.push(rstack, { chain: [], hop: 0, keys: new Set([identityKey(leafCert)]), anchor: anchors[rsi] });
    while (rstack.length) {
      ...
      for (var rpi = 0; rpi < pool.length; rpi++) {
        ...
        if (nameMatchSoft(pool[rpi].issuer.rdns, frontierName)) {
          intrinsic.push(rscored, { cand: pool[rpi], score: scoreCandidateReverse(pool[rpi], frontierCert, leafCert, opts.time) });
        }
      }
