...
async function build(leaf, opts) {
  ...
  if (opts.fetchAia === true) {
    ...
    if (opts.aiaTimeout !== undefined && (typeof opts.aiaTimeout !== "number" || !isFinite(opts.aiaTimeout) || opts.aiaTimeout < 1)) {
      throw E("path/bad-input", "build: opts.aiaTimeout must be a positive number of milliseconds");
    }
    aiaCtx = {
  ...
  // Drained DEEPEST-first (LIFO): a frame is always deferred AFTER its ancestor (the ancestor is processed first),
  // so popping the tail drains a descendant before its ancestor -- the actual dead end (the missing higher hop) is
  // fetched before an ancestor whose issuer the pool ALREADY supplies, so a scarce budget is never spent
  // re-retrieving a locally-resolved hop.
  var deferredAia = [];
  while ((stack.length || deferredAia.length) && !success) {
    if (!stack.length) {
      // The complete local search has failed without a path. Begin the fetch phase: drain ONE deferred frame,
      // fetch its AIA caIssuers issuer(s), and push them back for a fresh LOCAL exploration (which may itself
      // defer further fetches, drained only after it too is exhausted). A fetched cert joins the SHARED pool and
      // is scored + accepted through the exact same validate() gate as a static candidate -- never a trust anchor.
      var fb = deferredAia.pop();   // deepest-first: a descendant frame drains before its ancestor (see the queue note above)
      var fbCur = fb.chain[0];
      if (aiaCtx && aiaCtx.fetches < aiaCtx.maxFetches) {
        var fetched = await _fetchAiaIssuers(fbCur, aiaCtx);   // append newly-fetched issuer(s) to the SHARED pool (deduped, ceiling-bounded)
        for (var fj = 0; fj < fetched.length; fj++) if (pool.length < poolCeiling) pool.push(fetched[fj]);
      }
      // Re-expand fb against every pool cert added SINCE it was deferred -- its OWN just-fetched certs AND any a
      // SIBLING branch fetched into the shared pool. Scoring only fb's own fetched set would miss a sibling-fetched
      // issuer when fb's caIssuers URL was deduped (already fetched) and returned nothing new; the pool-index mark
      // skips the static pool fb already expanded against in its local phase (no redundant re-exploration). This
      // runs even when the fetch budget is exhausted, so a budget-capped frame still benefits from a sibling fetch.
    ...
    if (aiaCtx && frame.hop < maxDepth && aiaCtx.fetches < aiaCtx.maxFetches) {
      ...
      deferredAia.push({ chain: frame.chain, hop: frame.hop, keys: frame.keys, poolMark: pool.length });
    }
