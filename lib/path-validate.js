...
// (CVE-2015-9235).
var SIG_ALGS = {};
function _sig(name, verify, imp, ecdsa) {
  var entry = { verify: verify, imp: imp };
  if (ecdsa) entry.ecdsa = true;
  SIG_ALGS[oid.byName(name)] = entry;
}
// RSASSA-PKCS1-v1_5
_sig("sha256WithRSAEncryption", { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" });
_sig("sha384WithRSAEncryption", { name: "RSASSA-PKCS1-v1_5", hash: "SHA-384" }, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-384" });
_sig("sha512WithRSAEncryption", { name: "RSASSA-PKCS1-v1_5", hash: "SHA-512" }, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-512" });
// ECDSA (hash in the OID; the curve comes from the imported key)
_sig("ecdsaWithSHA256", { name: "ECDSA", hash: "SHA-256" }, { name: "ECDSA" }, true);
_sig("ecdsaWithSHA384", { name: "ECDSA", hash: "SHA-384" }, { name: "ECDSA" }, true);
_sig("ecdsaWithSHA512", { name: "ECDSA", hash: "SHA-512" }, { name: "ECDSA" }, true);
// EdDSA (one-shot, no hash parameter)
_sig("Ed25519", { name: "Ed25519" }, { name: "Ed25519" });
_sig("Ed448", { name: "Ed448" }, { name: "Ed448" });
// ML-DSA (FIPS 204)
_sig("id-ml-dsa-44", { name: "ML-DSA-44" }, { name: "ML-DSA-44" });
_sig("id-ml-dsa-65", { name: "ML-DSA-65" }, { name: "ML-DSA-65" });
_sig("id-ml-dsa-87", { name: "ML-DSA-87" }, { name: "ML-DSA-87" });
...
HASH_BY_OID[oid.byName("sha1")] = "SHA-1";
...
function resolveRsaPss(paramsBytes) {
  ...
  //   saltLength [2] INTEGER DEFAULT 20, ... }. Absent params default to SHA-1.
  var hash = "SHA-1", saltLength = 20;
  if (paramsBytes) {
    ...
    if (n.tagClass === "universal" && n.tagNumber === asn1.TAGS.SEQUENCE && n.children) {
      n.children.forEach(function (f) {
        if (f.tagClass !== "context" || !f.children) return;
        if (f.tagNumber === 0) {
          // hashAlgorithm [0] EXPLICIT wraps a HashAlgorithm AlgorithmIdentifier
          // SEQUENCE { algorithm OID, ... } (RFC 4055 3.1) — the OID is one
          // level deeper than the [0] wrapper.
          var inner = f.children[0];
          var oidNode = (inner && inner.children && inner.children.length) ? inner.children[0] : inner;
          var h = asn1.read.oid(oidNode);
          // A present-but-unrecognized hash OID must be REJECTED, never left at
          // the SHA-1 default — else a cert can declare an unsupported hash and
          // be verified under SHA-1 (a signatureAlgorithm bypass).
          if (!HASH_BY_OID[h]) throw E("path/unsupported-algorithm", "unsupported RSASSA-PSS hash algorithm " + h);
          hash = HASH_BY_OID[h];
        } else if (f.tagNumber === 2) {
          saltLength = Number(asn1.read.integer(f.children[0]));
        }
      });
    }
  }
...
function hostConstraintMatch(constraint, host) {
  var c = constraint.toLowerCase(), h = host.toLowerCase();
  if (c === "") return true;
...
// form? Only the forms the profile requires/supports are compared.
...
function certNameForms(cert) {
  ...
  if (san) {
    san.value.names.forEach(function (nm) {
      if (nm.value !== null && nm.value !== undefined) forms.push({ tag: nm.tagNumber, value: nm.value });
    });
  }
...
function checkNameConstraints(state, cert) {
  ...
  // Excluded: any match -> reject.
  for (var e = 0; e < state.excludedSubtrees.length; e++) {
    var ex = state.excludedSubtrees[e];
    for (var i = 0; i < forms.length; i++) {
      if (nameMatchesConstraint(forms[i].tag, forms[i].value, { tagNumber: ex.tag, value: ex.base }) === true) {
        return { ok: false, code: "path/name-constraint-excluded" };
      }
    }
  }
  // Permitted: for each name form, every generation that constrains that form
  // must admit it (intersection across generations).
  for (var f = 0; f < forms.length; f++) {
    ...
    for (var g = 0; g < state.permittedGenerations.length; g++) {
      ...
      var permitted = formSubtrees.some(function (s) {
        return nameMatchesConstraint(nf.tag, nf.value, { tagNumber: s.tag, value: s.base }) === true;
      });
      if (!permitted) return { ok: false, code: "path/name-constraint-not-permitted" };
    }
  }
  return { ok: true };
}
...
function selfIssued(cert) { return dnEqual(cert.subject.rdns, cert.issuer.rdns); }
...
function processPolicies(state, cert, i, checks) {
  ...
  if (cp && state.validPolicyTree) {
    ...
    if (anyPolicyPresent && (state.inhibitAnyPolicy > 0 || (i < state._n && selfIssued(cert)))) {
...
function applyPolicyMappings(state, mappings, i) {
  ...
  if (state.policyMapping > 0) {
    ...
    leavesAt(state.validPolicyTree, depth).forEach(function (node) {
      if (mappedFrom[node.validPolicy]) node.expectedPolicySet = mappedFrom[node.validPolicy].slice();
    });
  } else {
...
async function validate(path, opts) {
  ...
  for (var idx = 0; idx < n; idx++) {
    ...
    if (revocationChecker) {
      ...
      if (rv.status === "revoked") { checks.push({ name: "revocation", ok: false, code: "path/revoked" }); failed = true; }
      else if (rv.status === "unknown" && !softFail) { checks.push({ name: "revocation", ok: false, code: "path/revocation-undetermined" }); failed = true; }
      else checks.push({ name: "revocation", ok: true });
    }
...
function decodeIdp(ext) {
  ...
  var out = { hasDistributionPoint: false, onlyUser: false, onlyCa: false, onlySomeReasons: null, indirect: false, onlyAttr: false };
  var n = asn1.decode(ext.value);
  if (n.tagClass !== "universal" || n.tagNumber !== asn1.TAGS.SEQUENCE || !n.children) return out;
  n.children.forEach(function (f) {
    if (f.tagClass !== "context") return;
    if (f.tagNumber === 0) out.hasDistributionPoint = true;   // scoped to a specific DP
    else if (f.tagNumber === 1 && f.content && f.content[0] === 0xff) out.onlyUser = true;
    else if (f.tagNumber === 2 && f.content && f.content[0] === 0xff) out.onlyCa = true;
    else if (f.tagNumber === 3) out.onlySomeReasons = true;
    else if (f.tagNumber === 4 && f.content && f.content[0] === 0xff) out.indirect = true;
    else if (f.tagNumber === 5 && f.content && f.content[0] === 0xff) out.onlyAttr = true;
  });
  return out;
}
  ...
  return {
    check: async function (cert, issuer, ctx) {
      ...
      for (var k = 0; k < parsed.length; k++) {
        ...
        if (theCrl.nextUpdate && theCrl.nextUpdate < time) continue;   // stale
        ...
        for (var r = 0; r < theCrl.revokedCertificates.length; r++) {
          if (theCrl.revokedCertificates[r].serialNumberHex === cert.serialNumberHex) {
