...
function toAnchor(entry, verb) {
  ...
  try {
    ...
    if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && !guard.parsed.isRecorded(entry)) {
      ...
      // candidate and read its fields by PROPERTY ACCESS (follows the prototype chain, so Object.create(base)
      // is recognized), each exactly once. entry's own shape is NEVER enumerated (no `in` / ownKeys /
      // getOwnPropertyDescriptor) and the prototype chain is never walked, so a Proxy anchor cannot use an
      // enumeration trap to mutate state; each field is one plain `get` a hostile accessor answers at most once.
      // (Buffer.isBuffer / isRecorded above invoke getPrototypeOf a bounded number of times; the chain is never
      // WALKED, so a hostile getPrototypeOf cannot be driven into a loop.)
      //
      ...
      // publicKey is read and pinned to a private Buffer IMMEDIATELY, before the name / algorithm accessors run,
      // so a later caller getter cannot overwrite the key Buffer's bytes after the field is read but before it
      // is captured. The declared-vs-SPKI check and initialize()'s state.workingPublicKey both use the copy. A
      // non-tuple leaves pk null / isTuple false and falls through to coerceCert below.
