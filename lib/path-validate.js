...
function scoreCandidate(cand, childAki, anchors, time) {
  ...
  for (var i = 0; i < anchors.length; i++) {
    if (nameMatchSoft(cand.issuer.rdns, anchors[i].name.rdns)) { score += 100; bestPrefix = 0; break; }
    var p = rdnPrefixLength(cand.issuer.rdns, anchors[i].name.rdns);
    if (p > bestPrefix) bestPrefix = p;
  }
  score += bestPrefix * RDN_PREFIX_WEIGHT;
  return score;
}
...
function scoreCandidateReverse(cand, parentCert, leafCert, time) {
  ...
  else score += rdnPrefixLength(cand.subject.rdns, leafCert.issuer.rdns) * RDN_PREFIX_WEIGHT;
  return score;
}
