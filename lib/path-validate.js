...
async function validate(path, opts) {
  ...
  if (anchorList.length > 1) {
    ...
    var seedPermOnce = checkedSubtreeSeeds(permRawOnce, "initialPermittedSubtrees");
    var seedExclOnce = checkedSubtreeSeeds(exclRawOnce, "initialExcludedSubtrees");
    var singleOpts = function (a) {
      var keys = intrinsic.ownKeys(opts);
      var o = {};
      for (var ki = 0; ki < keys.length; ki++) { o[keys[ki]] = opts[keys[ki]]; }
      o.trustAnchors = a;
      if (permRawOnce != null) o.initialPermittedSubtrees = seedPermOnce;
      if (exclRawOnce != null) o.initialExcludedSubtrees = seedExclOnce;
      return o;
    };
...
async function build(leaf, opts) {
  ...
  if (buildPermRaw != null) forwarded.initialPermittedSubtrees = checkedSubtreeSeeds(buildPermRaw, "initialPermittedSubtrees");
  if (buildExclRaw != null) forwarded.initialExcludedSubtrees = checkedSubtreeSeeds(buildExclRaw, "initialExcludedSubtrees");
