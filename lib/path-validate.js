...
async function build(leaf, opts) {
  ...
  while ((stack.length || deferredAia.length) && !success) {
    if (!stack.length) {
      ...
      // Select the deepest frame; among equal depth, the earliest-deferred (highest DFS priority). See the queue note.
      var _bi = 0;
      for (var _di = 1; _di < deferredAia.length; _di++) {
        var _c = deferredAia[_di], _bf = deferredAia[_bi];
        if (_c.hop > _bf.hop || (_c.hop === _bf.hop && _c.seq < _bf.seq)) _bi = _di;
      }
      var fb = deferredAia.splice(_bi, 1)[0];
    ...
    if (aiaCtx && frame.hop < maxDepth && aiaCtx.fetches < aiaCtx.maxFetches) {
      ...
      deferredAia.push({ chain: frame.chain, hop: frame.hop, keys: frame.keys, poolMark: pool.length, seq: deferSeq });
      deferSeq += 1;
    }
