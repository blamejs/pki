...
function toAnchor(entry, verb) {
  ...
  if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && entry.name && entry.publicKey && entry.algorithm) {
    ...
    // accepted -- but an object MUST carry a canonical OID string. Built-in verification derives the key
    // algorithm from the (valid) publicKey SPKI, so an algorithm object with no `oid` would validate the
    // path anyway and leave the very fail-open this check exists to close. A number, an object without an
    // oid, or another non-shape is a malformed tuple and fails closed; a MISSING algorithm skips this
    // branch and is caught below as not-a-certificate.
    var alg = entry.algorithm;
    var algOk = typeof alg === "string" || (alg && typeof alg === "object" && typeof alg.oid === "string");
    if (!intrinsic.isArray(entry.name.rdns) || !Buffer.isBuffer(entry.publicKey) || !algOk) {
