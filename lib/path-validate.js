...
// The OID of an AlgorithmIdentifier node (a SEQUENCE { algorithm OID, params }),
// tolerating an EXPLICIT [n] wrapper one level up.
function algIdOid(node) {
  var inner = (node && node.children && node.children.length) ? node.children[0] : node;
  var seq = (inner && inner.children && inner.children.length) ? inner : node;
...
function resolveRsaPss(paramsBytes) {
  ...
  var hash = "SHA-1", saltLength = 20, mgfNode = null, trailer = 1;
  if (paramsBytes) {
