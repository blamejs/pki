...
function toAnchor(entry, verb) {
  ...
  var isTuple = false, alg = null, pubRaw = null, nameRaw = null;
  try {
  ...
  if (isTuple) {
    ...
    //
    // Pin the caller's publicKey to ONE private Buffer FIRST: the check here and the anchor key initialize()
    // binds (state.workingPublicKey) MUST be the same bytes. Read raw twice, a caller's publicKey getter
    // could answer the check with a matching SPKI and hand verification a different key -- the lying-buffer
    // TOCTOU the guard.bytes contract exists to close.
    var pk = guard.bytes.snapshot(pubRaw, PathError, "path/bad-input", who + ": trustAnchor publicKey");
