...
function toAnchor(entry, verb) {
  ...
  if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && entry.name && entry.publicKey && entry.algorithm) {
    ...
    if (!algIsObject) return entry;
    // Normalize the { oid, parameters } object form to the flat shape the certificate path yields WITHOUT
    // discarding any of the anchor's other properties: inherit from entry so every field validation reads
    // (purposes, distrustAfter and any inherited/non-enumerable constraint metadata) still resolves, and
    // shadow only the algorithm (a string OID -- an object never equals a cert's keyAlg.oid and would
    // wrongly clear an EC child's inherited parameters) and the parameters the object nests.
    var flat = intrinsic.create(entry);
    // Define the overrides as OWN data properties: a plain assignment travels through any inherited
    // descriptor, so a frozen/sealed anchor or one exposing `algorithm` via a getter would make it throw
    // (strict) or silently no-op, leaving the object algorithm in place. defineProperty shadows regardless.
    intrinsic.defineProperty(flat, "algorithm", { value: algStr, enumerable: true, configurable: true, writable: true });
    if (alg.parameters !== undefined) intrinsic.defineProperty(flat, "parameters", { value: alg.parameters, enumerable: true, configurable: true, writable: true });
    return flat;
  }
  var cert;
