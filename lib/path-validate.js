...
function _snapshotConstraintMap(m, E, code, who) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    intrinsic.defineProperty(out, keys[i], { value: v, enumerable: d.enumerable, configurable: true, writable: true });
  }
  return out;
}
...
function toAnchor(entry, verb) {
  ...
  try {
    ...
    if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && !guard.parsed.isRecorded(entry)) {
      ...
      purposesSnap = (pRaw !== null && typeof pRaw === "object" && !intrinsic.isBuffer(pRaw)) ? _snapshotConstraintMap(pRaw, E, "path/bad-input", who) : pRaw;
      var dRaw = _captureConstraintMap(entry, "distrustAfter", E, "path/bad-input", who);
      distrustSnap = (dRaw !== null && typeof dRaw === "object" && !intrinsic.isBuffer(dRaw)) ? _snapshotConstraintMap(dRaw, E, "path/bad-input", who) : dRaw;
