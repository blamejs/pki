 ...
 * `revocationChecked` answers separately: `false` when no `revocationChecker`
 * was supplied, `"determined"` when every certificate got an explicit answer,
 * and `"waived"` when `softFail` turned an undetermined one into a pass. The
 ...
 * that THROWS is reported as `status: "error"` carrying the fault, never
 * laundered into an unknown status.
 *
...
async function validate(path, opts) {
  ...
  var revocationRan = false, revocationWaived = false;
  ...
  for (var idx = 0; idx < n; idx++) {
    ...
    if (revocationChecker) {
      ...
      var rv, rvError = null;
      // A checker that THROWS is not a checker reporting "unknown". Laundering the two together
      // made a broken checker indistinguishable from a working one that could not reach the
      // responder -- and under softFail both became a pass. The fault is carried onto the check so
      // an operator can tell their own bug from a network condition.
      ...
      if (rv && rv.status === "good") { checks.push({ name: "revocation", ok: true, status: "good" }); }
        ...
        var waived = { name: "revocation", ok: true, status: rvStatus, waived: true };
        if (rvError) waived.error = rvError;
        checks.push(waived);
        revocationWaived = true;
      } else {
        var undet = { name: "revocation", ok: false, status: rvStatus, code: "path/revocation-undetermined" };
        if (rvError) undet.error = rvError;
        checks.push(undet);
        failed = true;
      }
      revocationRan = true;
  ...
  return {
    ...
    revocationChecked: revocationRan ? (revocationWaived ? "waived" : "determined") : false,
    anchorConstraints: {
