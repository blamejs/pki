...
var oid = require("./oid");
var ct = require("./ct");
var identity = require("./identity-match");
...
/** @internal Captured at load: `pki.shbs` is the same module object the public surface exposes, and its
 * `verify` is a writable property, so reading it when a signature is checked would let whoever wrote it
 * last decide the verdict. */
var _shbsHssVerify = require("./shbs").verify;
...
function pathCode(e, fallback) {
  if (!e || typeof e.code !== "string") return fallback;
  if (e.code.indexOf("path/") === 0) return e.code;
  if (e.code === "webcrypto/not-supported" || e.code === "webcrypto/data") return "path/unsupported-algorithm";
  return fallback;
}
...
var OID = intrinsic.assign(intrinsic.create(null), {
...
var PROCESSED_EXTENSIONS = intrinsic.create(null);
...
/** @internal RFC 9802: a stateful hash-based signature over the whole tbsCertificate rather than a digest
 * of it (sec. 7.1), with the RFC 8554 public key carried directly in the subjectPublicKey BIT STRING and the
 * signature directly in signatureValue, neither with any further ASN.1 wrapping (sec. 5, sec. 6). It verifies
 * through the toolkit's own engine rather than the crypto engine, so the descriptor names that route instead
 * of an import. Verification only: signing a stateful key belongs in hardware that owns the state. */
_sig("id-alg-hss-lms-hashsig", { name: "HSS-LMS" }, null, "absent", false, true).shbs = true;

var OID_RSA_PSS = oid.byName("rsassaPss");
var OID_RSA_ENCRYPTION = oid.byName("rsaEncryption");
var OID_MGF1 = oid.byName("mgf1");
var HASH_BY_OID = intrinsic.create(null);
...
function isDerNull(p) { return guard.der.isNull(p); }
...
var STATEFUL_HBS_OIDS = intrinsic.create(null);
["id-alg-hss-lms-hashsig", "id-alg-xmss-hashsig", "id-alg-xmssmt-hashsig"].forEach(function (n) { STATEFUL_HBS_OIDS[oid.byName(n)] = true; });

/** @internal RFC 9802 sec. 6 states the rule twice, and the two differ: a certification authority's
 * certificate may assert keyCertSign, an end entity's may not. Both must assert at least one of the values
 * their list names, and neither may assert any value outside it. An absent extension is unrestricted, as
 * RFC 5280 sec. 4.2.1.3 has it. */
/** @internal Whether the certificate states that it is a certification authority. An extension that does
 * not decode leaves the stricter end-entity rule in force. */
function _isCaCert(cert) {
  var bc;
  try { bc = decodeExt(cert, OID.basicConstraints); }
  // allow:swallow-unverified a basicConstraints that cannot be read is refused by its own check
  catch (_e) { return false; }
  return !!(bc && bc.value && bc.value.cA === true);
}

function statefulHbsKeyUsageCheck(cert, isCa) {
  var ku;
  try { ku = decodeExt(cert, OID.keyUsage); }
  catch (e) { return { ok: false, code: "path/hbs-key-usage", error: e }; }
  if (!ku) return { ok: true };
  var v = ku.value;
  var where = isCa ? "a certification authority" : "an end entity";
  var allowed = isCa ? "digitalSignature, nonRepudiation, keyCertSign or cRLSign"
    : "digitalSignature, nonRepudiation or cRLSign";
  var asserted = v.digitalSignature || v.nonRepudiation || v.cRLSign || v.keyCertSign;
  if (!asserted) {
    return { ok: false, code: "path/hbs-key-usage",
      error: E("path/hbs-key-usage", "a stateful hash-based key in " + where + " certificate must assert at least one of " + allowed + " (RFC 9802 sec. 6)") };
  }
  var forbidden = v.keyEncipherment || v.dataEncipherment || v.keyAgreement || v.encipherOnly ||
    v.decipherOnly || (!isCa && v.keyCertSign) || (v.reservedBitsSet === true);
  if (forbidden) {
    return { ok: false, code: "path/hbs-key-usage",
      error: E("path/hbs-key-usage", "a stateful hash-based key in " + where + " certificate must assert no keyUsage value outside " + allowed + " (RFC 9802 sec. 6)") };
  }
  return { ok: true };
}

var ML_KEM_OIDS = intrinsic.create(null);
...
/** @internal The RFC 8554 public key the SubjectPublicKeyInfo carries, and the verdict on a signature over
 * `message`. A key or signature the engine cannot read is a refusal, never a pass. */
function _shbsVerify(spkiBytes, sigBytes, message) {
  var rawKey;
  /** @internal An RFC 8554 public key is a whole number of octets, and the codec accepts a BIT STRING that
   * declares unused bits as long as they are zero, so the same key admits a second encoding unless the
   * count is checked. The signature is held to the same rule by its caller. */
  try {
    var bits = asn1.read.bitString(asn1.decode(spkiBytes).children[1]);
    if (!guard.crypto.isOctetAligned(bits)) {
      return guard.verdict.of({ ok: false, code: "path/bad-signature",
        error: E("path/bad-signature", "an HSS subjectPublicKey must be a whole number of octets, and this one declares " + bits.unusedBits + " unused bits") });
    }
    rawKey = bits.bytes;
  } catch (e) { return guard.verdict.of({ ok: false, code: pathCode(e, "path/bad-signature"), error: e }); }
  try {
    return guard.verdict.of({ ok: _shbsHssVerify(rawKey, message, sigBytes) === true });
  } catch (e) { return guard.verdict.of({ ok: false, code: pathCode(e, "path/bad-signature"), error: e }); }
}

function _weakIssuerKey(state) {
  var spkiBytes = state.workingPublicKey;
  var isRsa;
  try { isRsa = _issuerKeyIsRsa(spkiBytes); }
  catch (_e) { return null; }
  if (!isRsa) return null;
  var bits;
  try { bits = _rsaModulusBits(spkiBytes); }
  catch (_e) { bits = null; }
  if (bits === null) {
    return E("path/weak-key",
      "the issuer's key names an RSA algorithm and its modulus cannot be read, so its strength cannot be established");
  }
  var floor = state.minRsaModulusBits || constants.LIMITS.RSA_MIN_MODULUS_BITS;
  if (bits >= floor) return null;
  return E("path/weak-key", "the issuer's RSA modulus is " + bits + " bits, below the " + floor + "-bit minimum");
}

function _issuerKeyIsRsa(spkiBytes) {
  if (!spkiBytes || !spkiBytes.length) return false;
  var kids = pkix.rootSequenceChildren(asn1.decode(spkiBytes), 2, 2);
  if (!kids) return false;
  var algKids = pkix.rootSequenceChildren(kids[0], 1);
  if (!algKids) return false;
  var keyAlg = asn1.read.oid(algKids[0]);
  return keyAlg === OID_RSA_ENCRYPTION || keyAlg === OID_RSA_PSS;
}

function _rsaModulusBits(spkiBytes) {
  var spki = asn1.decode(spkiBytes);
  var keyBits = asn1.read.bitString(spki.children[1]);
  var innerKids = pkix.rootSequenceChildren(asn1.decode(keyBits.bytes), 1);
  if (!innerKids) return null;
  var modulus = innerKids[0].content;
  if (!modulus || !modulus.length) return null;
  var i = 0;
  while (i < modulus.length && modulus[i] === 0) i += 1;
  if (i >= modulus.length) return 0;
  var top = modulus[i], topBits = 0;
  while (top > 0) { topBits += 1; top >>= 1; }
  return (modulus.length - i - 1) * 8 + topBits;
}
...
function builtinVerify(state, cert) {
  ...
  /** @internal Resolving reads `then` off the value, so a synchronous verdict is built with an own one:
   * an inherited accessor would otherwise run with the verdict as its receiver and hand the caller a
   * different object (see guard-verdict). */
  } catch (e) { return _resolve(Promise, guard.verdict.of({ ok: false, code: pathCode(e, "path/unsupported-algorithm"), error: e })); }
  if (!guard.crypto.isOctetAligned(cert.signatureValue)) return _resolve(Promise, guard.verdict.of({ ok: false, code: "path/bad-signature" }));
  ...
  if (d.shbs) return _resolve(Promise, _shbsVerify(state.workingPublicKey, cert.signatureValue.bytes, cert.tbsBytes));
  var key;
...
var splitMailbox = guard.name.splitMailbox;
...
var stripTrailingDot = guard.name.stripTrailingDot;

/** @internal `reachIntoBase` asks the excluded question rather than the permitted one. A wildcard
 * dNSName stands for every name one label below its parent, so it is a set rather than a name. A
 * permitted subtree asks whether that whole set stays inside the base, which the literal comparison
 * already answers: `*.foo.com` ends with `.foo.com`, so a base of `foo.com` covers all of it. An
 * excluded subtree asks the other question, whether any member of the set falls inside the base, and
 * the literal comparison answers it in the fail-open direction: `*.example.com` is not the string
 * `bar.example.com`, so an exclusion of `bar.example.com` admitted a certificate that presents
 * exactly the name the exclusion forbids. Widening this for the permitted direction would be the
 * fail-open mirror, so it is asked for only where the caller wants reach. A wildcard reaches exactly
 * one label, so a base below the wildcard's parent is reached only when it is one label below it. */
function hostConstraintMatch(constraint, host, reachIntoBase) {
  ...
  if (h === c || (h.length > c.length && h.slice(-(c.length + 1)) === "." + c)) return true;
  if (reachIntoBase === true && h.length > 2 && _charCodeAt(h, 0) === 42 && _charCodeAt(h, 1) === 46) {
    var parent = _strSlice(h, 2);
    if (parent.length > 0 && c.length > parent.length && _strSlice(c, -(parent.length + 1)) === "." + parent) {
      return _strIndexOf(_strSlice(c, 0, c.length - parent.length - 1), ".") === -1;
    }
  }
  return false;
}
...
var isFqdnHost = guard.name.isFqdnHost;

/** @internal `URL.hostname` keeps an IPv6 literal's brackets, and `net.isIP` reads the address
 * without them. */
...
/** @internal The host a `uniformResourceIdentifier` constraint compares against. A URI written
 * without an authority has no host, and RFC 5280 sec. 4.2.1.10 constrains a host, so the caller
 * reads that as `unsupported` rather than as a name. The brackets an IPv6 literal arrives in are
 * kept, so the value stays the one the certificate carries and `isFqdnHost` refuses it. */
function uriHost(uri) {
  var p = guard.name.uriParts(uri);
  if (p === null || !p.hasAuthority || p.host === null) return null;
  return p.bracketed ? "[" + p.host + "]" : p.host;
}
...
/** @internal The one place a constraint base meets a name, so the base's form is read here and
 * nowhere else. A caller's base met `guard.name.constraintBaseRefusal` at the door; a base a
 * certificate carries in its own nameConstraints extension met no door, and the same rule decides it
 * here. Answering a base outside its tag's form with no match rather than no verdict would make an
 * excluded subtree that covers nothing exclude nothing. */
function nameMatchesConstraint(gnTag, gnValue, base, reachIntoBase) {
  if (base.tagNumber !== gnTag) return null;
  if (gnTag === 1 || gnTag === 2 || gnTag === 6) {
    if (guard.name.constraintBaseUnreadable(gnTag, base.value)) return "unsupported";
  }
  switch (gnTag) {
    case 1: return emailMatch(base.value, gnValue);
    case 2: return hostConstraintMatch(base.value, gnValue, reachIntoBase);
...
function checkNameConstraints(state, cert) {
  ...
  for (var e = 0; e < state.excludedSubtrees.length; e++) {
    ...
    for (var i = 0; i < forms.length; i++) {
      var m = nameMatchesConstraint(forms[i].tag, forms[i].value, { tagNumber: ex.tag, value: ex.base }, true);
...
/**
 * @internal The RFC 9618 valid_policy_graph. RFC 5280 sec. 6.1.3 step (d)(1)(i) creates one child per
 * matching parent, so a chain where several issuer policies map to one subject policy multiplies the
 * structure at every level: measured on a conforming chain carrying four policies mapped to each
 * other, the tree wants 5461 nodes at six certificates against this graph's 25, and the node cap
 * refuses the path. RFC 9618 sec. 5.2 keeps at most ONE node per (depth, policy identifier) and
 * gives it several parents instead, which holds the same information in linear size.
 *
 * `state.policyIndex[depth]` maps a policy identifier to the one node at that depth, so the "is there
 * already a node here for this policy" question every step asks is a lookup rather than a walk.
 */
function graphRoot() {
  return { depth: 0, validPolicy: OID.anyPolicy, qualifierSet: [], expectedPolicySet: [OID.anyPolicy], children: [], parents: [] };
}
function policyLayer(state, depth) {
  var layer = state.policyIndex[depth];
  if (layer === undefined) { layer = intrinsic.create(null); state.policyIndex[depth] = layer; }
  return layer;
}
function nodeAt(state, depth, policyOid) {
  var layer = state.policyIndex[depth];
  if (layer === undefined || !intrinsic.hasOwn(layer, policyOid)) return null;
  return layer[policyOid];
}
function nodesAt(state, depth) {
  var layer = state.policyIndex[depth];
  if (layer === undefined) return [];
  var out = [];
  intrinsic.forEach(intrinsic.keys(layer), function (k) { guard.list.append(out, layer[k]); });
  return out;
}
/** @internal One node per (depth, policy). Meeting the same pair twice joins the parents onto the
 *  node that is already there rather than making a second one, which is the invariant the wrap-up
 *  procedure's single-parent test depends on. */
function addPolicyNode(state, depth, validPolicy, qualifiers, expectedPolicySet, parents) {
  if (parents.length === 0) return null;
  var node = nodeAt(state, depth, validPolicy);
  if (node === null) {
    if (state.policyNodeCount >= state.maxPolicyNodes) { state._capHit = true; return null; }
    node = { depth: depth, validPolicy: validPolicy, qualifierSet: qualifiers ? [qualifiers] : [],
      expectedPolicySet: expectedPolicySet, children: [], parents: [] };
    policyLayer(state, depth)[validPolicy] = node;
    state.policyNodeCount++;
  }
  for (var i = 0; i < parents.length; i++) {
    if (node.parents.indexOf(parents[i]) !== -1) continue;
    guard.list.append(node.parents, parents[i]);
    guard.list.append(parents[i].children, node);
  }
  return node;
}
function removePolicyNode(state, node) {
  node.parents.forEach(function (p) {
    var idx = p.children.indexOf(node);
    if (idx !== -1) p.children.splice(idx, 1);
  });
  node.children.forEach(function (c) {
    var idx = c.parents.indexOf(node);
    if (idx !== -1) c.parents.splice(idx, 1);
  });
  node.parents = [];
  var layer = state.policyIndex[node.depth];
  if (layer !== undefined && layer[node.validPolicy] === node) delete layer[node.validPolicy];
  state.policyNodeCount--;
}
/** @internal RFC 9618 sec. 5.3 step (d)(3): delete every node of depth `depth` or less that has no
 *  children, and repeat, since deleting one can leave its own parents childless. Depth zero is the
 *  root, which is never deleted: an empty graph is a root with no children, which the caller reads
 *  as NULL. */
function prunePolicyChildless(state, depth) {
  for (var d = depth; d > 0; d--) {
    nodesAt(state, d).forEach(function (node) {
      if (node.children.length === 0) removePolicyNode(state, node);
    });
  }
}
function graphIsEmpty(state) { return state.validPolicyGraph.children.length === 0; }
/**
 * @internal The graph a caller reads, as a flat list of nodes linked by index. A graph is not a
 * tree, and handing back a root to walk would invite the caller to recurse into every child and
 * rebuild the exponential structure this replaced: `JSON.stringify` on such a value re-expands a
 * shared node once per path that reaches it. A node list is linear however it is read, holds the
 * parent relation the wrap-up procedure needs, and carries no cycle to serialize. The root is
 * index 0.
 */
function graphSnapshot(root) {
  if (!root) return null;
  var originals = [];
  var indexOfNode = new intrinsic.Map();
  (function walk(node) {
    if (intrinsic.mapHas(indexOfNode, node)) return;
    intrinsic.mapSet(indexOfNode, node, originals.length);
    guard.list.append(originals, node);
    node.children.forEach(walk);
  })(root);
  function idsOf(list) {
    var out = [];
    for (var j = 0; j < list.length; j++) {
      if (intrinsic.mapHas(indexOfNode, list[j])) guard.list.append(out, intrinsic.mapGet(indexOfNode, list[j]));
    }
    return out;
  }
  var nodes = [];
  for (var i = 0; i < originals.length; i++) {
    var o = originals[i];
    guard.list.append(nodes, {
      id: i, depth: o.depth, validPolicy: o.validPolicy, qualifierSet: o.qualifierSet,
      expectedPolicySet: o.expectedPolicySet, parents: idsOf(o.parents), children: idsOf(o.children),
    });
  }
  return { nodes: nodes };
}
...
function isSubtreeBaseValid(tag, base) {
  switch (tag) {
    case 1: case 2: case 6: return guard.name.constraintBaseRefusal(tag, base) === null;
...
/**
 * @internal An RFC 5914 trust anchor's `certPath` constraints, read off the anchor `pki.trust`
 * produced. Absent on an anchor from any other source, so every reader below sees empty lists and
 * nothing changes for a caller who does not use the format.
 */
function _anchorRfc5914Constraints(anchor) {
  var empty = { permitted: [], excluded: [], policySet: null, policyFlags: null, pathLenConstraint: null };
  var c = anchor === null || anchor === undefined ? undefined : anchor.constraints;
  if (!c || typeof c !== "object") return empty;
  var nc = c.nameConstraints;
  return {
    permitted: nc && intrinsic.isArray(nc.permitted) ? checkedSubtreeSeeds(nc.permitted, "trustAnchors[].constraints.nameConstraints.permitted") : [],
    excluded: nc && intrinsic.isArray(nc.excluded) ? checkedSubtreeSeeds(nc.excluded, "trustAnchors[].constraints.nameConstraints.excluded") : [],
    policySet: intrinsic.isArray(c.policySet) ? c.policySet : null,
    policyFlags: c.policyFlags && typeof c.policyFlags === "object" ? c.policyFlags : null,
    pathLenConstraint: intrinsic.isInteger(c.pathLenConstraint) ? c.pathLenConstraint : null,
  };
}

/**
 * @internal RFC 5937 sec. 3.2, the policy set. anyPolicy on either side means "any policy satisfies
 * me", so intersecting with it leaves the other side's set untouched: filtering it literally would
 * compare a concrete identifier against the wildcard identifier, find no match, and refuse a path
 * both sides accept. Only where BOTH name concrete policies is the intersection a comparison, and
 * an empty result there is a real answer rather than "unconstrained": the two name disjoint policy
 * spaces, so no policy satisfies both.
 */
function _mergedPolicySet(callerSet, anchorSet) {
  if (anchorSet === null) return callerSet;
  var callerAny = callerSet === undefined || callerSet === null || contains(callerSet, OID.anyPolicy);
  var anchorAny = contains(anchorSet, OID.anyPolicy);
  if (anchorAny && callerAny) return [OID.anyPolicy];
  if (anchorAny) return intrinsic.map(callerSet, function (p) { return p; });
  if (callerAny) return intrinsic.map(anchorSet, function (p) { return p; });
  return intrinsic.filter(anchorSet, function (p) { return contains(callerSet, p); });
}

/**
 * @internal RFC 5937 sec. 3.2, the flags. Each is one-directional: a flag the anchor asserts turns
 * the matching initial value on where it was off and never off where it was on, so an anchor can
 * only narrow what a caller asked for. That is what makes RFC 5914's "always enforced" safe.
 */
function _flagSet(callerValue, anchorFlags, name) {
  if (callerValue) return true;
  return anchorFlags !== null && anchorFlags[name] === true;
}

function initialize(certs, params, seeds, anchor, ta) {
  var n = certs.length;
  var state = _initialState(certs, params, seeds, anchor, n, ta);
  /** @internal The root is the depth-zero anyPolicy node, and it is indexed like any other, so the
   * "is there a node at this depth for this policy" lookup answers for depth zero too. */
  policyLayer(state, 0)[OID.anyPolicy] = state.validPolicyGraph;
  return state;
}

function _initialState(certs, params, seeds, anchor, n, ta) {
  /** @internal The anchor's constraints are read HERE rather than merged into a copy of `params`,
   * because a copy answers only for the own properties the copy loop enumerated. A caller option
   * defined non-enumerably would be dropped by the copy and the anchor would then WIDEN the
   * validation, which is the one thing sec. 3.2 does not allow. Reading the caller's object
   * directly keeps every option live whatever its descriptor. */
  var anchorPathLen = ta.pathLenConstraint;
  return {
    validPolicyGraph: graphRoot(),
    policyIndex: [],
    policyNodeCount: 1,
    maxPolicyNodes: params.maxPolicyNodes !== undefined ? params.maxPolicyNodes : constants.LIMITS.PATH_MAX_POLICY_NODES,
    minRsaModulusBits: params.minRsaModulusBits !== undefined ? params.minRsaModulusBits : constants.LIMITS.RSA_MIN_MODULUS_BITS,
    ...
    explicitPolicy: _flagSet(params.initialExplicitPolicy, ta.policyFlags, "requireExplicitPolicy") ? 0 : n + 1,
    inhibitAnyPolicy: _flagSet(params.initialAnyPolicyInhibit, ta.policyFlags, "inhibitAnyPolicy") ? 0 : n + 1,
    policyMapping: _flagSet(params.initialPolicyMappingInhibit, ta.policyFlags, "inhibitPolicyMapping") ? 0 : n + 1,
     ...
     * anchor and contains a pathLenConstraint value, set the max_path_length state variable equal
     * to the pathLenConstraint value". Taken as the lower of the two, since sec. 6.1.2's own
     * initial value is the path length and an anchor may only narrow. */
    maxPathLength: intrinsic.isInteger(anchorPathLen) && anchorPathLen < n ? anchorPathLen : n,
    userInitialPolicySet: _mergedPolicySet(params.userInitialPolicySet, ta.policySet) || [OID.anyPolicy],
    results: [],
  };
}
...
function processPolicies(state, cert, i, checks) {
  ...
  if (cp && state.validPolicyGraph) {
    ...
    /** @internal RFC 9618 sec. 5.3 step (d)(1): ONE node per policy, carrying every depth i-1 node
     * that expected it. Creating one per parent instead is what made the structure exponential. */
    policies.forEach(function (p) {
      if (p.policyIdentifier === OID.anyPolicy) { anyPolicyPresent = true; anyPolicyQualifiers = p.qualifiersBytes; return; }
      var parents = intrinsic.filter(nodesAt(state, prev), function (node) {
        return contains(node.expectedPolicySet, p.policyIdentifier);
      });
      if (parents.length === 0) {
        /** @internal (d)(1)(ii): no node expected it, so it hangs off the anyPolicy node if there
         * is one. That single anyPolicy parent is what step (g)(2) later reads. */
        var anyPrev = nodeAt(state, prev, OID.anyPolicy);
        if (anyPrev === null) return;
        parents = [anyPrev];
      }
      addPolicyNode(state, i, p.policyIdentifier, p.qualifiersBytes, [p.policyIdentifier], parents);
    });

    /** @internal (d)(2): for each policy some depth i-1 node expects, if depth i has no node for it
     * yet, one node carrying every parent that expected it. */
    if (anyPolicyPresent && anyPolicyActive) {
      var expected = [];
      nodesAt(state, prev).forEach(function (node) {
        node.expectedPolicySet.forEach(function (ep) {
          if (expected.indexOf(ep) === -1) guard.list.append(expected, ep);
        });
      });
      expected.forEach(function (ep) {
        if (nodeAt(state, i, ep) !== null) return;
        var parents = intrinsic.filter(nodesAt(state, prev), function (node) {
          return contains(node.expectedPolicySet, ep);
        });
        addPolicyNode(state, i, ep, anyPolicyQualifiers, [ep], parents);
      });
    }

    if (state._capHit) return { fatal: true, error: E("path/policy-tree-cap", "policy graph exceeded the node cap") };
    prunePolicyChildless(state, prev);
    if (graphIsEmpty(state)) state.validPolicyGraph = null;
  } else if (!cp) {
    state.validPolicyGraph = null;
  }

  if (!(state.explicitPolicy > 0 || state.validPolicyGraph !== null)) {
    guard.list.append(checks,{ name: "policy", ok: false, code: "path/policy-required" });
    return { fatal: true, error: E("path/policy-required", "explicit policy required but the valid-policy graph is empty") };
  }
  void checks;
  return { fatal: false };
}
...
function isNullOrAbsentParams(p) {
  return guard.der.paramsAbsentOrNull(p);
}
...
function prepareNext(state, cert, i, checks) {
  ...
  if (pm) {
    ...
    if (state.validPolicyGraph) applyPolicyMappings(state, pm.value, i);
  }
...
/** @internal RFC 9618 sec. 5.4 rewrites RFC 5280 sec. 6.1.4 step (b) over the graph. Each clause is
 *  singular where the tree's was plural, because at most one node at a depth carries a given policy.
 *
 * A mapping naming anyPolicy on either side is skipped here as well as refused by the caller
 * (sec. 6.1.4(a)), because mapping one in would rewrite the anyPolicy node's expected policy set and
 * the graph's invariant is that it is always {anyPolicy}. The caller refusing it keeps that from
 * happening today; skipping it here is what keeps it true of this function. */
function applyPolicyMappings(state, mappings, i) {
  if (state.policyMapping > 0) {
    var mappedFrom = intrinsic.create(null);
    mappings.forEach(function (m) {
      if (m.issuerDomainPolicy === OID.anyPolicy || m.subjectDomainPolicy === OID.anyPolicy) return;
      guard.list.append(mappedFrom[m.issuerDomainPolicy] = mappedFrom[m.issuerDomainPolicy] || [], m.subjectDomainPolicy);
    });
    intrinsic.forEach(intrinsic.keys(mappedFrom), function (idp) {
      var node = nodeAt(state, i, idp);
      if (node !== null) {
        /** @internal (b)(1) */
        node.expectedPolicySet = mappedFrom[idp].slice();
        return;
      }
      /** @internal (b)(2): no node for the issuer domain policy, so one is generated under the
       * anyPolicy node at depth i-1, taking the depth-i anyPolicy node's qualifiers. */
      var anyHere = nodeAt(state, i, OID.anyPolicy);
      if (anyHere === null) return;
      var anyPrev = nodeAt(state, i - 1, OID.anyPolicy);
      if (anyPrev === null) return;
      addPolicyNode(state, i, idp, anyHere.qualifierSet[0] || null, mappedFrom[idp].slice(), [anyPrev]);
    });
    return;
  }
  /** @internal (b)(3): mapping is inhibited, so the node for each issuer domain policy is deleted
   * and every ancestor it leaves childless goes with it. */
  if (!state.validPolicyGraph) return;
  var deleted = false;
  mappings.forEach(function (m) {
    if (m.issuerDomainPolicy === OID.anyPolicy) return;
    var node = nodeAt(state, i, m.issuerDomainPolicy);
    if (node !== null) { removePolicyNode(state, node); deleted = true; }
  });
  if (!deleted) return;
  prunePolicyChildless(state, i - 1);
  if (graphIsEmpty(state)) state.validPolicyGraph = null;
}

var TARGET_UNPROCESSED_IF_CRITICAL = intrinsic.create(null);
...
/** @internal A checker decides on more than a status: which source answered, which locations it
 * skipped, and for a revocation the CRLReason and the time the entry named. Those reach the caller
 * on the check row, because the status alone cannot tell a waiver what it waived or a revocation
 * why. The verdict is a caller-supplied object where `opts.revocationChecker` is, so each field is
 * read once into a local and the value is copied rather than the object. `reasonCode` is the name
 * `crlChecker` gives the CRLReason integer and `revocationReason` the name `ocspChecker` gives it;
 * both are read so the row does not depend on which checker answered. */
function _revocationRow(row, rv) {
  if (rv == null) return row;
  var reason = rv.reason;
  if (typeof reason === "string" && reason.length > 0) row.reason = reason;
  var code = rv.revocationReason;
  if (typeof code !== "number") code = rv.reasonCode;
  if (intrinsic.numberIsFinite(code)) row.revocationReason = code;
  var at = rv.revocationTime;
  if (guard.time.isDate(at)) row.revocationTime = new intrinsic.Date(guard.time.instantOf(at));
  return row;
}
 ...
 * `revocationChecker`. An optional `verifier` replaces the built-in signature
 * check with `verifier.verify({ cert, workingPublicKey, workingPublicKeyAlgorithm,
 * workingPublicKeyParameters })`, which is how a chain on a curve the engine does
 * not carry, such as a brainpool one, is validated. It replaces the signature step
 * only: the key-strength floor and every other RFC 5280 check still apply.
 * The value-carrying options (`time`, `maxPathCerts`,
 * `maxPolicyNodes`, `minRsaModulusBits`, the subtree seeds, `userInitialPolicySet`, `requiredEku`)
 ...
 * workingPublicKeyParameters, validPolicyGraph, userConstrainedPolicySet }` where `results[i].checks`
 ...
 *
 * Certificate policies are processed as the RFC 9618 `valid_policy_graph`, which replaces the
 * RFC 5280 sec. 6.1 `valid_policy_tree`. `validPolicyGraph` is a directed acyclic graph holding at
 * most one node per depth and policy identifier; a node reached from several policies carries them
 * all in `parents`, where the tree held a separate node under each. The graph is linear in the
 * number of policies and mappings on the path, so a chain whose tree would not fit inside
 * `maxPolicyNodes` validates. It is `{ nodes }`, a flat list linked by index, each entry
 * `{ id, depth, validPolicy, qualifierSet, expectedPolicySet, parents, children }` where `parents`
 * and `children` hold the ids of other entries and the root is id 0.
 *
 * A certificate signed with the RFC 9802 stateful hash-based algorithm
 * `id-alg-hss-lms-hashsig` is verified like any other, through the toolkit's own
 * HSS engine rather than the crypto engine. The signature covers the whole
 * tbsCertificate rather than a digest of it (RFC 9802 7.1), the RFC 8554 public
 * key is read from the subjectPublicKey BIT STRING and the signature from
 * signatureValue with no further ASN.1 wrapping (5, 6), the algorithm identifier
 * carries no parameters, and the subject public key must name the algorithm the
 * signature names. A `pki.crl.verify` CertificateList naming it takes the same
 * route, which 6 provides for. Signing is out of scope: a stateful key's
 * one-time state belongs in hardware that owns it.
 *
 * A certificate carrying any of the three stateful hash-based keys, HSS, XMSS or
 * XMSS^MT, is held to the RFC 9802 6 key-usage rule whichever algorithm signed
 * it, reported as the `hbsKeyUsage` check and refused with `path/hbs-key-usage`.
 * The rule differs by role: a certification authority's certificate must assert
 * at least one of digitalSignature, nonRepudiation, keyCertSign or cRLSign, an
 * end entity's at least one of digitalSignature, nonRepudiation or cRLSign, and
 * neither may assert any value outside its own list. An absent keyUsage is
 * unrestricted, as RFC 5280 4.2.1.3 has it.
 *
 ...
 * and you waived it", a distinction a stored verdict is re-read to settle. It
 * also carries what the checker decided on: `reason` (the source that answered,
 * the locations it skipped, the faults it noted), and for a revocation
 * `revocationReason` (the CRLReason integer) and `revocationTime`. A
 ...
 * subtree is refused with `path/bad-input` rather than left inert, and so is one
 * whose base is outside the form its tag names: a host name for tags 2 and 6, a
 * mailbox or host name for tag 1, at least one relative name with object
 * identifier types and string values for tag 4, and 8 or 32 bytes for tag 7. An
 * anchor whose `nameConstraints` names no subtree at all is refused the same way.
 * A certificate's own `nameConstraints` extension is compared as the certificate
 * wrote it; this rule is on the subtrees a caller supplies.
 *
 * @example
 ...
 */
var _VALIDATE_OPTS = intrinsic.assign(intrinsic.create(null), {
  checkPurpose: 1, ctLogList: 1, ctPolicy: 1, historicalMode: 1, identity: 1, initialAnyPolicyInhibit: 1,
  initialExcludedSubtrees: 1, initialExplicitPolicy: 1, initialPermittedSubtrees: 1,
  initialPolicyMappingInhibit: 1, maxPathCerts: 1, maxPolicyNodes: 1, minRsaModulusBits: 1, requireRevocation: 1,
...
async function validate(path, opts) {
  var checked = guard.identifier.optionsObject(opts, E, "path/bad-input", "validate: opts");
  guard.identifier.assertKnownKeys(checked, _VALIDATE_OPTS, E, "path/bad-input",
  ...
  /** @internal One validation, named by an object nothing outside this module can reach. A
   * revocation checker that budgets fetches is called once per certificate and again per anchor
   * attempt, so without a name for the run it cannot tell "the next certificate on this path" from
   * "a new validation", and the totals it documents become per-certificate totals. The checker
   * holds its state against this object; the object is unreachable once the validation returns, so
   * a checker stays reusable and nothing accumulates between runs. The anchor recursion carries the
   * same one, and a caller has no way to supply one, so two validations never share a budget. */
  return _validate(path, checked, intrinsic.freeze(intrinsic.create(null)));
}

/** @internal The options are the ones the door above checked, and each anchor attempt is handed a
 * copy this module built, so the door runs once per call rather than once per anchor. */
async function _validate(path, opts, run) {
  ...
  /** @internal RFC 9525 sec. 6.1.1 has the client build its reference identities before it asks,
   * and everything between this call and the comparison is awaited, so the list is read and copied
   * here. A caller that reuses its options object, or whose object is reachable from elsewhere,
   * then cannot change the question after asking it. */
  var identityRefs = null;
  if (opts.identity != null) {
    try { identityRefs = identity.snapshotReferences(opts.identity); }
    catch (e) { throw E("path/bad-input", "validate: opts.identity is not a reference identity this toolkit reads", e); }
  }
  ...
  if (opts.minRsaModulusBits != null) {
    if (typeof opts.minRsaModulusBits !== "number" || !intrinsic.isInteger(opts.minRsaModulusBits)) {
      throw E("path/bad-input", "validate: opts.minRsaModulusBits must be an integer");
    }
    if (opts.minRsaModulusBits < constants.LIMITS.RSA_MIN_MODULUS_BITS) {
      throw E("path/bad-input", "validate: opts.minRsaModulusBits may only raise the " + constants.LIMITS.RSA_MIN_MODULUS_BITS + "-bit minimum, not lower it");
    }
  }
  ...
  if (anchorList.length > 1) {
    ...
    var singleOpts = function (a) {
      ...
      /** @internal The snapshot taken in this call's prologue, so every anchor attempt asks the
       * question the caller asked. Copying `opts.identity` across would have each attempt read
       * the caller's own list again, after the awaits the earlier attempts spent. */
      if (identityRefs !== null) o.identity = identityRefs;
      return o;
    };
    ...
    for (var ai = 0; ai < tuples.length; ai++) {
      ...
      anyMatched = true;
      var oneRes = await _validate(path, singleOpts(tuples[ai]), run);
      if (oneRes && oneRes.valid) return oneRes;
      lastMatched = oneRes;
    }
    return anyMatched ? lastMatched : _validate(path, singleOpts(tuples[0]), run);
  }
  ...
  /** @internal RFC 5937 sec. 3.2 says how an RFC 5914 anchor's own constraints reach this
   * algorithm, and the rule is uniform: the stricter of the anchor's value and the caller's wins.
   * Its name constraints are a SEPARATE generation from the caller's and from the `nameConstraints`
   * overlay, because generations intersect and a replacement would let one loosen the other. */
  var taConstraints = _anchorRfc5914Constraints(anchor);
  var seeds = {
    permittedGenerations: [callerPermitted, anchorNc.permitted, taConstraints.permitted],
    excluded: intrinsic.concat(intrinsic.concat(callerExcluded, anchorNc.excluded), taConstraints.excluded),
  };
  var anchorNameConstraintsApplied = anchorNc.permitted.length > 0 || anchorNc.excluded.length > 0 ||
    taConstraints.permitted.length > 0 || taConstraints.excluded.length > 0;
  var purposeOpts = resolvePurposeOpts(opts);
  var requiredEku = purposeOpts.requiredEku;
  var checkPurpose = purposeOpts.checkPurpose;

  var state = initialize(certs, opts, seeds, anchor, taConstraints);
  state._n = n;
  ...
  for (var idx = 0; idx < n; idx++) {
    ...
    var sigRes;
    var weak = _weakIssuerKey(state);
    if (weak) {
      sigRes = { ok: false, code: "path/weak-key", error: weak };
    } else if (verifier) {
      var vv;
      try {
    ...
    /** @internal What authenticates a certificate is the chain above it, not its own signature:
     * a forged issuer fails its own check and its key still becomes the working key, so everything
     * below verifies against a key nothing vouched for. Once a signature on the path has failed,
     * no certificate under it is authenticated however its own check reads. */
    if (!sigRes.ok) prefixAuthentic = false;
    ...
    if (STATEFUL_HBS_OIDS[cert.subjectPublicKeyInfo.algorithm.oid]) {
      var hku = statefulHbsKeyUsageCheck(cert, _isCaCert(cert));
      guard.list.append(checks, { name: "hbsKeyUsage", ok: hku.ok, code: hku.ok ? undefined : hku.code });
      if (!hku.ok) failed = true;
    }
    ...
    /** @internal RFC 5019 sec. 3.2 has the status asked for after the chain signatures are
     * validated, and a certificate no chain has authenticated is a claim: the serial and issuer a
     * responder would answer about are ones nothing has bound to a key. A checker may open a
     * socket, so asking anyway lets an unauthenticated certificate choose a destination and spend
     * the fetch budget. The row says the status was not asked for, which is an undetermined
     * outcome and not a checked one. */
    if (revocationChecker && !prefixAuthentic) {
      guard.list.append(checks, { name: "revocation", ok: false, status: "unknown",
        code: "path/revocation-undetermined",
        reason: sigRes.ok
          ? "a signature above this certificate on the path did not verify, so no revocation status was asked for"
          : "the certificate's own signature did not verify, so no revocation status was asked for" });
      revocationUndetermined = true;
      revocationRan = true;
    } else if (revocationChecker) {
      var issuerCert = idx > 0 ? certs[idx - 1] : null;
      var rv, rvError = null;
      try { rv = await revocationChecker.check(cert, { workingIssuerName: state.workingIssuerName, workingPublicKey: state.workingPublicKey, workingPublicKeyAlgorithm: state.workingPublicKeyAlgorithm, issuerCert: issuerCert }, { time: opts.time, historicalMode: opts.historicalMode === true, run: run }); }
      catch (e) { rv = { status: "error" }; rvError = e; }
      var rvStatus = (rv && typeof rv.status === "string") ? rv.status : "unknown";
      if (rv && rv.status === "good") { guard.list.append(checks, _revocationRow({ name: "revocation", ok: true, status: "good" }, rv)); }
      else if (rv && rv.status === "revoked") { guard.list.append(checks, _revocationRow({ name: "revocation", ok: false, status: "revoked", code: "path/revoked" }, rv)); failed = true; }
      else if (rvError) {
        ...
        guard.list.append(checks, _revocationRow({ name: "revocation", ok: true, status: rvStatus, waived: true }, rv));
        revocationWaived = true;
      } else {
        guard.list.append(checks, _revocationRow({ name: "revocation", ok: false, status: rvStatus, code: "path/revocation-undetermined" }, rv));
        revocationUndetermined = true;
        failed = true;
      }
      revocationRan = true;
  ...
  /** @internal RFC 9525 sec. 1.2 has identity matching run in addition to the sec. 6.1 checks
   * rather than in place of any of them, and sec. 1.2 again has it read the leaf alone. So this
   * runs after the per-certificate loop, against `certs[n-1]`, and its row joins that certificate's
   * checks beside the sec. 6.1 rows rather than replacing them. */
  var identityChecked = false;
  if (identityRefs !== null) {
    var idVerdict = identity.match(certs[n - 1], identityRefs);
    var idRow = guard.verdict.of(idVerdict.matched
      ? { name: "identity", ok: true }
      : { name: "identity", ok: false, code: "path/identity-mismatch" });
    var idTarget = null;
    for (var ir = 0; ir < state.results.length; ir++) { if (state.results[ir].index === n - 1) { idTarget = state.results[ir]; break; } }
    if (idTarget) guard.list.append(idTarget.checks, idRow);
    if (!idVerdict.matched) failed = true;
    identityChecked = idVerdict.matched ? idVerdict.matchedReference : false;
  }
  ...
  return guard.verdict.of({
    ...
    workingPublicKeyAlgorithm: state.workingPublicKeyAlgorithm,
    workingPublicKeyParameters: state.workingPublicKeyParameters,
    validPolicyGraph: graphSnapshot(state.validPolicyGraph),
    userConstrainedPolicySet: ucps,
  });
}

/**
 * @internal RFC 9618 sec. 5.5 rewrites RFC 5280 sec. 6.1.5 step (g) over the graph. The set is read
 * from the nodes whose only parent is the anyPolicy node, at ANY depth rather than at the deepest
 * one, plus the anyPolicy node at depth n. Policy qualifiers are not collected: sec. 5.5 step
 * (g)(4)(ii) lets an application that does not use them skip it, and this returns identifiers.
 */
function userConstrainedPolicies(state, n) {
  if (!state.validPolicyGraph) return [];
  var authority = [];
  function note(node) {
    if (authority.indexOf(node.validPolicy) === -1) guard.list.append(authority, node.validPolicy);
  }
  /** @internal (g)(2) */
  for (var d = 1; d <= n; d++) {
    nodesAt(state, d).forEach(function (node) {
      if (node.validPolicy === OID.anyPolicy) return;
      if (node.parents.length === 1 && node.parents[0].validPolicy === OID.anyPolicy) note(node);
    });
  }
  /** @internal (g)(3) */
  var anyAtN = nodeAt(state, n, OID.anyPolicy);
  if (anyAtN !== null) note(anyAtN);

  /** @internal (g)(5) and (g)(6) */
  var uips = state.userInitialPolicySet;
  if (contains(uips, OID.anyPolicy)) return authority;
  var out = intrinsic.filter(authority, function (p) { return contains(uips, p); });
  if (contains(authority, OID.anyPolicy)) {
    uips.forEach(function (p) { if (out.indexOf(p) === -1) guard.list.append(out, p); });
  }
  return out;
}
...
/** @internal The structural read is shared with certificate-revocation-list linting through
 * schema-pkix, so the two cannot disagree about what a well-formed IssuingDistributionPoint is.
 * Only the reason-flags MASK is added here, because only path validation compares against it. */
function decodeIdp(ext) {
  var s = pkix.decodeIssuingDistributionPointValue(NS, ext.value, "path/bad-idp");
  var out = {
    hasDistributionPoint: s.hasDistributionPoint, distributionPoint: s.distributionPoint,
    onlyUser: s.onlyUser, onlyCa: s.onlyCa,
    onlySomeReasons: s.onlySomeReasonsPresent ? true : null,
    indirect: s.indirect, onlyAttr: s.onlyAttr, malformed: s.malformed,
    onlySomeReasonsMask: null,
  };
  if (out.malformed) return out;
  if (s.onlySomeReasonsPresent) {
    out.onlySomeReasonsMask = reasonMaskFromBitString(s.onlySomeReasonsValue);
 ...
 * `unspecified`), a `reason` naming it, and `revocationTime` from the entry's
 * revocationDate.
 *
 * @opts
...
function crlChecker(crls, opts) {
  ...
  return {
    check: async function (cert, issuer, ctx) {
      ...
      var certStatus = null, certTime = null;
      var reasonsMask = 0;
      ...
      function scanCrl(theCrl) {
        for (var r = 0; r < theCrl.revokedCertificates.length; r++) {
          ...
          return { code: rc === null ? 0 : rc,
            at: guard.time.isDate(entry.revocationDate) ? entry.revocationDate : null };
        }
        return null;
      }
      ...
      for (var ci = 0; ci < classified.completes.length; ci++) {
        ...
        if (status !== null && status.code === 8) status = null;

        if (status !== null && certStatus === null) { certStatus = status.code; certTime = status.at; }
      ...
      for (var dj = 0; dj < classified.deltas.length; dj++) {
        ...
        if (dStatus.code === 8) { releasedByUnmergedDelta = true; continue; }
        if (certStatus === null) { certStatus = dStatus.code; certTime = dStatus.at; }
      }
      ...
      if (certStatus !== null) {
        ...
        return guard.verdict.of({ status: "revoked", reasonCode: certStatus, reason: "serial listed in a CRL (" + reasonName + ")" },
          certTime === null ? null : { revocationTime: certTime });
      }
...
function _verifyWithSpki(sigAlg, rawSig, spkiBytes, tbsBytes) {
  ...
  /** @internal RFC 9802 sec. 6 names the CertificateList as well as the Certificate, so a revocation list
   * signed with a stateful hash-based key reaches the same engine the certificate path uses. */
  if (d.shbs) return _resolve(Promise, _shbsVerify(spkiBytes, rawSig, tbsBytes).ok === true);
 ...
 * @signature  pki.path.verifyOcspResponse(response, cert, issuerCert, time, opts?) -> Promise<{ valid, status, responderAuthorized, signatureValid, matched, nonceMatched, thisUpdate, nextUpdate, revocationReason?, revocationTime?, reason }>
 * @since       0.2.22
 * @status      stable
 ...
 * that ergonomic entry, which also handles DER/PEM decoding). It runs exactly the gates the path validator's `ocspChecker`
 ...
 *
 * `opts.requestNonce` is the nonce bytes the request carried. Given one, the response must echo it
 * (RFC 9654 sec. 2.1, RFC 6960 sec. 4.4.1), compared in constant time, and `nonceMatched` reports
 * the outcome: `true`, `false`, or `null` when no nonce was sent. An unechoed nonce takes a `good`
 * to `unknown`, because a response that does not carry it answers an earlier question; a `revoked`
 * keeps its status and carries `nonceMatched: false`, a stale revocation being no safer to accept.
 * This is where the rule is applied, so every caller reaching this gate gets it.
 *
 ...
 */
var _VERIFY_OCSP_OPTS = intrinsic.assign(intrinsic.create(null), { historicalMode: 1, requestNonce: 1 });

async function verifyOcspResponse(response, cert, issuerCert, time, opts) {
  var parsedResponse, subject, issuer, checkedOpts;
  try {
    /** @internal The options go through the same door every other verb's do. Without it a
     * misspelled `requestNonce` leaves the nonce unasked and the response is accepted against
     * bytes nobody compared, and an accessor supplies one value to the check and another to the
     * comparison. This verb is the acceptance gate, so both end at a response reported good. */
    checkedOpts = guard.identifier.optionsObject(opts, E, "path/bad-input", "verifyOcspResponse: opts");
    guard.identifier.assertKnownKeys(checkedOpts, _VERIFY_OCSP_OPTS, E, "path/bad-input",
      "pki.path.verifyOcspResponse has an unknown option (the request nonce to bind against is `requestNonce`). The unknown option was: ");
  ...
  var v = await _verifyOcspParsed(parsedResponse, subject, issuer, time, checkedOpts);
...
function _verifyOcspParsed(parsedResponse, cert, issuerCert, time, opts) {
  ...
  /** @internal Snapshotted in the synchronous prologue: the bytes compared are the bytes the caller
   * passed, not what its BufferSource holds once the verify has awaited. A value that is not bytes
   * at all is not a refusal, because a caller that asked for a nonce and supplied something that
   * cannot be one has still asked: `wantNonce` stays set with nothing to compare, so the response
   * cannot echo it and the verdict fails closed rather than passing the check by skipping it. */
  var wantNonce = opts.requestNonce != null;
  var nonce = (wantNonce && guard.bytes.isByteSource(opts.requestNonce))
    ? guard.bytes.snapshotSource(opts.requestNonce, PathError, "path/bad-input", "opts.requestNonce")
    : null;
  /** @internal A verdict decided before the response is read is still a verdict, and the field
   * that says what became of the nonce is on every one of them: absent is not the same answer as
   * "no nonce was sent", which is what `null` says. */
  function unbound(reason) {
    return _withNonceRule(guard.verdict.of({ status: "unknown", responderAuthorized: false, signatureValid: false, matched: false, thisUpdate: null, nextUpdate: null, reason: reason }), parsedResponse, wantNonce, nonce);
  }
  var boundName;
  ...
  return builtinVerify({ workingPublicKey: issuerCert.subjectPublicKeyInfo.bytes }, cert).then(function (sig) {
    ...
    return ocspCore.evaluateResponse(parsedResponse, cert, issuerCtx, issuerKeyBits, issuerNameCandidates, time, opts.historicalMode === true).then(function (v) {
      if (v.revoked) return _withNonceRule(guard.verdict.of({ status: "revoked", responderAuthorized: true, signatureValid: true, matched: true, thisUpdate: v.thisUpdate, nextUpdate: v.nextUpdate, revocationReason: v.revoked.revocationReason, revocationTime: v.revoked.revocationTime, reason: v.revoked.reason }), parsedResponse, wantNonce, nonce);
      if (v.sawGood) return _withNonceRule(guard.verdict.of({ status: "good", responderAuthorized: true, signatureValid: true, matched: true, thisUpdate: v.thisUpdate, nextUpdate: v.nextUpdate, reason: "good" }), parsedResponse, wantNonce, nonce);
      return _withNonceRule(guard.verdict.of({ status: "unknown", responderAuthorized: v.responderAuthorized === true, signatureValid: v.signatureValid === true, matched: v.matched === true, thisUpdate: v.thisUpdate, nextUpdate: v.nextUpdate, reason: v.reason }), parsedResponse, wantNonce, nonce);
    });
  });
}

/** @internal RFC 9654 sec. 2.1 with RFC 6960 sec. 4.4.1: a client that sent a nonce is asking this
 * responder this question now, and a response that does not echo it answers some earlier question.
 * The rule lives here because this is the acceptance gate every OCSP consumer reaches, and a copy
 * above it would leave a consumer that does not go through that copy with no check at all.
 *
 * An unechoed nonce takes a `good` to undetermined and leaves a `revoked` standing: a response
 * saying the certificate is revoked is not made safe by being stale, so the verdict keeps its
 * status and reports `nonceMatched: false` beside it. The comparison is constant-time. */
function _withNonceRule(verdict, parsedResponse, wantNonce, requestNonce) {
  if (!wantNonce) return guard.verdict.of(verdict, { nonceMatched: null });
  var respNonce = _responseNonce(parsedResponse);
  var matched = requestNonce !== null && respNonce != null &&
    guard.crypto.constantTimeEqual(respNonce, requestNonce);
  var out = guard.verdict.of(verdict, { nonceMatched: matched });
  if (matched || out.status !== "good") return out;
  return guard.verdict.of(out, { status: "unknown",
    reason: "the OCSP response nonce does not echo the request nonce (RFC 9654 sec. 2.1)" });
}

function _responseNonce(parsedResponse) {
  var br = parsedResponse && parsedResponse.basicResponse;
...
function _capturedRdns(base, who) {
  return guard.name.capturedRdns(base, E, "path/bad-input", who + ": a trustAnchor nameConstraints directoryName base");
}
...
var _TA_CONSTRAINT_KEYS = intrinsic.assign(intrinsic.create(null), {
  nameConstraints: 1, policySet: 1, policyFlags: 1, pathLenConstraint: 1 });
var _TA_FLAG_KEYS = intrinsic.assign(intrinsic.create(null), {
  inhibitPolicyMapping: 1, requireExplicitPolicy: 1, inhibitAnyPolicy: 1 });
/**
 * @internal An RFC 5914 anchor's own constraints, snapshotted the way every other anchor field is:
 * read once from its own data property, copied, and never read again. A caller object that answered
 * one value to the check and another to the enforcement would otherwise decide the namespace and
 * the policy set a path is held to.
 */
function _copyAnchorRfc5914Constraints(c, who) {
  var label = who + ": a trustAnchor constraints";
  if (c === null || typeof c !== "object" || intrinsic.isArray(c)) {
    throw E("path/bad-input", label + " must be a { nameConstraints, policySet, policyFlags, pathLenConstraint } object");
  }
  guard.identifier.assertPlainRecord(c, E, "path/bad-input", label);
  guard.identifier.assertKnownKeys(c, _TA_CONSTRAINT_KEYS, E, "path/bad-input",
    label + " names nameConstraints, policySet, policyFlags and pathLenConstraint. The unknown key was: ");
  var out = {};
  var ncD = intrinsic.getOwnPropertyDescriptor(c, "nameConstraints");
  if (ncD !== undefined) {
    if (!intrinsic.hasOwn(ncD, "value")) throw E("path/bad-input", label + ".nameConstraints must be a data property, not an accessor");
    intrinsic.defineProperty(out, "nameConstraints", { value: _copyAnchorNameConstraints(ncD.value, who + ": a trustAnchor constraints"), enumerable: true, configurable: true, writable: true });
  }
  var psD = intrinsic.getOwnPropertyDescriptor(c, "policySet");
  if (psD !== undefined) {
    if (!intrinsic.hasOwn(psD, "value")) throw E("path/bad-input", label + ".policySet must be a data property, not an accessor");
    var ps = psD.value;
    if (!intrinsic.isArray(ps) || intrinsic.types.isProxy(ps)) throw E("path/bad-input", label + ".policySet must be a plain array of policy identifiers");
    var psOut = [];
    for (var i = 0; i < ps.length; i++) {
      var pd = intrinsic.getOwnPropertyDescriptor(ps, i);
      if (pd === undefined || !intrinsic.hasOwn(pd, "value")) throw E("path/bad-input", label + ".policySet entry " + i + " must be a data property, not a hole or an accessor");
      if (typeof pd.value !== "string" || pd.value.length === 0) throw E("path/bad-input", label + ".policySet entries must be dotted-decimal policy identifiers");
      guard.list.append(psOut, guard.identifier.assertCanonicalOid(pd.value, E, "path/bad-input", label + ".policySet entry " + pd.value));
    }
    intrinsic.defineProperty(out, "policySet", { value: psOut, enumerable: true, configurable: true, writable: true });
  }
  var pfD = intrinsic.getOwnPropertyDescriptor(c, "policyFlags");
  if (pfD !== undefined) {
    if (!intrinsic.hasOwn(pfD, "value")) throw E("path/bad-input", label + ".policyFlags must be a data property, not an accessor");
    var pf = pfD.value;
    if (pf === null || typeof pf !== "object" || intrinsic.isArray(pf)) throw E("path/bad-input", label + ".policyFlags must be an object of boolean flags");
    guard.identifier.assertPlainRecord(pf, E, "path/bad-input", label + ".policyFlags");
    guard.identifier.assertKnownKeys(pf, _TA_FLAG_KEYS, E, "path/bad-input", label + ".policyFlags names the three RFC 5914 flags. The unknown key was: ");
    var fOut = {};
    intrinsic.forEach(intrinsic.keys(_TA_FLAG_KEYS), function (k) {
      var fd = intrinsic.getOwnPropertyDescriptor(pf, k);
      if (fd === undefined) return;
      if (!intrinsic.hasOwn(fd, "value")) throw E("path/bad-input", label + ".policyFlags." + k + " must be a data property, not an accessor");
      if (typeof fd.value !== "boolean") throw E("path/bad-input", label + ".policyFlags." + k + " must be a boolean");
      intrinsic.defineProperty(fOut, k, { value: fd.value, enumerable: true, configurable: true, writable: true });
    });
    intrinsic.defineProperty(out, "policyFlags", { value: fOut, enumerable: true, configurable: true, writable: true });
  }
  var plD = intrinsic.getOwnPropertyDescriptor(c, "pathLenConstraint");
  if (plD !== undefined) {
    if (!intrinsic.hasOwn(plD, "value")) throw E("path/bad-input", label + ".pathLenConstraint must be a data property, not an accessor");
    if (!intrinsic.isInteger(plD.value) || plD.value < 0) throw E("path/bad-input", label + ".pathLenConstraint must be a non-negative integer");
    intrinsic.defineProperty(out, "pathLenConstraint", { value: plD.value, enumerable: true, configurable: true, writable: true });
  }
  return out;
}
...
function _copyAnchorNameConstraints(nc, who) {
  ...
  /** @internal An anchor carrying the option but naming no subtree restricts nothing, and reads to
   * whoever built it as a namespace that has been applied. */
  if ((out.permitted === undefined || out.permitted.length === 0) &&
      (out.excluded === undefined || out.excluded.length === 0)) {
    throw E("path/bad-input", who + ": a trustAnchor nameConstraints must name at least one permitted or excluded subtree");
  }
  return out;
}
...
function toAnchor(entry, verb) {
  ...
  var subjectDerRaw, labelRaw, mozRaw, ncSnap, taSnap;
  ...
  try {
    if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && !guard.parsed.isRecorded(entry)) {
      ...
      var taRaw = _captureConstraintMap(entry, "constraints", E, "path/bad-input", who);
      taSnap = taRaw == null ? undefined : _copyAnchorRfc5914Constraints(taRaw, who);
  ...
  if (isTuple) {
    ...
    if (taSnap !== undefined) intrinsic.defineProperty(flat, "constraints", { value: taSnap, enumerable: true, configurable: true, writable: true });
  ...
  var out = { name: cert.subject, publicKey: spki.bytes, algorithm: spki.algorithm.oid, parameters: spki.algorithm.parameters, subjectDer: cert.subject.bytes };
  /** @internal The certificate's own basic constraints are associated with the anchor, so its
   * pathLenConstraint bounds the path below it (RFC 5937 sec. 3.2). Defined rather than assigned,
   * for the reason every other anchor field is. */
  var certPathLen = pkix.anchorPathLenFromExtensions(cert.extensions, NS, who + ": the trustAnchor certificate");
  if (certPathLen !== null) {
    var c = intrinsic.create(null);
    c.pathLenConstraint = certPathLen;
    guard.verdict.set(out, "constraints", c);
  }
  return out;
}
...
/** @internal A relative name that cannot be compared ends the shared prefix below; the count is a sort hint
 * and every candidate is still tried, so the comparison fault decides nothing. */
function _rdnEqualSoft(a, b) {
  try { return rdnEqual(a, b); }
  catch (_e) { return false; } // allow:swallow-unverified a name the comparison refuses ends the shared prefix
}

/** @internal How many leading relative names two distinguished names share, under the RFC 5280 sec. 7.1
 * comparison. Names in a public-key infrastructure are built as a tree, so in a mesh of cross-certified domains
 * a certificate in the target's own domain shares a prefix with it and one from a foreign domain shares little
 * or none (RFC 4158 sec. 3.5.16, sec. 3.5.19). */
function rdnPrefixLength(a, b) {
  if (!intrinsic.isArray(a) || !intrinsic.isArray(b)) return 0;
  var n = a.length < b.length ? a.length : b.length;
  for (var i = 0; i < n; i++) {
    if (!_rdnEqualSoft(a[i], b[i])) return i;
  }
  return n;
}

var RDN_PREFIX_WEIGHT = 5;
/** @internal Within the name term, a shared prefix is the weaker signal and stays under an exact match,
 * however many relative names it shares. The terms are summed the way the rest of the RFC 4158 sec. 3.5
 * hints are, so this bounds the name term rather than the whole score. */
var RDN_PREFIX_MAX = 95;
var EXACT_NAME_BONUS = 100;
function _prefixScore(a, b) {
  var s = rdnPrefixLength(a, b) * RDN_PREFIX_WEIGHT;
  return s > RDN_PREFIX_MAX ? RDN_PREFIX_MAX : s;
}

/** @internal RFC 4158 sec. 3.5.7: a certificate whose path length constraint cannot cover the certificates
 * already below it in the path cannot complete that path. The constraint counts the non-self-issued
 * certificates that follow (RFC 5280 sec. 6.1.4(m)), which forward building knows from the chain it holds, so
 * a key-rollover certificate below the candidate spends none of the budget. */
function _forwardPathLenIneligible(cand, chain) {
  /** @internal A forward chain is built by prepending, so the leaf is its last entry and every entry before
   * it is an intermediate the candidate would sit above. */
  var below = 0;
  for (var i = 0; i < chain.length - 1; i++) {
    if (!selfIssued(chain[i])) below += 1;
  }
  if (below === 0) return false;
  var bc = softDecode(cand, OID.basicConstraints);
  var lim = bc && bc.value ? bc.value.pathLenConstraint : undefined;
  if (typeof lim !== "number") return false;
  return lim < below;
}

/** @internal Reverse building holds no such count: it knows what lies between a candidate and the anchor, and
 * the constraint governs what lies between the candidate and the target, which is the part not yet built. Every
 * candidate at one frame would take the same penalty, which orders nothing, so the rule is forward-only here. */

/** @internal RFC 4158 sec. 3.5.19, a candidate whose subject shares more relative names with the target's
 * issuer, orders nothing in this direction: forward building selects a candidate BY its subject matching the
 * current certificate's issuer, so every candidate at one decision point carries that same name. Reverse
 * building selects by issuer instead, and there the rule is the ordering below. */
function scoreCandidate(cand, childAki, anchors, time) {
  ...
  for (var i = 0; i < anchors.length; i++) {
    if (nameMatchSoft(cand.issuer.rdns, anchors[i].name.rdns)) { best = EXACT_NAME_BONUS; break; }
    var p = _prefixScore(cand.issuer.rdns, anchors[i].name.rdns);
    if (p > best) best = p;
  }
  return score + best;
}
...
function scoreCandidateReverse(cand, parentCert, leafCert, time) {
  ...
  if (nameMatchSoft(leafCert.issuer.rdns, cand.subject.rdns)) score += EXACT_NAME_BONUS;
  else score += _prefixScore(cand.subject.rdns, leafCert.issuer.rdns);
  return score;
}
...
/** @internal Sorted so the candidate tried FIRST is last in the array, since the caller pushes onto a stack.
 * A candidate whose path length constraint cannot cover the certificates below it sorts under every candidate
 * that can, whatever the heuristics score it, and the score orders within each of the two groups. */
...
async function _aiaFetchOne(uri, aia) {
  var res = await guard.async.invoked(aia.transport,
    [{ method: "GET", url: uri, tls: aia.tls, timeout: aia.timeout, maxResponseBytes: aia.maxResponseBytes, blockPrivateAddresses: true }],
  ...
  var headers = guard.header.lowerCased(res.headers);
...
async function _fetchAiaIssuers(current, aia) {
  ...
  for (var k = 0; k < uris.length; k++) {
    ...
    aia.fetches += 1;
    var certs;
    /** @internal Every failure of a fetch is skipped and the next URL tried, because AIA is a
     * best-effort source of issuers and one unreachable host must not fail a build that another URL
     * can complete. A transport that throws where a rejecting one would only reject is the same
     * failure reported a different way, so the two are treated alike rather than one of them ending
     * the build. */
 ...
 * candidate).
 *
 * Where cross-certified domains meet, several candidates chain by name and the order they are
 * tried in decides how much of the mesh is walked. Two more RFC 4158 3.5 orderings apply there.
 * A candidate is ranked by how many leading relative names it shares with the name the search is
 * heading for, its issuer against a trust anchor's name going forward (3.5.16) and its subject
 * against the leaf's issuer going in reverse (3.5.16 and 3.5.19 reverse methods), so a
 * certificate in the target's own domain is tried before one from a cross-certified domain.
 * Forward building also sorts a candidate under every candidate that can complete the path when its
 * `basicConstraints` path length constraint cannot cover the certificates already below it (3.5.7,
 * counting the non-self-issued ones the way 5280 6.1.4(m) does). That one outranks the other hints
 * rather than competing with them, so a key identifier or an anchor-adjacent issuer does not raise
 * a candidate whose constraint the path already exceeds. All of it is ordering alone: no candidate
 * is removed, every assembled path is handed to `validate`, and an unbounded search still reaches
 * every path the pool holds. A bounded one spends its budget on whichever branches it is steered
 * to, so a build that reports `path/build-limit` where an earlier order succeeded wants a larger
 * `maxCandidatesConsidered`, and the same holds for the `maxAiaFetches` allowance.
 *
 * The search runs depth-first with backtracking: the first ordered path that
...
var _BUILD_OPTS = (function () {
  var o = intrinsic.assign(intrinsic.create(null), {
...
async function build(leaf, opts) {
  ...
  /** @internal Checked at the door, before any certificate is parsed, and whether or not AIA
   * fetching is on: supplying a transport that cannot be called is a wiring fault at the moment it
   * is written. The checked value is the one used below, since reading the option again would ask a
   * second time and an accessor can answer differently on each read. */
  var checkedTransport = guard.identifier.assertCallableOption(opts, "transport", E, "path/bad-input",
    "build: opts.transport", "(request) -> Promise<{ status, headers, body }>");
  var leafCert;
  ...
  if (opts.fetchAia === true) {
    ...
    var aiaTransport = checkedTransport || httpTransport.https({ E: E, errPrefix: "path" });
    aiaCtx = {
      transport: aiaTransport,
      /** @internal Read off the slot, not the chain: `Function.prototype` carries whatever has
       * been written to it for every function in the process, so a transport that declares
       * nothing would otherwise be read as promising it filters the address it resolved, and a
       * hostname taken from a certificate would be opened on that promise.
       * @guard-via guard.identifier.declaresOwn */
      transportGuardsAddresses: guard.identifier.declaresOwn(aiaTransport, "blocksPrivateAddresses"),
      tls: opts.tls || {},
      ...
      for (var pj = fb.poolMark; pj < pool.length; pj++) {
        if (nameMatchSoft(pool[pj].subject.rdns, fbCur.issuer.rdns)) {
          guard.list.append(fbScored, { cand: pool[pj], score: scoreCandidate(pool[pj], fbAki, anchors, opts.time),
            ineligible: _forwardPathLenIneligible(pool[pj], fb.chain) });
        }
      }
    ...
    for (var pi = 0; pi < pool.length; pi++) {
      if (nameMatchSoft(pool[pi].subject.rdns, current.issuer.rdns)) {
        guard.list.append(scored, { cand: pool[pi], score: scoreCandidate(pool[pi], childAki, anchors, opts.time),
          ineligible: _forwardPathLenIneligible(pool[pi], frame.chain) });
      }
    }
...
/** @internal The fetching checker composes the two gates below, so it loads after this module
 * rather than beside it. The require sits in the call so the cycle resolves at first use, which is
 * the documented reason an inline require is permitted. */
function fetchingChecker(opts) { return require("./revocation-fetch").fetchingChecker(opts); }
...
module.exports = {
  ...
  fetchingChecker: fetchingChecker,
