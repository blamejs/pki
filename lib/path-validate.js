...
function toAnchor(entry, verb) {
  ...
  try {
    if (entry && typeof entry === "object" && !Buffer.isBuffer(entry)) {
      // Read the tuple discriminator fields by PROPERTY ACCESS (which follows the prototype chain, so an
      // anchor built with Object.create(base) is still recognized), each exactly once. A certificate carries
      // subject / subjectPublicKeyInfo -- not name / publicKey / algorithm -- so these read undefined for a
      // cert and it falls through to coerceCert below with its own accessors (e.g. a throwing purposes
      // getter) never touched. entry's own shape is NEVER enumerated (no `in` / ownKeys /
      // getOwnPropertyDescriptor) and the prototype chain is never walked, so a Proxy anchor cannot use an
      // enumeration trap to mutate state mid-normalization -- each field is one plain `get` a hostile accessor
      // answers at most once. (Buffer.isBuffer above invokes getPrototypeOf once; the chain is never WALKED,
      // so a hostile getPrototypeOf cannot be driven into a loop.)
      // publicKey is read FIRST and pinned to a private Buffer IMMEDIATELY, before the name / algorithm and
      // the constraint-map accessors run: those are caller-controlled code that could otherwise overwrite the
      // key Buffer's bytes after the field was read but before it was captured. The declared-vs-SPKI check and
      // initialize()'s state.workingPublicKey both use the private copy. Only a tuple has a byte-source
      // publicKey; a certificate (or any non-tuple) leaves pk null and falls through to coerceCert below.
      ...
      nameRaw = entry.name;
      if (nameRaw && pubRaw) {
        // Tuple-ish: a certificate carries neither name nor publicKey, so reading the tuple's own metadata
        // here never touches a cert's accessors. Snapshot the constraint maps to fresh objects and read the
        // identity passthrough NOW, BEFORE the algorithm accessor (read below) runs -- a top-level `algorithm`
        // getter, or a later nested algorithm.oid / name.rdns getter, must not be able to mutate a captured
        // map (flip a denied purpose to allowed, move a distrust cutoff) between the snapshot and validation.
        // The identity fields (subjectDer, label, mozillaCaPolicy) are the closed producer-derived set beyond
        // the key/metadata contract, read ONCE each so a trust store's own entry round-trips onto the
        // normalized anchor; anything OUTSIDE the closed set is not carried.
        var pRaw = entry.purposes;
        purposesSnap = (pRaw !== null && typeof pRaw === "object" && !intrinsic.isBuffer(pRaw)) ? _snapshotConstraintMap(pRaw) : pRaw;
        var dRaw = entry.distrustAfter;
        distrustSnap = (dRaw !== null && typeof dRaw === "object" && !intrinsic.isBuffer(dRaw)) ? _snapshotConstraintMap(dRaw) : dRaw;
        subjectDerRaw = entry.subjectDer;
        labelRaw = entry.label;
        mozRaw = entry.mozillaCaPolicy;
      }
      alg = entry.algorithm;
  ...
  if (isTuple) {
    ...
    // copied to a fresh array so a later reassignment cannot swap the issuer DN name chaining re-reads) and
    // bytes (the DN DER a cert-derived name carries, for a lossless round-trip). Each is read ONCE as DATA; the
    // name object's own shape is never enumerated. path validation re-reads workingIssuerName.rdns during name
    // chaining, so a stateful rdns accessor that answered the shape check with one array must not hand chaining
    // a different issuer DN; the single read stores that one value. A throwing name / name.rdns accessor is a
    // malformed anchor -> path/bad-input, not a raw throw.
    var nameOut = {};
    try {
    ...
    try {
      ...
      nameHasRdns = !!(nameOut && intrinsic.isArray(nameOut.rdns));
