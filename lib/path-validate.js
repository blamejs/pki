...
// A code-point membership table built from a string of the allowed characters, and the shared scan that
// checks every character of a non-empty string against one -- replacing the hostname-charset regexes
// through one loop (no per-predicate character-class chain).
function _charTable(chars) { var t = []; for (var i = 0; i < chars.length; i++) t[_charCodeAt(chars, i)] = true; return t; }
var _FQDN_TABLE = _charTable("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.-");   // /^[a-z0-9.-]+$/i
var _DIGDOT_TABLE = _charTable("0123456789.");                                                     // /^[0-9.]+$/
function _allCharsIn(s, table) {
  if (s.length === 0) return false;
  for (var i = 0; i < s.length; i++) if (!table[_charCodeAt(s, i)]) return false;
  return true;
}
// /^[0-9]/ : begins with an ASCII digit (a dotted-OID attempt, vs an EKU / purpose name).
function _startsWithDigit(s) { if (s.length === 0) return false; var c = _charCodeAt(s, 0); return c >= 48 && c <= 57; }

var _SCHEME_TABLE = _charTable("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+.-");   // [a-z0-9+.-]
// /^[a-z][a-z0-9+.-]*:\/\/([^/?#]*)/i : the [start, end) range of the authority (up to the first / ? #
// or end), or null when the input is not a scheme "//" URI. Returns a range so the caller slices once
// (the slice that replaces the removed .exec), not a fresh substring here.
function _uriAuthorityRange(uri) {
  var n = uri.length;
  if (n === 0) return null;
  var c0 = _charCodeAt(uri, 0);
  if (!((c0 >= 65 && c0 <= 90) || (c0 >= 97 && c0 <= 122))) return null;                 // scheme starts with a letter
  var p = 1;
...
// /:\d+$/ : the index at which a trailing ":" + digits port starts (to slice off), or s.length when absent.
...
// /^\[(.*)\]$/ -> "$1" : an IPv6 literal's surrounding "[" "]" removed, else the string unchanged.
...
function isFqdnHost(host) {
  ...
  if (!_allCharsIn(h, _FQDN_TABLE)) return false;            // scheme/path/port/IPv6 chars, "@", etc.
  if (_allCharsIn(h, _DIGDOT_TABLE)) return false;           // IPv4 dotted-quad literal
  return true;
}
...
function isSubtreeBaseValid(tag, base) {
  switch (tag) {
    ...
    case 7: return Buffer.isBuffer(base) && (base.length === 8 || base.length === 32);   // iPAddress: address + mask (any BufferSource is normalized to a Buffer in checkedSubtreeSeeds)
...
function checkedSubtreeSeeds(list, optName) {
  ...
  return list.map(function (st) {
    if (!st || typeof st !== "object" || !Number.isInteger(st.tag) || st.tag < 0 || st.tag > 8) {
    ...
    // An iPAddress base (address+mask) may arrive as any BufferSource; normalize it to a
    // private Buffer so the matcher can index it by byte and a caller cannot mutate it once a
    // microtask has passed. The name forms (string / directoryName) are validated as they stand.
    var base = (st.tag === 7 && guard.bytes.isByteSource(st.base))
      ? guard.bytes.snapshotSource(st.base, PathError, "path/bad-input", "an iPAddress subtree base")
      : st.base;
    if (!isSubtreeBaseValid(st.tag, base)) {
      throw E("path/bad-input", "validate: opts." + optName + " entries must be { tag: <GeneralName tag number 0..8>, base: <that form's constraint value> }");
    }
    return { tag: st.tag, base: base };
  });
}
 ...
 */
// Every option pki.path.validate reads. The anchor option is `trustAnchors` (plural), the same
// spelling pki.path.build / pki.cms.verify / pki.cmp.verify use, and it takes a single anchor or a
// non-empty array of them. A one-element array reproduces the single-anchor behavior exactly.
...
// The opt-in Certificate Transparency gate (RFC 6962). When opts.ctLogList is supplied, the target's
// embedded SCTs are verified against the trusted log-list and the caller's CT policy, and the verdict is
// recorded on the target's checks. Fail-closed: a target that declares a CT policy but carries no
// SCT-list extension is refused. Reached only through opts.ctLogList, so every existing caller is
// unaffected. The SCT-list extension OID is looked up through the shared registry, never a literal.
...
async function ctGateCheck(leaf, certs, anchor, ctLogList, ctPolicy, n, at) {
  ...
  if (sctExt == null) return { name: "ct", ok: false, code: "path/ct-required" };
  // The leaf's issuer contributes issuerKeyHash: the certificate that signed it (certs[n-2] in the
  // anchor->target order), or the anchor itself when the target is validated directly beneath it.
  ...
  try {
    ...
    return { name: "ct", ok: false, code: pathCode(e, "path/ct-required") };
  }
  // verifySctList throws only for a mis-shaped caller configuration (a bad ctPolicy threshold value, a
  // ctLogList that is not a parseLogList result): that is the caller's error, surfaced at the path
  // boundary as path/bad-input rather than leaking the ct/* domain code of a layer they did not call.
  // verifySctList throws only for a mis-shaped caller configuration (a bad ctPolicy threshold value, a
  // ctLogList that is not a parseLogList result): that is the caller's error, surfaced at the path
  // boundary as path/bad-input rather than leaking the ct/* domain code of a layer they did not call.
  var verdict;
  ...
  return { name: "ct", ok: verdict.policyOk === true, code: verdict.policyOk ? undefined : "path/ct-policy-not-met" };
}
...
async function validate(path, opts) {
  ...
  // A CT policy without a log-list cannot be enforced -- refuse it at entry rather than silently
  // skipping the CT gate and returning a valid path the caller believes was CT-checked (fail-closed).
  ...
  // Validate the CT policy's KEYS at entry: a misspelled key (e.g. `minSCTs`) would otherwise read as
  // undefined and silently default to the RFC floor of 1, applying a weaker policy than the caller asked
  // for. The values are validated by verifySctList when the gate runs.
  // Validate the whole CT configuration at entry, independent of whether the target carries an SCT
  // extension, so a malformed policy or log-list is always path/bad-input -- never masked as a
  // per-certificate path/ct-required just because the target happens to lack SCTs.
  ...
  // trustAnchors is a single anchor OR a non-empty array of them. Normalize each at the door: a parsed
  // root CERTIFICATE becomes a { name, publicKey, algorithm } tuple, and a mis-shaped tuple fails closed
  // with path/bad-input rather than seeding an undefined working key a self-describing SPKI could still
  // validate against (a soft-verdict fail-open). With more than one anchor, select the one whose subject
  // issued the top path cert (certs[0], anchor->target order); a one-element list reproduces the
  // single-anchor behavior exactly, and no match falls to the first so the name-chaining check below
  // fails closed (an untrusted chain is never validated against a non-matching root).
  ...
  // With more than one anchor, validate the SAME path against each and accept if ANY yields a valid path
  // (the trust-store "does this chain lead to any of my roots" question). Two anchors can share a subject
  // DN with different keys (a rotated or cross-signed root), so choosing by name alone would reject a
  // valid chain when the wrong same-name anchor is tried first: try each, order-independent for validity.
  // Each recursion carries exactly one anchor, so it runs the single-anchor body below (no further fan-out).
  if (anchorList.length > 1) {
    // Normalize the COMPLETE array up front: toAnchor validates each tuple exactly as the single-anchor
    // path does, so a malformed anchor is refused (path/bad-input) regardless of its position, never
    // silently accepted because a valid anchor happened to be tried first.
    var tuples = [];
    for (var ti = 0; ti < anchorList.length; ti++) { tuples[ti] = toAnchor(anchorList[ti], "validate"); }
    // One anchor per recursion, so each runs the single-anchor body below (no further fan-out).
    ...
    // Only an anchor whose subject issued certs[0] can complete this path, so try each such candidate and
    // skip unrelated roots: the cost does not scale with a whole trust store. Same-subject anchors with
    // different keys (a rotated or cross-signed root) ALL match the terminus and are each tried, so validity
    ...
    // No supplied anchor's subject matched the path terminus: the chain leads to none of them. Run one to
    // produce the fail-closed valid:false verdict (the name-chaining check reports it) rather than null.
  ...
  // Opt-in CT gate: verify the target's embedded SCTs and record the verdict on its checks (RFC 6962).
  if (opts.ctLogList != null) {
    ...
    if (target) intrinsic.push(target.checks, ctCheck);
...
// And for a signed OCSPRequest (RFC 6960 sec. 4.1.1), so pki.ocsp.verifyRequest checks a requestor's
// signature through the same one engine that verifies a response, never a second, weaker request path.
    ...
    check: async function (cert, issuer, ctx) {
      ...
      for (var k = 0; k < parsed.length; k++) {
        ...
        if (v.revoked && !revokedResult) {
          revokedResult = { status: "revoked", revocationReason: v.revoked.revocationReason, revocationTime: v.revoked.revocationTime, reason: v.revoked.reason };
        }
 ...
 * @signature  pki.path.verifyOcspResponse(response, cert, issuerCert, time, opts?) -> Promise<{ status, responderAuthorized, signatureValid, matched, thisUpdate, nextUpdate, revocationReason?, revocationTime?, reason }>
 * @since       0.2.22
 * @status      stable
 * @spec        RFC 6960
...
function _verifyOcspParsed(parsedResponse, cert, issuerCert, time, opts) {
  ...
  return builtinVerify({ workingPublicKey: issuerCert.subjectPublicKeyInfo.bytes }, cert).then(function (sig) {
    ...
    return ocspCore.evaluateResponse(parsedResponse, cert, issuerCtx, issuerKeyBits, issuerNameCandidates, time, opts.historicalMode === true).then(function (v) {
      if (v.revoked) return { status: "revoked", responderAuthorized: true, signatureValid: true, matched: true, thisUpdate: v.thisUpdate, nextUpdate: v.nextUpdate, revocationReason: v.revoked.revocationReason, revocationTime: v.revoked.revocationTime, reason: v.revoked.reason };
...
function toAnchor(entry, verb) {
  ...
  try {
    ...
    if (entry && typeof entry === "object" && !Buffer.isBuffer(entry) && !guard.parsed.isRecorded(entry)) {
      ...
      if (guard.bytes.isByteSource(pubRaw)) {
        // snapshotSource, not snapshot: isByteSource accepts all four BufferSource forms (Buffer, any view,
        // DataView, ArrayBuffer), so the normalizer must too -- snapshot accepts only Buffer/Uint8Array and
        // would accept-then-throw on an ArrayBuffer / DataView anchor publicKey.
  ...
  if (isTuple) {
    ...
    try {
      ...
      // Preserve the parsed Name's dn (the rendered "CN=..., O=..." string a parser emits, used to display /
      // identify the selected root), so a pki.trust anchor or a cert-derived name round-trips its full shape --
      // { rdns, dn, bytes } -- through normalization. Only a STRING dn is carried: identity never flows through
      // this display string (name chaining compares rdns / bytes DER under RFC 5280 sec. 7.1), so an object- or
      // otherwise non-string-valued dn -- which could run coercion code in a display consumer -- is dropped, not
      // carried. Captured getter-free like the others (an accessor dn could otherwise mutate a sibling).
    ...
    // Gate on the NORMALIZED key `pk` (a private Buffer from snapshotSource iff publicKey was any of the four
    // BufferSource forms), not the raw `pubRaw` -- Buffer.isBuffer(pubRaw) would reject a Uint8Array / view /
    // ArrayBuffer publicKey that isByteSource + snapshotSource just accepted.
...
async function _aiaFetchOne(uri, aia) {
  ...
  // A binary view from an injected transport (Buffer / Uint8Array / ArrayBuffer / DataView) is re-viewed
  // through the byte guard, as the request path does; only a genuine string body falls back to a latin1
  // decode -- a typed array is never String()-coerced to "48,130,..." garbage that then fails to parse.
...
// would let the two lists drift the first time validate gains an option. Both verbs now spell the
// anchor option `trustAnchors`, so the union carries one key and build applies its own per-candidate.
...
async function build(leaf, opts) {
  ...
  function validateOpts(anchor) {
    ...
    vo.trustAnchors = anchor;   // one anchor per candidate; validate accepts a single anchor or an array
    return vo;
  }
