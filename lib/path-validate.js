...
// The anchor fields path validation READS off a tuple that a caller could legitimately supply as an accessor:
// name (name chaining), purposes and distrustAfter (per-purpose trust). Normalizing a tuple materializes
// these to plain DATA from ONE read each, because validation reads each more than once and a live accessor
// could answer a purpose / distrust check inconsistently. publicKey / algorithm / parameters are NOT here --
// they are derived from the validated SubjectPublicKeyInfo, never copied from the caller. EVERY OTHER field
// is handled by kind, not by an allowlist: an OWN DATA field (subjectDer, label, mozillaCaPolicy, any
// documented or caller data) is preserved so it survives Object.assign / spread / JSON, and an unrelated
// ACCESSOR is dropped UNREAD so no caller getter is evaluated. Only entry's OWN keys are walked -- the
// caller's prototype chain is never traversed, so a Proxy / hostile getPrototypeOf cannot force an unbounded
// walk, and an inherited restriction is still kept because the metadata pins above read through the chain.
var ANCHOR_METADATA_FIELDS = ["name", "purposes", "distrustAfter"];
...
function assertAnchorConstraints(ta, checkPurpose) {
  ...
  // caller and keeps this from being a live-getter multi-read even if a future path skips normalization.
...
// object, reading each own entry ONCE, so a caller getter that runs later during normalization cannot mutate
// the map's entries after it was captured but before validation reads them -- flipping a denied purpose to
// allowed, or deleting a distrust cutoff, on a map validation would otherwise hold by reference. Entry
// enumerability is preserved so the purpose gates (Object.keys / hasOwn) see the same shape.
function _snapshotConstraintMap(m) {
  var out = {};
  var keys = intrinsic.getOwnPropertyNames(m);
  for (var i = 0; i < keys.length; i++) {
    if (keys[i] === "__proto__") continue;
    var v = m[keys[i]];
    // A distrustAfter entry is a Date: copy it BY VALUE, or a later caller getter (algorithm.oid) could
    // setTime() the same Date after the map was captured and move the distrust cutoff. guard.time.instantOf
    // reads the instant through the intrinsic; the fresh Date holds a private copy the caller cannot move.
    if (intrinsic.types.isDate(v)) v = new intrinsic.Date(guard.time.instantOf(v));
    var d = intrinsic.getOwnPropertyDescriptor(m, keys[i]);
    intrinsic.defineProperty(out, keys[i], { value: v, enumerable: d ? d.enumerable : true, configurable: true, writable: true });
  }
  return out;
}
...
function toAnchor(entry, verb) {
  ...
  var isTuple = false, alg = null, pubRaw = null, nameRaw = null, pk = null;
  try {
    if (entry && typeof entry === "object" && !Buffer.isBuffer(entry)) {
      // Read publicKey FIRST and pin it IMMEDIATELY, before the algorithm and name accessors run: any of
      // those is caller-controlled code that could mutate the key Buffer between its read and its capture,
      // so the SPKI check and the bound working key would be a key the caller swapped in after the read.
      // Only a tuple has a byte-source publicKey; a certificate (or any non-tuple) leaves pk null and falls
      // through to coerceCert below. The snapshot copies the bytes read here; the declared-vs-SPKI check and
      // initialize()'s state.workingPublicKey binding both use that private copy, which no later caller code
      // can reach.
    ...
    throw E("path/bad-input", who + ": a trustAnchor name / publicKey / algorithm accessor threw", e);
  }
  if (isTuple) {
    ...
    // Pin each contract metadata field to ONE read into `meta`: the shape check below and the anchor the
    // path ultimately binds MUST see the same value. Reading entry.name for the rdns shape check and AGAIN
    // when storing would let a lying getter pass the check with one object and hand validation another
    // (validate-one, store-another); materializing from `meta` closes that and stores DATA, not a live
    // accessor. A field whose getter throws is a malformed anchor -> path/bad-input, not an uncaught throw.
    var meta = intrinsic.create(null);
    // name was read once into nameRaw at the discriminator. MATERIALIZE it to a fresh plain object of its own
    // fields, each read ONCE, so name.rdns is a stable value: path validation re-reads workingIssuerName.rdns
    // during name chaining, so a stateful rdns accessor that answered the shape check with one array must not
    // hand chaining a different issuer DN (a verdict against a name other than the one checked). Every field
    // is materialized as plain DATA at its original enumerability; a throwing name accessor or a hostile
    // ownKeys / getOwnPropertyDescriptor trap on the name object is a path/bad-input refusal, not a raw throw.
    try {
      var nameOut = {};
      var nsrc = nameRaw !== null && typeof nameRaw === "object" ? nameRaw : {};
      var nkeys = intrinsic.getOwnPropertyNames(nsrc);
      for (var nk = 0; nk < nkeys.length; nk++) {
        var nkey = nkeys[nk];
        if (nkey === "__proto__" || nkey === "rdns") continue;   // rdns handled through the chain below
        var nd = intrinsic.getOwnPropertyDescriptor(nsrc, nkey);
        intrinsic.defineProperty(nameOut, nkey, { value: nsrc[nkey], enumerable: nd ? nd.enumerable : true, configurable: true, writable: true });
      }
      // rdns may be OWN or INHERITED (a prototype getter, or Object.create({ rdns: [...] })) -- the tuple
      // contract does not require it to be own -- so read it through the chain ONCE; own-key enumeration alone
      // would drop an inherited rdns and the shape check would then wrongly reject a valid tuple. Store a fresh
      // array so a later reassignment of the caller's rdns cannot swap the issuer DN that chaining re-reads.
      var rdnsVal = nsrc.rdns;
      if (rdnsVal !== undefined) {
        intrinsic.defineProperty(nameOut, "rdns", { value: intrinsic.isArray(rdnsVal) ? intrinsic.arraySlice(rdnsVal, 0) : rdnsVal, enumerable: true, configurable: true, writable: true });
      }
      meta.name = nameOut;
    } catch (e) { throw E("path/bad-input", who + ": trustAnchor name could not be materialized", e); }
    // purposes and distrustAfter are read once each here, and each is SNAPSHOT to a fresh map: a caller getter
    // that runs LATER during normalization (e.g. algorithm.oid) must not be able to mutate the entries after
    // they are read but before validation uses them -- flipping a denied purpose to allowed, or removing a
    // distrust cutoff, on the map validation would otherwise still hold by reference.
    var mi, mf;
    for (mi = 0; mi < ANCHOR_METADATA_FIELDS.length; mi++) {
      mf = ANCHOR_METADATA_FIELDS[mi];
      if (mf === "name") continue;
      try {
        var mv = entry[mf];
        meta[mf] = (mv !== null && typeof mv === "object" && !intrinsic.isBuffer(mv)) ? _snapshotConstraintMap(mv) : mv;
      }
      catch (e) { throw E("path/bad-input", who + ": trustAnchor " + mf + " could not be read", e); }
    }
    // Read the two NESTED accessors (algorithm.oid and name.rdns) ONCE each, under the typed-error guard.
    // Reading algorithm.oid twice could answer the typeof probe with "string" and the value with a Symbol,
    // leaving algStr a Symbol so the later diagnostic concatenation throws a raw TypeError; a throwing
    // name.rdns getter would likewise escape as a raw throw. One guarded read each -> a malformed nested
    // field is the documented path/bad-input.
    var algStr, nameHasRdns;
    try {
      ...
      nameHasRdns = !!(meta.name && intrinsic.isArray(meta.name.rdns));
    } catch (e) { throw E("path/bad-input", who + ": a trustAnchor algorithm.oid / name.rdns accessor threw", e); }
    ...
    // Return a SELF-CONTAINED anchor: a fresh plain object of OWN DATA only, no live caller accessor.
    //  - Every OWN DATA field of entry is preserved by value -- documented identity / trust-store fields
    //    (subjectDer, label, mozillaCaPolicy, set as own data by their producers) and any caller data survive
    //    Object.assign / spread / JSON, and re-normalizing a ready tuple (anchorFromCert twice) is not lossy
    //    -- WITHOUT an allowlist that has to grow every time a store adds a field. An ACCESSOR that is not a
    //    validation-read contract field is NOT read here, so an unrelated caller getter is never evaluated.
    //    Only entry's OWN keys are walked: normalization does NOT traverse the caller's prototype chain, so a
    //    Proxy / hostile getPrototypeOf cannot drive an unbounded (cyclic) walk. `__proto__` is never named,
    //    so no caller field reaches Object.prototype's setter.
    //  - The validation-read metadata (name, purposes, distrustAfter) is materialized from the single `meta`
    //    read above, as DATA -- and those pins use property access, so an INHERITED restriction (an anchor
    //    built with Object.create(baseAnchor)) is still kept without a chain walk here. Storing DATA is
    //    load-bearing: assertAnchorConstraints reads distrustAfter several times (a truthy gate, a hasOwn
    //    gate, then the value) and _hasPurposeScopedMetadata / the purpose block read purposes at more than
    //    one site, so a live accessor could answer those reads inconsistently -- a restriction to the gates,
    //    {} to the value -- and silently drop a distrust or purpose control.
    ...
    var flat = {};
    try {
      // ownKeys, not getOwnPropertyNames: STRING and SYMBOL own keys both, so an own symbol-keyed data field
      // a store attaches for its own bookkeeping survives (it would round-trip through Object.assign / spread
      // and is inert here, since validation reads only the string contract fields). A symbol can never equal a
      // string contract name or "__proto__", so the skip list still applies exactly to the string fields.
      var keys = intrinsic.ownKeys(entry);
      for (var oi = 0; oi < keys.length; oi++) {
        var nm = keys[oi];
        // Set from the SPKI / materialized below, or never carried: skip so no accessor for them runs here.
        if (nm === "publicKey" || nm === "algorithm" || nm === "parameters" || nm === "__proto__" ||
            nm === "name" || nm === "purposes" || nm === "distrustAfter") continue;
        var dsc = intrinsic.getOwnPropertyDescriptor(entry, nm);
        // Preserve a DATA field by value AND its original enumerability -- a non-enumerable caller field must
        // not be promoted into public result data (Object.keys / spread / JSON). DROP an accessor unread (an
        // unrelated caller getter must not be evaluated, and a live accessor must not survive on the anchor).
        if (dsc && intrinsic.hasOwn(dsc, "value")) {
          intrinsic.defineProperty(flat, nm, { value: dsc.value, enumerable: dsc.enumerable, configurable: true, writable: true });
        }
      }
    } catch (e) {
      // A Proxy anchor whose ownKeys / getOwnPropertyDescriptor trap throws (or a revoked proxy) is a
      // malformed anchor -> the documented path/bad-input refusal, not a raw reflection exception.
      throw E("path/bad-input", who + ": trustAnchor own-property reflection threw", e);
    }
    for (mi = 0; mi < ANCHOR_METADATA_FIELDS.length; mi++) {
      mf = ANCHOR_METADATA_FIELDS[mi];
      if (meta[mf] !== undefined) {
        intrinsic.defineProperty(flat, mf, { value: meta[mf], enumerable: true, configurable: true, writable: true });
      }
    }
