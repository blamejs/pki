...
async function _fetchAiaIssuers(current, aia) {
  ...
  for (var k = 0; k < uris.length; k++) {
    ...
    for (var c = 0; c < certs.length && out.length < aia.maxCertsPerResponse; c++) {   // per-response cert-count cap
      var parsed;
...
async function build(leaf, opts) {
  ...
  if (opts.fetchAia === true) {
    aiaCtx = {
      ...
      maxResponseBytes: guard.limits.cap(opts.maxResponseBytes, "build: opts.maxResponseBytes", constants.LIMITS.PATH_AIA_MAX_RESPONSE_BYTES, { E: E, code: "path/bad-input", min: 1, max: constants.LIMITS.HTTP_MAX_RESPONSE_BYTES }),
  ...
  while (stack.length && !success) {
    ...
    // Opt-in AIA caIssuers fetch on a POOL MISS only (RFC 4158 sec. 7.2 "local before remote"): when no static
    // pool cert name-chains to `current`'s issuer, try to fetch the missing intermediate from `current`'s AIA.
    // A fetched cert joins the SHARED pool (later branches reuse it, bounded by the pool ceiling) and is scored
    // + accepted through the exact same path as a pool cert -- validate() remains the sole accept authority.
    // Fetch only when `current` is NOT anchor-adjacent: a cert whose issuer matches a configured anchor is
    // already terminated by the anchor above (validate ran), so fetching its issuer is wasted work + wasted
    // budget (RFC 4158 sec. 7.2 local-before-remote) -- and burning the budget on such dead ends must never
    // deny a valid sibling path.
    if (aiaCtx && scored.length === 0 && !anchorAdjacent && aiaCtx.fetches < aiaCtx.maxFetches) {
      ...
      for (var fj = 0; fj < fetched.length; fj++) {
        ...
        if (nameMatchSoft(fetched[fj].subject.rdns, current.issuer.rdns)) {
          scored.push({ cand: fetched[fj], score: scoreCandidate(fetched[fj], childAki, anchors, opts.time) });
        }
      }
    }
