...
var OID = {
  ...
  qcStatements: oid.byName("qcStatements"),
};
...
// qcStatements is recognized: RFC 3739 sec. 3.2.6 permits the extension critical (all-or-nothing
// criticality), it carries no path-validation constraint (an informational qualified-certificate
// marker), and a critical instance has its structure validated via the registered decoder.
[OID.basicConstraints, OID.keyUsage, OID.nameConstraints, OID.certificatePolicies,
 ...
 OID.extKeyUsage, OID.cRLDistributionPoints, OID.qcStatements].
...
function validateCriticalExtensionStructure(cert) {
  for (var i = 0; i < cert.extensions.length; i++) {
    ...
    var decoded;
    try { decoded = dec(ext.value); }
    catch (e) { return pathCode(e, "path/bad-extension-value"); }
    // A recognized CRITICAL extension MUST be fully processed (RFC 5280 sec. 6.1.4). qcStatements is
    // recognized, but its decoder preserves an unknown statementId opaque -- so a critical qcStatements
    // carrying a statement the validator cannot process is an unprocessable critical extension: reject it.
    if (ext.oid === OID.qcStatements && Array.isArray(decoded) &&
        decoded.some(function (s) { return s.info && s.info.opaque; })) {
      return "path/unrecognized-critical-extension";
    }
  }
  return null;
}
