  ...
  if (direction === "reverse") {
    ...
    while (!success && rstack.length) {
      ...
      if (nameMatchSoft(leafCert.issuer.rdns, frontierName)) {
        ...
        if (rres.valid) { success = { valid: true, path: rpath, trustAnchor: rframe.anchor, result: rres }; break; }
        if (!bestFail) bestFail = { path: rpath, trustAnchor: rframe.anchor, result: rres };
      }
      if (success || rframe.hop >= maxDepth) continue;
      var rscored = [];
