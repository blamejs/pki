    ...
    if (n.tagClass === "universal" && n.tagNumber === asn1.TAGS.SEQUENCE && n.children) {
      n.children.forEach(function (f) {
        ...
        if (f.tagNumber === 0) {
          ...
          if (HASH_BY_OID[h]) hash = HASH_BY_OID[h];
...
async function validate(path, opts) {
  ...
  // 6.1.5(g) success condition.
  var policyOk = state.explicitPolicy > 0 || state.validPolicyTree !== null;
  if (!policyOk) {
  ...
  return {
    ...
    userConstrainedPolicySet: userConstrainedPolicies(state, n),
  };
}
...
function decodeIdp(ext) {
  ...
  var out = { hasDistributionPoint: false, onlyUser: false, onlyCa: false, onlySomeReasons: null, indirect: false };
