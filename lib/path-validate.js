...
function builtinVerify(state, cert) {
  ...
  try {
    ...
    // complaint and such an issuer key verifies a forged signature -- validate the working key's
    // point before importKey, through the shared gate every EdDSA verify path routes through.
    if (d.eddsa) edwardsPoint.validateSpki(state.workingPublicKey, d.eddsa, PathError, "path/bad-signature");
