...
async function validate(path, opts) {
  ...
  guard.identifier.assertKnownKeys(opts, _VALIDATE_OPTS, E, "path/bad-input",
    ...
    "pki.path.build takes `trustAnchors`): ");
...
// them wholesale. So the accepted set is the UNION, derived from _VALIDATE_OPTS rather than
// retyped, or the two lists drift the first time validate gains an option.
//
...
async function build(leaf, opts) {
  ...
  guard.identifier.assertKnownKeys(opts, _BUILD_OPTS, E, "path/bad-input",
    ...
    "pki.path.validate takes `trustAnchor`): ");
  var leafCert;
  ...
  var forwarded = {};
  // Read the same surface the option check accepted. `Object.keys` here dropped a recognized
  // option that arrived on a prototype or non-enumerably -- `Object.create({ requireRevocation:
  // true })` passed the check and then never reached a single internal validate, so a caller who
  // demanded a determined revocation result could get a valid path without one. The later copy
  // in validateOpts cannot recover what this one never took.
  guard.identifier.readableNames(opts).forEach(function (k) {
    if (!BUILD_ONLY_OPT[k]) forwarded[k] = opts[k];
  });
  function validateOpts(anchor) {
    var vo = {};
    // The same surface the option check accepted, not `Object.keys`. An option carried on a
    // prototype -- `requireRevocation` on a shared defaults object, say -- passed the check as
    // a recognized name and was then dropped here, so each internal validate ran without it and
    // could return a valid path the caller had asked to depend on a determined revocation
    // result. Validating one set and forwarding a smaller one is worse than refusing the bag.
    guard.identifier.readableNames(forwarded).forEach(function (f) { vo[f] = forwarded[f]; });
