...
module.exports = {
  ...
  // @internal -- the shared CRL signature-verify seam. pki.crl.verify composes THIS (as ocsp-verify shares
  // verifyWithSpki) so the CRL producing side and pki.path.crlChecker route through ONE signature engine
  // (algorithm-confusion + EdDSA low-order + composite gates), never a second parallel verifier.
  verifyCrlSignature: verifyCrlSignature,
