...
async function validate(path, opts) {
  ...
  // Through the same door `build` uses. A claimed-parsed certificate that is not a COMPLETE one was
  // passed straight into the sec. 6.1 walk here while build refused it -- one rule, two doors of the
  // same module, and the permissive one is the one an operator calls directly.
  var certs = path.map(function (c, ci) {
    return guard.parsed.accept(c, "certificate", x509.parse, E, "path/bad-input", "validate: path[" + ci + "]");
  });
  var n = certs.length;
...
function crlChecker(crls, opts) {
  var parsed = (crls || []).map(function (c, ci) {
    return guard.parsed.accept(c, "crl", crl.parse, E, "path/bad-input", "crlChecker: crls[" + ci + "]");
  });
...
function verifyOcspResponse(response, cert, issuerCert, time, opts) {
  var parsedResponse;
  try { parsedResponse = _ocspFromBytes(response); } catch (e) { return Promise.reject(e); }
  return _verifyOcspParsed(parsedResponse, cert, issuerCert, time, opts);
}
...
// The verification over a response this module has ALREADY parsed from bytes -- what crlChecker's
// OCSP sibling and pki.ocsp.verify call, each having parsed the caller's own bytes first.
...
// parsed (and its typed error normalized by the caller). The completeness rule the
// build search and the validate hand-off both depend on lives in guard-parsed, so
// the two doors of this module cannot answer differently -- which they did, and
// which is exactly the failure the rule exists to prevent.
function coerceCert(input) {
  return guard.parsed.accept(input, "certificate", x509.parse, E, "path/bad-input", "build: an input");
}
...
module.exports = {
  ...
  // @internal -- the already-parsed arm, for a sibling module that has parsed the caller's bytes
  // itself (pki.ocsp.verify). Not on pki.path.*; the public verb is the byte door above.
  _verifyOcspParsed: _verifyOcspParsed,
