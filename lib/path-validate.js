...
async function build(leaf, opts) {
  ...
  while ((stack.length || deferredAia.length) && !success) {
    if (!stack.length) {
      ...
      if (aiaCtx && aiaCtx.fetches < aiaCtx.maxFetches) {
        ...
        for (var fj = 0; fj < fetched.length; fj++) {
          ...
          if (nameMatchSoft(fetched[fj].subject.rdns, fbCur.issuer.rdns)) {
            fscored.push({ cand: fetched[fj], score: scoreCandidate(fetched[fj], fAki, anchors, opts.time) });
          }
        }
        considered += _pushCandidates(fb, fscored, stack, counter);
      }
      continue;
    }
    var frame = stack.pop();
    ...
    if (aiaCtx && frame.hop < maxDepth && aiaCtx.fetches < aiaCtx.maxFetches) {
      deferredAia.push({ chain: frame.chain, hop: frame.hop, keys: frame.keys });
    }
