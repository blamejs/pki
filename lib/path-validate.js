...
function checkedSubtreeSeeds(list, optName) {
  ...
  for (var si = 0; si < list.length; si++) {
    intrinsic.push(seedsOut, (function (st) {
...
async function validate(path, opts) {
  ...
  if (anchorList.length > 1) {
    var preNc = _precaptureAnchorNameConstraints(anchorList, "validate");
    var tuples = [];
    for (var ti = 0; ti < anchorList.length; ti++) {
      tuples[ti] = toAnchor(anchorList[ti], "validate");
...
async function build(leaf, opts) {
  ...
  var buildPreNc = _precaptureAnchorNameConstraints(opts.trustAnchors, "build");
  var anchors = [];
  var buildAnchorList = opts.trustAnchors;
  for (var bai = 0; bai < buildAnchorList.length; bai++) {
    var bAnchor = toAnchor(buildAnchorList[bai], "build");
    ...
    intrinsic.push(anchors, bAnchor);
  }
