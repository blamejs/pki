...
async function build(leaf, opts) {
  ...
  while (stack.length && !success) {
    ...
    // The fallback is gated on `success` being unset (guaranteed by the continue above), NOT on the issuer name
    // failing to match an anchor: an issuer DN that matches an anchor whose KEY did not validate this chain (a CA
    // key rollover -- the real same-DN, different-key intermediate is missing and reachable only via AIA) still
    // needs the fetch. A validated anchor path has already short-circuited via `success`, so no wasted fetch.
