...
// An EncryptedPrivateKeyInfo root is a SEQUENCE of exactly two children — a
// SEQUENCE (encryptionAlgorithm) then an OCTET STRING (encryptedData). Disjoint
// from PrivateKeyInfo (whose first child is an INTEGER) and the signed envelopes
// (three children).
function matchesEncrypted(root) {
  ...
  if (!(k[0].tagClass === "universal" && k[0].tagNumber === TAGS.SEQUENCE)) return false;
  if (!(k[1].tagClass === "universal" && k[1].tagNumber === TAGS.OCTET_STRING)) return false;
  return true;
}
