 ...
 * @nav        Schema
 * @title      PKCS#8
 * @fullname   PKCS#8 private keys, encrypted and unencrypted
 * @order      140
 * @slug       pkcs8
 *
 * @intro
...
var PRIVATE_KEY_INFO = schema.seq([
  ...
  build: function (m) {
    ...
    return {
      ...
      privateKey: m.fields.privateKey.value,
...
var ENCRYPTED_PRIVATE_KEY_INFO = schema.seq([
  ...
  build: function (m) {
    return {
      ...
      encryptedData: m.fields.encryptedData.value,
    };
  },
});
 ...
 * @signature  pki.schema.pkcs8.parse(input, caps?) -> privateKey
 * @since      0.1.9
 * @status     stable
 ...
 * @signature  pki.schema.pkcs8.parseEncrypted(input, caps?) -> encrypted
 * @since      0.1.9
 * @status     stable
 ...
 * @example
 *   var pair = await pki.key.generate("Ed25519");
 *   var der = await pki.key.encrypt(await pki.key.export(pair.privateKey), "s3cr3t");
 ...
 * @example
 *   var pair = await pki.key.generate("Ed25519");
 *   var pemText = await pki.key.export(pair.privateKey, { format: "pem" });
...
/** @internal RFC 5958 sec. 3: `EncryptedPrivateKeyInfo ::= SEQUENCE { encryptionAlgorithm
 * AlgorithmIdentifier, encryptedData OCTET STRING }`. Exactly two children where a
 * `PrivateKeyInfo` has three or more, and an `AlgorithmIdentifier` where that one has an INTEGER
 * version, so the two are told apart by their first child and neither is read as the other. */
function matchesEncrypted(root) {
  var TAGS = asn1.TAGS;
  var k = pkix.rootSequenceChildren(root, 2, 2);
  if (!k) return false;
  if (!schema.isUniversal(k[0], TAGS.SEQUENCE) || !k[0].children || k[0].children.length < 1) return false;
  if (!schema.isUniversal(k[0].children[0], TAGS.OBJECT_IDENTIFIER)) return false;
  return schema.isUniversal(k[1], TAGS.OCTET_STRING) && !k[1].constructed;
}


/** @internal `ctx` is the walk context of the parse that reached this key, when another format
 *  carries it. Only its caps and budget cross over: the key is still read in this namespace, so a
 *  fault in it keeps the PKCS#8 error class and code. */
function walkPrivateKeyInfo(node, ctx) {
  return schema.walk(PRIVATE_KEY_INFO, node, schema.withResources(NS, ctx)).result;
}
function walkEncryptedPrivateKeyInfo(node, ctx) {
  return schema.walk(ENCRYPTED_PRIVATE_KEY_INFO, node, schema.withResources(NS, ctx)).result;
}
...
module.exports = {
  ...
  pemEncode: pemEncode,
  matches: matches,
  matchesEncrypted: matchesEncrypted,
