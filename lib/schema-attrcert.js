...
var HOLDER = schema.seq([
  ...
  /** @internal Sec. 4.2.2 states three requirements for the holder's baseCertificateID issuer that
   *  sec. 4.2.3 states in the same words for the AC issuer: "The PKC issuer MUST have a non-empty
   *  distinguished name that is to be present as the single value of the
   *  holder.baseCertificateID.issuer construct in the directoryName field." V2_FORM's build enforces
   *  the AC issuer's copy, so the holder's is enforced here. It is checked in THIS build rather than
   *  in ISSUER_SERIAL, because that schema also reads a targetInformation targetCertificate, which
   *  sec. 4.2.2 says nothing about. */
  build: function (m, ctx) {
    var base = m.fields.baseCertificateID.present ? m.fields.baseCertificateID.value.result : null;
    if (base !== null) {
      var names = base.issuer.names;
      if (names.length !== 1) {
        throw ctx.E("attrcert/bad-holder", "the baseCertificateID issuer must contain one and only one GeneralName (RFC 5755 sec. 4.2.2)");
      }
      if (names[0].tagNumber !== 4) {
        throw ctx.E("attrcert/bad-holder", "the baseCertificateID issuer GeneralName must be a directoryName [4] (RFC 5755 sec. 4.2.2)");
      }
      var dirName = pkix.decodeNested(names[0].bytes, ctx, "the baseCertificateID issuer directoryName").children[0];
      if (!dirName.children || dirName.children.length < 1) {
        throw ctx.E("attrcert/bad-holder", "the baseCertificateID issuer directoryName must be a non-empty distinguished name (RFC 5755 sec. 4.2.2)");
      }
    }
    return {
      baseCertificateID: base,
...
var V2_FORM = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    var dirName = pkix.decodeNested(issuerName.names[0].bytes, ctx, "the issuerName directoryName").children[0];
    if (!dirName.children || dirName.children.length < 1) {
...
var ATTRIBUTE_CERTIFICATE = pkix.signedEnvelope(NS, ACINFO, {
  ...
  build: function (e, ctx) {
    ...
    attributes.forEach(function (a) {
      ...
      a.decoded = a.values.map(function (v) { return dec ? dec(v, ctx) : { opaque: true, bytes: v }; });
    });
    ...
    extensions.forEach(function (x) {
      ...
      x.decoded = dec ? dec(x.value, ctx) : { opaque: true, bytes: x.value };
    });
 ...
 * @signature  pki.schema.attrcert.parse(input, caps?) -> attributeCertificate
 * @since      0.1.14
 * @status     stable
 * @spec       RFC 5755, X.509
 ...
 */
var PARSE_OPTS = intrinsic.assign(intrinsic.create(null), { pemLabel: "ATTRIBUTE CERTIFICATE", PemError: PemError, ErrorClass: AttrCertError, prefix: "attrcert", what: "attribute certificate", ns: NS });
...
function _parseAc(input, callerOpts) {
  return pkix.runParseWith(input, PARSE_OPTS, callerOpts, function (root, ns) {
...
function parseV1(input, callerOpts) {
  return pkix.runParseWith(input, PARSE_OPTS, callerOpts, function () {
 ...
 * type, or an explicit `null` to accept a block of any type. The text holds one
 * object: a file of several is refused with `pem/multiple-blocks` and read with
...
function readGeneralName(der, code) {
  // allow:bare-decode reached from the builder, not from a parse, so no parse context carries caps here.
  return _oneGN(asn1.decode(der), code || "attrcert/bad-input");
}

function _svceAuthInfo(authInfoAllowed, code) {
  return function (buf, ctx) {
    var kids = _seqChildren(buf, code, "SvceAuthInfo", ctx);
...
function _ietfAttrSyntax(buf, ctx) {
  var C = "attrcert/bad-ietf-attr";
  var kids = _seqChildren(buf, C, "IetfAttrSyntax", ctx);
  ...
  if (_isCtx(kids[0], 0)) { policyAuthority = schema.walk(_GNS_0, kids[0], schema.withResources(NS, ctx)).result; i = 1; }
...
function _role(buf, ctx) {
  var C = "attrcert/bad-role";
  var kids = _seqChildren(buf, C, "RoleSyntax", ctx);
  ...
  if (_isCtx(kids[0], 0)) { roleAuthority = schema.walk(_GNS_0, kids[0], schema.withResources(NS, ctx)).result; i = 1; }
  var rn = kids[i];
...
function _decodeClassList(node, C) {
  ...
  // allow:registry-table-inherits-object-prototype -- the classList record an operator reads by field name, whose keys are the fixed _CLASSLIST_NAMES; the omitted-classList default at the foot of this file builds the same shape
...
function _clearance(buf, ctx) {
  var C = "attrcert/bad-clearance";
  var kids = _seqChildren(buf, C, "Clearance", ctx);
...
function _acAuditIdentity(buf, ctx) {
  var C = "attrcert/bad-audit-identity";
  var n = _decodeTop(buf, C, "AuditIdentity", ctx);
...
function _noRevAvail(buf, ctx) {
  var C = "attrcert/bad-no-rev-avail";
  var n = _decodeTop(buf, C, "NoRevAvail", ctx);
...
function _seqOfTargets(buf, C, allowTargetCert, ctx) {
  var kids = _seqChildren(buf, C, "SEQUENCE OF Targets", ctx);
...
function _targetInformation(buf, ctx) { return _seqOfTargets(buf, "attrcert/bad-targets", true, ctx); }
function _acProxying(buf, ctx) { return _seqOfTargets(buf, "attrcert/bad-proxy-info", false, ctx); }
...
function _aaControls(buf, ctx) {
  var C = "attrcert/bad-aa-controls";
  var kids = _seqChildren(buf, C, "AAControls", ctx);
...
/** @internal The three RFC 5280 extensions RFC 5755 sec. 4.3.3 to 4.3.5 profile for an AC are read
 *  through the certificate decoders, so a relying party running the sec. 6 "pointer in AC" scheme
 *  reads the distribution point and the OCSP location off the parse rather than off raw bytes. */
_AC_EXT_DECODERS[_Oav("authorityKeyIdentifier")] = pkix.scopedCertDecoder(NS, "authorityKeyIdentifier");
_AC_EXT_DECODERS[_Oav("authorityInfoAccess")] = pkix.scopedCertDecoder(NS, "authorityInfoAccess");
_AC_EXT_DECODERS[_Oav("cRLDistributionPoints")] = pkix.scopedCertDecoder(NS, "cRLDistributionPoints");
...
/** @internal The criticality RFC 5755 fixes for each extension it profiles for an attribute
 *  certificate: sec. 4.3's six, and the ProxyInfo whose criticality sec. 7.2 fixes the same way. The
 *  AC signer refuses to write an extension against this table, the extension door reports one that
 *  arrived against it, and the RFC 5755 lint profile takes its own view of it, from this one row set.
 *  Each row carries the clause number and the citation is rendered from it, so a reader cannot be
 *  given a clause that names a different section than the rule it states.
 *  An extension whose clause says MAY has no row, because a MAY fixes no criticality. AAControls is
 *  that case: sec. 7.4 states "criticality MAY be TRUE" and places it in "CA and AC issuer PKCs",
 *  so for an attribute certificate it is one of the extensions sec. 4.2.9 governs, where a
 *  non-critical one conforms and a critical one does not. That is profile membership, which the lint
 *  profile answers from the keys of this table, and not a criticality to enforce here. */
var AC_FIXED_CRITICALITY_ROWS = [
...
function extensionCriticality(oidRegistry) {
  var table = intrinsic.create(null);
  intrinsic.forEach(AC_FIXED_CRITICALITY_ROWS, function (r) {
    table[oidRegistry.byName(r[0])] =
      { name: r[0], critical: r[1], clause: r[2], citation: "RFC 5755 sec. " + r[2] };
  });
  return table;
}

/** @internal The attribute-certificate scope. The table is this module's own, passed to the shared
 *  door rather than reached for from it, because a registry in `schema-pkix.js` that required this
 *  module would invert the dependency. */
var AC_SCOPES = intrinsic.assign(intrinsic.create(null), {
  "attribute-certificate": {
    /** @internal Every decoder in this table takes the walk context and threads it into its own
     *  decode, so the one table serves every call and the caller's caps still reach the value. */
    decoders: function () { return _AC_EXT_DECODERS; },
    criticality: extensionCriticality(oid),
  },
});

var DOOR = pkix.extensionDoor({
  ns: NS, ErrorClass: AttrCertError, key: "attrcert", kind: "attributeCertificate",
  what: "the attribute certificate", parse: parse, scopes: AC_SCOPES,
  defaultScope: "attribute-certificate",
  segments: function (ac) { return [{ list: ac.extensions || [], scope: "attribute-certificate" }]; },
});

/**
 * @primitive  pki.schema.attrcert.decodeExtensions
 * @signature  pki.schema.attrcert.decodeExtensions(acOrBytes, opts?) -> rows
 * @since      0.8.11
 * @status     stable
 * @spec       RFC 5755 sec. 4.2.9, RFC 5755 sec. 4.3
 * @related    pki.schema.attrcert.parse, pki.schema.x509.decodeExtensions
 *
 * Read an attribute certificate's extensions as decoded records. The argument is the object `parse`
 * returned or the bytes `parse` takes.
 *
 * Each row is `{ oid, name, critical, value, scope, index, containerIndex, state, decoded, code,
 * profile }`, with `scope` `"attribute-certificate"` and `containerIndex` `null`. `state` is the
 * discriminator, and `profile` reports the criticality RFC 5755 fixes for the extension.
 *
 * RFC 5755 sec. 5 item 7 has a verifier reject an AC carrying a critical extension it does not
 * recognize. That is the caller's rule to apply, and the two facts it needs, `critical` and
 * `state`, are on the row.
 *
 * @opts
 ...
 *   var pair = await pki.key.generate("Ed25519");
 *   var key = await pki.key.export(pair.privateKey);
 *   var aaCert = await pki.x509.sign({ subject: "Example AA", subjectPublicKey: await pki.key.export(pair.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z") }, { key: key });
 *   var acDer = await pki.attrcert.sign({ holder: { entityName: [{ directoryName: "CN=Holder" }] },
 *     attributes: { role: { roleName: "https://example.test/role/admin" } }, serialNumber: 1n,
 *     notBeforeTime: new Date("2026-01-01T00:00:00Z"), notAfterTime: new Date("2027-01-01T00:00:00Z"),
 *     extensions: { noRevAvail: true } }, { cert: aaCert, key: key });
 *   var rows = pki.schema.attrcert.decodeExtensions(acDer);
 *   rows.filter(function (r) { return r.critical && r.state === "unrecognized"; }).length;  // -> 0
 */
var decodeExtensions = DOOR.decodeExtensions;

/**
 * @primitive  pki.schema.attrcert.decodeExtension
 * @signature  pki.schema.attrcert.decodeExtension(ext, opts?) -> row
 * @since      0.8.11
 * @status     stable
 * @spec       RFC 5755 sec. 4.3
 * @related    pki.schema.attrcert.decodeExtensions
 *
 * Read one attribute-certificate extension record as a decoded row, at scope
 * `"attribute-certificate"`. The argument is `{ oid, name, critical, value }`.
 *
 * @opts
 *   - `strict` (boolean) -- throw the decoder's own typed error instead of reporting an
 *     undecodable value.
 ...
 *     notBeforeTime: new Date("2026-01-01T00:00:00Z"), notAfterTime: new Date("2027-01-01T00:00:00Z"),
 *     extensions: { noRevAvail: true } }, { cert: aaCert, key: key });
 *   var ac = pki.schema.attrcert.parse(acDer);
 *   pki.schema.attrcert.decodeExtension(ac.extensions[0]).profile.citation;  // -> "RFC 5755 sec. 4.3.6"
 */
...
module.exports = {
  ...
  extensionCriticality: extensionCriticality,
