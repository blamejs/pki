...
var DIGESTED_OBJECT_TYPE = schema.decode(function (n, ctx) {
  ...
  if (!Object.prototype.hasOwnProperty.call(ODT_NAMES, k)) {
    throw ctx.E("attrcert/bad-digested-object-type", "digestedObjectType " + k + " is not in {0,1,2} (RFC 5755 §4.1)");
  }
...
var OBJECT_DIGEST_INFO = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    var otidPresent = m.fields.otherObjectTypeID.present;
    // otherObjectTypeID names the type of a non-certificate object, so it is
    // meaningful only when digestedObjectType is otherObjectTypes(2) (RFC 5755 §4.1).
    if (otidPresent && t.code !== 2) {
      throw ctx.E("attrcert/bad-object-digest-info", "otherObjectTypeID is present only when digestedObjectType is otherObjectTypes(2) (RFC 5755 §4.1)");
    }
    return {
      digestedObjectType: t,
      otherObjectTypeID: otidPresent ? m.fields.otherObjectTypeID.value : null,
 ...
 */
var parse = pkix.makeParser({ pemLabel: null, PemError: PemError, ErrorClass: AttrCertError, prefix: "attrcert", what: "attribute certificate", topSchema: ATTRIBUTE_CERTIFICATE, ns: NS });
...
function parseV1(input) {
  var opts = { pemLabel: null, PemError: PemError, ErrorClass: AttrCertError, prefix: "attrcert", what: "attribute certificate" };
  pkix.decodeRoot(pkix.coerceToDer(input, opts), opts);
  throw new AttrCertError("attrcert/legacy-v1-not-supported", "AttributeCertificateV1 (X.509-1997) is obsolete and not parsed by this build (RFC 5755 §1)");
}
