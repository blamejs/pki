...
var _CERT_EXT_DECODERS = pkix.certExtensionDecoders(NS).byOid;
/** @internal The certificate decoders close over the namespace they were built with, so a parse
 *  that named caps gets its own table and every decode below these three reads the caller's
 *  figure. A caller outside a parse passes no context and reuses the module's table. */
function _certExtDecoder(name) {
  var key = _Oav(name);
  return function (buf, ctx) {
    return (ctx ? pkix.certExtensionDecoders(schema.withResources(NS, ctx)).byOid : _CERT_EXT_DECODERS)[key](buf);
  };
}
_AC_EXT_DECODERS[_Oav("authorityKeyIdentifier")] = _certExtDecoder("authorityKeyIdentifier");
_AC_EXT_DECODERS[_Oav("authorityInfoAccess")] = _certExtDecoder("authorityInfoAccess");
_AC_EXT_DECODERS[_Oav("cRLDistributionPoints")] = _certExtDecoder("cRLDistributionPoints");
