...
// AuditIdentity ::= OCTET STRING (sec. 4.3.1) -- a non-empty opaque audit tag.
function _acAuditIdentity(buf) {
  ...
  var v = Buffer.concat([asn1.read.octetString(n)]);
  if (!v.length) throw NS.E(C, "AuditIdentity must be a non-empty OCTET STRING (RFC 5755 sec. 4.3.1)");
  return v;
}

// NoRevAvail ::= NULL / absent (sec. 4.3.6) -- the extnValue is empty or an encoded NULL.
function _noRevAvail(buf) {
  var C = "attrcert/bad-no-rev-avail";
  if (!buf || buf.length === 0) return { noRevAvail: true };
  var n = _decodeTop(buf, C, "NoRevAvail");
  try { asn1.read.nullValue(n); } catch (e) { throw NS.E(C, "NoRevAvail must be empty or a NULL", e); }
...
function _acTarget(node, C) {
  ...
  if (_isCtx(node, 2)) return _acTargetCert(node, C);
...
function _acDecodeTargets(node, C) {
  ...
  return node.children.map(function (t) { return _acTarget(t, C); });
}
// targetInformation ::= Targets (sec. 4.3.2).
function _targetInformation(buf) {
  var C = "attrcert/bad-targets";
  return _acDecodeTargets(_decodeTop(buf, C, "TargetInformation"), C);
}
// ProxyInfo ::= SEQUENCE OF Targets (sec. 7.4) -- reuses the same Targets decoder per element.
function _acProxying(buf) {
  var C = "attrcert/bad-proxy-info";
  var kids = _seqChildren(buf, C, "ProxyInfo");
  if (!kids.length) throw NS.E(C, "ProxyInfo must be a non-empty SEQUENCE OF Targets");
  return kids.map(function (targetsNode) { return _acDecodeTargets(targetsNode, "attrcert/bad-targets"); });
}
