...
var DIGESTED_OBJECT_TYPE = schema.decode(function (n, ctx) {
  ...
  return { code: _Number(v), name: ODT_NAMES[k] };
});
...
// Recording, because pki.attrcert.verify reaches a verdict from this result: the signed byte range,
// the holder the privileges attach to and the attributes themselves are separate properties here,
// so the record is what binds them to one another.
...
// readGeneralName(der) -> { tagNumber, value }, the shape every GeneralName in a parsed attribute
// certificate is surfaced in.
//
// @internal -- exported so a verifier comparing a caller's GeneralName against one this module
// decoded puts BOTH sides through this one decoder. A second decoder would be a second set of
// per-form conventions, and a comparison whose two halves were produced by different rules is a
// comparison of the rules rather than of the names.
...
function _svceAuthInfo(authInfoAllowed, code) {
  return function (buf) {
    ...
    if (kids.length === 3) {
      ...
      authInfo = _bufferConcat([asn1.read.octetString(kids[2])]);
    }
...
function _ietfAttrSyntax(buf) {
  ...
  var values = valuesNode.children.map(function (v) {
    if (_isU(v, TAGS.OCTET_STRING)) { kinds.octets = true; return { kind: "octets", value: _bufferConcat([asn1.read.octetString(v)]) }; }
  ...
  return { policyAuthority: policyAuthority, values: values, homogeneous: _keys(kinds).length === 1 };
}
...
function _decodeClassList(node, C) {
  ...
  for (var bit = 0; bit < total; bit++) {
    ...
    if (bit < _CLASSLIST_NAMES.length) { _push(names, _CLASSLIST_NAMES[bit]); flags[_CLASSLIST_NAMES[bit]] = true; }
    else _push(reserved, bit);
  }
...
function _securityCategory(node, C) {
  ...
  return { type: type, valueBytes: _bufferConcat([valueNode.children[0].bytes]) };
}
