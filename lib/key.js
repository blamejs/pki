...
function _compositeKemOidOf(der) {
  var algOid;
  try {
    /** @internal The AlgorithmIdentifier OID only, read without the strict OneAsymmetricKey rules. A
     * composite key that breaks one of those rules is still a composite key, and the module that owns
     * the algorithm is the one that should say so: parsing strictly here would send a composite key
     * carrying parameters to the runtime decoder, which reports that it cannot read the key rather
     * than that the parameters are not allowed (draft sec. 5.3). */
  ...
  return typeof algOid === "string" && compositeKem.isCompositeKem(algOid) ? algOid : null;
}
...
function _pssRestrictionOf(key, isPss) {
  ...
  for (var i = 0; i < _PSS_PINS.length; i++) {
    ...
    if (Object.prototype.hasOwnProperty.call(details, name) && details[name] != null) out[name] = details[name];
  }
  return out;
}
...
async function correspondsTo(privateKey, spkiDer) {
  ...
  try {
    ...
    var signKey = how.options ? Object.assign(Object.create(null), { key: priv }, how.options) : priv;
    var verifyKey = how.options ? Object.assign(Object.create(null), { key: pub }, how.options) : pub;
