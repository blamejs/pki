...
var schemaPkcs1 = require("./schema-pkcs1");
var schemaSec1 = require("./schema-sec1");
...
/** @internal The curve each SEC1 registry name is known by in WebCrypto, which names its curves
 * the way RFC 5480 sec. 2.1.1.1 writes them and not the way the ASN.1 registry does. A curve with
 * no row is one the engine cannot take, and saying so names the curve. */
var WEBCRYPTO_CURVE = _assign(_create(null), {
  "prime256v1": "P-256", "secp384r1": "P-384", "secp521r1": "P-521",
});

/** @internal The byte doors the two named formats come in through. Each keeps its own PEM label,
 * so an `EC PRIVATE KEY` block handed in as `pkcs1` is refused by label before it is read. */
var _PKCS1_ENTRY = _assign(_create(null), { pemLabel: "RSA PRIVATE KEY", PemError: PemError, ErrorClass: KeyError, prefix: "key" });
var _SEC1_ENTRY = _assign(_create(null), { pemLabel: "EC PRIVATE KEY", PemError: PemError, ErrorClass: KeyError, prefix: "key" });
var _PKCS1_PUBLIC_ENTRY = _assign(_create(null), { pemLabel: "RSA PUBLIC KEY", PemError: PemError, ErrorClass: KeyError, prefix: "key" });

/** @internal RFC 8017 App. A.1 gives the PKCS#1 encoding two structures, `RSAPrivateKey` and
 * `RSAPublicKey`, with an armor label each. `opts.format` names the encoding, so both labels are
 * that encoding: the one on the armor says which structure to expect, and the structure the bytes
 * carry has to agree, so a key armored under the label of the half it is not is refused instead of
 * being read as the other half. Bare DER carries no label and is classified by its structure. */
function _pkcs1Der(input) {
  var armored = typeof input === "string" ||
    (guard.bytes.isByteSource(input) && pkix.isPemArmor(guard.bytes.source(input, KeyError, "key/bad-input", "import")));
  if (!armored) return pkix.coerceToDer(input, _PKCS1_ENTRY);
  var der, saidPublic;
  try { der = pkix.coerceToDer(input, _PKCS1_ENTRY); saidPublic = false; }
  catch (e) {
    if (!e || e.code !== "pem/label-mismatch") throw e;
    der = pkix.coerceToDer(input, _PKCS1_PUBLIC_ENTRY);
    saidPublic = true;
  }
  if (_looksLikeRsaPublic(der) !== saidPublic) {
    throw _err("key/bad-input", "the PEM label says this is an RSA " + (saidPublic ? "PUBLIC" : "PRIVATE") +
      " KEY and the bytes carry the other half (RFC 8017 App. A.1)");
  }
  return der;
}

/** @internal PKCS#1 and SEC1 both carry a key and no algorithm identifier, so importing one means
 * wrapping it in the structure that does. RFC 5958 sec. 2 for the private half, RFC 5280
 * sec. 4.1.2.7 for the public one. The inner bytes are carried through unchanged: what the caller
 * handed in is what the engine reads. */
function _pkcs1ToPkcs8(der) {
  return b.sequence([b.integer(0n),
    b.sequence([b.oid(O("rsaEncryption")), b.nullValue()]),
    b.octetString(der)]);
}
function _pkcs1ToSpki(der) {
  return b.sequence([b.sequence([b.oid(O("rsaEncryption")), b.nullValue()]), b.bitString(der, 0)]);
}
function _sec1ToPkcs8(der, curveOid) {
  return b.sequence([b.integer(0n),
    b.sequence([b.oid(O("ecPublicKey")), b.oid(curveOid)]),
    b.octetString(der)]);
}
 ...
 *
 * `format` also writes the encodings an operator's other tools read: `pkcs1` for the RFC 8017 App. A.1
 * `RSAPrivateKey` or `RSAPublicKey`, `sec1` for the RFC 5915 `ECPrivateKey`, and `jwk` for an RFC 7517
 * JSON Web Key. The two DER forms are pulled out of the wrapper the engine wrote, so the inner bytes are
 * the engine's own; a SEC1 key is given the `[0] parameters` field the standalone form needs, which
 * RFC 5915 sec. 3 omits while the key sits inside a PKCS#8. A key whose algorithm the encoding does not
 * describe, an Ed25519 key asked for as PKCS#1 for instance, is refused with `key/bad-input`.
 *
 * @opts
 *   - `format` (string) -- `der` (default), `pem`, `pkcs1`, `sec1` or `jwk`.
...
async function export_(key, opts) {
  ...
  if (fmt === "jwk") {
    try {
      return await webcrypto.exportAnyKey(key, _err, "key/bad-input", "jwk");
    } catch (e) {
      if (e && e.isPkiError) throw e;
      throw _err("key/bad-input", "exporting the key as a JWK failed", e);
    }
  }
  ...
  if (fmt === "pkcs1") return _toPkcs1(der, key);
  if (fmt === "sec1") return _toSec1(der, key);
  throw _err("key/bad-input", "unsupported format " + guard.text.showValue(opts.format) + " (der / pem / pkcs1 / sec1 / jwk)");
}
 ...
 *
 * `opts.format` reads an encoding that is not PKCS#8 or SubjectPublicKeyInfo: `pkcs1` for the RFC 8017
 * App. A.1 `RSA PRIVATE KEY` and `RSA PUBLIC KEY`, `sec1` for the RFC 5915 `EC PRIVATE KEY`, and `jwk`
 * for an RFC 7517 JSON Web Key. Neither DER encoding names an algorithm, so `opts.algorithm` is required
 * with both and nothing is inferred; the key is wrapped in the structure that does name one and handed to
 * the engine with its bytes unchanged. A multi-prime RSA key reads as a structure through
 * `pki.schema.pkcs1.parse` and is refused here with `key/unsupported-key`.
 *
 * @opts
 *   - `algorithm` (string | object) -- the WebCrypto algorithm (required for RSA / EC / pkcs1 / sec1 / jwk; overrides inference).
 *   - `format` (string) -- `pkcs1`, `sec1` or `jwk`; omit to read PKCS#8 or SubjectPublicKeyInfo.
 ...
 */
var _IMPORT_OPTS = _assign(_create(null), { algorithm: 1, extractable: 1, password: 1, usages: 1, format: 1 });
var _IMPORT_FORMATS = _assign(_create(null), { pkcs1: 1, sec1: 1, jwk: 1 });

/** @internal Which named encoding the caller asked for, or null when they named none and the door
 * detects PKCS#8 or SubjectPublicKeyInfo as it always has. A value outside the set is refused
 * here, before any byte of the input is read. */
function _namedImportFormat(value) {
  if (value == null) return null;
  var name = typeof value === "string" ? guard.text.keyOf(value) : null;
  if (name === null || _IMPORT_FORMATS[name] !== 1) {
    throw _err("key/bad-input", "opts.format is pkcs1, sec1 or jwk; leave it out to read PKCS#8 or SubjectPublicKeyInfo. Got " + guard.text.showValue(value));
  }
  return name;
}
...
async function import_(input, opts) {
  ...
  if (_namedImportFormat(opts.format) !== null) return await _importNamedFormat(input, opts);
...
function _dhAsPkcs3(der, at, what) {
  ...
  /** @internal A field the codec refuses to read as the type the structure states is a malformed
   * key, reported as one rather than as the codec's own complaint. */
  var dom, y = null;
  try {
    dom = x942Domain(algNode.children[1], KeyError, "key/bad-input", what);
    if (at === 0) {
      var pub = asn1.read.bitString(node.children[1]);
      if (!guard.crypto.isOctetAligned(pub)) {
    ...
    if (e instanceof KeyError) throw e;
    throw _err("key/bad-input", what + " X9.42 Diffie-Hellman key could not be read", e);
  }
...
async function _compositeCorrespondsTo(privDer, spkiDer, privateOid) {
  /** @internal The private half is read first, as on the classical route: a composite key the KEM
   * cannot read is bad input whatever the public half names, decided before the family shortcut
   * answers that the two are not a pair. The derivation's result is not what is compared; the
   * read is what it answers for. */
  try { await compositeKem.publicFromPrivate(privDer); }
  catch (e0) {
    if (_isKemStructureFault(e0)) throw _err("key/bad-input", "the private key could not be read", e0);
    throw _err("key/unsupported-algorithm", "the key pair could not be exercised to decide whether its two halves correspond", e0);
  }
...
async function correspondsTo(privateKey, spkiDer) {
  ...
  var spki = _bufferFrom(spkiDer);
  var compositeOid = _compositeKemOidOf(privDer);
  if (compositeOid !== null) return _compositeCorrespondsTo(privDer, spki, compositeOid);
  /** @internal The private half is read FIRST, whatever the public half names: a private key that
   * cannot be read is bad input, and a caller tells that apart from a pair that is not one. Only
   * then is the family question asked, from the two identifiers, before the public structure is
   * validated: a composite or a finite-field half against a half of another family is not a pair
   * whatever it states, so the runtime is never asked to read a structure it cannot, and a DH
   * structure's validation, which proves its modulus prime, is never run to say so. */
  var priv, pub;
  ...
  var publicOid = _algorithmOidAt(spki, 0);
  if (publicOid !== null && compositeKem.isCompositeKem(publicOid)) return false;
  if (_isDhOid(_algorithmOidAt(privDer, 1)) !== _isDhOid(publicOid)) return false;
  var pubAsPkcs3 = _dhAsPkcs3(spki, 0, "the public key's");
...
/** @internal Import from an encoding the caller named. PKCS#1 and SEC1 carry no algorithm, so
 * `opts.algorithm` is required for both and there is no inference to fall back on. JWK carries its
 * own key type and goes to the engine as it arrived. */
async function _importNamedFormat(input, opts) {
  var fmt = guard.text.keyOf(opts.format);
  var extractable = opts.extractable != null ? opts.extractable : false;
  if (fmt === "jwk") {
    if (input === null || typeof input !== "object" || _isArray(input)) {
      throw _err("key/bad-input", "a JWK is a JSON object (RFC 7517 sec. 4)");
    }
    if (typeof input.kty !== "string" || input.kty === "") {
      throw _err("key/bad-input", "a JWK carries a kty naming its key type (RFC 7517 sec. 4.1)");
    }
    if (opts.algorithm == null) throw _err("key/bad-input", "importing a JWK needs opts.algorithm");
    var jwkPublic = input.d === undefined && input.priv === undefined;
    var jwkUsages = opts.usages || _importUsages(_algName(opts.algorithm), jwkPublic);
    try {
      return await subtle.importKey("jwk", input, opts.algorithm, extractable, jwkUsages);
    } catch (e) {
      if (e && e.isPkiError) throw e;
      throw _err("key/bad-input", "importing the JWK failed", e);
    }
  }

  if (opts.algorithm == null) {
    throw _err("key/bad-input", "a " + (fmt === "pkcs1" ? "PKCS#1" : "SEC1") +
      " key names no algorithm (RFC 8017 App. A.1, RFC 5915 sec. 3), so opts.algorithm says what it is");
  }
  /** @internal `opts.format` names the ENCODING, and RFC 8017 App. A.1 gives that encoding two
   * structures with an armor label each. So both labels are read, and the structure the bytes
   * carry decides which half arrived; a label naming the half the bytes are not is refused
   * rather than read as the other. SEC1 has one structure and keeps its single label. */
  var der;
  if (fmt === "pkcs1") {
    der = _pkcs1Der(input);
  } else {
    der = pkix.coerceToDer(input, _SEC1_ENTRY);
  }
  var wrapped, isPublic = false;
  if (fmt === "pkcs1") {
    var isPub = _looksLikeRsaPublic(der);
    isPublic = isPub;
    if (isPub) {
      schemaPkcs1.parsePublic(der);
      wrapped = _pkcs1ToSpki(der);
    } else {
      var rsa = schemaPkcs1.parse(der);
      if (rsa.otherPrimeInfos.length > 0) {
        throw _err("key/unsupported-key", "a multi-prime RSA key is readable as a structure and has no arm in this toolkit's crypto engine (RFC 8017 App. A.1.2)");
      }
      wrapped = _pkcs1ToPkcs8(der);
    }
  } else {
    var ec = schemaSec1.parse(der);
    var named = WEBCRYPTO_CURVE[ec.curve];
    if (named === undefined) {
      throw _err("key/unsupported-key", "the curve " + ec.curve + " has no WebCrypto name in this toolkit");
    }
    var wanted = opts.algorithm && opts.algorithm.namedCurve;
    if (wanted != null && wanted !== named) {
      throw _err("key/bad-input", "the key names the curve " + named + " and opts.algorithm names " + guard.text.showValue(wanted));
    }
    wrapped = _sec1ToPkcs8(der, ec.curveOid);
  }
  var usages = opts.usages || _importUsages(_algName(opts.algorithm), isPublic);
  try {
    return await subtle.importKey(isPublic ? "spki" : "pkcs8", wrapped, opts.algorithm, extractable, usages);
  } catch (e) {
    if (e && e.isPkiError) throw e;
    throw _err("key/bad-input", "importKey failed", e);
  }
...
/** @internal The inverse of the wrap: pull the inner key out of the PKCS#8 or SPKI the engine
 * wrote. The bytes come back as they went in, since the engine carries the inner structure
 * unchanged and this reads it rather than re-encoding it. */
function _toPkcs1(der, key) {
  var algOid = _wrappedAlgorithmOid(der, key);
  if (algOid !== O("rsaEncryption")) {
    throw _err("key/bad-input", "PKCS#1 is an RSA encoding (RFC 8017 App. A.1) and this key is " + _algName(key.algorithm));
  }
  var node = asn1.decode(der);
  if (key.type === "public") {
    var bits = asn1.read.bitString(node.children[1]);
    if (bits.unusedBits !== 0) throw _err("key/bad-input", "the exported public key does not hold whole octets");
    schemaPkcs1.parsePublic(bits.bytes);
    return bits.bytes;
  }
  var inner = asn1.read.octetString(node.children[2]);
  schemaPkcs1.parse(inner);
  return inner;
}
function _toSec1(der, key) {
  var algOid = _wrappedAlgorithmOid(der, key);
  if (algOid !== O("ecPublicKey")) {
    throw _err("key/bad-input", "SEC1 is an elliptic-curve private-key encoding (RFC 5915 sec. 3) and this key is " + _algName(key.algorithm));
  }
  if (key.type !== "private") {
    throw _err("key/bad-input", "SEC1 encodes a private key; export the public half as SubjectPublicKeyInfo");
  }
  var node = asn1.decode(der);
  var inner = asn1.read.octetString(node.children[2]);
  /** @internal RFC 5915 sec. 3 says the [0] parameters field SHOULD be omitted when the key sits
   * inside a PKCS#8, whose AlgorithmIdentifier already names the curve, and SHOULD be present when
   * the key stands alone. So the curve is carried across from the wrapper here: without it the
   * standalone key would not say which curve its scalar belongs to. */
  var curveOid = asn1.read.oid(node.children[1].children[1]);
  var innerNode = asn1.decode(inner);
  var hasParams = false;
  for (var i = 0; i < innerNode.children.length; i++) {
    if (innerNode.children[i].tagClass === "context" && innerNode.children[i].tagNumber === 0) hasParams = true;
  }
  if (!hasParams) {
    var stored;
    for (var j = 0; j < innerNode.children.length; j++) {
      var c = innerNode.children[j];
      if (c.tagClass === "context" && c.tagNumber === 1) stored = asn1.read.bitString(c.children[0]);
    }
    inner = schemaSec1.encode({
      version: 1,
      privateKey: asn1.read.octetString(innerNode.children[1]),
      curveOid: curveOid,
      publicKey: stored === undefined ? null : stored.bytes,
    });
  }
  schemaSec1.parse(inner);
  return inner;
}

/** @internal The algorithm the wrapper names, read from whichever wrapper the key type produced:
 * a PrivateKeyInfo names it second, a SubjectPublicKeyInfo first. */
function _wrappedAlgorithmOid(der, key) {
  var node;
  try { node = asn1.decode(der); } catch (e) { throw _err("key/bad-input", "the exported key is not DER", e); }
  var algSeq = key.type === "public" ? node.children[0] : node.children[1];
  if (!algSeq || !algSeq.children || algSeq.children.length === 0) {
    throw _err("key/bad-input", "the exported key names no algorithm");
  }
  return asn1.read.oid(algSeq.children[0]);
}

/** @internal An RSAPublicKey is two INTEGERs and an RSAPrivateKey is nine or ten, so the two
 * encodings are told apart by counting the children of the top-level SEQUENCE. Neither door reads
 * the other's structure, and the count decides which one this is before either runs. */
function _looksLikeRsaPublic(der) {
  var node;
  try { node = asn1.decode(der); } catch (e) { throw _err("key/bad-input", "the input is not DER", e); }
