...
async function import_(input, opts) {
  ...
  // Checked at this verb's own door rather than left for the engine to discover: a non-list reached
  // the CryptoKey copy, where `Array.prototype.slice` on a number yields an empty list, so the key
  // was minted permitted for NOTHING instead of the call naming the argument that was wrong.
...
async function generate(algorithm, opts) {
  ...
  // The same door as import: a non-list is named here rather than becoming an empty usage set.
  if (opts.usages != null && !Array.isArray(opts.usages)) throw _err("key/bad-input", "opts.usages must be an array of key-usage strings");
