...
function _pairDigest(priv) {
  /** @internal An RSASSA-PSS key may pin the hash it is usable with, and signing under any other is
   * refused by the engine, so the pinned one is what proves the pair. */
  if (priv.asymmetricKeyType === "rsa-pss") return priv.asymmetricKeyDetails.hashAlgorithm || "sha256";
  return _PAIR_DIGEST[guard.text.keyOf(priv.asymmetricKeyType)] || null;
}
...
async function correspondsTo(privateKey, spkiDer) {
  ...
  if (priv.asymmetricKeyType !== pub.asymmetricKeyType) return false;
  ...
  try {
    ...
    if (_PAIR_AGREE[kind]) {
      ...
      return guard.crypto.constantTimeEqual(
        ...
        nodeCrypto.diffieHellman({ privateKey: ephemeral.privateKey, publicKey: pub }));
    }
    var digest = _pairDigest(priv);
    return nodeCrypto.verify(digest, _PAIR_PROBE, pub, nodeCrypto.sign(digest, _PAIR_PROBE, priv)) === true;
