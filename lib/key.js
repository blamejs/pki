...
/** @internal The private exponent a Diffie-Hellman PrivateKeyInfo states, in either encoding, held
 * inside the group before the runtime raises g to it: the import derives the public value by one
 * modular exponentiation whose cost is the width of the exponent, and a delivered key states any
 * width it likes. An exponent is at least 1 and below p (RFC 2631 sec. 2.2 narrows it further, to
 * [2, q-2], which is the agreement's test); outside that it is a malformed key. Anything that is
 * not a DH PrivateKeyInfo is returned as given. */
function _assertDhExponentWithin(der, what) {
  var algOid = _algorithmOidAt(der, 1);
  if (algOid !== _OID_DH_X942 && algOid !== _OID_DH_PKCS3) return;
  var node = asn1.decode(der);
  var algNode = node.children[1];
  if (!algNode.children || algNode.children.length !== 2 || !_isUniversal(node.children[2], 4)) {
    throw _err("key/bad-input", what + " Diffie-Hellman PrivateKeyInfo does not carry parameters and a privateKey OCTET STRING");
  }
  ...
  var x = asn1.read.integer(asn1.decode(asn1.read.octetString(node.children[2])));
  if (x <= 0n || x >= p) {
...
async function correspondsTo(privateKey, spkiDer) {
  ...
  try {
    _assertDhExponentWithin(privAsPkcs3, "the private key's");
