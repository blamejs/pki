...
// RSA (sign vs OAEP) and EC (ECDSA vs ECDH) are deliberately ABSENT -- they are ambiguous, so import
// fails closed and asks for opts.algorithm rather than guess a plausible use (guards never guess).
var INFER_ALG = {};
 ...
 * RSA and EC are ambiguous between signing and key agreement, so `opts.algorithm` MUST be supplied for them
...
function _readAlgOid(detected) {
  ...
  return asn1.read.oid(asn1.decode(detected.der).children[0].children[0]);   // SPKI: algorithm SEQ -> OID
}
...
function _importUsages(name, isPublic) {
  if (name === "X25519" || name === "X448" || name === "ECDH") return isPublic ? [] : ["deriveBits", "deriveKey"];
  if (/^ML-KEM/.test(name)) return isPublic ? ["encapsulateBits"] : ["decapsulateBits"];
  if (name === "RSA-OAEP") return isPublic ? ["encrypt"] : ["decrypt"];
  return isPublic ? ["verify"] : ["sign"];   // signing default (Ed / ECDSA / RSA-* sign / ML-DSA)
}

function _generateUsages(name) {
  if (name === "X25519" || name === "X448" || name === "ECDH") return ["deriveBits", "deriveKey"];
  if (/^ML-KEM/.test(name)) return ["encapsulateBits", "decapsulateBits"];
  if (name === "RSA-OAEP") return ["encrypt", "decrypt"];
  return ["sign", "verify"];
}
