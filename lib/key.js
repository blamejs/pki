...
function _pairSignOptions(priv, pub) {
  ...
  if (_pinsPss(privDetails) && _pinsPss(pubDetails) && !_samePssRestriction(privDetails, pubDetails)) return null;
...
function _pinsPss(d) {
  for (var i = 0; i < _PSS_PINS.length; i++) { if (d[_PSS_PINS[i]] != null) return true; }
  return false;
}
