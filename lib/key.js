 ...
 * p, g and value. The order and cofactor it dropped constrain a subgroup, not the identity of the
 * value, which is all that a correspondence decides. Anything else is returned as given. */
function _dhAsPkcs3(der, at) {
  if (_algorithmOidAt(der, at) !== _OID_DH_X942) return der;
  var node = asn1.decode(der);
  var params = node.children[at].children[1];
  if (!params || !params.children || params.children.length < 2) return der;
  var alg = b.sequence([b.oid(_OID_DH_PKCS3), b.sequence([b.raw(params.children[0].bytes), b.raw(params.children[1].bytes)])]);
  var fields = [];
...
async function correspondsTo(privateKey, spkiDer) {
  ...
  var privAsPkcs3 = _dhAsPkcs3(privDer, 1);
  ...
  try { pub = nodeCrypto.createPublicKey({ key: _dhAsPkcs3(_bufferFrom(spkiDer), 0), format: "der", type: "spki" }); }
