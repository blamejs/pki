 ...
 */
// Every option pki.key.encrypt reads. `iterations` here answers a different question from
// `maxIterations` on decrypt. This side chooses a PBKDF2 count, and that side caps the one it
// will honor. Passing either name to the other verb is silently nothing, so both are refused.
...
async function encrypt(privateKey, password, opts) {
  ...
  if (!cipherName) throw _err("key/bad-input", "unsupported cipher " + _stringify(opts.cipher) + " (aes-128-cbc / aes-192-cbc / aes-256-cbc)");
 ...
 */
// Every option pki.key.decrypt reads. `maxIterations` is the cap this side honors, not the
// `iterations` the encrypt side chooses; see the note above _ENCRYPT_OPTS.
 ...
 */
// Every option pki.key.export reads. Adding one here is the only way to make it accepted.
...
async function export_(key, opts) {
  ...
  // This verb does NOT encrypt, and `password` is the option a caller reaches for when they
  // think it does. Ignoring it writes the private key in the clear while the call site reads
  // as though it were protected. Every other unknown option here costs a setting that does
  // nothing; this one costs a plaintext key on disk. `pki.key.encrypt(key, password, opts)`
  // is the verb that encrypts; export serializes whatever it is handed.
  ...
  throw _err("key/bad-input", "unsupported format " + _stringify(opts.format) + " (der / pem)");
}
 ...
 */
// Every option pki.key.import reads.
 ...
 */
// Every option pki.key.generate reads. `extractable` follows the WebCrypto spelling; a
// misspelling would leave a key extractable by default, which is the permissive direction.
 ...
 */
// Every option pki.key.publicFromPrivate reads.
...
function _importUsages(name, isPublic) {
  var n = _String(name == null ? "" : name).toUpperCase();
