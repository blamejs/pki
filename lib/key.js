...
/** @internal One past the largest Diffie-Hellman operand the tests below compute with: the largest
 * group the toolkit agrees over. Built once, as two raised to a count of BITS, so the bound states
 * the width it is named for. */
 ...
 * tests whose cost grows with the modulus, a modular exponentiation and two primality proofs, are the
 * certifying authority's. Gently is not freely: the operands are bounded to the largest group the
 * toolkit agrees over BEFORE the multiplication and the modulo run on them, so a structure stating a
 * modulus of a megabyte is refused on its size rather than reduced. A cofactor states (p-1)/q, so a
 * cofactor with no order beside it states nothing. Errors are the caller's, as in `x942Domain`. */
function assertDhDomainCoherent(p, g, q, j, ErrorClass, code, what) {
  if (p >= _DH_MAX || g >= _DH_MAX || (q !== null && q >= _DH_MAX) || (j !== null && j >= _DH_MAX)) {
  ...
  /** @internal Each operand's sign is decided before the modulo and the multiplication that take it:
   * the bound above is an upper one, and a negative order or cofactor of any width would otherwise
  ...
  if (j !== null && (q === null || j <= 0n || j * q !== p - 1n)) {
