...
/** @internal Whether a stated order is the order of g: g raised to q is 1 modulo p. This is one
 * modular exponentiation, so it is asked only of operands already bounded by `assertDhOperandsWithin`,
 * and after `assertDhDomainCoherent` has answered the cheaper questions. Errors are the caller's. */
 ...
 * reads it), held to the toolkit's group bound before the runtime imports it: the import derives or
 * compares a public value by modular exponentiation whose cost is the width of p and of the
 * exponent, and a delivered key states any width it likes. So p, g and the value the structure
 * carries (the private exponent at `at` 1, the public value at `at` 0) are capped at the largest
 * group the toolkit agrees over, the cap that every other path computing in a DH group applies, and
 * a private exponent is held inside its group: at least 1 and below p (RFC 2631 sec. 2.2 narrows it
...
function _assertDhWithin(der, at, what) {
  if (_algorithmOidAt(der, at) !== _OID_DH_PKCS3) return;
  var node = asn1.decode(der);
  var algNode = node.children[at];
  var valueNode = node.children[at + 1];
  if (!algNode.children || algNode.children.length !== 2 || !valueNode || !_isUniversal(valueNode, at === 0 ? 3 : 4)) {
    throw _err("key/bad-input", what + " Diffie-Hellman structure does not carry parameters and a value");
  }
  var params = asn1.decode(algNode.children[1].bytes);
  if (!_isUniversal(params, 16) || !params.children || params.children.length < 2) {
    throw _err("key/bad-input", what + " Diffie-Hellman parameters do not state p and g");
  }
  var p = asn1.read.integer(params.children[0]);
  var g = asn1.read.integer(params.children[1]);
  var value = asn1.read.integer(asn1.decode(at === 0 ? asn1.read.bitString(valueNode).bytes : asn1.read.octetString(valueNode)));
  assertDhOperandsWithin([p, g, value], KeyError, "key/bad-input", what);
  if (at === 1 && (value <= 0n || value >= p)) {
...
async function _compositeCorrespondsTo(privDer, spkiDer, privateOid) {
  ...
  try {
    sealed = await compositeKem.encapsulate(spkiDer);
    opened = await compositeKem.decapsulate(privDer, sealed.ciphertext);
