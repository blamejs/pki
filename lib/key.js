 ...
 */
// Every option pki.key.encrypt reads. Note `iterations` here against `maxIterations` on
// decrypt: this side CHOOSES a PBKDF2 count, that side CAPS the one it will honor. Passing
// either name to the other verb is silently nothing, so both are refused by name.
...
async function export_(key, opts) {
  ...
  // as though it were protected -- the one unknown option here whose cost is a plaintext key
  // on disk rather than a setting that did nothing. `pki.key.encrypt(key, password, opts)` is
  // the verb that encrypts; export serializes whatever it is handed.
  guard.identifier.assertKnownKeys(opts, _EXPORT_OPTS, _err, "key/bad-input",
    ...
    "pki.key.encrypt(key, password) first and export its result): ");
