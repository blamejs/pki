...
RECOGNIZED_ATTRIBUTE_VALUE[oid.byName("extensionRequest")] = function (node, ctx) {
  ...
  try { return { extensions: schema.walk(EXTENSIONS, node, ctx).result }; }
    ...
    throw ctx.E("csr/bad-attribute-value",
      "extensionRequest value must be a well-formed Extensions SEQUENCE (RFC 2985 sec. 5.4.2): " + ((e && e.message) || _String(e)), e);
  }
};
  ...
  build: function (m, ctx) {
    var attributes = m.fields.attributes.value.items.map(function (it) {
      ...
      if (checkValue) {
        ...
        if (enriched) { _keys(enriched).forEach(function (k) { a[k] = enriched[k]; }); }
      }
      return a;
    });
    return {
 ...
 */
// Recording, because pki.csr.verify reaches a verdict from this result: the certificationRequestInfo
// byte range, the key it proves possession of, and the subject and requested extensions that range
// encodes are separate properties here, so the record is what binds them to one another.
