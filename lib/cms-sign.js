...
// this signer exists to serve are often exactly the ones that cannot be: a
// non-extractable CryptoKey, an HSM handle, or a composite `{ mldsa, trad }` pair
// that is two keys rather than one. Every one of those can still be checked the
// direct way -- the signature it just produced either verifies under the declared
// public key or it does not -- so there is no key kind this has to take on trust.
//
// This is the same post-sign self-check every other signer in the toolkit runs,
// through the same shared helper, so a key-only SignerInfo is held to the standard
// a certificate-bearing one already meets.
function _assertKeyMatchesSpki(keyOnly, soKey, soSpki, scheme, sig, signedBytes) {
  if (!keyOnly) return Promise.resolve();
  return Promise.resolve()
    .then(function () { return _b.assertSignatureVerifies(signedBytes, sig, soSpki, scheme); })
    .then(null, function (e) {
      if (e && typeof e.code === "string" && e.code.indexOf("cms/") === 0) throw e;
      throw _signE("bad-input",
        "a key-only signer's `key` does not match its `spki`: the signature it produced does not verify " +
        "under the public key the SignerInfo declares", e);
    });
}
