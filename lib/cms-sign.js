...
function sign(content, signers, opts) {
  // A streaming (async-iterable) content is consumed exactly once and never re-read, so it is not a
  // mutable value to snapshot -- and it CANNOT be deep-copied at all (an async iterator has no byte
  // length to bound the copy). It is passed through the entry copy while signers + opts, which are
  // caller-owned and re-read across the hashing await, are still copied.
  if (guard.bytes.isAsyncIterable(content)) {
    return guard.bytes.fixedCall(CmsError, "cms/bad-input", [
      [signers, "the signer list"], [opts, "pki.cms.sign options"],
    ], function (copiedSigners, copiedOpts) { return _signStream(content, copiedSigners, copiedOpts); });
  }
...
function _signStream(content, signers, opts) {
  ...
  return subtle.digestStream(wcNames, content).then(function (digests) {
    var byName = {};
