...
function _buildSignerInfo(signer, content, eContentType, opts) {
  ...
  return Promise.resolve().then(function () {
    ...
    return signScheme.signOverTbs(scheme, soKey, signedBytes, _signE).then(function (sig) {
      return _assertKeyMatchesSpki(keyOnly, soKey, soSpki, scheme, sig, signedBytes).then(function () { return sig; });
    }).then(function (sig) {
...
function _assertKeyMatchesSpki(keyOnly, soKey, soSpki, scheme, sig, signedBytes) {
  ...
  if (scheme && scheme.composite) {
    return compositeSig.compositeVerify(
      soSpki, Buffer.from(sig), signedBytes, scheme.composite,
  ...
  return key.publicFromPrivate(soKey).then(function (derived) {
    if (!guard.crypto.constantTimeEqual(Buffer.from(derived), soSpki)) {
      throw _signE("bad-input",
    ...
    var opaque = soKey && typeof soKey === "object" && !Buffer.isBuffer(soKey) &&
      typeof soKey.type === "string" && soKey.algorithm != null;
