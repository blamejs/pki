...
function _finishSignerInfo(rc, md, content, eContentType, opts) {
  return Promise.resolve().then(function () {
    ...
    return signScheme.signOverTbs(rc.scheme, rc.soKey, signedBytes, _signE).then(function (sig) {
      return _assertKeyMatchesSpki(rc.keyOnly, rc.soKey, rc.soSpki, rc.scheme, sig, signedBytes, rc.cert).then(function () { return sig; });
    }).then(function (sig) {
      ...
      fields.push(rc.scheme.sigAlgId, b.octetString(sig));
...
function _signDispatch(content, signers, opts) {
  // A streaming content is classified by ACQUIRING its async iterator once (guard.bytes.asyncIteratorOf
  // reads Symbol.asyncIterator a single time); the iterator, consumed exactly once and never re-read, is
  // passed through the entry copy while signers + opts, caller-owned and re-read across the hashing
  // await, are still copied. It CANNOT be deep-copied itself (an iterator has no byte length to bound
  // the copy). A byte source (or any non-iterable) yields null here and takes the buffered path.
  var streamIterator = guard.bytes.asyncIteratorOf(content);
  if (streamIterator) {
    return guard.bytes.fixedCall(CmsError, "cms/bad-input", [
      [signers, "the signer list"], [opts, "pki.cms.sign options"],
    ], function (copiedSigners, copiedOpts) { return _signStream(streamIterator, copiedSigners, copiedOpts); });
  }
...
function _signStream(streamIterator, signers, opts) {
  ...
  var list = intrinsic.isArray(signers) ? signers : [signers];
  ...
  return subtle.digestStream(wcNames, streamIterator).then(function (digests) {
    var byName = {};
    intrinsic.forEach(digestNames, function (n, i) { byName[n] = intrinsic.bufferFrom(digests[i]); });
    return Promise.all(intrinsic.map(rcs, function (r) {
      return _finishSignerInfo(r, byName[r.scheme.digest], null, eContentType, opts);
    })).then(function (built) { return _assembleSignedData(built, eContentType, opts, null); });
  }, function (e) {
...
module.exports = {
  sign: sign, countersign: countersign,
