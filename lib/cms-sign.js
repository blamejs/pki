...
function _buildSignerInfo(signer, content, eContentType, opts) {
  ...
  return Promise.resolve().then(function () {
    ...
    // Everything downstream -- the signature scheme, the SignerIdentifier, and in
    // CMC the binding to a certification request -- is derived from `spki`, while
    // the signature itself is made with `key`. If the two are different keys the
    // result is a well-formed SignerInfo nobody can verify: the recipient resolves
    // the identifier to the declared key and checks a signature made by another.
    // A certificate-bearing signer needs no such check -- the certificate is the
    // binding, and resolveSignScheme already reads its key.
