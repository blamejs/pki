...
var key = require("./key");   // to check a key-only signer's private key against the SPKI it declares
var compositeSig = require("./composite-sig");   // ... and the composite form, checked from the signature
...
function _buildSignerInfo(signer, content, eContentType, opts) {
  ...
  var cert = keyOnly ? _keyOnlyCertStandIn(so) : x509.parse(certDer);
  ...
  return Promise.resolve().then(function () {
    ...
    return signScheme.signOverTbs(scheme, so.key, signedBytes, _signE).then(function (sig) {
      return _assertKeyMatchesSpki(keyOnly, so, scheme, sig, signedBytes).then(function () { return sig; });
    }).then(function (sig) {
...
// A key-only signer names a public key it does not otherwise prove it holds: the
// signature scheme, the SignerIdentifier, and in CMC the binding to a certification
// request all come from `spki`, while the signature comes from `key`. Two different
// keys make a well-formed SignerInfo nobody can verify -- the recipient resolves the
// identifier to the declared key and checks a signature made by another -- and with
// no certificate present nothing else would catch it.
//
// The check needs the private key's own public half, which for a NON-EXTRACTABLE
// CryptoKey cannot be had: that is the point of such a key, and refusing it would
// lock out the HSM-backed enrollment keys this signer exists to serve. So the
// mismatch is caught wherever the key material allows it and the opaque handle is
// taken as the caller's assertion -- which is what an opaque handle is. Any other
// export failure is a real fault and propagates.
function _assertKeyMatchesSpki(keyOnly, so, scheme, sig, signedBytes) {
  if (!keyOnly) return Promise.resolve();
  // A COMPOSITE key is `{ mldsa, trad }` -- two component private keys, not a form
  // publicFromPrivate can turn into one public key. The correspondence is still
  // provable, and more directly: verify the signature just produced against the
  // DECLARED spki. If they are different keys it does not verify. This is the same
  // evidence the derivation gives, obtained from the signature rather than the key.
  if (scheme && scheme.composite) {
    return compositeSig.compositeVerify(
      Buffer.from(so.spki), Buffer.from(sig), signedBytes, scheme.composite,
      _signE, "unsupported-algorithm", "bad-input"
    ).then(function (res) {
      if (!res || res.ok !== true) {
        throw _signE("bad-input",
          "a composite key-only signer's `key` does not match its `spki`: the signature it produced " +
          "does not verify under the public key the SignerInfo declares");
      }
    });
  }
  return key.publicFromPrivate(so.key).then(function (derived) {
    if (!guard.crypto.constantTimeEqual(Buffer.from(derived), Buffer.from(so.spki))) {
      throw _signE("bad-input",
        "a key-only signer's `key` does not match its `spki`: the SignerInfo would declare one public key and be signed by another");
    }
  }, function (e) {
    // The skip is for ONE situation: an opaque key handle that will not give up
    // its private half. Both spellings of that are accepted -- the engine's own
    // refusal, and the one raised for a CryptoKey from another WebCrypto
    // implementation -- but only when the key really is such a handle.
    //
    // Matching on the code alone would swallow more than that. A composite key is
    // a plain `{ mldsa, trad }` object that publicFromPrivate also rejects with
    // key/bad-input, and treating that as "opaque, cannot check" would skip the
    // comparison for a key whose halves are perfectly readable -- the fail-open
    // this check exists to prevent. Anything that is not an opaque handle must be
    // checkable or refused.
    var opaque = so.key && typeof so.key === "object" && !Buffer.isBuffer(so.key) &&
      typeof so.key.type === "string" && so.key.algorithm != null;
    if (opaque && e && (e.code === "webcrypto/invalid-access" || e.code === "key/bad-input")) return;
    throw _signE("bad-input",
      "a key-only signer's `key` could not be checked against its `spki`: " +
      (e && e.message ? e.message : "the public key could not be derived from it"), e);
  });
}
...
function _keyOnlyCertStandIn(so) {
  var spkiDer = Buffer.isBuffer(so.spki) ? so.spki : Buffer.from(so.spki);
  var alg;
  try {
...
function _keyOnlyKeyId(so) {
  ...
  var id = Buffer.isBuffer(so.keyIdentifier) ? so.keyIdentifier : Buffer.from(so.keyIdentifier);
