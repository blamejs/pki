...
function _digest(digestName, content) {
  return subtle.digest(DIGEST_HASH[digestName], content).then(function (d) { return _bufferFrom(d); });
}
...
function _buildSignedAttrs(pairs) {
  ...
  var wire = _bufferFrom(setOf); wire[0] = 0xA0;   // the on-wire [0] IMPLICIT tag
...
function _buildUnsignedAttrs(list) {
  ...
  if (!_isArray(list)) throw _err("cms/bad-input", "opts.unsignedAttributes must be an array of { type, values }");
  ...
  var wire = _bufferFrom(setOf); wire[0] = 0xA1;   // SET OF -> [1] IMPLICIT UnsignedAttributes
  return wire;
}
...
function _buildSignerInfo(signer, content, eContentType, opts) {
  ...
  return Promise.resolve().then(function () {
    ...
    return _digest(scheme.digest, content).then(function (md) {
      ...
      if (opts.signingTime !== false) _push(pairs, { type: O("signingTime"), values: [_timeValue(opts.signingTime)] });
    ...
    return signScheme.signOverTbs(scheme, soKey, signedBytes, _signE).then(function (sig) {
      ...
      var fields = [b.integer(_BigInt(version)), sid, scheme.digestAlgId];
      if (toSign.wire) _push(fields, toSign.wire);              // [0] IMPLICIT signedAttrs
      _push(fields, scheme.sigAlgId, b.octetString(sig));
      var ua = _buildUnsignedAttrs(opts.unsignedAttributes);
      if (ua) _push(fields, ua);                                 // [1] IMPLICIT unsignedAttrs
...
function _timeValue(when) {
  var d = guard.time.isDate(when) ? when : new Date();
...
function _normCertDer(c) {
  ...
  if (c instanceof Uint8Array || _isBuffer(c)) {
...
function _pemToDer(text) {
  ...
  return _bufferFrom(m[1].replace(/[^A-Za-z0-9+/=]/g, ""), "base64");
}
...
function _sign(content, signers, opts) {
  ...
  var list = _isArray(signers) ? signers : [signers];
  ...
  return Promise.all(list.map(function (s) { return _buildSignerInfo(s, contentBuf, eContentType, opts); })).then(function (built) {
    ...
    var seen = {}, digestAlgs = [];
    built.forEach(function (x) { var k = x.digestAlgId.toString("hex"); if (!seen[k]) { seen[k] = 1; _push(digestAlgs, x.digestAlgId); } });
    ...
    if (!opts.detached) _push(encapFields, b.explicit(0, b.octetString(contentBuf)));
    var encap = b.sequence(encapFields);
    // certificates [0] IMPLICIT SET OF (the signer certs), deduped + SET-OF-ordered, when embedded.
    var sdFields = [b.integer(_BigInt(version)), b.set(digestAlgs), encap];
    if (opts.certificates !== false) {
      ...
      if (certDers.length) _push(sdFields, b.contextConstructed(0, _bufferConcat(certDers)));   // [0] IMPLICIT SET OF
    }
    _push(sdFields, b.set(built.map(function (x) { return x.si; })));                 // signerInfos SET OF
...
function _dedupe(ders) {
  var seen = {}, out = [];
  ders.forEach(function (d) { var k = d.toString("hex"); if (!seen[k]) { seen[k] = 1; _push(out, d); } });
  return out;
}

function _toBuf(v, what) {
  if (_isBuffer(v) || v instanceof Uint8Array) return guard.bytes.snapshot(v, CmsError, "cms/bad-input", what);
...
function _resolveSignerIndices(spec, n) {
  ...
  if (spec === "all") { var all = []; for (var i = 0; i < n; i++) _push(all, i); return all; }
  var arr = _isArray(spec) ? spec : [spec];
  if (!arr.length) throw _err("cms/bad-input", "signerIndex must select at least one signer");
  arr.forEach(function (i) { if (typeof i !== "number" || !_isInteger(i) || i < 0 || i >= n) throw _err("cms/bad-input", "signerIndex out of range: " + i); });
  return arr;
}
...
function _buildCountersignature(targetSigOctets, countersigner, opts) {
  ...
  return Promise.resolve().then(function () {
    ...
    return signScheme.signOverTbs(scheme, soKey, attrs ? attrs.setOf : targetSigOctets, _signE).then(function (sig) {
      var fields = [b.integer(_BigInt(sidv.version)), sidv.sid, scheme.digestAlgId];
      if (attrs) _push(fields, attrs.wire);          // [0] IMPLICIT signedAttrs
      _push(fields, scheme.sigAlgId, b.octetString(sig));
...
function _mergeCountersig(uaNode, newCsValues) {
  ...
  if (uaNode) uaNode.children.forEach(function (attr) {
    if (asn1.read.oid(attr.children[0]) === CS) attr.children[1].children.forEach(function (v) { _push(csValues, v.bytes); });
    else _push(others, attr.bytes);
  });
  newCsValues.forEach(function (v) { _push(csValues, v); });
...
function _appendCountersigs(siNode, newCsValues) {
  ...
  _push(base, _mergeCountersig(hasUa ? last : null, newCsValues));
...
function _spliceNested(siNode, j, newCsValues) {
  ...
  var setOf = b.set(attrs); var wire = _bufferFrom(setOf); wire[0] = 0xA1;
...
function _targetPreimage(siNode, opts) {
  ...
  if (typeof j !== "number" || !_isInteger(j) || j < 0 || j >= values.length) throw _err("cms/bad-input", "countersignatureOf out of range: " + j);
...
function _countersign(cmsInput, signers, opts) {
  ...
  if (!_isArray(parsed.signerInfos)) throw _err("cms/bad-input", "pki.cms.countersign input is not a CMS SignedData");
  ...
  targets.forEach(function (t) {
    ...
    list.forEach(function (cs) { _push(jobs, { t: t, p: _buildCountersignature(preimage, cs, opts) }); });
  });
  return Promise.all(jobs.map(function (j) { return j.p; })).then(function (built) {
    var byTarget = {}, certDers = [];
    built.forEach(function (res, i) { (byTarget[jobs[i].t] = byTarget[jobs[i].t] || []).push(res.value); _push(certDers, res.certDer); });
    ...
    var existing = [];
    if (certsNode) certsNode.children.forEach(function (c) { _push(existing, c.bytes); });
    if (opts.certificates !== false) certDers.forEach(function (d) { _push(existing, d); });
    ...
    if (allCerts.length) _push(newSdFields, b.contextConstructed(0, _bufferConcat(allCerts)));
    if (crlsNode) _push(newSdFields, crlsNode.bytes);
    _push(newSdFields, newSiSet);
