...
function _signDispatch(content, signers, opts) {
  ...
  // exactly once, is passed through the entry copy while signers + opts, caller-owned and re-read
  // across the hashing await, are still copied. It CANNOT be deep-copied itself. A byte source (or any
  ...
  if (stream) {
    ...
    ], function (copiedSigners, copiedOpts) { return _signStream(stream, copiedSigners, copiedOpts); });
  }
...
function _signStream(stream, signers, opts) {
  ...
  return subtle.digestStream(wcNames, stream).then(function (digests) {
    var byName = {};
