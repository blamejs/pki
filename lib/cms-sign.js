...
function _assertKeyMatchesSpki(keyOnly, so) {
  ...
  return key.publicFromPrivate(so.key).then(function (derived) {
    ...
    // Both spellings of "this key will not give up its private half": the engine's
    // own refusal, and the one raised for a CryptoKey from another WebCrypto
    // implementation that is likewise not extractable. Either way there is nothing
    // to compare against. Any OTHER failure is a real fault and propagates.
    if (e && (e.code === "webcrypto/invalid-access" || e.code === "key/bad-input")) return;
    throw e;
  });
}
...
function sign(content, signers, opts) {
  ...
  // one SignerInfo in the SignedData." A key-only signer has no certificate for a
  // verifier to find, so a second signer would produce a SignedData whose signers
  // cannot all be resolved -- refuse the combination rather than emit it.
  if (list.length > 1 && list.some(function (s) { return s && s.cert == null && s.spki != null; })) {
    throw _err("cms/bad-input",
      "a key-only signer must be the ONLY SignerInfo in the SignedData (RFC 5272 sec. 3.2)");
  }
