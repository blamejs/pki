...
async function _signStream(stream, signers, opts) {
  ...
  try {
    ...
    // The engine reports malformed streamed content in its own domain; report it in this verb's domain
    // instead, exactly as a buffered content's bad bytes are, rather than leaking the engine code. A
    // non-byte chunk is webcrypto/data; a source whose async-iterator protocol is malformed (a
    // non-callable next, or a factory returning a non-iterator) is webcrypto/syntax. The digest
    // algorithm names are this module's own, so webcrypto/syntax here is never an algorithm fault.
    if (e && e.code === "webcrypto/data") throw _err("cms/bad-input", "a streamed content chunk is not a byte source", e);
    if (e && e.code === "webcrypto/syntax") throw _err("cms/bad-input", "the streamed content is not a valid async iterable of byte chunks", e);
    throw e;
  }
  var byName = {};
