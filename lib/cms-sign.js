...
function _scheme(cert, so) {
  ...
  if (keyOid === O("rsaEncryption")) {
    ...
    if (so.pss) return { digest: d, digestAlgId: _algId(d, "absent"), sigAlgId: _pssAlgId(d), imp: { name: "RSA-PSS", hash: HASH[d] }, sign: { name: "RSA-PSS", saltLength: PSS_SALT[HASH[d]] }, ecdsaDer: false };
...
function _buildSignerInfo(signer, content, eContentType, opts) {
  ...
  return _importKey(so.key, scheme.imp).then(function (priv) {
    return Promise.resolve().then(function () {
      ...
      var attrs = [
        b.sequence([b.oid(O("contentType")), b.set([b.oid(eContentType)])]),
        b.sequence([b.oid(O("messageDigest")), b.set([b.octetString(_digest(scheme.digest, content))])]),
      ];
      if (opts.signingTime !== false) attrs.push(b.sequence([b.oid(O("signingTime")), b.set([_timeValue(opts.signingTime)])]));
      // Caller-supplied signed attributes (e.g. an RFC 3161 signing-certificate attribute): each
      // { type: <OID name>, values: [<DER value Buffer>] }. build.set canonical-sorts them in.
      (opts.additionalSignedAttributes || []).forEach(function (a) {
        ...
        var typeOid = /^\d+(\.\d+)+$/.test(a.type) ? a.type : O(a.type);   // an OID name or a dotted string
        attrs.push(b.sequence([b.oid(typeOid), b.set(vals)]));
      });
