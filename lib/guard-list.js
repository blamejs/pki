...
function containsAll(list, values) {
  ...
  for (var i = 0; i < values.length; i++) if (!contains(list, values[i])) return false;
  return true;
}
...
function anyMatches(list, predicate) {
  if (!list) return false;
  for (var i = 0; i < list.length; i++) if (predicate(list[i], i)) return true;
  return false;
}
...
function allMatch(list, predicate) {
  if (!list) return true;
  for (var i = 0; i < list.length; i++) if (!predicate(list[i], i)) return false;
  return true;
}
