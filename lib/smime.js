...
var pkiBuild = require("./pki-build.js");
...
var VERIFY_OPTS = Object.assign(Object.create(null), {
  ...
  legacyHeaderProtection: 1, expectedSender: 1, allowWeakDigests: 1,
});
...
/** @internal Checks the option names and RETURNS the options taken once. A verb reads one option to
 *  decide whether a rule applies and reads it again for the value the rule uses, and an option
 *  reached through an accessor answers each read separately. Callers assign the return value. */
function _knownOpts(opts, known, verb) {
  ...
  return guard.identifier.snapshotOptions(opts, known);
}
...
function _hpHeaderList(headers) {
  ...
  if (Array.isArray(headers)) {
    ...
    var hn = guard.identifier.optionNames(headers);
    for (var hi = 0; hi < hn.length; hi++) {
      var hv = headers[hn[hi]];
      out.push({ name: hn[hi], value: hv == null ? "" : String(hv) });
    }
  } else {
...
var EMAIL_PROTECTION_OID = oid.byName("emailProtection");
var ANY_EKU_OID = oid.byName("anyExtendedKeyUsage");
var DESCRIPTOR_MAX_PROTOTYPES = 32;
var DESCRIPTOR_MAX_FIELDS = 64;
...
/** @internal A signer certificate fit to sign mail. RFC 8550 sec. 4.4.2: a receiving agent "MUST
 * NOT accept the signature of a message if it was verified using a certificate that contains a key
 * usage extension without at least one of the digitalSignature or nonRepudiation bits set", and
 * sec. 4.4.4: if the extended key usage extension is present, a receiving agent "MUST check that it
 * contains either the id-kp-emailProtection OID or the anyExtendedKeyUsage OID". A message signed
 * under such a certificate is refused by every conforming reader, so it is not produced. An absent
 * extension permits signing: sec. 4.4.2 has a reader presume both bits, and sec. 4.4.4 lets a
 * certificate with no extended key usage sign. */
function _assertSignerFitForMail(certDer, position) {
  var parsed;
  try { parsed = schemaX509.parse(certDer); }
  catch (e) { throw _err("smime/bad-input", "signer " + position + "'s certificate is not a well-formed X.509 certificate", e); }
  var label = "signer " + position + "'s certificate";
  var ku = pkix.keyUsageOf(_extNs, parsed, _err, "smime/bad-signer-certificate", label);
  if (ku !== null && !(ku.digitalSignature === true || ku.nonRepudiation === true)) {
    throw _err("smime/bad-signer-certificate", label + " carries a keyUsage extension with neither digitalSignature nor nonRepudiation, and a receiving agent must not accept a signature under it (RFC 8550 sec. 4.4.2)");
  }
  var purposes = pkix.extKeyUsageOf(_extNs, parsed, _err, "smime/bad-signer-certificate", label);
  if (purposes !== null && !guard.list.contains(purposes, EMAIL_PROTECTION_OID) && !guard.list.contains(purposes, ANY_EKU_OID)) {
    throw _err("smime/bad-signer-certificate", label + " carries an extendedKeyUsage extension with neither emailProtection nor anyExtendedKeyUsage, and a receiving agent must check for one of them (RFC 8550 sec. 4.4.4)");
  }
}

/** @internal The signer descriptors as this verb read them, held to the rule above before anything
 * is signed, and handed on to pki.cms.sign as the SAME values: every field a descriptor carries, own
 * or inherited, is read once with the descriptor as its receiver into a record, and the certificate
 * is the DER it coerced to, so a `cert` accessor cannot answer one certificate to this check and
 * another to the signing beneath, and an accessor over private state answers to the object it was
 * defined for. A single descriptor is the list of one pki.cms.sign accepts. Anything that is not a
 * descriptor travels as given, for pki.cms.sign to refuse in its own words. */
function _snapshotSignersFitForMail(signers) {
  /** @internal A proxy, as the list or as a descriptor, is refused before anything trusts what its
   * traps answer, as the toolkit's guarded snapshots refuse one; nothing is read through it. A
   * sparse list is refused at its first missing slot, as pki.cms.sign refuses it, before any slot
   * is visited; a list whose own faults surface is this verb's error too. */
  if (nodeUtil.types.isProxy(signers)) throw _err("smime/bad-input", "the signer list must not be a proxy");
  var list;
  try { list = Array.isArray(signers) ? pkiBuild.reqDenseArray(signers, "the signer list", _err, "smime/bad-input") : [signers]; }
  catch (e) {
    if (e instanceof SmimeError) throw e;
    throw _err("smime/bad-input", "the signer list could not be read", e);
  }
  var out = [];
  for (var i = 0; i < list.length; i++) {
    var s = list[i];
    /** @internal An object or a function is a descriptor pki.cms.sign reads. An absent one travels
     * as given for pki.cms.sign to refuse in its own words; a primitive is refused here, since the
     * only fields it could answer with are ones a built-in prototype was made to carry. */
    if (s === null || s === undefined) { guard.list.append(out, s); continue; }
    if (typeof s !== "object" && typeof s !== "function") throw _err("smime/bad-input", "signer " + (i + 1) + " is not a signer descriptor (an object carrying cert and key)");
    if (nodeUtil.types.isProxy(s)) throw _err("smime/bad-input", "signer " + (i + 1) + " must not be a proxy");
    var copy = _readDescriptor(s, i + 1);
    /** @internal The one read of `cert`, whatever it answered: an absent certificate stays absent,
     * so an accessor cannot answer nothing here and a certificate to the signing beneath. A present
     * one is an owned copy of the bytes, not a view, since a later descriptor's accessor runs after
     * this check and could rewrite a buffer the caller still holds. */
    if (copy.cert != null) {
      /** @internal A certificate that cannot be read, in either encoding, is this verb's bad
       * input, never the PEM codec's own code. */
      try { copy.cert = guard.bytes.snapshot(pkix.coerceToDer(copy.cert, { pemLabel: "CERTIFICATE", PemError: SmimeError, ErrorClass: SmimeError, prefix: "smime" }), SmimeError, "smime/bad-input", "signer " + (i + 1) + "'s certificate"); }
      catch (e) { throw _err("smime/bad-input", "signer " + (i + 1) + "'s certificate could not be read as DER or PEM", e); }
      _assertSignerFitForMail(copy.cert, i + 1);
    }
    guard.list.append(out, copy);
  }
  return out;
}

/** @internal Whether a prototype-level property is a method: a data property holding a function. */
function _isMethod(proto, name) {
  var d = Object.getOwnPropertyDescriptor(proto, name);
  return !!d && !d.get && !d.set && typeof d.value === "function";
}

/** @internal Every named field of a descriptor, own or inherited up to the built-in roots (the
 * object and function prototypes carry no descriptor field, and the function one carries accessors
 * that throw on being read), read once with the descriptor as receiver into a record with no
 * prototype. An accessor that throws is a caller fault reported in this verb's class. */
function _readDescriptor(s, position) {
  var copy = Object.create(null), seen = Object.create(null), fields = 0;
  try {
    /** @internal The descriptor's own level carries every key, Symbol-keyed and `constructor`
     * included, as the caller set them; a prototype level's `constructor` is the class, not a
     * field. A descriptor carries a handful of fields, so one answering more than the cap is
     * refused before they are read. */
    var symbols = Object.getOwnPropertySymbols(s);
    for (var y = 0; y < symbols.length; y++) {
      if (++fields > DESCRIPTOR_MAX_FIELDS) throw _err("smime/bad-input", "signer " + position + " carries more than " + DESCRIPTOR_MAX_FIELDS + " fields");
      copy[symbols[y]] = s[symbols[y]];
    }
    /** @internal The walk is bounded: a proxy may answer itself, or another visited object, as its
     * prototype, and a chain has a depth no descriptor reaches. A prototype level's methods are
     * the class's, not signer fields: neither copied nor counted. */
    var visited = [], depth = 0;
    for (var proto = s; proto !== null && proto !== Object.prototype && proto !== Function.prototype; proto = Object.getPrototypeOf(proto)) {
      if (visited.indexOf(proto) !== -1 || ++depth > DESCRIPTOR_MAX_PROTOTYPES) {
        throw _err("smime/bad-input", "signer " + position + "'s prototype chain is cyclic or deeper than " + DESCRIPTOR_MAX_PROTOTYPES);
      }
      if (nodeUtil.types.isProxy(proto)) throw _err("smime/bad-input", "signer " + position + " must not inherit from a proxy");
      guard.list.append(visited, proto);
      var names = Object.getOwnPropertyNames(proto);
      for (var n = 0; n < names.length; n++) {
        var name = names[n];
        if ((name === "constructor" && proto !== s) || seen[name] === 1) continue;
        if (proto !== s && _isMethod(proto, name)) continue;
        seen[name] = 1;
        if (++fields > DESCRIPTOR_MAX_FIELDS) throw _err("smime/bad-input", "signer " + position + " carries more than " + DESCRIPTOR_MAX_FIELDS + " fields");
        copy[name] = s[name];
      }
    ...
    if (e instanceof SmimeError) throw e;
    throw _err("smime/bad-input", "signer " + position + " could not be read as a signer descriptor", e);
  }
  return copy;
}
 ...
 * signer: the S/MIME layer is algorithm-agnostic). Each signer's certificate must be one a conforming
 * receiving agent accepts a mail signature under: a `keyUsage` extension it carries must assert
 * `digitalSignature` or `nonRepudiation` (RFC 8550 sec. 4.4.2) and an `extendedKeyUsage` extension it
 * carries must include `emailProtection` or `anyExtendedKeyUsage` (sec. 4.4.4); a certificate with neither
 * extension signs, and one failing either rule is refused with `smime/bad-signer-certificate`. The rule is
 * about the certificate the message carries: a key-only signer (`{ key, spki, keyIdentifier }`, the form
 * RFC 8550 sec. 3 permits where correspondents hold each other's certificates by other means) carries none,
 * and the reader holds the certificate it supplies to the same rule. Two forms via `opts.form`:
...
async function sign(content, signers, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, SIGN_OPTS, "sign");
  signers = _snapshotSignersFitForMail(signers);
 ...
 * `emailProtection` key purpose (RFC 8550 sec. 4.4.4), because a certificate restricted to `serverAuth`
...
async function verify(message, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, VERIFY_OPTS, "verify");
  ...
  if (opts.allowWeakDigests === true) vOpts.allowWeakDigests = true;
...
async function encrypt(content, recipients, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, ENCRYPT_OPTS, "encrypt");
  var hp = opts.protectHeaders === true;
  var hpList = hp ? _hpHeaderList(opts.headers) : null;
...
async function decrypt(message, keyMaterial, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, DECRYPT_OPTS, "decrypt");
  var ent = mime.parse(message, SmimeError, "smime/bad-mime");
  var ct = ent.contentType;
...
async function compress(content, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, COMPRESS_OPTS, "compress");
...
async function decompress(message, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, DECOMPRESS_OPTS, "decompress");
  var ent = mime.parse(message, SmimeError, "smime/bad-mime");
  var ct = ent.contentType;
...
function buildCertsOnly(certs, opts) {
  opts = opts || {};
  opts = _knownOpts(opts, CERTS_ONLY_OPTS, "buildCertsOnly");
