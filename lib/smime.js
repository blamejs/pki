...
function _declaresHp(content) {
  ...
  // Allow whitespace before the colon (mime.parse trims the field name, so it accepts "Content-Type :") --
  // the prescan must not be stricter than the parser, or a valid hp-bearing payload silently downgrades.
  return /^content-type[ \t]*:[^\r\n]*;[^\r\n]*\bhp[ \t]*=/im.test(head);
}
