...
// The micalg for a detached SignerInfo's digest -- read from the CMS the caller just produced.
function _micalgOf(p7Der) {
  ...
  var si = parsed.signerInfos && parsed.signerInfos[0];
  var name = si && si.digestAlgorithm && si.digestAlgorithm.name;
  return MICALG[name] || (name ? name.replace(/^sha/, "sha-") : null);
}
...
async function sign(content, signers, opts) {
  ...
  if (form === "pkcs7-mime") {
    ...
    return Buffer.concat([head, _base64Body(_toBuf(p7m)), mime.CRLF]);
  }
  ...
  return Buffer.concat([head2, dl, entity, mime.CRLF, dl, sigPart, mime.CRLF, close]);
}
...
async function verify(message, opts) {
  ...
  if (ct.type === "multipart/signed") {
    ...
    if (opts.strictMicalg && micalg && micalg.toLowerCase() !== (_micalgOf(p7s) || "").toLowerCase()) {
      throw _err("smime/micalg-mismatch", "the multipart/signed micalg " + JSON.stringify(micalg) + " disagrees with the SignerInfo digest");
    }
