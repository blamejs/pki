...
function _snapshotSignersFitForMail(signers) {
  ...
  for (var i = 0; i < list.length; i++) {
    var s = list[i];
    if (!s || typeof s !== "object") { guard.list.append(out, s); continue; }
...
/** @internal Every named field of a descriptor, own or inherited up to the root object, read once
 * with the descriptor as receiver into a record with no prototype. An accessor that throws is a
 * caller fault reported in this verb's class. */
function _readDescriptor(s, position) {
  var copy = Object.create(null), seen = Object.create(null);
  try {
    for (var proto = s; proto !== null && proto !== Object.prototype; proto = Object.getPrototypeOf(proto)) {
      var names = Object.getOwnPropertyNames(proto);
      for (var n = 0; n < names.length; n++) {
        var name = names[n];
        if (name === "constructor" || seen[name] === 1) continue;
        seen[name] = 1;
        copy[name] = s[name];
      }
    }
