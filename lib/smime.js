...
function _hpSurface(content, outerEnt, expectedMode, authenticated) {
  ...
  inner.headers.forEach(function (h) {
    ...
    seen[h.lname] = 1;
    protectedHeaders[h.name] = _utf8Header(h.value);   // SURFACE the UTF-8-decoded value (RFC 6532)
    // ...but retain the BYTE-PRESERVING (latin1) value for the mismatch comparison: a lossy UTF-8 decode maps
    // distinct invalid 8-bit sequences (0x80 vs 0x81) to the same replacement char, which would let an
    // attacker alter the displayed From without tripping fromMismatch. Compare the raw octets instead.
