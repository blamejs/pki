...
function _snapshotSignersFitForMail(signers) {
  ...
  var list = Array.isArray(signers) ? pkiBuild.reqDenseArray(signers, "the signer list", _err, "smime/bad-input") : [signers];
  var out = [];
  for (var i = 0; i < list.length; i++) {
    var s = list[i];
...
function _readDescriptor(s, position) {
  var copy = Object.create(null), seen = Object.create(null);
  try {
