...
// Every operation this module assembles, canonicalizes and compares with, taken at load. Each is an
// ordinary writable property of a global, so a read at call time lets whoever replaced it decide
// which header fields a signature covers, which identity a signer certificate is read as asserting,
// and which bytes reach the CMS layer.
...
function _knownOpts(opts, known, verb) {
  guard.identifier.assertKnownKeys(opts, known, _err, "smime/bad-input", function (k) {
    return "unknown option " + _stringify(k) + " for pki.smime." + verb + " -- accepted: " +
...
// A null prototype: the key is a digest name recovered from the message, so an inherited
// Object.prototype member must not answer for a micalg row this table never held.
var MICALG = _assign(_create(null), { sha1: "sha-1", sha224: "sha-224", sha256: "sha-256", sha384: "sha-384", sha512: "sha-512", md5: "md5", shake128: "shake128", shake256: "shake256" });
...
function _isStructural(lname) {
  return lname === "mime-version" || _strIndexOf(lname, "content-") === 0;
}
...
function _hpHeaderList(headers) {
  ...
  if (_isArray(headers)) {
    _forEach(headers, function (h) {
      ...
      _push(out, { name: name, value: v == null ? "" : _String(v) });
    });
  } else if (typeof headers === "object") {
    _forEach(_keys(headers), function (k) { var v = headers[k]; _push(out, { name: k, value: v == null ? "" : _String(v) }); });
  } else {
  ...
  _forEach(out, function (h) {
    var ln = _toLowerCase(h.name);
    if (_isStructural(ln)) throw _err("smime/bad-input", "opts.headers must not include the Structural header " + _stringify(h.name) + " (RFC 9787); only Non-Structural fields (Subject / From / To / ...) are protected");
    ...
    if (seen[ln]) throw _err("smime/bad-input", "opts.headers must not repeat a protected field name (" + _stringify(h.name) + "); a repeated protected field is not supported");
    seen[ln] = 1;
  });
  return out;
}
...
function _outerHeaderList(list, mode, hcp) {
  ...
  if (policy !== "hcp_baseline" && policy !== "hcp_no_confidentiality") throw _err("smime/bad-input", "unknown opts.hcp policy " + _stringify(hcp) + " (only \"hcp_baseline\" or \"hcp_no_confidentiality\")");
  var out = [];
  _forEach(list, function (h) {
    if (mode === "cipher") {
      var v = _applyHcp(h.name, h.value, policy);
      if (v !== null) _push(out, { name: h.name, value: v });
    } else {
      _push(out, { name: h.name, value: h.value });
    }
  });
  return out;
}
...
function _protectedInnerEntity(content, opts, mode, hpList, outerList) {
  ...
  if (opts.contentType != null && mime.hasParam(_String(opts.contentType), "hp")) throw _err("smime/bad-input", "opts.contentType must not set the hp parameter -- header protection sets it (hp=" + _stringify(mode) + ")");
  ...
  _forEach(hpList, function (h) { _push(fields, h); });
  ...
  if (mode === "cipher") {
    _forEach(outerList, function (h) { _push(fields, { name: "HP-Outer", value: h.name + ": " + h.value }); });
  }
...
function _utf8Header(s) { return _bufferFrom(s, "latin1").toString("utf8"); }
...
function _declaresHp(content) {
  ...
  var s = _bufToString(content, "latin1");
  var m = s.match(/\r\n\r\n/);
  var head = (m ? _strSlice(s, 0, m.index) : s).replace(/\r\n[ \t]+/g, " ");
...
function _contentTypeCount(headers) { var n = 0; _forEach(headers, function (h) { if (h.lname === "content-type") n++; }); return n; }
...
function _extractProtected(headerList) {
  ...
  var protectedHeaders = _create(null), innerFrom = null, seen = _create(null), dup = null, dupSingleton = null;
  ...
  _forEach(headerList, function (h) {
    ...
    if (h.lname === "hp-outer") {
      ...
      var ci = _strIndexOf(h.rawValue, ":");
      if (ci >= 0) _push(refouter, { name: _strSlice(h.rawValue, 0, ci).trim().toLowerCase(), value: _strSlice(h.rawValue, ci + 1).trim() });
      return;
    }
    var val = _utf8Header(h.rawValue);                    // the exact authenticated field body (RFC 6532 UTF-8)
    _push(entries, { name: h.name, value: val, raw: h.rawValue });   // EVERY occurrence: `value` (UTF-8) for surfacing, `raw` (latin1) for the octet-exact confidentiality match
...
function _computeConfidential(entries, refouter) {
  ...
  var byName = new Map();
  _forEach(refouter, function (r) {
    var m = byName.get(r.name); if (!m) { m = new Map(); byName.set(r.name, m); }
    m.set(r.value, (m.get(r.value) || 0) + 1);
  });
  var confidential = [], seen = _create(null);
  _forEach(entries, function (e) {
    var m = byName.get(_toLowerCase(e.name)), val = _trim(e.raw), n = (m && m.get(val)) || 0;
    if (n > 0) { m.set(val, n - 1); return; }   // an outer occurrence accounts for (exposes) this one
    if (!seen[e.name]) { seen[e.name] = 1; _push(confidential, e.name); }
  });
  return confidential;
}
...
function _computeFromMismatch(innerFrom, outerEnt) {
  ...
  var outerFroms = [];
  _forEach(outerEnt.headers, function (h) { if (h.lname === "from") _push(outerFroms, _trim(h.value)); });   // byte-preserving (latin1)
  return outerFroms.length !== 1 || outerFroms[0] !== _trim(innerFrom);
}
...
function _outerRefouter(outerEnt) {
  var r = [];
  _forEach(outerEnt.headers, function (h) { if (!_isStructural(h.lname) && h.lname !== "hp-outer") _push(r, { name: h.lname, value: _trim(h.rawValue) }); });
  return r;
}
...
function _legacyHpSurface(canon, outerEnt, mode, enabled) {
  ...
  var headers = _map(ex.entries, function (e) { return { name: e.name, value: e.value }; });
...
function _signerEmails(certDer) {
  ...
  (parsed.extensions || []).forEach(function (e) {
    ...
    (dec.names || []).forEach(function (n) {
      if (n.tagNumber === 1) { _push(out.addresses, _String(n.value)); return; }   // rfc822Name
      ...
      if (_String(typeId) === SMTP_UTF8_MAILBOX) out.unreadable++;
    });
  });
  ...
  (rdns || []).forEach(function (rdn) {
    (rdn || []).forEach(function (attr) {
      ...
      if (_indexOf(out.addresses, attr.value) === -1) _push(out.addresses, attr.value);
    });
  });
  return out;
}
...
function _senderSurface(signers, outerEnt, expectedSender) {
  ...
  if (expectedSender != null) {
    ...
    var froms = [];
    _forEach(outerEnt.headers, function (h) { if (h.lname === "from") _push(froms, _trim(h.value)); });
  ...
  (signers || []).forEach(function (s) {
    ...
    _forEach(e.addresses, function (a) { sawIdentity = true; if (_indexOf(identities, a) === -1) _push(identities, a); });
  });
...
function _addrSpec(v) {
  var lt = _strIndexOf(v, "<"), gt = _strLastIndexOf(v, ">");
  if (lt === -1 && gt === -1) return _strIndexOf(v, ",") === -1 ? _trim(v) : null;
  ...
  if (_strSlice(v, gt + 1).trim().length) return null;
  var inner = _strSlice(v, lt + 1, gt).trim();
  return inner.length && _strIndexOf(inner, ",") === -1 && _strIndexOf(inner, "<") === -1 ? inner : null;
}
...
function _hpSurface(content, outerEnt, expectedMode, authenticated, legacyEnabled) {
  ...
  var hp = _toLowerCase(raw);
  if (hp !== "clear" && hp !== "cipher") throw _err("smime/bad-header-protection", "the header-protected payload declares an invalid hp value " + _stringify(raw) + " (only clear / cipher)");
  if (hp !== expectedMode) throw _err("smime/bad-header-protection", "the payload hp=" + _stringify(hp) + " contradicts the cryptographic envelope (a " + (expectedMode === "cipher" ? "decrypted" : "signed") + " message requires hp=" + _stringify(expectedMode) + ")");
  ...
  if (ex.dup) throw _err("smime/bad-header-protection", "a header-protected payload has a duplicate protected header field " + _stringify(ex.dup));
...
function _micalgSet(value) {
  var seen = [];
  _String(value).split(",").forEach(function (m) { var t = _trim(m).toLowerCase(); if (t && _indexOf(seen, t) < 0) _push(seen, t); });
  return _sort(seen).join(",");
}
...
function _micalgOf(p7Der) {
  ...
  (parsed.signerInfos || []).forEach(function (si) {
    ...
    if (mic && _indexOf(out, mic) < 0) _push(out, mic);
  });
  return out.length ? _sort(out).join(",") : null;
}
...
function _base64Body(der) {
  return _bufferFrom(_bufToString(der, "base64").replace(/(.{64})/g, "$1\r\n").replace(/\r\n$/, ""), "latin1");
}
...
async function sign(content, signers, opts) {
  ...
  if (form === "pkcs7-mime") {
    ...
    var head = _bufferFrom(outer + "Content-Type: application/pkcs7-mime; smime-type=signed-data; name=smime.p7m\r\nContent-Transfer-Encoding: base64\r\nContent-Disposition: attachment; filename=smime.p7m\r\n\r\n", "utf8");
    return _capped(_bufferConcat([head, _base64Body(_toBuf(p7m)), mime.CRLF]));
  }
  ...
  var head2 = _bufferFrom(outer + "Content-Type: multipart/signed; protocol=\"application/pkcs7-signature\"; micalg=" + micalg + "; boundary=\"" + boundary + "\"\r\n\r\n", "utf8");
  var sigPart = _bufferConcat([
    _bufferFrom("Content-Type: application/pkcs7-signature; name=smime.p7s\r\nContent-Transfer-Encoding: base64\r\nContent-Disposition: attachment; filename=smime.p7s\r\n\r\n", "latin1"),
    _base64Body(p7s),
  ]);
  var dl = _bufferFrom("--" + boundary + "\r\n", "latin1"), close = _bufferFrom("--" + boundary + "--\r\n", "latin1");
  return _capped(_bufferConcat([head2, dl, entity, mime.CRLF, dl, sigPart, mime.CRLF, close]));
}
...
async function verify(message, opts) {
  ...
  if (_isPkcs7(ct.type, "mime")) {
    if (ct.params["smime-type"] && ct.params["smime-type"] !== "signed-data") throw _err("smime/unsupported-type", "unsupported smime-type " + _stringify(ct.params["smime-type"]) + " (only signed-data)");
    ...
    return _assign({ valid: res.valid, trusted: res.trusted, signers: res.signers, form: "pkcs7-mime", content: inner, micalg: null, sender: _senderSurface(res.signers, ent, opts.expectedSender) }, _hpSurface(inner, ent, "clear", res.valid, opts.legacyHeaderProtection === true));
  }
  if (ct.type === "multipart/signed") {
    ...
    var res2 = await cms.verify(p7s, _assign({}, vOpts, { content: canon }));
    ...
    if (opts.strictMicalg && micalg && _micalgSet(micalg) !== (_micalgOf(p7s) || "")) {
      throw _err("smime/micalg-mismatch", "the multipart/signed micalg " + _stringify(micalg) + " disagrees with the SignerInfo digests");
    }
    return _assign({ valid: res2.valid, trusted: res2.trusted, signers: res2.signers, form: "multipart/signed", content: parts[0], micalg: micalg, sender: _senderSurface(res2.signers, ent, opts.expectedSender) }, _hpSurface(parts[0], ent, "clear", res2.valid, opts.legacyHeaderProtection === true));
  }
  throw _err("smime/unsupported-type", "not a signed S/MIME message (Content-Type " + _stringify(ct.type) + ")");
}
...
function _pkcs7MimeHead(smimeType, name) {
  return _bufferFrom("Content-Type: application/pkcs7-mime; smime-type=" + smimeType + "; name=" + name + "\r\nContent-Transfer-Encoding: base64\r\nContent-Disposition: attachment; filename=" + name + "\r\n\r\n", "latin1");
}
...
async function encrypt(content, recipients, opts) {
  ...
  var outer = hp ? _bufferFrom(_outerPrefix(outerList), "utf8") : _bufferAlloc(0);
  ...
  var recips = _isArray(recipients) ? recipients : [recipients];
  ...
  return _capped(_bufferConcat([outer, head, _base64Body(der), mime.CRLF]));
}
...
async function decrypt(message, keyMaterial, opts) {
  ...
  var ent = mime.parse(message, SmimeError, "smime/bad-mime");
  var ct = ent.contentType;
  if (!_isPkcs7(ct.type, "mime")) throw _err("smime/unsupported-type", "not an encrypted S/MIME message (Content-Type " + _stringify(ct.type) + ")");
  ...
  var stLower = st ? _toLowerCase(st) : st;
  if (st && stLower !== "enveloped-data" && stLower !== "authenveloped-data") throw _err("smime/unsupported-type", "unsupported smime-type " + _stringify(st) + " (only enveloped-data / authEnveloped-data)");
  var der = _decodeCms(ent);
  var smimeType = _envelopedTypeOf(der, "smime/unsupported-type");
  if (opts.strictSmimeType && st && stLower !== _toLowerCase(smimeType)) throw _err("smime/smime-type-mismatch", "the header smime-type " + _stringify(st) + " disagrees with the CMS body (" + smimeType + ")");
  var cmsOpts = {};
...
async function compress(content, opts) {
  ...
  return _capped(_bufferConcat([head, _base64Body(der), mime.CRLF]));
}
...
async function decompress(message, opts) {
  ...
  var ent = mime.parse(message, SmimeError, "smime/bad-mime");
  var ct = ent.contentType;
  if (!_isPkcs7(ct.type, "mime")) throw _err("smime/unsupported-type", "not a compressed S/MIME message (Content-Type " + _stringify(ct.type) + ")");
  // The smime-type header is advisory (the CMS body is authoritative) -- compare case-insensitively.
  var st = ct.params["smime-type"];
  if (st && _toLowerCase(st) !== "compressed-data") throw _err("smime/unsupported-type", "unsupported smime-type " + _stringify(st) + " (only compressed-data)");
  var der = _decodeCms(ent);
  var dOpts = {};
...
function _decodeCms(ent) {
  ...
  if (ent.cte === "base64") return guard.encoding.base64(_bufToString(ent.body, "latin1").replace(/[\r\n\t ]/g, ""), C.LIMITS.MIME_MAX_BYTES, _err, "smime/bad-mime", "the CMS body");
  return ent.body;
}

function _toBuf(v) { return _isBuffer(v) ? v : _bufferFrom(v); }
