...
function _senderSurface(signers, outerEnt, expectedSender) {
  ...
  for (var i = 0; i < identities.length; i++) {
    ...
    if (v === "match") return { checked: true, expected: expected, source: source, identities: identities, match: true };
    if (v === "not-comparable") sawNotComparable = true;
  }
  return {
    checked: true, expected: expected, source: source, identities: identities,
    match: (sawNotComparable || undecidable) ? null : false,
  };
}
