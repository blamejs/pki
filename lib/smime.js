...
// A null-prototype set so a lookup never sees an inherited Object.prototype key (a header field named
// "Constructor" / "toString" / "hasOwnProperty" must not be mistaken for a Structural field).
var _STRUCTURAL = Object.assign(Object.create(null), { "content-type": 1, "content-transfer-encoding": 1, "content-disposition": 1, "content-id": 1, "content-description": 1, "mime-version": 1 });
...
function _hpHeaderList(headers) {
  ...
  out.forEach(function (h) {
    ...
    if (_STRUCTURAL[ln]) throw _err("smime/bad-input", "opts.headers must not include the Structural header " + JSON.stringify(h.name) + " (RFC 9787); only Non-Structural fields (Subject / From / To / ...) are protected");
...
function _protectedInnerEntity(content, opts, mode) {
  ...
  var raw = guard.bytes.view(content, SmimeError, "smime/bad-input", "content");
  // Header protection sets the hp parameter itself; a caller hp= in opts.contentType would emit two hp
  // parameters (which verify then rejects), so reject it here rather than produce an unconsumable message.
  if (opts.contentType != null && mime.paramCount(String(opts.contentType), "hp") > 0) throw _err("smime/bad-input", "opts.contentType must not set the hp parameter -- header protection sets it (hp=" + JSON.stringify(mode) + ")");
...
function _declaresHp(content) {
  // Normalize line endings the SAME way mime.canonicalize / mime.parse do (bare CR and bare LF -> CRLF)
  // BEFORE the prescan. A transport that rewrites a CRLF fold to a bare CR still verifies (the canonicalizer
  // maps it back), so if the prescan did not normalize identically an attacker could strip the hp signal from
  // a valid signed message -- returning it downgraded to protectedHeaders:null. content is a Buffer (the
  // verify/decrypt recovered bytes).
  var s = mime.canonicalizeText(content).toString("latin1");
  var m = s.match(/\r\n\r\n/);
  // mime.parse trims the field name (leading + trailing whitespace), so it accepts both a leading-whitespace
  // line (" Content-Type:") and whitespace before the colon ("Content-Type :"). The prescan must not be
  // stricter than the parser on either, or a valid hp-bearing payload silently downgrades to protectedHeaders:null.
...
function _hpSurface(content, outerEnt, expectedMode, authenticated) {
  ...
  // A duplicate hp parameter is ambiguous (mime.parse keeps the LAST value, but a recipient honoring the
  // first would see a different mode) -- fail closed, like the duplicate Content-Type. Counted quote-aware
  // (an hp= inside a quoted param value is not the hp parameter, so a message we emit is never self-rejected).
  if (mime.paramCount(inner.contentType.value, "hp") > 1) throw _err("smime/bad-header-protection", "a header-protected payload declares the hp parameter more than once");
  ...
  var protectedHeaders = Object.create(null), innerFrom = null, seen = Object.create(null), dup = null;
  ...
  var outerFroms = [];
  outerEnt.headers.forEach(function (h) { if (h.lname === "from") outerFroms.push(_utf8Header(h.value).trim()); });
