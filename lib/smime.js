...
function _hpSurface(content, outerEnt, expectedMode, authenticated) {
  ...
  // first would see a different mode) -- fail closed, like the duplicate Content-Type.
  if ((inner.contentType.value.match(/;[ \t]*hp[ \t]*=/gi) || []).length > 1) throw _err("smime/bad-header-protection", "a header-protected payload declares the hp parameter more than once");
  ...
  var fromMismatch = innerFrom != null && (outerFroms.length > 1 || (outerFroms.length === 1 && outerFroms[0] !== innerFrom.trim()));
