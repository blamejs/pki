...
function _outerHeaderList(list, mode, hcp) {
  ...
  if (typeof policy !== "string") throw _err("smime/bad-input", "opts.hcp must be a policy name string");
  var out = [];
...
function _hpSurface(content, outerEnt, expectedMode) {
  ...
  var hp = inner.contentType && inner.contentType.params ? inner.contentType.params.hp : undefined;
  ...
  inner.headers.forEach(function (h) {
    ...
    protectedHeaders[h.name] = h.value;
    if (h.lname === "from") innerFrom = h.value;
  });
  var outerFrom = outerEnt.header("From");
  var fromMismatch = innerFrom != null && outerFrom != null && String(outerFrom).trim() !== String(innerFrom).trim();
...
async function verify(message, opts) {
  ...
  if (_isPkcs7(ct.type, "mime")) {
    ...
    return Object.assign({ valid: res.valid, signers: res.signers, form: "pkcs7-mime", content: inner, micalg: null }, _hpSurface(inner, ent, "clear"));
  }
  if (ct.type === "multipart/signed") {
    ...
    return Object.assign({ valid: res2.valid, signers: res2.signers, form: "multipart/signed", content: parts[0], micalg: micalg }, _hpSurface(parts[0], ent, "clear"));
  }
...
async function decrypt(message, keyMaterial, opts) {
  ...
  }, _hpSurface(res.content, ent, "cipher"));
}
