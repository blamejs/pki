...
function _noneSurface() { return { protectedHeaders: null, headerProtection: { present: false, mode: null, fromMismatch: false, confidential: [], legacy: false } }; }
...
// Returns { protectedHeaders (UTF-8), protectedRaw (byte-preserving), innerFrom, refouter (HP-Outer records), dup }.
function _extractProtected(headerList) {
  ...
  return { protectedHeaders: protectedHeaders, protectedRaw: protectedRaw, innerFrom: innerFrom, refouter: refouter, dup: dup };
}

// RFC 9788 sec. 4.3.1: a protected field is end-to-end confidential (encrypted-only) unless a `refouter` record
// carries its EXACT name + value -- i.e. it was copied verbatim to the visible outer section. A field with no
// matching record (removed, or obscured to a different outer value) was made confidential. The comparison is
// octet-exact (latin1), never lossy-decoded. For the standard path `refouter` is the HP-Outer records; for the
// legacy path it is the actual outer Header Section (part A).
function _computeConfidential(protectedHeaders, protectedRaw, refouter) {
  var confidential = [];
  Object.keys(protectedHeaders).forEach(function (name) {
    var ln = name.toLowerCase(), val = protectedRaw[name].trim(), exposed = false;
...
// (`enabled`): the default posture stays protectedHeaders:null so an ordinary signed/forwarded message/rfc822 is
// never mis-surfaced. Identification (sec. 4.10.1) is the four conjunctive conditions C1-C4; every failure -- an
...
// identified. On identification (sec. 4.10.2) the inner message's (part D's) Non-Structural fields ARE the
// surfaced set, the mode is inferred from the envelope (`mode`: clear for signed, cipher for encrypted), and the
// confidential set is derived from the actual outer section (part A). `legacy:true` marks the weaker, structurally
// inferred provenance so a caller never conflates it with a cryptographically-declared hp= set.
function _legacyHpSurface(canon, outerEnt, mode, enabled) {
  ...
  if (ex.dup) return _noneSurface();                                     // an ambiguous duplicate protected field -- fail soft (unlike the standard path's throw)
  if (!Object.keys(ex.protectedHeaders).length) return _noneSurface();   // no Non-Structural fields -- nothing to surface
  var confidential = mode === "cipher" ? _computeConfidential(ex.protectedHeaders, ex.protectedRaw, _outerRefouter(outerEnt)) : [];
  return { protectedHeaders: ex.protectedHeaders, headerProtection: { present: true, mode: mode, fromMismatch: _computeFromMismatch(ex.innerFrom, outerEnt), confidential: confidential, legacy: true } };
}
...
function _hpSurface(content, outerEnt, expectedMode, authenticated, legacyEnabled) {
  ...
  var confidential = hp === "cipher" ? _computeConfidential(ex.protectedHeaders, ex.protectedRaw, ex.refouter) : [];
  return { protectedHeaders: ex.protectedHeaders, headerProtection: { present: true, mode: hp, fromMismatch: _computeFromMismatch(ex.innerFrom, outerEnt), confidential: confidential, legacy: false } };
}
 ...
 * (`smime/bad-header-protection`), never a silent downgrade. `legacy` is `true` only when the set was recovered
 * from a legacy RFC 8551 `message/rfc822` wrap via `opts.legacyHeaderProtection` (a weaker, structurally inferred
 * provenance -- see that option); it is `false` for a standard hp= set and for a non-protected message.
 *
 ...
 * @opts legacyHeaderProtection  opt in to detecting a LEGACY RFC 8551 header-protected message (RFC 9788 sec. 4.10): a Cryptographic Payload that is a bare `message/rfc822` wrap with no `hp=` parameter. When set, a precisely-identified legacy message surfaces the inner message's headers as `protectedHeaders` with `headerProtection.legacy: true` and the mode inferred from the envelope (`clear` here); anything not precisely identified stays `protectedHeaders: null` (fail-safe). Off by default, so an ordinary signed/forwarded `message/rfc822` is never mis-surfaced. The signed-and-encrypted form (RFC 9788 Appendix C.3.17) surfaces here as `clear` via the caller's re-`verify` step -- the non-recursive layered API exposes no single seam holding both the inner signature verdict and the outer header section.
 * @example
 ...
 * `legacy` is `true` only for a legacy RFC 8551 `message/rfc822` wrap recovered via `opts.legacyHeaderProtection`.
 *
 ...
 * @opts legacyHeaderProtection  opt in to detecting a LEGACY RFC 8551 header-protected message (RFC 9788 sec. 4.10): an encrypted Cryptographic Payload that is a bare `message/rfc822` wrap with no `hp=` parameter. When set, a precisely-identified legacy message surfaces the inner headers as `protectedHeaders` with `headerProtection.legacy: true`, `mode: "cipher"`, and a `confidential` set derived from the actual visible outer Header Section; anything not precisely identified stays `protectedHeaders: null` (fail-safe). Off by default.
 * @example
