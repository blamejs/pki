...
function _hpSurface(content, outerEnt, expectedMode, authenticated) {
  ...
  inner.headers.forEach(function (h) {
    ...
    if (h.lname === "hp-outer") {
      ...
      // -> refouter. HP-Outer is NEVER surfaced as a protected header nor counted toward the duplicate check
      // (sec. 2.2: it "can appear multiple times"). A valueless HP-Outer (no inner colon) is ignored.
      var ci = h.rawValue.indexOf(":");
      if (ci >= 0) refouter.push({ name: h.rawValue.slice(0, ci).trim().toLowerCase(), value: _utf8Header(h.rawValue.slice(ci + 1)).trim() });
      return;
    }
  ...
  if (hp === "cipher") {
    Object.keys(protectedHeaders).forEach(function (name) {
      var ln = name.toLowerCase(), val = protectedHeaders[name].trim(), exposed = false;
