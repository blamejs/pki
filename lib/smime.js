...
function _protectedInnerEntity(content, opts, mode, hpList, outerList) {
  ...
  var raw = guard.bytes.view(content, SmimeError, "smime/bad-input", "content");
  if (opts.contentType != null && typeof opts.contentType !== "string") throw _err("smime/bad-input", "opts.contentType must be a string, got " + guard.text.showValue(opts.contentType));
  if (opts.contentType != null && mime.hasParam(String(opts.contentType), "hp")) throw _err("smime/bad-input", "opts.contentType must not set the hp parameter -- header protection sets it (hp=" + guard.text.showValue(mode) + ")");
...
function _signerEmails(certDer) {
  ...
  (parsed.extensions || []).forEach(function (e) {
    ...
    (dec.names || []).forEach(function (n) {
      if (n.tagNumber === 1) { out.addresses.push(String(n.value)); return; }
