...
function _snapshotSignersFitForMail(signers) {
  /** @internal A sparse list is refused at its first missing slot, as pki.cms.sign refuses it,
   * before any slot is visited; a list whose own traps throw is this verb's error too. */
  var list;
...
function _readDescriptor(s, position) {
  ...
  try {
    ...
    for (var y = 0; y < symbols.length; y++) copy[symbols[y]] = s[symbols[y]];
    /** @internal The walk is bounded: a proxy may answer itself, or another visited object, as its
     * prototype, and a chain has a depth no descriptor reaches. */
    var visited = [], depth = 0;
