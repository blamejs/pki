 ...
 * is signed, and handed on to pki.cms.sign carrying the SAME certificate: each descriptor's `cert`
 * is read once and coerced to DER, and pki.cms.sign receives a descriptor that inherits every other
 * field from the caller's (own or inherited, exactly as it would have read them) and carries that
 * DER as its one own property, so a `cert` accessor cannot answer one certificate to this check and
 * another to the signing beneath. A single descriptor is the list of one pki.cms.sign accepts.
 * Anything that is not a descriptor travels as given, for pki.cms.sign to refuse in its own words. */
function _snapshotSignersFitForMail(signers) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    /** @internal The one read of `cert`, whatever it answers: an absent certificate is forwarded as
     * absent, so an accessor cannot answer nothing here and a certificate to the signing beneath.
     * A present one is an owned copy of the bytes, not a view, since a later descriptor's accessor
     * runs after this check and could rewrite a buffer the caller still holds. */
    var cert = s.cert;
    if (cert != null) {
      cert = guard.bytes.snapshot(pkix.coerceToDer(cert, { pemLabel: "CERTIFICATE", PemError: SmimeError, ErrorClass: SmimeError, prefix: "smime" }), SmimeError, "smime/bad-input", "signer " + (i + 1) + "'s certificate");
      _assertSignerFitForMail(cert, i + 1);
    }
    var copy = Object.create(s);
    Object.defineProperty(copy, "cert", { value: cert, enumerable: true, writable: false, configurable: false });
