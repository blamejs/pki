...
// ---- header parsing by character scan (no regex) ----------------------------------------------
// ASCII lowercase of a code point (A-Z -> a-z), for the case-insensitive "content-type" literal match.
function _asciiLower(c) { return (c >= 65 && c <= 90) ? c + 32 : c; }
var _CONTENT_TYPE = "content-type";

// RFC 5322 header unfolding: a CRLF followed by one-or-more SP/HTAB (a folded continuation) collapses
// to a single space (matching /\r\n[ \t]+/g -> " ").
...
// If the line beginning at `start` is a Content-Type field -- ^[ \t]*content-type[ \t]*: -- return the
// whole line up to (not including) the next CR or LF (the [^\r\n]* tail); else null. Case-insensitive
// on the literal, matching the /i flag over the ASCII field name.
...
// Every Content-Type field in the header block (the /gim match, /m making ^ match each line start):
// scan the block start and each position after an LF. Returns the matched lines, or null when none.
...
// The ^[ \t]*content-type[ \t]*: prefix removed from a matched Content-Type line (the value remains).
...
// Base64 wrapped to 64-column lines joined by CRLF, no trailing CRLF (matching (.{64})->"$1\r\n"
// then a trailing-CRLF strip; base64 output carries no CRLF of its own).
