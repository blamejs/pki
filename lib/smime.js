...
// Returns { protectedHeaders (UTF-8), protectedRaw (byte-preserving), innerFrom, refouter (HP-Outer records),
// dup (any duplicate field name), dupSingleton (a duplicate of an RFC 5322 singleton field) }.
function _extractProtected(headerList) {
  ...
  var protectedHeaders = Object.create(null), protectedRaw = Object.create(null), innerFrom = null, seen = Object.create(null), dup = null, dupSingleton = null;
  ...
  return { protectedHeaders: protectedHeaders, protectedRaw: protectedRaw, innerFrom: innerFrom, refouter: refouter, dup: dup, dupSingleton: dupSingleton };
}
...
function _legacyHpSurface(canon, outerEnt, mode, enabled) {
  ...
  // caller cannot mistake an opt-in heuristic (indistinguishable from a forwarded message/rfc822) for authenticated headers.
  return { protectedHeaders: null, headerProtection: { present: false, mode: null, fromMismatch: false, confidential: [], legacy: { headers: ex.protectedHeaders, mode: mode, fromMismatch: _computeFromMismatch(ex.innerFrom, outerEnt), confidential: confidential } } };
}
 ...
 * object -- a legacy inference is NEVER placed in `protectedHeaders` and never sets `present: true`, because it is
 ...
 * legacy RFC 8551 `message/rfc822` wrap (its own `{ headers, mode, fromMismatch, confidential }` object, never
 * merged into `protectedHeaders` / `present`).
 *
