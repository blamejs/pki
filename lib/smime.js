...
function _snapshotSignersFitForMail(signers) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (copy.cert != null) {
      copy.cert = guard.bytes.snapshot(pkix.coerceToDer(copy.cert, { pemLabel: "CERTIFICATE", PemError: SmimeError, ErrorClass: SmimeError, prefix: "smime" }), SmimeError, "smime/bad-input", "signer " + (i + 1) + "'s certificate");
