...
// Returns { protectedHeaders (UTF-8 map, last-wins), protectedRaw (byte-preserving map), entries (an ORDERED
// array of every { name, value } occurrence -- a real message repeats trace fields, and last-wins would drop
// all but one), innerFrom, refouter (HP-Outer records), dup (any duplicate field name), dupSingleton (a
// duplicate of an RFC 5322 singleton field) }.
function _extractProtected(headerList) {
  ...
  headerList.forEach(function (h) {
    ...
    entries.push({ name: h.name, value: val });           // record EVERY occurrence (a legacy message legitimately repeats Received / trace fields)
    if (seen[h.lname]) { dup = h.name; if (RFC5322_SINGLETON[h.lname]) dupSingleton = h.name; }
    seen[h.lname] = 1;
    protectedHeaders[h.name] = val;                       // SURFACE (map, last-wins -- the ergonomic single-value view; `entries` keeps all)
    protectedRaw[h.name] = h.rawValue;                    // ...retain the BYTE-PRESERVING (latin1) body for the octet-exact confidentiality comparison
  ...
  return { protectedHeaders: protectedHeaders, protectedRaw: protectedRaw, entries: entries, innerFrom: innerFrom, refouter: refouter, dup: dup, dupSingleton: dupSingleton };
}
...
function _legacyHpSurface(canon, outerEnt, mode, enabled) {
  ...
  // headers. `headers` is the ORDERED [{ name, value }] list (every occurrence -- a real message repeats trace fields).
  return { protectedHeaders: null, headerProtection: { present: false, mode: null, fromMismatch: false, confidential: [], legacy: { headers: ex.entries, mode: mode, fromMismatch: _computeFromMismatch(ex.innerFrom, outerEnt), confidential: confidential } } };
}
