...
function _protectedInnerEntity(content, opts, mode, hpList) {
...
// Content-Type in a header-protected message. Each field is guard-validated (no CR/LF/NUL injection). hpList
// is the SAME normalized snapshot the inner entity used.
function _outerPrefix(opts, mode, hpList) {
  var list = _outerHeaderList(hpList, mode, opts.hcp);
  var s = "";
...
function _declaresHp(content) {
  ...
  for (var i = 0; i < cts.length; i++) {
    if (mime.paramCount(cts[i].replace(/^[ \t]*content-type[ \t]*:/i, ""), "hp") > 0) return true;
  }
  return false;
}
...
function _hpSurface(content, outerEnt, expectedMode, authenticated) {
  ...
  var hp = inner.contentType.params.hp;
...
async function sign(content, signers, opts) {
  ...
  var entity = hp ? _protectedInnerEntity(content, opts, "clear", hpList) : _entityBytes(content, opts);
  var outer = hp ? _outerPrefix(opts, "clear", hpList) : "";
...
async function encrypt(content, recipients, opts) {
  ...
  var entity = hp ? _protectedInnerEntity(content, opts, "cipher", hpList) : _entityBytes(content, opts);
  ...
  var outer = hp ? Buffer.from(_outerPrefix(opts, "cipher", hpList), "utf8") : Buffer.alloc(0);
