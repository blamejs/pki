...
var POLICY_INFORMATION = schema.seq([
  ...
  build: function (m, ctx) {
    return {
      ...
      policyQualifiers: m.fields.policyQualifiers.present ? m.fields.policyQualifiers.node.bytes : null,
    };
  },
});
 ...
 * @example
 *   var b = pki.asn1.build;
 *   var av = b.sequence([b.sequence([b.octetString(Buffer.alloc(32, 2))])]);
...
function decodeAttribute(attr) {
  ...
  if (attr.type === OID_SMIME_CAPABILITIES) return { kind: "smimeCapabilities", capabilities: parseSmimeCapabilities(value).capabilities };
