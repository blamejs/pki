...
// then narrows to the ESS profile: RFC 5035 §5 — for a (non-attribute) certificate
// reference, the issuer MUST contain ONLY the certificate's issuer name encoded in
// the directoryName [4] choice, so exactly one directoryName GeneralName is legal.
...
var ISSUER_SERIAL = schema.seq([
  ...
  build: function (m, ctx) {
    var issuer = m.fields.issuer.value.result;
    var names = issuer.names;
    if (names.length !== 1 || names[0].tagClass !== "context" || names[0].tagNumber !== GENERAL_NAME_DIRECTORY) {
      throw ctx.E("smime/bad-issuer-serial",
        "IssuerSerial issuer MUST be exactly one directoryName [4] GeneralName (the certificate issuer name, RFC 5035 §5)");
    }
    return {
      issuer: issuer,
