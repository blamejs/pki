...
function session(opts) {
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    try { res = await _engine.build(leaf, { trustAnchors: _asCertList(opts.trustAnchors), intermediates: pool, validate: true, time: opts.time != null ? opts.time : new Date() }); }
      ...
      // path.build THROWS path/no-path (and its siblings) when the issued leaf cannot chain to a supplied anchor
      // -- a CA that grants a certificate whose issuer it never delivered (no caPubs, not in extraCerts). Re-type
      // it to the domain error so the transaction fails closed before confirmation, never leaking a path/* error.
