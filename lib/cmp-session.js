...
function session(opts) {
  ...
  async function _send(bodySpec, arm) {
    ...
    // Size the signer-path reserve to the extraCerts cmp.verify may APPEND AS ISSUERS -- the response's distinct
    // extraCerts MINUS its signer (RFC 9483 sec. 3.3 places the protection cert first, and cmp.verify removes that
    // signer from the appended pool), so a response carrying only its signer reserves 0 and forfeits no caller
    // intermediate below the ceiling. cmp.transfer has already parsed res.responseBytes as a PKIMessage (its
    // cmp.parse gate on every return path), so re-reading its extraCerts here cannot throw.
    var appendReserve = Math.max(0, _responseExtraCerts(res.responseBytes).length - 1);
    ...
    if ((verdict.valid !== true || verdict.trusted !== true) && isSig && fallbackSigner != null && recoverable) {
      verdict = await cmp.verify(res.responseBytes, _verifyOpts(fresh, fallbackSigner, cachedChain, appendReserve));
      usedFallback = true;
    }
