...
function session(opts) {
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    var pool = [];
    if (Array.isArray(opts.intermediates)) pool = pool.concat(opts.intermediates);
    if (Array.isArray(caPubs)) caPubs.forEach(function (c) { if (Buffer.isBuffer(c)) pool.push(c); });
    var res;
    try { res = await _engine.build(leaf, { trustAnchors: opts.trustAnchors, intermediates: pool, validate: true, time: opts.time != null ? opts.time : new Date() }); }
    catch (e) {
