...
function session(opts) {
  ...
  async function _send(bodySpec, arm) {
    ...
    // Fallback: a signature-flavor response's OWN signer resolution can fail THREE ways that the earlier,
    // authenticated signer still recovers, all because extraCerts is OUTSIDE the protected part and a meddler
    // controls it -- (a) NO signer resolved (a later leg omitted extraCerts once the recipient holds the cert,
    // RFC 9483 sec. 3.3) -> signer-cert-not-found; (b) a WRONG signer resolved: a same-subject decoy with a
    // DIFFERENT key the resolver selects first -> protection-failed; (c) a decoy carrying the REAL signer's
    // public key + identity but chaining to an UNTRUSTED issuer -> the signature verifies (valid) yet trust
    // fails -> untrusted-signer. All three retry against the cached (already trusted) signer + its chain. The
    // response's own certs are still preferred on the FIRST attempt, so a clustered CA's legitimately rotated
    // cert wins there; the retry can only rescue a message ACTUALLY signed by the cached signer's fixed key,
    // never accept a genuinely tampered or genuinely untrusted one (a wrong-key message stays valid:false).
    var usedFallback = false;
    var recoverable = verdict.code === "cmp/signer-cert-not-found" || verdict.code === "cmp/protection-failed" || verdict.code === "cmp/untrusted-signer";
