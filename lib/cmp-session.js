...
function session(opts) {
  ...
  function _classify(verdict) {
    ...
    if (arm === expectedRespArm) {
      ...
      // array would retain unbounded material before validation. Cap the distinct count at the path-builder ceiling
      // it ultimately feeds, drop a byte-identical repeat, and REJECT a non-X.509 entry (an authenticated CA must
      ...
      if (body.decoded && Array.isArray(body.decoded.caPubs)) body.decoded.caPubs.forEach(function (c) {
        if (!Buffer.isBuffer(c) || caPubsAccum.length >= constants.LIMITS.PATH_BUILD_MAX_CANDIDATES) return;
