...
function session(opts) {
  ...
  // The caPubs byte + count budgets, proportional to the per-response cap. A leg's caPubs cannot exceed one
  // response, so reserving one response's worth (`_perRespCap` bytes + MAX_EXTRA_CERTS slots) below each budget for
  // the GRANT leg guarantees a flood of WAITING caPubs can never fill capacity and starve the issued leaf's own
  // required issuer -- while the total stays bounded (a count-only cap would still admit ~16 GiB of large certs).
  ...
  function _verifyOpts(fresh, signerCertOverride, extraIntermediates, distinctExtra) {
    ...
    // pool fills the rest, and a reserve is held below the ceiling so cmp.verify can append the response's OWN
    // extraCerts (its delivered issuers) -- so a ceiling-filling caller pool cannot starve either the cached chain
    // or the response's own issuer, and the signer still builds to the anchor. The reserve is the count cmp.verify
    // will actually APPEND from `distinctExtra`: on the PRIMARY attempt (no override) the signer is resolved FROM
    // extraCerts and cmp.verify removes it, so reserve one fewer; with an explicit signerCert (the cached or
    // prebound signer) the target is NOT taken from extraCerts, so EVERY extraCert can be appended as an issuer --
    // reserve the full count (a response whose extraCerts carries only the issuer, not the signer, needs it).
    var reserve = signerCertOverride != null ? distinctExtra : Math.max(0, distinctExtra - 1);
    var extra = [];
    ...
    var ints = _boundedPool(extra, _asCertList(opts.intermediates), reserve);
  ...
  async function _send(bodySpec, arm) {
    ...
    // The response's distinct extraCerts count -- _verifyOpts turns it into the signer-path reserve, differently
    // for the primary attempt (signer resolved FROM extraCerts, one fewer) vs a signerCert-override retry (every
    // extraCert appendable). cmp.transfer has already parsed res.responseBytes as a PKIMessage (its cmp.parse gate
    // on every return path), so re-reading its extraCerts here cannot throw.
    var distinctExtra = _responseExtraCerts(res.responseBytes).length;
    var verdict = await cmp.verify(res.responseBytes, _verifyOpts(fresh, null, null, distinctExtra));
    ...
    if ((verdict.valid !== true || verdict.trusted !== true) && isSig && fallbackSigner != null && recoverable) {
      verdict = await cmp.verify(res.responseBytes, _verifyOpts(fresh, fallbackSigner, cachedChain, distinctExtra));
      usedFallback = true;
    }
  ...
  function _classify(verdict) {
    ...
    if (arm === expectedRespArm) {
      ...
      // array would retain unbounded material before validation. Enforce a count AND a byte budget, holding one
      // response's worth of each in reserve for the GRANT leg so a WAITING flood cannot starve the issued leaf's own
      // required issuer; drop a byte-identical repeat; and REJECT a non-X.509 entry (an authenticated CA must
      // deliver real certificates; this is the fail-closed check the leaf validation would otherwise defer to _finish).
      var countCap = isGrantLeg ? constants.LIMITS.PATH_BUILD_MAX_CANDIDATES : (constants.LIMITS.PATH_BUILD_MAX_CANDIDATES - MAX_EXTRA_CERTS);
      var byteCap = isGrantLeg ? caPubsByteBudget : (caPubsByteBudget - _perRespCap);
      if (body.decoded && Array.isArray(body.decoded.caPubs)) body.decoded.caPubs.forEach(function (c) {
        if (!Buffer.isBuffer(c) || caPubsAccum.length >= countCap || caPubsBytes + c.length > byteCap) return;
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    try { res = await _engine.build(leaf, { trustAnchors: _asCertList(opts.trustAnchors), intermediates: pool, validate: true, time: opts.time != null ? opts.time : new Date() }); }
      ...
      /* c8 ignore next -- path.build's build() throws ONLY path/bad-input (its error contract; a chain/signature
         failure RETURNS valid:false, handled below). This branch is a defensive belt for a future non-path/bad-input
         throw and cannot be reached today; a leaf/anchor/time that would trigger one is already validated before here. */
