...
var RESUME_TOKEN_KEYS = Object.assign(Object.create(null), {
  ...
  signer: 1, signerCache: 1, chain: 1, caPubs: 1, nextPollAt: 1,
});
...
/** @internal A cap that is not a positive number does not refuse anything, so a bound built from a
 * session option is checked where it is built rather than where it is applied. `total` is the one bound
 * a list may go without, and it says so with null rather than by being absent. */
function _assertLimit(limit, name) {
  var ok = _isPositive(limit.count) && _isPositive(limit.each) &&
    (limit.total === null || _isPositive(limit.total));
  if (!ok) throw _err("cmp/bad-input", "the " + name + " restore limit is not a usable bound");
  return limit;
}
function _isPositive(n) { return typeof n === "number" && isFinite(n) && n > 0; }
/** @internal The widest request identifier a CertReqMsg can carry, written out in decimal: an INTEGER of
 * the codec's maximum width takes about 2.41 digits per byte. Converting a decimal string to a BigInt
 * costs more than linear time, so the length is checked before the conversion runs. */
 ...
 * grant arrives on, the polls already spent, the key the grant must certify, the signer identity the
 ...
 * enrollment's own state is stored. The authority's polling interval is waited out
 ...
 * hands back a token again, so a wait can span any number of processes. A token is refused before a
...
function session(opts) {
   ...
   * hold. The authority's own pool arrives in responses, so it keeps both the response size and the
  ...
  _assertLimit(CAPUBS_LIMIT, "caPubs");
  _assertLimit(CHAIN_LIMIT, "chain");
  ...
  function _readResumeToken(token) {
    ...
    var raw = {
      ...
      nextPollAt: token.nextPollAt,
    };
    var txn = _hexField(raw.transactionId, "transactionId");
    if (txn.length === 0) throw _err("cmp/bad-input", "the resumeToken transactionId must name the transaction to continue");
    var nonce = _b64Field(raw.recipNonce, "recipNonce");
    if (nonce.length === 0) throw _err("cmp/bad-input", "the resumeToken recipNonce must carry the nonce the next request echoes (RFC 9810 sec. 5.1.1)");
  ...
  async function resumePoll(token) {
    ...
    try {
      ...
      cachedSignerCert = t.signerCache != null ? t.signerCache : t.signer;
      cachedChain = t.chain;
      ...
      if (t.nextPollAt != null) {
        ...
        if (waitMs > 0) {
          ...
          if (waitSeconds > maxTotalWait) {
            return _terminal("poll-timeout", { status: null, polls: t.polls, resumeToken: _resumeToken(t.polls, t.nextPollAt) });
          }
          await sleep(waitMs);
      ...
      if (polled.timeout) return _terminal("poll-timeout", { status: polled.status, polls: spent, resumeToken: _resumeToken(spent, _pollDueAt(polled.dueIn)) });
