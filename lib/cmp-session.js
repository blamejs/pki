...
// entity picks one it can do. So the check refuses an algId ONLY when it names a KNOWN RSA algorithm
// (whose requirement is stated with rsaKeyLen instead); every other value -- a known non-RSA algorithm,
// or an OID this toolkit does not recognize -- is surfaced to the caller. Refusing an unrecognized
...
//
// RSA is registered across arcs -- PKCS#1 holds rsaEncryption, rsassaPss, rsaesOaep and the
// shaNWithRSAEncryption set; id-rsa-kem sits on the S/MIME arc; id-kem-rsa on the ISO 18033-2 arc --
// so the set is seeded from every RSA name at load rather than one arc or one OID. A composite
// signature that names RSA as one half is a composite key, not an RSA one, so it is deliberately
// absent and is surfaced. Identifiers rather than names, because pki.oid.register can replace a
// display name after load. This inverts a former non-RSA allowlist that wrongly refused conforming
// non-RSA algorithms it did not happen to list.
var RSA_ALG_OIDS = Object.create(null);
["rsaEncryption", "rsassaPss", "rsaesOaep",
...
function session(opts) {
  ...
  // True only for a KNOWN RSA algorithm, which sec. 4.3.3 excludes from a keySpec algId (RSA uses
  // rsaKeyLen). RSA is registered across arcs, so RSA_ALG_OIDS is seeded from every RSA name at load
  // rather than one arc or one OID; a newly named RSA algorithm is covered by adding it there. An OID
  // the registry cannot name is NOT RSA for this purpose -- it is surfaced, not refused (see below).
  function _isRsaAlgorithm(dotted) { return RSA_ALG_OIDS[dotted] === 1; }
