...
function session(opts) {
  ...
  async function info(request) {
    ...
    if (op.value === false) {
      ...
      itav.infoValue = guard.bytes.snapshot(request[names[0]], CmpError, "cmp/bad-input", "info request rootCaCert");
  ...
  function _asCertificate(der, what) {
    try { x509.parse(der); }
    catch (e) { throw _err("cmp/bad-info-value", what + " is not a valid X.509 certificate (RFC 9483 sec. 4.3)", e); }
  ...
  var OID_RSA_ENC = oid.byName("rsaEncryption");
  ...
  function _checkKeySpec(ctrl) {
    ...
    if (ctrl.type === OID_REG_CTRL_ALG_ID) {
      ...
      if (alg.oid === OID_RSA_ENC) throw _err("cmp/bad-info-value", "a keySpec algId must give an algorithm other than RSA -- an RSA requirement is stated with rsaKeyLen (RFC 9483 sec. 4.3.3)");
