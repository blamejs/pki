 ...
 *   - `expectedSender` -- optional CA signer CERTIFICATE (DER Buffer / PEM / parsed); when set, every signed response's authenticated header sender MUST bind to it under the RFC 5280 sec. 7 subject-or-subjectAltName rule cmp.verify uses, so a re-encoded-but-equivalent DN and an empty-subject CA named only by a directoryName SAN both match. Absent, the session pins the first signed response's signer certificate and requires every later leg's sender to bind to it -- rejecting a switch to a different trusted signer while permitting same-identity certificate/key rotation.
...
function session(opts) {
  ...
  // -- is bound correctly. Parse it at construction; a non-certificate value would otherwise consume the one-shot
  // transaction before the first verify could reject it.
  var _expectedSenderCert = null;
  if (opts.expectedSender != null) {
    try { _expectedSenderCert = x509.parse(opts.expectedSender); }
    catch (e) { throw _err("cmp/bad-input", "opts.expectedSender must be the CA signer certificate (DER Buffer / PEM / parsed) so every signed response can be bound to it -- " + ((e && e.message) || e), e); }
  }
  ...
  function _verifyOpts(fresh, signerCertOverride, extraIntermediates, appendReserve) {
    ...
    // pool fills the rest, and `appendReserve` slots are held below the ceiling so cmp.verify can append the
    // response's OWN extraCerts (its delivered issuers) -- so a ceiling-filling caller pool cannot starve either
    // the cached chain or the response's own issuer, and the signer still builds to the anchor. The reserve is
    // sized to THIS response's distinct extraCerts (which _responseExtraCerts dedups and bounds at MAX_EXTRA_CERTS),
    // not a static count: a response carrying only its signer (which cmp.verify removes from the appended pool)
    // needs almost none of it, so a fixed reserve would forfeit caller intermediates the transaction needs to chain.
    var extra = [];
    ...
    var ints = _boundedPool(extra, _asCertList(opts.intermediates), appendReserve);
  ...
  async function _send(bodySpec, arm) {
    ...
    // Size the signer-path reserve to THIS response's distinct extraCerts (the certs cmp.verify may append),
    // so a response that carries only its signer does not forfeit caller-intermediate slots (fix for a static
    // reserve truncating a needed issuer below the ceiling). cmp.transfer has already parsed res.responseBytes
    // as a PKIMessage (its cmp.parse gate on every return path), so re-reading its extraCerts here cannot throw.
    var appendReserve = _responseExtraCerts(res.responseBytes).length;
    var verdict = await cmp.verify(res.responseBytes, _verifyOpts(fresh, null, null, appendReserve));
    ...
    if ((verdict.valid !== true || verdict.trusted !== true) && isSig && cachedSignerCert != null && recoverable) {
      verdict = await cmp.verify(res.responseBytes, _verifyOpts(fresh, cachedSignerCert, cachedChain, appendReserve));
      usedFallback = true;
    }
