...
function session(opts) {
  ...
  //
  // The residual, stated rather than assumed: an RSA algorithm registered under a name that does
  // not mention RSA would pass the first half. Re-open if the registry ever gains one.
  function _namesRsa(name) { return typeof name === "string" && name.toLowerCase().indexOf("rsa") !== -1; }
  ...
  function _checkKeySpec(ctrl) {
    ...
    if (ctrl.type === OID_REG_CTRL_ALG_ID) {
      ...
      if (_namesRsa(alg.name)) throw _err("cmp/bad-info-value", "a keySpec algId must give an algorithm other than RSA -- an RSA requirement is stated with rsaKeyLen (RFC 9483 sec. 4.3.3); got " + alg.name);
