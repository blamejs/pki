...
// Candidate slots reserved below the path-builder ceiling when handing intermediates to cmp.verify's signer
// path, so it keeps room to append the response's OWN extraCerts (its delivered issuer chain) -- matches the
// count cmp.verify itself bounds those extraCerts to, so a ceiling-filling caller pool never starves them.
var SIGNER_EXTRACERTS_RESERVE = 32;
...
function _boundedPool(base, added, reserve) {
  ...
  // entry is kept as-is (path.build judges it) and cannot dedup an added cert. `reserve` lowers the effective
  // ceiling so a DOWNSTREAM appender keeps headroom (cmp.verify appends the response's OWN extraCerts to the
  // signer path after these intermediates); without it a ceiling-filling caller pool would starve that material.
  var ceiling = constants.LIMITS.PATH_BUILD_MAX_CANDIDATES - (reserve || 0);
 ...
 *   - `expectedSender` -- optional CA signer subject DN string (as `pki.schema.x509.parse(...).subject.dn` surfaces it); when set, every signed response's authenticated signer subject MUST equal it. Absent, the session pins the first signed response's signer subject and rejects a later switch to a different trusted signer (same-identity certificate rotation is still allowed).
...
function session(opts) {
  ...
  var pinnedSigner = null, signerPinned = false;   // the authenticated signer identity of the FIRST signed response -- every later leg must match it (a boolean flag, so a legitimately EMPTY/absent identity is still "pinned", not re-initialized each leg)
  ...
  function _verifyOpts(fresh, signerCertOverride, extraIntermediates) {
    ...
    // Bound + dedup the intermediates for cmp.verify's signer path, prioritizing the SIGNER's OWN chain material.
    // The cached signer chain (the intermediates that authenticated the signer on an earlier leg) is the PRIORITY
    // base -- MINUS the override signer itself, which is the path TARGET (passed as signerCert), not an issuer, so
    // leaving it in would spend a slot on the signer and truncate the real issuer it needs. The caller's broad
    // pool fills the rest, and SIGNER_EXTRACERTS_RESERVE slots are held below the ceiling so cmp.verify can append
    // the response's OWN extraCerts (its delivered issuers) -- so a ceiling-filling caller pool cannot starve
    // either the cached chain or the response's own issuer, and the signer still builds to the anchor.
    var extra = [];
    ...
    var ints = _boundedPool(extra, _asCertList(opts.intermediates), SIGNER_EXTRACERTS_RESERVE);
  ...
  async function _send(bodySpec, arm) {
    ...
    var recoverable = verdict.code === "cmp/signer-cert-not-found" || verdict.code === "cmp/protection-failed" ||
      verdict.code === "cmp/sender-mismatch" || verdict.code === "cmp/untrusted-signer";
    if ((verdict.valid !== true || verdict.trusted !== true) && isSig && cachedSignerCert != null && recoverable) {
    ...
    // sign a forged response under its OWN subject and still pass the trust gate. So: (1) if the caller pinned the
    // CA subject (opts.expectedSender), every signed response's authenticated signer subject MUST equal it; and
    // (2) the FIRST signed response pins the CA subject and every later leg must present the SAME subject --
    // permitting same-identity certificate/key rotation (the subject is stable) while rejecting a mid-transaction
    // switch to a different trusted signer. cmp.verify has already bound the header sender to this signer subject.
    if (isSig && verdict.signer) {
      var signerSubject = verdict.signer.subject;
      if (opts.expectedSender != null && signerSubject !== opts.expectedSender) {
        throw _err("cmp/untrusted-signer", "the response signer subject " + JSON.stringify(signerSubject) + " does not match the expected CA identity opts.expectedSender " + JSON.stringify(opts.expectedSender) + " -- refusing a response from a different trusted signer", null);
      }
      // Pin on the AUTHENTICATED header sender bytes (cmp.verify bound them to the signer's subject OR -- for an
      // empty-subject signer -- its subjectAltName), so the pin distinguishes even two empty-subject CAs a plain
      // subject-string pin would both read as null. A same-identity certificate/key rotation keeps the sender
      // identity, so its bytes are stable; a switch to a different trusted signer changes them.
      var senderPin = (verdict.header && verdict.header.sender && Buffer.isBuffer(verdict.header.sender.bytes)) ? verdict.header.sender.bytes.toString("base64") : (signerSubject || "");
      if (!signerPinned) { pinnedSigner = senderPin; signerPinned = true; }   // a boolean flag: an empty/absent identity is still pinned, never re-initialized (a null-sentinel would collide with a valid empty subject)
      else if (senderPin !== pinnedSigner) {
        throw _err("cmp/untrusted-signer", "the response signer identity changed mid-transaction (" + JSON.stringify(signerSubject) + ") -- a different trusted signer, not a same-identity certificate rotation; the transaction is NOT advanced", null);
      }
    }
    lastPeerNonce = verdict.senderNonce;   // may be absent; _baseHeader enforces its presence before a follow-up leg
    haveResponse = true;
    // Refresh the cached signer + its validated chain material ONLY from a response that verified ON ITS OWN --
    // then its extraCerts IS the chain that established trust (the MOST RECENTLY authenticated cert + the
    // intermediates that chained it; a clustered CA may rotate its protection cert across legs), so a later leg
    // that omits extraCerts falls back to the current signer AND can rebuild its path. A response that verified
    // only via the cached-signer FALLBACK did NOT authenticate its own extraCerts (which may hold a meddler's
    // decoy or omit the real intermediate); keep the existing cached signer + chain -- the material that actually
    // established trust -- rather than overwriting it with the failing response's untrusted pool. Already trusted here.
    if (isSig && !usedFallback && verdict.signer && Buffer.isBuffer(verdict.signer.cert)) {
  ...
  function _classify(verdict) {
    ...
    if (arm === expectedRespArm) {
      ...
      if (body.decoded && Array.isArray(body.decoded.caPubs)) body.decoded.caPubs.forEach(function (c) { if (Buffer.isBuffer(c)) caPubsAccum.push(c); });
      var code = resp.status && resp.status.status ? resp.status.status.code : null;
      if (_isGranted(code)) return { state: "granted", resp: resp, code: code };
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    var base = [];
    if (Array.isArray(caPubs)) caPubs.forEach(function (c) { if (Buffer.isBuffer(c)) base.push(c); });
    ...
    var pool = _boundedPool(base, added);
    var res;
