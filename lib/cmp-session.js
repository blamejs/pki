...
function session(opts) {
  ...
  /** @internal What a restored token's certificates are held to. The authority's own pool arrives in
   * responses, so it keeps the response bounds. The chain that validates the signer can hold a
   * certificate the caller passed in `opts.intermediates`, which never crossed the wire and can be
   * larger than any response the session would read, so it is bounded by the path length and the codec. */
  var CAPUBS_LIMIT = { count: CAPUBS_MAX, each: _perRespCap, total: caPubsByteBudget };
  var CHAIN_LIMIT = { count: constants.LIMITS.PATH_MAX_CERTS, each: CERT_MAX_BYTES, total: CERT_MAX_BYTES };
