...
function session(opts) {
  ...
  var cachedChain = [];   // the VALIDATED signer chain (signer + the intermediates path.build used) from cmp.verify -- parsed certs, so a later leg can rebuild the signer path from only trusted-path material, not paddable extraCerts
  ...
  async function _send(bodySpec, arm) {
    ...
    if (isSig && !usedCachedFallback && verdict.signer && Buffer.isBuffer(verdict.signer.cert)) {
      ...
      if (extras.length) {
        cachedSignerCert = verdict.signer.cert;
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    // cachedChain holds the VALIDATED signer chain from cmp.verify -- already-parsed certificates (or delivered
    // extraCerts DER on the defensive fallback), both forms path.build's coerceCert accepts, so pass each through.
    cachedChain.forEach(function (c) { if (c != null) added.push(c); });
