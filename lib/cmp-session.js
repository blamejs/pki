...
function _certConfHash(certDer) {
  ...
  catch (_e) { /* allow:swallow-unverified the issued leaf reaching _confirm is always a cmp.build-validated X.509 certificate, so x509.parse cannot throw here; the SHA-256 default is a defense-in-depth fallback */ name = ""; }
...
function session(opts) {
  ...
  async function enroll(request) {
    ...
    // Preserve its type (a bigint stays a bigint) so a large id is echoed and compared without Number rounding.
    var armSpec = request[arms[0]];
    var cid = (armSpec && typeof armSpec === "object" && !Buffer.isBuffer(armSpec)) ? armSpec.certReqId : undefined;
    activeCertReqId = (typeof cid === "bigint" || typeof cid === "number") ? cid : DEFAULT_CERT_REQ_ID;
