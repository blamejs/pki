...
function session(opts) {
  ...
  var pinnedSigner = null;   // the authenticated signer subject of the FIRST signed response -- the CA identity every later leg must match
  ...
  async function _send(bodySpec, arm) {
    ...
    if (isSig && verdict.signer) {
      ...
      if (pinnedSigner == null) pinnedSigner = signerSubject;
      else if (signerSubject !== pinnedSigner) {
        throw _err("cmp/untrusted-signer", "the response signer subject changed mid-transaction (" + JSON.stringify(pinnedSigner) + " -> " + JSON.stringify(signerSubject) + ") -- a different trusted signer, not a same-identity certificate rotation; the transaction is NOT advanced", null);
      }
    }
