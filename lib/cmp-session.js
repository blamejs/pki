...
function session(opts) {
  ...
  function _verifyOpts(fresh, signerCertOverride, extraIntermediates, responseExtra) {
    ...
    // by priority: the response's extraCerts issuers first, then the cached signer chain, then the broad caller
    // pool fills to the ceiling -- _boundedPool dedups the whole thing, so a response cert duplicating a caller
    // one spends no slot. cmp.verify re-appends the response's extraCerts, all already present here, so path.build
    // dedups them and the total never exceeds the ceiling. On the PRIMARY attempt the signer is resolved FROM
    // extraCerts (RFC 9483 sec. 3.3 places it first) and is the path TARGET, not an issuer, so it is excluded;
    // with an explicit signerCert (a cached or prebound signer) the target is that override, so EVERY response
    // extraCert is an appendable issuer.
    var issuers = signerCertOverride != null ? responseExtra : responseExtra.slice(1);   // responseExtra is always the deduped _responseExtraCerts array
    var extra = [];
