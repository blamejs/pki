...
// Every operation this session decides with, bound at load. A transaction runs across many awaits
// on a transport the caller supplies, so anything read from the runtime while it is suspended is
// answerable by whoever it handed control to: an `equals` answering true matches the signer cert
// against any decoy, a `toString` returning a constant collapses every certificate in the pool to
// one dedup key, an `isArray` answering false routes a caPubs list past the list door, and `push`
// replaced with a no-op empties the candidate pool a chain is then built from. See guard-intrinsic.
...
// The conversions and constructors, for the same reason. `BigInt` decides whether two certReqIds
// are the same request, `isFinite` decides whether the transcript ceiling is a real bound, the
// `Uint8Array` a nonce is drawn into is the storage that nonce lives in, and `Date` is the instant
// the issued certificate's validity is judged against.
...
var HASHLESS_SIG_OIDS = _create(null);
(function () {
  var names = ["Ed25519", "Ed448", "id-ml-dsa-44", "id-ml-dsa-65", "id-ml-dsa-87"];
  // allow:guard-reads-runtime-live -- module-init, the OID set is resolved once before any caller runs
...
var SIG_OID_TO_HASH = _create(null);
...
var HASH_OID_TO_DIGEST = _create(null);
...
var COMPOSITE_PH_HASHALG = _assign(_create(null), { "SHA-256": "sha256", "SHA-384": "sha384", "SHA-512": "sha512" });
...
// Both tables are keyed by a name the CALLER chose (the single arm of enroll(request)), so both are
// null-prototyped: a plain object literal answers `constructor` and `toString` for an arm nobody
// registered, which reads as a recognized request arm and routes to a response arm that is not one.
var ENROLL_ARMS = _assign(_create(null), { ir: 1, cr: 1, kur: 1, p10cr: 1 });   // the initial request arms enroll() accepts
...
var RESPONSE_ARM = _assign(_create(null), { ir: "ip", cr: "cp", kur: "kup", p10cr: "cp" });
...
var PKI_STATUS_NAMES = _assign(_create(null), { 0: "accepted", 1: "grantedWithMods", 2: "rejection", 3: "waiting" });
...
function _certReqIdEq(a, b) { return a != null && b != null && _BigInt(a) === _BigInt(b); }
...
function _asCertList(v) { return v == null ? [] : (_isArray(v) ? _arraySlice(v) : [v]); }
...
function _boundedPool(base, added) {
  ...
  var seen = _create(null), distinctBase = [];
  _forEach(base, function (c) { var k = _certIdentity(c); if (k == null) { _push(distinctBase, c); return; } if (!seen[k]) { seen[k] = 1; _push(distinctBase, c); } });
  var room = ceiling - distinctBase.length;
  if (room <= 0) return distinctBase;
  var useful = [];
  _forEach(added, function (c) { var k = _certIdentity(c); if (k != null && seen[k]) return; if (k != null) seen[k] = 1; _push(useful, c); });
  return _concat(distinctBase, _arraySlice(useful, 0, room));
}
...
function _spkiKeyIdentity(spkiDer) {
  ...
  if (algOid === OID_RSA_ENCRYPTION) {
    ...
    params = (pn == null || _isDerNull(pn)) ? "" : _toString(pn.bytes, "latin1");
  } else {
    // Every other algorithm's parameters ARE part of the key identity (an EC curve OID, an RSASSA-PSS constraint set).
    params = pn ? _toString(pn.bytes, "latin1") : "";
  }
  return algOid + "|" + params + "|" + _toString(node.children[1].bytes, "latin1");
}
...
function _responseExtraCerts(responseBytes) {
  ...
  if (!_isArray(extra)) return [];
  var out = [], seen = _create(null);
  for (var i = 0; i < extra.length && out.length < MAX_EXTRA_CERTS && i < MAX_EXTRA_SCAN; i++) {
    var c = extra[i];
    if (!_isBuffer(c)) continue;
    var key = _toString(c, "base64");
    if (seen[key]) continue;
    seen[key] = true;
    try { x509.parse(c); }
    catch (_e) { continue; }   // drop a malformed entry, mirroring cmp.verify's bounding
    _push(out, _bufferFrom(c));   // COPY out of the parser's subarray -- retaining the slice would pin the whole multi-MB response buffer if this entry is cached (the caPubs accumulator copies for the same reason)
  }
  return out;
}
...
function _pssDigest(paramsBytes) {
  if (!_isBuffer(paramsBytes) || paramsBytes.length === 0) return null;
  try {
...
function session(opts) {
  if (opts == null) opts = {};
  if (typeof opts !== "object" || _isBuffer(opts)) throw _err("cmp/bad-input", "opts must be an object");
  ...
  if (isSig) {
    var hasAnchors = opts.trustAnchors != null && !(_isArray(opts.trustAnchors) && opts.trustAnchors.length === 0);
  ...
  if (isMac && _isArray(opts.trustAnchors) && opts.trustAnchors.length === 0) opts.trustAnchors = null;
  ...
  if (opts.trustAnchors != null && _engine && _engine.toAnchor) _forEach(_asCertList(opts.trustAnchors), function (a) { try { _engine.toAnchor(a); } catch (e) { throw _err("cmp/bad-input", "opts.trustAnchors: each entry must be a certificate (DER/PEM/parsed) or a { name, publicKey, algorithm } anchor tuple -- " + ((e && e.message) || e), e); } });
  if (opts.intermediates != null && _engine && _engine.coerceCert) _forEach(_asCertList(opts.intermediates), function (c) { try { _engine.coerceCert(c); } catch (e) { throw _err("cmp/bad-input", "opts.intermediates: each entry must be a certificate (DER/PEM/parsed) -- " + ((e && e.message) || e), e); } });
  ...
  if (opts.expectedSender != null) {
    ...
    try {
      if (_es && _isBuffer(_es.tbsBytes)) {   // the documented already-parsed form (pki.schema.x509.parse output), detected like _certIdentity
      ...
      else if (_isBuffer(_es) || _es instanceof Uint8Array) { _expectedSenderDer = _bufferFrom(_es); _expectedSenderCert = x509.parse(_expectedSenderDer); }
  ...
  if (opts.intermediates != null) {
    var seenInt = _create(null), distinctInt = 0;
    _forEach(_asCertList(opts.intermediates), function (c) { var k = _certIdentity(c); if (k == null) { distinctInt++; return; } if (!seenInt[k]) { seenInt[k] = 1; distinctInt++; } });
  ...
  var transactionID = _bufferFrom(webcrypto.webcrypto.getRandomValues(new _Uint8Array(16)));
  ...
  var caPubsSeen = _create(null);   // byte-identity dedup for caPubsAccum, bounding cross-leg accumulation (below)
  ...
  var _perRespCap = (typeof opts.maxResponseBytes === "number" && _isFinite(opts.maxResponseBytes) && opts.maxResponseBytes > 0) ? opts.maxResponseBytes : constants.LIMITS.HTTP_MAX_RESPONSE_BYTES;
  ...
  function _baseHeader(fresh) {
    ...
    if (haveResponse) {
      if (!_isBuffer(lastPeerNonce) || lastPeerNonce.length === 0) throw _err("cmp/bad-nonce", "the previous CMP response omitted its senderNonce, so this follow-up request cannot echo it as recipNonce (RFC 9810 sec. 5.1.1)");
  ...
  function _verifyOpts(fresh, signerCertOverride, extraIntermediates, responseExtra) {
    ...
    var issuers = _isArray(responseExtra) ? responseExtra : [];
    var extra = [];
    if (_isArray(extraIntermediates)) {
      extra = signerCertOverride == null ? extraIntermediates : _filter(extraIntermediates, function (c) {
        return !(_isBuffer(c) && _isBuffer(signerCertOverride) && _bufEquals(c, signerCertOverride));
      });
    }
    var ints = _boundedPool(_concat(issuers, extra), _asCertList(opts.intermediates));
  ...
  function _transferOpts() {
    var o = {};
    _forEach(["transport", "tls", "headers", "timeout", "maxResponseBytes"], function (k) { if (opts[k] != null) o[k] = opts[k]; });
    return o;
  }
  ...
  async function _send(bodySpec, arm) {
    var fresh = _bufferFrom(webcrypto.webcrypto.getRandomValues(new _Uint8Array(16)));
    var header = _assign(_baseHeader(fresh), ENROLL_ARMS[arm] && opts.implicitConfirm ? { generalInfo: [{ infoType: "implicitConfirm" }] } : {});
    ...
    if (isSig && !usedCachedFallback && verdict.signer && _isBuffer(verdict.signer.cert)) {
      ...
      if (extras.length) {
        cachedSignerCert = _bufferFrom(verdict.signer.cert);   // COPY: an extraCerts-resolved signer.cert is a slice of the response, so caching it raw pins that allocation (the chain is copied by cmp.verify; keep this one consistent)
        cachedChain = (_isArray(verdict.signer.chain) && verdict.signer.chain.length) ? verdict.signer.chain : extras;
      }
    }
    return verdict;
  }
  ...
  function _classify(verdict) {
    ...
    if (arm === expectedRespArm) {
      ...
      if (body.decoded && _isArray(body.decoded.caPubs)) _forEach(body.decoded.caPubs, function (c) {
        if (!_isBuffer(c)) return;
        var k = _toString(c, "base64");
        if (caPubsSeen[k]) {
          ...
          if (isGrantLeg) { for (var pi = 0; pi < caPubsAccum.length; pi++) { if (_toString(caPubsAccum[pi], "base64") === k) { _push(caPubsAccum, caPubsAccum.splice(pi, 1)[0]); if (pi < caPubsWaitingCount) caPubsWaitingCount--; break; } } }
          return;
        }
        ...
        if (isGrantLeg) {
          ...
          while (caPubsWaitingCount > 0 && (caPubsAccum.length >= CAPUBS_MAX || caPubsBytes + c.length > caPubsByteBudget)) {
            ...
            caPubsWaitingCount--;
            delete caPubsSeen[_toString(evicted, "base64")];   // keep the dedup set == the retained set, so a grant re-delivering an evicted issuer (also seen in a waiting leg) can re-add it
          }
        }
        ...
        _push(caPubsAccum, _bufferFrom(c));   // COPY out of the parser's subarray -- retaining the slice would pin the whole multi-MB response buffer, so caPubsBytes (the cert length) would not bound the real allocation across polls
    ...
    return { state: "unexpected", reason: "response arm " + _stringify(arm) + " has no transition in an enrollment transaction" };
  }
  ...
  function _transcriptSnapshot() {
    return _map(transcript, function (e) {
      var c = _assign({}, e);
      if (_isBuffer(c.bytes)) c.bytes = _bufferFrom(c.bytes);
      if (c.verdict) c.verdict = _assign({}, c.verdict);
      return c;
    });
  }
  function _terminal(outcome, extra) {
    ...
    return _assign({
      ...
      confirmed: false, implicitConfirm: false, transactionID: _bufferFrom(transactionID),   // a copy: mutating it must not corrupt the session identity
  ...
  async function _confirm(certDer, header, info) {
    ...
    var accept = true;
    if (typeof opts.acceptCert === "function") accept = (await opts.acceptCert(_bufferFrom(certDer), info)) === true;
    var h = _certConfHash(certDer);
    var certHash = _bufferFrom(await webcrypto.webcrypto.subtle.digest(h.digest, certDer));
    ...
    if (!verdict.body || verdict.body.arm !== "pkiconf") throw _err("cmp/bad-confirmation", "expected a pkiConf acknowledgement to the certConf but got " + _stringify(verdict.body && verdict.body.arm) + " (RFC 9810 sec. 5.3.18)");
  ...
  function _leafOf(resp) {
    ...
    if (!_isBuffer(cert)) throw _err("cmp/unexpected-arm", "a granted CertResponse carried no plain issued certificate (an encryptedCert form is out of enrollment v1 scope) (RFC 9810 sec. 5.3.4)");
  ...
  function _extractRequestedSpki(arm, armSpec) {
    if (arm === "p10cr") {
      if (!_isBuffer(armSpec) && !(armSpec instanceof Uint8Array)) return null;
    ...
    if (_isBuffer(pk) || pk instanceof Uint8Array) return guard.bytes.snapshot(pk, CmpError, "cmp/bad-input", "the certTemplate publicKey");
    return null;
  }
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    var caPubsList = [];
    if (_isArray(caPubs)) _forEach(caPubs, function (c) { if (_isBuffer(c)) _push(caPubsList, c); });
    var added = _asCertList(opts.intermediates);   // the caller pool (config-bounded to SESSION_MAX_INTERMEDIATES)
    if (_isBuffer(cachedSignerCert)) _push(added, cachedSignerCert);   // the CMP signer MAY be the issued leaf's issuer (a combined CA)
    ...
    _forEach(cachedChain, function (c) { if (_isBuffer(c)) _push(added, c); });
    ...
    var res;
    try { res = await _engine.build(leaf, { trustAnchors: _asCertList(opts.trustAnchors), intermediates: pool, validate: true, time: opts.time != null ? opts.time : new _Date() }); }
    catch (e) {
  ...
  async function _finish(granted, header) {
    ...
    var chain = [leaf], seenChain = _create(null);
    _forEach(caPubsAccum, function (c) {
      if (!_isBuffer(c)) return;
      try { x509.parse(c); }
    ...
    var certOut = _bufferFrom(leaf), chainOut = _map(chain, function (c) { return _bufferFrom(c); });
  ...
  async function enroll(request) {
    ...
    if (!request || typeof request !== "object" || _isBuffer(request)) throw _err("cmp/bad-input", "enroll(request) requires a body spec object { ir | cr | kur | p10cr }");
    var arms = _filter(_keys(request), function (k) { return request[k] != null; });
    ...
    if (armSpec && typeof armSpec === "object" && !_isBuffer(armSpec) && armSpec.messages != null) {
    ...
    if (arms[0] !== "p10cr" && armSpec != null && armSpec.pop != null && armSpec.pop.type != null && armSpec.pop.type !== "signature") {
      throw _err("cmp/bad-input", "a session ir / cr / kur proves possession by signing the CRMF proof of possession; a non-signature POP mode (" + _stringify(armSpec.pop.type) + ") is not supported (RFC 4211 sec. 4)");
    }
    ...
    try {
      ...
      var cid = (typeof armSpec === "object" && !_isBuffer(armSpec)) ? armSpec.certReqId : undefined;
  ...
  return {
    enroll: enroll,
    get transactionID() { return _bufferFrom(transactionID); },   // a copy: a caller mutating it must not desync the transaction
