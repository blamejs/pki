...
function _boundedPool(base, added) {
  // Dedup the BASE (caller intermediates) FIRST: duplicate copies would otherwise inflate its length, drive the
  ...
  // entry is kept as-is (path.build judges it) and cannot dedup an added cert.
  ...
  var room = constants.LIMITS.PATH_BUILD_MAX_CANDIDATES - distinctBase.length;
...
function session(opts) {
  ...
  if (isSig) {
    ...
    // Parse + validate each anchor NOW (through the SAME path-builder normalization the response verify uses), so
    // an unusable anchor (a non-certificate string, a malformed tuple, junk bytes) is a construction error rather
    // than a failure discovered only after the one-shot transaction has already engaged the transport.
    if (_engine && _engine.toAnchor) _asCertList(opts.trustAnchors).forEach(function (a) { try { _engine.toAnchor(a); } catch (e) { throw _err("cmp/bad-input", "opts.trustAnchors: each entry must be a certificate (DER/PEM/parsed) or a { name, publicKey, algorithm } anchor tuple -- " + ((e && e.message) || e), e); } });
  ...
  // Likewise parse each supplied intermediate at construction (the pool is coerced the same way at path build),
  // so a malformed chain-pool entry fails closed before the transaction rather than after it is consumed.
  ...
  function _verifyOpts(fresh, signerCertOverride, extraIntermediates) {
    ...
    // ALWAYS bound + dedup the intermediates for cmp.verify's signer path. On the FIRST attempt there is no extra
    // material, but the base is still deduplicated so a duplicate-filled caller pool does not fill the candidate
    // ceiling and leaves cmp.verify room to append the response's OWN extraCerts issuer. On the FALLBACK, the
    // cached signer chain is appended -- MINUS the override signer itself, which is the path TARGET (passed as
    // signerCert), not an issuer, so leaving it in would spend a scarce slot on the signer and truncate the real
    // issuer it needs -- (part-)derived from an unsigned extraCerts, bounded to the remaining room.
    var extra = [];
    ...
    var ints = _boundedPool(_asCertList(opts.intermediates), extra);
