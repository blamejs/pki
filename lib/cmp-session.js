...
// CertResponse element inside it, which RFC 9483 sec. 4.4 spells as certReqId -1.
...
function session(opts) {
  ...
  async function info(request) {
    ...
    if (op.value === false) {
      ...
      itav.infoValue = cmpBuild.buildCrlStatusList(request[names[0]]);
    }
    expectedInfoOp = op;
    inFlight = true;
    try {
      ...
      if (t.state === "granted") {
        ...
        if (raw != null) {
          ...
          value = _checkInfoValue(op, value);
        }
  ...
  function _checkInfoValue(op, value) {
    if (op.name === "rootCaCert") {
      ...
      return {
        newWithNew: _asCertificate(value.newWithNew, "the rootCaKeyUpdate newWithNew"),
        newWithOld: _asCertificate(value.newWithOld, "the rootCaKeyUpdate newWithOld"),
        oldWithNew: value.oldWithNew == null ? null : _asCertificate(value.oldWithNew, "the rootCaKeyUpdate oldWithNew"),
      };
    }
    ...
    if (op.name === "crlUpdate") return value.map(_asCrl);
  ...
  // "An algorithm other than RSA" is a statement about the whole PKCS#1 arc, not about one OID:
  // rsaEncryption, rsassaPss, rsaesOaep and every shaNWithRSAEncryption are assigned under it, and
  // naming a single one would let a responder state an RSA requirement in any other spelling. The
  // prefix is derived from a registered member rather than written out, so this stays one place the
  // registry decides. Every arc is dot-delimited and the prefix keeps its trailing dot, so a
  // sibling arc cannot match by sharing a numeric prefix.
  var OID_RSA_ARC = oid.byName("rsaEncryption").replace(/\.\d+$/, ".");
  ...
  function _checkKeySpec(ctrl) {
    ...
    if (ctrl.type === OID_REG_CTRL_ALG_ID) {
      ...
      if (alg.oid.indexOf(OID_RSA_ARC) === 0) throw _err("cmp/bad-info-value", "a keySpec algId must give an algorithm other than RSA -- an RSA requirement is stated with rsaKeyLen (RFC 9483 sec. 4.3.3); got " + (alg.name || alg.oid));
