 ...
 * refused before the `certConf` that would accept it. Without the option, a granted `CertResponse`
 * carrying a `privateKey` is refused, and a request that would ask for one is refused before it is
 * sent. A `poll-timeout` on such a transaction resumes like any other; its `resumeToken` records that
