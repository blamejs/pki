...
function session(opts) {
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    // The caller's OWN intermediates are the sacred base -- never truncated (if they alone exceed the ceiling,
    // that is a genuine config error path.build reports). Everything the SESSION adds to help the leaf build --
    // the authenticated caPubs (this leaf's CA-delivered issuer chain, RFC 9810 sec. 5.3.4) first, then the
    // cached protection signer + its chain (part of which derives from the response's UNSIGNED extraCerts, so a
    // meddler can inflate it) -- is bounded TOGETHER to the remaining room below the path builder's candidate
    // ceiling, deduped so a copy wastes no slot. So neither a large-but-valid caller pool plus a legitimate
    // caPubs, nor an extraCerts flood, can push the validation pool over the ceiling and fail an otherwise-valid
    // grant. The returned chain still carries every caPubs (surfaced separately); only this validation pool is capped.
    ...
    if (room > 0) {
      ...
      // Dedup the added certs against each other AND against the caller's Buffer intermediates, so a caPubs /
      // cached copy of an existing candidate never spends a slot; then truncate to the remaining room.
      var seen = Object.create(null), useful = [];
      pool.forEach(function (c) { if (Buffer.isBuffer(c)) seen[c.toString("base64")] = 1; });
      added.forEach(function (c) { var k = c.toString("base64"); if (!seen[k]) { seen[k] = 1; useful.push(c); } });
