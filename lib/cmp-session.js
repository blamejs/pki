...
function session(opts) {
  ...
  function _classifyError(body) {
    ...
    if (code === 3) {
      // sec. 4.4 puts waiting in an ip/cp/kup for an enrollment and in an error message for anything
      // else, so a waiting error drives the poll loop only for revoke/info; an enrollment's error stays a
      // rejection. A wait is not a failure, so sec. 4.4 forbids failInfo on it -- a waiting error carrying
      // one is contradictory and refused rather than polled.
      if (txnKind === "enroll") return { state: "rejected", status: si };
