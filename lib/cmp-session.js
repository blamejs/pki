...
var KNOWN_SESSION_OPTS = Object.assign(Object.create(null), {
  ...
  acceptCert: 1, senderKID: 1, recipKID: 1, expectedSender: 1, acceptCentralKeyGeneration: 1,
...
var INFO_OPS = Object.assign(Object.create(null), {
  ...
  /** @internal The KEM ciphertext a peer encapsulated to this client's KEM public key, which the
   * client then decapsulates to protect its own messages (RFC 9810 sec. 5.3.19.18 / 5.1.3.4). The
   * request carries no value; the answer carries the algorithm and the ciphertext. */
  kemCiphertext: { name: "kemCiphertext", requestOid: "kemCiphertextInfo", responseOid: "kemCiphertextInfo", value: false, read: "readKemCiphertextInfo" },
});
var KEM_CIPHERTEXT_ALGORITHMS = Object.assign(Object.create(null));
["id-ml-kem-512", "id-ml-kem-768", "id-ml-kem-1024"].forEach(function (n) {
  KEM_CIPHERTEXT_ALGORITHMS[oid.byName(n)] = n;
});
var KNOWN_REVOKE_KEYS = Object.assign(Object.create(null), { certificate: 1, certDetails: 1, reason: 1 });
/** @internal The keys the CRMF signature proof-of-possession arm takes, which is the only arm a
 * session sends. A key belonging to another arm names that arm whether or not `type` says so. */
var SESSION_POP_KEYS = Object.assign(Object.create(null), { type: 1, sender: 1 });
...
function _asCertList(v) { return v == null ? [] : (Array.isArray(v) ? guard.list.snapshot(v) : [v]); }
...
function _isRecoverableVerify(code) {
  /** @internal `cmp/bad-sender-kid` belongs here for the same reason `cmp/signer-cert-not-found` does:
   * a response whose extraCerts carry no certificate matching the declared key identifier is one this
   * session may already hold the signer for. Retrying is safe because the fallback certificate is put
   * through the same identifier check, so the retry can only succeed on a certificate the message
   * actually names. */
  return code === "cmp/signer-cert-not-found" || code === "cmp/protection-failed" ||
    code === "cmp/sender-mismatch" || code === "cmp/untrusted-signer" ||
    code === "cmp/bad-sender-kid";
}
 ...
 * issued leaf plus any authenticated `caPubs` the CA delivered (chain material, never trust anchors).
 *
 * Central key pair generation (RFC 9483 sec. 4.1.6) is available under `opts.acceptCentralKeyGeneration`,
 * for an entity that cannot generate its own key pair. A request in the shape sec. 4.1.6 defines, a
 * `certTemplate` with no `publicKey` or one whose `subjectPublicKey` is a zero-length BIT STRING,
 * carries no proof of possession, since this entity holds no key to prove possession of, and goes out
 * under `cmp2021` because the response will carry an `EnvelopedData` (RFC 9480 sec. 2.20). The session
 * opens the delivered container with the credential it enrolled under, its private key for the key
 * transport and key agreement techniques and its shared secret for the password technique, authorizes
 * the key generation authority against `opts.trustAnchors` at `opts.time`, and surfaces the result on
 * the verdict as `deliveredKey`. A PBMAC1 session may instead authorize that authority by the secret
 * that protects its requests, which sec. 4.1.6 permits a MAC-protected exchange, and states so with
 * `opts.authorizedBySharedSecret`; it is never inferred from an absent `trustAnchors`, since a missing
 * option would then silently drop both the chain check and the `id-kp-cmKGA` assertion. What binds the grant is then the delivered key rather than a
 * requested one: the issued certificate MUST certify it, and a grant whose certificate does not is
 * refused before the `certConf` that would accept it. The package carries exactly one key, which is
 * what sec. 4.1.6 profiles RFC 5958's `SIZE (1..MAX)` down to, because one grant certifies one key and
 * every further key would be one the issued certificate says nothing about. Without the option, a
 * granted `CertResponse` carrying a `privateKey` is refused, and a request that would ask for one is
 * refused before it is sent. A `poll-timeout` on such a transaction resumes like any other; its `resumeToken` records that
 * the grant is bound to the delivered key, and a session resuming it must set the option too.
 *
 ...
 *   - `acceptCentralKeyGeneration` (boolean) -- accept a key pair the authority generates on this entity's behalf (RFC 9483 sec. 4.1.6). It admits a request that names no key to certify, opens the delivered container with the credential the session enrolled under, and surfaces the result as `deliveredKey` on the issued verdict. The issued certificate must certify the delivered key. Without it, such a request and such a grant are both refused.
 *   - `authorizedBySharedSecret` (boolean) -- authorize the key generation authority by the secret that protects this session's requests rather than by a chain, which sec. 4.1.6 permits a MAC-protected exchange. PBMAC1 sessions only, and it is the alternative to `opts.trustAnchors` for that authority rather than an addition to it: a delivered key package is authorized one way or the other, and a session accepting central key generation with neither is refused when it is built. It pairs with `opts.trustAnchors`, which on a session also path-validates the issued certificate, a job the exemption does not replace.
...
function session(opts) {
  ...
  if (opts.acceptCentralKeyGeneration != null && typeof opts.acceptCentralKeyGeneration !== "boolean") {
    throw _err("cmp/bad-input", "opts.acceptCentralKeyGeneration is a boolean stating that this caller accepts a key pair the authority generated (RFC 9483 sec. 4.1.6)");
  }
  if (opts.authorizedBySharedSecret != null && typeof opts.authorizedBySharedSecret !== "boolean") {
    throw _err("cmp/bad-input", "opts.authorizedBySharedSecret is a boolean stating that this caller authorizes the key generation authority by the secret that protects its requests (RFC 9483 sec. 4.1.6)");
  }
  if (opts.authorizedBySharedSecret === true && !isMac) {
    throw _err("cmp/bad-input", "opts.authorizedBySharedSecret authorizes the key generation authority by the secret that protected the request, so it applies only to a PBMAC1 session (RFC 9483 sec. 4.1.6)");
  }
  /** @internal A session that accepts a generated key pair states how it authorizes the authority
   * that generates one, and a MAC session holding neither anchors nor the exemption states neither.
   * Every input this needs is here, so it is refused now: the same request refused on the grant would
   * be refused after the authority had generated and delivered the key, spending the session's one
   * transaction slot and discarding the delivery, and the retry costs a second key generation. */
  if (opts.acceptCentralKeyGeneration === true && isMac
    && opts.trustAnchors == null && opts.authorizedBySharedSecret !== true) {
    throw _err("cmp/bad-input", "accepting a centrally generated key pair needs opts.trustAnchors to chain the key generation authority, or opts.authorizedBySharedSecret to authorize it by the secret that protects this session's requests (RFC 9483 sec. 4.1.6)");
  }
  ...
  /** @internal Whether the request this session sent asked the authority to generate the key pair. It
   * decides what binds the grant: a key this entity already holds, or the delivered private key the
   * grant arrives with (RFC 9483 sec. 4.1.6). */
  var centralKeyGeneration = false;
  ...
  function _baseHeader(fresh) {
    ...
    /** @internal RFC 9480 sec. 2.20 needs cmp2021 whenever an exchange uses EnvelopedData, "for the
     * request being sent or for the expected response". A central key generation transaction expects
     * one on its grant, so every message in it carries the version, not only the initial request whose
     * body triggers it: a poll asking for that grant expects the EnvelopedData just as the request
     * did, and a resumed poll is the same transaction in a later process. */
    if (centralKeyGeneration) h.pvno = 3;
  ...
  function _resumeToken(polls, nextPollAt) {
    ...
    return {
      ...
      /** @internal What the grant is held to, when it is not a key this entity already holds. A
       * central key generation request names no key to certify (RFC 9483 sec. 4.1.6), so the binding
       * is the delivered private key the grant arrives with, and the resumed poll applies it the way
       * the first process would have. Recorded so a resumed session cannot silently drop to no
       * binding at all. */
      centralKeyGeneration: centralKeyGeneration,
  ...
  function _leafOf(resp) {
    ...
    if (ckp.privateKey != null && !opts.acceptCentralKeyGeneration) {
      throw _err("cmp/unexpected-arm", "a granted CertResponse carried a server-generated privateKey; accepting a key this entity did not generate is a decision the caller states with opts.acceptCentralKeyGeneration (RFC 9483 sec. 4.1.6)");
    }
    var parsed;
  ...
  /** @internal The two shapes RFC 9483 sec. 4.1.6 gives a request for a centrally generated key: no
   * publicKey at all, or one carrying an algorithm preference over a zero-length subjectPublicKey. A
   * publicKey that is present and carries key bits is an ordinary request for a key this entity
   * already holds. */
  function _requestsCentralKeyGeneration(armSpec) {
    if (!armSpec || typeof armSpec !== "object" || Buffer.isBuffer(armSpec)) return false;
    var pk = armSpec.certTemplate ? armSpec.certTemplate.publicKey : null;
    if (pk == null) return true;
    if (!guard.bytes.isByteSource(pk)) return false;
    var bits;
    try { bits = asn1.read.bitString(asn1.decode(guard.bytes.view(pk, CmpError, "cmp/bad-input", "the certTemplate publicKey")).children[1]); }
    catch (_e) { return false; }
    return bits.bytes.length === 0;
  }

  /** @internal One copy of the caller's arm, and of the `certTemplate` inside it, so every check below
   * and the message that goes on the wire read the same values. A `p10cr` arm is a byte source rather
   * than a record, and is left to the builder's own snapshot. */
  function _snapshotArm(armSpec) {
    if (!_isRecord(armSpec)) return armSpec;
    var copy = guard.identifier.ownOptions(armSpec);
    if (_isRecord(copy.certTemplate)) copy.certTemplate = guard.identifier.ownOptions(copy.certTemplate);
    /** @internal The proof of possession is copied for the same reason the template is: the session
     * decides on `pop.type` which modes it will send, and a field answering one way to that decision
     * and another to the builder would put a request on the wire in a mode the session refused. */
    if (_isRecord(copy.pop)) copy.pop = guard.identifier.ownOptions(copy.pop);
    return copy;
  }
  /** @internal A record the session copies field by field, as opposed to a byte source, which every
   * byte-taking door snapshots whole. A `p10cr` arm is a CertificationRequest in any of the shapes
   * `guard.bytes` accepts, and copying one field by field would leave a plain object of indices. */
  function _isRecord(v) {
    return !!v && typeof v === "object" && !guard.bytes.isByteSource(v);
  }
  ...
  /** @internal RFC 9483 sec. 4.1.6: the key management technique that protected the content-encryption
   * key follows the protection of the REQUEST, so the session opens the container with the credential
   * it enrolled under and no other. A signature session holds a private key and reaches a key
   * transport or key agreement container (sec. 4.1.6.1, sec. 4.1.6.2); a MAC session holds the shared
   * secret and reaches a password container (sec. 4.1.6.3). A container built for the other technique
   * does not open, which is the rule enforcing itself. */
  /** @internal A session certificate option takes DER, PEM, or an already-parsed certificate, and the
   * path builder reads all three; `opts.trustAnchors` additionally takes a `{ name, publicKey,
   * algorithm }` tuple. Both pools are forwarded to `pki.cmp.openKeyPackage`, which takes bytes, so a
   * parsed certificate is handed on as the bytes it was parsed from rather than lost on the way: a
   * certificate that goes missing makes the authority's chain unbuildable and reads as an unauthorized
   * authority. An anchor tuple is passed through, since it is what the anchor form is; anything else
   * is named here, because the certificate pool is the one place a value is dropped rather than
   * refused. */
  function _asOpenableCerts(v, label, allowAnchorTuple) {
    return _asCertList(v).map(function (c, i) {
      if (typeof c === "string" || guard.bytes.isByteSource(c)) return c;
      var der = guard.parsed.sourceOf(c);
      if (der != null) return der;
      if (allowAnchorTuple && _isRecord(c)) return c;
      throw _err("cmp/bad-input", label + "[" + i + "] is a parsed certificate this session cannot hand on as bytes; supply it as DER or PEM");
    });
  }

  async function _openDeliveredKey(ckp) {
    /** @internal A transaction that asked the authority to generate the key pair is bound to the key
     * it delivers, and to nothing else: no requested public key was submitted for the grant to be
     * held to. A grant carrying only a certificate would therefore be confirmed with nothing checked,
     * and the entity would hold a certificate for a private key it never received. */
    if (ckp.privateKey == null && centralKeyGeneration) {
      throw _err("cmp/bad-cert-response", "this transaction asked the authority to generate the key pair, and the granted CertResponse carries no privateKey -- there is no key to go with the issued certificate, and nothing the grant is bound to (RFC 9483 sec. 4.1.6)");
    }
    if (ckp.privateKey == null) return null;
    /** @internal The sec. 4.1.6 exemption is what the CALLER said, never what this session inferred
     * from its own configuration. Reading it off an absent `trustAnchors` would turn a missing option
     * into a decision to authorize the key generation authority by the shared secret alone, silently
     * dropping both the chain check and the id-kp-cmKGA assertion. `pki.cmp.openKeyPackage` requires
     * the statement, so a session that inferred it would admit containers the verb refuses. The verb's
     * own door decides: one of the two rules, never neither and never both. */
    var open = opts.mac != null
      ? { password: opts.mac.secret, authorizedBySharedSecret: opts.authorizedBySharedSecret === true }
      : { key: opts.key, cert: opts.cert };
    /** @internal On a session `opts.trustAnchors` does two jobs: it authorizes the key generation
     * authority, and it path-validates the issued certificate. The exemption replaces the first job
     * only, so the anchors are withheld from the verb, whose door knows the first job alone and
     * refuses the pair, and they keep the second. A caller therefore states both options to authorize
     * the authority by the secret and still chain the certificate it was issued. */
    if (opts.trustAnchors != null && !open.authorizedBySharedSecret) {
      open.trustAnchors = _asOpenableCerts(opts.trustAnchors, "opts.trustAnchors", true);
      if (opts.intermediates != null) open.intermediates = _asOpenableCerts(opts.intermediates, "opts.intermediates", false);
    }
    /** @internal The instant the session validates every other certificate at governs the authority's
     * chain too, so a historical enrollment does not judge one certificate at a stated time and
     * another at the current one. */
    if (opts.time != null) open.time = opts.time;
    if (ckp.privateKey.bytes == null) {
      throw _err("cmp/bad-key-package", "a delivered key package is carried as an EnvelopedData, and this grant carries the legacy RFC 4211 EncryptedValue form (RFC 9483 sec. 4.1.6, RFC 9810 sec. 5.2.2)");
    }
    return cmp.openKeyPackage(ckp.privateKey.bytes, open);
  }

  /** @internal A centrally generated key and the certificate granted alongside it are one pair, and
   * only the caller can tell: the authority's signature says the key package is authentic, and says
   * nothing about which certificate it belongs with. An unbound pair confirms an enrollment whose
   * certificate certifies a key this entity does not hold, so the two are compared here, before the
   * certConf that accepts the grant (RFC 9483 sec. 4.1.6). */
  async function _assertDeliveredKeyCertified(deliveredKey, leaf) {
    /** @internal RFC 9483 sec. 4.1.6 gives the eContent "a sequence of one OneAsymmetricKey element",
     * and here that count is what the grant is bound to rather than a matter of form: one grant
     * certifies one key, so every further key in the package is one the issued certificate says
     * nothing about and this entity would hold uncertified. The standalone `pki.cmp.openKeyPackage`
     * surfaces a whole RFC 5958 package, which is SIZE (1..MAX), and leaves the binding to its caller;
     * a session performs the sec. 4.1.6 enrollment, so it holds the profile's count. */
    if (deliveredKey.keys.length !== 1) {
      throw _err("cmp/bad-key-package", "a delivered key package carries one key, and this one carries " +
        deliveredKey.keys.length + "; the issued certificate certifies one of them and says nothing about the rest (RFC 9483 sec. 4.1.6)");
    }
    var certified, pair;
    try { certified = x509.parse(leaf).subjectPublicKeyInfo.bytes; }
    catch (e) { throw _err("cmp/bad-cert-response", "the issued certificate's public key cannot be compared with the delivered key", e); }
    try { pair = await key.correspondsTo(deliveredKey.keys[0], certified); }
    catch (e) { throw _err("cmp/bad-key-package", "the delivered private key and the issued certificate could not be exercised together, so the two cannot be shown to be a pair", e); }
    if (!pair) {
      throw _err("cmp/bad-key-package", "the issued certificate does not certify the delivered private key -- the two are not a pair, so the certificate is unusable with the key this enrollment received (RFC 9483 sec. 4.1.6)");
    }
  }

  async function _finish(granted, header) {
    var leaf = _leafOf(granted.resp);
    var deliveredKey = await _openDeliveredKey(granted.resp.certifiedKeyPair);
    if (deliveredKey !== null) await _assertDeliveredKeyCertified(deliveredKey, leaf);
  ...
  /** @internal Claim the session's one transaction slot. It is claimed BEFORE the caller's request
   * object is read, because reading one runs whatever accessors it carries and an accessor can call
   * back into this session: a slot claimed only after validation would let the re-entrant call open a
   * transaction that the outer call then writes its own state over. Every entry point pairs this with
   * a `finally` that releases the slot, so a request refused during validation leaves the session
   * retryable exactly as it did before. */
  ...
  async function enroll(request) {
    ...
    try { return await _enroll(request); }
    finally {
      inFlight = false;
      completed = started;
    }
  ...
  async function _enroll(request) {
    guard.identifier.assertPlainRecord(request, _err, "cmp/bad-input", "enroll(request), a body spec { ir | cr | kur | p10cr },");
    /** @internal The request's own door, rather than the builder's arm count downstream: the session
     * sends the ONE arm it read out of this object, so a key the session does not recognize would
     * otherwise be dropped silently instead of naming the caller's mistake. */
    guard.identifier.assertKnownKeys(request, ENROLL_ARMS, _err, "cmp/bad-input", "unknown enroll request field ");
    var arms = guard.identifier.optionNames(request, ENROLL_ARMS).filter(function (k) { return request[k] != null; });
    ...
    /** @internal The arm is copied out of the caller's object ONCE, and everything after this reads and
     * sends the copy. A request object can carry accessors, so reading a field twice can get two
     * answers: the request that goes on the wire has to be the one the checks below were applied to,
     * or a second answer could turn a validated client-generated enrollment into a central key
     * generation request the session never admitted. The certTemplate is copied for the same reason,
     * since the field that decides which of the two this is lives in it. */
    var armSpec = _snapshotArm(request[arms[0]]);
    ...
    /** @internal RFC 9483 sec. 4.1.6: an entity requesting central key generation "MUST omit the
     * publicKey field from the certTemplate or ... fill the subjectPublicKey sub-field with a
     * zero-length BIT STRING", and the popo "MUST be absent". A request in that shape asks the
     * authority to generate the key, so it is admitted only when the caller has said it accepts one. */
    var central = arms[0] !== "p10cr" && _requestsCentralKeyGeneration(armSpec);
    if (central && !opts.acceptCentralKeyGeneration) {
      throw _err("cmp/bad-input", "this request asks the authority to generate the key pair (RFC 9483 sec. 4.1.6: a certTemplate with no publicKey, or a zero-length subjectPublicKey); accepting a key this entity did not generate is a decision the caller states with opts.acceptCentralKeyGeneration");
    }
    if (arms[0] !== "p10cr" && reqSpki == null && !central) {
      throw _err("cmp/bad-input", "an ir / cr / kur enrollment must submit certTemplate.publicKey -- a session enrolls a client-generated key unless the request asks for central key generation (RFC 9483 sec. 4.1.6) and opts.acceptCentralKeyGeneration is set");
    }
    if (central && armSpec.pop != null) {
      throw _err("cmp/bad-input", "a central key generation request carries no proof of possession, since this entity holds no key to prove possession of (RFC 9483 sec. 4.1.6)");
    }
    /** @internal A session sends the signature proof of possession and no other, so the arm is read
     * the way the builder reads it rather than from `type` alone. `pop: { raVerified: true }` names
     * the raVerified arm without stating a type, and a door that asked only about `type` would pass
     * it through to be refused downstream, after the request had been assembled. */
    if (arms[0] !== "p10cr" && armSpec != null && _isRecord(armSpec.pop)) {
      var popType = armSpec.pop.type;
      if (popType != null && popType !== "signature") {
        throw _err("cmp/bad-input", "a session ir / cr / kur proves possession by signing the CRMF proof of possession; a non-signature POP mode (" + guard.text.showValue(popType) + ") is not supported (RFC 4211 sec. 4)");
      }
      guard.identifier.assertKnownKeys(armSpec.pop, SESSION_POP_KEYS, _err, "cmp/bad-input",
        "a session ir / cr / kur proves possession by signing the CRMF proof of possession, so its `pop` names the signature arm only: ");
    }
    if (isMac && arms[0] !== "p10cr" && !central && (armSpec == null || armSpec.key == null)) {
    ...
    /** @internal Everything this request contributes is read out of the caller's object HERE, while no
     * session state has been written. Reading an object a caller supplied runs whatever accessors it
     * carries, and one of those can call back into this session; a write placed among those reads
     * would let the re-entrant call and this one interleave and leave the two variables that bind the
     * grant, `centralKeyGeneration` and `requestedSpki`, describing different transactions. */
    var cid = (typeof armSpec === "object" && !Buffer.isBuffer(armSpec)) ? armSpec.certReqId : undefined;
    var certReqId = _normalizeCertReqId(cid, arms[0] === "p10cr" ? P10CR_CERT_REQ_ID : DEFAULT_CERT_REQ_ID);
    /** @internal A central key generation request carries no proof of possession at all, so the arm
     * names a null key to stop `pki.cmp.build` falling back to the message protection key. */
    if (central) armSpec.key = null;
    var sendSpec = Object.create(null);
    sendSpec[arms[0]] = armSpec;

    txnKind = "enroll";
    expectedRespArm = RESPONSE_ARM[arms[0]];
    activeCertReqId = certReqId;
    /** @internal The pair is installed together, from this request alone. A session whose earlier
     * enroll failed while building is still retryable, so a retry that submits an ordinary
     * client-generated key must not carry the earlier request's binding into its own transaction:
     * the zero-length subjectPublicKey form names an algorithm preference and not a key to certify,
     * so a central request is bound to the delivered private key instead. */
    centralKeyGeneration = central;
    requestedSpki = central ? null : reqSpki;
    var initial = await _send(sendSpec, arms[0]);
  ...
  function _readResumeToken(token) {
    ...
    var raw = {
      ...
      protection: token.protection, centralKeyGeneration: token.centralKeyGeneration,
    };
    ...
    var wasCentral = raw.centralKeyGeneration === true;
    if (wasCentral && !opts.acceptCentralKeyGeneration) {
      throw _err("cmp/bad-input", "the resumeToken continues a central key generation transaction, so the session resuming it must also set opts.acceptCentralKeyGeneration (RFC 9483 sec. 4.1.6)");
    }
    if (raw.requestedSpki == null && !wasCentral) {
      throw _err("cmp/bad-input", "the resumeToken requestedSpki must name the key the granted certificate certifies -- without it the grant would be bound to nothing, unless the transaction requested central key generation, where the delivered key is the binding");
    }
    if (raw.requestedSpki != null && wasCentral) {
      throw _err("cmp/bad-input", "the resumeToken names both a requested key and central key generation; a transaction is bound one way or the other, not both");
    }
    var spki = wasCentral ? null : _b64Field(raw.requestedSpki, "requestedSpki", CERT_MAX_BYTES);
    ...
    return { transactionID: txn, recipNonce: nonce, certReqId: certReqId, arm: arm,
      implicitConfirm: implicitConfirm, centralKeyGeneration: wasCentral,
  ...
  async function resumePoll(token) {
    ...
    var t;
    try { t = _readResumeToken(token); }
    catch (e) { inFlight = false; completed = started; throw e; }
    try {
      txnKind = "enroll";
      ...
      centralKeyGeneration = t.centralKeyGeneration;
  ...
  async function revoke(request) {
    ...
    try { return await _revoke(request); }
    finally {
      inFlight = false;
      completed = started;
    }
  ...
  async function _revoke(request) {
  ...
  async function info(request) {
    ...
    try { return await _info(request); }
    finally {
      inFlight = false;
      completed = started;
    }
  ...
  async function _info(request) {
    ...
    var keys = guard.identifier.optionNames(request, INFO_OPS), names = [], values = [];
  ...
  function _checkInfoValue(op, value, requestValue) {
    /** @internal A ciphertext is only useful if the client can decapsulate it, so the answer is held
     * to a KEM this toolkit establishes secrets under and to a non-empty ciphertext; anything else
     * could not be turned into a protection key (RFC 9810 sec. 5.3.19.18 / 5.1.3.4). */
    if (op.name === "kemCiphertext") {
      if (!KEM_CIPHERTEXT_ALGORITHMS[value.kem.oid]) {
        throw _err("cmp/bad-info-value", "a KEM ciphertext answer names " +
          (value.kem.name || value.kem.oid) + ", which this client cannot decapsulate under (ML-KEM-512/768/1024)");
      }
      /** @internal An ML-KEM ciphertext is a fixed size, so one of any other length could not be
       * decapsulated under the algorithm the answer names. Refusing it here reports the authority's
       * answer as malformed rather than handing back material that cannot be used. */
      var want = oid.kemParams(value.kem.oid);
      if (!value.ct || value.ct.length !== want.ct) {
        throw _err("cmp/bad-info-value", "a KEM ciphertext answer for " +
          (value.kem.name || value.kem.oid) + " carries " + (value.ct ? value.ct.length : 0) +
          " bytes, and that algorithm's ciphertext is " + want.ct);
      }
      return value;
    }
...
module.exports = {
  ...
  verify: cmp.verify,
  openKeyPackage: cmp.openKeyPackage,
  session: session,
