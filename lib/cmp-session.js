...
function _boundedPool(base, added) {
  var room = constants.LIMITS.PATH_BUILD_MAX_CANDIDATES - base.length;
  if (room <= 0) return base.slice();
  var seen = Object.create(null), useful = [];
  base.forEach(function (c) { var k = _certIdentity(c); if (k != null) seen[k] = 1; });
  added.forEach(function (c) { var k = _certIdentity(c); if (k != null && seen[k]) return; if (k != null) seen[k] = 1; useful.push(c); });
  return base.concat(useful.slice(0, room));
}
...
// tbsCertificate bytes uniquely identify a certificate's content (serial / issuer / subject / key / validity).
// Returns null when the identity cannot be derived; a NON-deduped entry is safe (a redundant slot), a wrong merge is not.
function _certIdentity(cert) {
  try {
    ...
    return Buffer.isBuffer(p.tbsBytes) ? p.tbsBytes.toString("base64") : null;
...
function _spkiKeyIdentity(spkiDer) {
  ...
  var params = (algOid === OID_RSA_ENCRYPTION) ? "" : (algId.children[1] ? algId.children[1].bytes.toString("latin1") : "");
...
function session(opts) {
  ...
  } else if (opts.trustAnchors != null) {
    // A PBMAC1 session authenticates every response by the shared secret, not a certificate path -- cmp.verify's
    // MAC branch REJECTS trustAnchors as a stray signature credential. Catch it at construction, before the
    // one-shot transaction is spent on a request that can only fail when its response reaches verify.
    throw _err("cmp/bad-input", "opts.trustAnchors is a signature-flavor credential; a MAC (PBMAC1) session authenticates responses by the shared secret -- drop it");
  }
  ...
  function _verifyOpts(fresh, signerCertOverride, extraIntermediates) {
    ...
    // Fallback: the cached signer chain lets the override signer rebuild to the anchor. It is (part-)derived from
    // an UNSIGNED extraCerts, so bound it below the candidate ceiling after the caller's own intermediates -- else
    // a large-but-valid caller pool plus the cached chain overflows cmp.verify's signer path (the same bound the
    // issued-leaf path applies), failing an otherwise-valid later leg. Drop the override signer ITSELF from the
    // chain first: it is the path TARGET (passed as signerCert), not an issuer, so leaving it in would spend a
    // scarce intermediate slot on the signer and truncate the real issuer it needs to reach the anchor.
    var ints = _asCertList(opts.intermediates);
    if (Array.isArray(extraIntermediates)) {
      var issuers = signerCertOverride == null ? extraIntermediates : extraIntermediates.filter(function (c) {
        return !(Buffer.isBuffer(c) && Buffer.isBuffer(signerCertOverride) && c.equals(signerCertOverride));
      });
      ints = _boundedPool(ints, issuers);
    }
  ...
  async function _send(bodySpec, arm) {
    ...
    // Fallback: a signature-flavor response's OWN signer resolution can fail two ways that the earlier,
    // authenticated signer still recovers -- (a) NO signer resolved (a later leg omitted extraCerts once the
    // recipient holds the cert, RFC 9483 sec. 3.3), or (b) a WRONG signer resolved: extraCerts is outside the
    // protected part, so a meddler can prepend a same-subject decoy the resolver selects first, failing
    // protection under a key that never signed. Both cases retry against the cached signer (+ its chain, which
    // lets the path rebuild to the anchor). The response's own certs are still preferred on the FIRST attempt,
    // so a clustered CA's legitimately rotated cert wins there; this fallback only rescues an otherwise-valid
    // leg, never accepts a genuinely tampered one (the cached signer's key is fixed, not attacker-chosen).
    var usedFallback = false;
    if (verdict.valid !== true && isSig && cachedSignerCert != null &&
        (verdict.code === "cmp/signer-cert-not-found" || verdict.code === "cmp/protection-failed")) {
  ...
  function _classify(verdict) {
    ...
    if (arm === expectedRespArm) {
      ...
      if (_isGranted(code)) return { state: "granted", resp: resp, caPubs: body.decoded.caPubs, code: code };
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    // Bound the SESSION-supplied pool material below the candidate ceiling AFTER the caller's own intermediates:
    // the authenticated caPubs (this leaf's CA-delivered issuer chain, RFC 9810 sec. 5.3.4) first, then the
    // cached protection signer + its chain. The returned chain still carries every caPubs (surfaced separately);
    // only this validation pool is capped, so a valid caller pool + a legitimate caPubs / an extraCerts flood
    // cannot overflow the ceiling and fail an otherwise-valid grant.
    var added = [];
    if (Array.isArray(caPubs)) caPubs.forEach(function (c) { if (Buffer.isBuffer(c)) added.push(c); });
    ...
    var pool = _boundedPool(_asCertList(opts.intermediates), added);
    var res;
  ...
  async function _finish(granted, header) {
    ...
    if (conf.rejected) {
      return _terminal("rejected", { certificate: leaf, chain: chain, status: _statusOf(granted.resp), polls: granted.polls || 0 });
    }
  ...
  async function enroll(request) {
    ...
    try {
      ...
      if (t.state === "granted") { var r = await _finish({ resp: t.resp, caPubs: t.caPubs, polls: pollCount, code: t.code }, grantHeader); r.polls = pollCount; return r; }
