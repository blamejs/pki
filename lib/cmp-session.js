...
function _certConfHash(certDer) {
  ...
  // A composite signature (draft-ietf-lamps-pq-composite-sigs): its OID conveys no single message hash, so
  // (like the hashless PQC signatures) compute certHash under SHA-256 and DECLARE it in the explicit hashAlg.
  if (compositeSig.COMPOSITE_ALGS[sa.oid]) return { digest: "SHA-256", hashAlg: "sha256" };
...
function session(opts) {
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    // The issued leaf may be signed by the CMP protection signer, delivered only in the response's extraCerts.
    // Include that verified signer certificate in the pool so its (already-authenticated) chain material lets
    // the leaf build a path -- it is pool material, still re-validated to the anchor, never a trust anchor itself.
