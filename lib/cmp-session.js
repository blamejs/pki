...
//
// "RSA" is decided by OID FAMILY, not a hand-list. An RSA-DEDICATED arc -- one whose members are RSA --
// is matched by its prefix, so a standardized member this registry has not named is caught with the ones
// it has: PKCS#1 (rsaEncryption, PSS, OAEP, every sha*/md*WithRSAEncryption), the TeleTrusT rsaSignature
// arc (RSA with SHA-1 and the RIPEMD family), the TeleTrusT signatureScheme arc (the ISO/IEC 9796-2 RSA
// signature-with-message-recovery schemes -- deterministic 1.3.36.3.4.2 and randomized .4.3 -- plus ISO/IEC
// 9796-1 on .4.1), and the BSI TR-03110 id-TA-RSA arc (EAC Terminal Authentication, RSA v1.5/PSS over
// SHA-1/256/512). Each prefix is the arc a registered member sits directly under, read from the registry,
// so no dotted-decimal literal appears here and pki.oid.register cannot move it. The strict-descendant
// match skips an arc NODE itself, so a scheme's bare arc node (the 9796 1.3.36.3.4.1/.2/.3 nodes) is named
// off-arc below; the generic signatureScheme PARENT (1.3.36.3.4) and the sibling authentication arc
// (1.3.36.3.5, whose .5.3 children are ECC) are NOT RSA-dedicated, so neither is prefix-matched. The PKCS#1
// arc also carries two auxiliary identifiers that are NOT RSA algorithms --
// mgf1 and pSpecified, the mask-generation and OAEP-label parameters -- so those are excluded from the
// match by name (RSA_ARC_EXCLUDE) and surfaced. RSA also sits off those arcs on ones it SHARES with non-RSA
...
// ML-DSA and SLH-DSA; RSASSA-PSS-with-SHAKE (RFC 8692) on the PKIX arc; every RSA identifier the OIW SIG
// assigned on the OIW Secsig arc (the md4/md5-WithRSA, rsaSignature, mdc2/sha-WithRSA, rsaKeyTransport,
// and md2/md5/sha1-WithRSASignature identifiers), which is shared with DES, DSA, DH, and the hashes so no
// prefix isolates it; and the X.500 directory RSA OIDs on the 2.5.8 arc -- rsa (2.5.8.1.1), distinct from
// PKCS#1 rsaEncryption, and sqMod-nWithRSA / mdc2WithRSA on the signatureAlgorithm sub-arc; and the
// Chinese GM/T sm3WithRSAEncryption (1.2.156.10197.1.504), an RSA-over-SM3 signature on the SM arc it
// shares with SM2 and SM3. A composite signature naming RSA as one half is a composite key, not an RSA
// one, and lives on the PKIX arc, so it is outside all of these.
...
["id-rsa-kem", "id-kem-rsa",
  ...
  "rsa",   // the X.509 directory rsa public-key OID (2.5.8.1.1), distinct from PKCS#1 rsaEncryption
  "sqMod-nWithRSA", "mdc2WithRSA",   // the X.500 directory signatureAlgorithm RSA OIDs (2.5.8.3.1, 2.5.8.3.100)
  "sm3WithRSAEncryption",   // the Chinese GM/T RSA-with-SM3 signature (1.2.156.10197.1.504)
...
// The PKCS#1 arc carries two identifiers that are NOT RSA algorithms: mgf1 (the mask-generation function
// RSA-PSS and RSA-OAEP parametrize) and pSpecified (the OAEP label source). The prefix match would sweep
// them in, but a keySpec algId naming one states no RSA key requirement -- the entity does not generate an
// "mgf1 key" -- so they are surfaced, not refused, and are excluded from the arc match by name.
...
function session(opts) {
  ...
  function _classify(verdict) {
    ...
    if (arm === expectedRespArm) {
      ...
      // RFC 9483 sec. 4.4: a "waiting" CertResponse must not carry failInfo -- a wait is not a failure. Refuse
      // a contradictory failInfo (mirroring the _classifyError waiting check). A granting status carrying
      // failInfo is already refused upstream: the parser rejects failInfo alongside a certifiedKeyPair (RFC 9810
      // sec. 5.3.4), and a grant with no certificate is refused as a missing issued certificate.
  ...
  function _classifyError(body) {
    ...
    if (code === 3) {
      // sec. 4.4 puts waiting in an ip/cp/kup when answering an enrollment and in an error message for
      // anything else, so a waiting error drives the poll loop only for revoke/info. For an enrollment a
      // waiting ERROR is off-profile and refused, not coerced to a rejection whose own status reads
      // "waiting". A wait is not a failure, so sec. 4.4 forbids failInfo on it -- a waiting error carrying
      // one is contradictory and refused rather than polled.
    ...
    // sec. 4.2 / 3.6.4: an error message that is not delayed delivery is a rejection, whose status is
    // rejection(2). accepted(0), grantedWithMods(1), a status in 4..6, an unknown value, or an absent
    // status is malformed for an error message, so it is refused rather than surfaced as a definitive
    // rejected verdict a caller would act on -- the same call the rp classifier makes for an unsupported
    // status (a trusted "rejected" whose PKIStatusInfo says "accepted" is a contradiction, not a verdict).
  ...
  async function info(request) {
    ...
    if (op.value === false) {
      ...
      // sec. 4.3.2: rootCaCert names the CURRENT root the update chains from, which is a CA. An
      // end-entity certificate here would let a cross-certificate signed by that non-CA key read as a
      // rollover (the response path verifies newWithOld / oldWithNew against this certificate's key), so
      // it is held to being a CA before the one-shot transaction engages rather than after.
  ...
  function _checkInfoValue(op, value, requestValue) {
    ...
    if (op.name === "crlUpdate") {
      // sec. 4.3.4: the request carries exactly one CRLStatus (one source), and the response "represents
      // the latest CRL available from the referenced CRL source" -- one CRL. The ASN.1 list is SIZE
      // (1..MAX), so the responder could return several; accepting them all would surface an older CRL
      // beside the latest and let a caller act on a stale value[0]. Exactly one is required, and it is
      // then bound to the query that asked for it.
  ...
  function _assertCaCapable(cert, which, code) {
    code = code || "cmp/bad-info-value";   // a response cert is cmp/bad-info-value; a caller-supplied one (rootCaCert request) is cmp/bad-input
  ...
  function _isRsaAlgorithm(dotted) {
    if (RSA_ARC_EXCLUDE[dotted] === 1) return false;   // mgf1 / pSpecified: auxiliary OIDs on the PKCS#1 arc, not RSA algorithms
