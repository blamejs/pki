...
//
// "RSA" is decided by OID FAMILY, not a hand-list: every OID under the PKCS#1 arc is an RSA algorithm,
// so a standardized member this registry has not named -- sha1WithRSAEncryption, the md* and sha224
// variants -- is caught with the ones it has. The arc is read from the registry (rsaEncryption's own
// arc, less its last component), so no dotted-decimal literal appears here and pki.oid.register cannot
// move it. RSA also sits off PKCS#1 on arcs it SHARES with non-RSA algorithms, where no prefix can
// isolate it, so those are listed by name: id-rsa-kem (S/MIME) and id-kem-rsa (ISO 18033-2); the
// RSASSA-PKCS1-v1_5-with-SHA-3 set on the NIST signature arc, which it shares with ML-DSA and SLH-DSA;
// RSASSA-PSS-with-SHAKE (RFC 8692) on the PKIX arc; and the legacy OIW md2/md5/sha1-WithRSASignature
// set on the OIW Secsig arc, shared with DSA and the SHA-1 hash. A composite signature naming RSA as
// one half is a composite key, not an RSA one, and lives on the PKIX arc, so it is outside all of these.
var RSA_PKCS1_ARC = oid.toArcs(oid.byName("rsaEncryption")).slice(0, -1);   // the PKCS#1 arc [1,2,840,113549,1,1]
...
function session(opts) {
  ...
  function _isRsaAlgorithm(dotted) {
    ...
    var underPkcs1 = arcs.length > RSA_PKCS1_ARC.length;   // the arc node itself is not an algorithm
    for (var i = 0; underPkcs1 && i < RSA_PKCS1_ARC.length; i++) { underPkcs1 = arcs[i] === RSA_PKCS1_ARC[i]; }
    return underPkcs1 || RSA_OFF_ARC[dotted] === 1;
  }
