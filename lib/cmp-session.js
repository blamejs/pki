  ...
  try {
    ...
    for (var i = 0; i < node.children.length; i++) {
      ...
      if (f.tagClass === "context" && f.tagNumber === 0 && f.children && f.children.length === 1) {
        ...
        var nm = oid.name(asn1.read.oid(algSeq.children[0]));
        var mm = /^sha-?(256|384|512)$/i.exec(nm || "");
        return mm ? "SHA-" + mm[1] : null;
      }
    }
...
function _certConfHash(certDer) {
  ...
  catch (_e) { /* allow:swallow-unverified _leafOf already x509-validated this certificate before _confirm calls _certConfHash, so parse cannot throw here; the SHA-256 default is a defense-in-depth fallback */ return { digest: "SHA-256", hashAlg: "sha256" }; }
  var m = /sha-?(256|384|512)/i.exec(sa.name || "");
  if (m) return { digest: "SHA-" + m[1], hashAlg: null };
  if (sa.name === "rsassaPss") { var pd = _pssDigest(sa.parameters); if (pd) return { digest: pd, hashAlg: null }; }
 ...
 * and read-only `transactionID` + `transcript`. The granted certificate must be valid X.509 AND carry the
 * key the request submitted (else it is a misrouted certificate the caller cannot use). The `certConf`
...
function session(opts) {
  ...
  function _verifyOpts(fresh, signerCertOverride) {
  ...
  async function _send(bodySpec, arm) {
    ...
    // extraCerts once the recipient holds the cert, RFC 9483 sec. 3.3), retry with the signer cached from an
    // earlier leg. The response's own certs are preferred first, so a clustered CA's rotated cert still wins.
    if (verdict.valid !== true && isSig && cachedSignerCert != null && verdict.code === "cmp/signer-cert-not-found") {
      verdict = await cmp.verify(res.responseBytes, _verifyOpts(fresh, cachedSignerCert));
    }
    ...
    haveResponse = true;
    // Refresh the cached signer from EVERY verified signature response -- the MOST RECENTLY authenticated cert
    // (a clustered CA may rotate its protection cert across legs), so a later leg that omits extraCerts falls
    // back to the current signer, not a stale earlier one. The verdict is already valid + trusted at this point.
    if (isSig && verdict.signer && Buffer.isBuffer(verdict.signer.cert)) cachedSignerCert = verdict.signer.cert;
    return verdict;
  }
  ...
  async function _finish(granted, header) {
    ...
    // ONLY in the CMP response can assemble the full chain. They are chain material, NOT trust anchors.
    var chain = [leaf];
    if (Array.isArray(granted.caPubs)) granted.caPubs.forEach(function (c) { if (Buffer.isBuffer(c)) chain.push(c); });
