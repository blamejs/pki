...
function session(opts) {
  ...
  /** @internal Captured once at construction and used from here on. Reading the option again would
   * ask a second time, and an accessor can answer differently on each read, so the value that
   * passed the check need not be the one that gets called. */
