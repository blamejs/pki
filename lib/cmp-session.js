...
// The public-key algorithms a keySpec algId MAY name (RFC 9483 sec. 4.3.3: the control "specifies
// the public key types optionally with parameters ... for which a certificate may be requested"),
// resolved to identifiers at LOAD.
//
// This is a positive list on purpose. Written as an exclusion it kept losing: first one RSA
// identifier, which missed rsassaPss and the shaNWithRSAEncryption set; then one arc, which missed
// id-rsa-kem and id-kem-rsa on two others; and an exclusion still says nothing about sha256 or an
// AES mode, which are registered, are not RSA, and are not key algorithms either. Naming what an
// entity can be asked to generate answers all of those at once, and anything else -- RSA included,
// which sec. 4.3.3 sends to rsaKeyLen -- falls outside by construction.
//
// Identifiers rather than names, because pki.oid.register can replace the display name of an OID
// already registered; read at load, before any caller has run.
var KEY_ALG_OIDS = Object.create(null);
["ecPublicKey", "Ed25519", "Ed448", "X25519", "X448",
  ...
  "id-slh-dsa-shake-128s", "id-slh-dsa-shake-128f", "id-slh-dsa-shake-192s", "id-slh-dsa-shake-192f",
  "id-slh-dsa-shake-256s", "id-slh-dsa-shake-256f",
  // The composite signatures: a hybrid is a composite key rather than an RSA one even where RSA is
  // one of its halves, and rsaKeyLen cannot state a requirement for one.
  ...
  if (dotted) KEY_ALG_OIDS[dotted] = 1;
});
...
function session(opts) {
  ...
  async function _checkRootCaKeyUpdate(value, oldRootDer) {
    ...
    _assertCaCapable(pNewNew, "newWithNew");
    _assertCaCapable(pNewOld, "newWithOld");
    ...
    if (oldWithNew !== null) {
      ...
      _assertCaCapable(pOldNew, "oldWithNew");
  ...
  // A root key update moves CA authority, so each certificate in it has to carry that authority.
  // Names, a key and a signature are satisfied by an ordinary end-entity certificate the old root
  // once issued for the same subject and key, and such a certificate cannot sign anything: the
  // transition would be reported as usable while producing an authority that certifies nothing.
  // The rule is RFC 5280 sec. 6.1.4's, the one pki.path.validate applies to any intermediate --
  // basicConstraints cA TRUE, and keyUsage, where present, allowing keyCertSign. The extensions are
  // read through the shared certificate-extension decoders rather than walked here.
  function _assertCaCapable(cert, which) {
    var bc = null, ku = null;
    cert.extensions.forEach(function (e) {
      if (e.oid === OID_BASIC_CONSTRAINTS) bc = cmpBuild.decodeCertExtension(e.oid, e.value);
      if (e.oid === OID_KEY_USAGE) ku = cmpBuild.decodeCertExtension(e.oid, e.value);
    });
    if (!bc || bc.cA !== true) {
      throw _err("cmp/bad-info-value", "the rootCaKeyUpdate " + which + " must be a CA certificate (basicConstraints cA TRUE): a certification authority is what this update transfers, and an end-entity certificate for the same key transfers nothing (RFC 5280 sec. 6.1.4)");
    }
    if (ku && ku.keyCertSign !== true) {
      throw _err("cmp/bad-info-value", "the rootCaKeyUpdate " + which + " carries a keyUsage that withholds keyCertSign, so it cannot act as the authority this update names (RFC 5280 sec. 6.1.4)");
    }
  }
  ...
  function _bindCrlToQuery(der) {
    ...
    if (crlQuery && crlQuery.dpn != null) {
      var idpDpn = null, crlExts = parsed.crlExtensions || [];
      ...
      // for the point this request named or it does not, and the answer does not depend on which
      // verb is asking. A CRL naming no point is a complete list and is left to the freshness rule.
      if (idpDpn !== null && !guard.name.dpnCorresponds(crlQuery.dpn, idpDpn, _err, "cmp/bad-info-value", "the crlUpdate distribution point")) {
        throw _err("cmp/bad-info-value", "the returned CRL is scoped to a distribution point other than the one this request named (RFC 9483 sec. 4.3.4, RFC 5280 sec. 5.2.5)");
      }
    }
  ...
  //
  function _isKeyAlgorithm(dotted) { return KEY_ALG_OIDS[dotted] === 1; }
  ...
  function _checkKeySpec(ctrl) {
    ...
    if (ctrl.type === OID_REG_CTRL_ALG_ID) {
      ...
      if (!_isKeyAlgorithm(alg.oid)) {
        throw _err("cmp/bad-info-value", "a keySpec algId must name a public-key algorithm an entity can be asked to generate, and other than RSA, whose requirement is stated with rsaKeyLen instead (RFC 9483 sec. 4.3.3); got " + (alg.name || alg.oid));
      }
