...
function _certConfHash(certDer) {
  ...
  // A RECOGNIZED signature whose OID conveys no hash (Ed25519 / Ed448, ML-DSA / SLH-DSA): declare SHA-256.
  if (name !== "") return { digest: "SHA-256", hashAlg: "sha256" };
  // An UNRECOGNIZED signature-algorithm OID (the name did not resolve): the required certConf hash is
  // indeterminate -- do NOT guess SHA-256 (a wrong certHash the CA rejects); fail the transaction closed.
  throw _err("cmp/bad-cert-response", "the issued certificate's signature algorithm is unrecognized, so the certConf hash cannot be determined (RFC 9810 sec. 5.3.18)");
}
