...
function session(opts) {
  ...
  // basicConstraints cA TRUE, marked critical (sec. 4.2.1.9), and keyUsage, where present, allowing
  // keyCertSign. Criticality matters because a relying party that skips extensions it does not
  // recognize would not see the cA bit at all, which is why the path validator refuses a non-critical
  // cA:TRUE; this check holds to the same line so the two never disagree on what a CA certificate is.
  // The extensions are read through the shared certificate-extension decoders rather than walked here.
  function _assertCaCapable(cert, which) {
    ...
    if (!bcCritical) {
      throw _err("cmp/bad-info-value", "the rootCaKeyUpdate " + which + " marks basicConstraints non-critical, so a relying party that skips unrecognized extensions would not see the cA bit it must act on (RFC 5280 sec. 4.2.1.9)");
    }
