...
// The RSA algorithms a keySpec algId may not name (RFC 9483 sec. 4.3.3), resolved to identifiers at
// LOAD. They are registered on three separate arcs -- PKCS#1, the S/MIME arc, and ISO/IEC 18033-2 --
// so no prefix test spans them; and the identifier, not the display name, is the part that cannot be
// re-registered underneath this check.
var RSA_ALG_OIDS = Object.create(null);
["rsaEncryption", "rsaesOaep", "rsassaPss", "sha256WithRSAEncryption", "sha384WithRSAEncryption",
  "sha512WithRSAEncryption", "id-rsa-kem", "id-kem-rsa"].forEach(function (n) {
  var dotted = oid.byName(n);
  if (dotted) RSA_ALG_OIDS[dotted] = 1;
});
 ...
 * named source. Terminal `outcome`: `answered` (with `operation`, `present`, and the decoded `value`),
...
function session(opts) {
  ...
  async function revoke(request) {
    ...
    if ((request.certificate == null) === (request.certDetails == null)) {
    ...
    if (request.reason != null && (typeof request.reason !== "string" || !CRL_REASON_NAMES[request.reason])) {
      throw _err("cmp/bad-input", "revoke request.reason must be a CRLReason name (RFC 5280 sec. 5.3.1); got " + JSON.stringify(request.reason));
    }
    var certDetails = request.certDetails;
    if (request.certificate != null) {
      var target;
      try {
        target = guard.parsed.acceptDerived(request.certificate, "certificate", x509.parse, _err, "cmp/bad-input", "revoke request.certificate");
      } catch (e) {
        if (e && e.isCmpError) throw e;
    ...
    _assertRevokingOwnCertificate(certTemplateDer);
    ...
    var body = { rr: [{ certDetails: certTemplateDer, crlEntryDetails: { reason: request.reason == null ? undefined : request.reason } }] };
    inFlight = true;
    try {
      ...
      if (t.state === "granted") {
        return _terminal("revoked", {
          ...
          revokedCerts: t.revCerts || null,   // the CertIds the responder confirms it revoked, when it names them
  ...
  async function info(request) {
    ...
    var names = Object.keys(request).filter(function (k) { return request[k] != null; });
    if (names.length !== 1) {
    ...
    if (op.value === false) {
      if (request[names[0]] !== true) throw _err("cmp/bad-input", "info request." + op.name + " takes `true`: this operation's genm carries no infoValue (RFC 9483 sec. 4.3.1 and sec. 4.3.3)");
    } else if (op.value === "cert") {
      // sec. 4.3.2: the request SHOULD carry the root CA certificate the update is requested for,
      // and the field is a CMPCertificate. Holding it to that here keeps a caller's mistake a local
      // refusal; sending it unchecked would put a malformed request on the wire and consume this
      // one-shot transaction to learn what the caller could have been told before it started.
      itav.infoValue = _asCertificate(
        guard.bytes.snapshot(request[names[0]], CmpError, "cmp/bad-input", "info request rootCaCert"),
        "the rootCaCert this update is requested for", "cmp/bad-input");
    } else if (op.value === "crlStatus") {
      var built = cmpBuild.buildCrlStatusList(request[names[0]]);
      ...
      // Reduced to an instant HERE, not kept as the caller's Date. The comparison happens after the
      // transport round-trip, and a Date the caller still holds can be moved in between, which would
      // judge the response against a query the request never made.
      var askedAt = request[names[0]].thisUpdate;
      crlQuery = {
        issuerName: built.issuerName,
        thisUpdate: askedAt == null ? null : guard.time.instantOf(askedAt, _err, "cmp/bad-input", "crlUpdate.thisUpdate"),
      };
    }
    expectedInfoOp = op;
    inFlight = true;
  ...
  function _checkInfoValue(op, value, requestValue) {
    ...
    if (op.name === "caCerts") return value.map(function (d) { return _asCertificate(d, "a caCerts entry"); });
  ...
  //
  // The issuer arm is bound by name. The dpn arm cannot be: a distribution point name is a pointer
  // the responder resolves internally, and a CertificateList need not name the point it came from,
  // so there is nothing in the response to compare it against. The freshness rule applies to both.
  ...
  // "An algorithm other than RSA" is not a statement about an arc. RSA algorithms are registered on
  // three of them: PKCS#1 holds rsaEncryption, rsassaPss, rsaesOaep and the shaNWithRSAEncryption
  // set; id-rsa-kem sits on the S/MIME arc; id-kem-rsa on the ISO 18033-2 arc; and the composite
  // signatures name RSA inside a PKIX arc. Any test written against one arc, or against one OID,
  // leaves the others accepted. What is common to all of them is the registry: each resolves to a
  // name that says RSA, so the question is asked of the name the registry gives, and a newly
  // registered RSA algorithm is covered on the day it lands.
  //
  // An OID the registry cannot name at all is refused as well. This value is a requirement an
  // entity is meant to act on by generating a key, and an algorithm this toolkit cannot identify is
  // one it cannot tell an operator to generate -- nor rule out as RSA. Both halves fail closed.
  //
  // The set is of IDENTIFIERS, taken at load. A display name is a label a caller may override
  // (pki.oid.register replaces the forward name of an OID already registered), so asking whether a
  // name says RSA asks a mutable question: renaming rsaEncryption would answer no while the
  // algorithm stayed exactly what it is. The identifiers do not move, and reading them before any
  // caller has run means the answer is fixed by the time a response can ask it.
  //
  // The composite ML-DSA + RSA signatures are deliberately absent: a hybrid is not an RSA key, it
  // is a composite one, and rsaKeyLen cannot state it either -- refusing them would leave a
  // conforming responder no way to require one.
  function _namesRsa(dotted) { return RSA_ALG_OIDS[dotted] === 1; }
  ...
  function _checkKeySpec(ctrl) {
    ...
    if (ctrl.type === OID_REG_CTRL_ALG_ID) {
      ...
      if (alg.name == null) throw _err("cmp/bad-info-value", "a keySpec algId names an algorithm this toolkit does not recognize (" + alg.oid + "), so it cannot be acted on as a key requirement nor ruled out as RSA (RFC 9483 sec. 4.3.3)");
      if (_namesRsa(alg.oid)) throw _err("cmp/bad-info-value", "a keySpec algId must give an algorithm other than RSA -- an RSA requirement is stated with rsaKeyLen (RFC 9483 sec. 4.3.3); got " + alg.oid);
