    ...
    if (arm === expectedRespArm) {
      ...
      if (body.decoded && Array.isArray(body.decoded.caPubs)) body.decoded.caPubs.forEach(function (c) {
        ...
        if (caPubsSeen[k]) {
          ...
          if (isGrantLeg) { for (var pi = 0; pi < caPubsAccum.length; pi++) { if (caPubsAccum[pi].toString("base64") === k) { caPubsAccum.push(caPubsAccum.splice(pi, 1)[0]); break; } } }
          return;
        }
        ...
        if (isGrantLeg) {
          while (caPubsAccum.length > 0 && (caPubsAccum.length >= CAPUBS_MAX || caPubsBytes + c.length > caPubsByteBudget)) {
        ...
        if (caPubsAccum.length >= CAPUBS_MAX || caPubsBytes + c.length > caPubsByteBudget) return;   // a single certificate larger than the whole budget
        caPubsSeen[k] = 1;
