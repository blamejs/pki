...
function session(opts) {
  ...
  async function _confirm(certDer, header, info) {
    // Consult the caller's acceptance policy BEFORE confirming. A grantedWithMods response (RFC 9810 sec. 5.2.3)
    // carries a certificate the CA CHANGED (subject / validity / extensions), and an accepting certConf is
    // irrevocable, so the caller may inspect + veto it here. The policy MUST return true to accept; anything
    // else (including a thrown rejection surfacing as a non-true value is NOT swallowed -- a throw propagates)
    // is a veto. No policy accepts (the default). The cert is already structurally + path valid at this point.
    var accept = true;
    ...
    if (opts.implicitConfirm && _implicitConfirmGranted(header)) {
      // Implicit confirmation is final at delivery -- there is no reject leg to send. Surface the caller's
      // verdict, but a veto cannot un-deliver the certificate (a caller who must vet a grant must NOT request
      // implicitConfirm). rejected:true here reflects the caller's decision, not a protocol rejection.
      return { confirmed: accept, implicit: true, rejected: !accept };
    }
  ...
  async function _validateLeaf(leaf, caPubs) {
    ...
    // that authenticated it) so the leaf builds a path -- pool material, still re-validated to the anchor, never
    // trust anchors themselves. But that cached material comes (in part) from the response's UNSIGNED extraCerts,
    // so a meddler can inflate it; append it only up to the path builder's candidate ceiling AFTER the caller's
    // own intermediates + the authenticated caPubs (mirroring cmp.verify's signer-chain bounding), so an
    // extraCerts flood can never push a valid caller configuration over the ceiling and fail an otherwise-valid
    // grant. If the caller's own pool already exceeds the ceiling, that is a genuine config error path.build reports.
    var room = constants.LIMITS.PATH_BUILD_MAX_CANDIDATES - pool.length;
    if (room > 0) {
      var cached = [];
      if (Buffer.isBuffer(cachedSignerCert)) cached.push(cachedSignerCert);
      cachedChain.forEach(function (c) { if (Buffer.isBuffer(c)) cached.push(c); });
      // Dedup by byte-exact identity (the signer appears as BOTH cachedSignerCert and cachedChain[0]) so a slot
      // is not spent on a copy; then truncate to the remaining room.
      var seen = Object.create(null), useful = [];
      cached.forEach(function (c) { var k = c.toString("base64"); if (!seen[k]) { seen[k] = 1; useful.push(c); } });
