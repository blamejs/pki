...
// A canonical identity for a SubjectPublicKeyInfo: the algorithm OID + the raw subjectPublicKey BIT STRING,
// IGNORING the AlgorithmIdentifier parameter encoding (an rsaEncryption SPKI is valid with the NULL parameter
// or with it omitted -- both name the same key). So the issued-cert key-match compares KEYS, not byte encodings.
function _spkiKeyIdentity(spkiDer) {
  var node = asn1.decode(spkiDer);   // SEQUENCE { AlgorithmIdentifier, BIT STRING }
  return asn1.read.oid(node.children[0].children[0]) + "|" + node.children[1].bytes.toString("latin1");
}

// The PARSEABLE extraCerts (signer + intermediates) of an already-verified response, cached so a later leg
// that omits extraCerts can still rebuild the signer path. Drops any non-X.509 entry (which cmp.verify already
// bounds away) so a malformed entry is never fed to path.build as bogus config material. The bytes already
// parsed during verify, so schemaCmp.parse cannot throw here.
function _responseExtraCerts(responseBytes) {
  var m = schemaCmp.parse(responseBytes);
  if (!Array.isArray(m.extraCerts)) return [];
  return m.extraCerts.filter(function (c) {
    if (!Buffer.isBuffer(c)) return false;
    try { x509.parse(c); return true; }
    catch (_e) { return false; }   // drop a malformed extraCerts entry, mirroring cmp.verify's bounding
  });
}
