...
function session(opts) {
  ...
  async function _send(bodySpec, arm) {
    ...
    // The response's OWN issuers AND the whole caller pool fit in ONE candidate pool (SESSION_MAX_INTERMEDIATES
    // reserves room below the ceiling), so a single verify sees every candidate -- no priority attempt has to
    // choose between them, and a signer path assembled from both sources builds. No signer identification is
    // needed (senderKID may name a cert that is not extraCerts[0]); every candidate is simply present.
