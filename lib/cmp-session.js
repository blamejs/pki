...
// move it. RSA also sits off PKCS#1 on two arcs -- id-rsa-kem (S/MIME) and id-kem-rsa (ISO 18033-2) --
// listed explicitly. A composite signature naming RSA as one half is a composite key, not an RSA one,
// and lives on a PKIX arc, so it is outside all of these and is surfaced.
...
["id-rsa-kem", "id-kem-rsa"].forEach(function (n) { var d = oid.byName(n); if (d) RSA_OFF_ARC[d] = 1; });
