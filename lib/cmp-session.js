...
// The completeness rule is the shared one (guard.parsed.isCert), used as a PREDICATE rather than the
// throwing door, because an underivable identity is a redundant pool slot here and not a fault.
//
// This reads the fields off the object rather than re-deriving them from the parser's record, which
// every DECIDING door now does. The difference is deliberate and the reasoning is worth keeping:
// colliding two DISTINCT certificates onto one slot would need a rebuilt entry carrying another
// certificate's exact tbsBytes AND signature -- and anything holding both of those is that
// certificate. A caller can only collide their own pool with itself, and each entry is re-derived
// again at the path-building door before it decides anything. If that ever stops being true --
// if an identity here starts feeding a decision rather than a dedupe -- this must move to the
// derived form with the rest.
function _certIdentity(cert) {
  try {
    var p = (cert && cert.tbsBytes !== undefined) ? cert : x509.parse(cert);   // x509.parse accepts a DER Buffer OR a PEM string
