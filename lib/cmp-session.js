...
// can occupy: CAPUBS_MAX (a grant's caPubs) OR MAX_EXTRA_CERTS (a response's extraCerts), PLUS MAX_EXTRA_CERTS for
// the cached signer chain. So every candidate pool the session builds (signer primary/fallback, leaf validation)
// holds the whole caller pool AND all authenticated material at once -- one build, no priority choice, no retry.
...
// Merge a freshly-authenticated leg's extraCerts (the new signer + any issuers it carries) with the previously
// cached chain, deduped by byte identity and capped at MAX_EXTRA_CERTS. The fresh certs take priority; the prior
// issuers then fill. This RETAINS the establishing intermediate across a same-identity signer rotation: a signer
// delivered BARE (only itself) chains through an issuer that arrived on an EARLIER leg, and overwriting the cached
// chain with the bare extraCerts would discard it -- so a SUBSEQUENT bare leg from the rotated signer could no
// longer chain. Every input is already authenticated (this runs only after a self-authenticated verify), so a
// retained older cert is a harmless extra candidate, deduped + ceiling-bounded wherever the chain is consumed.
function _mergeCachedChain(fresh, prior) {
  // Both inputs are already-filtered Buffer arrays (_responseExtraCerts output / a prior cachedChain), so no
  // per-entry type guard is needed here. Fresh certs lead so the newest signer + its own issuers take priority;
  // the prior issuers then fill. Dedup by byte identity, then bound to MAX_EXTRA_CERTS so the chain cannot grow
  // across legs and the pool math (responseExtra + cachedChain + caller <= ceiling) holds.
  var out = [], seen = Object.create(null);
  fresh.concat(prior).forEach(function (c) {
    var k = c.toString("base64");
    if (seen[k]) return;
    seen[k] = 1;
    out.push(c);
  });
  return out.slice(0, MAX_EXTRA_CERTS);
}
...
function session(opts) {
  ...
  // signature branch above already rejects [] outright. Normalize the MAC empty list to absent at construction so
  // _validateLeaf / _verifyOpts treat it uniformly -- otherwise the empty trust store reaches _engine.build and
  // rejects the issued certificate only in _finish, AFTER the authenticated grant has consumed the one-shot session.
  ...
  // Reject a DISTINCT intermediate pool exceeding SESSION_MAX_INTERMEDIATES (the candidate ceiling MINUS the room
  // reserved for the CA's own authenticated material). The caller pool is never truncated, so the whole pool + that
  // material must fit under path.build's ceiling; an oversized one would otherwise raise path/bad-input only at the
  // first verify, after the one-shot transaction is consumed. (Duplicates collapse, so the count is by distinct id.)
  if (opts.intermediates != null) {
    ...
    if (distinctInt > SESSION_MAX_INTERMEDIATES) throw _err("cmp/bad-input", "opts.intermediates has " + distinctInt + " distinct certificates, exceeding the " + SESSION_MAX_INTERMEDIATES + " limit (room is reserved below the path-builder ceiling for the CA's own delivered issuer certificates)");
  }
  ...
  async function _send(bodySpec, arm) {
    ...
    // cmp.transfer runs its LOCAL validation (URL parse, transfer-budget caps, TLS-anchor check) BEFORE engaging the
    // transport, rejecting a purely-local config error without sending anything. Mark the one-shot session consumed
    // ONLY once a request may have reached the transport -- so the caller can fix such a config error and retry.
    var res;
    try {
      res = await cmp.transfer(opts.url, reqDer, _transferOpts());
    } catch (e) {
      if (!(e && (e.code === "cmp/bad-url" || e.code === "cmp/no-trust-anchors" || e.code === "cmp/bad-input"))) started = true;
      throw e;
    }
    ...
    if (isSig && !usedCachedFallback && verdict.signer && Buffer.isBuffer(verdict.signer.cert)) {
      ...
      // MERGE with the prior chain (not overwrite): a bare rotation leg carries only the new signer, whose issuer
      // came on an earlier leg -- retaining it lets a SUBSEQUENT bare leg from the rotated signer still chain.
      if (extras.length) { cachedSignerCert = verdict.signer.cert; cachedChain = _mergeCachedChain(extras, cachedChain); }
    }
    return verdict;
  }
