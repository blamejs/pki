...
function constraintBaseUnreadable(tag, base) {
   ...
   * admits an underscore and refuses an empty atom, and the domain by the empty-label rule, an
   * address literal being a domain the comparison reads whole. */
  if (tag === 1 && _lastIndexOf(base, "@") !== -1) {
    ...
    if (_isAddressLiteral(mb[1])) return false;
...
function constraintBaseRefusal(tag, base) {
   ...
   * carries in its own extension never reaches a door, and `uriMatch` is what holds it to the rule
   * there, answering `path/name-constraint-unsupported` for a host it cannot read as fully qualified.
   * `constraintBaseUnreadable` does not judge label count and does not stand in for that check. */
  if (tag === 6) {
