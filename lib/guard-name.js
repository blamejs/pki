 ...
 * comparison asks only whether any name could ever match, and answers a base that none could with no
 * verdict rather than with no match, since an excluded subtree that matches nothing excludes nothing.
 * The two differ in both directions. A label carrying an underscore or edged with a hyphen is not a
 * host name the door accepts, and the suffix comparison still applies it to exactly the names it
 * names, so it stays enforced. A base whose body is empty names the root of the namespace and matches
 * every name, which the door refuses a caller and a certificate states as a bare dot, a zero-length
 * base being refused earlier when the extension is decoded. The reduction here is the comparison's
 * own, so the two cannot part company over a base like "..", whose body is empty only once a leading
 * dot the comparison keeps has been taken off.
...
function constraintBaseUnreadable(tag, base) {
  ...
  var body = base;
  if (tag === 1) {
    var at = _lastIndexOf(body, "@");
    if (at !== -1) body = _strSlice(body, at + 1);
  }
  body = stripTrailingDot(body);
