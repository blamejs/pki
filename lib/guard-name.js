...
/** @internal Whether the comparison can read a constraint base at all. This is a weaker question than
 * whether a caller may state one, and the two differ by a single case. A caller is refused a base
 * whose host body is empty, since a subtree that names the whole namespace is not the restriction
 * they believe they applied; a certificate states its own namespace, and a dNSName base of a bare dot
 * names its root, so the comparison reads it as every name. Every other base outside its tag's form
 * is unreadable, and answering one with no match rather than no verdict would leave an excluded
 * subtree that covers nothing excluding nothing.
 * @enforced-by behavioral -- the pairing with constraintBaseRefusal has no rename-proof code shape,
 * and the RED vectors in path-validate.test.js driving a CA's own nameConstraints are the guard. */
function constraintBaseUnreadable(tag, base) {
  if (tag === 2 && typeof base === "string" && _hostConstraintBody(base) === "") return false;
  return constraintBaseRefusal(tag, base) !== null;
}
...
function constraintBaseRefusal(tag, base) {
   ...
   * a fully qualified domain name and MAY specify a host or a domain. Examples would be
   * host.example.com and .example.com." Its dNSName paragraph states no such requirement, so a single
   * label is a valid dNSName base and is not a valid URI base. A caller meets this rule at the door,
   * whether through `pki.trust.anchor` or through the validator's subtree seeds; a base a certificate
   * carries in its own extension meets `constraintBaseUnreadable` when the comparison reads it, and a
   * single label fails both, so it yields `path/name-constraint-unsupported` rather than no match. */
  if (tag === 6) {
    var h = _hostConstraintBody(_toLowerCase(base));
    var dotted = _isHostName(h) && _indexOf(h, ".") !== -1 && !_allCharsIn(h, _DIGDOT_TABLE);
    return dotted ? null
      : "must be a fully qualified domain name, which RFC 5280 sec. 4.2.1.10 requires of a URI constraint and not of a dNSName one";
  }
  return null;
}
