...
function _isBlockedIp(ip) {
  ...
  if (fam === 6) {
    ...
    if (l.indexOf("2001:db8:") === 0 || l.indexOf("2001:0db8:") === 0) return true;   // 2001:db8::/32 documentation (RFC 3849)
    var h = parseInt(l.split(":")[0], 16);                                  // first hextet ("" for a leading "::" -> NaN -> not in 2000::/3 -> blocked)
    return !(h >= 0x2000 && h <= 0x3fff);                                   // ALLOW ONLY global unicast 2000::/3; block loopback / ULA / link-local / site-local / multicast (ff00::/8) / unspecified / unallocated
  }
  return false;
}
