...
function _tlsExporter(socket, proto) {
  ...
  var out;
  try { out = socket.exportKeyingMaterial(TLS_EXPORTER_LENGTH, TLS_EXPORTER_LABEL, Buffer.alloc(0)); }
  catch (_ee) { return null; }
  return (Buffer.isBuffer(out) && out.length === TLS_EXPORTER_LENGTH) ? out : null;
}
...
function _pemifyAnchors(anchors) {
  var list = Array.isArray(anchors) ? guard.list.copyMap(anchors, _pemifyAnchor) : [_pemifyAnchor(anchors)];
  return list;
}
...
function _copyAnchor(a) { return (a !== null && typeof a === "object" && nodeUtil.types.isProxy(a)) ? a : (Buffer.isBuffer(a) ? Buffer.from(a) : a); }
...
function _copyableRecord(p) {
  ...
  for (var j = 0; j < readable.length; j++) {
    var rd = Object.getOwnPropertyDescriptor(p, readable[j]);
    if (!rd || !rd.enumerable || rd.get || rd.set) return false;
  }
  return true;
}
function _copyableArray(a) {
  if (a === null || typeof a !== "object" || nodeUtil.types.isProxy(a) || !Array.isArray(a) || Object.getPrototypeOf(a) !== Array.prototype) return false;
  ...
  for (var i = 0; i < names.length; i++) {
    var d = Object.getOwnPropertyDescriptor(a, names[i]);
...
function snapshotProxy(p) {
  try {
    ...
    var out = Object.assign(Object.create(null), p);
    if (_copyableRecord(p.auth)) out.auth = Object.assign(Object.create(null), p.auth);
    if (_copyableRecord(p.tls)) {
      out.tls = Object.assign(Object.create(null), p.tls);
      var a = p.tls.anchors;
      if (_copyableArray(a)) {
        var copied = [];
        for (var i = 0; i < a.length; i++) copied[i] = _copyAnchor(a[i]);
        out.tls.anchors = copied;
      } else if (typeof a === "string") out.tls.anchors = a;
      else if (a !== null && typeof a === "object" && !nodeUtil.types.isProxy(a) && Buffer.isBuffer(a)) out.tls.anchors = Buffer.from(a);
    }
    return out;
  } catch (_e) {
...
function _validateProxy(request, defaults, E, C, url) {
  ...
  if (secure) {
    ...
    if (pHasAnchors) {
      if (Array.isArray(pAnchors) && !_copyableArray(pAnchors)) throw E(C("bad-proxy"), "opts.proxy.tls.anchors must be a plain array of PEM strings or DER Buffers");
      var pAnchorList = Array.isArray(pAnchors) ? pAnchors : [pAnchors];
      for (var pai = 0; pai < pAnchorList.length; pai++) {
        var pan = pAnchorList[pai];
        if ((pan !== null && typeof pan === "object" && nodeUtil.types.isProxy(pan)) || (typeof pan !== "string" && !Buffer.isBuffer(pan))) throw E(C("bad-proxy"), "opts.proxy.tls.anchors must be a PEM string or a DER Buffer (or an array of them)");
      }
    }
  ...
  var originPort = url.port || (url.protocol === "http:" ? 80 : 443);
  return { secure: secure, host: phost, port: purl.port ? +purl.port : (secure ? 443 : 80), originHost: originHost, originAuthority: originAuthHost + ":" + originPort, auth: auth, proxyTls: proxyTls };
}
...
function _digestProxyCredential(headers, pinfo, C, E) {
  ...
  var offered = String((headers && headers["proxy-authenticate"]) || "");
  var policy = {
...
function httpsTransport(defaults) {
  ...
  function _prepare(request) {
    ...
    if (url.protocol !== "https:" && !(plaintextAllowed && url.protocol === "http:")) {
      throw E(C("insecure-url"), "transport requires https (RFC 7030 sec. 3.3), got " + url.protocol);
    }
    ...
    if (url.protocol === "https:" && !hasAnchors && !useSystem) {
    ...
    var reqHeaders = Object.assign(Object.create(null), request.headers || {});
    ...
    if (bodyIsFn) {
      Object.keys(reqHeaders).forEach(function (k) { var lk = k.toLowerCase(); if (lk === "content-length" || lk === "transfer-encoding" || lk === "expect") delete reqHeaders[k]; });
    ...
    var plaintext = url.protocol === "http:";
    var options = {
      method: request.method || "GET",
      hostname: host,
      port: url.port || (plaintext ? 80 : 443),
    ...
    var proxyInfo;
    try { proxyInfo = _validateProxy(request, defaults, E, C, url); }
    catch (e) {
  ...
  var _transportFn = function transport(request) {
    ...
    return new Promise(function (resolve, reject) {
          ...
          if (typeof prep.body === "function") {
            req.once("socket", function (socket) {
              function _writeBoundBody() {
                ...
                Promise.resolve().then(function () { return prep.body(info); }).then(function (b) {
                  if (settled) return;
                  try {
                  ...
                  try {
                    if (b !== "") { req.setHeader("Content-Length", Buffer.byteLength(b)); req.write(b); }
                    req.end();
                  } catch (e2) { fail(C("transport-error"), "the request body could not be written: " + ((e2 && e2.message) || String(e2)), e2); }
      ...
      function issueConnect(credential) {
        var pheaders = Object.assign(Object.create(null), { Host: pinfo.originAuthority, Connection: "keep-alive" });
      ...
      function onProxyResponse(res, socket) {
        ...
        if (socket && sc >= 200 && sc < 300) {
          ...
          if (prep.plaintext) {
            issue(Object.assign({}, prep.options, { createConnection: function () { return socket; } }));
            return;
          }
