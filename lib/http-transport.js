...
function _validateProxy(request, defaults, E, C, url) {
  ...
  if (secure) {
    ...
    if (!pHasAnchors && !pUseSystem) throw E(C("no-trust-anchors"), "an https proxy requires explicit tls trust: set proxy.tls.anchors or proxy.tls.useSystemStore");
    var pMinVersion = (ptls && ptls.minVersion) || DEFAULT_MIN_VERSION;
    if (pMinVersion !== "TLSv1.2" && pMinVersion !== "TLSv1.3") throw E(C("bad-input"), "proxy.tls.minVersion must be 'TLSv1.2' or 'TLSv1.3', got " + pMinVersion);
    var pCa = [];
...
function _proxyErrorClass(e, C) {
  var s = (String((e && e.code) || "") + " " + String((e && e.message) || "")).toLowerCase();
  if (s.indexOf("cert") !== -1 || s.indexOf("verify") !== -1 || s.indexOf("altname") !== -1 || s.indexOf("hostname") !== -1 ||
      _hasSelfSigned(s) || _hasWordBounded(s, "ssl") || _hasWordBounded(s, "tls")) return C("proxy-tls-failed");
...
function httpsTransport(defaults) {
  ...
  var _transportFn = function transport(request) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var pheaders = { Host: pinfo.originAuthority };
      ...
      try {
        if (pinfo.secure) {
          ...
          var psni = pinfo.proxyTls.servername || pinfo.host;
          if (psni && !nodeNet.isIP(psni)) connectOpts.servername = psni;
      ...
      cr.on("error", function (e) {
        var code = (e && e.pkiBlockedAddress) ? C("blocked-address") : (pinfo.secure ? _proxyErrorClass(e, C) : C("proxy-connect-failed"));
        fail(code, "the CONNECT request failed: " + ((e && e.message) || String(e)), e);
      });
      cr.end();
    });
  };
