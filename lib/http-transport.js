...
var PROXY_KEYS = _keySet(["url", "auth"]);
...
function _validateProxy(request, defaults, E, C, url) {
  ...
  if (purl.protocol === "https:") throw E(C("proxy-unsupported-scheme"), "an https proxy (TLS to the proxy) is not supported in this release; use an http proxy URL");
  if (purl.protocol !== "http:") throw E(C("bad-proxy"), "opts.proxy.url must be an http URL, got " + purl.protocol);
  var auth = null;
  if (proxy.auth !== undefined && proxy.auth !== null) {
    ...
    if (scheme === "basic" && proxy.auth.username.indexOf(":") !== -1) throw E(C("bad-proxy"), "a Basic proxy user-id must not contain a colon (RFC 7617 sec. 2)");
    auth = { scheme: scheme, username: proxy.auth.username, password: proxy.auth.password, allowMD5: proxy.auth.allowMD5 === true, allowLegacyQop: proxy.auth.allowLegacyQop === true };
  }
  ...
  return { host: phost, port: purl.port ? +purl.port : 80, originHost: originHost, originAuthority: originAuthHost + ":" + (url.port || 443), auth: auth };
}

function _basicProxyHeader(auth) {
  return "Basic " + Buffer.from(auth.username + ":" + auth.password, "utf8").toString("base64");
}
function _digestProxyCodes(C) {
  var req = C("proxy-auth-required");
  return { unsupportedAlgorithm: req, weakAlgorithm: req, noQop: req, badChallenge: req };
}
function _proxyAuthHeader(proxyInfo, resHeaders, E, C) {
  var auth = proxyInfo.auth;
  if (auth.scheme === "basic") return _basicProxyHeader(auth);
  var httpDigest = require("./http-digest");   // allow:inline-require -- circular load http-transport -> http-digest -> schema-pkix -> ct -> http-transport
  var codes = _digestProxyCodes(C);
  var policy = { codes: codes, allowMD5: auth.allowMD5, allowLegacyQop: auth.allowLegacyQop };
  var challenge = httpDigest.parseChallenge((resHeaders || {})["proxy-authenticate"], E, C("proxy-auth-required"), policy);
  if (challenge === null) throw E(C("proxy-auth-required"), "the proxy required authentication but offered no Digest challenge this client supports");
  return httpDigest.answer(challenge, { method: "CONNECT", uri: proxyInfo.originAuthority, username: auth.username, password: auth.password, policy: policy }, E);
}
 ...
 *   - `blockPrivateAddresses` -- boolean; when true, an IP-literal host OR a hostname resolving to a private / loopback / link-local address is refused (`transport/blocked-address`), and a resolved address is pinned for the connection. For fetching an untrusted-certificate URL (AIA caIssuers); default false. With a proxy the block applies to the proxy address, since the origin is resolved by the proxy.
 *   - `proxy` -- reach the https origin through a forward HTTP proxy: `{ url: "http://proxy:3128", auth?: { scheme: "basic" | "digest", username, password, allowMD5?, allowLegacyQop? } }`. A CONNECT tunnel is opened to the proxy and the origin's TLS is negotiated inside it under the identical trust policy, so the proxy never sees the origin traffic and cannot substitute the origin certificate. A `407` is answered with `Proxy-Authorization` (`transport/proxy-auth-required` when no usable credentials, `transport/proxy-auth-failed` when the proxy rejects them); a non-2xx CONNECT is `transport/proxy-connect-failed`; a malformed option is `transport/bad-proxy`; an https proxy URL is `transport/proxy-unsupported-scheme`. Off by default (a direct connection).
 * @example
...
function httpsTransport(defaults) {
  ...
  function _prepare(request) {
    ...
    if (blockPrivate) {
      if (proxyInfo) {
        if (_isBlockedIp(proxyInfo.host)) throw E(C("blocked-address"), "refusing to connect to the private / loopback / link-local proxy address literal " + proxyInfo.host);
      } else {
    ...
    return { options: options, timeout: timeout, maxBytes: maxBytes, body: body, proxy: proxyInfo, blockPrivate: blockPrivate };
  }
  ...
  var _transportFn = function transport(request) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      function failWith(err) { _settleReject(err); }
      ...
      function establishTunnel(authHeader, isRetry) {
        if (settled) return;
        var headers = { Host: pinfo.originAuthority };
        if (authHeader) headers["Proxy-Authorization"] = authHeader;
        var connectOpts = { host: pinfo.host, port: pinfo.port, method: "CONNECT", path: pinfo.originAuthority, headers: headers, agent: false };
        if (prep.blockPrivate) connectOpts.lookup = _guardedLookup;
        var cr;
        try { cr = nodeHttp.request(connectOpts); }
        catch (e) { fail(C("proxy-connect-failed"), "the CONNECT request could not be initiated: " + ((e && e.message) || String(e)), e); return; }
        creq = cr;
        var done = false;
        function onResponse(res, socket) {
            ...
            issue(Object.assign({}, prep.options, {
              createConnection: function () {
                return nodeTls.connect({
                  ...
                  rejectUnauthorized: true,
                  checkServerIdentity: prep.options.checkServerIdentity,
          ...
          if (sc === 407 && !isRetry && pinfo.auth) {
            var next;
            try { next = _proxyAuthHeader(pinfo, res.headers, E, C); }
            catch (e) { failWith(e); return; }
            establishTunnel(next, true);
            return;
          }
          if (sc === 407) { fail(C(pinfo.auth ? "proxy-auth-failed" : "proxy-auth-required"), pinfo.auth ? "the proxy rejected the supplied credentials (HTTP 407)" : "the proxy required authentication but no credentials were supplied (HTTP 407)"); return; }
          fail(C("proxy-connect-failed"), "the proxy answered CONNECT with HTTP " + sc);
        }
        cr.on("connect", function (res, socket) { onResponse(res, socket); });
        ...
        cr.on("error", function (e) { fail(C("proxy-connect-failed"), "the CONNECT request failed: " + ((e && e.message) || String(e)), e); });
        cr.end();
      }
      var initialAuth = (pinfo.auth && pinfo.auth.scheme === "basic") ? _basicProxyHeader(pinfo.auth) : null;
      establishTunnel(initialAuth, !!(pinfo.auth && pinfo.auth.scheme === "basic"));
    });
  };
