      ...
      function issue(options) {
        try {
          req = nodeHttps.request(options, function (res) {
            ...
            var _reused = res.socket && res.socket.isSessionReused ? res.socket.isSessionReused() : false;
            var tlsUnique = (res.socket && res.socket.getFinished && res.socket.getPeerFinished && proto === "TLSv1.2")
              ? (_reused ? res.socket.getPeerFinished() : res.socket.getFinished()) : null;
            ...
            res.on("end", function () {
              ...
              resolve({
                ...
                tls: { protocol: proto, cipher: cipher, peerCertificate: peer && peer.raw ? peer.raw : null, tlsUnique: tlsUnique },
              });
            });
