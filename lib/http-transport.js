 ...
 *   channel, `{ protocol, cipher, peerCertificate, tlsUnique }`, facts a caller cannot
 *   recover from the response bytes. `tlsUnique` is the RFC 5929 channel binding (the first
 *   TLS Finished) on a TLS 1.2 connection, which `pki.est.challengePasswordFromTlsUnique`
 *   base64-encodes into an EST challengePassword (RFC 7030 sec. 3.5); it is `null` on TLS 1.3,
 *   where RFC 5929 defines no tls-unique. An INJECTED substitute should return these too:
 ...
 *   connection the request is sent over. The callback returns bytes or a string; a throw
 *   or a non-bytes, non-string return fails the request closed.
 *
...
function _tlsInfo(socket) {
  ...
  return { protocol: proto, cipher: cipher, peerCertificate: peer && peer.raw ? peer.raw : null, tlsUnique: tlsUnique };
}
  ...
  function _prepare(request) {
    ...
    try { proxyInfo = _validateProxy(request, defaults, E, C, url); }
      ...
      try {
        ...
        typedProxyError = ec === C("bad-proxy") || ec === C("proxy-auth-requires-tls") || ec === C("proxy-unsupported-scheme");
          ...
          if (typeof prep.body === "function") {
            req.once("socket", function (socket) {
              function _writeBoundBody() {
                if (settled) return;
                var b;
                try { b = prep.body(_tlsInfo(socket)); }
                catch (e) { fail(C("transport-error"), "the channel-binding body callback threw", e); return; }
                try {
                  if (guard.bytes.isByteSource(b)) b = guard.bytes.source(b, E, C("bad-input"), "the channel-binding body");
                  else if (b != null && b !== "" && typeof b !== "string") { fail(C("bad-input"), "the request body callback must return bytes, a string, null, or an empty string"); return; }
                  if (b != null && b !== "") { req.setHeader("Content-Length", Buffer.byteLength(b)); req.write(b); }
                  req.end();
