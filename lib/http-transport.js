...
function _isBlockedIp(ip) {
  ...
  if (fam === 6) {
    ...
    if (l === "::1" || l === "::" || l.indexOf("::ffff:") === 0) return true;   // loopback / unspecified / IPv4-mapped
    var h = parseInt(l.split(":")[0], 16);                                      // first hextet (leading "::" handled above)
    return h >= 0xfc00 && h <= 0xfeff;                                          // fc00::/7 ULA + fe80::/10 link-local + fec0::/10 deprecated site-local (RFC 3879) + the reserved fe00::/9 gaps -- none are global unicast (2000::/3), so blocking the whole span is fail-closed
  }
  return false;
}
 ...
 *   - `blockPrivateAddresses` -- boolean; when true, a hostname resolving to a private / loopback / link-local address is refused at resolution time (`transport/blocked-address`) and the checked address is pinned for the connection. For fetching an untrusted-certificate URL (AIA caIssuers); default false.
 * @example
...
function httpsTransport(defaults) {
  ...
  function _prepare(request) {
    ...
    if (blockPrivate) options.lookup = _guardedLookup;
...
module.exports = { https: httpsTransport, isBlockedIp: _isBlockedIp, _makeGuardedLookup: _makeGuardedLookup };
