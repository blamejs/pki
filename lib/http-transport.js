...
function _pemifyAnchors(anchors) {
  var list = _isArray(anchors) ? anchors.map(_pemifyAnchor) : [_pemifyAnchor(anchors)];
  return list;
}
...
function _systemCa() {
  ...
  if (typeof nodeTls.getCACertificates === "function") {
    ["system", "bundled"].forEach(function (t) {
      ...
      try { var c = nodeTls.getCACertificates(t); if (_isArray(c)) out = out.concat(c); } catch (_e) { /* unsupported type */ }
    });
  }
  _systemCaCache = out;
  return out;
...
function _isBlockedIp(ip) {
  ...
  if (fam === 6) {
    ...
    var parts = l.split(":");
    var h = _parseInt(parts[0], 16);                                         // first hextet ("" for a leading "::" -> NaN -> not in 2000::/3 -> blocked)
    ...
    var h2 = parts[1] ? _parseInt(parts[1], 16) : 0;
...
function _makeGuardedLookup(lookupFn) {
  return function guardedLookup(hostname, options, callback) {
    lookupFn(hostname, options || {}, function (err, address, family) {
      ...
      if (_isArray(address)) {   // options.all -> [{ address, family }, ...]; reject if ANY resolved address is blocked
...
function _classifyError(e, C) {
  ...
  var s = _String((e && e.code) || "") + " " + _String((e && e.message) || "");
...
function httpsTransport(defaults) {
  ...
  function _prepare(request) {
    var url;
    try { url = new URL(_String(request.url)); }
    catch (e) { throw E(C("bad-url"), "the request URL did not parse: " + _String(request.url), e); }
    ...
    if (_isBuffer(body)) body = guard.bytes.view(body, E, C("bad-input"), "the request body");
    ...
    var reqHeaders = _assign({}, request.headers || {});
    if (body != null && body !== "") {
      ...
      _keys(reqHeaders).forEach(function (k) { var lk = k.toLowerCase(); if (lk === "content-length" || lk === "transfer-encoding") delete reqHeaders[k]; });
    ...
    return new Promise(function (resolve, reject) {
      ...
      try {
        req = nodeHttps.request(prep.options, function (res) {
          ...
          var declared = _parseInt((res.headers || {})["content-length"], 10);
          ...
          res.on("data", function (chunk) {
            ...
            if (len + chunk.length > buf.length) {
              var want = _min(prep.maxBytes, _max(buf.length ? buf.length * 2 : 8192, len + chunk.length));
          ...
          res.on("end", function () {
            ...
            clearTimer();
            var lower = {};
            _keys(res.headers || {}).forEach(function (k) { lower[k.toLowerCase()] = res.headers[k]; });
            resolve({
              status: res.statusCode,
              headers: lower,
              body: _bufferFrom(buf.subarray(0, len)),
        ...
        req.on("error", function (e) { fail(_classifyError(e, C), "the request failed: " + ((e && e.message) || _String(e)), e); });
        ...
        req.end();
      } catch (e) { fail(C("transport-error"), "the request could not be initiated: " + ((e && e.message) || _String(e)), e); }
    });
  };
