  ...
  if (secure) {
    ...
    if (pHasAnchors) {
      ...
      for (var pai = 0; pai < pAnchorList.length; pai++) {
        ...
        if (typeof pan !== "string" && !(pan != null && typeof pan === "object" && ArrayBuffer.isView(pan))) throw E(C("bad-proxy"), "opts.proxy.tls.anchors must be a PEM string or a DER byte source (or an array of them)");
      }
    }
