...
// The JS regex \s code points representable in a latin1 string (0..255): TAB LF VT FF CR SP NBSP.
// Built at load so the source stays pure ASCII. The armor scan runs over a latin1 prefix, where these
// are the only \s characters that can appear, so this is exactly /^\s*/ for that input.
var _LATIN1_WS = String.fromCharCode(0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x20, 0xa0);

// PEM armor at the START after optional whitespace, decided by character scan (no regex): skip the
// leading \s run, then require "-----BEGIN ".
function _looksLikePemArmor(s) {
  var i = 0;
  while (i < s.length && _LATIN1_WS.indexOf(s.charAt(i)) !== -1) i += 1;
  return s.slice(i, i + 11) === "-----BEGIN ";
}
// Base64 wrapped to <=64-column lines, each terminated by LF (matching /.{1,64}/g -> "$&\n", which
// keeps a trailing LF on the final line).
function _wrap64Lines(s) {
  var out = "";
  for (var i = 0; i < s.length; i += 64) out += s.slice(i, i + 64) + "\n";
  return out;
}
// The surrounding "[" "]" of an IPv6 literal removed, else the string unchanged (matching /^\[(.*)\]$/).
...
// SELF.?SIGNED : "self", an optional single non-line-terminator character, then "signed" (in a lowercased string).
...
// \bWORD\b : `word` (lowercase) bounded on both sides by a non-word character ([a-z0-9_]) or a string edge.
...
function _classifyError(e, C) {
  ...
  var l = s.toLowerCase();
  // /PROTOCOL_VERSION|UNSUPPORTED_PROTOCOL|VERSION_TOO_LOW|WRONG_VERSION|NO_PROTOCOLS_AVAILABLE|INAPPROPRIATE_FALLBACK/i
  ...
  // /CERT|SELF.?SIGNED|VERIFY|ALTNAME|HOSTNAME|DEPTH_ZERO|LOCAL_ISSUER|HANDSHAKE|\bSSL\b|\bTLS\b/i
...
function httpsTransport(defaults) {
  ...
  function _prepare(request) {
    ...
    // The per-request servername falls back to the factory default only when it is ABSENT
    // (as cert/key do); an explicit value, INCLUDING null, overrides the default, so a caller
    // that sets servername to null on a cross-origin hop suppresses a configured default SNI
    // rather than falsy-coalescing back to it.
