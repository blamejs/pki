...
var RFC5280_RULES = [
  ...
  // a linter is for, so each rule carries the severity its normative word does.
  {
    ...
    check: function (cert, ctx) {
      var over = _policyDisplayTexts(ctx).filter(function (d) { return d.chars > pkix.DISPLAY_TEXT_MAX; });
  ...
  },
  {
    ...
    appliesTo: _hasPolicyDisplayText,
    check: function (cert, ctx) {
      var bad = _policyDisplayTexts(ctx).filter(function (d) { return d.field === "explicitText" && _hasControlChar(d.text); });
  ...
  },
  {
    ...
    check: function (cert, ctx) {
      ...
      var bad = _policyDisplayTexts(ctx).filter(function (d) {
        return d.field === "explicitText" && d.tagNumber === _T_UTF8 && d.text.normalize("NFC") !== d.text;
      });
      return bad.length ? { context: { count: bad.length } } : null;
    },
  },
