...
// A claimed-parsed certificate is COMPLETE or it is not one. Every lint rule reads a sibling field
// off this object and none re-derives it from tbsBytes, so a partial object produces a report about
// fields that were never there -- and a lint report reading "clean" is the answer an operator acts
// on. Uses the shared predicate rather than the throwing door, because _ingest's contract is to
// return a finding for bad DATA and reserve throws for a wrong-TYPE argument.
function _looksParsed(o) {
  return o && typeof o === "object" && !Buffer.isBuffer(o) && guard.parsed.isCert(o);
}
