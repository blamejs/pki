 ...
 * whose top bit is set, so that octet is encoding overhead rather than part of the number, and the
 * 20-octet ceilings RFC 5280 states bound the number. Every rule that measures one against a
 * ceiling reads it here, so no two of them can answer differently for one value. */
...
var RFC5280_CRL_RULES = [
  ...
  {
    id: "lint/rfc5280-crl/idp-name-relative-to-crl-issuer", severity: "warn", source: "rfc5280-crl", citation: "RFC 5280 5.2.5 / 4.2.1.13",
    /** @internal Section 5.2.5 gives distributionPoint the syntax and semantics section 4.2.1.13
     * states, and there naming a distribution point relative to the CRL issuer is a SHOULD NOT, so
     * this is a warning rather than an error. The MUST NOT stated beside it is conditioned on
     * cRLIssuer carrying more than one name, and an IssuingDistributionPoint has no cRLIssuer
     * field, so that sentence cannot reach a CRL. */
    ...
    check: function (crl) {
      ...
      if (!Buffer.isBuffer(e.value)) return null;
      var idp = pkix.decodeIssuingDistributionPointValue(NS, e.value, _LINT_VALUE_CODE);
      if (idp.malformed || !idp.distributionPoint) return null;
      return idp.distributionPoint.kind === "rdn" ? { context: { form: "nameRelativeToCRLIssuer" } } : null;
    },
  },
  {
 ...
 *
 * Four section 5 rules never appear here because the strict parser refuses them first, and a rule
 * for them could not fire. A CRL carrying extensions without the version that admits them
 * (section 5.1.2.1), an empty issuer name (section 5.1.2.3), a present-but-empty
 * `revokedCertificates` (section 5.1.2.6), and an extension repeated on the CRL or on an entry
 * (section 4.2) each arrive as `lint/unparseable` carrying the parser's own code.
 *
