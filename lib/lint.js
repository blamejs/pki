...
/** @internal Whether the CRL declares itself indirect, read from the same IssuingDistributionPoint
 * decoder certification-path validation uses. A malformed IDP is not a declaration of anything, so
 * it reads as direct and the value-syntax row reports the malformation separately. */
function _crlIsIndirect(crl) {
  var e = _crlExtByName(crl, "issuingDistributionPoint");
  if (!e || !Buffer.isBuffer(e.value)) return false;
  var idp = path.decodeIdp({ value: e.value });
  return !idp.malformed && idp.indirect === true;
}
...
function _integerOctets(v) {
  ...
  try { return asn1.decode(asn1.build.integer(v)).content.length; }
  catch (_e) { return null; }
}
...
CRL_VALUE_READER[oid.byName("issuingDistributionPoint")] = function (bytes) {
  if (path.decodeIdp({ value: bytes }).malformed) throw _cfg(_LINT_VALUE_CODE, "malformed IssuingDistributionPoint");
};
...
function _valueSyntaxFault(e, readerTable) {
  var dec = EXT_DECODERS[e.oid];
...
var RFC5280_CRL_RULES = [
  ...
  },
  {
    ...
    check: function (crl) {
      ...
      var octets = e ? _integerOctets(e.value) : null;
      return (octets !== null && octets > 20) ? { context: { octets: octets } } : null;
    },
  },
  {
  ...
  },
  {
     ...
     * from the one here. */
    appliesTo: function (crl) { return !_crlIsIndirect(crl); },
  ...
  },
  {
    ...
    check: function (crl) {
      var idp = path.decodeIdp({ value: _crlExtByName(crl, "issuingDistributionPoint").value });
      if (idp.malformed) return null;
      var out = [];
      if (!idp.hasDistributionPoint && !idp.onlyUser && !idp.onlyCa && !idp.onlySomeReasons && !idp.indirect && !idp.onlyAttr) {
  ...
  },
  {
    ...
    check: function (crl) {
      ...
      _crlExtList(crl).forEach(function (e) {
        var why = _valueSyntaxFault(e, CRL_VALUE_READER);
        if (why) out.push({ context: { extension: e.name || e.oid, scope: "crlExtensions", reason: why } });
      });
      (crl.revokedCertificates || []).forEach(function (row, i) {
        (row.crlEntryExtensions || []).forEach(function (e) {
          var why = _valueSyntaxFault(e, CRL_ENTRY_VALUE_READER);
