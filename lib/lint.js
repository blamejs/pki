...
/** @internal The DER encoding of ASN.1 NULL, which RFC 5754 sec. 2 makes interchangeable with
 *  absent parameters on a digest AlgorithmIdentifier: "Implementations MUST accept SHA2
 *  AlgorithmIdentifiers with absent parameters. Implementations MUST accept SHA2
 *  AlgorithmIdentifiers with NULL parameters." */
var DER_NULL = Buffer.from([0x05, 0x00]);
function _isAbsentOrNullParams(p) {
  if (p === null || p === undefined) return true;
  return Buffer.isBuffer(p) && p.equals(DER_NULL);
}
/** @internal Whether two DIGEST AlgorithmIdentifiers name one algorithm. The parameters are read,
 *  because two identifiers carrying the same OID and different parameters are different
 *  algorithms, with the one exception RFC 5754 states: a digest identifier that omits its
 *  parameters and one that writes NULL name the same digest, and an implementation must accept
 *  both. Only digest identifiers pass through here, so no row is loosened for an algorithm whose
 *  parameters carry meaning. */
function _cmsSameAlgorithm(a, c) {
  if (!a || !c || a.oid !== c.oid) return false;
  var pa = a.parameters, pc = c.parameters;
  if (_isAbsentOrNullParams(pa)) return _isAbsentOrNullParams(pc);
  if (_isAbsentOrNullParams(pc)) return false;
  return Buffer.isBuffer(pa) && Buffer.isBuffer(pc) && pa.equals(pc);
}
...
/** @internal The criticality sec. 4.3 fixes for each extension it defines, each stated in that
 *  subsection as "criticality MUST be TRUE" or "MUST be FALSE". An extension outside this table is
 *  one the profile does not define, and sec. 4.2.9 answers for it: "If any other critical extension
 *  is used, the AC does not conform to this profile. However, if any other non-critical extension
 *  is used, the AC does conform." */
 ...
 * past 20 octets, a repeated attribute type, an empty attribute sequence, and a GeneralizedTime
 * carrying no seconds or a fractional part.
 *
 * The rows cover what survives that: the criticality section 4.3 fixes for each extension it
 * defines, a critical extension the profile does not define, a Holder naming more than one of its
 * three options, and an issuerUniqueID, which section 4.2.8 permits only where the issuer's own
 * certificate carries one and which is therefore a warning naming that condition.
 *
