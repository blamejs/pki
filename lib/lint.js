...
var RFC9935_RULES = [
  ...
  },
  {
    ...
    message: "an ML-KEM subjectPublicKey must be the raw encapsulation key at its exact FIPS 203 size for the OID",
    appliesTo: _isMlKem,
    check: function (cert) {
      var spki = cert.subjectPublicKeyInfo;
      var want = ML_KEM_EK_LEN[spki.algorithm.name];
      var got = spki.publicKey && spki.publicKey.bytes && spki.publicKey.bytes.length;
      return got === want ? null : { context: { expected: want, got: got == null ? null : got } };
    },
  },
];
