...
// The extension OIDs a v1 profile recognizes as legitimately critical (so an UNKNOWN
// critical extension is a real interop hazard). Resolved by name -- no dotted literal.
var _KNOWN_CRITICAL = ["basicConstraints", "keyUsage", "extKeyUsage", "subjectAltName", "issuerAltName",
  "nameConstraints", "policyConstraints", "inhibitAnyPolicy", "certificatePolicies", "precertificatePoison"];
function _knownCriticalOids() { var m = {}; _KNOWN_CRITICAL.forEach(function (n) { m[oid.byName(n)] = 1; }); return m; }
var KNOWN_CRITICAL_OIDS = _knownCriticalOids();
  ...
  },
  {
    ...
    check: function (cert) {
      ...
      (cert.extensions || []).forEach(function (e) {
        if (e.critical === true && !KNOWN_CRITICAL_OIDS[e.oid]) out.push({ context: { oid: e.oid, name: e.name || null } });
      });
...
var CABF_TLS_RULES = [
  ...
  },
  {
    ...
    check: function (cert) {
      ...
      try {
        if (name === "rsaEncryption") { var bits = _rsaModulusBits(spki); return (bits != null && bits < 2048) ? { context: { rsaBits: bits } } : null; }
