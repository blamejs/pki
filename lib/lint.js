 ...
 * re-read from the bytes it came from, and an object that merely carries a response's field names
 * is refused as config-time misuse rather than linted, since its fields could describe different
...
function _ingestOcsp(input) {
  ...
  if (Buffer.isBuffer(input)) der = input;
    ...
    return { response: guard.parsed.fromTrustedSource(input, "ocspResponse", _OCSP_CLAIM, function (src) {
      if (!Buffer.isBuffer(src)) throw _cfg("lint/bad-input", "pki.lint.ocsp input must be a parsed OCSP response, a DER Buffer, or a PEM string");
