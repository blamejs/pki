...
function _isSignedData(cms) { return Array.isArray(cms.signerInfos); }
...
function _cmsAllSignerInfos(cms) {
  ...
  function visit(signerInfos, signerIndex, isCounter, depth) {
    ...
    signerInfos.forEach(function (si, i) {
      ...
      (si.unsignedAttrs || []).forEach(function (attr) {
...
var RFC5652_CMS_RULES = [
  {
    ...
    check: function (cms) {
      ...
      _cmsAllSignerInfos(cms).forEach(function (entry) {
        ...
        if (!attr || !attr.values.length) return;
  ...
  },
  {
    ...
    check: function (cms) {
      ...
      var out = [];
      cms.signerInfos.forEach(function (si, index) {
        var mine = si.digestAlgorithm;
        if (!mine) return;
        var found = listed.some(function (alg) { return _cmsSameAlgorithm(alg, mine); });
        if (!found) out.push({ context: { signerIndex: index, digestAlgorithm: mine.name || mine.oid } });
      });
      return out.length ? out : null;
    },
  },
...
/** @internal The certificate the token's own SignerInfo identifies, which is "the certificate that
 *  is to be used to verify the token", or null when the token does not carry it. Carrying ONE
 *  certificate is not that fact: a token may embed an intermediate and leave the signer's
 *  certificate to be supplied out of band, and comparing a hint against an intermediate reports a
 *  conforming token. The identifier is the SignerInfo's own sid, an issuer and serial or a subject
 *  key identifier, which is exact. */
function _tspSignerCertificate(token) {
  var sid = token.signerInfo && token.signerInfo.sid;
  if (!sid) return null;
  var certs = Array.isArray(token.certificates) ? token.certificates : [];
  for (var i = 0; i < certs.length; i++) {
    var element = certs[i];
    if (!element || element.tagClass !== "universal" || !Buffer.isBuffer(element.bytes)) continue;
    var parsed;
    // allow:swallow-unverified a certificate this profile cannot read is not the one the sid names
    try { parsed = x509.parse(element.bytes); } catch (_e) { continue; }
    if (Buffer.isBuffer(sid.subjectKeyIdentifier)) {
      var ski = _decodeOrNull(parsed, "subjectKeyIdentifier");
      if (ski && Buffer.isBuffer(ski.value) && ski.value.equals(sid.subjectKeyIdentifier)) return parsed;
      continue;
    }
    if (sid.issuer && typeof sid.serialNumber === "bigint" &&
      parsed.serialNumber === sid.serialNumber && _sameName(parsed.issuer, sid.issuer)) {
      return parsed;
    }
  }
  return null;
...
var AC_FIXED_CRITICALITY = Object.create(null);
[["acAuditIdentity", true, "4.3.1"], ["targetInformation", true, "4.3.2"],
  ["authorityKeyIdentifier", false, "4.3.3"], ["authorityInfoAccess", false, "4.3.4"],
  ["cRLDistributionPoints", false, "4.3.5"], ["noRevAvail", false, "4.3.6"],
  /** @internal Sec. 7.2 defines ProxyInfo for an attribute certificate and fixes its criticality
   * the same way sec. 4.3 fixes the six above, so it belongs in this table and in the profile. */
  ["acProxying", true, "7.2"],
].forEach(function (row) {
  var dotted = oid.byName(row[0]);
  if (dotted) AC_FIXED_CRITICALITY[dotted] = { name: row[0], critical: row[1], clause: row[2] };
});
