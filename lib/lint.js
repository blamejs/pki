...
var VALID_SEVERITY = _keys(SEVERITY);
...
function _derivedCert(o) {
  if (!o || typeof o !== "object" || _isBuffer(o) || _isView(o) || o instanceof ArrayBuffer) return null;
  try {
...
function _hasEku(cert, ekuName) {
  ...
  return !!(d && _isArray(d.value) && d.value.indexOf(oid.byName(ekuName)) !== -1);
}
...
function _runLints(rules, cert, ctx) {
  ...
  rules.forEach(function (rule) {
    ...
    var details = _isArray(res) ? res : [res];
...
function _sanDnsNames(ctx) {
  ...
  if (!d || !d.value || !_isArray(d.value.names)) return [];
...
function _subjectCNs(cert) {
  ...
  ((cert.subject && cert.subject.rdns) || []).forEach(function (rdn) {
    rdn.forEach(function (a) { if (a.type === oid.byName("commonName") && typeof a.value === "string") _push(out, a.value); });
  });
  return out;
}
...
function _ecCurveName(spki) {
  try {
    ...
    if (!_isBuffer(params)) return null;
  ...
  },
  {
    ...
    check: function (cert) {
      ...
      (cert.extensions || []).forEach(function (e) {
        if (e.critical === true && _isUnknownExtension(e.oid)) _push(out, { context: { oid: e.oid, name: e.name || null } });
      });
    ...
    check: function (cert) {
      ...
      (cert.extensions || []).forEach(function (e) {
        ...
        try { dec(e.value); }
          ...
          _push(out, { context: { oid: e.oid, name: e.name || null, code: err.code } });
        }
      });
      return out.length ? out : null;
    },
...
var CABF_TLS_RULES = [
  ...
  },
  {
    ...
    check: function (cert, ctx) {
      ...
      _subjectCNs(cert).forEach(function (cn) {
        ...
        if (sans.indexOf(cn.toLowerCase()) === -1) _push(out, { context: { cn: cn } });
      });
      return out.length ? out : null;
    },
  },
  {
    ...
    check: function (cert, ctx) {
      var out = [];
      _sanDnsNames(ctx).forEach(function (name) { var p = _dnsNameProblem(name); if (p) _push(out, { context: { dnsName: name, problem: p } }); });
      return out.length ? out : null;
    },
  },
  {
...
function _selectRules(profile) {
  ...
  if (!PROFILES[profile]) throw _cfg("lint/unknown-profile", "unknown lint profile \"" + profile + "\" (known: " + _keys(PROFILES).join(", ") + ")");
 ...
 */
function profiles() { return _keys(PROFILES); }
