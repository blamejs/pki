...
// unknown everywhere (precertificatePoison is the one legitimately-critical exception); and an
// extension that IS processed for an intermediate but is unprocessed on the target/leaf
// (policyMappings) is unknown when this cert is a leaf. A CA cert is not a target, so its critical
// policyMappings -- processed by sec. 6.1.4 -- is not flagged.
function _isUnknownExtension(extOid, isTarget) {
  if (path.PROCESSED_EXTENSIONS[extOid] !== true) return _CRITICAL_LEGIT[extOid] !== true;
  return isTarget === true && path.TARGET_UNPROCESSED_IF_CRITICAL[extOid] === true;
}
...
var RFC5280_RULES = [
  ...
  },
  {
    ...
    check: function (cert, ctx) {
      // A cert with basicConstraints cA=TRUE is a CA (an intermediate/root in a path); anything else is
      // an end-entity, i.e. a path target -- the position path-validate applies its target exception at.
      var bc = ctx.decode("basicConstraints");
      var isTarget = !(bc && bc.value && bc.value.cA === true);
      var out = [];
      (cert.extensions || []).forEach(function (e) {
        if (e.critical === true && _isUnknownExtension(e.oid, isTarget)) out.push({ context: { oid: e.oid, name: e.name || null } });
      });
