...
[["code-signing", "code-signing"], ["code-signing-ev", "code-signing"],
  ["code-signing-timestamping", "timestamping"]].forEach(function (row) {
  var dotted = oid.byName(row[0]);
  if (dotted) CS_KIND_BY_POLICY[dotted] = row[1];
});
...
/** @internal The kind the asserted reserved identifier declares, or null where none is asserted. */
function _csKind(ctx) {
  var d = ctx.decode("certificatePolicies");
  if (!d || !Array.isArray(d.value)) return null;
  var found = null;
  for (var i = 0; i < d.value.length; i++) {
    var k = CS_KIND_BY_POLICY[d.value[i].policyIdentifier];
    if (k && found === null) found = k;
  }
  return found;
}
...
function _isCs(cert, ctx) {
  ...
  return _csKind(ctx) !== null && !ctx.isCaCert;
}
...
function _isCsTimestamp(cert, ctx) { return _isCs(cert, ctx) && _csKind(ctx) === "timestamping"; }
function _isCsCodeSigning(cert, ctx) { return _isCs(cert, ctx) && _csKind(ctx) !== "timestamping"; }
...
var CABF_CS_RULES = [
  ...
  },
  {
    ...
    check: function (cert, ctx) {
      var d = ctx.decode("extKeyUsage");
      if (!d || !Array.isArray(d.value)) return null;
      var required = _isCsTimestamp(cert, ctx) ? oid.byName("timeStamping") : oid.byName("codeSigning");
      var out = [];
      d.value.forEach(function (dotted) {
        if (dotted === required) return;
...
var ALL_RULES = RFC5280_RULES.concat(RFC9881_RULES).concat(RFC9909_RULES).concat(RFC9935_RULES)
  .concat(CABF_TLS_RULES).concat(CABF_SMIME_RULES).concat(CABF_CS_RULES);
