...
/** @internal Every policy qualifier of every PolicyInformation, as its qualifier OID. The bytes
 *  are re-decoded the way the display-text walk above re-decodes them. */
function _policyQualifierIds(ctx) {
  ...
  d.value.forEach(function (pi) {
    if (!pi.qualifiersBytes || !pi.qualifiersBytes.length) return;
    var quals;
    // allow:swallow-unverified bytes that already decoded under the policy-qualifier door cannot throw
    try { quals = asn1.decode(pi.qualifiersBytes).children; } catch (_e) { return; }
    (quals || []).forEach(function (pq) {
      // allow:swallow-unverified the same door already read this OID
      try { out.push(asn1.read.oid(pq.children[0])); } catch (_e2) { void 0; }
    });
  });
  return out;
}
...
/** @internal RFC 5280 sec. 4.2 fixes each extension's syntax wherever it is carried, and a
 *  requested extension whose value does not decode is the same fault in a request as in a
 *  certificate. The certificate's own row reads `extensions` and the request view supplies it, so
 *  the row is reused rather than restated. Without it a malformed requested value counted as a
 *  passing check, because the shared decoder returns null and every row that asks by name reads
 *  that null as absence. */
var RFC5280_CSR_RULES = RFC5280_RULES.filter(function (r) {
  return r.id === "lint/rfc5280/extension-undecodable";
});
var CSR_STRUCTURAL_RULES = RFC2986_CSR_RULES.concat(RFC5280_CSR_RULES);
