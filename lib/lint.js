 ...
 *   the post-quantum certificate profiles for ML-KEM, ML-DSA and SLH-DSA, and a
 *   representative CA/Browser Forum TLS Baseline Requirements subset.
 *
...
/** @internal The children of an AuthorityKeyIdentifier value. Bytes that do not decode carry no
 * keyIdentifier child, so the caller's search finds none and reports the fault; nothing here
 * returns a verdict. */
function _akiChildren(bytes) {
  // allow:swallow-unverified a value that does not decode holds no keyIdentifier for the caller to find
  try { return asn1.decode(bytes).children || []; }
  catch (_e) { return []; }
}
...
var RFC5280_CRL_RULES = [
  ...
  {
    id: "lint/rfc5280-crl/crl-number-too-long", severity: "error", source: "rfc5280-crl", citation: "RFC 5280 5.2.3",
    message: "a cRLNumber must not be longer than 20 octets",
    check: function (crl) {
      var e = _crlExtByName(crl, "cRLNumber");
      /** @internal The CRL parser decodes this extension to its integer value, so the octet count
       * the clause bounds is measured by re-encoding it. A non-minimal encoding cannot reach here,
       * since strict DER refuses one at parse, so the re-encoded length is the length on the wire. */
      if (!e || typeof e.value !== "bigint") return null;
      var octets;
      // allow:swallow-unverified a value the codec cannot re-encode is not one this row can size
      try { octets = asn1.decode(asn1.build.integer(e.value)).content.length; }
      catch (_e) { return null; }
      return octets > 20 ? { context: { octets: octets } } : null;
    },
  },
  {
    id: "lint/rfc5280-crl/aki-missing", severity: "warn", source: "rfc5280-crl", citation: "RFC 5280 5.2.1",
  ...
  {
    id: "lint/rfc5280-crl/aki-without-key-identifier", severity: "warn", source: "rfc5280-crl", citation: "RFC 5280 5.2.1",
    message: "a CRL's authorityKeyIdentifier must use the key identifier method",
    /** @internal Section 5.2.1 states two requirements: include the extension, and use the key
     * identifier method. The row above answers the first. This answers the second, which an
     * extension present but carrying no keyIdentifier fails. The CRL parser leaves this value as
     * opaque bytes, so the keyIdentifier [0] child is looked for here. */
    ...
    check: function (crl) {
      ...
      var kids = _akiChildren(e.value);
      for (var i = 0; i < kids.length; i++) {
        if (kids[i].tagClass === "context" && kids[i].tagNumber === 0) return null;
      }
      return true;
    },
  },
  ...
  },
  {
    ...
    check: function (crl) {
      ...
      var rows = [];
      (crl.revokedCertificates || []).forEach(function (r) {
 ...
 *
 * The rows cover the fields (version against the extensions present, nextUpdate presence, update
 * ordering, a non-empty issuer) and the extensions (cRLNumber presence and length,
 * authorityKeyIdentifier presence, the criticality every profiled extension is required to carry,
 * freshestCRL in a delta CRL, and an unrecognized critical extension), for the CRL and for each
 * revoked entry.
 *
 * Some section 5 rules never appear here because the strict parser refuses them first, and a rule
 * for them could not fire: a present-but-empty `revokedCertificates` (section 5.1.2.6) arrives as
 * `lint/unparseable` carrying `crl/bad-revoked-certificates`.
 *
 ...
 *   report.worst;                                       // -> "warn", for the missing AKI
