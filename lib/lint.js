...
function _dnsNameProblem(s) {
  ...
  for (var i = 0; i < labels.length; i++) {
    ...
    // A leftmost "*" wildcard label is permitted; every other label must be LDH (letters,
    // digits, hyphen) per RFC 1034 preferred name syntax -- so a dNSName carrying any other
    // character (an addressing/injection character like "$", "@", "/") is malformed.
    if (i === 0 && label === "*") continue;
    if (!/^[a-z0-9-]+$/i.test(label)) return "invalid character in label";
  }
  return null;
}
