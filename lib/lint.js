...
var RFC5280_RULES = [
  ...
  {
    // 4.2.1.3: "When present, conforming CAs SHOULD mark this extension as critical." A SHOULD
    // (warn), scoped to CA certificates -- the recommendation names conforming CAs, and a
    // non-critical keyUsage is common and accepted in end-entity certificates, so gating on the
    // certificate's CA-ness avoids a spurious warning on ordinary leaf certificates.
    id: "lint/rfc5280/key-usage-not-critical", severity: "warn", source: "rfc5280", citation: "RFC 5280 4.2.1.3",
    message: "a CA certificate's keyUsage extension should be marked critical",
    appliesTo: function (cert, ctx) { var bc = ctx.decode("basicConstraints"); return !!(ctx.raw("keyUsage") && bc && bc.value && bc.value.cA === true); },
    check: function (cert, ctx) { var e = ctx.raw("keyUsage"); return (e && e.critical !== true) ? true : null; },
  },
  {
