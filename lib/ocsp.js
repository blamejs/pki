...
function verify(response, opts) {
  ...
  return pathValidate.verifyOcspResponse(parsed, cert, issuerCert, time, { historicalMode: opts.historicalMode === true }).then(function (verdict) {
    ...
    if (opts.requestNonce == null) {
      out = Object.assign({}, verdict, { nonceMatched: null });
    } else {
      ...
      out = Object.assign({}, verdict, { nonceMatched: matched });
      if (!matched && verdict.status === "good") {
        out = Object.assign(out, { status: "unknown", reason: "the OCSP response nonce does not echo the request nonce (RFC 9654)" });
      }
    }
    /** @internal Defined, not assigned: an assignment runs an inherited setter, so a prototype
     * polluted while this verification was pending could swallow the write and answer its getter. */
    Object.defineProperty(out, "valid",{ value: out.status === "good", enumerable: true, configurable: true, writable: true });
    return out;
  });
}
