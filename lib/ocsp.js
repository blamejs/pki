...
// position-dependent pick: the responder path-validates them to select a usable one, and no valid
// match is hidden behind an unusable one that happened to appear first. `strict` separates the
...
async function _findRequestSigners(candidates, strict, parsed, sig, sigBits) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    var cert;
    try { cert = x509.parse(candidates[i]); }
    catch (e) {
      if (strict) throw _err("ocsp/bad-signer-cert", "an opts.certs entry does not parse as an X.509 certificate", e);
      continue;
    }
    if (await ocspRequestVerify.verifyRequestSignature(sig.signatureAlgorithm, sigBits, cert.subjectPublicKeyInfo.bytes, parsed.tbsRequestBytes)) {
      matches.push({ der: candidates[i], subject: cert.subject });
    }
  }
  return matches;
}
 ...
 * key may appear under an expired certificate beside its renewal), so the responder path-validates
 * them to pick a usable one rather than depend on ordering. `signerCert` and the decoded
 * `signerSubject` are the first of those, the common single-signer case. `certs` is every certificate
 * the request carried (or opts.certs supplied), including intermediates that do not sign and so are
 * absent from `signerCerts`: the responder passes it as the candidate pool when it runs
 * `pki.path.validate` on a `signerCerts` entry, then compares that certificate's subject to the
 * `requestorName` it expects. The decoded `requestList`, `requestExtensions` and `version` are
 ...
 *   // v.signed && v.signatureValid -> pick a path-valid certificate from v.signerCerts (several may
 *   // share the signing key, e.g. an expired one beside its renewal), then bind that certificate's
 *   // subject to the requestor you expect before honoring v.requestList.
 */
