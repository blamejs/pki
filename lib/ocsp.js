...
async function _findRequestSigners(candidates, strict, parsed, sig, sigBits) {
  ...
  // candidate that verifies: every further match shares that key's SPKI and is found by byte
  // comparison below, not another verify. This bounds the crypto an untrusted request can demand to
  // the position of the signer (the certificate count is already capped at OCSP_MAX_CERTS by the
  // parser), rather than verifying every embedded certificate even after the signer is found.
  var signerSpki = null;
  for (var j = 0; j < parsedList.length; j++) {
    if (await ocspRequestVerify.verifyRequestSignature(sig.signatureAlgorithm, sigBits, parsedList[j].cert.subjectPublicKeyInfo.bytes, parsed.tbsRequestBytes)) {
      signerSpki = parsedList[j].cert.subjectPublicKeyInfo.bytes;
      break;
    }
  }
  if (signerSpki === null) return [];
  // signerCerts is every candidate carrying the verified signing key, in the order the request
  // listed them, so a usable certificate (e.g. a renewal beside an expired one) is not hidden.
  var matches = [];
  for (var k = 0; k < parsedList.length; k++) {
    // constantTimeEqual over the SPKI bytes (public, so timing is not the point): it is the captured
    ...
    if (guard.crypto.constantTimeEqual(parsedList[k].cert.subjectPublicKeyInfo.bytes, signerSpki)) {
      matches.push({ der: parsedList[k].der, subject: parsedList[k].cert.subject });
    }
  }
  return matches;
}
// The verdict from the collected signature-matching certificates: signerCerts is every match (so
// the caller's trust decision does not depend on ordering), signerCert / signerSubject the first for
// the common single-signer case. `candidates` is the FULL set the request carried (or opts.certs
// supplied), surfaced as `certs` so the responder has the intermediate certificates it needs to
// chain a signer to an anchor -- those do not sign the request, so they never appear in signerCerts.
// No match is signatureValid:false with the failure reason, still surfacing certs for context.
function _signerVerdict(parsed, matches, candidates, failReason) {
  if (!matches.length) return _reqVerdict(parsed, { signed: true, signatureValid: false, certs: candidates, reason: failReason });
  return _reqVerdict(parsed, {
    signed: true, signatureValid: true, signerCert: matches[0].der,
    signerCerts: matches.map(function (m) { return m.der; }), certs: candidates,
    signerSubject: { dn: matches[0].subject.dn, rdns: matches[0].subject.rdns }, reason: null,
  });
}
 ...
 * are the first of those, the common single-signer case. `certs` is every certificate the request
 * carried (or opts.certs supplied), including intermediates that do not sign and so are absent from
 * `signerCerts`: the responder passes it as the `opts.candidates` pool to `pki.path.build`, which
 * discovers and validates the ordered path from a `signerCerts` entry (leaf) up to a trust anchor,
 * then compares that certificate's subject to the `requestorName` it expects. The decoded
 ...
 *   // the key, e.g. an expired one beside its renewal); then bind that certificate's subject to the
 *   // requestor you expect before honoring v.requestList.
 */
async function verifyRequest(request, opts) {
  ...
  if (sig.certs.length) {
    return _signerVerdict(parsed, await _findRequestSigners(sig.certs, false, parsed, sig, sigBits), sig.certs,
  ...
  return _signerVerdict(parsed, await _findRequestSigners(suppliedCerts, true, parsed, sig, sigBits), suppliedCerts,
