...
/** @internal Only the signature self-check is taken from the shared builder here; the name and
 * SPKI schemas it also serves belong to the certificate verbs, and this module reads neither. */
var _b = pkiBuild.makeBuilder({ ErrorClass: OcspError, prefix: "ocsp", O: O, NS: null, NAME_SCHEMA: null, SPKI_SCHEMA: null, EXT_DECODERS: {} });
...
function _buildRequest(query, opts) {
  ...
  return Promise.all(queries.map(function (q) {
    ...
    return signScheme.signOverTbs(scheme, opts.signer.key, tbsRequest, _signE).then(function (sig) {
      /** @internal The request is held to the same proof a certificate is: the signature verifies
       * against the public key the certificate it carries declares. */
      return Promise.resolve(_b.assertSignatureVerifies(tbsRequest, sig, signerCert.subjectPublicKeyInfo.bytes, scheme)).then(function () {
  ...
  return _responderID(responseData.responderID, respCert).then(function (ridNode) {
    return Promise.all(responses.map(function (r) { return _buildSingleResponse(r, opts); })).then(function (built) {
      ...
      return signScheme.signOverTbs(scheme, responderKey, responseDataDer, _signE).then(function (sig) {
        /** @internal The response is held to the same proof a certificate is: the signature verifies
         * against the public key the responder certificate declares. */
        return Promise.resolve(_b.assertSignatureVerifies(responseDataDer, sig, respCert.subjectPublicKeyInfo.bytes, scheme)).then(function () {
...
function _snapshotSignerKey(key, owned) {
  ...
  if (key && typeof key === "object" && (key.mldsa != null || key.trad != null)) {
    var ks = guard.identifier.ownOptions(key);
    ks.mldsa = _snapshotSignerKey(key.mldsa, owned);
    ks.trad = _snapshotSignerKey(key.trad, owned);
    return ks;
  }
  return key;
}
...
function verify(response, opts) {
  ...
  /** @internal The nonce rule is applied by the acceptance gate rather than restated here, so this
   * door and every other consumer of that gate answer alike. The bytes are snapshotted here so a
   * fault in them carries this door's own code; a value that is not bytes is passed through for the
   * gate to fail closed on, which is what a caller asking for a nonce with something that cannot be
   * one has earned. */
  var reqNonce = opts.requestNonce;
  if (guard.bytes.isByteSource(reqNonce)) {
    try { reqNonce = guard.bytes.snapshotSource(reqNonce, OcspError, "ocsp/bad-input", "opts.requestNonce"); }
    catch (e) { return Promise.reject(e); }
  }
  return pathValidate.verifyOcspResponse(parsed, cert, issuerCert, time,
    { historicalMode: opts.historicalMode === true, requestNonce: reqNonce }).then(function (verdict) {
    return guard.verdict.of(verdict, { valid: verdict.status === "good" });
  });
}
...
/** @internal RFC 5019 sec. 5 measures the GET decision over "the scheme and delimiters (http://),
 * server name and base64-encoded OCSPRequest structure", which is the whole encoded URL. */
var HTTP_GET_MAX_URL_BYTES = 255;

/**
 * @primitive  pki.ocsp.httpRequest
 * @signature  pki.ocsp.httpRequest(der, responderUrl, opts?) -> { method, url, headers, body }
 * @since      0.8.8
 * @status     stable
 * @spec       RFC 6960, RFC 5019
 * @related    pki.ocsp.buildRequest, pki.path.fetchingChecker
 *
 * Shape a DER `OCSPRequest` into the HTTP request RFC 6960 Appendix A.1 defines, without
 * opening a socket: the result is the method, the URL, the headers and the body a caller
 * hands to its own transport.
 *
 * A GET carries the request in the URL, as the base64 of the DER percent-encoded into one
 * path segment appended to the responder URI. RFC 5019 sec. 5 requires GET when the whole
 * encoded URL is 255 bytes or fewer, counting the scheme, the delimiters, the server name and
 * the encoded request, and that is what is measured; anything longer is a POST carrying
 * `Content-Type: application/ocsp-request` and the DER as its body. The base64 carries no CR
 * and no LF, which that section also requires.
 *
 * The encoding escapes every byte outside RFC 3986 sec. 2.3's unreserved set, so the `+`, `/`
 * and `=` of base64 become triplets and a request cannot reach past the segment it was placed
 * in. A responder URL carrying a query or a fragment is refused, since a path segment appended
 * after either would not be part of the path.
 *
 * @opts  method  `"GET"` or `"POST"` to override the sec. 5 measurement. A GET asked for on a
 *                request too long to carry that way is refused rather than truncated.
 * @example
 *   var ca = await pki.key.generate("Ed25519");
 *   var caDer = await pki.x509.sign({ subject: "Example CA", subjectPublicKey: await pki.key.export(ca.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z"),
 *     extensions: { basicConstraints: { cA: true }, keyUsage: ["keyCertSign"], subjectKeyIdentifier: true } },
 *     { key: await pki.key.export(ca.privateKey) });
 *   var req = await pki.ocsp.buildRequest({ cert: caDer, issuer: caDer }, { profile: "lightweight" });
 *   var http = pki.ocsp.httpRequest(req, "http://ocsp.example/");
 *   console.log(http.method, http.url.length <= 255);
 */
function httpRequest(der, responderUrl, opts) {
  opts = guard.identifier.optionsObject(opts, E, "ocsp/bad-input", "httpRequest: opts");
  guard.identifier.assertKnownKeys(opts, HTTP_REQUEST_OPTS, E, "ocsp/bad-input",
    "pki.ocsp.httpRequest has an unknown option (the only option is `method`). The unknown option was: ");
  var body = guard.bytes.snapshotSource(der, OcspError, "ocsp/bad-input", "httpRequest: the OCSPRequest DER");
  if (body.length === 0) throw E("ocsp/bad-input", "httpRequest: the OCSPRequest DER is empty");
  if (typeof responderUrl !== "string" || responderUrl === "") {
    throw E("ocsp/bad-input", "httpRequest: the responder URL must be a non-empty string");
  }
  var parts = guard.name.uriParts(responderUrl);
  if (parts === null || !parts.hasAuthority || parts.host === null) {
    throw E("ocsp/bad-input", "httpRequest: the responder URL must carry a scheme and a host (RFC 5280 sec. 4.2.2.1)");
  }
  if (responderUrl.indexOf("?") !== -1 || responderUrl.indexOf("#") !== -1) {
    throw E("ocsp/bad-input", "httpRequest: the responder URL carries a query or a fragment, and a path segment appended after either would not be part of the path (RFC 3986 sec. 3.3)");
  }
  /** @internal The bound below is a byte count, and a character is not a byte above US-ASCII. A
   * URI is US-ASCII by RFC 3986 sec. 2; a responder URL carrying more than that is an IRI, whose
   * conversion to a URI is RFC 3987 work this toolkit does not do, so it is refused rather than
   * measured as though the two were the same length. */
  if (!guard.encoding.isAsciiText(responderUrl)) {
    throw E("ocsp/bad-input", "httpRequest: the responder URL must be US-ASCII (RFC 3986 sec. 2); percent-encode the rest before passing it, since the RFC 5019 sec. 5 size bound counts the bytes that go on the wire");
  }
  var asked = opts.method === undefined ? null : opts.method;
  if (asked !== null && asked !== "GET" && asked !== "POST") {
    throw E("ocsp/bad-input", "httpRequest: opts.method must be \"GET\" or \"POST\"");
  }
  /** @internal The base64 goes through the buffer encoder, which emits one line and therefore no
   * CR and no LF, which RFC 5019 sec. 5 states as a MUST NOT. */
  var b64 = body.toString("base64");
  var segment = guard.encoding.uriPathSegment(b64, E, "ocsp/bad-input", "the encoded OCSPRequest");
  /** @internal The URL is handed to a transport, which parses it before opening a socket, and RFC
   * 3986 sec. 5.2.4 removes a `.` or `..` segment during that parse. Appending to an unnormalized
   * path would therefore build one URL and send another, at a path the responder was never named
   * at. Normalizing the base first makes what this returns what goes on the wire, which is also
   * the string the sec. 5 size bound has to be measured on. The appended segment is base64 under
   * the sec. 2.3 unreserved set, which holds no `.`, so it cannot become a dot segment itself. */
  var normalized;
  try { normalized = new URL(responderUrl).href; }
  catch (e) { throw E("ocsp/bad-input", "httpRequest: the responder URL does not parse as a URL: " + guard.text.describeThrown(e), e); }
  var base = normalized.charAt(normalized.length - 1) === "/"
    ? normalized : normalized + "/";
  var getUrl = base + segment;
  /** @internal Both parts are US-ASCII by the checks above, so a character is a byte and the
   * count is the one RFC 5019 sec. 5 states. */
  var getUrlBytes = Buffer.byteLength(getUrl, "ascii");
  var fitsGet = getUrlBytes <= HTTP_GET_MAX_URL_BYTES;
  if (asked === "GET" && !fitsGet) {
    throw E("ocsp/bad-input", "httpRequest: the encoded GET URL is " + getUrlBytes +
      " bytes, over the " + HTTP_GET_MAX_URL_BYTES + " RFC 5019 sec. 5 allows; send this request as a POST");
  }
  var useGet = asked === "GET" || (asked === null && fitsGet);
  if (useGet) {
    return guard.verdict.of({ method: "GET", url: getUrl, headers: _frozenHeaders({ accept: "application/ocsp-response" }), body: null });
  }
  return guard.verdict.of({
    method: "POST", url: normalized,
    headers: _frozenHeaders({ "content-type": "application/ocsp-request", accept: "application/ocsp-response" }),
    body: body,
  });
}

var HTTP_REQUEST_OPTS = Object.assign(Object.create(null), { method: 1 });

function _frozenHeaders(h) {
  var out = guard.identifier.ownOptions(h);
