...
// Try each candidate certificate as the signer, returning the verified verdict or null. The signer
// is whichever certificate's key made the signature: RFC 6960 sec. 4.1.1 certs is an unordered
// SEQUENCE OF Certificate, so a position is never assumed. `strict` separates the responder's own
// opts.certs (a malformed entry is its config error, a typed throw) from the request's untrusted
// embedded certs (a malformed one is skipped, never fatal, so a valid signer beside it still wins).
async function _findRequestSigner(candidates, strict, parsed, sig, sigBits) {
  for (var i = 0; i < candidates.length; i++) {
    ...
    if (await ocspRequestVerify.verifyRequestSignature(sig.signatureAlgorithm, sigBits, cert.subjectPublicKeyInfo.bytes, parsed.tbsRequestBytes)) {
      return _reqVerdict(parsed, { signed: true, signatureValid: true, signerCert: candidates[i],
        signerSubject: { dn: cert.subject.dn, rdns: cert.subject.rdns }, reason: null });
    }
  }
  return null;
}
 ...
 * @signature  pki.ocsp.verifyRequest(request, opts?) -> Promise<{ signed, signatureValid, signerCert, signerSubject, requestorName, requestList, requestExtensions, version, reason }>
 * @since      0.6.0
 * @status     stable
 * @spec       RFC 6960
 ...
 *
 * `signatureValid` means only that the requestor certificate's key made this signature over this
 * request. It says nothing about whether that certificate is trusted: `signerCert` is surfaced raw
 * and `signerSubject` decoded so the responder decides, running `pki.path.validate` on `signerCert`
 * and comparing `signerSubject` to the `requestorName` it expects. The decoded `requestList`,
 ...
 *              travel out of band). The first is taken as the signer. Ignored when the request
 *              embeds its own certificates.
 *
 * @example
...
async function verifyRequest(request, opts) {
  ...
  if (sig.certs.length) {
    var v = await _findRequestSigner(sig.certs, false, parsed, sig, sigBits);
    return v || _reqVerdict(parsed, { signed: true, signatureValid: false,
      reason: "the OCSP request signature does not verify over tbsRequest under any certificate the request carries (RFC 6960 sec. 4.1.1)" });
  }
  var suppliedCerts = [];
  if (opts.certs != null) {
  ...
  var v2 = await _findRequestSigner(suppliedCerts, true, parsed, sig, sigBits);
  return v2 || _reqVerdict(parsed, { signed: true, signatureValid: false,
    reason: "the OCSP request signature does not verify over tbsRequest under any opts.certs entry" });
}
