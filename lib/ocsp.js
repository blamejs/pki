...
// the common single-signer case. No match is signatureValid:false with the failure reason.
function _signerVerdict(parsed, matches, failReason) {
  if (!matches.length) return _reqVerdict(parsed, { signed: true, signatureValid: false, reason: failReason });
  return _reqVerdict(parsed, {
    signed: true, signatureValid: true, signerCert: matches[0].der,
    signerCerts: matches.map(function (m) { return m.der; }),
 ...
 * @signature  pki.ocsp.verifyRequest(request, opts?) -> Promise<{ signed, signatureValid, signerCert, signerCerts, signerSubject, requestorName, requestList, requestExtensions, version, reason }>
 * @since      0.6.0
 * @status     stable
 * @spec       RFC 6960
 ...
 * `signerSubject` are the first of those, the common single-signer case. The responder decides trust,
 * running `pki.path.validate` on a `signerCerts` entry and comparing its subject to the
