 ...
 * here with the code the parser would have raised.
 *
 * @opts
...
function _asDate(d) {
  ...
  /** @internal The guard also refuses a year DER cannot carry, so a Date this verb writes as a
   *  GeneralizedTime is one the codec will encode, and the fault stays in this verb's domain. */
  return guard.time.assertValid(dt, _err, "ocsp/bad-input", "the date " + guard.text.showValue(d));
}
...
function _buildSingleResponse(r, opts) {
  ...
  return certIdP.then(function (certID) {
    var thisUpdate = _asDate(r.thisUpdate) || new Date();
    var nextUpdate = r.nextUpdate === null ? null : (_asDate(r.nextUpdate) || _defaultNextUpdate());
    ...
    if (r.singleExtensions != null) {
      var singleDer = _singleExtensionsDer(r.singleExtensions);
...
function _singleExtensionsDer(spec) {
  if (Array.isArray(spec)) {
    ...
    if (ders.length) ocspSchema.readSingleExtensionsDer(ders);
    return ders;
  }
  ...
  if (spec.archiveCutoff != null) {
    guard.time.assertValid(spec.archiveCutoff, _err, "ocsp/bad-input", "singleExtensions archiveCutoff");
  ...
  if (spec.crlReferences != null) {
    ...
    if (c.crlTime != null) {
      guard.time.assertValid(c.crlTime, _err, "ocsp/bad-input", "crlReferences crlTime");
 ...
 * the request). It also compares the certificate's subject to the `requestorName` it expects. The decoded
