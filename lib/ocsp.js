...
var REASON_CODE = {};
_keys(constants.NAMES.CRL_REASON).forEach(function (k) { REASON_CODE[constants.NAMES.CRL_REASON[k]] = _parseInt(k, 10); });
var OID_OCSP_NONCE = O("ocspNonce");
var OID_OCSP_BASIC = O("ocspBasic");
var OID_EXTENDED_REVOKE = O("ocspExtendedRevoke");

function _digest(wcHash, buf) { return subtle.digest(wcHash, buf).then(function (h) { return _bufferFrom(h); }); }
...
function _toDer(input, what) {
  if (_isBuffer(input) || input instanceof Uint8Array) return guard.bytes.snapshot(input, OcspError, "ocsp/bad-input", what || "input");
...
function _assertNonce(nonceBytes) {
  if (!_isBuffer(nonceBytes) || nonceBytes.length < 1 || nonceBytes.length > 128) {
...
function _buildCertID(cert, issuer, hashName) {
  ...
  if (!wc) throw _err("ocsp/bad-input", "unsupported CertID hashAlgorithm " + _stringify(hashName) + " (sha1 / sha256 / sha384 / sha512)");
  return Promise.all([
...
function _buildRequest(query, opts) {
  ...
  var queries = _isArray(query) ? query : [query];
  ...
  return Promise.all(queries.map(function (q) {
    ...
    return _buildCertID(cert, issuer, hashName).then(function (certID) {
      ...
      if (q.singleRequestExtensions && q.singleRequestExtensions.length) {
        ...
        _push(reqChildren, b.explicit(0, b.sequence(q.singleRequestExtensions)));
      }
    ...
    if (opts.requestorName) {
      ...
      _push(tbsChildren, b.explicit(1, b.contextConstructed(4, nameDer)));
    }
    _push(tbsChildren, b.sequence(requests));
    var reqExts = [];
    if (opts.nonce) {
      ...
      _push(reqExts, _nonceExt(nonceBytes));
    }
    if (reqExts.length) _push(tbsChildren, b.explicit(2, b.sequence(reqExts)));
...
function _nameDer(name) {
  if (_isBuffer(name)) return guard.bytes.snapshot(name, OcspError, "ocsp/bad-input", "requestorName");
...
function _normCertDer(cert, what) {
  if (_isBuffer(cert) || cert instanceof Uint8Array) return guard.bytes.snapshot(cert, OcspError, "ocsp/bad-input", what || "a certificate");
...
function _signResponse(responseData, responder, respCert, respCertDer, responderKey, opts) {
  ...
  return _responderID(responseData.responderID, respCert).then(function (ridNode) {
    return Promise.all(responses.map(function (r) { return _buildSingleResponse(r, opts); })).then(function (srNodes) {
      ...
      var respExts = [];
      if (opts.nonce != null) { _assertNonce(opts.nonce); _push(respExts, _nonceExt(opts.nonce)); }
      if (opts.extendedRevoke) _push(respExts, b.sequence([b.oid(OID_EXTENDED_REVOKE), b.octetString(b.nullValue())]));
      if (respExts.length) _push(rdChildren, b.explicit(1, b.sequence(respExts)));
      ...
      return signScheme.signOverTbs(scheme, responderKey, responseDataDer, _signE).then(function (sig) {
        ...
        if (opts.embedCert !== false) _push(basicChildren, b.explicit(0, b.sequence([b.raw(respCertDer)])));
...
function _asDate(d) {
  ...
  if (isNaN(guard.time.instantOf(dt))) throw _err("ocsp/bad-input", "an invalid date value " + _stringify(d));
  return dt;
}
...
function _snapshotSignerKey(key, owned) {
  if (_isBuffer(key) || key instanceof Uint8Array) {
  ...
  if (key && typeof key === "object" && (key.mldsa != null || key.trad != null)) {
    return _assign({}, key, { mldsa: _snapshotSignerKey(key.mldsa, owned), trad: _snapshotSignerKey(key.trad, owned) });
  }
  return key;
}
...
function _buildSingleResponse(r, opts) {
  ...
  return certIdP.then(function (certID) {
    ...
    if (r.nextUpdate !== null) _push(srChildren, b.explicit(0, b.generalizedTime(_asDate(r.nextUpdate) || _defaultNextUpdate())));
    if (r.singleExtensions && r.singleExtensions.length) _push(srChildren, b.explicit(1, b.sequence(r.singleExtensions)));
    return b.sequence(srChildren);
  });
}
function _defaultNextUpdate() { return new Date(_dateNow() + constants.TIME.days(7)); }
function _certStatusNode(status, opts) {
  if (status == null || status === "good") return b.contextPrimitive(0, _bufferAlloc(0));   // good [0] IMPLICIT NULL
  if (status === "unknown") return b.contextPrimitive(2, _bufferAlloc(0));                   // unknown [2] IMPLICIT NULL
  if (typeof status === "object" && status.revoked != null) {
    ...
    if (status.revocationReason != null) {
      ...
      if (code == null) throw _err("ocsp/bad-input", "unknown revocationReason " + _stringify(status.revocationReason));
      _push(ri, b.explicit(0, b.enumerated(_BigInt(code))));   // revocationReason [0] EXPLICIT CRLReason
    }
    return b.contextConstructed(1, _bufferConcat(ri));       // revoked [1] IMPLICIT RevokedInfo
  }
...
function buildErrorResponse(status) {
  ...
  if (code == null) throw _err("ocsp/bad-input", "an error responseStatus must be one of " + _keys(ERROR_STATUS).join(" / "));
  return b.sequence([b.enumerated(_BigInt(code))]);
}
...
function verify(response, opts) {
  ...
  return pathValidate.verifyOcspResponse(parsed, cert, issuerCert, time, { historicalMode: opts.historicalMode === true }).then(function (verdict) {
    ...
    if (opts.requestNonce == null) return _assign({}, verdict, { nonceMatched: null });
    ...
    var reqNonce = (_isBuffer(opts.requestNonce) || opts.requestNonce instanceof Uint8Array)
    ...
    var out = _assign({}, verdict, { nonceMatched: matched });
    ...
    if (!matched && verdict.status === "good") {
      return _assign(out, { status: "unknown", reason: "the OCSP response nonce does not echo the request nonce (RFC 9654)" });
    }
    return out;
  });
}
