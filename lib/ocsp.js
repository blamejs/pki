...
var pkiBuild = require("./pki-build");   // reqDenseArray: reject a sparse/nullish caller array as a typed ocsp/bad-input
var oid = require("./oid");
...
var ocspRequestVerify = require("./ocsp-request-verify");   // path-validate (required above) injects its signature engine into this seam at load
...
// A certificate's signing-key identity for signerCerts membership: the algorithm identifier and the
// key material together, with a NULL AlgorithmIdentifier parameter treated as absent (rsaEncryption is
// encoded both ways for the one key) while any real parameter (an EC namedCurve) is kept. Comparing
// this rather than the raw subjectPublicKeyInfo lets a key reused under a different parameter encoding
// still match, yet an unrelated algorithm whose key bytes happen to coincide (an X25519 key equal to
// an Ed25519 signer's) does NOT -- its algorithm OID differs. The normalized AlgorithmIdentifier is a
// self-delimiting TLV, so concatenating it with the raw key bytes is unambiguous.
function _keyIdentity(spkiDer) {
  var spki = asn1.decode(spkiDer), algId = spki.children[0], params = algId.children[1];
  var keep = params && !(params.tagClass === "universal" && params.tagNumber === asn1.TAGS.NULL);
  var normAlg = keep ? b.sequence([Buffer.from(algId.children[0].bytes), Buffer.from(params.bytes)])
    : b.sequence([Buffer.from(algId.children[0].bytes)]);
  return Buffer.concat([normAlg, _keyValue(spkiDer)]);
}
...
function _buildRequest(query, opts) {
  ...
  // A batch array is density-checked before the map below (which would otherwise skip a hole and reach
  // Promise.all/b.sequence with an undefined); a single {cert,issuer} wraps to a dense one-element array.
...
// Parsed from BYTES so the signature, the algorithm that verifies it and the tbsRequest it covers
// are bound to one byte string, the same discipline pki.ocsp.verify follows for a response. A
// BufferSource is SNAPSHOT first: parseRequest surfaces tbsRequest / signature / embedded certs as
// views into it, and the signature check awaits, so without a copy a caller could mutate the buffer
// mid-verification and be judged on bytes other than the parser read. A PEM string decodes to a
// fresh buffer already; parseRequest owns that decode and a non-bytes input is its typed refusal.
...
// Collect EVERY candidate certificate whose key verified the request signature. RFC 6960 sec. 4.1.1
// certs is an unordered SEQUENCE OF Certificate, and several may share the signing key (an expired
// certificate carried beside its renewal), so all matches are surfaced rather than one
// position-dependent pick: the responder builds a trusted path to one of them to select a usable
// one, and no valid match is hidden behind an unusable one that happened to appear first. `strict` separates the
// responder's own opts.certs (a malformed entry is its config error, a typed throw) from the
// request's untrusted embedded certs (a malformed one is skipped, never fatal).
async function _findRequestSigners(candidates, strict, parsed, sig, sigBits) {
  ...
  // The certs surfaced for path building are the PARSEABLE ones: a malformed embedded entry (skipped
  // above rather than fatal) is excluded so it is never handed to pki.path.build as a candidate.
  var parseable = parsedList.map(function (p) { return p.der; });
  // A request carries ONE signature under ONE key, so the signature verification stops at the first
  // candidate that verifies: every further match carries that same key and is found by comparing
  // canonical public-key material below, not another verify. This bounds the crypto an untrusted
  // request can demand to the position of the signer (the certificate count is already capped at
  // OCSP_MAX_CERTS by the parser), rather than verifying every embedded certificate even after the
  // signer is found.
  var signerKey = null;
  for (var j = 0; j < parsedList.length; j++) {
    if (await ocspRequestVerify.verifyRequestSignature(sig.signatureAlgorithm, sigBits, parsedList[j].cert.subjectPublicKeyInfo.bytes, parsed.tbsRequestBytes)) {
      signerKey = _keyIdentity(parsedList[j].cert.subjectPublicKeyInfo.bytes);
      break;
    }
  }
  if (signerKey === null) return { matches: [], parseable: parseable };
  // signerCerts is every candidate carrying the verified signing key, in the order the request listed
  // them, so a usable certificate (e.g. a renewal beside an expired one) is not hidden. Membership
  // compares the algorithm-plus-key identity, not the whole subjectPublicKeyInfo: two certificates that
  // reuse a key can encode its AlgorithmIdentifier differently (rsaEncryption with NULL versus absent
  // parameters), so a raw SPKI byte comparison would drop the second even though its key signed, while
  // the algorithm OID stays part of the identity so an unrelated key that coincides is not admitted.
  var matches = [];
  for (var k = 0; k < parsedList.length; k++) {
    // constantTimeEqual over the key identity (public, so timing is not the point): it is the captured
    // comparison this module already uses for the nonce, so a co-resident cannot swap the membership
    // test to slip a certificate whose key did NOT sign into signerCerts.
    if (guard.crypto.constantTimeEqual(_keyIdentity(parsedList[k].cert.subjectPublicKeyInfo.bytes), signerKey)) {
      matches.push({ der: parsedList[k].der, subject: parsedList[k].cert.subject });
    }
  }
  return { matches: matches, parseable: parseable };
}
// The verdict from `_findRequestSigners`: signerCerts is every match (so the caller's trust decision
// does not depend on ordering), signerCert / signerSubject the first for the common single-signer
// case. `certs` is every PARSEABLE certificate the request carried (or opts.certs supplied), so the
// responder has the intermediates it needs to chain a signer to an anchor -- those do not sign the
// request, so they never appear in signerCerts. No match is signatureValid:false with the reason.
 ...
 * @signature  pki.ocsp.verifyRequest(request, opts?) -> Promise<{ signed, signatureValid, signerCert, signerCerts, certs, signerSubject, requestorName, requestList, requestExtensions, version, reason }>
 * @since      0.6.0
 * @status     stable
 * @spec       RFC 6960
...
async function verifyRequest(request, opts) {
  ...
  // sig.signature is the raw signature the parser already refused unless octet-aligned; the seam
  // re-checks alignment, so it is handed the BIT STRING shape the seam verifies.
  var sigBits = { unusedBits: 0, bytes: sig.signature };
  // Prefer the certificates the request carries; fall back to opts.certs only when it carries none.
  if (sig.certs.length) {
