...
function sign(responseData, responder, opts) {
  ...
  var responderKey = _snapshotSignerKey(responder.key);
...
function _snapshotSignerKey(key) {
  if (Buffer.isBuffer(key) || key instanceof Uint8Array) {
    return guard.bytes.snapshot(key, OcspError, "ocsp/bad-input", "the responder key");
  }
  if (key && typeof key === "object" && (Buffer.isBuffer(key.mldsa) || key.mldsa instanceof Uint8Array ||
      Buffer.isBuffer(key.trad) || key.trad instanceof Uint8Array)) {
    return Object.assign({}, key, { mldsa: _snapshotSignerKey(key.mldsa), trad: _snapshotSignerKey(key.trad) });
  }
  return key;
}
