...
function _certOf(arg, what) {
  ...
  return guard.parsed.accept(arg, "certificate", function (bytes) {
    var der = bytes;
    if (typeof bytes === "string") {
      try { der = x509.pemDecode(bytes); } catch (e) { throw _err("ocsp/bad-input", label + " PEM could not be decoded", e); }
    } else if (!Buffer.isBuffer(der)) { der = Buffer.from(der); }
...
function verify(response, opts) {
  ...
  // The caller's own argument goes to the verdict verb, not the object parsed here: it takes bytes
  // or the parser's recorded result and derives from that record itself, so both verbs read the
  // same source and there is no internal back door around the door. `parsed` is used below only for
  // the request-nonce comparison, which this module owns.
  return pathValidate.verifyOcspResponse(response, cert, issuerCert, time, { historicalMode: opts.historicalMode === true }).then(function (verdict) {
