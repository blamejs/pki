...
async function _findRequestSigners(candidates, strict, parsed, sig, sigBits) {
  ...
  // signerCerts is every candidate whose key VERIFIES the request signature, identified by the
  // verification itself rather than by comparing encoded key bytes. A request carries one signature
  // under one key, but that key can appear across several certificates in the unordered certs field
  // (an expired certificate beside its renewal), and the one key can be encoded differently between
  // them -- rsaEncryption with NULL versus absent parameters, an EC point compressed versus
  // uncompressed -- so a byte comparison would drop a genuine match, while a coincidental byte match
  // under a different algorithm would be a false one. Verifying each candidate answers both exactly,
  // through the algorithm-confusion gate (RFC 9814 sec. 4) in the shared engine. The work is bounded:
  // the parser already caps the certificate count at OCSP_MAX_CERTS, and the engine returns false
  // (never throws) for a candidate whose key does not match the signature, so a non-signer is skipped
  // rather than aborting the verdict.
  var matches = [];
