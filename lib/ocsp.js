...
function sign(responseData, responder, opts) {
  ...
  return _responderID(responseData.responderID, respCert).then(function (ridNode) {
    return Promise.all(responses.map(function (r) { return _buildSingleResponse(r, opts); })).then(function (srNodes) {
      ...
      // The copy taken at the entry is cleared as soon as signing is done, on the failing path
      // as well as the succeeding one -- a wrong or malformed key is the case an attacker can
      // force, so a success-only wipe would keep the secret exactly when it matters.
