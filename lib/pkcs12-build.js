...
async function _assertKeyPairs(spec, form, safeContentsSpecs) {
  ...
  if (!form.full) {
     ...
     *  certificate to no one key and is refused before anything is probed. */
   ...
   *  A composite ML-KEM key, which the runtime cannot read, is proven the same way through the
   *  toolkit's own KEM: a secret encapsulated to the certificate's key decapsulates under the private
   *  key to the same value only for the key pair. */
  for (var p = 0; p < pairs.length; p++) {
    ...
    var algOid;
    try { algOid = pkcs8.parse(keyDer).privateKeyAlgorithm.oid; }
    catch (e2) { throw _err("pkcs12/bad-input", pairs[p].what + ": the key is not a well-formed PKCS#8 PrivateKeyInfo", e2); }
    var corresponds;
    try {
      if (compositeKem.isCompositeKem(algOid)) {
        var sealed = await compositeKem.encapsulate(certSpki);
        var opened = await compositeKem.decapsulate(keyDer, sealed.ciphertext);
        corresponds = guard.crypto.constantTimeEqual(opened, sealed.sharedSecret);
        guard.secret.zeroizeAll([sealed.sharedSecret, opened], Pkcs12Error, "pkcs12/bad-input", "the correspondence probe secrets");
      } else {
        corresponds = await key.correspondsTo(keyDer, certSpki);
      }
 ...
 * another key, or carrying one `localKeyId` on two key bags, is refused with `pkcs12/bad-input`.
