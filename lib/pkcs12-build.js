...
function _decryptLegacyPbe(ea, ct, password, opts) {
  ...
  // PKCS#12 content regions are normatively BER (RFC 7292 sec. 4.1) and the parser accepts BER anywhere, so a
  // conforming legacy store may encode pkcs-12PbeParams with an indefinite-length SEQUENCE or a constructed
  // OCTET STRING salt -- decode in BER mode (asn1.read.octetString reassembles a constructed salt).
