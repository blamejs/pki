...
var P12_KDF_UV = intrinsic.assign(intrinsic.create(null), {
  sha1: { u: 20, v: 64 }, sha224: { u: 28, v: 64 }, sha256: { u: 32, v: 64 },
  sha384: { u: 48, v: 128 }, sha512: { u: 64, v: 128 },
});
var PBMAC1_PRF = intrinsic.assign(intrinsic.create(null), {
...
var PRF_WC = intrinsic.assign(intrinsic.create(null), { hmacWithSHA1: "SHA-1", hmacWithSHA256: "SHA-256", hmacWithSHA384: "SHA-384", hmacWithSHA512: "SHA-512" });
var CIPHER_NAME = intrinsic.assign(intrinsic.create(null), { "aes-128-cbc": "aes128-CBC", "aes-192-cbc": "aes192-CBC", "aes-256-cbc": "aes256-CBC" });
var DIGEST_NAME = intrinsic.assign(intrinsic.create(null), { sha1: "sha1", sha256: "sha256", sha384: "sha384", sha512: "sha512" });
...
var _INTEGRITY_OPTS = intrinsic.assign(intrinsic.create(null), { mode: 1, signer: 1, signers: 1, certificates: 1, sid: 1, signingTime: 1 });
var _BUILD_SPEC_FULL_KEYS = intrinsic.assign(intrinsic.create(null), { safeContents: 1 });
var _BUILD_SPEC_SHORTHAND_KEYS = intrinsic.assign(intrinsic.create(null), { key: 1, cert: 1, ca: 1, friendlyName: 1, localKeyId: 1 });
var _BUILD_OPTS_FULL = intrinsic.assign(intrinsic.create(null), { integrity: 1, mac: 1, password: 1, pem: 1 });
var _BUILD_OPTS_SHORTHAND = intrinsic.assign(intrinsic.create(null), { integrity: 1, mac: 1, password: 1, pem: 1, recipientCerts: 1 });
var _SAFE_RECIPIENTS_KEYS = intrinsic.assign(intrinsic.create(null), { bags: 1, recipients: 1, contentEncryptionAlgorithm: 1 });
var _SAFE_PASSWORD_KEYS = intrinsic.assign(intrinsic.create(null), { bags: 1, encrypt: 1 });
var _SAFE_PLAINTEXT_KEYS = intrinsic.assign(intrinsic.create(null), { bags: 1 });
var _BAG_KEYS_BY_TYPE = intrinsic.assign(intrinsic.create(null), {
...
var _ENCRYPT_KEYS = intrinsic.assign(intrinsic.create(null), { password: 1, cipher: 1, salt: 1, iterations: 1, prf: 1 });
var _MAC_KEYS = intrinsic.assign(intrinsic.create(null), { algorithm: 1, hash: 1, salt: 1, iterations: 1, keyLength: 1, password: 1 });
var _MAC_KEYS_HMAC = intrinsic.assign(intrinsic.create(null), { algorithm: 1, hash: 1, salt: 1, iterations: 1, password: 1 });
var _MAC_KEYS_PBMAC1 = intrinsic.assign(intrinsic.create(null), { algorithm: 1, hash: 1, salt: 1, iterations: 1, keyLength: 1, password: 1 });
var _OPEN_OPTS = intrinsic.assign(intrinsic.create(null), {
...
var _VERIFY_MAC_OPTS = intrinsic.assign(intrinsic.create(null), { maxIterations: 1 });
...
function _bagAttributes(bag) {
  var attrs = [];
  if (bag.friendlyName != null) {
    if (typeof bag.friendlyName !== "string") {
      throw _err("pkcs12/bad-input", "a bag's friendlyName must be a string, since it is the text a key store lists the bag under");
    }
    attrs.push(b.sequence([b.oid(O("friendlyName")), b.set([b.bmpString(bag.friendlyName)])]));
  }
...
function _buildBag(bag, opts, depth) {
  ...
  if (intrinsic.hasOwn(_BAG_KEYS_BY_TYPE, guard.text.keyOf(bag.type))) {
    guard.identifier.assertKnownKeys(bag, _BAG_KEYS_BY_TYPE[bag.type], _err, "pkcs12/bad-input", function (k) {
      return "unknown field " + JSON.stringify(k) + " on a " + guard.text.showValue(bag.type) +
  ...
  switch (bag.type) {
    ...
    default:
      throw _err("pkcs12/bad-input", "unknown bag type " + guard.text.showValue(bag.type) + " (key / shroudedKey / cert / crl / secret / safeContents)");
  }
}
...
function _pbeOpts(enc) {
  ...
  if (enc.cipher != null && typeof enc.cipher !== "string") throw _err("pkcs12/bad-input", "unsupported PBES2 cipher " + guard.text.showValue(enc.cipher) + " (aes-128-cbc / aes-192-cbc / aes-256-cbc)");
  var cipher = CIPHER_NAME[enc.cipher || "aes-256-cbc"];
  if (!cipher) throw _err("pkcs12/bad-input", "unsupported PBES2 cipher " + guard.text.showValue(enc.cipher) + " (aes-128-cbc / aes-192-cbc / aes-256-cbc)");
  return {
    cipher: cipher,
...
async function _buildAuthSafeElement(sc, opts) {
  ...
  if (hasRecipients) {
    ...
    if (alg !== "aes-128-cbc" && alg !== "aes-192-cbc" && alg !== "aes-256-cbc") throw _err("pkcs12/bad-input", "a public-key privacy safe requires an AES-CBC content cipher (aes-128|192|256-cbc), not " + guard.text.showValue(alg) + " -- RFC 7292 sec. 4.1 admits only id-envelopedData, not GCM/AuthEnvelopedData");
...
async function _buildMacData(macOpts, sharedPassword, authSafeDer) {
  ...
  if (algorithm === "hmac") {
    var node = DIGEST_NAME[guard.text.keyOf(hash)];
    if (!node || !P12_KDF_UV[node]) throw _err("pkcs12/unsupported-algorithm", "unsupported classic MAC hash " + guard.text.showValue(hash) + " (sha1 / sha256 / sha384 / sha512)");
  ...
  if (algorithm === "pbmac1") {
    var prf = PBMAC1_PRF[guard.text.keyOf(hash)];
  ...
  throw _err("pkcs12/bad-input", "opts.mac.algorithm must be 'hmac' or 'pbmac1', got " + guard.text.showValue(algorithm));
}
...
/** @internal The convenience form pairs one key with one certificate, and a localKeyId pairs a key
 *  bag with a certificate bag (PKCS #9 sec. 5.5.2: "the local key id ... links a certificate to a
 *  private key"). A pair is held to being a KEY PAIR, the key's public half being the certificate's
 *  subjectPublicKey, since a store that links a certificate to somebody else's key is one nothing
 *  can use and the reader trusts the link. Bags of the full form pair across every safe. */
function _collectPairBags(bags, depth, keyBags, certBags) {
  if (!intrinsic.isArray(bags)) return;
  ...
  bags = pkiBuild.reqDenseArray(bags, "bags", _err, "pkcs12/bad-input");
  for (var j = 0; j < bags.length; j++) {
    var bag = bags[j];
    if (!bag || typeof bag !== "object") continue;
    if (bag.type === "safeContents") {
      if (depth + 1 <= C.LIMITS.PKCS12_MAX_BAG_DEPTH) _collectPairBags(bag.nested, depth + 1, keyBags, certBags);
      continue;
    }
    if (bag.localKeyId == null || bag.localKeyId === "ski") continue;
    if (bag.type === "key" || bag.type === "shroudedKey") guard.list.append(keyBags, bag);
    else if (bag.type === "cert") guard.list.append(certBags, bag);
  }
}
async function _assertKeyPairs(spec, form, safeContentsSpecs) {
  var pairs = [];
  if (!form.full) {
    if (spec.key != null && spec.cert != null) guard.list.append(pairs, { key: spec.key, cert: spec.cert, what: "the { key, cert } pair" });
  } else {
    var keyBags = [], certBags = [];
    for (var i = 0; i < safeContentsSpecs.length; i++) {
      _collectPairBags(safeContentsSpecs[i] && safeContentsSpecs[i].bags, 0, keyBags, certBags);
    }
    /** @internal A localKeyId identifies ONE private key (PKCS #9 sec. 5.5.2), so the key bags are
     *  indexed by id and each certificate bag looks its key up: the pairing costs one probe per
     *  certificate bag, never the product of the two sets, and an id carried by two key bags links a
     *  certificate to no one key and is refused before anything is probed. The pairs a store links
     *  across all its safes are capped at the element cap, decided while they are collected and
     *  before any is probed, so the work a store can ask for is bounded by one constant rather than
     *  by the product of the safe cap and the per-safe element cap. */
    var keyById = intrinsic.create(null);
    for (var k = 0; k < keyBags.length; k++) {
      var kid = intrinsic.bufToString(_bytes(keyBags[k].localKeyId, "localKeyId"), "hex");
      if (keyById[kid] !== undefined) throw _err("pkcs12/bad-input", "localKeyId " + kid + " is carried by more than one key bag; a localKeyId names one private key (PKCS #9 sec. 5.5.2)");
      keyById[kid] = keyBags[k];
    }
    for (var c = 0; c < certBags.length; c++) {
      var cid = intrinsic.bufToString(_bytes(certBags[c].localKeyId, "localKeyId"), "hex");
      var keyBag = keyById[cid];
      if (keyBag === undefined) continue;
      if (pairs.length >= C.LIMITS.PKCS12_MAX_ELEMENTS) throw _err("pkcs12/bad-input", "the store links more than " + C.LIMITS.PKCS12_MAX_ELEMENTS + " certificate bags to keys through localKeyId; the linked pairs are capped at the element cap");
      guard.list.append(pairs, { key: keyBag.key, cert: certBags[c].cert, what: "the key and certificate bags sharing localKeyId " + cid });
    }
  }
  /** @internal The correspondence is proven with the PRIVATE half (a signature, an agreement or an
   *  encapsulation against the certificate's key), since a PKCS#8 EC or RSA key carries public
   *  components of its own that a caller could plant beside private material they do not belong to.
   *  The key module owns the proof for every algorithm, the composite ML-KEM route included.
   *  Proving a finite-field Diffie-Hellman pair includes proving its group's modulus and order prime,
   *  about a second each at the widest group the toolkit agrees over and synchronous, so the DH
   *  pairs one store may link are budgeted, and the budget is decided from the keys' algorithms
   *  before any pair is probed. */
  var halves = [], dhPairs = 0;
  for (var a = 0; a < pairs.length; a++) {
    var keyDer = _coerceDer(pairs[a].key, "the paired key");
    var certDer = _coerceDer(pairs[a].cert, "the paired certificate");
    var keyAlg, certSpki;
    try { keyAlg = pkcs8.parse(keyDer).privateKeyAlgorithm.oid; }
    catch (e2) { throw _err("pkcs12/bad-input", pairs[a].what + ": the key is not a well-formed PKCS#8 PrivateKeyInfo", e2); }
    try { certSpki = x509.parse(certDer).subjectPublicKeyInfo; }
    catch (e) { throw _err("pkcs12/bad-input", pairs[a].what + ": the certificate is not a well-formed X.509 certificate", e); }
    if (key.isDhOid(keyAlg) || key.isDhOid(certSpki.algorithm.oid)) {
      dhPairs++;
      if (dhPairs > C.LIMITS.PKCS12_MAX_DH_PAIRS) throw _err("pkcs12/bad-input", "the store links more than " + C.LIMITS.PKCS12_MAX_DH_PAIRS + " finite-field Diffie-Hellman pairs; proving each costs a primality proof of its group, so a store is budgeted that many");
    }
    guard.list.append(halves, { keyDer: keyDer, certSpki: certSpki.bytes });
  }
  for (var p = 0; p < pairs.length; p++) {
    var pairKey = halves[p].keyDer;
    var pairSpki = halves[p].certSpki;
    var corresponds;
    try { corresponds = await key.correspondsTo(pairKey, pairSpki); }
    catch (e3) { throw _err("pkcs12/bad-input", pairs[p].what + ": the key could not be held to the certificate's public key", e3); }
    if (corresponds !== true) {
      throw _err("pkcs12/bad-input", pairs[p].what + ": the key is not the private half of the certificate's public key; a store that links a certificate to another key is refused");
    }
  }
}
 ...
 * safeContents bags. Keys and certs are validated before wrapping, and a key paired with a certificate
 * (the `{ key, cert }` form, or a key bag and a certificate bag sharing a `localKeyId`, PKCS #9 sec.
 * 5.5.2) must be the private half of that certificate's public key; a store linking a certificate to
 * another key, or carrying one `localKeyId` on two key bags, is refused with `pkcs12/bad-input`, as is
 * one linking more than 1024 certificate bags to keys, more than 8 finite-field Diffie-Hellman pairs,
 * or carrying more than 1024 safes.
 * Password integrity (the default) MACs the
 ...
 *   - `mac` -- `false` for a MAC-less store, or `{ algorithm: 'hmac'(default)|'pbmac1', hash: 'sha256'(default)|'sha1'|'sha384'|'sha512', salt?, iterations?, password? }`. `password` overrides the shared integrity password for the MAC. `keyLength` is a `pbmac1` field: the classic Appendix B derivation produces a key at the hash's own output length, and naming one under `hmac` is refused.
...
async function _build(spec, opts) {
  ...
  if (opts.integrity != null) {
    ...
    if (opts.integrity.mode !== "public-key") {
      throw _err("pkcs12/bad-integrity-mode", "opts.integrity.mode must be \"public-key\" (the only mode it selects); omit opts.integrity for password integrity, got " + guard.text.showValue(opts.integrity.mode));
    }
  }
  ...
  /** @internal The cap the parser puts on the AuthenticatedSafe, applied before any safe is built:
   *  a store of more safes than the parser reads back would be encrypted in full and then refused
   *  by the re-parse. */
  if (safeContentsSpecs.length > C.LIMITS.PKCS12_MAX_ELEMENTS) throw _err("pkcs12/bad-input", "the AuthenticatedSafe exceeds the element cap " + C.LIMITS.PKCS12_MAX_ELEMENTS + " (safeContents entries)");
  await _assertKeyPairs(spec, form, safeContentsSpecs);
  var elements = [];
...
async function verifyMac(pfx, password, opts) {
  if (opts != null && typeof opts === "object" && !Buffer.isBuffer(opts)) {
    ...
    /** @internal Taken once here, for the reason the same call in `open` states. */
    opts = guard.identifier.snapshotOptions(opts, _VERIFY_MAC_OPTS);
  }
...
/** @internal The caller's bound is read once and the shape check and the comparison both read that
 * one answer. Read separately, an option reached through an accessor satisfies every check as a
 * number and then answers the comparison with something that is not one, so `Math.min` yields NaN.
 * Nothing compares above NaN, so the cap stopped bounding the work it exists to bound. */
function _capWork(iterations, salt, opts, keyLength, hardCap) {
  var cap = hardCap;
  var wanted = opts.maxIterations;
  if (wanted != null) {
    if (typeof wanted !== "number" || !isFinite(wanted) || wanted < 1 || Math.floor(wanted) !== wanted) throw _err("pkcs12/bad-input", "maxIterations must be a positive integer");
    cap = Math.min(wanted, cap);
  }
 ...
 * structured bundle `{ valid, integrityMode, macVerified, signers, keys, certs, crls, secrets }` with each private key as PKCS#8
...
async function open(pfx, password, opts) {
  ...
  /** @internal Every option is taken once, here, and everything below reads the snapshot. A store
   * is opened in many passes, the MAC and then each bag, so an option left on the caller's object is
   * read again by every one of them. An option reached through an accessor answers each read
   * separately: one that returned a valid integer to this check and something that is not a number
   * to a later bag left the work cap as NaN, and no count is above NaN. */
  opts = guard.identifier.snapshotOptions(opts, _OPEN_OPTS);
  ...
  var out = guard.verdict.of({ valid: macVerified || m.integrityMode === "public-key", integrityMode: m.integrityMode, macVerified: macVerified, signers: signers, keys: [], certs: [], crls: [], secrets: [] });
  var i;
  ...
  if (opts.keys === "crypto") {
    for (i = 0; i < out.keys.length; i++) {
      guard.verdict.set(out.keys[i], "key", await key.import(out.keys[i].pkcs8, { algorithm: opts.importAlgorithm }));
    }
  }
  return out;
}
...
function _openBag(bag, password, opts, out, depth, budget) {
  switch (bag.type) {
    case "keyBag":
      ...
      guard.list.append(out.keys, { pkcs8: bag.keyDer, encrypted: false, friendlyName: bag.friendlyName, localKeyId: bag.localKeyId });
      break;
    case "pkcs8ShroudedKeyBag":
      guard.list.append(out.keys, { pkcs8: _decryptShroudedKey(bag.encrypted, password, opts, budget), encrypted: true, friendlyName: bag.friendlyName, localKeyId: bag.localKeyId });
      break;
    case "certBag":
      guard.list.append(out.certs, { cert: bag.certValue, certType: bag.certType, friendlyName: bag.friendlyName, localKeyId: bag.localKeyId });
      break;
    case "crlBag":
      guard.list.append(out.crls, { crl: bag.crlValue, crlType: bag.crlType, friendlyName: bag.friendlyName, localKeyId: bag.localKeyId });
      break;
    case "secretBag":
      guard.list.append(out.secrets, { secretTypeId: bag.secretTypeId, secretTypeName: bag.secretTypeName, secretValue: bag.secretValue, friendlyName: bag.friendlyName, localKeyId: bag.localKeyId });
      break;
...
function _decryptLegacyPbe(ea, ct, password, opts, budget) {
  ...
  var wantedLegacy = opts.maxIterations;
  var cap = wantedLegacy != null ? Math.min(wantedLegacy, CLASSIC_MAC_MAX_ITERATIONS) : CLASSIC_MAC_MAX_ITERATIONS;
