...
var cmsSign = require("./cms-sign");   // the signer-descriptor field tables, for the integrity signers this verb authors
...
var pkiBuild = require("./pki-build");   // reqDenseArray: reject a sparse/nullish caller array as a typed pkcs12/bad-input
var guard = require("./guard-all");
var intrinsic = require("./guard-intrinsic");
var _toLower = intrinsic.uncurry(String.prototype.toLowerCase);   // bound at load so a replaced String.prototype.toLowerCase cannot steer a hash-name comparison
...
// Per MAC algorithm: the classic Appendix B key derivation produces a key at the hash's own output
// length and has no keyLength to set, so admitting one there took an explicit security parameter and
// emitted the same MAC as a call that never named it. Only PBMAC1 (RFC 9579) derives to a length.
...
//
// One table here, not one per branch, and the distinction is who selects the branch. A builder's
// options are checked against the form the CALLER chose, because a field the chosen form never
// reads is a request that silently did not happen. Reading a store, the branch is chosen by the
// BYTES: which integrity mode it carries, and whether a safe is password- or recipient-encrypted,
// are not known until it is parsed. These options are the policy a caller declares in advance for
// whatever the store turns out to be, so one that goes unread means the store did not need it --
// signerCerts on a MAC'd store, recipientKey on one with no enveloped safe. Refusing those would
// make a correct, store-agnostic call fail on the shape of its input.
var _OPEN_OPTS = {
...
function _buildSafeContents(bags, opts, depth) {
  // The element cap is enforced BEFORE the density scan, so a huge dense array is rejected in O(1) rather
  // than walked. reqDenseArray then rejects a non-array, or a sparse hole / nullish entry that
  // Array.prototype.map would skip into Buffer.concat as a native error, testing own-property presence
  // and failing at the first bad index.
...
async function _build(spec, opts) {
  ...
  if (pubKey) {
    ...
    // `signer` is the one-signer spelling of `signers`, and the list wins outright, so supplying
    // both signed the store under the list alone and dropped the single signer without a word.
    ...
    // The signer descriptors are authoring input for this store, so they answer to the same rule as
    // every other field the caller writes here, against the form each one selects. Every field
    // beyond the identity has a default: without the door a misspelled `pss` or `digestAlgorithm`
    // signed the store under PKCS#1 with SHA-256 while the caller had asked for something else, and
    // the store records only what was used, never what was requested.
    ...
    if (opts.mac != null && opts.mac !== false) {
      // The algorithm selects the derivation, so the table that runs is the one belonging to it.
      // An unrecognized spelling keeps the union; _buildMacData refuses it by name a moment later.
...
async function verifyMac(pfx, password, opts) {
  ...
  // _verifyMacOfStore throws pkcs12/bad-input for a store carrying no password MAC, so m.mac is present
  // once it returns. Surface the MAC algorithm the store used so a caller can reject an integrity
  // algorithm it does not accept (e.g. a legacy SHA-1 classic MAC), instead of a bare boolean that hides it.
...
async function open(pfx, password, opts) {
  ...
  // `valid` is the integrity verdict: open THROWS on an integrity failure for both a password MAC and a
  // public-key CMS signature, so a returned store with either mode is verified; only an allowUnauthenticated
  // (MAC-less) store reaches here unverified. macVerified alone would read false for a valid public-key store.
