...
async function _build(spec, opts) {
  ...
  if (spec && typeof spec === "object" && !Buffer.isBuffer(spec)) {
    var full = spec.safeContents != null;
    guard.identifier.assertKnownKeys(spec, full ? _BUILD_SPEC_FULL_KEYS : _BUILD_SPEC_SHORTHAND_KEYS,
      _err, "pkcs12/bad-input", function (k) {
        return "unknown PKCS#12 spec field " + JSON.stringify(k) + " for the " +
          (full ? "safeContents form; the shorthand fields belong to { key, cert, ca } and are not read here"
