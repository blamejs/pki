...
function _storeFromBytes(pfx) {
  if (pfx && typeof pfx === "object" && !Buffer.isBuffer(pfx) && !(pfx instanceof Uint8Array) &&
      (pfx.integrityMode !== undefined || pfx.mac !== undefined || pfx.macedBytes !== undefined)) {
    throw _err("pkcs12/bad-input", "pfx must be the store's DER bytes or a PEM string, not a parsed store: the MAC is verified over a byte range carried on the object, and the bags returned as verified are a separate property of it, so a parsed argument lets the two describe different stores");
  }
  return schemaPkcs12.parse(_coerceDer(pfx, "pfx"));
}
 ...
 *
 * Verify a password-integrity PKCS#12 store's MAC. `pfx` is the store's DER `Buffer` or a PEM string --
 * a parsed store is refused, because the MAC is verified over a byte range the object carries and a
 * parsed argument lets that range and the store it describes come from different places. The password is BMPString+NULL encoded (RFC 7292 App. B.1), the MAC is
 ...
 * `friendlyName` / `localKeyId` for pairing. `pfx` is the store's DER `Buffer` or a PEM string; a parsed
 * store is refused, since the bytes whose integrity is checked and the bags returned as checked would
 * then be separate properties of one caller-supplied object.
 *
