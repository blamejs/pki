...
var OID_DH_PKCS3 = O("dhKeyAgreement");
var OID_DH_X942 = O("dhpublicnumber");
...
async function _assertKeyPairs(spec, form, safeContentsSpecs) {
  ...
  var keyDers = [], dhPairs = 0;
  for (var a = 0; a < pairs.length; a++) {
    var der = _coerceDer(pairs[a].key, "the paired key");
    var algOid;
    try { algOid = pkcs8.parse(der).privateKeyAlgorithm.oid; }
    catch (e2) { throw _err("pkcs12/bad-input", pairs[a].what + ": the key is not a well-formed PKCS#8 PrivateKeyInfo", e2); }
    if (algOid === OID_DH_PKCS3 || algOid === OID_DH_X942) {
      dhPairs++;
    ...
    guard.list.append(keyDers, der);
  }
  for (var p = 0; p < pairs.length; p++) {
    var keyDer = keyDers[p];
    var certDer = _coerceDer(pairs[p].cert, "the paired certificate");
    var certSpki;
    try { certSpki = x509.parse(certDer).subjectPublicKeyInfo.bytes; }
    catch (e) { throw _err("pkcs12/bad-input", pairs[p].what + ": the certificate is not a well-formed X.509 certificate", e); }
    var corresponds;
    try { corresponds = await key.correspondsTo(keyDer, certSpki); }
