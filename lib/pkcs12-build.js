...
function _assertMacIter(n) {
  ...
  if (n > C.LIMITS.PBKDF2_MAX_ITERATIONS) throw _err("pkcs12/bad-input", "MAC iterations exceeds the cap " + C.LIMITS.PBKDF2_MAX_ITERATIONS);
  return n;
}
...
function _buildBag(bag, opts, depth) {
  ...
  switch (bag.type) {
    ...
    case "secret": {
      ...
      return _safeBag("secretBag", b.sequence([b.oid(O(bag.secretTypeId)), b.explicit(0, secretValueDer)]), bag);
    }
    case "safeContents": {
...
async function _buildMacData(macOpts, sharedPassword, authSafeDer) {
  ...
  if (algorithm === "hmac") {
    ...
    var iter = _assertMacIter(macOpts.iterations == null ? DEFAULT_MAC_ITER : macOpts.iterations);
  ...
  if (algorithm === "pbmac1") {
    ...
    var iter2 = _assertMacIter(macOpts.iterations == null ? DEFAULT_PBMAC1_ITER : macOpts.iterations);
    var keyLen = macOpts.keyLength != null ? macOpts.keyLength : prf.keyLen;
    if (typeof keyLen !== "number" || !Number.isInteger(keyLen) || keyLen < 20) throw _err("pkcs12/bad-input", "PBMAC1 keyLength must be an integer >= 20 (RFC 9579 sec. 9)");
    var mac = await pbes2.pbmac1(_pbePassword(password), salt, iter2, keyLen, prf.wc, authSafeDer);   // PBKDF2 -> UTF-8
...
async function verifyMac(pfx, password, opts) {
  ...
  if (m.mac.kind === "hmac") {
    ...
    var wc = PRF_WC[kdf.prfName];
    if (!wc) throw _err("pkcs12/unsupported-algorithm", "unsupported PBMAC1 prf " + kdf.prfName);
    computed = await pbes2.pbmac1(_pbePassword(password), kdf.salt, kdf.iterationCount, kdf.keyLength, wc, m.macedBytes);   // PBKDF2 -> UTF-8
  }
