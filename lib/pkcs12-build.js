...
function _p12PasswordOwned(pw) {
  if (_isBuffer(pw)) return { bytes: guard.bytes.view(pw, Pkcs12Error, "pkcs12/bad-input", "the password"), owned: false };
...
function _p12Encode(pw) {
  ...
  if (_isBuffer(pw) || pw instanceof Uint8Array) return guard.bytes.snapshot(pw, Pkcs12Error, "pkcs12/bad-input", "the password");
  if (typeof pw !== "string") throw _err("pkcs12/bad-input", _MISSING_PASSWORD);
  var out = _bufferAlloc(pw.length * 2 + 2);   // + the 2-byte NULL terminator
...
function _pbePasswordOwned(pw) {
  ...
  if (typeof pw !== "string") throw _err("pkcs12/bad-input", _MISSING_PASSWORD);
  return { bytes: _bufferFrom(pw, "utf8"), owned: true };
}
...
function _blockFill(src, blockSize) {
  if (src.length === 0) return _bufferAlloc(0);
  var total = blockSize * _ceil(src.length / blockSize);
  var out = _bufferAlloc(total);
...
function _p12Kdf(hashName, id, pwBytes, salt, iterations, nBytes) {
  ...
  var D = _bufferAlloc(v, id);                                          // step 1: v copies of the ID diversifier
  ...
  var out = _bufferAlloc(c * u);
  for (var i = 0; i < c; i++) {                                         // step 6
    var A = _bufferConcat([D, I]);
...
function _assertMacIter(n, cap) {
  if (typeof n !== "number" || !_isFinite(n) || n < 1 || _floor(n) !== n) throw _err("pkcs12/bad-input", "MAC iterations must be a positive integer");
...
function _bagAttributes(bag) {
  var attrs = [];
  if (bag.friendlyName != null) _push(attrs, b.sequence([b.oid(O("friendlyName")), b.set([b.bmpString(_String(bag.friendlyName))])]));
  if (bag.localKeyId != null) {
    ...
    _push(attrs, b.sequence([b.oid(O("localKeyId")), b.set([b.octetString(_bytes(bag.localKeyId, "localKeyId"))])]));
  }
...
function _buildBag(bag, opts, depth) {
  ...
  switch (bag.type) {
    ...
    default:
      throw _err("pkcs12/bad-input", "unknown bag type " + _stringify(bag.type) + " (key / shroudedKey / cert / crl / secret / safeContents)");
  }
}
...
function _pbeOpts(enc) {
  ...
  if (!cipher) throw _err("pkcs12/bad-input", "unsupported PBES2 cipher " + _stringify(enc.cipher) + " (aes-128-cbc / aes-192-cbc / aes-256-cbc)");
  return {
    cipher: cipher,
...
function _buildSafeContents(bags, opts, depth) {
  if (!_isArray(bags)) throw _err("pkcs12/bad-input", "bags must be an array");
...
async function _buildAuthSafeElement(sc, opts) {
  ...
  if (hasRecipients) {
    if (!_isArray(sc.recipients) || !sc.recipients.length) throw _err("pkcs12/bad-input", "safeContents.recipients must be a non-empty array of recipient descriptors (RFC 5652 sec. 6.1)");
    ...
    if (!/^aes-(128|192|256)-cbc$/.test(alg)) throw _err("pkcs12/bad-input", "a public-key privacy safe requires an AES-CBC content cipher (aes-128|192|256-cbc), not " + _stringify(alg) + " -- RFC 7292 sec. 4.1 admits only id-envelopedData, not GCM/AuthEnvelopedData");
...
async function _buildMacData(macOpts, sharedPassword, authSafeDer) {
  ...
  if (algorithm === "hmac") {
    ...
    if (!node || !P12_KDF_UV[node]) throw _err("pkcs12/unsupported-algorithm", "unsupported classic MAC hash " + _stringify(hash) + " (sha1 / sha256 / sha384 / sha512)");
    ...
    try {
      ...
      return b.sequence([digestInfo, b.octetString(salt), b.integer(_BigInt(iter))]);
    } finally {
  ...
  if (algorithm === "pbmac1") {
    ...
    if (typeof keyLen !== "number" || !_isInteger(keyLen) || keyLen < 20 || keyLen > MAX_PBMAC1_KEYLEN) throw _err("pkcs12/bad-input", "PBMAC1 keyLength must be an integer in [20, " + MAX_PBMAC1_KEYLEN + "] (RFC 9579 sec. 9)");
    ...
    return b.sequence([digestInfo2, b.octetString(salt), b.integer(_BigInt(iter2))]);
  }
  throw _err("pkcs12/bad-input", "opts.mac.algorithm must be 'hmac' or 'pbmac1', got " + _stringify(algorithm));
}
...
function _normalizeSpec(spec, opts) {
  if (spec && _isArray(spec.safeContents)) return spec.safeContents;
  if (spec && (spec.key != null || spec.cert != null)) {
    var enveloped = opts.recipientCerts != null;
    if (enveloped && (!_isArray(opts.recipientCerts) || !opts.recipientCerts.length)) throw _err("pkcs12/bad-input", "opts.recipientCerts must be a non-empty array of recipient certificates");
    var certBags = [];
    if (spec.cert != null) _push(certBags, { type: "cert", cert: spec.cert, friendlyName: spec.friendlyName, localKeyId: spec.localKeyId });
    if (spec.ca != null && !_isArray(spec.ca)) throw _err("pkcs12/bad-input", "spec.ca must be an array of certificates");
    (spec.ca || []).forEach(function (ca) { _push(certBags, { type: "cert", cert: ca }); });
    var sc = [];
    if (enveloped) {
      ...
      if (spec.key != null) _push(bags, { type: "key", key: spec.key, friendlyName: spec.friendlyName, localKeyId: spec.localKeyId });
      _push(sc, { recipients: opts.recipientCerts.map(function (c) { return { cert: c }; }), bags: bags });
      return sc;
    }
    if (certBags.length) _push(sc, { encrypt: { password: opts.password }, bags: certBags });
    if (spec.key != null) _push(sc, { bags: [{ type: "shroudedKey", key: spec.key, encrypt: { password: opts.password }, friendlyName: spec.friendlyName, localKeyId: spec.localKeyId }] });
    return sc;
  }
...
async function _build(spec, opts) {
  ...
  if (opts.integrity != null) {
    if (typeof opts.integrity !== "object" || _isArray(opts.integrity)) throw _err("pkcs12/bad-input", "opts.integrity must be an object { mode, signer|signers, ... }");
    ...
    if (opts.integrity.mode !== "public-key") {
      throw _err("pkcs12/bad-integrity-mode", "opts.integrity.mode must be \"public-key\" (the only mode it selects); omit opts.integrity for password integrity, got " + _stringify(opts.integrity.mode));
    }
  }
  ...
  if (!_isArray(safeContentsSpecs) || !safeContentsSpecs.length) throw _err("pkcs12/bad-input", "the store has no safe contents");
  var elements = [];
  for (var i = 0; i < safeContentsSpecs.length; i++) _push(elements, await _buildAuthSafeElement(safeContentsSpecs[i], opts));
  ...
  if (pubKey) {
    ...
    if (!_isArray(signers) || !signers.length) throw _err("pkcs12/bad-input", "public-key integrity requires opts.integrity.signer or opts.integrity.signers (a cms.sign signer descriptor)");
    ...
    if (opts.mac !== false) _push(pfxChildren, await _buildMacData(opts.mac || {}, opts.password, authSafeDer));
...
function _capWork(iterations, salt, opts, keyLength, hardCap) {
  ...
  if (opts.maxIterations != null) {
    if (typeof opts.maxIterations !== "number" || !_isFinite(opts.maxIterations) || opts.maxIterations < 1 || _floor(opts.maxIterations) !== opts.maxIterations) throw _err("pkcs12/bad-input", "maxIterations must be a positive integer");
...
function _decryptLegacyPbe(ea, ct, password, opts, budget) {
  ...
  try {
    ...
    salt = _bufferFrom(asn1.read.octetString(params.children[0]));
  ...
  var cap = opts.maxIterations != null ? _min(opts.maxIterations, CLASSIC_MAC_MAX_ITERATIONS) : CLASSIC_MAC_MAX_ITERATIONS;
  ...
  budget.rounds -= (_ceil(scheme.keyLen / u) + _ceil(scheme.ivLen / u)) * iterations;
  ...
  try {
    ...
    return _bufferConcat([d.update(ct), d.final()]);
