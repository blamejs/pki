...
async function _assertKeyPairs(spec, form, safeContentsSpecs) {
  ...
  if (!form.full) {
    ...
    for (var k = 0; k < keyBags.length; k++) {
      var kid = _bytes(keyBags[k].localKeyId, "localKeyId");
      for (var c = 0; c < certBags.length; c++) {
        if (intrinsic.bufferEquals(kid, _bytes(certBags[c].localKeyId, "localKeyId"))) guard.list.append(pairs, { key: keyBags[k].key, cert: certBags[c].cert, what: "the key and certificate bags sharing localKeyId " + intrinsic.bufToString(kid, "hex") });
      }
    }
  }
 ...
 * another key is refused with `pkcs12/bad-input`. Password integrity (the default) MACs the
