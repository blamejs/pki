...
async function _buildMacData(macOpts, sharedPassword, authSafeDer) {
  ...
  if (algorithm === "hmac") {
    ...
    var macPw = _p12Password(password);
    var macKey = _p12Kdf(node, 3, macPw, salt, iter, P12_KDF_UV[node].u);   // classic KDF -> BMPString
    guard.secret.zeroize(macPw, Pkcs12Error, "pkcs12/bad-input", "the password encoding");
    try {
...
async function verifyMac(pfx, password, opts) {
  ...
  if (m.mac.kind === "hmac") {
    ...
    var vMacPw = _p12Password(password);
    var macKey = _p12Kdf(node, 3, vMacPw, m.mac.macSalt, m.mac.iterations, P12_KDF_UV[node].u);   // classic KDF -> BMPString
    guard.secret.zeroize(vMacPw, Pkcs12Error, "pkcs12/bad-input", "the password encoding");
...
function _decryptLegacyPbe(ea, ct, password, opts, budget) {
  ...
  guard.secret.zeroize(p12pw, Pkcs12Error, "pkcs12/bad-input", "the password encoding");
