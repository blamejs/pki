...
var _BUILD_SPEC_KEYS = { safeContents: 1, key: 1, cert: 1, ca: 1, friendlyName: 1, localKeyId: 1 };
...
var _BUILD_OPTS = { integrity: 1, mac: 1, password: 1, pem: 1, recipientCerts: 1 };
...
var _SAFE_KEYS = { bags: 1, encrypt: 1, recipients: 1, contentEncryptionAlgorithm: 1 };
// The fields a bag carries, across every bag type; `type` itself is dispatched against its own
// whitelist and an unknown value already throws. A misspelled attribute is dropped from the emitted
// bag, so the store is missing the friendlyName or localKeyId an importer matches on.
var _BAG_KEYS = {
  type: 1, key: 1, encrypt: 1, cert: 1, crl: 1,
  secretTypeId: 1, secretValue: 1, nested: 1, friendlyName: 1, localKeyId: 1,
};
...
function _buildBag(bag, opts, depth) {
  ...
  guard.identifier.assertKnownKeys(bag, _BAG_KEYS, _err, "pkcs12/bad-input", function (k) {
    return "unknown bag field " + JSON.stringify(k) +
      "; a bag is { type, ...the fields that type reads, friendlyName?, localKeyId? }";
  });
  switch (bag.type) {
    case "key": {
...
async function _buildAuthSafeElement(sc, opts) {
  ...
  // BEFORE the privacy dispatch below, because that dispatch reads presence: an unrecognized name
  // is not a missing field it can notice, it is a directive that was never seen.
  guard.identifier.assertKnownKeys(sc, _SAFE_KEYS, _err, "pkcs12/bad-input", function (k) {
    return "unknown safeContents field " + JSON.stringify(k) +
      "; a safeContents entry is { bags, encrypt? | recipients?, contentEncryptionAlgorithm? }";
  });
...
async function _build(spec, opts) {
  ...
  if (spec && typeof spec === "object" && !Buffer.isBuffer(spec)) {
    guard.identifier.assertKnownKeys(spec, _BUILD_SPEC_KEYS, _err, "pkcs12/bad-input", function (k) {
      return "unknown PKCS#12 spec field " + JSON.stringify(k) +
        "; the spec is { safeContents: [...] } or the shorthand { key, cert, ca }";
    });
  }
  guard.identifier.assertKnownKeys(opts, _BUILD_OPTS, _err, "pkcs12/bad-input", "pki.pkcs12.build has an unknown option ");
