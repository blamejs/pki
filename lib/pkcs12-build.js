...
async function verifyMac(pfx, password, opts) {
  ...
  if (m.mac.kind === "hmac") {
    ...
    _capWork(m.mac.iterations, m.mac.macSalt, opts);
    ...
    _capWork(kdf.iterationCount, kdf.salt, opts, kdf.keyLength);
...
function _capWork(iterations, salt, opts, keyLength) {
  ...
  if (keyLength != null && keyLength > MAX_PBMAC1_KEYLEN) throw _err("pkcs12/bad-input", "the PBMAC1 keyLength exceeds the " + MAX_PBMAC1_KEYLEN + "-octet cap");
}
