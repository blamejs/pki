...
async function _buildAuthSafeElement(sc, opts) {
  ...
  if (sc.encrypt && sc.recipients) throw _err("pkcs12/bad-input", "a safeContents cannot combine encrypt (password) and recipients (public-key) -- one ContentInfo is one privacy type (RFC 7292 sec. 4.1)");
...
async function _openEncryptedSafe(encSafe, pwBytes, opts, out, depth) {
  if (encSafe.type === "envelopedData") {
    ...
    var ci = b.sequence([b.oid(O("envelopedData")), b.explicit(0, b.raw(encSafe.envelopedDataDer))]);   // reconstruct the ContentInfo from the raw bytes (cms.decrypt re-parses strict DER)
    var res;
    try { res = await cms.decrypt(ci, { key: opts.recipientKey, cert: opts.recipientCert }, opts); }
