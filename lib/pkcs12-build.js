...
async function open(pfx, password, opts) {
  ...
  var i;
  for (i = 0; i < m.safeBags.length; i++) _openBag(m.safeBags[i], password, opts, out, 0);
  for (i = 0; i < m.encryptedSafes.length; i++) await _openEncryptedSafe(m.encryptedSafes[i], password, opts, out, 0);
  if (opts.keys === "crypto") {
...
function _openBag(bag, password, opts, out, depth) {
  switch (bag.type) {
    case "keyBag":
    ...
    case "pkcs8ShroudedKeyBag":
      out.keys.push({ pkcs8: _decryptShroudedKey(bag.encrypted, password, opts), encrypted: true, friendlyName: bag.friendlyName, localKeyId: bag.localKeyId });
      break;
    case "certBag":
    ...
    case "safeContentsBag":
      ...
      for (var n = 0; n < (bag.nested || []).length; n++) _openBag(bag.nested[n], password, opts, out, depth + 1);
      break;
    default:
...
var LEGACY_PBE = Object.assign(Object.create(null), {
  "pbeWithSHAAnd3-KeyTripleDES-CBC": { cipher: "des-ede3-cbc", keyLen: 24, ivLen: 8, hash: "sha1" },
  "pbeWithSHAAnd2-KeyTripleDES-CBC": { cipher: "des-ede-cbc", keyLen: 16, ivLen: 8, hash: "sha1" },
  "pbeWithSHAAnd128BitRC2-CBC": { rc2: 128, keyLen: 16, ivLen: 8, hash: "sha1" },
  "pbeWithSHAAnd40BitRC2-CBC": { rc2: 40, keyLen: 5, ivLen: 8, hash: "sha1" },
  "pbeWithSHAAnd128BitRC4": { rc4: true },
  "pbeWithSHAAnd40BitRC4": { rc4: true },
});
...
function _decryptLegacyPbe(ea, ct, password, opts) {
  var scheme = LEGACY_PBE[ea.name];
  ...
  var params = asn1.decode(ea.parameters);
  // Coverage residual -- the tagNumber!==SEQUENCE arm is exercised (a crafted 0x2f params tag); the !children
  // and length<2 arms are defensive-unreachable via a conforming decode (asn1.decode of a constructed node
  // always sets a children array, and every real pkcs-12PbeParams carries exactly the two salt+iterations).
  if (params.tagNumber !== asn1.TAGS.SEQUENCE || !params.children || params.children.length < 2) throw _err("pkcs12/bad-der", "malformed pkcs-12PbeParams (RFC 7292 App. C)");
  var salt = Buffer.from(asn1.read.octetString(params.children[0]));
  var iterations = guard.range.positiveInt31(asn1.read.integer(params.children[1]), _err, "pkcs12/bad-input", "legacy PBE iterations");
  ...
  if (scheme.rc2) return rc2.cbcDecrypt(keyM, scheme.rc2, iv, ct, _err, "pkcs12/decrypt-failed");   // RC2 is unavailable in node; the in-tree RFC 2268 primitive fills the gap
  try { var d = nodeCrypto.createDecipheriv(scheme.cipher, keyM, iv); return Buffer.concat([d.update(ct), d.final()]); }
...
function _decryptShroudedKey(encrypted, password, opts) {
  var der = _decryptBag(encrypted.encryptionAlgorithm, encrypted.encryptedData, password, opts);
...
async function _openEncryptedSafe(encSafe, password, opts, out, depth) {
  if (encSafe.type === "envelopedData") {
    ...
    for (var k = 0; k < envBags.length; k++) _openBag(envBags[k], password, opts, out, depth);
    return;
  }
  ...
  var safeContentsDer = _decryptBag(eci.contentEncryptionAlgorithm, eci.encryptedContent, password, opts);
  ...
  for (var i = 0; i < bags.length; i++) _openBag(bags[i], password, opts, out, depth);
}
