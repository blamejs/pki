  ...
  return Promise.resolve().then(function () {
    ...
    if (body.cms && body.cms.contentTypeName === "authData") {
      ...
      if (sent.recipient != null) {
        // The MAC is computed over BYTES, and pki.cms.decrypt deliberately refuses a
        // pre-parsed object so nothing can hand it a parse result that skipped DER
        // validation. So this needs the response as bytes; when the caller used the
        // already-parsed input form there are none to check, and saying exactly that
        // is the honest answer -- reporting "did not authenticate" would blame the
        // message for what is a missing input.
