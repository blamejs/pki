...
var OUTCOME_BY_STATUS = intrinsic.assign(intrinsic.create(null), {
  success: "issued",
  pending: "pending",
  partial: "pending",
...
var OUTCOME_SEVERITY = intrinsic.assign(intrinsic.create(null), { rejected: 4, "pop-required": 3, "confirm-required": 2, pending: 1, issued: 0 });
 ...
 *
 * The verdict carries `valid`, the toolkit-wide alias the other verify verbs use, true only for the
 * `issued` outcome. Every other outcome, including `pending`, is `valid: false`, and which one it
 * was stays in `outcome` alongside `failInfo` and `pendToken`.
 *
 ...
 *   - `recipient` (object) -- `{ identifier, secret }` for a response MACed under the shared secret
 *     the client enrolled with, which derives the key the way RFC 5272 sec. 3.2(c) states, or key
 *     material for an AuthenticatedData carrier, in the shape
...
function verify(response, sent) {
  var frozenResponse, frozenSent, ownedSecret = null;
  /** @internal The options snapshot owns a copy of a byte-valued shared secret from the moment it is
   * taken, so it is wiped when the verification settles however it settles: a malformed response, a
   * carrier that never needed the secret, a refusal on the way to the MAC, and a failure in the rest
   * of this prologue all reach here. A failure DURING the snapshot is cleared by the snapshot itself,
   * which is the only place that can still reach a half-built copy. The caller's own buffer is theirs
   * and is never touched. */
  function _wipeOwnedSecret() {
    if (ownedSecret) guard.secret.zeroize(ownedSecret, CmcError, "cmc/bad-input", "the shared-secret copy");
    ownedSecret = null;
  }
  try {
    frozenSent = _snapshotSent(_assertOpts(sent));
    ownedSecret = frozenSent.recipient && guard.bytes.isByteSource(frozenSent.recipient.secret)
      ? frozenSent.recipient.secret : null;
  ...
  return guard.async.deferred(function () { return _verify(frozenResponse, frozenSent); })
    .then(function (v) { _wipeOwnedSecret(); return v; },
      function (e) { _wipeOwnedSecret(); throw e; });
}
...
function _snapshotSent(sent) {
  var out = guard.identifier.ownOptions(sent);
  ...
  if (Array.isArray(out.bodyPartIDs)) out.bodyPartIDs = guard.list.snapshot(out.bodyPartIDs);
  if (Array.isArray(out.bodyPartPaths)) {
    out.bodyPartPaths = guard.list.copyMap(out.bodyPartPaths, function (p) { return Array.isArray(p) ? guard.list.snapshot(p) : p; });
  }
  /** @internal Once a copy of a shared secret exists, EVERYTHING that follows is protected, to the
   * return: the remaining copies, the last option reads, all of it. A caller field is read through
   * whatever accessor the caller wrote, so any of these steps can throw, and the copy would then be
   * left behind with nothing holding it. Guarding one step at a time is what let three earlier
   * versions of this each miss the next one. */
  var takenSecret = null;
  try {
    if (out.recipient && typeof out.recipient === "object") {
      var r = intrinsic.create(null);
      var rNames = guard.identifier.optionNames(out.recipient);
      for (var ri = 0; ri < rNames.length; ri++) {
        var rk = rNames[ri];
        r[rk] = _copyAnyBytes(out.recipient[rk]);
        if (rk === "secret" && guard.bytes.isByteSource(r[rk])) takenSecret = r[rk];
      }
      out.recipient = r;
    }
    if (Array.isArray(out.certs)) out.certs = guard.list.copyMap(out.certs, _copyAnyBytes);
    out.allowUnverified = sent.allowUnverified === true;
    out.allowUnbound = sent.allowUnbound === true;
    return out;
  } catch (e) {
    if (takenSecret) guard.secret.zeroize(takenSecret, CmcError, "cmc/bad-input", "the shared-secret copy");
    throw e;
  }
}
  ...
  return Promise.resolve().then(function () {
    ...
    if (body.cms && body.cms.contentTypeName === "authData") {
      if (sent.recipient != null) {
        ...
        /** @internal A caller naming the shared secret it enrolled with gets the sec. 3.2(c) derivation
         * from the same function the producer uses, rather than deriving it again by hand. Raw key
         * material is still taken as it was, which is what an implementation that derived the key
         * itself supplies. */
        var recipient = sent.recipient, derivedKey = null;
        /** @internal The concatenation this derivation owns. The secret copy it was built from belongs
         * to the options snapshot, which `verify` wipes when the verification settles. */
        function _wipeDerived() {
          if (derivedKey) guard.secret.zeroize(derivedKey, CmcError, "cmc/bad-input", "the derived shared-secret key");
          derivedKey = null;
        }
        if (recipient && typeof recipient === "object" && recipient.mac == null &&
            (recipient.identifier != null || recipient.secret != null) && recipient.password == null) {
          derivedKey = cmcBuild.macPassword(recipient);
          recipient = { password: derivedKey };
        }
        return cmsDecrypt.decrypt(responseBytes, recipient).then(function (r) { _wipeDerived(); return r; },
          function (e) {
            _wipeDerived();
 ...
 * is the `{ cert, key }` that signs the enclosing CMS SignedData. RFC 5272 sec. 3.2 gives a Full PKI
 * Request a second carrier for a client that has no key to sign with: pass
 * `{ mac: { identifier, secret } }` in place of `{ cert, key }` and the request is an
 * AuthenticatedData whose MAC the shared secret keys, through the PasswordRecipientInfo sec. 3.2(a)
 * requires. The key is derived from the identifier encoded as UTF-8 followed by the secret
 * (sec. 3.2(c)), which is the derivation `pki.cmc.verify` reads a `recipient` under, so a caller names
 * what it enrolled with at both ends rather than concatenating by hand. The request also carries the
 * identifier in an Identification control, because a PasswordRecipientInfo names no recipient and that
 * control is what "assists the server in locating the shared-secret" (RFC 5272 sec. 6.2.3); supplying
 * a different name through `spec.identityProof.identity` is refused rather than chosen between. Since
 * carrying that control also changes how an Identity Proof witness is derived (sec. 6.2.3), a request
 * carrying both derives the witness with the name it carries, and its MAC and its Identity Proof are
 * keyed from one secret rather than two. Naming a signature and a shared secret together is refused,
 * since a request has one carrier. A `tcr` request is verified against
 ...
 *
 * `spec.popChallenge: { challenge, recipient }` answers an authority's challenge-response proof of
 * possession, which is how a key that cannot sign for itself proves the client holds it (RFC 5272
 * sec. 6.7). `challenge` is the Encrypted POP control's value and `recipient` is the key material
 * `pki.cms.decrypt` takes. The proof value is decrypted, hashed under the witness algorithm the
 * challenge names and compared with the witness it carries; a value that does not decrypt or does not
 * match is not the one the authority issued, so the request is refused with `cmc/pop-failed` and no
 * proof is sent. The answer is a MAC over the request the challenge carried, keyed by that value
 * (truncated to 64 bytes, sec. 6.7) under the algorithm the challenge named, and it names the
 * certification request in THIS request, so a request carrying more than one is refused. The
 * algorithms are the ones RFC 5274 sec. 4.2 requires of a client: SHA-1 or SHA-256 for the witness,
 * HMAC-SHA1 or HMAC-SHA256 for the proof.
 *
 * A `tcr` request signed with `id-alg-noSignature` (RFC 5272 App. C.1) is the request a decrypt-only
 * key produces, and it carries no proof of its own. It is accepted only when this message answers a
 * challenge for that same request and the key that opened the challenge is the request's own subject
 * public key, compared as keys rather than as encodings. A challenge opened by a password or a
 * key-encryption key identifies no key and does not qualify, and its parameters must be present and
 * encoded as NULL. Without such an answer the request is refused with `cmc/bad-popo`, and a request
 * that does carry a signature is still held to it.
 *
 ...
 */
var cmcBuild = require("./cmc-build");
var build = cmcBuild.build;
