  ...
  if (Array.isArray(sent.bodyPartIDs)) {
    ...
    body.statuses.forEach(function (s) {
      (s.bodyList || []).forEach(function (ref) {
        ...
        // A bodyPartPath descends INTO a nested message. Its first element names a
        // body part in THIS request, which the retained set can speak to; every
        // element after that names one inside a message this client did not compose
        // and holds no identifiers for. Checking the head and waving the tail
        // through would accept `[a part we sent, 999]` -- a status about something
        // arbitrary, wearing a reference that passes. Since the tail cannot be
        // checked, a multi-element path is refused outright rather than half-checked.
        if (path && path.length > 1) {
          throw E("cmc/body-part-unknown",
            "a status control reports on a body part nested inside another message (path " + path.join("/") +
            "); this request carries no nested messages, so nothing here can confirm what that names " +
            "(RFC 5272 sec. 6.1.1)");
        }
...
function _snapshotSent(sent) {
  ...
  if (out.recipient && typeof out.recipient === "object") {
    ...
    for (rk in out.recipient) {
      ...
      var rv = out.recipient[rk];
      r[rk] = (Buffer.isBuffer(rv) || rv instanceof Uint8Array) ? Buffer.from(rv) : rv;
    }
    out.recipient = r;
  }
