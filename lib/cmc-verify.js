...
// Every operation this verb decides with, bound at load. Each is an ordinary writable property
// until it is captured, and the exchange binding below is built out of them: an `equals` answering
// true matches any Data Return against any other, an `isArray` answering false skips the body-part
// check over a real list, and `String` chooses what a body-part identifier renders as, which is
// what decides whether it is in the set the request retained. See guard-intrinsic.
...
// The settle half of the surface. This verb is documented Promise-returning, so a replaced
// `reject` that hands back a resolved promise turns a refusal into a pass at the one place the
// exit shape is decided.
...
// Null-prototyped, so a status name the table never held cannot be answered for by something
// Object.prototype carries.
var OUTCOME_BY_STATUS = _assign(_create(null), {
  success: "issued",
  pending: "pending",
  partial: "pending",
...
var OUTCOME_SEVERITY = _assign(_create(null),
  { rejected: 4, "pop-required": 3, "confirm-required": 2, pending: 1, issued: 0 });
...
function _bagOf(parsedCms, originatorKey, topKey) {
  ...
  return _map(list || [], function (c) { return c.bytes; });
}
...
function _assertBound(body, sent) {
  ...
  function _carries(v) {
    ...
    if (_isBuffer(v) || v instanceof Uint8Array) return v.length > 0;
  ...
  var bound = {
    ...
    bodyPartIDs: _isArray(sent.bodyPartIDs),
  };
  ...
  if (sent.senderNonce != null) {
    ...
    if (!guard.crypto.constantTimeEqual(_octets(rn, "Recipient Nonce"), _bufferFrom(sent.senderNonce))) {
  ...
  if (sent.dataReturn != null) {
    ...
    if (!_bufEquals(_octets(dr, "Data Return"), _bufferFrom(sent.dataReturn))) {
  ...
  function _pathRetained(retained, want) {
    if (!_isArray(retained)) return false;
    return _some(retained, function (have) {
      return _isArray(have) && have.length === want.length && _every(have, function (seg, i) {
        return _String(seg) === _String(want[i]);
      });
    });
  }
  if (_isArray(sent.bodyPartIDs)) {
    var known = _create(null);
    _forEach(sent.bodyPartIDs, function (id) { known[_String(id)] = true; });
    ...
    known["0"] = true;
    _forEach(body.statuses, function (s) {
      _forEach(s.bodyList || [], function (ref) {
        ...
        if (path && path.length > 1) {
          ...
          throw E("cmc/body-part-unknown",
            "a status control reports on a body part nested inside another message (path " + _join(path, "/") +
        ...
        if (id == null || known[_String(id)]) return;
        throw E("cmc/body-part-unknown",
...
function _assertOpts(sent) {
  if (sent == null) return {};
  if (typeof sent !== "object" || _isArray(sent) || _isBuffer(sent)) {
...
function _snapshotIfBytes(input) {
  ...
  if (_isView(input) || input instanceof ArrayBuffer) {
...
function _snapshotSent(sent) {
  ...
  if (_isArray(out.bodyPartIDs)) out.bodyPartIDs = _arraySlice(out.bodyPartIDs);
  ...
  if (_isArray(out.bodyPartPaths)) {
    out.bodyPartPaths = _map(out.bodyPartPaths, function (p) { return _isArray(p) ? _arraySlice(p) : p; });
  }
  ...
  if (_isArray(out.certs)) out.certs = _map(out.certs, _copyAnyBytes);
...
function _assertAuthentic(body, sent, responseBytes) {
  return _promiseResolve(_Promise).then(function () {
    ...
    var haveBytes = _isBuffer(responseBytes) || responseBytes instanceof Uint8Array ||
    ...
    if (body.cms && body.cms.contentTypeName === "authData") {
      ...
      if (sent.recipient != null) {
        ...
        return cmsDecrypt.decrypt(responseBytes, sent.recipient).then(null, function (e) {
          ...
          if (!eContent || !_isBuffer(authed) || !guard.crypto.constantTimeEqual(authed, eContent)) {
            throw E("cmc/unverified-response",
    ...
    return cmsVerify.verify(responseBytes, sent.certs && sent.certs.length ? { certs: sent.certs } : {})
      .then(function (res) {
        ...
        var failed = _filter(res.signers || [], function (s) { return !s.ok; });
        var code = failed.length ? failed[0].code : null;
        var onlyMissing = failed.length > 0 && _every(failed, function (s) {
...
function _shape(body, sent, signatureVerified, bound) {
  ...
  return {
    ...
    cmsSequence: _arraySlice(body.cmsSequence || []),
    otherMsgs: _arraySlice(body.otherMsgs || []),
    publishTrustAnchors: anchors ? _arraySlice(anchors.values) : null,
