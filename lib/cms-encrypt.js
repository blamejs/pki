...
var pkiBuild = require("./pki-build");   // reqDenseArray: reject a sparse/nullish caller array as a typed cms/bad-input
var oid = require("./oid");
...
function encrypt(content, recipients, opts) {
  // The options and recipients are copied BEFORE the content is classified. Classifying reads
  // content[Symbol.asyncIterator] -- a caller accessor that runs in-realm -- which could otherwise
  // change the content type, the output encoding, or a recipient after the call has begun. With the
  // copies taken first, that probe cannot reach them; the streamed sign path copies the same way. The
  // copy runs inside guard-async's boundary, so a fault in it leaves as a rejection, and the copies
  // (which may hold a symmetric key or a password) are cleared on every path out.
...
function _assertAuthAttrs(pairs, forbidden) {
  // Reject a sparse/nullish attribute array as a typed error before b.setOf reaches it as a native one.
  // This gate runs before the setOf on both the AuthEnveloped and AuthenticatedData paths.
  // Validate AND SNAPSHOT: reqDenseArray returns a fresh dense copy, and it is that copy the caller must
  // iterate and encode. Iterating the original `pairs` would let an indexed getter that sparsified it
  // during validation slip a hole into b.setOf as a native error; the returned snapshot cannot.
