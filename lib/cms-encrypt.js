...
async function encrypt(content, recipients, opts) {
  ...
  // The output encoding is decided at the door and every emission path reads this snapshot, not the
  // live option. A streamed plaintext drives caller-controlled iterator code during the encrypting
  // await, so a stream (or concurrent caller code) that set opts.pem after the call began would
  // otherwise flip a DER Buffer into a PEM string after the format was meant to be fixed.
  var pem = opts.pem;
  ...
  try {
    ...
    if (!Array.isArray(recipients)) return await _encryptedData(contentBytes, contentStream, recipients, ca, contentType, opts, cek, pem);
    ...
    if (ca.aead) return _emit(await _authEnvelopedData(contentBytes, contentStream, cek, ca, contentType, opts, riNodes, recips), "authEnvelopedData", pem);
    return _emit(await _envelopedData(contentBytes, contentStream, cek, ca, contentType, riNodes, recips), "envelopedData", pem);
  } finally {
    guard.secret.zeroize(cek, CmsError, "cms/bad-input", "the content-encryption key");
  }
}

function _emit(inner, ctName, pem) {
  var ci = b.sequence([b.oid(O(ctName)), b.explicit(0, inner)]);
  return pem ? schemaCms.pemEncode(ci, "CMS") : ci;
}
...
async function _encryptedData(contentBytes, contentStream, desc, ca, contentType, opts, cek, pem) {
  ...
  if (desc && desc.cek != null) {
    ...
    return _encryptedDataPbes2(contentBytes, contentStream, desc, ca, contentType, iv, pem);
  } else {
  ...
  return _emit(inner, "encryptedData", pem);
}

async function _encryptedDataPbes2(contentBytes, contentStream, desc, ca, contentType, iv, pem) {
  // A string / Uint8Array password is encoded into a buffer this toolkit allocated, a credential
  // copy, so it is cleared once the derivation has consumed it. A caller-supplied Buffer is
  // borrowed and left intact.
