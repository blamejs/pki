...
var _CERT_RECIPIENT_KEYS = { cert: 1, oaepHash: 1, keyIdentifier: 1, ukm: 1 };
...
async function _buildRecipient(cek, desc, opts) {
  ...
  // one belonging to the arm that will build.
  ...
  if (desc.cert != null) {
    _assertRecipientKeys(desc, _CERT_RECIPIENT_KEYS, "certificate");
...
async function encrypt(content, recipients, opts) {
  opts = opts || {};
  guard.identifier.assertKnownKeys(opts, _ENCRYPT_OPTS, _err, "cms/bad-input", "pki.cms.encrypt has an unknown option ");
...
// The single descriptor an EncryptedData takes. `cek` and `password` select the arm; the rest are
// PBE parameters that default, so a misspelled one derives the key at the shipped work factor.
var _ENCRYPTED_DATA_KEYS = { cek: 1, password: 1, iterations: 1, salt: 1, prf: 1, pem: 1 };
...
function _encryptedData(contentBytes, desc, ca, contentType, opts, cek) {
  ...
  if (desc != null && typeof desc === "object") {
    guard.identifier.assertKnownKeys(desc, _ENCRYPTED_DATA_KEYS, _err, "cms/bad-input",
      "an EncryptedData descriptor has an unknown field ");
  }
  var iv = nodeCrypto.randomBytes(16);
...
async function authenticate(content, recipients, opts) {
  opts = opts || {};
  guard.identifier.assertKnownKeys(opts, _AUTHENTICATE_OPTS, _err, "cms/bad-input", "pki.cms.authenticate has an unknown option ");
  var contentBytes = guard.bytes.view(content, CmsError, "cms/bad-input", "content");
