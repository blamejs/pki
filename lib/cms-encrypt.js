...
async function _buildKemri(cek, cert, opts) {
  ...
  try {
    ...
    kek = Buffer.from(await subtle.deriveBits({ name: "HKDF", hash: "SHA-256", salt: Buffer.alloc(0), info: _kemOtherInfo(wrapName, kekBytes, ukm) }, ssKey, kekBytes * 8));
    var encryptedKey = await _aesKwWrap(kek, cek);
    var rid = _rid(cert, opts.keyIdentifier);
    ...
    guard.secret.zeroizeAll([ss, kek, kem.sharedKey ? new Uint8Array(kem.sharedKey) : null],
