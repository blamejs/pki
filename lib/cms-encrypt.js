...
async function _buildKtri(cek, cert, opts) {
  ...
  if (!OAEP_HASH[hashName]) throw _err("cms/bad-input", "unsupported oaepHash " + guard.text.showValue(hashName));
...
function _prfHash(prf) { if (!PRF_HASH[prf]) throw _err("cms/bad-input", "unsupported pwri prf " + guard.text.showValue(prf)); return PRF_HASH[prf]; }
...
async function authenticate(content, recipients, opts) {
  ...
  try {
    ...
    if (withAttrs) {
      ...
      if (!SUPPORTED_DIGEST[digestName]) throw _err("cms/bad-input", "unsupported digestAlgorithm " + guard.text.showValue(digestName) + " (sha256/384/512)");
