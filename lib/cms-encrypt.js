...
async function _buildPwri(cek, desc) {
  ...
  var iterations = desc.iterations == null ? 600000 : desc.iterations;
  if (typeof iterations !== "number" || !isFinite(iterations) || iterations < 1) throw _err("cms/bad-input", "pwri iterations must be a positive integer");
...
function _encryptedData(contentBytes, desc, ca, contentType, opts, cek) {
  ...
  if (desc && desc.cek != null) {
    ...
    return _encryptedDataPbes2(contentBytes, desc, ca, contentType, iv);
  } else {
...
function _encryptedDataPbes2(contentBytes, desc, ca, contentType, iv) {
  var password = _passwordBytes(desc.password);
  var iterations = desc.iterations == null ? 600000 : desc.iterations;
  var salt = desc.salt ? Buffer.from(desc.salt) : nodeCrypto.randomBytes(16);
  var prf = desc.prf || "hmacWithSHA256";
  ...
  var inner = b.sequence([b.integer(0n), eci]);
  return _emit(inner, "encryptedData", { pem: desc.pem });
}
