...
async function authenticate(content, recipients, opts) {
  ...
  if (withAttrs) {
    ...
    // Attributes are unique per type (RFC 5652): a caller-supplied authAttr must not duplicate the
    // auto-built content-type / message-digest or another custom attribute, and must be well-formed.
    var seenTypes = {};
    pairs.forEach(function (p) {
      var t;
      try { t = asn1.read.oid(asn1.decode(p).children[0]); }
      catch (e) { throw _err("cms/bad-input", "an authenticated attribute is not a well-formed Attribute SEQUENCE", e); }
