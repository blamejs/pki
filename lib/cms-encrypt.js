...
function _rid(cert, form) {
  ...
  // Reject an unrecognized keyIdentifier rather than silently emitting issuerAndSerialNumber -- a typo
  // would otherwise produce a rid the caller never asked for.
  if (form != null && form !== "issuerAndSerialNumber") throw _err("cms/bad-input", "unsupported keyIdentifier " + JSON.stringify(form) + " (use \"issuerAndSerialNumber\" or \"subjectKeyIdentifier\")");
...
// A PBKDF2 iterationCount MUST be a positive integer -- a fractional / non-finite / <1 value would
// otherwise reach node's pbkdf2 (or BigInt() on encode) as a native RangeError instead of a typed error.
...
async function _buildPwri(cek, desc) {
  ...
  var salt = desc.salt ? guard.bytes.view(desc.salt, CmsError, "cms/bad-input", "salt") : nodeCrypto.randomBytes(16);
