...
var COMPOSITE_KEM = Object.create(null);
...
/** @internal The encoding is this helper's own allocation and holds the traditional private scalar, so
 * it is cleared here. A caller's own cleanup cannot reach a buffer it never sees, and both the
 * derivation and the decapsulation reach the curve through this one function. */
function _okpPrivFromRaw(d, raw) {
  ...
  try {
    ...
    guard.secret.zeroize(p8, KemError, "kem/bad-input", "the " + d.trad.curve + " component key encoding");
  }
}
...
function _tradDecaps(d, tradSK, tradCT) {
  ...
  /** @internal The same canonical point the derivation states, read from the one function that spells
   * it. The combiner hashes these bytes, so a stored spelling here and a generated one there would
   * reach different secrets from the two sides of one key. */
  var tradPK;
  try {
    tradPK = trad.kind === "ec" ? _ecPointOf(d, tradSK) : _rawPubOf(nodeCrypto.createPublicKey(priv2));
  } catch (e) {
    /** @internal The agreement already produced a secret, and this rejection happens after it, so the
     * secret is wiped here: the caller's settle-or-wipe covers a value it was handed, and a rejected
     * promise hands it nothing. */
    guard.secret.zeroize(dh, KemError, "kem/bad-input", "the " + trad.curve + " component secret");
    return Promise.reject(e);
  }
  return Promise.resolve({ ss: dh, tradPK: tradPK });
}
...
function _isCompositeKem(algOid) { return COMPOSITE_KEM[algOid] != null; }

/** @internal The EC component's public point, generated from its scalar. RFC 5915 sec. 3 gives an
 * ECPrivateKey an optional public point which may be written compressed, and the key engine keeps
 * whatever encoding it was handed. The composite concatenation carries the uncompressed point at a
 * fixed width (draft sec. 4.1), and the combiner hashes those bytes, so BOTH the derivation and the
 * decapsulation read this one function: a point spelled two ways would agree on the secret nowhere. */
function _ecPointOf(d, tradSK) {
  /** @internal The scalar is a fresh copy of private key material, so it is wiped here rather than
   * left for the caller's own copies to cover: those wipe the PKCS#8 and the key octets, and this
   * buffer is neither. */
  var scalar = _ecScalarOf(d, tradSK);
  var point;
  try {
    var ecdh = nodeCrypto.createECDH(d.trad.node);
    ecdh.setPrivateKey(scalar);
    point = ecdh.getPublicKey(null, "uncompressed");
  } catch (e) {
    if (e instanceof KemError) throw e;
    throw _err("kem/bad-key", "the EC component scalar does not generate a point on " + d.trad.node, e);
  } finally {
    guard.secret.zeroize(scalar, KemError, "kem/bad-input", "the EC component scalar");
  }
  if (point.length !== d.trad.ctLen) throw _err("kem/bad-key", "the EC component point is not the expected " + d.trad.ctLen + " bytes for " + d.name);
  return point;
}

/** @internal The private scalar of a SEC1 ECPrivateKey (RFC 5915 sec. 3), left-padded to the field
 * width the curve uses. The structure states the scalar as an OCTET STRING in its second field, and
 * the point generation below is positional, so a short one is padded rather than shifted. */
function _ecScalarOf(d, tradSK) {
  var width = (d.trad.ctLen - 1) / 2;
  var raw;
  try {
    var node = asn1.decode(tradSK);
    raw = Buffer.from(asn1.read.octetString(node.children[1]));
  } catch (e) { throw _err("kem/bad-key", "the EC component private key is not a well-formed ECPrivateKey", e); }
  /** @internal `raw` holds private key material. It is handed on only when it IS the returned buffer,
   * which the caller then wipes; on every other exit, the padded one and the refusal alike, this is
   * the last reference and the copy is cleared here. */
  var handedOn = false;
  try {
    if (raw.length === 0 || raw.length > width) throw _err("kem/bad-key", "the EC component scalar is not a " + d.trad.node + " field element");
    if (raw.length === width) { handedOn = true; return raw; }
    var out = Buffer.alloc(width);
    raw.copy(out, width - raw.length);
    return out;
  } finally {
    if (!handedOn) guard.secret.zeroize(raw, KemError, "kem/bad-input", "the EC component scalar copy");
  }
}

/** @internal The traditional component's public half, derived from its private half by the same route
 * `_tradDecaps` takes to report `tradPK`. Each kind states its private key in its own encoding: an
 * RSAPrivateKey, a SEC1 ECPrivateKey, or the raw scalar an OKP curve carries. */
function _tradPubOf(d, tradSK) {
  var trad = d.trad;
  if (trad.kind === "rsa") {
    var rp;
    try {
      asn1.decode(tradSK);
      rp = nodeCrypto.createPrivateKey({ key: tradSK, format: "der", type: "pkcs1" });
    } catch (e) { throw _err("kem/bad-key", "the RSA component private key is not a valid RSAPrivateKey", e); }
    if (rp.asymmetricKeyDetails.modulusLength !== trad.bits) {
      throw _err("kem/bad-key", "the RSA component modulus is not " + trad.bits + " bits (draft sec. 6)");
    }
    return Buffer.from(nodeCrypto.createPublicKey(rp).export({ format: "der", type: "pkcs1" }));
  }
  var priv;
  try {
    if (trad.kind === "ec") {
      asn1.decode(tradSK);
      priv = nodeCrypto.createPrivateKey({ key: tradSK, format: "der", type: "sec1" });
    } else {
      priv = _okpPrivFromRaw(d, tradSK);
    }
  } catch (e) { throw _err("kem/bad-key", "the " + trad.curve + " key material is invalid", e); }
  /** @internal A SEC1 ECPrivateKey names its own curve, and the composite OID names the curve the
   * component must be on. Exporting the point of one curve under an OID that names another states a
   * public key nothing can use, so the two are held to each other here. Decapsulation reaches the same
   * conclusion by failing the agreement; a derivation performs no operation that would notice. */
  if (trad.kind === "ec") {
    var named = priv.asymmetricKeyDetails && priv.asymmetricKeyDetails.namedCurve;
    if (named !== trad.node) {
      throw _err("kem/bad-key", "the EC component is on " + guard.text.showValue(named) + ", and " + d.name + " names " + trad.node);
    }
    /** @internal Recomputed from the scalar rather than exported. RFC 5915 gives an ECPrivateKey an
     * optional public point, the key engine keeps whatever encoding it was handed, and a compressed
     * point is neither the width nor the form the composite concatenation carries (draft sec. 4.1).
     * Generating the point from the scalar answers with the key this component actually holds, so a
     * point stored beside it cannot stand in for one it does not generate. */
    return _ecPointOf(d, tradSK);
  }
  return _rawPubOf(nodeCrypto.createPublicKey(priv));
}

/** @internal The composite `SubjectPublicKeyInfo` for a composite ML-KEM private key. The runtime key
 * decoder does not know a composite OID, so a caller deriving one cannot go through it; the key is
 * split the way `_decapsulate` splits it (draft sec. 4.1: `seed || tradSK`) and each half states its
 * own public key, concatenated under the same OID as `mlkemEK || tradPK` (sec. 5.1). The ML-KEM half
 * is recovered by importing the seed and reading the public key it generates, so a stored public copy
 * could not answer in its place. The private material is wiped on every path. */
async function _publicFromPrivate(privateKey) {
  var pkcs8 = _toDer(privateKey, "PRIVATE KEY", "the composite ML-KEM private key");
  /** @internal Every buffer this derivation allocates that holds private key material is registered
   * here and cleared once, on every exit. A copy added later is covered by registering it rather than
   * by a further nested block, which is how the ML-KEM seed encoding came to outlive two of them. */
  var secrets = [];
  guard.list.append(secrets, pkcs8);
  var d;
  try {
    var sk;
    try {
    ...
    var ek;
    try {
      /** @internal The seed re-encoded as a OneAsymmetricKey the key engine reads, which is a second
       * copy of the seed and is registered as one. */
      var seedPkcs8 = _mlkemSeedPkcs8(d, seed);
      guard.list.append(secrets, seedPkcs8);
      var mlPriv = nodeCrypto.createPrivateKey({ key: seedPkcs8, format: "der", type: "pkcs8" });
      ek = _rawPubOf(nodeCrypto.createPublicKey(mlPriv));
    } catch (e) {
      if (e instanceof KemError) throw e;
      throw _err("kem/bad-key", "the ML-KEM component private key is invalid", e);
    }
    if (ek.length !== d.mlkemEk) throw _err("kem/bad-key", "the ML-KEM component public key is not the expected " + d.mlkemEk + " bytes for " + d.name);
    var composite = Buffer.concat([ek, _tradPubOf(d, tradSK)]);
    return _b.sequence([_b.sequence([_b.oid(oid.byName(d.name))]), _b.bitString(composite, 0)]);
  } finally {
    guard.secret.zeroizeAll(secrets, KemError, "kem/bad-input", "the composite private key material");
  }
}
...
module.exports = {
  ...
  isCompositeKem: _isCompositeKem,
  publicFromPrivate: _publicFromPrivate,
};
