...
function _ecPointOf(d, tradSK) {
  ...
  try {
    ...
    ecdh.setPrivateKey(_ecScalarOf(d, tradSK));
...
async function _publicFromPrivate(privateKey) {
  ...
  try {
    ...
    try {
      ...
      var ek;
      try {
        var mlPriv = nodeCrypto.createPrivateKey({ key: _mlkemSeedPkcs8(d, seed), format: "der", type: "pkcs8" });
        ek = _rawPubOf(nodeCrypto.createPublicKey(mlPriv));
      } catch (e) { throw _err("kem/bad-key", "the ML-KEM component private key is invalid", e); }
