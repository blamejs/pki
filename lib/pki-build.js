...
// A caller-supplied array a builder is about to map over: reject a sparse hole or a nullish entry as a
// typed error. Array.prototype.map SKIPS a hole, which would otherwise reach Buffer.concat as a native
// ERR_INVALID_ARG_TYPE and, on a huge sparse length, traverse the empty slots. Tests OWN-property presence
// (intrinsic.hasOwn) at each index BEFORE reading it, so a polluted Array.prototype[i] cannot slip a hole
// past the check, and fails fast at the first bad index so the length alone can never force a walk. Takes
// the caller's error factory E (invoked E(fullCode, msg)) so a builder WITHOUT a makeBuilder instance
// routes its caller arrays through the same choke point; makeBuilder's own reqDenseArray delegates here.
function reqDenseArrayImpl(list, what, E, code) {
  if (!_isArray(list)) throw E(code, what + " must be an array");
  var n = list.length;
  // Snapshot each own index into a FRESH dense array and return THAT, never `list`. Reading list[i] can
  // fire a caller-defined indexed accessor that mutates `list` (extending it with holes past the captured
  // n, or deleting an already-checked index); returning `list` would then hand the caller's map a
  // newly-sparse array and the native error this guard exists to prevent. The copy captures the value as
  // read, so a later mutation of `list` cannot reintroduce a hole into what the caller iterates.
  var out = [];
  for (var i = 0; i < n; i++) {
    ...
    // defineProperty, not `out[i] = v`: a plain assignment invokes an inherited Array.prototype[i] SETTER
    // ([[Set]]) if one is polluted in, which could leave `out` sparse; [[DefineOwnProperty]] always makes
    // an own data property, so the snapshot is dense regardless of prototype pollution.
...
function makeBuilder(ctx) {
  ...
  // The bound 2-arg form every makeBuilder consumer calls (`_b.reqDenseArray(list, what)`); it delegates
  // to the standalone guard above with this builder's namespace, so the density rule lives in ONE place.
  function reqDenseArray(list, what) { return reqDenseArrayImpl(list, what, rawErr, ctx.prefix + "/bad-input"); }
  // A caller-supplied DER a builder validates with a PEM-accepting parser and then embeds verbatim with
  // b.raw. Those parsers unwrap PEM from a byte source, so a PEM-armored input (a Buffer straight from
  // fs.readFileSync) passes the parse, but b.raw embeds the raw ASCII armor -- which the enclosing
  // structure's own parse rejects only later, after protection is computed, and with the wrong code.
  // Require a DER SEQUENCE first (a Certificate / CRL / CSR / PKIMessage / CertReqMessages is one; PEM
  // opens with 0x2D), so a PEM or non-DER input is a typed bad-input up front and the bytes the parse
  // checks are the bytes embedded.
...
// Frozen so a co-resident cannot reassign an exported guard (e.g. reqDenseArray) to bypass it: the
// standalone consumers read reqDenseArray off this object at call time, so the object holding it must be
// immutable, the way the guard-* modules freeze their surfaces.
// Frozen so a co-resident cannot reassign an exported guard (e.g. reqDenseArray) to bypass it: the
// standalone consumers read reqDenseArray off this object at call time, so the object holding it must be
// immutable, the way the guard-* modules freeze their surfaces.
