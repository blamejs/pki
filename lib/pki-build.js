...
function makeBuilder(ctx) {
  ...
  // A caller-supplied array a builder is about to map over: reject a sparse hole or a nullish entry HERE
  // as a typed error. Array.prototype.map SKIPS a hole, which would otherwise reach Buffer.concat as a
  // native ERR_INVALID_ARG_TYPE and, on a huge sparse length, traverse the empty slots. Test OWN-property
  // presence at each index BEFORE reading it: a bare `list[i]` on a hole walks the prototype chain, so a
  // polluted `Array.prototype[i]` would read as a value and slip a hole past the check while map still
  // skips it. Fails fast at the first bad index, so the length alone can never force a walk. Returns the
  // validated dense array for the caller to map normally.
  function reqDenseArray(list, what) {
    if (!_isArray(list)) throw E("bad-input", what + " must be an array");
    var n = list.length;
    for (var i = 0; i < n; i++) {
      if (!intrinsic.hasOwn(list, i)) throw E("bad-input", what + "[" + i + "] is missing (a sparse array is not allowed)");
      var v = list[i];
      if (v === undefined || v === null) throw E("bad-input", what + "[" + i + "] is missing (a nullish array entry is not allowed)");
    }
    return list;
  }
