...
function _validUriAuthority(s, from, to) {
  ...
  // A non-empty host is required: RFC 5280 sec. 4.2.1.6 says a URI SAN with an authority MUST carry an
  // FQDN or IP host, so an empty-host authority (e.g. the "file:///path" form RFC 3986 generic syntax
  // would allow) is non-conformant for a certificate and uses the object form instead.
...
// Every dot-separated label in [from,to) satisfies `labelFn`. Scans with indexOf; no split allocation.
function _labelsValid(s, from, to, labelFn) {
  var start = from, dot;
  for (;;) {
...
function _looksLikeDnsName(s) {
  ...
  return _labelsValid(s, start, n, _isDnsLabelRange);
}
...
function _looksLikeEmail(s) {
  ...
  return _labelsValid(s, dStart, s.length, _isHostLabelRange);                          // strict LDH, no underscore
}
