...
function makeBuilder(ctx) {
  ...
  function atvString(attrName, value) {
    if (attrName === "countryName") {
      if (_String(value).length !== 2) throw E("bad-name", "countryName must be a two-letter ISO 3166 code (PrintableString SIZE(2))");
  ...
  function encodeAtv(attrName, value) {
    ...
    if (typeOid == null) throw E("bad-name", "unknown distinguished-name attribute " + _stringify(attrName));
    var valueTlv;
  ...
  function encodeRdn(rdnSpec) {
    if (!rdnSpec || typeof rdnSpec !== "object" || _isBuffer(rdnSpec)) throw E("bad-name", "each RDN must be an object of { attributeName: value }");
    var keys = _keys(rdnSpec);
  ...
  function encodeName(spec) {
    if (_isBuffer(spec)) { assertValidNameDer(spec); return spec; }
  ...
  function ia5Content(s) {
    ...
    for (var i = 0; i < s.length; i++) {
      if (s.charCodeAt(i) > 0x7F) throw E("bad-input", "value requires 7-bit ASCII (IA5String): " + _stringify(s));
    }
    return _bufferFrom(s, "latin1");
  }
  function encodeGeneralName(entry) {
    if (!entry || typeof entry !== "object" || _isBuffer(entry)) throw E("bad-input", "a GeneralName must be an object with exactly one name form");
    var keys = _keys(entry);
    ...
    switch (k) {
      ...
      case "iPAddress":
        ...
        if (!_isBuffer(ipBuf) || (ipBuf.length !== 4 && ipBuf.length !== 16)) throw E("bad-input", "iPAddress must be a 4- or 16-octet Buffer or an IPv4/IPv6 string");
      ...
      case "otherName":
        ...
        // never written.
        if (typeof v !== "object" || _isBuffer(v)) throw E("bad-input", "otherName must be an object { typeId, value }");
        if (typeof v.typeId !== "string" || !v.typeId) throw E("bad-input", "otherName requires a `typeId` OID string");
        if (!_isBuffer(v.value) || v.value.length === 0) {
        ...
        catch (e) { throw E("bad-input", "invalid otherName type-id OID " + _stringify(v.typeId) + " (violates the X.660 arc bounds)", e); }
        return b.contextConstructed(0, _bufferConcat([typeIdDer, b.explicit(0, v.value)]));
      default: throw E("bad-input", "unsupported GeneralName form " + _stringify(k) + " (supported: rfc822Name, dNSName, uniformResourceIdentifier, iPAddress, directoryName, otherName)");
    }
  }
  ...
  function extKeyUsage(names) {
    ...
    var positions = names.map(function (n) {
      var pos = KU_BIT[n];
      if (pos == null) throw E("bad-input", "unknown keyUsage bit " + _stringify(n));
      return pos;
    });
  ...
  function _resolveOid(n, label) {
    ...
    if (typeof n === "string" && DOTTED_OID_RE.test(n)) {
      ...
      try { b.oid(n); }
      catch (e) { throw E("bad-input", "invalid " + label + " OID " + _stringify(n) + " (violates the X.660 arc bounds)", e); }
      return n;
    }
    throw E("bad-input", "unknown " + label + " " + _stringify(n) + " (expected a registered name or a dotted-decimal OID)");
  }
  ...
  function extBasicConstraints(spec) {
    var children = [];
    if (spec.cA === true) _push(children, b.boolean(true));   // cA=FALSE omitted (DER DEFAULT)
    if (spec.pathLen != null) _push(children, b.integer(pathLen(spec.pathLen)));
    return b.sequence(children);
  }
  function pathLen(v) {
    if (typeof v !== "number" || !_isFinite(v) || v < 0 || (v | 0) !== v) throw E("bad-input", "basicConstraints pathLenConstraint must be a non-negative integer");
    return _BigInt(v);
  }
  ...
  function encodeGeneralNames(entries, implicitTag) {
    ...
    return b.contextConstructed(implicitTag, _bufferConcat(members));
  }
  ...
  function extCertPolicies(names) {
    ...
    return b.sequence(names.map(function (n) {
      ...
      if (seen[pOid]) throw E("bad-input", "duplicate certificate policy " + _stringify(n) + " (RFC 5280 sec. 4.2.1.4)");
      seen[pOid] = true;
  ...
  function ext(oidStr, critical, valueDer) {
    ...
    if (critical) _push(children, b.boolean(true));
    _push(children, b.octetString(valueDer));
    return b.sequence(children);
  }
  ...
  function skiKeyId(val, spkiDer) {
    if (_isBuffer(val)) return guard.bytes.snapshot(val, ErrorClass, ctx.prefix + "/bad-input", "subjectKeyIdentifier");
  ...
  function reqDer(v, what) {
    if (_isBuffer(v) || v instanceof Uint8Array) return guard.bytes.snapshot(v, ErrorClass, ctx.prefix + "/bad-input", what);
  ...
  function requestedExtensions(extSpec, spki) {
    ...
    guard.identifier.assertKnownKeys(extSpec, REQ_EXT_KEYS, E, "bad-input", function (k) {
      return "unknown requested extension " + _stringify(k) + "; pass a pre-encoded Extension DER via the array form for a custom extension";
    });
    var out = [];
    if (extSpec.subjectKeyIdentifier != null) {
      ...
      _push(out, ext(O("subjectKeyIdentifier"), false, extSki(skiKeyId(extSpec.subjectKeyIdentifier, spki))));
    }
    if (extSpec.keyUsage != null) _push(out, ext(O("keyUsage"), extSpec.keyUsageCritical !== false, extKeyUsage(extSpec.keyUsage)));
    if (extSpec.extendedKeyUsage != null) _push(out, ext(O("extKeyUsage"), !!extSpec.extendedKeyUsageCritical, extExtKeyUsage(extSpec.extendedKeyUsage)));
    if (extSpec.basicConstraints != null) { validateBcSpec(extSpec.basicConstraints); _push(out, ext(O("basicConstraints"), extSpec.basicConstraints.critical !== false, extBasicConstraints(extSpec.basicConstraints))); }
    if (extSpec.subjectAltName != null) _push(out, ext(O("subjectAltName"), false, extSan(extSpec.subjectAltName)));
    if (extSpec.certificatePolicies != null) _push(out, ext(O("certificatePolicies"), !!extSpec.certificatePoliciesCritical, extCertPolicies(extSpec.certificatePolicies)));
  ...
  function serialInteger(serial) {
    ...
    if (serial == null) {
      ...
      v = _BigInt("0x" + rnd.toString("hex"));
    } else if (typeof serial === "bigint") { v = serial; }
    else if (typeof serial === "number") { if (!_isSafeInteger(serial)) throw E("bad-serial", "serialNumber number must be a safe integer (pass a BigInt, hex string, or Buffer for a value above 2^53-1)"); v = _BigInt(serial); }
    else if (typeof serial === "string") { try { v = _BigInt(serial); } catch (e) { throw E("bad-serial", "serialNumber string must be a decimal or 0x-hex integer", e); } }
