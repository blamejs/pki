    ...
    switch (k) {
      ...
      case "otherName":
        ...
        if (!Buffer.isBuffer(v.value) || v.value.length === 0) {
          throw E("bad-input", "otherName requires a `value` Buffer holding one complete DER element");
        }
        // The value is spliced in raw, so it is DECODED before it is wrapped rather than
        // taken on the caller's word. A non-empty Buffer is not an element: two concatenated
        // TLVs, or bytes that are not DER at all, would otherwise be sealed inside a [0]
        // EXPLICIT wrapper whose contract says exactly one -- and this verb SIGNS the result,
        // so the malformed encoding would go out under a real signature. The strict decoder
        // rejects trailing bytes, which is what makes "exactly one" checkable here.
        try { asn1.decode(v.value); }
        catch (e) { throw E("bad-input", "otherName `value` must be exactly one complete DER element", e); }
