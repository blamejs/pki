...
// A reg-name (host) byte: unreserved / sub-delim (percent-encoding is handled separately). No ":" or "@".
...
// excluded, so an IPv6-literal host is refused by the shorthand and uses the object form.
...
// (userinfo and reg-name each exclude it), a non-empty reg-name host (an IP-literal host, needing "[" "]",
// is out of scope and uses the object form), and an all-digit -- possibly empty -- port after the host's
// single ":" (a reg-name has no ":"). Each component's bytes are validated against its own RFC 3986 class.
function _validUriAuthority(s, from, to) {
  ...
  if (hostEnd === hostStart) return false;                                             // empty host
  if (!_componentBytesOk(s, hostStart, hostEnd, _isRegNameByte)) return false;          // reg-name host
  if (colon !== -1) {
