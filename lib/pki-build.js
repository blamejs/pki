...
// A URI hier-part/query/fragment byte: an RFC 3986 unreserved byte, a sub-delim, or a non-bracket
// gen-delim (":" "/" "?" "#" "@"). "%" is handled separately (it must open a "%HEXHEX" triplet), and the
// IP-literal brackets "[" "]" are excluded because they need an authority-structure parse a form picker
// does not do (an IPv6-literal-host URL uses the object form).
function _isUriByte(c) {
  if (_isAlphaCode(c) || _isDigitCode(c)) return true;
  switch (c) {
    case 45: case 46: case 95: case 126:                          // - . _ ~   (unreserved)
    case 58: case 47: case 63: case 35: case 64:                  // : / ? # @  (gen-delims, no brackets)
    case 33: case 36: case 38: case 39: case 40: case 41:         // ! $ & ' ( )
    case 42: case 43: case 44: case 59: case 61:                  // * + , ; =  (sub-delims)
      return true;
...
// scheme "://" then a non-empty authority (first byte not "/","?","#") then only URI bytes or "%HEXHEX".
function _looksLikeUri(s) {
  ...
  i += 3;
  if (i >= n) return false;                                                            // empty authority
  c = _charCodeAt(s, i);
  if (c === 47 || c === 63 || c === 35) return false;                                  // "/","?","#" first
  while (i < n) {
    c = _charCodeAt(s, i);
    if (c === 37) {                                                                    // "%" -> %HEXHEX
      if (i + 2 >= n || !_isHexCode(_charCodeAt(s, i + 1)) || !_isHexCode(_charCodeAt(s, i + 2))) return false;
      i += 3;
    } else if (_isUriByte(c)) { i += 1; }
    else return false;
  }
  return true;
}

// A single DNS label over the byte range [from,to): 1-63 bytes, LDH plus a leading/embedded underscore,
// with no hyphen at either edge (matching the historical label grammar the SAN forms accept).
function _isDnsLabelRange(s, from, to) {
  ...
  for (i = from; i < to; i += 1) {
    c = _charCodeAt(s, i);
    edge = (i === from || i === to - 1);
    if (_isAlphaCode(c) || _isDigitCode(c) || c === 95) continue;                      // A-Z a-z 0-9 _
...
// Every dot-separated label in [from,to) is a valid DNS label. Scans with indexOf; no split allocation.
function _labelsValid(s, from, to) {
  var start = from, dot;
  for (;;) {
    dot = _stringIndexOf(s, ".", start);
    var end = (dot === -1 || dot >= to) ? to : dot;
    if (!_isDnsLabelRange(s, start, end)) return false;
    if (end === to) return true;
    start = end + 1;
  }
}

// A dNSName: at most 253 bytes, an optional single leading "*." wildcard, then dot-separated labels.
function _looksLikeDnsName(s) {
  var n = s.length, start = 0;
  if (n < 1 || n > 253) return false;
  if (n >= 2 && _charCodeAt(s, 0) === 42 && _charCodeAt(s, 1) === 46) start = 2;       // "*."
  if (start >= n) return false;
  return _labelsValid(s, start, n);
}

// An rfc822Name: an unquoted RFC 5321 dot-atom local part (atext runs joined by single dots), one "@",
// and a dotted DNS domain (at least one dot -- an FQDN, no wildcard).
function _looksLikeEmail(s) {
  var at = _stringIndexOf(s, "@", 0);
  if (at < 1 || _stringLastIndexOf(s, "@") !== at) return false;                       // exactly one "@", non-empty local
  ...
  return _labelsValid(s, dStart, s.length);
}
