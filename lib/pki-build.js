...
/** @internal The bytes RFC 3986 sec. 3.2.2 admits in a reg-name, beside a percent-encoding. A DNS
 *  label admits fewer, which is why a reader does not hold a host it was given to that rule. */
function _isRegNameByte(c) { return _isUnreserved(c) || _isSubDelim(c); }
...
/** @internal A port as RFC 3986 sec. 3.2.3 writes it: ":" *DIGIT, or nothing. An empty port after
 *  the colon is what the grammar allows, so it is not a reason to refuse. */
function _validUriPort(s, at, to) {
  if (at === to) return true;
  if (_charCodeAt(s, at) !== 58) return false;
  for (var i = at + 1; i < to; i += 1) { if (!_isDigitCode(_charCodeAt(s, i))) return false; }
  return true;
}
/** @internal An IPvFuture literal, RFC 3986 sec. 3.2.2: "v" 1*HEXDIG "." 1*( unreserved /
 *  sub-delims / ":" ). */
function _validIpvFuture(s, from, to) {
  if (to - from < 4) return false;
  var c = _charCodeAt(s, from);
  if (c !== 118 && c !== 86) return false;
  var i = from + 1, hex = 0;
  while (i < to && _isHexCode(_charCodeAt(s, i))) { i += 1; hex += 1; }
  if (hex === 0 || i >= to || _charCodeAt(s, i) !== 46) return false;
  i += 1;
  if (i >= to) return false;
  for (; i < to; i += 1) {
    c = _charCodeAt(s, i);
    if (!_isUnreserved(c) && !_isSubDelim(c) && c !== 58) return false;
  }
  return true;
}
/** @internal The authority of a URL a reader was GIVEN, as RFC 3986 sec. 3.2 writes it:
 *  [ userinfo "@" ] host [ ":" port ], where host is a non-empty reg-name, an IPv6 literal in
 *  brackets, or an IPvFuture literal. RFC 7230 sec. 2.7.1 forbids an empty host for http, so the
 *  host is required here. The structure is checked; the reg-name's own bytes are not held to DNS
 *  label rules, which is what a URL this toolkit EMITS must satisfy and is not what RFC 3986
 *  conformance means. Applying the emit rule to a value someone else wrote rejects legal URLs, and
 *  dropping the structure with it accepts "http://:80". */
function _validReadAuthority(s, from, to) {
  var i, at = -1;
  for (i = from; i < to; i += 1) { if (_charCodeAt(s, i) === 64) at = i; }
  if (at !== -1 && !_componentBytesOk(s, from, at, _isUserinfoByte)) return false;
  var hostStart = at === -1 ? from : at + 1;
  if (hostStart >= to) return false;
  if (_charCodeAt(s, hostStart) === 91) {
    var close = -1;
    for (i = hostStart + 1; i < to; i += 1) { if (_charCodeAt(s, i) === 93) { close = i; break; } }
    if (close === -1) return false;
    if (!_validUriIp6Host(s, hostStart + 1, close) && !_validIpvFuture(s, hostStart + 1, close)) return false;
    return _validUriPort(s, close + 1, to);
  }
  var colon = -1;
  ...
  if (!_componentBytesOk(s, hostStart, hostEnd, _isRegNameByte)) return false;
  return colon === -1 ? true : _validUriPort(s, colon, to);
}

function _validUriAuthority(s, from, to, shape) {
  if (shape.anyAuthority) return _validReadAuthority(s, from, to);
  var i, c, at = -1;
  for (i = from; i < to; i += 1) {
    c = _charCodeAt(s, i);
...
/** @internal An http or https URL as RFC 7230 sec. 2.7.1 and 2.7.2 write it: "http:" "//"
 *  authority path-abempty [ "?" query ], where path-abempty MAY be empty, so a query sits directly
 *  after the authority, and where a fragment is allowed on the URI reference (RFC 3986 sec. 3.5).
 *  This is the shape a reader holds a URL it was GIVEN to. The shape above is what the builders
 *  hold the URLs they EMIT to, which is deliberately narrower, and applying it to a value someone
 *  else wrote reports a conforming URL. */
var _HTTP_URL_READ_SHAPE = intrinsic.assign(intrinsic.create(null), { singleLabel: true, userinfo: true, emptyAuthority: false, emptyHost: false, queryNeedsPath: false, fragment: true, anyAuthority: true });
...
/** @internal The scheme of a well-formed http or https URL as a READER judges one, or null where
 *  the string is not one of either. Carrying the scheme token is not the same as being a URI of
 *  that scheme: RFC 7230 sec. 2.7.1 writes http-URI as "http:" "//" authority path-abempty and
 *  forbids an empty host, so "http:example.com/a" and "http://:80" are neither. A caller that
 *  admits one scheme and not the other compares the answer rather than testing a prefix. */
function httpUrlScheme(s) {
  if (typeof s !== "string" || !_uriShape(s, _HTTP_URL_READ_SHAPE)) return null;
  if (guard.name.uriSchemeIs(s, "http")) return "http";
  if (guard.name.uriSchemeIs(s, "https")) return "https";
  return null;
}
...
/** @internal The distinguished-name attributes RFC 5280 App. A.1 types as something other than
 *  DirectoryString, with the SIZE constraint the module states. Every other attribute is a
 *  DirectoryString and takes UTF8String. A row here, not a branch, so a newly registered
 *  attribute whose module entry names a type is a data addition. */
var ATV_TYPE = intrinsic.assign(intrinsic.create(null), {
  countryName: { encode: b.printable, tag: asn1.TAGS.PRINTABLE_STRING, size: [2, 2], what: "X520countryName, PrintableString SIZE(2)" },
  jurisdictionCountryName: { encode: b.printable, tag: asn1.TAGS.PRINTABLE_STRING, size: [2, 2], what: "a jurisdiction country code, PrintableString SIZE(2)" },
  distinguishedNameQualifier: { encode: b.printable, tag: asn1.TAGS.PRINTABLE_STRING, what: "X520dnQualifier, PrintableString" },
  serialNumber: { encode: b.printable, tag: asn1.TAGS.PRINTABLE_STRING, size: [1, 64], what: "X520SerialNumber, PrintableString" },
  emailAddress: { encode: b.ia5, tag: asn1.TAGS.IA5_STRING, size: [1, 255], what: "EmailAddress, IA5String" },
  domainComponent: { encode: b.ia5, tag: asn1.TAGS.IA5_STRING, what: "DomainComponent, IA5String (RFC 4519 sec. 2.4)" },
});

/** @internal The DirectoryString CHOICE of RFC 5280 App. A.1. Every attribute the toolkit
 *  publishes a short name for and does not give its own row above is valued as one, so a
 *  pre-encoded value for one of them must be a member. An attribute with no published short
 *  name has a syntax this toolkit does not claim to know, and its value stays opaque. */
var DIRECTORY_STRING_TAGS = intrinsic.assign(intrinsic.create(null), {});
[asn1.TAGS.TELETEX_STRING, asn1.TAGS.PRINTABLE_STRING, asn1.TAGS.UNIVERSAL_STRING,
  asn1.TAGS.UTF8_STRING, asn1.TAGS.BMP_STRING].forEach(function (t) { DIRECTORY_STRING_TAGS[t] = 1; });
...
function makeBuilder(ctx) {
  ...
  function assertAtvValue(attrName, value) {
    if (value == null || value === "") throw E("bad-name", "the " + attrName + " attribute value must be a non-empty string");
    var row = ATV_TYPE[attrName];
    if (!row || !row.size) return;
    var len = _String(value).length;
    if (len < row.size[0] || len > row.size[1]) {
      throw E("bad-name", attrName + " must be " + (row.size[0] === row.size[1]
        ? "exactly " + row.size[0] + " characters" : row.size[0] + " to " + row.size[1] + " characters") +
        " (" + row.what + ", RFC 5280 App. A.1)");
    }
  }
  function atvString(attrName, value) {
    assertAtvValue(attrName, value);
    var row = ATV_TYPE[attrName];
    if (!row) return b.utf8(value);
    return row.encode(value);
  }
  function encodeAtv(attrName, value) {
    ...
    var valueTlv;
    if (typeof value === "string" && _charCodeAt(value, 0) === 0x23) {
      var raw = guard.encoding.hex(_sliceStr(value, 1), null, rawErr, ctx.prefix + "/bad-name", "a #hex attribute value");
      guard.der.tlv(raw, rawErr, ctx.prefix + "/bad-name", "a #hex attribute value");
      var rawNode = asn1.decode(raw);
      var typeRow = ATV_TYPE[attrName];
      var universalPrimitive = rawNode.tagClass === "universal" && !rawNode.constructed;
      if (typeRow && (!universalPrimitive || rawNode.tagNumber !== typeRow.tag)) {
        throw E("bad-name", "the " + attrName + " #hex value must be encoded as " + typeRow.what);
      }
      if (!typeRow && intrinsic.hasOwn(C.NAMES.DN_SHORT, attrName) &&
          (!universalPrimitive || !DIRECTORY_STRING_TAGS[rawNode.tagNumber])) {
        throw E("bad-name", "the " + attrName + " #hex value must be a DirectoryString: a TeletexString, PrintableString, UniversalString, UTF8String or BMPString (RFC 5280 App. A.1)");
      }
      var rawText;
      try {
        rawText = asn1.read.string(rawNode);
      } catch (_e) {
        rawText = null;
      }
      if (rawText !== null) {
        try { atvString(attrName, rawText); }
        catch (e) { if (e instanceof ErrorClass) throw e; throw E("bad-name", "the " + attrName + " #hex value carries characters invalid for that attribute's string type", e); }
      }
      return b.sequence([b.oid(typeOid), raw]);
    }
    if (typeof value === "string" && _charCodeAt(value, 0) === 0x5c) value = _sliceStr(value, 1);
    if (value === "") throw E("bad-name", "the " + attrName + " attribute value must be a non-empty string");
  ...
  function encodeRdn(rdnSpec) {
    ...
    var keys = guard.identifier.optionNames(rdnSpec);
  ...
  function certLikeFromSpki(spkiDer) {
    ...
    return { subjectPublicKeyInfo: { bytes: spkiDer, algorithm: { oid: keyOid, parameters: alg.children.length > 1 ? alg.children[1].bytes : undefined } } };
  }
...
/** @internal Whether two SubjectPublicKeyInfo encodings carry the same public key: byte-equal, or
 *  equal under the runtime's key comparison once both import. */
...
module.exports = intrinsic.freeze({ makeBuilder: makeBuilder, reqDenseArray: reqDenseArrayImpl, KU_BIT: KU_BIT, tbsNameField: tbsNameField, tbsSerialNumber: tbsSerialNumber, randomSerial: randomSerial, skiValueOf: skiValueOf, isHttpUrl: isHttpUrl, httpUrlScheme: httpUrlScheme, isLdapUrl: isLdapUrl });
