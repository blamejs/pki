...
/** @internal RFC 5280 sec. 4.2 fixes the criticality of these certificate extensions. Each entry maps
 *  the extension OID to the flag the RFC requires and the clause that sets it. */
var CERT_FIXED_CRITICALITY = intrinsic.create(null);
[["authorityKeyIdentifier", false, "RFC 5280 sec. 4.2.1.1"], ["subjectKeyIdentifier", false, "RFC 5280 sec. 4.2.1.2"],
  ...
  ["subjectInfoAccess", false, "RFC 5280 sec. 4.2.2.2"]].forEach(function (r) {
  CERT_FIXED_CRITICALITY[oid.byName(r[0])] = { critical: r[1], citation: r[2] };
});
...
function makeBuilder(ctx) {
  ...
  function extAuthorityInfoAccess(list) {
    if (!_isArray(list) || !list.length) {
      throw E("bad-input", "authorityInfoAccess must name at least one { accessMethod, accessLocation } (RFC 5280 sec. 4.2.2.1)");
    }
    var dense = reqDenseArray(list, "authorityInfoAccess");
    return b.sequence(_mapIntrinsic(dense, function (d, i) {
      var who = "authorityInfoAccess entry " + i;
  ...
  function distributionPoint(entry, what, idx) {
    ...
    if (spec.cRLIssuer != null) {
      ...
      var named = guard.list.anyMatches(issuers, function (e) {
        return !!e && typeof e === "object" && !_isArray(e) && !_isBufferChk(e) && e.directoryName != null;
      });
      if (!named) {
        throw E("bad-input", who + " cRLIssuer must name the CRL issuer as a directoryName (RFC 5280 sec. 4.2.1.13); a cRLIssuer carrying no directoryName matches no CRL issuer");
      }
  ...
  function extCertPolicies(names) {
    ...
    return b.sequence(names.map(function (n) {
      ...
      if (seen[pOid]) throw E("bad-input", "duplicate certificate policy " + guard.text.showValue(n) + " (RFC 5280 sec. 4.2.1.4)");
      seen[pOid] = true;
...
module.exports = intrinsic.freeze({ makeBuilder: makeBuilder, reqDenseArray: reqDenseArrayImpl, KU_BIT: KU_BIT, tbsNameField: tbsNameField, tbsSerialNumber: tbsSerialNumber, randomSerial: randomSerial });
