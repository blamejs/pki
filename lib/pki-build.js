...
// (the same rule the top-level classifier uses), and an IPv6-literal host -- needing the excluded "[" "]"
...
// (userinfo and reg-name each exclude it), a host validated by _validUriHost (RFC 5280 FQDN or IPv4), and
// an all-digit -- possibly empty -- port after the host's single ":" (a reg-name has no ":"). The userinfo
// bytes are validated against the RFC 3986 userinfo class.
