...
// a line (at the body start, or immediately after an LF) AND be a complete delimiter token -- followed
// by "--" (close), CR/LF, or transport-padding whitespace, or the buffer end. So a "--boundary"
// substring mid-content, a "--boundaryextra" (a different, longer token), or one not at a line start is
// NOT matched -- an attacker cannot inject a fake delimiter inside a part's content.
function _findDelim(body, delim, from) {
  for (var at = body.indexOf(delim, from); at >= 0; at = body.indexOf(delim, at + delim.length)) {
    var lineStart = at === 0 || body[at - 1] === 0x0a;
    var after = body[at + delim.length];   // undefined at the buffer end
    var wellFormed = after === undefined || after === 0x2d || after === 0x0d || after === 0x0a || after === 0x20 || after === 0x09;
    if (lineStart && wellFormed) return at;
  }
  return -1;
}
