...
// Line-ending + whitespace helpers, decided character by character (no regex): _toLf collapses any
// CRLF / bare CR / bare LF to a single LF; _toCrlf then expands every LF to CRLF; _stripLeadingHtab
// drops a leading run of SP/HTAB. They match the /\r\n/g, /\r/g, /\n/g and /^[ \t]+/ replacements.
function _toLf(s) {
  var out = "";
  for (var i = 0; i < s.length; i++) {
    var c = s.charAt(i);
    if (c === "\r") { if (s.charAt(i + 1) === "\n") continue; out += "\n"; }   // CRLF -> its LF next; bare CR -> LF
    else out += c;
  }
  return out;
}
