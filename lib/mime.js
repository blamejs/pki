...
// Does the structured header value name a parameter with this attribute -- WITH OR WITHOUT a value? paramCount
// only counts "name=value" parts, so a bare/valueless attribute (e.g. "Content-Type: text/plain; hp") is
// invisible to it; hasParam still sees the attribute name so a consumer can fail closed on a malformed bare
// parameter rather than silently ignore it. Comment- + quoted-string-aware, like paramCount; part 0 is the
// media type (not a parameter), so attributes start at index 1.
function hasParam(headerValue, name) {
  var ln = name.toLowerCase();
  var parts = _splitSemicolons(headerValue);
  for (var i = 1; i < parts.length; i++) {
    var p = _stripComments(parts[i]);
    var eq = p.indexOf("=");
    if ((eq >= 0 ? p.slice(0, eq) : p).trim().toLowerCase() === ln) return true;
  }
  return false;
}
