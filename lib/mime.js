...
function _splitSemicolons(s) {
  ...
  for (var i = 0; i < s.length; i++) {
    var ch = s[i];
    // RFC 2045 quoted-pair: inside a quoted-string a backslash escapes the next char (so an escaped `"` does
    // NOT end the quote, and a `;` in an escaped value does not split) -- keep both bytes in the value.
    if (inQ && ch === "\\" && i + 1 < s.length) { cur += ch + s[i + 1]; i++; }
...
function paramCount(headerValue, name) {
  ...
  _splitSemicolons(headerValue).forEach(function (part) {
    var eq = part.indexOf("=");
    if (eq >= 0 && part.slice(0, eq).trim().toLowerCase() === ln) n++;
  });
  return n;
}
