...
// Everything this verifier reads from the runtime while it is deciding something, taken at load
// through the guard family's home for that. Each of these is an ordinary writable property until it
// is captured, and every one of them answers a question the verdict rests on: a replaced
// `Buffer.prototype.equals` makes the apple nonce and the TPM certInfo Name compare equal to
// anything, a replaced `indexOf` empties the origin and algorithm allowlists, and a replaced
// `Array.isArray` sends a caller's trust anchors past the door that copies them. See guard-intrinsic.
...
// The settle operations, and the constructor they are invoked against. Every public verb here is
// documented `-> Promise<...>`, so a replaced `reject` returning a resolved promise turns a refusal
// into a pass at the one place the exit shape is decided.
...
function _ucmp(a, b) {
  function strip(x) { var i = 0; while (i < x.length - 1 && x[i] === 0) i++; return _subarray(x, i); }
  return _compare(strip(a), strip(b)) === 0;
}
...
function _coseAlgHash(alg, E) {
  var h = COSE_ALG_HASH[_String(alg)];
...
function _parseAuthData(buf, E) {
  if (!_isBuffer(buf) || buf.length < 37) throw E("webauthn/bad-auth-data", "authenticatorData is shorter than the 37-byte minimum (RFC WebAuthn sec. 6.1)");
  var flags = buf[32];
  var out = {
    rpIdHash: _subarray(buf, 0, 32),
  ...
  if (out.flags.at) {
    ...
    out.aaguid = _subarray(buf, off, off + AAGUID_LEN); off += AAGUID_LEN;
    ...
    out.credentialId = _subarray(buf, off, off + credLen); off += credLen;
    // credentialPublicKey is a COSE_Key: a single CBOR map occupying [off, its end).
    var keyNode;
    try { keyNode = cbor.decode(_subarray(buf, off), { allowTrailing: true }); }
    catch (e) { throw E("webauthn/bad-cose-key", "the credential public key is not well-formed CBOR", e); }
    out.credentialPublicKeyBytes = _subarray(buf, off, off + keyNode.bytes.length);
  ...
  if (out.flags.ed) {
    ...
    var extNode;
    try { extNode = cbor.decode(_subarray(buf, off)); }
    ...
    out.extensions = _subarray(buf, off);
...
function _certEcCurveOid(cert, E) {
  ...
  if (!_isBuffer(params)) throw E("webauthn/key-mismatch", "the attestation certificate EC key carries no named-curve parameters");
...
function _assertCertKeyAlgorithm(cert, cose, E) {
  ...
  if (!want.length || _indexOf(want, alg.name) < 0) {
    throw E("webauthn/key-mismatch", "the attestation certificate carries a " + _stringify(alg.name) +
...
function _certPubKeyEqualsCose(cert, cose, E) {
  ...
  if (cose.kty === 2) {
    ...
    var cx = _subarray(raw, 1, 1 + coordLen), cy = _subarray(raw, 1 + coordLen);
  ...
  if (cose.kty === 1) {
    ...
    if (!cose.x || !_bufEquals(_bufferFrom(raw), cose.x)) throw E("webauthn/key-mismatch", "the attestation certificate OKP key does not equal the credential public key");
    return;
  }
...
function parseAttestationObject(bytes) {
  ...
  if (attStmtN.majorType !== wantMajor) {
    throw _err("webauthn/bad-attestation-object", "attestation object 'attStmt' must be a CBOR " + (wantMajor === 4 ? "array" : "map") + " for format " + _stringify(cbor.read.textString(fmtN)));
  }
...
function _algOf(attStmt) { return _Number(_attRead(attStmt, "alg", cbor.read.int, "an integer")); }
...
function _requireAttShape(attStmt, allowed, required) {
  ...
  _keys(have).forEach(function (k) { if (_indexOf(allowed, k) === -1) throw _err("webauthn/bad-att-stmt", "the attestation statement carries an unexpected field '" + k + "'"); });
  _forEach(required, function (k) { if (!have[k]) throw _err("webauthn/bad-att-stmt", "the attestation statement is missing the '" + k + "' field"); });
}
...
// allow:guard-reads-runtime-live -- module-init, the table is built once as this file loads
...
function _cloneParsed(v, depth) {
  ...
  if (_isBuffer(v) || v instanceof Uint8Array) return guard.bytes.snapshot(v, _err, "webauthn/bad-input", "a byte field of opts.rootCertificates[]");
  if (_isArray(v)) return _map(v, function (x) { return _cloneParsed(x, depth + 1); });
  if (guard.time.isDate(v)) return new _Date(guard.time.instantOf(v));
  if (v && typeof v === "object") {
    var out = {};
    _keys(v).forEach(function (k) { out[k] = _cloneParsed(v[k], depth + 1); });
    return out;
  }
...
function _isBufferSource(v) { return _isView(v) || v instanceof ArrayBuffer; }
...
function _snapshotRoots(supplied) {
  if (!_isArray(supplied)) return supplied;
  return _map(supplied, function (root) {
...
function _applyCallerRoots(res, supplied, vopts, onlyPaths) {
  if (!_isArray(supplied) || !supplied.length) {
  ...
  var roots = _map(supplied, function (root, i) {
  ...
  var paths = onlyPaths || ((res.fmt === "compound" && _isArray(res.compound))
    ? _filter(res.compound, function (el) { return el.trustPath && el.trustPath.length; })
  ...
  if (!onlyPaths) {
    res.anchoredElements = { total: (res.fmt === "compound" && _isArray(res.compound)) ? res.compound.length : 1,
  ...
  var at = vopts.time !== undefined ? vopts.time : new _Date();
  ...
  return paths.reduce(function (p, info) {
    return p.then(function () {
      ...
      return mds.chainToAnchor(_arraySlice(info.tp).reverse(), roots, pathAt,
        "attestation trust path (against the roots supplied as opts.rootCertificates)");
    });
  }, _promiseResolve(_Promise)).then(function () {
...
function _safetyNetAnchored(res) {
  ...
  if (res.fmt === "compound" && _isArray(res.compound)) {
    return _filter(res.compound, function (el) { return el && el.fmt === "android-safetynet"; }).length;
  }
  return 0;
}
...
function _anchoredRoutes(res, base) {
  var routes = base ? _strSplit(base, "+") : [];
  if (_safetyNetAnchored(res) && _indexOf(routes, "safetyNetRoots") < 0) _push(routes, "safetyNetRoots");
  return routes.length ? _join(routes, "+") : null;
}
...
function _applyBindings(authData, coseKey, opts) {
  ...
  if (opts.expectedRpId !== undefined) {
    ...
    if (!guard.crypto.constantTimeEqual(_sha("sha256", _bufferFrom(opts.expectedRpId, "utf8")),
      _bufferFrom(authData.rpIdHash))) {
  ...
  var alg65535Allowed = _isArray(opts.allowedAlgorithms) && _indexOf(opts.allowedAlgorithms, -65535) !== -1;
  ...
  if (opts.allowedAlgorithms !== undefined) {
    if (!_isArray(opts.allowedAlgorithms) || !opts.allowedAlgorithms.length ||
        ...
        !_every(opts.allowedAlgorithms, function (a) { return typeof a === "number" && _isSafeInteger(a); })) {
    ...
    if (_indexOf(opts.allowedAlgorithms, alg) === -1) {
      throw _err("webauthn/algorithm-not-allowed",
...
function _readX5c(attStmt) {
  ...
  return _map(x5cN.children, function (c) {
    var der;
...
var VERIFIERS = {
  ...
  packed: function (att, clientDataHash) {
    ...
    var message = _bufferConcat([att.authDataBytes, clientDataHash]);
    if (isX5c) {
  ...
  "fido-u2f": function (att, clientDataHash) {
    ...
    var publicKeyU2F = _bufferConcat([_bufferFrom([0x04]), key.x, key.y]);
    var verificationData = _bufferConcat([_bufferFrom([0x00]), att.authData.rpIdHash, clientDataHash, att.authData.credentialId, publicKeyU2F]);
  ...
  apple: function (att, clientDataHash) {
    ...
    var chain = _readX5c(att.attStmt), leaf = chain[0];
    _requireV3(leaf);
    var nonce = _sha("sha256", _bufferConcat([att.authDataBytes, clientDataHash]));
    ...
    if (!_bufEquals(embedded, nonce)) throw _err("webauthn/verify-failed", "the apple attestation nonce does not equal SHA-256(authData || clientDataHash)");
    _certPubKeyEqualsCose(leaf, att.authData.credentialPublicKey, _err);   // 8.8 item 30
    return _promiseResolve(_Promise, _result("apple", "AnonCA", chain, att));
  },
  ...
  tpm: function (att, clientDataHash, opts) {
    ...
    var attToBeSigned = _bufferConcat([att.authDataBytes, clientDataHash]);
    if (!_bufEquals(certInfo.extraData, _sha(_coseAlgHash(alg, _err), attToBeSigned))) throw _err("webauthn/verify-failed", "certInfo extraData does not equal the hash of authData || clientDataHash");

    // attested.name == nameAlg || H_nameAlg(pubArea).
    var nameHash = validator.tpm.TPM_ALG_HASH[pub.nameAlg];
    if (!nameHash) throw _err("webauthn/bad-tpm", "unsupported TPM nameAlg 0x" + _numToString(pub.nameAlg, 16));
    var computedName = _bufferConcat([pub.nameAlgBytes, _sha(nameHash, pubAreaBytes)]);
    if (!_bufEquals(certInfo.attestedName, computedName)) throw _err("webauthn/verify-failed", "certInfo attested Name does not match the pubArea TPM Name");
  ...
  "android-safetynet": function (att, clientDataHash, opts) {
    ...
    if (!_isArray(roots) || roots.length === 0) {
    ...
    try {
      ...
      sigBytes = _bufferFrom(jose.base64url.decode(segs[2]));
    ...
    if (header.alg !== "RS256") throw _err("webauthn/unsupported-algorithm", "the android-safetynet JWS alg must be RS256, got " + _stringify(header.alg));
    if (!_isArray(header.x5c) || header.x5c.length === 0) {
    ...
    var chain = _map(header.x5c, function (entry, i) {
    ...
    var wantNonce = _sha("sha256", _bufferConcat([att.authDataBytes, clientDataHash])).toString("base64");
    if (typeof payload.nonce !== "string" || !guard.crypto.constantTimeEqual(_bufferFrom(payload.nonce, "utf8"), _bufferFrom(wantNonce, "utf8"))) {
    ...
    if (opts.requireCtsProfileMatch === true && signals.ctsProfileMatch !== true) {
      throw _err("webauthn/safetynet-cts-profile", "the android-safetynet response reports ctsProfileMatch " + _stringify(signals.ctsProfileMatch) + ", and opts.requireCtsProfileMatch demands true");
    }

    return _verifySig(-257, sigBytes, leaf.subjectPublicKeyInfo.bytes,
      _bufferFrom(segs[0] + "." + segs[1], "ascii"), _err).then(function (ok) {
      ...
      chainAt = opts.time !== undefined ? opts.time
        : (typeof payload.timestampMs === "number" && _isFinite(payload.timestampMs) && payload.timestampMs > 0
          ? new _Date(payload.timestampMs) : undefined);
  ...
  compound: function (att, clientDataHash, opts) {
    ...
    var elements = _map(kids, function (el, i) {
    ...
    return elements.reduce(function (p, e) {
      return p.then(function () {
        ...
        return _promiseResolve(_Promise).then(function () { return VERIFIERS[e.fmt](e.att, clientDataHash, opts); })
          .then(function (r) { _push(out, r); }, function (err) {
  ...
  none: function (att) {
    ...
    return _promiseResolve(_Promise, _result("none", "None", [], att));
  },
};
...
function _isPlainObject(v) { return !!v && typeof v === "object" && !_isArray(v); }
...
function _safetyNetHostnameOk(leaf) {
  ...
  var entries = san && _isArray(san.value) ? san.value : [];
  var dns = _filter(entries, function (gn) { return gn && gn.type === "dNSName" && typeof gn.value === "string"; });
  // A SAN carrying dNSName entries is authoritative: the commonName is not consulted at all.
  if (dns.length) return _some(dns, function (gn) { return _toLowerCase(gn.value) === want; });
  // rdns is a sequence of RDNs, each a set of attribute/value pairs -- hence the nested walk.
  return _some(leaf.subject.rdns, function (rdn) {
    return _some(rdn, function (atv) {
      return atv.name === "commonName" && typeof atv.value === "string" && _toLowerCase(atv.value) === want;
    });
  });
}
...
function _safetyNetChainTrusted(chain, roots, time) {
  ...
  try {
    ...
    anchors = _map(roots, function (root, i) {
  ...
  return mds.chainToAnchor(chain, anchors, time === undefined ? new _Date() : time,
...
function verify(attestationObject, clientDataHash, opts) {
  ...
  clientDataHash = cdh;
  try { att = parseAttestationObject(attestationObject); } catch (e) { return _promiseReject(_Promise, e); }
  ...
  if (!att.authData.flags.at || !att.authData.credentialPublicKey) {
    return _promiseReject(_Promise, _err("webauthn/bad-auth-data", "attestation requires attestedCredentialData (the AT flag must be set)"));
  }
  var verifier = VERIFIERS[att.fmt];
  if (!verifier) return _promiseReject(_Promise, _err("webauthn/unsupported-format", "attestation statement format '" + att.fmt + "' is not supported"));
  ...
  if (opts.tpmPolicy !== undefined && !_formatCarries(att, "tpm")) {
    return _promiseReject(_Promise, _err("webauthn/tpm-policy", "opts.tpmPolicy requires a TPM attestation, but this attestation is format '" + att.fmt + "', which carries no TPM public area"));
  }
  ...
  if (opts.requireCtsProfileMatch === true && !_formatCarries(att, "android-safetynet")) {
    return _promiseReject(_Promise, _err("webauthn/safetynet-cts-profile", "opts.requireCtsProfileMatch requires an android-safetynet attestation, but this attestation is format '" + att.fmt + "', whose statement carries no device-integrity signals"));
  }
  ...
  return _promiseResolve(_Promise).then(function () { return verifier(att, clientDataHash, vopts); })
    .then(function (res) {
    ...
    .then(function (res) {
      ...
      return _promiseResolve(_Promise).then(function () { return _applyMetadata(res, att, vopts); })
        .then(function (out) { out.anchoredTo = _anchoredRoutes(out, "metadata"); return out; }, function (e) {
          ...
          return _promiseResolve(_Promise, _applyCallerRoots(res, vopts.rootCertificates, vopts, e.missedPaths))
            .then(function (out) {
              ...
              out.metadata = { aaguid: primary.entry.aaguid, keyIdentifiers: primary.entry.keyIdentifiers || [],
                entry: primary.entry, entries: _map(applied, function (a) { return a.entry; }),
...
function _applyMetadata(res, att, opts) {
  ...
  var paths = (res.fmt === "compound" && _isArray(res.compound))
    ? _filter(res.compound, function (el) { return el.trustPath && el.trustPath.length; })
  ...
  function govern(info) {
    ...
    _push(applied, { entry: entry, anchors: anchors, identifier: identifier });
  ...
  var at = opts.time !== undefined ? opts.time : new _Date();
  ...
  for (var gi = 0; gi < paths.length; gi++) {
    try {
      _push(governed, { info: paths[gi], g: govern(paths[gi]) });
    } catch (ge) {
      ...
      if (ge && ge.code === "webauthn/metadata-not-found") { missed = missed || ge; _push(missedPaths, paths[gi]); continue; }
  ...
  return governed.reduce(function (p, item) {
    return p.then(function () {
      ...
      return mds.chainToAnchor(_arraySlice(info.tp).reverse(), g.anchors, pathAt,
        "attestation trust path for " + g.identifier + " (against the roots its metadata entry registers)");
    });
  }, _promiseResolve(_Promise))
    .then(function () {
...
function parseClientData(bytes, opts) {
  ...
  var doc = guard.json.parse(_bufferFrom(bytes), _err, {
  ...
  if (typeof doc.type !== "string" || CLIENT_DATA_TYPE[doc.type] !== 1) {
    throw _err("webauthn/bad-client-data",
      "clientDataJSON type must be \"webauthn.create\" or \"webauthn.get\", got " + _stringify(doc.type) +
      " (WebAuthn sec. 5.8.1)");
  }
  ...
  if (opts.expectedOrigin !== undefined) {
    var allowed = _isArray(opts.expectedOrigin) ? opts.expectedOrigin : [opts.expectedOrigin];
    if (!allowed.length || !_every(allowed, function (o) { return typeof o === "string" && o.length; })) {
    ...
    // "https://example.com".
    if (_indexOf(allowed, doc.origin) === -1) {
      throw _err("webauthn/client-data-mismatch",
        "the clientDataJSON origin " + _stringify(doc.origin) + " is not one this relying party accepts " +
  ...
  if (opts.expectedTopOrigin !== undefined) {
    ...
    if (wantTop === null) {
      if (framed || doc.topOrigin !== undefined) {
        throw _err("webauthn/client-data-mismatch",
          "the clientDataJSON describes a cross-origin ceremony (crossOrigin " + _stringify(doc.crossOrigin) +
          ", topOrigin " + _stringify(doc.topOrigin === undefined ? null : doc.topOrigin) +
          "), and this relying party requires an unframed one (WebAuthn sec. 5.8.1)");
      }
    } else {
      var allowedTop = _isArray(wantTop) ? wantTop : [wantTop];
      if (!allowedTop.length || !_every(allowedTop, function (o) { return typeof o === "string" && o.length; })) {
      ...
      if (!framed || _indexOf(allowedTop, doc.topOrigin) === -1) {
        throw _err("webauthn/client-data-mismatch",
          "the clientDataJSON topOrigin " + _stringify(doc.topOrigin === undefined ? null : doc.topOrigin) +
          " (crossOrigin " + _stringify(doc.crossOrigin) + ") is not a framing this relying party accepts (WebAuthn sec. 5.8.1)");
      }
    }
...
function _snapshotAssertion(input) {
  ...
  if (_isArray(out.allowedAlgorithms)) out.allowedAlgorithms = _arraySlice(out.allowedAlgorithms);
  if (_isArray(out.expectedOrigin)) out.expectedOrigin = _arraySlice(out.expectedOrigin);
  if (_isArray(out.expectedTopOrigin)) out.expectedTopOrigin = _arraySlice(out.expectedTopOrigin);
  return out;
}
...
function verifyAssertion(input) {
  ...
  var frozen;
  try { frozen = _snapshotAssertion(input); } catch (e) { return _promiseReject(_Promise, e); }
  return _promiseResolve(_Promise).then(function () {
    ...
    if (haveJson) {
      ...
      clientDataHash = _sha("sha256", _bufferFrom(input.clientDataJSON));
    } else {
    ...
    if (_isBuffer(coseKey) || _isView(coseKey) || coseKey instanceof ArrayBuffer) {
    ...
    if (prev !== undefined && (typeof prev !== "number" || !_isSafeInteger(prev) || prev < 0 || prev > 0xFFFFFFFF)) {
    ...
    var message = _bufferConcat([_bufferFrom(input.authenticatorData), clientDataHash]);
    var spki = _coseKeyToSpki(coseKey);
    return _verifySig(coseKey.alg, _bufferFrom(input.signature), spki, message, _err).then(function (ok) {
      if (!ok) {
