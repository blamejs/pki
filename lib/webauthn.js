...
function _applyCallerRoots(res, supplied, vopts, onlyPaths) {
  ...
  var roots = supplied.map(function (root, i) {
    ...
    // A two-field test (a subject and an SPKI) accepted what the sibling door in this same namespace
    // refuses by name. These become trust anchors, so the shape they must satisfy is the shape
    // pki.path.validate dereferences -- one rule, checked where the object enters.
    return guard.parsed.accept(root, "certificate", function (bytes) {
      try {
...
function _safetyNetChainTrusted(chain, roots, time) {
  ...
  try {
    ...
    // anchors, so the shape they must satisfy is the shape pki.path.validate dereferences. A
    // two-field test (a subject and an SPKI) accepted what a parsed CSR also has.
