...
function _decodeCoseKey(node, E) {
  ...
  if (kty === 2n) { key.crv = ii(-1) != null ? Number(ii(-1)) : null; key.x = ib(-2); key.y = ib(-3); }
  else if (kty === 1n) { key.crv = ii(-1) != null ? Number(ii(-1)) : null; key.x = ib(-2); }
  else if (kty === 3n) { key.n = ib(-1); key.e = ib(-2); }
  return key;
}
