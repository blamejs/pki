...
function _AAGUID_IS_SIGNED(res) {
  if (res.fmt === "compound") {
    // A compound is only as good as its weakest element: if any element that actually carries a
    // trust path is fido-u2f, the shared authenticatorData it was verified against was not bound
    // to the AAGUID by that element, so the field cannot be trusted for the statement as a whole.
    return (res.compound || []).every(function (el) {
      if (!el.trustPath || !el.trustPath.length) return true;
      return _AAGUID_SIGNED_BY_FMT[el.fmt] !== false;
    });
  }
  return _AAGUID_SIGNED_BY_FMT[res.fmt] !== false;
}
...
function _applyMetadata(res, att, opts) {
  ...
  if (!md || typeof md !== "object" || !md.byAaguid) {
  ...
  var paths = (res.fmt === "compound" && Array.isArray(res.compound))
    ? res.compound.map(function (el) { return el.trustPath; }).filter(function (p) { return p && p.length; })
    : (res.trustPath && res.trustPath.length ? [res.trustPath] : []);
  ...
  // How the catalogue names this authenticator depends on what the authenticator declares. Most
  // carry an AAGUID and are keyed by it. A U2F authenticator carries none -- its AAGUID is all
  // zeroes -- and the catalogue keys it by the key identifiers of its attestation certificates
  // instead. Looking up only the AAGUID would refuse every U2F registration as unlisted, on
  // authenticators the catalogue does describe.
  // The catalogue's two key spaces are DISJOINT, and which one applies is decided by what the
  // authenticator itself declares -- never by trying one and falling back to the other. An
  // authenticator that declares a model identity is looked up by it and by nothing else, so a model
  // the catalogue does not list is refused instead of being resolved out of the U2F key space under
  // some other authenticator's entry. That fallback would matter: the fido-u2f signature covers
  // `rpIdHash || clientDataHash || credentialId || publicKeyU2F` and NOT the AAGUID field, so those
  // 16 bytes are attacker-editable, and accepting a forged model identity would leave `res.aaguid`
  // reporting a value nothing vouched for while the call looked like it passed the catalogue.
  ...
  var declared = _AAGUID_IS_SIGNED(res) && aaguid && aaguid !== mds.ZERO_AAGUID;
  var entry, identifier;
  if (declared) {
    entry = mds.metadataFor(md, aaguid);
    identifier = "aaguid " + aaguid;
  } else {
    // No model identity: this is the U2F case, keyed by the attestation certificate instead. A path
    // is anchor-adjacent first, so that certificate is its LAST element -- the one whose key the
    // catalogue identifies, not the root it chains to.
    var keyIds = paths.map(function (p) { return mds.certKeyIdentifier(p[p.length - 1]); });
    for (var ki = 0; ki < keyIds.length && !entry; ki++) entry = mds.metadataForKeyIdentifier(md, keyIds[ki]);
    identifier = "attestation certificate key identifier " + keyIds.join(" / ");
  }
  if (!entry) throw _err("webauthn/metadata-not-found", "no metadata entry matches this authenticator (" + identifier + ")");
  if (mds.statusDenied(entry, md)) {
  ...
  // The attestation's trust path must VALIDATE to one of the roots this model registered -- signature
  // chaining, validity, basic constraints and the rest -- not merely resemble one. A name comparison
  // against the top of the path would accept any certificate asserting the registered issuer's name,
  // which is the assertion an attacker controls; and comparing certificates for equality is not the
  // question either, since a registered root normally ISSUES the attestation certificate rather than
  // being it. So this composes the same path validation the BLOB's own chain goes through, with the
  // registered roots as the anchors. trustPath is anchor-adjacent first and that helper takes
  // leaf-first, so the order is reversed on the way in.
  ...
  // EVERY path must reach a registered root, not merely one of them: for a compound attestation
  // each element is an independent claim, and accepting the whole because one element anchored
  // would let an unanchored element ride along on its neighbour's trust.
  return paths.reduce(function (p, tp) {
    return p.then(function () {
      return mds.chainToAnchor(tp.slice().reverse(), anchors, at,
        "attestation trust path for " + identifier + " (against the roots its metadata entry registers)");
    });
  }, Promise.resolve())
    .then(function () {
      ...
      // hand the caller a value that matches nothing and reads like an identifier.
      res.metadata = { aaguid: entry.aaguid, keyIdentifiers: entry.keyIdentifiers || [],
        entry: entry, anchors: anchors.length };
      return res;
    });
}
