...
function _applyMetadata(res, att, opts) {
  ...
  // Every path is resolved and enforced against ITS OWN entry. A compound carries independent
  // claims and its element order is not signed, so choosing one entry for the whole statement would
  // let that order decide whose status report is consulted: a healthy element listed first would
  // suppress a revoked sibling's report while both still anchor successfully. Each path answers for
  // itself, and all of them must pass.
  var applied = [];
  function govern(tp) {
  ...
  return paths.reduce(function (p, tp) {
    return p.then(function () {
      var g = govern(tp);
      return mds.chainToAnchor(tp.slice().reverse(), g.anchors, at,
