...
// validity bound, BigInt for a serial) are preserved rather than flattened. The depth
// bound is a loop backstop, not a policy: a parsed certificate is a fixed, shallow shape.
function _cloneParsed(v, depth) {
  if (depth > 12) throw _err("webauthn/bad-input", "opts.rootCertificates[] is nested too deeply to be a parsed certificate");
...
function _applyCallerRoots(res, supplied, opts) {
  ...
  return paths.reduce(function (p, info) {
    ...
    res.anchoredTo = "rootCertificates";
    return res;
  });
}
 ...
 *   route ran -- `"metadata"`, `"rootCertificates"`, or `null` when neither was asked
 *   for and the path went unchecked -- and `anchoredElements` reports how much of the
...
function verify(attestationObject, clientDataHash, opts) {
  ...
  var bindingChecked;
  var pinnedRoots, verifierOpts;
  try {
    bindingChecked = _applyBindings(att.authData, att.authData.credentialPublicKey, opts);
    pinnedRoots = _snapshotRoots(opts.rootCertificates);
    // `safetyNetRoots` is the SAME aliasing window one level down: the format verifier
    // reads it a microtask after this entry returns, so the caller's synchronous code
    // still runs first. Snapshot it into a shallow copy of opts rather than threading a
    // second argument through every format verifier's signature -- the anchor set a
    // verifier trusts must be the one the caller passed, for both root options alike.
    verifierOpts = opts.safetyNetRoots === undefined
      ? opts
      : Object.assign({}, opts, { safetyNetRoots: _snapshotRoots(opts.safetyNetRoots) });
  } catch (e) { return Promise.reject(e); }
  return Promise.resolve().then(function () { return verifier(att, clientDataHash, verifierOpts); })
    .then(function (res) {
      ...
      if (opts.metadata === undefined) {
        if (opts.rootCertificates !== undefined) return _applyCallerRoots(res, pinnedRoots, opts);
        res.anchoredTo = null;
        return res;
      }
      return Promise.resolve().then(function () { return _applyMetadata(res, att, opts); })
        .then(function (out) { out.anchoredTo = "metadata"; return out; }, function (e) {
          ...
          if (!e || e.code !== "webauthn/metadata-not-found" || opts.rootCertificates === undefined) throw e;
          return _applyCallerRoots(res, pinnedRoots, opts);
        });
    })
...
function _applyMetadata(res, att, opts) {
  ...
  for (var gi = 0; gi < paths.length; gi++) {
    try {
      ...
      if (ge && ge.code === "webauthn/metadata-not-found") { missed = missed || ge; continue; }
