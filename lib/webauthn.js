 ...
 *   android-key, apple, fido-u2f) -- the attestation-statement signature and the
 *   x5c certificate chain to a caller-supplied trust anchor. The attestation CBOR
 *   is decoded by the strict, fail-closed `pki.cbor` codec (WebAuthn keys are
 *   CTAP2-canonical), the signature by `pki.webcrypto`, and the chain by
 *   `pki.path.validate`. A verifier, not a ceremony client: the relying party
 *   supplies the clientDataHash + the trust anchors; this module never touches a
 ...
 * @card Verify a WebAuthn / passkey attestation (packed / tpm / android-key / apple / fido-u2f).
 */
...
function _parseAuthData(buf, E) {
  ...
  if (out.flags.ed) {
    // The extensions map (if any) is the trailing CBOR; surface it raw.
    if (off < buf.length) out.extensions = buf.subarray(off);
...
function _decodeCoseKey(node, E) {
  ...
  if (kty === 2n) {
    ...
    // Only Ed25519 (crv 6, a 32-byte key) is a supported OKP credential key.
    if (key.crv !== 6 || !key.x || key.x.length !== 32) throw E("webauthn/bad-cose-key", "an OKP COSE_Key must be Ed25519 (crv 6) with a 32-byte x (-2)");
  } else if (kty === 3n) {
...
function _verifySig(alg, sig, spkiBytes, message, E) {
  ...
  return subtle.importKey("spki", spkiBytes, d.imp, false, ["verify"])
    .then(function (key) { return subtle.verify(d.verify, key, s, message); })
...
function _coseKeyToSpki(key, E) {
  ...
  if (key.kty === 1 && key.crv === 6 && key.x && key.x.length === 32) {
    // OKP Ed25519 (COSE crv 6): the SPKI is AlgorithmIdentifier { id-Ed25519 } (no
    // parameters) over the 32-byte public key -- RFC 8410.
    var eb = asn1.build;
    return eb.sequence([eb.sequence([eb.oid(oid.byName("Ed25519"))]), eb.bitString(key.x)]);
  }
...
function _certPubKeyEqualsCose(cert, cose, E) {
  ...
  if (cose.kty === 1) {
    // OKP Ed25519: the SPKI BIT STRING body IS the 32-byte public key (fixed width,
    // so a byte-exact compare, not a leading-zero-stripping unsigned compare).
    if (!cose.x || !Buffer.from(raw).equals(cose.x)) throw E("webauthn/key-mismatch", "the attestation certificate Ed25519 key does not equal the credential public key");
    return;
  }
...
function _findExt(cert, oidName) {
  var target;
  try { target = oid.byName(oidName); } catch (_e) { return null; }
...
function parseAttestationObject(bytes) {
  ...
  if (!fmtN || fmtN.majorType !== 3) throw _err("webauthn/bad-attestation-object", "attestation object 'fmt' must be a text string");
  if (!authDataN || authDataN.majorType !== 2) throw _err("webauthn/bad-attestation-object", "attestation object 'authData' must be a byte string");
  var authDataBytes = cbor.read.byteString(authDataN);
  return {
    fmt: cbor.read.textString(fmtN),
    attStmt: attStmtN || null,
...
function _algOf(attStmt) { return Number(cbor.read.int(_reqAttr(attStmt, "alg"))); }
function _sigOf(attStmt) { return cbor.read.byteString(_reqAttr(attStmt, "sig")); }
function _readX5c(attStmt) {
  ...
  return x5cN.children.map(function (c) {
    var der = cbor.read.byteString(c);
...
// An attestation LEAF certificate MUST NOT be a CA (WebAuthn 8.2.1 / 8.3.1): a
// present basicConstraints must assert cA=false. This is the gate that stops a
// genuine CA certificate under the caller's pinned root from being repurposed as an
// attestation leaf -- RFC 5280 path validation does not reject a CA-asserting leaf.
// An absent basicConstraints is a non-CA end entity and is accepted.
function _assertNotCa(cert) {
  var bc = _decodeExt(cert, "basicConstraints");
  if (bc && bc.value && bc.value.cA === true) throw _err("webauthn/bad-att-cert", "an attestation leaf certificate MUST NOT be a CA (basicConstraints cA=true)");
}
// Packed attestation certificate requirements (WebAuthn 8.2.1): v3, non-CA, and the
// subject OU literally "Authenticator Attestation".
function _checkPackedCert(cert) {
  _requireV3(cert);
  _assertNotCa(cert);
  var ou = null, orgUnit = oid.byName("organizationalUnitName");
  (cert.subject.rdns || []).forEach(function (rdn) { rdn.forEach(function (a) { if (a.type === orgUnit) ou = a.value; }); });
  if (ou !== "Authenticator Attestation") throw _err("webauthn/bad-att-cert", "the packed attestation certificate subject OU MUST be \"Authenticator Attestation\" (WebAuthn 8.2.1)");
}
var AAGUID_EXT_OID = null;   // resolved lazily to avoid a dotted literal at module top
...
function _checkAaguidExt(cert, aaguid) {
  if (!AAGUID_EXT_OID) { try { AAGUID_EXT_OID = oid.byName("idFidoGenCeAaguid"); } catch (_e) { AAGUID_EXT_OID = null; } }
  var ext = AAGUID_EXT_OID && (cert.extensions || []).filter(function (e) { return e.oid === AAGUID_EXT_OID; })[0];
...
var VERIFIERS = {
  ...
  tpm: function (att, clientDataHash) {
    ...
    var alg = _algOf(att.attStmt), sig = _sigOf(att.attStmt);
    var pubAreaBytes = cbor.read.byteString(_reqAttr(att.attStmt, "pubArea"));
    var certInfoBytes = cbor.read.byteString(_reqAttr(att.attStmt, "certInfo"));
...
function _result(fmt, attestationType, trustPath, att) {
  return { verified: true, fmt: fmt, attestationType: attestationType, trustPath: trustPath, aaguid: att.authData.aaguid, credentialPublicKey: att.authData.credentialPublicKey };
}
...
function _checkAndroidKeyDescription(cert, clientDataHash) {
  ...
  // (3) [702] origin == KM_ORIGIN_GENERATED (0) in hardwareEnforced.
  var originNode = _alGet(hardwareEnforced, 702);
  if (!originNode || asn1.read.integer(originNode) !== 0n) throw _err("webauthn/verify-failed", "android key origin is not KM_ORIGIN_GENERATED (WebAuthn 8.4.1)");
  // (4) [1] purpose (SET OF INTEGER) MUST contain KM_PURPOSE_SIGN (2) in hardwareEnforced.
  var purposeNode = _alGet(hardwareEnforced, 1);
  var hasSign = false;
  if (purposeNode && purposeNode.children) {
    for (var i = 0; i < purposeNode.children.length; i++) { if (asn1.read.integer(purposeNode.children[i]) === 2n) { hasSign = true; break; } }
  }
  if (!hasSign) throw _err("webauthn/verify-failed", "android key purpose does not contain KM_PURPOSE_SIGN (WebAuthn 8.4.1)");
}
 ...
 *   Chaining the returned `trustPath` (the x5c chain) to a trusted root is the caller's
