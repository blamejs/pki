...
function _checkAndroidKeyDescription(cert, clientDataHash) {
  ...
  // (3)+(4) origin == KM_ORIGIN_GENERATED (0) and purpose contains KM_PURPOSE_SIGN
  ...
  if (!(_originIsGenerated(hardwareEnforced) || _originIsGenerated(softwareEnforced))) {
    throw _err("webauthn/verify-failed", "android key origin is not KM_ORIGIN_GENERATED (WebAuthn 8.4.1)");
  }
  if (!(_purposeHasSign(hardwareEnforced) || _purposeHasSign(softwareEnforced))) {
    throw _err("webauthn/verify-failed", "android key purpose does not contain KM_PURPOSE_SIGN (WebAuthn 8.4.1)");
  }
}
// [702] origin present in the list and == KM_ORIGIN_GENERATED (0).
function _originIsGenerated(list) { var o = _alGet(list, 702); return !!o && asn1.read.integer(o) === 0n; }
// [1] purpose (SET OF INTEGER) present in the list and containing KM_PURPOSE_SIGN (2).
function _purposeHasSign(list) {
  var p = _alGet(list, 1);
  if (!p || !p.children) return false;
  for (var i = 0; i < p.children.length; i++) { if (asn1.read.integer(p.children[i]) === 2n) return true; }
  return false;
}
