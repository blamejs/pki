...
function verify(attestationObject, clientDataHash, opts) {
  ...
  try {
    ...
    if (haveJson) {
      ...
      clientData = parseClientData(opts.clientDataJSON, {
        expectedType: "webauthn.create",
        expectedChallenge: opts.expectedChallenge,
        expectedOrigin: opts.expectedOrigin,
        expectedTopOrigin: opts.expectedTopOrigin,
      });
      cdh = _sha("sha256", _bytesArg(opts.clientDataJSON, "opts.clientDataJSON"));
    } else {
 ...
 * which an origin list cannot express.
 *
 * @opts
...
function parseClientData(bytes, opts) {
  ...
  if (opts.expectedTopOrigin !== undefined) {
    ...
    if (wantTop === null) {
      if (doc.topOrigin !== undefined) {
        throw _err("webauthn/client-data-mismatch",
          "the clientDataJSON carries topOrigin " + JSON.stringify(doc.topOrigin) + ", and this relying party requires an unframed ceremony (WebAuthn sec. 5.8.1)");
      }
    } else {
      ...
      if (allowedTop.indexOf(doc.topOrigin) === -1) {
        throw _err("webauthn/client-data-mismatch",
          "the clientDataJSON topOrigin " + JSON.stringify(doc.topOrigin === undefined ? null : doc.topOrigin) +
          " is not one this relying party accepts (WebAuthn sec. 5.8.1)");
      }
    }
