...
// properties to answer one question, and either answering true admits a name the table never had.
...
function _result(fmt, attestationType, chain, att) {
  return {
    // `valid` / `attestationVerified` say the attestation format was validly processed, which the `none`
    // format (WebAuthn 8.7, no attestation) satisfies too. The ASSURANCE level is `attestationType`
    // (`None` for none): a relying party that requires cryptographic attestation gates on it, not on `valid`.
...
function verifyAssertion(input) {
  ...
  return Promise.resolve().then(function () {
    ...
    return _verifySig(coseKey.alg, Buffer.from(input.signature), spki, message, _err).then(function (ok) {
      ...
      return {
        valid: true,   // canonical toolkit-wide verdict alias, beside the verb's own `signatureVerified`
