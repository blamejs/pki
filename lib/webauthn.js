...
var COSE_ALG_HASH = intrinsic.assign(intrinsic.create(null), { "-7": "sha256", "-9": "sha256", "-257": "sha256", "-37": "sha256", "-35": "sha384", "-51": "sha384", "-258": "sha384", "-38": "sha384", "-36": "sha512", "-52": "sha512", "-259": "sha512", "-39": "sha512", "-65535": "sha1" });
...
var COSE_ALG = intrinsic.assign(intrinsic.create(null), {
...
var ED_OID_NAME = intrinsic.create(null);
...
function _requireAttShape(attStmt, allowed, required) {
  var have = intrinsic.create(null);
...
var ATT_STMT_MAJOR = intrinsic.assign(intrinsic.create(null), { compound: 4 });
...
function _cloneParsed(v, depth) {
  ...
  if (Array.isArray(v)) return guard.list.copyMap(v, function (x) { return _cloneParsed(x, depth + 1); });
...
function _anchoredRoutes(res, base) {
  ...
  if (_safetyNetAnchored(res) && routes.indexOf("safetyNetRoots") < 0) guard.list.append(routes, "safetyNetRoots");
...
var _exts = intrinsic.assign(intrinsic.create(null), { find: _findExt, decode: _decodeExt });
...
var VERIFIERS = intrinsic.assign(intrinsic.create(null), {
...
function verify(attestationObject, clientDataHash, opts) {
  ...
  try {
    ...
    vopts = guard.identifier.ownOptions(opts);
    vopts.rootCertificates = _snapshotRoots(opts.rootCertificates);
    vopts.safetyNetRoots = _snapshotRoots(opts.safetyNetRoots);
  } catch (e) { return Promise.reject(e); }
...
function _applyMetadata(res, att, opts) {
  ...
  function govern(info) {
    ...
    guard.list.append(applied, { entry: entry, anchors: anchors, identifier: identifier });
  ...
  for (var gi = 0; gi < paths.length; gi++) {
    try {
      guard.list.append(governed, { info: paths[gi], g: govern(paths[gi]) });
    } catch (ge) {
      if (ge && ge.code === "webauthn/metadata-not-found") { missed = missed || ge; guard.list.append(missedPaths, paths[gi]); continue; }
      throw ge;
    }
  }
  if (missed) {
...
function parseClientData(bytes, opts) {
  ...
  /** @internal Each expected* option is read to decide whether to compare and read again for the
   *  value compared, and `checked` then records that the comparison ran. An option reached through an
   *  accessor answers each read separately, so `checked` could describe a comparison against a value
   *  the caller never supplied. Take each option once. */
  opts = guard.identifier.snapshotOptions(opts, _CLIENT_DATA_OPTS);
...
function _snapshotAssertion(input) {
  ...
  var out = guard.identifier.ownOptions(input);
  ...
  if (_isBufferSource(out.credentialPublicKey)) {
    ...
    var key = intrinsic.create(null);
    var coseNames = guard.identifier.optionNames(out.credentialPublicKey);
    for (var ci = 0; ci < coseNames.length; ci++) {
      var kk = coseNames[ci];
      var v;
  ...
  if (Array.isArray(out.allowedAlgorithms)) out.allowedAlgorithms = guard.list.snapshot(out.allowedAlgorithms);
  if (Array.isArray(out.expectedOrigin)) out.expectedOrigin = guard.list.snapshot(out.expectedOrigin);
  if (Array.isArray(out.expectedTopOrigin)) out.expectedTopOrigin = guard.list.snapshot(out.expectedTopOrigin);
  return out;
}
