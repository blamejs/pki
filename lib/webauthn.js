...
function _applyMetadata(res, att, opts) {
  ...
  // An attestation with no trust path (`none`, or a self-attestation) has nothing to anchor, so a
  // caller who asked for metadata enforcement must be told it could not be applied rather than
  // receiving a pass that looks like it was.
  if (!res.trustPath || res.trustPath.length === 0) {
  ...
  if (declared) {
    ...
    // No model identity: this is the U2F case, keyed by the attestation certificate instead.
    // trustPath is anchor-adjacent first, so that certificate is the LAST element -- the one whose
    // key the catalogue identifies, not the root it chains to.
    var keyId = mds.certKeyIdentifier(res.trustPath[res.trustPath.length - 1]);
  ...
  return mds.chainToAnchor(res.trustPath.slice().reverse(), anchors, at,
