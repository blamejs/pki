...
/** @internal The parameters of a KEM-based MAC protectionAlg (RFC 9810 sec. 5.1.3.4). `len` is the
 * size of the key the KDF produces, and it is the size the MAC algorithm needs, so the two travel
 * together and a verifier derives exactly what the protector derived. */
var KEM_BM_PARAMETER = schema.seq([
  schema.field("kdf", ALG_ID),
  /** @internal The CMP module is DEFINITIONS EXPLICIT TAGS, so `[0]` wraps the OCTET STRING rather
   * than replacing its tag. */
  schema.optional("kemContext", schema.octetString(), { tag: 0, explicit: true }),
  schema.field("len", schema.integerLeaf()),
  schema.field("mac", ALG_ID),
], {
  assert: "sequence", arity: { min: 3, max: 4 }, code: "cmp/bad-kem-data", what: "KemBMParameter",
  build: function (m, ctx) {
    var n = guard.range.int(m.fields.len.value, 1n, BigInt(constants.LIMITS.CMP_KEM_MAX_KEY_BYTES),
      ctx.E, "cmp/bad-kem-data", "a KemBMParameter len");
    return {
      kdf: m.fields.kdf.value.result,
      kemContext: m.fields.kemContext.present ? m.fields.kemContext.value : null,
      len: n,
      mac: m.fields.mac.value.result,
    };
  },
});

/** @internal The KEM ciphertext a peer sends so the holder of the KEM private key can derive the same
 * secret (RFC 9810 sec. 5.3.19.18). */
var KEM_CIPHERTEXT_INFO = schema.seq([
  schema.field("kem", ALG_ID),
  schema.field("ct", schema.octetString()),
], {
  assert: "sequence", arity: { exact: 2 }, code: "cmp/bad-kem-ciphertext", what: "KemCiphertextInfo",
  build: function (m) { return { kem: m.fields.kem.value.result, ct: m.fields.ct.value }; },
});
...
var CERT_TEMPLATE_LEAF = schema.decode(function (n, ctx) { return crmf.walkCertTemplate(n, ctx); });
...
var ENCRYPTED_KEY = schema.choice([
  ...
  { when: { tagClass: "context", tagNumber: 0 },
    schema: schema.decode(function (n, ctx) {
      var env;
      try { env = cms.walkEnvelopedData(pkix.decodeNested(asn1.sequenceTlv(n), ctx, "an EncryptedKey envelopedData"), ctx); }
      catch (e) {
      ...
      return { envelopedData: env, bytes: asn1.sequenceTlv(n) };
    }) },
...
var CRMF_ARMS = Object.assign(Object.create(null), { 0: true, 2: true, 7: true, 9: true, 13: true });
var DECODED_ARMS = Object.assign(Object.create(null), {
...
var BODY = schema.decode(function (n, ctx) {
  ...
  if (CRMF_ARMS[n.tagNumber]) {
    ...
    out.decoded = crmf.walkCertReqMessages(inner, { allowSigningAlg: isCcr, allowV1Version: isCcr }, ctx);
  ...
  build: function (m, ctx) {
      ...
      for (var cc = 0; cc < body.decoded.messages.length; cc++) {
        ...
        if (controls) {
          for (var cn = 0; cn < controls.length; cn++) {
            ...
            var arch;
            try { arch = pkix.decodeNested(controls[cn].value, ctx, "a pkiArchiveOptions control value"); }
      ...
      for (var q = 0; q < body.decoded.messages.length; q++) {
        ...
        if (archCtrls && header.pvno !== 3) {
          for (var ac = 0; ac < archCtrls.length; ac++) {
            ...
            var opt = pkix.decodeNested(archCtrls[ac].value, ctx, "a pkiArchiveOptions control value");
    ...
    if (header.generalInfo) {
      for (var gi = 0; gi < header.generalInfo.length; gi++) {
        ...
        var profileCount = pkix.decodeNested(header.generalInfo[gi].value, ctx, "a certProfile infoValue").children.length;
        var bodyCount = null;
    ...
    if (body.arm === "ckuann" && header.pvno !== 3) {
      var caKeyUpd = pkix.decodeNested(body.bytes, ctx, "a ckuann body");
    ...
    if (body.arm === "popdecc" && header.pvno !== 3) {
      var challenges = pkix.decodeNested(body.bytes, ctx, "a popdecc body");
...
var CA_CERTS_VALUE = schema.seqOf(rawSequence("cmp/bad-info-value"), {
  min: 1, code: "cmp/bad-info-value", what: "CACertsValue",
  build: function (m) { return m.items.map(function (it) { return it.value; }); },
});
...
var ROOT_CA_KEY_UPDATE = schema.seq([
  ...
  assert: "sequence", code: "cmp/bad-info-value", what: "RootCaKeyUpdateContent",
  build: function (m) {
    return {
...
var ATTR_TYPE_AND_VALUE = schema.seq([
  ...
  assert: "sequence", code: "cmp/bad-info-value", what: "AttributeTypeAndValue",
  build: function (m, ctx) {
...
var CERT_REQ_TEMPLATE_VALUE = schema.seq([
  ...
  schema.optional("keySpec", schema.seqOf(ATTR_TYPE_AND_VALUE, {
    min: 1, code: "cmp/bad-info-value", what: "CertReqTemplate keySpec",
    build: function (m) { return m.items.map(function (it) { return it.value.result; }); },
  }), { whenUniversal: [TAGS.SEQUENCE] }),
], {
  assert: "sequence", code: "cmp/bad-info-value", what: "CertReqTemplateValue",
  build: function (m) {
    return {
...
var CRLS_VALUE = schema.seqOf(rawSequence("cmp/bad-info-value"), {
  min: 1, code: "cmp/bad-info-value", what: "CRLsValue",
  build: function (m) { return m.items.map(function (it) { return it.value; }); },
});

function _valueParser(what, topSchema) {
  return pkix.makeParser({ pemLabel: "CMP", PemError: PemError, ErrorClass: CmpError, prefix: "cmp", what: what, topSchema: topSchema, ns: NS });
}
// @internal
 ...
 * @signature  pki.schema.cmp.parse(input, caps?) -> pkiMessage
 * @since      0.1.19
 * @status     stable
 ...
 * surfaces raw with `decoded` absent: `p10cr` (feed `body.bytes` to
 * `pki.schema.csr.parse`), the challenge-response and announcement arms,
 * and `nested` (never auto-recursed). `certReqId` values
 ...
 *   var cert = await pki.x509.sign({ subject: "client", subjectPublicKey: await pki.key.export(pair.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z") }, { key: key });
 *   var csrDer = await pki.csr.sign({ subject: "client", subjectPublicKey: await pki.key.export(pair.publicKey) }, { key: key });
 *   var der = await pki.cmp.build(
 *     { header: { sender: { directoryName: "CN=client" }, recipient: { directoryName: "CN=CA" } },
 *       body: { p10cr: csrDer } }, { key: key, cert: cert });
 ...
 *     { header: { sender: { directoryName: "CN=client" }, recipient: { directoryName: "CN=CA" } },
 *       body: { p10cr: csrDer } }, { key: key, cert: cert });
 *   var pemText = pki.schema.cmp.pemEncode(msg);
...
/** @internal One embedded KEM structure, read under this module's namespace so every field of it
 * reports in the CMP domain rather than the codec's. */
function _readKem(sch, der, code, what) {
  return schema.embeddedDer(sch, der, NS, { code: code, what: what }).result;
}
function readKemBMParameter(p) { return _readKem(KEM_BM_PARAMETER, p, "cmp/bad-kem-data", "KemBMParameter"); }
function readKemCiphertextInfo(d) { return _readKem(KEM_CIPHERTEXT_INFO, d, "cmp/bad-kem-ciphertext", "KemCiphertextInfo"); }
...
module.exports = {
  ...
  matches: matches,
  // @internal
  readCaCerts: readCaCerts,
  readRootCaKeyUpdate: readRootCaKeyUpdate,
  readCertReqTemplate: readCertReqTemplate,
  readCrls: readCrls,
  readAlgorithmIdentifier: readAlgorithmIdentifier,
  readName: readName,
  readKemBMParameter: readKemBMParameter,
  readKemCiphertextInfo: readKemCiphertextInfo,
};
