...
function rawSequence(code) {
  return schema.decode(function (n, ctx) {
    if (!(n.tagClass === "universal" && n.tagNumber === TAGS.SEQUENCE && n.children)) {
      throw ctx.E(code, "expected a universal SEQUENCE (Certificate / CertificateList / PKIPublicationInfo), not a " + n.tagClass + "/" + n.tagNumber);
    }
    return n.bytes;
  });
}
...
var CERT_OR_ENC_CERT = schema.choice([
  { when: { tagClass: "context", tagNumber: 0 },
    schema: schema.explicit(0, schema.decode(function (n, ctx) {
      if (!(n.tagClass === "universal" && n.tagNumber === TAGS.SEQUENCE && n.children)) {
        throw ctx.E("cmp/bad-cert-response", "a certificate [0] must be a CMPCertificate (a universal SEQUENCE)");
      }
