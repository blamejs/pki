...
var PKI_FAILURE_INFO = schema.decode(function (n, ctx) {
  ...
  for (var i = 0; i < total; i++) {
    if ((bs.bytes[i >> 3] >> (7 - (i % 8))) & 1) _push(bits, i < FAIL_INFO_NAMES.length ? FAIL_INFO_NAMES[i] : i);
  }
  ...
  build: function (m, ctx) {
    ...
    if (body.decoded && body.decoded.response) {
      for (var r = 0; r < body.decoded.response.length; r++) {
        if (body.decoded.response[r].certifiedKeyPair) _push(ckps, body.decoded.response[r].certifiedKeyPair);
      }
    }
    if (body.decoded && body.decoded.keyPairHist) {
      for (var kh = 0; kh < body.decoded.keyPairHist.length; kh++) _push(ckps, body.decoded.keyPairHist[kh]);
    }
    ...
    if (header.generalInfo) {
      for (var gi = 0; gi < header.generalInfo.length; gi++) {
        ...
        else if (body.arm === "genm" && _isArray(body.decoded)) bodyCount = body.decoded.length;
