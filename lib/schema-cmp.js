...
var BODY = schema.decode(function (n, ctx) {
  ...
  if (CRMF_ARMS[n.tagNumber]) {
    ...
    // sec. 5 for this one arm). Those relaxations are allowed. The D.6 profile
    // also expects exactly one CertReqMsg, but sec. 5.3.11 defines a ccr as a
    // CertReqMessages (SIZE (1..MAX)) and App. D is OPTIONAL, so the base parser
    // does not enforce the single-request cardinality (an App. D.6 producer keeps
    // to one by its own policy, sending multiple cross-certificates in separate
    // messages); only the no-private-key restriction below is normative.
...
var PKI_MESSAGE = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    if (body.arm === "ccp" && body.decoded && body.decoded.response) {
      // RFC 9810 Appendix D.6 profiles a one-way ccp as exactly one CertResponse (the mirror of the ccr
      // one-CertReqMsg profile), but sec. 5.3.12 defines a ccp as a CertRepMessage (SIZE (1..MAX)) and App. D
      // is OPTIONAL, so the base parser does not enforce the single-response cardinality; only the
      // no-private-key restriction is normative.
...
// ---- support-message infoValue structures (RFC 9480 sec. 2.14-2.17) -----
//
// A genm / genp carries its payload as an ANY, so the InfoTypeAndValue leaf above
// surfaces it as raw bytes. These are the strict decoders for the four values RFC 9483
// sec. 4.3 defines, each a standalone parser so a malformed payload is refused with the
// same caps, minimal-encoding and trailing-byte rules the message itself is held to.
// The CMP module is EXPLICIT TAGS, so every context tag here wraps its value.

// CACertsValue ::= SEQUENCE SIZE (1..MAX) OF CMPCertificate. An absent infoValue is how
// "no CA certificate is available" is said (sec. 4.3.1), so an EMPTY sequence is malformed
// rather than a second spelling of the same answer.
var CA_CERTS_VALUE = schema.seqOf(rawSequence("cmp/bad-info-value"), {
  min: 1, code: "cmp/bad-info-value", what: "CACertsValue",
  build: function (m) { return m.items.map(function (it) { return it.value; }); },
});

// RootCaKeyUpdateContent ::= SEQUENCE { newWithNew CMPCertificate,
//   newWithOld [0] CMPCertificate OPTIONAL, oldWithNew [1] CMPCertificate OPTIONAL }
// (RFC 9480 sec. 2.15). Both optionals are OPTIONAL in the syntax; RFC 9483 sec. 4.3.2
// requires newWithOld of a response, which the caller of this decoder enforces.
...
// AttributeTypeAndValue ::= SEQUENCE { type OBJECT IDENTIFIER, value ANY }, the element of
// the Controls that carries a CertReqTemplate keySpec (RFC 9480 sec. 2.16). The value stays
// raw here: which syntax it must have depends on the type, which sec. 4.3.3 constrains.
...
// (RFC 9480 sec. 2.16). keySpec is untagged, so it is recognized by its universal SEQUENCE tag
// following the CertTemplate, which is itself IMPLICIT [tagged] by the CRMF module.
...
// CRLsValue ::= SEQUENCE SIZE (1..MAX) OF CertificateList (RFC 9480 sec. 2.17).
...
// @internal -- pki.cmp.session reads a support-message response through these.
...
// A CertReqTemplate keySpec states a key requirement as an AlgorithmIdentifier (RFC 9480 sec. 2.16),
// which is the shared sub-schema every format already reads one with: `SEQUENCE { algorithm OID,
// parameters ANY OPTIONAL }` and nothing after it. Reading it through the same definition is what
// keeps a third field from passing as a well-formed one.
var readAlgorithmIdentifier = _valueParser("AlgorithmIdentifier", ALG_ID);
// A bare Name, so a caller-encoded issuer can be read back into the RDN sequence that guard-name's
// canonical comparison takes. pki.cmp.session binds a returned CRL to the issuer it asked for.
