...
var PKI_FAILURE_INFO = schema.decode(function (n, ctx) {
  ...
  if (total > 0) {
    ...
    if (((lastByte >> bs.unusedBits) & 1) !== 1) {
      throw ctx.E("cmp/bad-fail-info", "a named-bit BIT STRING must drop trailing zero bits (X.690 §11.2.2)");
    }
  }
  var bits = [];
...
// A raw element surfaced byte-exact (CMPCertificate, CertificateList, ...).
var RAW_BYTES = schema.decode(function (n) { return n.bytes; });
  ...
  build: function (m, ctx) {
    ...
    if (valueNode !== null) {
      if (t === OID_IMPLICIT_CONFIRM &&
          !(valueNode.tagClass === "universal" && valueNode.tagNumber === TAGS.NULL)) {
        throw ctx.E("cmp/bad-info-value", "an implicitConfirm value must be NULL (RFC 9810 §5.1.1.1)");
      }
      if (t === OID_CONFIRM_WAIT_TIME &&
          !(valueNode.tagClass === "universal" && valueNode.tagNumber === TAGS.GENERALIZED_TIME)) {
        throw ctx.E("cmp/bad-info-value", "a confirmWaitTime value must be a GeneralizedTime (RFC 9810 §5.1.1.2)");
      }
      if (t === OID_CERT_PROFILE) {
        var ok = valueNode.tagClass === "universal" && valueNode.tagNumber === TAGS.SEQUENCE &&
                 valueNode.children && valueNode.children.length >= 1;
        if (ok) {
          for (var i = 0; i < valueNode.children.length; i++) {
            if (!(valueNode.children[i].tagClass === "universal" && valueNode.children[i].tagNumber === TAGS.UTF8_STRING)) { ok = false; break; }
          }
        }
        if (!ok) throw ctx.E("cmp/bad-info-value", "a certProfile value must be a non-empty SEQUENCE OF UTF8String (RFC 9810 §5.1.1.4)");
      }
    }
...
var REV_REP_CONTENT = schema.seq([
  ...
  schema.trailing([
    ...
    { tag: 1, name: "crls", schema: schema.seqOf(RAW_BYTES, {
...
var CERT_OR_ENC_CERT = schema.choice([
  { when: { tagClass: "context", tagNumber: 0 },
    schema: schema.explicit(0, schema.decode(function (n) { return { certificate: n.bytes }; }),
...
var CERTIFIED_KEY_PAIR = schema.seq([
  ...
  schema.trailing([
    ...
    { tag: 1, name: "publicationInfo", schema: RAW_BYTES, explicit: true, emptyCode: "cmp/bad-cert-response" },
...
var CERT_RESPONSE = schema.seq([
  ...
  build: function (m) {
    return {
      certReqId: m.fields.certReqId.value,
      status: m.fields.status.value.result,
      certifiedKeyPair: m.fields.certifiedKeyPair.present ? m.fields.certifiedKeyPair.value.result : null,
...
var CERT_REP_MESSAGE = schema.seq([
  schema.optional("caPubs", schema.seqOf(RAW_BYTES, {
...
var PKI_MESSAGE = schema.seq([
  ...
  schema.trailing([
    ...
    { tag: 1, name: "extraCerts", schema: schema.seqOf(RAW_BYTES, {
...
function matches(root) {
  ...
  var k = root.children;
  if (k.length < 2 || k.length > 4) return false;
  var header = k[0];
  if (!(header.tagClass === "universal" && header.tagNumber === TAGS.SEQUENCE && header.children && header.children.length >= 3)) return false;
  var pvno = header.children[0];
  if (!(pvno.tagClass === "universal" && pvno.tagNumber === TAGS.INTEGER)) return false;
  var body = k[1];
  return body.tagClass === "context" && !!body.children && body.tagNumber >= 0 && body.tagNumber <= 26;
}
