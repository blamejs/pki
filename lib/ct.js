...
async function verifyLogListSignature(json, signature, publicKey) {
  ...
  if (alg.algOid === oid.byName("rsaEncryption")) {
    ...
    // A DER ECDSA-Sig-Value: route through the strict conformance gate (primitive, minimal, positive,
    // bounded r/s -- defeating the CVE-2022-21449 r=s=0 forgery) before the raw r||s conversion.
    sig = validator.sig.ecdsaSigToRaw(sig, ec.coordLen, CtError, "ct/bad-signature");
  } else {
