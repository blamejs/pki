...
var _dateNow = intrinsic.dateNow;
...
function _parseSct(r, sctLen) {
  ...
  var timestampMs = timestamp <= MAX_SAFE ? _Number(timestamp) : null;
  return {
    version: 0,
    logId: logId, logIdHex: logId.toString("hex"),
    timestamp: timestamp,
    timestampMs: timestampMs,
    timestampDate: new Date(timestampMs != null ? timestampMs : _Number(timestamp)),
...
function parseSctList(extValue) {
  ...
  while (!outer.atEnd()) {
    ...
    if (one.unknown) { var u = { version: one.version, rawSct: one.rawSct }; _push(unknownScts, u); _push(all, u); }
    else { _push(scts, one); _push(all, one); }
  }
...
function reconstructSignedData(entry, sct) {
  ...
  var parts = [];
  _push(parts, _bufferFrom([sct.version & 0xff]));                          // Version -- v1(0)
  _push(parts, _bufferFrom([0]));                                          // SignatureType -- certificate_timestamp(0)
  var ts = _bufferAlloc(8); ts.writeBigUInt64BE(tsVal); _push(parts, ts);   // uint64 timestamp
  _push(parts, _bufferFrom([(entryType >> 8) & 0xff, entryType & 0xff]));  // LogEntryType (2 bytes BE)
  if (entryType === 0) {
    var cert = _toBuffer(entry.leafCert, "leafCert");
    _push(parts, _u24Bytes(cert.length)); _push(parts, cert);                // ASN.1Cert<1..2^24-1>
  } else {
    ...
    _push(parts, ikh);
    _push(parts, _u24Bytes(tbs.length)); _push(parts, tbs);                  // PreCert.tbs_certificate<1..2^24-1>
  }
  ...
  _push(parts, _bufferFrom([(ext.length >> 8) & 0xff, ext.length & 0xff])); _push(parts, ext);   // CtExtensions
...
async function verifySct(entry, sct, logPublicKey) {
  ...
  if (sct.logId != null) {
    var keyId = _bufferFrom(await subtle.digest("SHA-256", spki));
  ...
  if (!hashName) throw new CtError("ct/unsupported-algorithm", "unsupported SCT hash algorithm " + _stringify(sigInfo.hashName) + " (RFC 6962 sec. 2.1.4 mandates sha256)");
  ...
  if (sigInfo.signatureName === "ecdsa") {
    ...
    throw new CtError("ct/unsupported-algorithm", "unsupported SCT signature algorithm " + _stringify(sigInfo.signatureName) + " (RFC 6962 sec. 2.1.4 supports ecdsa/rsa)");
  }
...
function _encodeSctBody(sct) {
  ...
  if (sct.version !== 0) {
    ...
    if (typeof sct.version !== "number" || !_isInteger(sct.version) || sct.version < 0 || sct.version > 255) {
...
function encodeSctList(scts) {
  if (!_isArray(scts)) throw new CtError("ct/bad-input", "encodeSctList expects an array of SCTs");
  ...
  lw.vector(2, 1, 0xffff, _bufferConcat(elements, total), "ct/too-large");   // sct_list<1..2^16-1>
...
function _logKeyMaterial(logKey) {
  ...
  try {
    ...
    else if (_isBuffer(logKey)) keyObj = nodeCrypto.createPrivateKey({ key: logKey, format: "der", type: "pkcs8" });
...
async function signSct(entry, logKey, opts) {
  ...
  var timestamp;
  if (opts.timestamp == null) timestamp = _BigInt(_dateNow());
  else if (typeof opts.timestamp === "bigint") timestamp = opts.timestamp;
  else if (typeof opts.timestamp === "number" && _isSafeInteger(opts.timestamp) && opts.timestamp >= 0) timestamp = _BigInt(opts.timestamp);
  else throw new CtError("ct/bad-input", "timestamp must be a finite non-negative integer or _BigInt(RFC 6962 sec. 3.2)");
  var extensions = opts.extensions == null ? _bufferAlloc(0) : _toBuffer(opts.extensions, "opts.extensions");
  var logId = _bufferFrom(await subtle.digest("SHA-256", mat.spki));
  ...
  var sigRaw = _bufferFrom(await subtle.sign(sign, priv, preimage));
...
function parseLogList(json, opts) {
  ...
  if (doc == null || typeof doc !== "object" || !_isArray(doc.operators)) throw _ctErr("ct/bad-log-list", "the CT log list must be a JSON object with an operators array");
  var logs = [], byLogId = _create(null);
  for (var i = 0; i < doc.operators.length; i++) {
    ...
    for (var a = 0; a < arrays.length; a++) {
      ...
      if (!_isArray(arr)) throw _ctErr("ct/bad-log-list", "a CT log-list operator's logs / tiled_logs must be an array");
...
async function verifySctWithLogList(entry, sct, logList, opts) {
  ...
  if (!log.state.trusted) {
    ...
    if (ts >= _BigInt(guard.time.instantOf(log.state.since))) throw _ctErr("ct/log-untrusted", "the CT log is retired and the SCT is not timestamped before its retirement (" + log.state.since.toISOString() + ")");
  }
...
async function verifyLogListSignature(json, signature, publicKey) {
  var message = typeof json === "string" ? _bufferFrom(json) : _toBuffer(json, "the CT log list JSON");
...
function _fetchCtypeToken(headers) { return _String((headers || {})["content-type"] || "").split(";")[0].trim().toLowerCase(); }
...
function _parseHttpsUrl(u, label) {
  var parsed;
  try { parsed = new URL(_String(u)); }
  catch (e) { throw _ctErr("ct/bad-url", "the CT log-list " + label + " did not parse: " + _String(u), e); }
  if (parsed.protocol !== "https:") throw _ctErr("ct/insecure-url", "the CT log-list " + label + " must be https, got " + parsed.protocol + " for " + _String(u));
  return parsed;
}
...
function _sigUrlFor(parsed) {
  if (!/\.json$/.test(parsed.pathname)) throw _ctErr("ct/bad-input", "cannot derive the detached-signature URL from a non-.json path (" + _stringify(parsed.pathname) + "); pass opts.sigUrl explicitly");
...
function _fetchBody(transport, url, req, label) {
  ...
  return Promise.resolve().then(function () {
    ...
    res = res || {};
    var h = {};
    _keys(res.headers || {}).forEach(function (k) { h[k.toLowerCase()] = res.headers[k]; });
    ...
    if (res.status !== 200) throw _ctErr("ct/http-error", "the CT server returned HTTP " + _stringify(res.status) + " for the " + label);
...
async function fetchLogList(opts) {
  ...
  var headers = {};
  _keys(opts.headers || {}).forEach(function (k) {
  ...
  if (opts.requireJsonContentType === true && jsonRes.contentType !== "application/json") throw _ctErr("ct/bad-content-type", "the CT log-list JSON GET returned content-type " + _stringify(jsonRes.contentType || null) + " (opts.requireJsonContentType is set)");
