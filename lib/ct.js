...
// The TLS SignatureAndHashAlgorithm hash names an SCT may carry -> the WebCrypto digest.
// RFC 6962 sec. 2.1.4 requires SHA-256; the others are accepted defensively for a
// non-conformant log rather than silently mis-hashed.
var CT_HASH = { sha256: "SHA-256", sha384: "SHA-384", sha512: "SHA-512" };
// The EC named curve an SCT log key may use, keyed by the curve OID carried in its SPKI ->
// the WebCrypto curve name + the r/s coordinate width. RFC 6962 sec. 2.1.4 mandates NIST
// P-256 for ECDSA logs; P-384 / P-521 are accepted defensively.
var CT_EC_CURVE = {};
CT_EC_CURVE[oid.byName("prime256v1")] = { curve: "P-256", coordLen: 32 };
CT_EC_CURVE[oid.byName("secp384r1")] = { curve: "P-384", coordLen: 48 };
CT_EC_CURVE[oid.byName("secp521r1")] = { curve: "P-521", coordLen: 66 };
...
function _spkiAlg(spki) {
  ...
  var out = { algOid: asn1.read.oid(algId.children[0]) };
 ...
 * @example
 *   var sct = pki.ct.parseSctList(sctExtValue).scts[0];
 *   // logKey is the CT log's DER SubjectPublicKeyInfo, resolved from sct.logIdHex.
 *   var logKey = pki.schema.x509.parse(certDer).subjectPublicKeyInfo.bytes;
