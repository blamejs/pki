 ...
 * would fall back to the default client and fetch the log list over the network for a caller that
 * believed it had injected one. */
function _assertTransport(transport) {
  if (transport != null && typeof transport !== "function") {
    throw _ctErr("ct/bad-input", "opts.transport must be a function (request) => Promise<{ status, headers, body }>, got " + guard.text.showValue(transport));
  }
}
