...
// Strip origin-bound credential headers (case-insensitive) so a cross-origin request never carries them.
function _stripCredHeaders(headers) {
  var out = {};
  Object.keys(headers).forEach(function (k) {
    var lk = k.toLowerCase();
    if (lk !== "authorization" && lk !== "cookie" && lk !== "proxy-authorization") out[k] = headers[k];
  });
  return out;
}

// Strip the mTLS client certificate/key so a cross-origin request does not present them to a second host.
function _stripClientCert(tls) {
  var out = {};
  Object.keys(tls).forEach(function (k) { if (k !== "cert" && k !== "key") out[k] = tls[k]; });
  return out;
}
...
function _fetchBody(transport, url, req, label) {
  return Promise.resolve(transport({ method: "GET", url: url, headers: req.headers, tls: req.tls, timeout: req.timeout, maxResponseBytes: req.maxResponseBytes })).then(function (res) {
    res = res || {};
    var h = {};
 ...
 * @opts sigUrl OPTIONAL -- the detached `log_list.sig` URL (https); default `url` with `.json` -> `.sig` (a non-.json url requires an explicit sigUrl).
...
async function fetchLogList(opts) {
  ...
  var jsonReq = { headers: headers, tls: _tlsForFetch(opts), timeout: timeout, maxResponseBytes: maxResponseBytes };
  // Two bounded GETs; BOTH must succeed before verify. The JSON is fetched first (its content-type is the
  // one opts.requireJsonContentType polices).
  var jsonRes = await _fetchBody(transport, jsonUrl, jsonReq, "CT log-list JSON");
  ...
  // The detached signature may live on a SEPARATE origin (a caller may host log_list.sig on its own CDN).
  // Origin-bound credentials -- an Authorization / Cookie / Proxy-Authorization header and the mTLS client
  // certificate -- are meant for the origin they were configured for, so they are STRIPPED from a
  // cross-origin sig request: the log-list endpoint's credentials must never leak to the signature host (the
  // est/acme cross-origin credential-strip discipline). A same-origin sig reuses the request unchanged.
  var sigReq = _sameOrigin(jsonUrl, sigUrl) ? jsonReq
    : { headers: _stripCredHeaders(headers), tls: _stripClientCert(jsonReq.tls), timeout: timeout, maxResponseBytes: maxResponseBytes };
  var sigRes = await _fetchBody(transport, sigUrl, sigReq, "CT log-list signature");
