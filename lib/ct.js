...
function _encodeSctBody(sct) {
  ...
  if (sct.version !== 0) {
    ...
    if (raw.length < 1 || raw[0] !== (sct.version & 0xff)) {
 ...
 * inverse of `parseSctList`, such that `parseSctList(encodeSctList(scts))` round-trips to
 * identical SCTs. Each element is either a decoded v1 SCT (the shape `parseSctList().scts[]`
 ...
 * `rawSct` is re-emitted verbatim (forward compatibility, sec. 3.3).
 *
 ...
 * the parser's caps (`SCT_MAX_COUNT` elements / `SCT_MAX_BYTES` total) so encode cannot emit
 * what parse would reject. Throws a typed `CtError` (`ct/empty-list`, `ct/bad-input`,
 * `ct/too-large`, `ct/too-many-scts`) on malformed input.
 *
 * @example
 *   var list = pki.ct.parseSctList(sctExtValue);
 *   var reEncoded = pki.ct.encodeSctList(list.scts);   // byte-identical to sctExtValue
 */
function encodeSctList(scts) {
  ...
  for (var i = 0; i < scts.length; i++) {
    ...
    elements.push(ew.build());
  }
  var lw = new TlsWriter();
  lw.vector(2, 1, 0xffff, Buffer.concat(elements), "ct/too-large");     // sct_list<1..2^16-1>
  var blob = lw.build();
  // Coverage residual: defense-in-depth. The sct_list vector above caps the list body at 0xffff, so
  // blob (2 + body) is at most 65537 == SCT_MAX_BYTES and this bound cannot be exceeded here; it stays
  // as an explicit floor should the framing ever widen.
  if (blob.length > C.LIMITS.SCT_MAX_BYTES) throw new CtError("ct/too-large", "SCT list " + blob.length + " bytes exceeds the cap " + C.LIMITS.SCT_MAX_BYTES);
  return asn1.build.octetString(blob);                                  // RFC 6962 sec. 3.3 DER OCTET STRING wrap
}
...
function _logKeyMaterial(logKey) {
  ...
  } catch (e) { throw new CtError("ct/bad-input", "the CT log private key could not be loaded", e); }
