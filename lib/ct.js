...
// Derive the detached-signature URL by the Chrome/Apple convention: the log_list.json URL with its final
// ".json" path suffix rewritten to ".sig". A url whose path does NOT end in ".json" cannot yield an
// unambiguous sig URL, so the caller must pass opts.sigUrl explicitly (guards never guess a value).
function _sigUrlFor(u) {
  var parsed;
  try { parsed = new URL(String(u)); }
  catch (e) { throw _ctErr("ct/bad-url", "the CT log-list URL did not parse: " + String(u), e); }
  ...
  parsed.pathname = parsed.pathname.replace(/\.json$/, ".sig");
  return parsed.href;
}
 ...
 *
 * @opts url REQUIRED -- the `log_list.json` URL (no baked-in vendor URL).
 * @opts signerKey REQUIRED, PINNED -- the distributor SubjectPublicKeyInfo as a DER Buffer; no baked-in key.
 * @opts sigUrl OPTIONAL -- the detached `log_list.sig` URL; default `url` with `.json` -> `.sig` (a non-.json url requires an explicit sigUrl).
...
async function fetchLogList(opts) {
  ...
  var jsonUrl;
  try { jsonUrl = new URL(String(opts.url)).href; }   // parse only -- NO client scheme gate (the transport owns socket security)
  catch (e) { throw _ctErr("ct/bad-url", "the CT log-list URL did not parse: " + String(opts.url), e); }
  var sigUrl;
  if (opts.sigUrl != null) {
    try { sigUrl = new URL(String(opts.sigUrl)).href; }
    catch (e2) { throw _ctErr("ct/bad-url", "the CT log-list signature URL did not parse: " + String(opts.sigUrl), e2); }
  } else {
    sigUrl = _sigUrlFor(jsonUrl);
  }
