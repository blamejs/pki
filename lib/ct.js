...
async function verifySctWithLogList(entry, sct, logList, opts) {
  ...
  // The crypto verdict alone hid WHICH trusted log accepted the SCT and when. Surface the resolved log
  // record so a caller can record the operator / log state / timestamp for a policy decision instead of
  // re-deriving them, rather than reading a bare boolean that drops the whole record it just resolved.
  var valid = await verifySct(entry, sct, c.log.key);   // re-checks the logId binding
  return {
    valid: valid,
    logId: c.log.logId,
...
async function verifySctList(entry, list, logList, opts) {
  ...
  // A present certNotAfter must be a valid Date: a non-Date is a caller mis-shape that throws, never a
  // value silently ignored (entryType 0 would fall back to the certificate's own notAfter) or demoted to
  // a per-SCT temporal failure (entryType 1) -- the malformed-options contract, matching opts.at.
  ...
  for (var i = 0; i < scts.length; i++) {
    ...
    // Classification can THROW for a malformed per-SCT field (a bad timestamp on a retired-log match):
    // that is a recorded per-SCT outcome, never a thrown aggregate abort, so it matches the usable-log
    // malformed-SCT path below and one bad SCT cannot sink the verdict or a valid sibling.
    var c;
    ...
    // issued it after the validation time, so it is recorded as a rejection and never counted. An SCT
    // dated after the certificate's OWN notAfter is deliberately NOT rejected: RFC 6962 sec. 5.1 permits
    // a log to accept an already-expired certificate, so such an SCT is legitimate.
  ...
  return {
    valid: policyOk,   // canonical toolkit-wide verdict alias, beside this verb's `policyOk`
