...
// The checkpoint signature-line marker is EM DASH (U+2014) + space; built at runtime so the source
// file stays pure ASCII. The line is parsed by character scan (no regex): "<EM DASH> <keyId> <sig>",
// two whitespace-separated non-empty tokens, nothing after.
var _EM_DASH = String.fromCharCode(0x2014);

// The \S token scan uses the shared JS regex \s code-point predicate (pkix.isJsWhitespace) so the
// note-line grammar stays exactly the /\S/ complement without a second copy of the code-point set.
function _parseNoteSig(line) {
  ...
  if (p === t1s) return null;                     // \S+ requires at least one character
  var keyId = line.slice(t1s, p);
  if (line.charAt(p) !== " ") return null;        // the literal single space between the tokens
  p += 1;
  var t2s = p;
  while (p < n && !pkix.isJsWhitespace(line.charCodeAt(p))) p += 1;
  if (p === t2s) return null;
  var sig = line.slice(t2s, p);
  if (p !== n) return null;                        // $ -- nothing after the second token
  return { keyId: keyId, sig: sig };
}

// A run of trailing "=" removed (the /=+$/ base64-padding strip).
...
// The recognized bundle media types (draft: the inline ".v0.N+json" form and the
// "+json;version=0.N" form, N in 1..3) -- an explicit set in place of the anchored alternation regex.
...
function _verifyRekor(te, envelope, rekorKeys, leafDer) {
  ...
  // Surface which attested Rekor log entry this is: the global log index and the log's key id (hex), so a
  // caller can locate the exact transparency-log record for an audit rather than re-deriving it.
...
async function verifyBundle(bundle, opts) {
  ...
  // The instant comes through the time guard's intrinsic read. `getTime` is an ordinary method a
  // subclass answers, and the slot predicate above accepts a subclass from any realm, so reading
  // the method off the value let a caller name the moment the Fulcio chain is validated at --
  // epoch zero, say, where every certificate in it is still unissued.
  ...
  return {
    valid: true,   // canonical toolkit-wide verdict alias, beside the verb's own `verified`
    verified: true,
    payload: payload,
    statement: st,
    subjects: st.subject,
    ...
    // Whether the caller pinned a predicateType (opts.predicateType) that was checked. The verdict is
    // only reached when a pinned predicate matched (a mismatch throws sigstore/predicate-mismatch), so
    // a caller that omitted the pin gets `false` and knows the SLSA predicate went unverified.
    predicateTypeChecked: opts.predicateType != null,
    integratedTime: integratedTime,
    // The attested Rekor entry this verdict rests on: `logIndex` is its global log index and `logId` the
    // log's key id (hex), so a caller can fetch or audit the exact transparency-log record.
