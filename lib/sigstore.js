...
/** @internal The shared table's entry, so the SAN this verb reads is read under the rules every
 * other consumer of a GeneralNames reads it under. It is built on the certificate namespace rather
 * than this module's, so a malformed name raises a CertificateError that `_identity` re-types to
 * `sigstore/bad-certificate` with that fault as its cause. That is the code this module already
 * reports for a certificate it cannot read, so the fix adds no new code to the public surface. */
var _sanDecoder = pkix.certExtensionDecoders(
  pkix.makeNS("x509", frameworkError.CertificateError, oid)).byOid[SAN_OID];
...
/** @internal The GeneralNames come from the shared extension decoder, which is where the RFC 5280
 * sec. 4.2.1.6 rules live: at least one name, a non-empty IA5String for the text forms, and no
 * control byte inside one (CVE-2009-2408). Reading the SEQUENCE here instead left this verb
 * accepting a dNSName carrying a NUL or a CR and handing it on as the signer's identity.
 *
 * The one-identity rule counts every name the SAN carries, not the subset this verb turns into an
 * identity string. Counting only the recognized forms let a certificate pair a URI with an
 * iPAddress and pass as single-identity, with the second binding invisible to the caller's policy
 * -- which is the thing the rule exists to stop. A SAN of one name this verb has no string form
 * for still yields null, so a policy gates on the null rather than on a name nothing read. */
function _sanValue(ext) {
  /** @internal The shared decoder is the gate: it applies the sec. 4.2.1.6 rules and throws on a
   * name that breaks one. What it returns is a GeneralNames view rather than the codec nodes the
   * mapping below reads, so the count comes from it and the one name is then taken from the raw
   * decode, which carries the node shape `octetStringImplicit` and the otherName walk need. */
  var validated = _sanDecoder(ext.value).names || [];
  if (validated.length > 1) throw _err("sigstore/bad-certificate", "the Fulcio certificate carries multiple SAN identities");
  var n = (asn1.decode(ext.value).children || [])[0];
  if (!n || n.tagClass !== "context") return null;
  if (!n.constructed && GN_TYPE[n.tagNumber]) {
    return { type: GN_TYPE[n.tagNumber], value: _bufToString(asn1.read.octetStringImplicit(n, n.tagNumber), "utf8") };
  }
  if (n.constructed && n.tagNumber === 0 && (n.children || []).length >= 2) {
    var inner = (n.children[1].children || [])[0] || n.children[1];
    var v;
    try { v = asn1.read.string(inner); }
    catch (_e) { v = null; }      // allow:swallow-unverified an otherName body with no string form has no identity string, which the caller's policy then gates on
    return { type: "otherName", oid: asn1.read.oid(n.children[0]), value: v };
  }
  return null;
}
