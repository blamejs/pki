...
function _inWindow(timeMs, vf) {
  if (!vf) return true;
  if (vf.start != null && !(timeMs >= vf.start)) return false;
  if (vf.end != null && !(timeMs <= vf.end)) return false;
  return true;
}
...
function _bindEntry(te, envelope, leafDer) {
  ...
  // The entry's verifier certificate MUST be the bundle's leaf cert -- otherwise a
  // valid inclusion proof for an entry signed by a DIFFERENT Fulcio cert could be
  // paired with this envelope (the signature match alone leaves cert substitution).
...
async function verifyBundle(bundle, opts) {
  ...
  var integratedTime = _verifyRekor(tlogs[0], env, rekorKeys, leafDer);
