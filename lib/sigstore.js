...
var _HASH_BY_NUMBER = Object.freeze(Object.assign(Object.create(null), {
...
function _hashOf(algorithm) {
  ...
  if (typeof algorithm === "number" && Number.isSafeInteger(algorithm)) {
...
function _plainCopy(v, depth, budget) {
  ...
  if (Array.isArray(v)) {
    ...
    for (i = 0; i < v.length; i++) {
       ...
       * with it: deleting a content arm the walk has already listed. A hole reads as absent, which is
       * what JSON writes it as. */
      out.push(_valueOf(v, String(i), depth, budget));
    }
    return out;
  }
  out = {};
