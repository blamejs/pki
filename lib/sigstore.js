 ...
 * The two oneof rules are decided on the object AS HANDED OVER, before it is serialized. Serializing
 * drops a value JSON cannot carry, a function or a symbol, so a bundle setting a second arm to one of
 * those would arrive here as a bundle setting a single arm: a refusal sanitized into an accept. No
 * other field needs this, because dropping one removes something a check requires and can only turn
 * an accept into a refusal. */
function _snapshotBundle(bundle) {
  if (typeof bundle === "string" || Buffer.isBuffer(bundle)) return bundle;
  _assertContentOneOf(bundle);
  var vm = bundle.verificationMaterial;
  if (vm && typeof vm === "object") _assertMaterialOneOf(vm);
  var text;
