...
// Build the path [links..., leaf] from the leaf up through the BUNDLE-supplied
// intermediates only (never anchors), stopping at the issuer DN a caller anchor
// must take over. A bundle cert may serve as a path step, never as the trust root.
function _buildPath(leaf, links) {
  var linkBySubject = {};
  ...
  while (guardN++ < 16) {
    ...
    if (!link || link === cur) return { path: chain, issuerDn: issuerDn };
    chain.unshift(link);
    cur = link;
  }
...
async function _verifyChain(leaf, chainDers, fulcioRoots, timeMs) {
  ...
  var built = _buildPath(leaf, links);
  // Candidate anchors: every caller root whose subject == the top issuer DN and
  // whose validFor window contains the log time (the trusted_root carries several
  // rotations under the same DN). Try each -- path.validate does the cryptographic
  // issuer check, so the correct rotation is the one that validates.
  var candidates = roots.filter(function (r) { return _dn(r.cert, "subject") === built.issuerDn && _inWindow(timeMs, r.validFor); });
  if (!candidates.length) throw _err("sigstore/chain-incomplete", "no caller-supplied Fulcio anchor (within its validity window) issues the chain");
  var lastErr = null;
  for (var i = 0; i < candidates.length; i++) {
    var anchor = candidates[i].cert;
    var spki = anchor.subjectPublicKeyInfo;
    try {
      var res = await pathValidate.validate(built.path, {
...
function _sanValue(ext) {
  ...
  for (var i = 0; i < (seq.children || []).length; i++) {
    ...
    if (!n.constructed && GN_TYPE[n.tagNumber]) {
      ...
      if (n.tagNumber === 6) return val;   // URI (machine identity) preferred
