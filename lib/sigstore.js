...
function _armPresent(obj, name) {
  return !!obj && typeof obj === "object" &&
    Object.prototype.hasOwnProperty.call(obj, name) && obj[name] !== undefined && obj[name] !== null;
}

/** @internal The HashAlgorithm enum members a message signature may name its digest under, keyed by
 * the name protojson writes. SHA3_256 and SHA3_384 are marked deprecated in the enum and have no
 * counterpart in the Rekor hashedrekord schema, whose own hash algorithm is one of three; a member
 * this table does not carry is refused rather than mapped by guessing at its name, so a member added
 * to the enum later cannot be read as one of these. HASH_ALGORITHM_UNSPECIFIED is absent for the same
 * reason it exists: it names no algorithm. */
var _HASH_BY_ENUM = Object.freeze(Object.assign(Object.create(null), {
  SHA2_256: { rekor: "sha256", node: "sha256", bytes: 32 },
  SHA2_384: { rekor: "sha384", node: "sha384", bytes: 48 },
  SHA2_512: { rekor: "sha512", node: "sha512", bytes: 64 },
}));
...
var _HASH_BY_REKOR = Object.freeze(Object.assign(Object.create(null), {
...
var _BUNDLE_MEDIA_TYPES = Object.assign(Object.create(null), {
...
function parseBundle(input) {
  ...
  /** @internal The verification material's certificate is a oneof of the same kind. Both arms set
   * draws the leaf from one and the chain from the other, so the certificate whose key is checked and
   * the certificates it is checked through come from different statements about the same signer. */
  var vmArms = 0;
  if (_armPresent(obj.verificationMaterial, "certificate")) vmArms++;
  if (_armPresent(obj.verificationMaterial, "x509CertificateChain")) vmArms++;
  if (_armPresent(obj.verificationMaterial, "publicKey")) vmArms++;
  if (vmArms > 1) {
   ...
   * bundle carries. The two refusals are separate because a caller can act on one and not the other. */
...
/** @internal The bundle a verdict is computed from: JSON text as it came, or an object serialized
 * once so its accessors cannot answer a second time. A structure that cannot be serialized is not a
 * bundle, and is refused here rather than part-way through a check that reads it. */
function _snapshotBundle(bundle) {
  if (typeof bundle === "string" || Buffer.isBuffer(bundle)) return bundle;
  var text;
  try { text = JSON.stringify(bundle); }
  catch (e) { throw _err("sigstore/bad-bundle", "the bundle could not be read as JSON", e); }
  if (typeof text !== "string") throw _err("sigstore/bad-bundle", "bundle must be a JSON object");
  return text;
}
...
function _parseMessageSignature(ms) {
  ...
  /** @internal Read for being a string BEFORE it is used as a key. A value whose own conversion
   * throws would otherwise escape as an untyped error out of a verb whose contract is that malformed
   * input is refused with a typed one, and a bundle is caller-supplied JSON. */
  var alg = typeof md.algorithm === "string" ? _HASH_BY_ENUM[md.algorithm] : null;
  if (!alg) {
...
var _ENTRY_ROWS = Object.freeze(Object.assign(Object.create(null), {
...
function _bindDsseEntry(body, envelope, leafDer) {
  var spec = body.spec;
  if (!spec || typeof spec !== "object" || !Array.isArray(spec.signatures) || !spec.signatures.length ||
...
function _bindHashedRekordEntry(body, ms, leafDer) {
  ...
  var vDer;
  try { vDer = x509.pemDecode(_b64(pk.content, "Rekor entry verifier").toString("utf8"), "CERTIFICATE"); }
  catch (e) { throw _err("sigstore/bad-tlog-entry", "the Rekor entry verifier is not a valid certificate", e); }
  if (!vDer.equals(leafDer)) {
...
function _verifyRekorEntry(te, rekorKeys, leafDer, contentType, bind) {
  ...
  return {
    integratedTime: t, logIndex: Number.isFinite(li) ? li : null,
    logId: te.logId && te.logId.keyId ? _b64(te.logId.keyId, "logId.keyId").toString("hex") : null,
...
function _pathToAnchor(leaf, anchor, intBySubject) {
  ...
  var above = [], cur = leaf, seen = Object.create(null), guardN = 0;
  function assemble() {
    var path = [];
...
function _normCtLogs(ctLogs) {
  if (!Array.isArray(ctLogs)) throw _err("sigstore/bad-input", "ctLogs must be an array of { keyId, spki, validFor? }");
  ...
  return ctLogs.map(function (l, i) {
    if (l === null || typeof l !== "object") throw _err("sigstore/bad-input", "ctLogs[" + i + "] must be { keyId, spki, validFor? }");
    if (!Buffer.isBuffer(l.keyId)) throw _err("sigstore/bad-input", "ctLogs[" + i + "].keyId must be the log id as a Buffer");
    if (!Buffer.isBuffer(l.spki)) throw _err("sigstore/bad-input", "ctLogs[" + i + "].spki must be the log's SubjectPublicKeyInfo DER");
...
function _sctMs(sct) { return Number(sct.timestamp || 0); }

function _ctLogFor(logs, logIdBytes, timeMs) {
  var want = Buffer.isBuffer(logIdBytes) ? logIdBytes : Buffer.from(logIdBytes || []);
  for (var i = 0; i < logs.length; i++) {
    if (logs[i].keyId.equals(want) && _inWindow(timeMs, logs[i].validFor)) return logs[i];
  }
  return null;
}
...
async function _verifyChain(leaf, chainDers, fulcioRoots, timeMs) {
  ...
  var intBySubject = Object.create(null);
...
function _identity(leaf) {
  var out = { san: null, extensions: Object.create(null) };
  try {
...
var GN_TYPE = Object.assign(Object.create(null), { 1: "rfc822Name", 2: "dNSName", 6: "uri" });
...
var IDENTITY_KEYS = Object.assign(Object.create(null), { san: 1, issuer: 1, sourceRepositoryURI: 1 });
var _VERIFY_BUNDLE_OPTS = Object.assign(Object.create(null), { fulcioRoots: 1, rekorKeys: 1, ctLogs: 1, identity: 1, predicateType: 1, time: 1, artifact: 1 });
...
async function verifyBundle(bundle, opts) {
  ...
  /** @internal A bundle handed over as an object is taken as a snapshot before anything reads it.
   * Its fields are the caller's properties, and an accessor may answer differently each time it is
   * read, so a value can be checked in one place and a different one used in another: a signature
   * bound to a log entry while a different entry is proven, a proof folded to one root while another
   * is compared against the signed checkpoint. Serializing once and reading the result back means
   * every check below runs on plain data that cannot change under it, whichever field it reads and
   * however many times. A bundle given as JSON text is already fixed and is passed through as it
   * came. This also brings the reader's size and depth caps to an object input. */
  var b = parseBundle(_snapshotBundle(bundle));
   ...
   * there is none of. */
  if (!isMsg && _armPresent(opts, "artifact")) {
  ...
  if (isMsg && _armPresent(opts, "predicateType")) {
  ...
  if (isMsg) {
    ...
    artifact = _artifactBytes(opts.artifact);
  }
  ...
  for (var ti = 0; ti < tlogs.length; ti++) {
    ...
    try {
      rekor = isMsg ? _verifyRekorMessage(tlogs[ti], msParsed, rekorKeys, leafDer)
   ...
   * was signed. The artifact is then held to THAT hash rather than to the digest the bundle carries,
   * which no signature covers. Only after the artifact is the one the log recorded is the signature
   * checked over its bytes. */
  var artifactDigest = null, digestAlgorithm = null, messageDigestChecked = false;
  if (isMsg) {
    digestAlgorithm = rekor.artifactHash.algorithm;
    var computed = nodeCrypto.createHash(rekor.artifactHash.node).update(artifact).digest();
    artifactDigest = computed.toString("hex");
    if (!guard.crypto.constantTimeEqual(computed, Buffer.from(rekor.artifactHash.value, "hex"))) {
      throw _err("sigstore/artifact-mismatch", "the artifact does not match the one the transparency-log entry records; its " +
        digestAlgorithm + " digest is " + artifactDigest + " and the entry names " + rekor.artifactHash.value);
    }
  ...
  var chain = await _verifyChain(leaf, _chainDers(vm), fulcioRoots, checkTime);
  /** @internal With no caller instant the reference is the authenticated log-entry time, whose whole
   * second is inclusive: a certificate logged in the same second as its entry is not dated after it,
   * and nothing beyond that second is accepted. */
  var sctBound = guard.time.isDate(opts.time) ? checkTime : C.TIME.seconds(integratedTime) + 999;
  var sctResult = await _verifyEmbeddedScts(leaf, leafDer, chain.issuer, opts.ctLogs, checkTime, sctBound);
  ...
  var st = isMsg ? null : _statement(payload, env.payloadType, opts.predicateType);
