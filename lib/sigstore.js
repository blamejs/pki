...
/** @internal The operations the bundle copy decides with, taken at load. That copy is what every
 * later check runs on, so which function answers "what does this object own" and "what does it hold"
 * settles what is verified; one replaced after this module loads would otherwise be asked at the
 * moment of the call. An emptied key list alone turns a bundle into one carrying no content arm. */
var _ownNames = Object.getOwnPropertyNames;
var _ownDescriptor = Object.getOwnPropertyDescriptor;
...
function _numOf(v) {
  ...
  if (t === "string" || t === "boolean") return Number(v);
  return NaN;
}
...
function _snapshotBundle(bundle) {
  ...
  try {
    return _plainCopy(bundle, 0, { n: 0 });
  } catch (e) {
