...
async function verifyBundle(bundle, opts) {
  ...
  var rekor = null, lastErr = null, artifactComputed = null;
  for (var ti = 0; ti < tlogs.length; ti++) {
    ...
    try {
      ...
      /** @internal Which entry to take is decided by what it records, not only by whether it binds.
       * A log can hold several entries for one signature and certificate, so an entry that binds and
       * names a different artifact is passed over here rather than taken and then failed after the
       * loop, which refused a bundle whose later entry recorded the artifact the caller supplied.
       * Passing over is not accepting: an entry is selected only once this comparison has held on
       * it, so every path below runs on an entry that records this artifact. */
      var candidateDigest = null;
      if (isMsg) {
        candidateDigest = nodeCrypto.createHash(candidate.artifactHash.node).update(artifact).digest();
        if (!guard.crypto.constantTimeEqual(candidateDigest, Buffer.from(candidate.artifactHash.value, "hex"))) {
          throw _err("sigstore/artifact-mismatch", "the artifact does not match the one the transparency-log entry records; its " +
            candidate.artifactHash.algorithm + " digest is " + candidateDigest.toString("hex") + " and the entry names " + candidate.artifactHash.value);
        }
      }
      rekor = candidate;
  ...
  var checkTime = haveCallerTime ? guard.time.instantOf(o.time) : C.TIME.seconds(integratedTime);
  ...
  var sctBound = haveCallerTime ? checkTime : C.TIME.seconds(integratedTime) + 999;
  var sctResult = await _verifyEmbeddedScts(leaf, leafDer, chain.issuer, o.ctLogs, checkTime, sctBound);
