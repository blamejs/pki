...
function _bindEntry(te, envelope) {
  ...
  if (body.kind !== "dsse" || !spec || typeof spec !== "object" || !Array.isArray(spec.signatures) || !spec.signatures.length ||
    ...
    throw _err("sigstore/unsupported-content", "unsupported or malformed Rekor entry: " + (body && body.kind));
  }
...
function _verifyRekor(te, envelope, rekorKeys) {
  ...
  // signs the root, the SET signs the entry. At least one MUST verify.
  ...
  // integratedTime is the trusted check-date for the Fulcio chain: a non-finite
  // value is a malformed entry, not a silent NaN that leaks out of the time math.
