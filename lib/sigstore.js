...
function parseBundle(input) {
  ...
  if (typeof mt !== "string" || !/^application\/vnd\.dev\.sigstore\.bundle(\.v0\.\d+)?\+json(;version=0\.\d+)?$/.test(mt)) {
    throw _err("sigstore/bad-bundle-version", "unknown bundle media type: " + (mt === undefined ? "(none)" : mt));
  }
...
function _rekorKey(rekorKeys, keyIdBuf, hint) {
  for (var i = 0; i < rekorKeys.length; i++) {
    var k = rekorKeys[i];
    if (keyIdBuf && k.keyId && k.keyId.equals(keyIdBuf)) return k;
    if (hint && k.keyId && k.keyId.length >= 4 && k.keyId.subarray(0, 4).equals(hint)) return k;
  }
  return null;
}
...
function _verifyCheckpoint(envelope, ip, rekorKeys) {
  ...
  for (var i = 0; i < sigBlock.length; i++) {
    ...
    var k = _rekorKey(rekorKeys, null, hint);
    if (!k) continue;
...
function _verifySet(te, rekorKeys) {
  ...
  var k = _rekorKey(rekorKeys, keyId, null);
  if (!k) return false;
...
function _bindEntry(te, envelope) {
  ...
  if (spec.payloadHash && spec.payloadHash.value !== _sha256(payload).toString("hex")) {
    throw _err("sigstore/entry-mismatch", "the Rekor entry payloadHash does not match the bundle payload");
  }
}

function _verifyRekor(te, envelope, rekorKeys) {
  ...
  if (!included) throw _err("sigstore/inclusion-proof-mismatch", "the inclusion proof does not reconstruct the signed tree root");
  // The reconstructed root is only trust once a Rekor key signs it: the checkpoint
  // signs the root, the SET signs the entry (+ its integratedTime). At least one
  // MUST verify for the inclusion to be attested.
  var checkpointOk = ip.checkpoint && typeof ip.checkpoint.envelope === "string" && _verifyCheckpoint(ip.checkpoint.envelope, ip, rekorKeys);
  var setOk = _verifySet(te, rekorKeys);
  if (!checkpointOk && !setOk) throw _err("sigstore/unsigned-root", "neither the checkpoint nor the SET verifies under the caller Rekor key -- the tree root is not attested");
  // The integratedTime dates the ephemeral (~10-min) Fulcio cert, so it must be
  // Rekor-attested: ONLY the SET signs it (the checkpoint signs the root, not the
  // time). Without a verified SET the time is attacker-controlled and could date
  // an expired cert into validity -- require it (the RFC 3161 timestamp source is
...
function _buildChain(leaf, anchors, links) {
  // Walk issuer<-subject from the leaf up to a CALLER-supplied anchor. `anchors`
  // (opts.fulcioRoots) are the ONLY certs that may terminate the chain -- a
  // bundle-supplied cert (`links`) may serve only as an intermediate path step,
  // never as the trust root, or an attacker ships a self-signed chain and
  // verifies against their own root. Returns { anchor, path: [intermediates..., leaf] }.
  var anchorBySubject = {};
  anchors.forEach(function (c) { anchorBySubject[_dn(c, "subject")] = c; });
  var linkBySubject = {};
  links.forEach(function (c) { linkBySubject[_dn(c, "subject")] = c; });
  var chain = [leaf], cur = leaf, guardN = 0;
  while (guardN++ < 16) {
    var issuerDn = _dn(cur, "issuer");
    // A caller anchor issued cur -> it is the trust root; stop (path.validate
    // does the cryptographic issuer check against this anchor's key).
    if (anchorBySubject[issuerDn]) return { anchor: anchorBySubject[issuerDn], path: chain };
    var link = linkBySubject[issuerDn];
    if (!link || link === cur) throw _err("sigstore/chain-incomplete", "the Fulcio chain does not terminate at a caller-supplied trust anchor");
    chain.unshift(link);
    cur = link;
  }
  throw _err("sigstore/chain-incomplete", "the Fulcio certificate chain is longer than the maximum depth");
}

async function _verifyChain(leafDer, chainDers, fulcioRootDers, timeMs) {
  var leaf = x509.parse(leafDer);
  var anchors = fulcioRootDers.map(function (d) { return x509.parse(d); });
  // Bundle-inline intermediates are path links ONLY -- never trust anchors.
  var links = chainDers.slice(1).map(function (d) { return x509.parse(d); });
  var built = _buildChain(leaf, anchors, links);
  var anchor = built.anchor;
  var spki = anchor.subjectPublicKeyInfo;
  var res;
  try {
    res = await pathValidate.validate(built.path, {
      time: new Date(timeMs),
      historicalMode: true,
      trustAnchor: { name: anchor.subject, publicKey: spki.bytes, algorithm: spki.algorithm.algorithm, parameters: spki.algorithm.parameters },
      requiredEku: ["codeSigning"],
    });
  } catch (e) {
    throw _err("sigstore/chain-invalid", "the Fulcio certificate chain failed validation: " + e.message, e);
  }
  if (!res.valid) throw _err("sigstore/chain-invalid", "the Fulcio certificate chain is not valid as of the log time");
  return leaf;
}
...
function _sanValue(ext) {
  ...
  for (var i = 0; i < (seq.children || []).length; i++) {
    ...
    if (n.tagClass !== "context" || n.constructed) continue;
    var val = { type: GN_TYPE[n.tagNumber] || ("tag" + n.tagNumber), value: asn1.read.octetStringImplicit(n, n.tagNumber).toString("utf8") };
    if (n.tagNumber === 6) return val;
    if (!first) first = val;
  }
  return first;
}
...
function _checkIdentity(id, policy) {
  ...
  if (policy.issuer && id.extensions.issuer !== policy.issuer) throw _err("sigstore/identity-mismatch", "the certificate OIDC issuer does not match the expected issuer");
...
function _statement(payload, payloadType) {
  ...
  if (!st || st._type !== "https://in-toto.io/Statement/v1") throw _err("sigstore/bad-statement", "the payload is not an in-toto Statement v1");
 ...
 *
 * @opts
 *   fulcioRoots:  Buffer[],   // the Fulcio CA certificates (DER), the trust anchor
 *   rekorKeys:    Array,      // [{ keyId: Buffer, spki: Buffer }] the Rekor log public keys
...
async function verifyBundle(bundle, opts) {
  ...
  var leafKey = _pubFromSpki(x509.parse(leafDer).subjectPublicKeyInfo.bytes, "Fulcio leaf");
  ...
  // Leg 2 -- Rekor inclusion + Rekor-signed root + entry binding (yields the trusted time).
  ...
  if (!tlogs[0] || typeof tlogs[0] !== "object") throw _err("sigstore/bad-bundle", "the transparency-log entry is not an object");
  var integratedTime = _verifyRekor(tlogs[0], env, rekorKeys);
  ...
  var leaf = await _verifyChain(leafDer, _chainDers(vm), fulcioRoots, checkTime);
  var identity = _identity(leaf);
  _checkIdentity(identity, opts.identity);

  // Leg 4 -- the in-toto SLSA statement + subject binding.
  var st = _statement(payload, env.payloadType);
