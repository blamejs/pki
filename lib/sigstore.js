...
async function _verifyChain(leaf, chainDers, fulcioRootDers, timeMs) {
  var anchors = fulcioRootDers.map(function (d, i) { return _parseCert(d, "Fulcio CA root [" + i + "]"); });
  // Bundle-inline intermediates are path links ONLY -- never trust anchors.
  var links = chainDers.slice(1).map(function (d, i) { return _parseCert(d, "bundle intermediate [" + i + "]"); });
