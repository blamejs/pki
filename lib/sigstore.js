...
function parseBundle(input) {
  ...
  if (Buffer.isBuffer(input) || typeof input === "string") {
    ...
    obj = _jsonParse(_snapshotBundle(input), "sigstore/bad-bundle", "bundle");
  } else {
...
function _bindVerifier(content, leafDer) {
  var text = _b64(content, "Rekor entry verifier").toString("utf8");
  var vDer;
  ...
  if (!spki.equals(Buffer.from(leafSpki))) {
...
async function verifyBundle(bundle, opts) {
   ...
   * there is none of. */
  /** @internal Read the way the option gate reads an option, and the way the verdict reports one:
   * whatever `opts.<name>` resolves to, inherited or own. The presence test beside this one is about
   * a protobuf oneof arm and asks for an OWN property, which is right for a bundle and wrong here.
   * Using it made an option supplied on the options object's prototype look absent, so an inherited
   * predicateType passed this refusal and was then counted as checked by a verdict that had no
   * statement to check it against. */
  if (!isMsg && opts.artifact != null) {
  ...
  if (isMsg && opts.predicateType != null) {
