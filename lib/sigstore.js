 ...
 * the name protojson writes. All five the enum defines are here, the two SHA-3 members included: they
...
var JSON_MAX_DEPTH = C.LIMITS.JSON_MAX_DEPTH;
...
function _plainCopy(v, depth, budget) {
   ...
   * of values holding tens of megabytes. Charging one byte per value plus the length of every string
   * and key counts no more than the serialized form does, so nothing the size limit would have
   * admitted is refused here. */
  _charge(budget, 1);
  if (depth > JSON_MAX_DEPTH) {
    throw _err("sigstore/bad-bundle", "the bundle nests deeper than " + JSON_MAX_DEPTH + " levels");
  }
  if (v === null) return null;
  var t = typeof v;
  if (t === "string") { _charge(budget, v.length); return v; }
  if (t === "boolean") return v;
  if (t === "number") {
    if (!isFinite(v)) throw _err("sigstore/bad-bundle", "the bundle carries a number JSON cannot represent");
    return v;
  }
  if (t !== "object") {
  ...
  if (Array.isArray(v)) {
    out = [];
    for (i = 0; i < v.length; i++) out.push(_plainCopy(v[i], depth + 1, budget));
    return out;
  }
  out = {};
  var keys = Object.keys(v);
  for (i = 0; i < keys.length; i++) {
    /** @internal Charged by the name it was enumerated under, BEFORE the value is read. A property
     * whose value is skipped still costs the walk a step, so charging after the read would leave a
     * structure made of skipped properties bounded by nothing. */
    _charge(budget, keys[i].length);
    /** @internal The one read of this property. Nothing below reads it again. */
    var val = v[keys[i]];
    if (val === undefined) continue;
    out[keys[i]] = _plainCopy(val, depth + 1, budget);
  }
  return out;
}
...
function _snapshotBundle(bundle) {
  ...
  /** @internal Reading a caller's object runs the caller's own accessors, and one that throws must
   * arrive as this module's refusal rather than as whatever it threw: a bundle this cannot read is
   * refused with a typed error like any other it cannot read. This module's own refusals pass
   * through unchanged, so a size or depth verdict keeps its own message. */
  try {
    return JSON.stringify(_plainCopy(bundle, 0, { n: 0 }));
  } catch (e) {
