...
var NOTE_SIG = new RegExp("^" + _fromCharCode(0x2014) + " (\\S+) (\\S+)$");
...
function pae(payloadType, payloadBytes) {
  ...
  return _bufferConcat([
    DSSEV1, SP,
    _bufferFrom(_String(type.length), "ascii"), SP, type, SP,
    _bufferFrom(_String(body.length), "ascii"), SP, body,
  ]);
}
...
function _verifySet(te, rekorKeys, timeMs) {
  ...
  if (!k) return false;
  var canonical = _stringify({
    body: te.canonicalizedBody,
    integratedTime: _Number(te.integratedTime),
    logID: keyId.toString("hex"),
    logIndex: _Number(te.logIndex),
  });
...
function _bindEntry(te, envelope, leafDer) {
  ...
  var spec = body.spec;
  if (body.kind !== "dsse" || !spec || typeof spec !== "object" || !_isArray(spec.signatures) || !spec.signatures.length ||
...
function _verifyRekor(te, envelope, rekorKeys, leafDer) {
  ...
  var claimedSec = _Number(te.integratedTime);
  ...
  try {
    included = merkle.verifyInclusion({
      leafIndex: _BigInt(ip.logIndex),
      treeSize: _BigInt(ip.treeSize),
  ...
  var t = _Number(te.integratedTime);
  ...
  try {
    ...
    (leaf.extensions || []).forEach(function (ext) {
      ...
      if (ext.oid.indexOf(FULCIO_PREFIX) === 0) {
        out.extensions[ext.name || ext.oid] = _fulcioExtValue(ext, _Number(ext.oid.slice(FULCIO_PREFIX.length)));
      }
    });
  } catch (e) {
...
function _sanValue(ext) {
  ...
  for (var i = 0; i < (seq.children || []).length; i++) {
    ...
    if (!n.constructed && GN_TYPE[n.tagNumber]) {
      ...
      try { v = asn1.read.string(inner); } catch (_e) { v = _bufferFrom(inner.content || []).toString("utf8"); }
      val = { type: "otherName", oid: asn1.read.oid(n.children[0]), value: v };
    }
    if (val) _push(names, val);
  }
...
function _checkIdentity(id, policy) {
  ...
  if (policy.san && sanValue !== policy.san) throw _err("sigstore/identity-mismatch", "the certificate SAN " + _stringify(sanValue) + " does not match the expected identity");
...
function _statement(payload, payloadType, expectedPredicate) {
  ...
  if (expectedPredicate && st.predicateType !== expectedPredicate) {
    throw _err("sigstore/predicate-mismatch", "the statement predicateType " + _stringify(st.predicateType) + " does not match the expected " + _stringify(expectedPredicate));
  }
  return st;
}
