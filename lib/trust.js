 ...
 *   nameConstraints: { permitted?: [{ tag, base }], excluded?: [{ tag, base }] }   // the root program's applied namespace
 *
 * @example
...
function _assertSubtreeBaseForm(tag, base, where, index) {
  ...
  if (tag === 1 || tag === 2 || tag === 6) {
    if (typeof base !== "string") throw E("trust/bad-input", who + " base must be a string for tag " + tag);
    return;
  }
  if (tag === 7) {
...
function anchor(entry, opts) {
   ...
   * verb returned before the option existed. */
  if (nameConstraints) out.nameConstraints = nameConstraints;
  return out;
}
