...
// The operations this module decides with, bound at load. A store entry's pairing check is a byte
// comparison, its dedup key is a byte-to-text step, and its trust-bit lookup is an own-membership
// test. Each is an ordinary writable property of a global, so reading one at the moment it runs
// hands the trust verdict to whoever last wrote it.
...
// allow:guard-reads-runtime-live -- module-init, the table is built before any caller runs
...
function _readOctalBody(lines, idx) {
  ...
  for (var i = idx; i < lines.length; i++) {
    var t = lines[i].trim();
    if (t === "END") return { bytes: _bufferFrom(out), next: i + 1 };
    if (t === "") throw E("trust/bad-octal", "MULTILINE_OCTAL body interrupted by a blank line before END");
    var chunks = _strSplit(t, /\s+/);
    for (var c = 0; c < chunks.length; c++) {
      var chunk = chunks[c];
      for (var k = 0; k < chunk.length; k += 4) {
        if (_charCodeAt(chunk, k) !== 92 /* backslash */) {
          throw E("trust/bad-octal", "MULTILINE_OCTAL expects \\ooo escapes, got " + _stringify(_strSlice(chunk, k, k + 4)));
        }
        var d = _strSlice(chunk, k + 1, k + 4);
        if (!/^[0-7]{3}$/.test(d)) {
          throw E("trust/bad-octal", "a MULTILINE_OCTAL escape is a backslash + exactly three octal digits, got " + _stringify("\\" + d));
        }
        if (_charCodeAt(d, 0) > 51 /* '3' */) {
...
function _lexObjects(text) {
  ...
  while (i < n) {
    var t = lines[i].trim();
    if (t === "") {
      if (cur) { _push(objects, cur); cur = null; }
      i++; continue;
    }
    if (_charAt(t, 0) === "#") { i++; continue; }
    var m = _reExec(ATTR_RE, t);
    if (!m) throw E("trust/bad-block", "unexpected line in the certdata object stream: " + _stringify(_strSlice(t, 0, 64)));
    ...
    if (type === "CK_BBOOL") {
      ...
      if (!CKT_RECOGNIZED[rawVal]) throw E("trust/bad-trust-value", "unrecognized CK_TRUST value " + _stringify(rawVal) + " on " + name);
      ...
      throw E("trust/bad-block", "unrecognized attribute type " + _stringify(type) + " on " + name);
    }
    cur.attrs[name] = entry;
    i++;
  }
  if (cur) _push(objects, cur);
  return objects;
}
...
function _strictTime(payload, code, label) {
  var tlv;
  if (payload.length === 13) tlv = _bufferConcat([_bufferFrom([0x17, 0x0d]), payload]);
  else if (payload.length === 15) tlv = _bufferConcat([_bufferFrom([0x18, 0x0f]), payload]);
...
function _certEntry(obj) {
  ...
  if (!_equals(issuer, cert.issuer.bytes)) {
  ...
  try {
    ...
    serialContentHex = _bufToString(sn.content, "hex");
  } catch (e) {
  ...
  if (subject && (subject.type !== "MULTILINE_OCTAL" || !_equals(subject.bytes, cert.subject.bytes))) {
  ...
  return {
    key: _bufToString(issuer, "hex") + "/" + _bufToString(serial, "hex"),
    der: der,
    cert: cert,
...
function _trustEntry(obj) {
  ...
  _forEach(PURPOSE_ATTRS, function (pair) {
  ...
  return { key: _bufToString(issuer, "hex") + "/" + _bufToString(serial, "hex"), purposes: purposes };
}
...
function _mkAnchor(cert, meta) {
  ...
  _DERIVED_FROM.set(entry, {
    ...
    publicKey: _bufferFrom(spki.bytes),
    algorithm: spki.algorithm.oid,
    parameters: spki.algorithm.parameters == null ? spki.algorithm.parameters : _bufferFrom(spki.algorithm.parameters),
...
function _copyDistrustAfter(src) {
  ...
  _forEach(_keys(src), function (k) {
    out[k] = guard.time.isDate(src[k]) ? new _Date(guard.time.instantOf(src[k])) : src[k];
  });
  return out;
}
...
function _copyName(name) {
  if (!name || !_isArray(name.rdns)) return name;
  var out = _assign({}, name);
  out.rdns = _map(name.rdns, function (rdn) {
    return _isArray(rdn) ? _map(rdn, function (atv) { return _assign({}, atv); }) : rdn;
  });
  out.bytes = _isBuffer(name.bytes) ? _bufferFrom(name.bytes) : name.bytes;
  return out;
}

function _datesEqual(x, y) {
  var kx = _sort(_keys(x)), ky = _sort(_keys(y));
  if (_join(kx, ",") !== _join(ky, ",")) return false;
  return _every(kx, function (k) { return guard.time.instantOf(x[k]) === guard.time.instantOf(y[k]); });
}

function _purposesEqual(x, y) {
  return _every(PURPOSES, function (p) { return x[p] === y[p]; });
}

function _paramsEqual(x, y) {
  if (x === null || y === null) return x === y;
  return _isBuffer(x) && _isBuffer(y) && _equals(x, y);
}
...
function _dedupAnchors(anchors) {
  ...
  var out = [];
  _forEach(anchors, function (a) {
    var key = _bufToString(a.subjectDer, "hex") + "/" + _bufToString(a.publicKey, "hex");
    var prev = seen[key];
    if (!prev) { seen[key] = a; _push(out, a); return; }
...
function parseCertdata(text) {
  ...
  _forEach(objects, function (obj) {
    ...
    if (cls.value === "CKO_CERTIFICATE") {
      ...
      if (prev) {
        ...
        if (!_equals(prev.der, e.der) || prev.label !== e.label ||
  ...
  return {
    anchors: _dedupAnchors(_map(certs, function (c) {
...
function _csvRows(text) {
  ...
  function endField() { _push(row, field); field = ""; quoted = false; closed = false; }
  function endRow() {
    endField();
  ...
  while (i < n) {
    var ch = _charAt(text, i);
    if (inQuotes) {
      if (ch === "\"") {
        if (_charAt(text, i + 1) === "\"") { appendChar("\""); i += 2; continue; }
...
function _trustBits(cell) {
  var purposes = { serverAuth: false, emailProtection: false, codeSigning: false };
  _forEach(_strSplit(_String(cell), ";"), function (tok) {
    var t = _trim(tok);
    if (t === "") return;
    var l = _toLowerCase(t);
    if (!_hasOwn(_TRUST_BIT_TOKENS, l)) {
      throw E("trust/bad-csv", "unrecognized Trust Bits token " + _stringify(t));
    }
...
function _csvDate(cell, label) {
  var s = _trim(_String(cell));
  if (s === "") return null;
  var m = /^(\d{4})[./-](\d{1,2})[./-](\d{1,2})$/.exec(s);
  if (!m) throw E("trust/bad-csv", label + " must be a Y-M-D date, got " + _stringify(s));
  var pad = function (x) { return x.length === 1 ? "0" + x : x; };
  var payload = _bufferFrom(m[1] + pad(m[2]) + pad(m[3]) + "235959Z", "latin1");
  try { return _strictTime(payload, "trust/bad-csv", label); }
  catch (e) {
    if (e && e.code === "trust/bad-csv") throw e;
    throw E("trust/bad-csv", label + " is not a real calendar date: " + _stringify(s), e);
  }
}
...
function _pemCell(cell) {
  var s = _trim(_String(cell));
  if (_charAt(s, 0) === "'") s = _strSlice(s, 1);
  if (_charAt(s, s.length - 1) === "'") s = _strSlice(s, 0, -1);
  s = _trim(s);
...
function parseCcadbCsv(text) {
  ...
  var header = rows[0].map(function (h) { return _trim(h); });
  var col = _create(null);
  _forEach(header, function (h, idx) {
    if (col[h] !== undefined) {
      ...
      if (_indexOf(CSV_REQUIRED, h) !== -1) throw E("trust/bad-csv", "duplicate required column " + _stringify(h));
      return;
    }
    col[h] = idx;
  });
  _forEach(CSV_REQUIRED, function (r) {
    if (col[r] === undefined) throw E("trust/bad-csv", "CSV is missing the required column " + _stringify(r));
  });
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    _push(anchors, _mkAnchor(_pemCell(row[col["PEM Info"]]), {
      distrustAfter: distrustAfter,
...
function anchor(entry, opts) {
  ...
  if (!derived && (!_isBuffer(entry.publicKey) || typeof entry.algorithm !== "string" ||
      !entry.name || !_isArray(entry.name.rdns))) {
    throw E("trust/bad-input", "anchor expects a trust-store entry ({ name, publicKey, algorithm, ... })");
  }
  if (opts.purpose !== undefined) {
    if (_indexOf(PURPOSES, opts.purpose) === -1) {
      throw E("trust/bad-input", "anchor: opts.purpose must be one of " + _join(PURPOSES, " | "));
    }
  ...
  if (!derived && (entry.purposes != null || (entry.distrustAfter && _keys(entry.distrustAfter).length))) {
  ...
  return {
    ...
    publicKey: _isBuffer(meta.publicKey) ? _bufferFrom(meta.publicKey) : meta.publicKey,
    algorithm: meta.algorithm,
    parameters: _isBuffer(meta.parameters) ? _bufferFrom(meta.parameters)
