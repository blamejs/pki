...
// replaceable properties to answer one thing, and here it decides which trust bits an anchor carries.
...
// ---- certdata scanners by character (no regex) ----------------------------------------------------
function _isNameChar(c) { return (c >= 65 && c <= 90) || (c >= 48 && c <= 57) || c === 95; }   // A-Z 0-9 _
function _isDigitCode(c) { return c >= 48 && c <= 57; }
function _isDateSepCode(c) { return c === 46 || c === 47 || c === 45; }   // . / -
function _isOctalDigit(c) { return c >= 48 && c <= 55; }   // 0-7
function _isWsCode(c) { return pkix.isJsWhitespace(c); }
function _containsWs(s) { for (var i = 0; i < s.length; i++) { if (_isWsCode(_charCodeAt(s, i))) return true; } return false; }   // /\s/.test(s)

// An attribute line `CKA_<NAME> [<type> [<value>]]`, matching
// /^(CKA_[A-Z0-9_]+)(?:\s+(\S+)(?:\s+(.*))?)?$/. Returns [name, type|undefined, value|undefined] or null.
...
// A single double-quoted value with no interior quote (matching /^"([^"]*)"$/): the inner content, or
// null. Empty content ("\"\"") returns "" (a valid value), distinct from the null no-match.
function _unquote(s) {
  var n = s.length;
  if (n < 2 || _charCodeAt(s, 0) !== 34 || _charCodeAt(s, n - 1) !== 34) return null;   // surrounding "
  for (var i = 1; i < n - 1; i++) if (_charCodeAt(s, i) === 34) return null;             // no interior "
  return _strSlice(s, 1, n - 1);
}

// String.prototype.split(/\s+/): the pieces between whitespace runs (a leading/trailing run yields a
// "" element; "" -> [""]). Index assignment (not .push) so the scanner adds no live intrinsic read.
...
// Exactly three octal digits (matching /^[0-7]{3}$/).
...
// String.prototype.split(/\r\n|\n|\r/): split on CRLF, LF, or CR line terminators.
function _splitLines(text) {
  var out = [], start = 0, i = 0, n = text.length;
  while (i < n) {
    var c = _charCodeAt(text, i);
    if (c === 13) {                                            // CR (consume a following LF as one)
...
// A Y-M-D date with dotted/dashed/slashed separators, matching /^(\d{4})[./-](\d{1,2})[./-](\d{1,2})$/.
// Returns [year, month, day] (raw digit strings) or null.
...
function _csvDate(cell, label) {
  ...
  // _strictTime tags every rejection (a rolled-over month/day among them) with the code passed here,
  // so the re-throw carries it unchanged; the wrap normalizes any other failure to the same typed code
  // so a bad row is always trust/bad-csv, never an untyped exception escaping parseCcadbCsv.
 ...
 */
// `purpose` is the only option, and it gates the authorization itself, so a near-miss spelling must not
// pass silently: a caller who wrote the plural `purposes` (or carried a sibling's word) would otherwise get
// the anchor with NO purpose restriction rather than an error naming the option they meant.
