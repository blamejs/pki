...
function _mkAnchor(cert, meta) {
  ...
  _DERIVED_FROM.set(entry, {
    ...
    name: {
      rdns: cert.subject.rdns.map(function (rdn) {
        return rdn.map(function (atv) { return { type: atv.type, value: atv.value }; });
      }),
      dn: cert.subject.dn,
      bytes: Buffer.from(cert.subject.bytes),
    },
...
function anchor(entry, opts) {
  ...
  return {
    name: derived ? derived.name : entry.name,
    publicKey: derived ? derived.publicKey : entry.publicKey,
    algorithm: derived ? derived.algorithm : entry.algorithm,
    parameters: derived ? (derived.parameters !== undefined ? derived.parameters : null)
      : (entry.parameters !== undefined ? entry.parameters : null),
