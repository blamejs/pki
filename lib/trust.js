...
function _mkAnchor(cert, meta) {
  ...
  _DERIVED_FROM.set(entry, {
    ...
    // every { type, value } pair, so editing one in place would still reach the record.
...
// A parsed Name, copied to the depth the path validator reads: the RDN sequence, each RDN's
// attribute pairs, and the raw DER. Used on both sides of the record -- writing it in and handing
// it out -- so neither direction shares a mutable object with the caller.
function _copyName(name) {
  if (!name || !Array.isArray(name.rdns)) return name;
  return {
    rdns: name.rdns.map(function (rdn) {
      return Array.isArray(rdn) ? rdn.map(function (atv) { return { type: atv.type, value: atv.value }; }) : rdn;
    }),
    dn: name.dn,
    bytes: Buffer.isBuffer(name.bytes) ? Buffer.from(name.bytes) : name.bytes,
  };
}
...
function anchor(entry, opts) {
  ...
  var src = derived || entry;
  return {
    name: _copyName(src.name),
    publicKey: Buffer.isBuffer(src.publicKey) ? Buffer.from(src.publicKey) : src.publicKey,
    algorithm: src.algorithm,
    parameters: Buffer.isBuffer(src.parameters) ? Buffer.from(src.parameters)
      : (src.parameters !== undefined ? src.parameters : null),
    // The METADATA is copied too. It is not decoration -- `purposes` is what the purpose gate above
    // reads, so handing back the entry's own object let a consumer write
    ...
    distrustAfter: _copyDistrustAfter(entry.distrustAfter),
    purposes: entry.purposes
      ? { serverAuth: entry.purposes.serverAuth === true, emailProtection: entry.purposes.emailProtection === true, codeSigning: entry.purposes.codeSigning === true }
      : { serverAuth: false, emailProtection: false, codeSigning: false },
  };
}
