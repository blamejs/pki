...
function buildCrlStatusList(spec) {
  ...
  var status = [source];
  if (spec.thisUpdate != null) _push(status, b.raw(_b.timeDer(spec.thisUpdate, "crlUpdate.thisUpdate")));
  return { der: b.sequence([b.sequence(status)]), issuerName: issuerNameDer };
}
