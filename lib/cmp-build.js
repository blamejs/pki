...
var _objectProto = _getPrototypeOf({});   // the realm's Object.prototype, taken at load
...
function _encodeCrlEntryDetails(spec) {
  ...
  // does not catch these (an empty array or a keyless exotic has no unknown key to reject), so the shape
  // is refused here by requiring a plain-object prototype rather than a bare typeof-object test.
  var proto = typeof spec === "object" ? _getPrototypeOf(spec) : false;
  if (proto !== _objectProto && proto !== null) throw _err("cmp/bad-rev-req", "RevDetails.crlEntryDetails must be { reason } or a pre-encoded Extensions DER");
