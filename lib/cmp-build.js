...
// arms whose content is a CertReqMessages via pki.crmf.build (RFC 9810 sec. 5.3.7 krr, sec. 5.3.11 ccr).
var CRMF_BODY = { ir: 1, cr: 1, kur: 1, krr: 1, ccr: 1 };
...
function _encodeBody(bodySpec, key, opts) {
  ...
  if (CRMF_BODY[arm]) {
    ...
    // A ccr carries exactly one CertReqMsg (RFC 9810 Appendix D.6, the mirror of the one-CertResponse ccp
    // rule): multiple cross-certificates go in separate messages. Refused up front, as the ccp builder
    // refuses a multi-response ccp, rather than only at the round-trip self-verify.
    if (arm === "ccr" && reqSpec && typeof reqSpec === "object" && intrinsic.isArray(reqSpec.messages) && reqSpec.messages.length > 1) {
      throw _err("cmp/bad-input", "a cross-certification request (ccr) carries exactly one CertReqMsg; multiple cross-certificates go in separate messages (RFC 9810 Appendix D.6)");
    }
 ...
 * private key, so a ccr carrying the private-key-transport encryptedKey proof-of-possession is refused, and
 * it carries exactly one CertReqMsg per Appendix D.6); each
