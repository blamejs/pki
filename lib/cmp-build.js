...
// Every runtime operation this module decides an encoding with, captured at load. Until then each
// is an ordinary writable property of a global, so a caller who replaces one after this file is
// required chooses what gets built: an `Array.isArray` answering true admits a non-list where a
// SEQUENCE OF is required, a `Number.isInteger` answering true carries a fractional pvno or
// iteration count past its bound, a `RegExp.prototype.test` answering false lets a path separator
// through the well-known segment gate, and a `Promise.resolve` of the caller's own settles the
// build with bytes nothing here produced. See guard-intrinsic.
...
var FAIL_INFO_INDEX = {};
// allow:guard-reads-runtime-live -- module-init, the index is inverted from the literal above at load
...
function _encodePkiFreeText(strings, code, what) {
  if (!_isArray(strings) || !strings.length) throw _err(code, what + " must be a non-empty array of strings");
  return b.sequence(_map(strings, function (s) {
...
function _encodeInfoTypeAndValue(itav) {
  if (!itav || typeof itav !== "object" || _isBuffer(itav)) throw _err("cmp/bad-info-value", "an InfoTypeAndValue must be an object { infoType, infoValue? }");
  if (typeof itav.infoType !== "string" || O(itav.infoType) == null) throw _err("cmp/bad-name", "unknown infoType " + _stringify(itav.infoType));
  var children = [b.oid(O(itav.infoType))];
  var val = _encodeInfoValue(itav);
  if (val != null) _push(children, val);
...
function _encodeGeneralInfo(itavs, code, what) {
  if (!_isArray(itavs) || !itavs.length) throw _err(code, what + " must be a non-empty array of InfoTypeAndValue");
  return b.sequence(_map(itavs, _encodeInfoTypeAndValue));
}
...
function _encodeHeader(headerSpec, protectionAlgDer, pvno) {
  ...
  if (headerSpec.messageTime != null) {
    ...
    _push(children, b.explicit(0, b.generalizedTime(headerSpec.messageTime)));   // GeneralizedTime ONLY (never UTCTime)
  }
  if (protectionAlgDer != null) _push(children, b.explicit(1, b.raw(protectionAlgDer)));
  if (headerSpec.senderKID != null) _push(children, b.explicit(2, _reqOctets(headerSpec.senderKID, "header.senderKID")));
  if (headerSpec.recipKID != null) _push(children, b.explicit(3, _reqOctets(headerSpec.recipKID, "header.recipKID")));
  if (headerSpec.transactionID != null) _push(children, b.explicit(4, _reqOctets(headerSpec.transactionID, "header.transactionID")));
  if (headerSpec.senderNonce != null) _push(children, b.explicit(5, _reqOctets(headerSpec.senderNonce, "header.senderNonce")));
  if (headerSpec.recipNonce != null) _push(children, b.explicit(6, _reqOctets(headerSpec.recipNonce, "header.recipNonce")));
  if (headerSpec.freeText != null) _push(children, b.explicit(7, _encodePkiFreeText(headerSpec.freeText, "cmp/bad-freetext", "header.freeText")));
  if (headerSpec.generalInfo != null) _push(children, b.explicit(8, _encodeGeneralInfo(headerSpec.generalInfo, "cmp/bad-general-info", "header.generalInfo")));
  return b.sequence(children);
}
...
function _encodeFailInfo(names, code) {
  if (!_isArray(names)) throw _err(code, "statusInfo.failInfo must be an array of PKIFailureInfo bit names");
  return b.namedBitString(_map(names, function (n) {
    if (typeof n !== "string" || FAIL_INFO_INDEX[n] === undefined) throw _err(code, "unknown PKIFailureInfo bit " + _stringify(n));
...
function _encodePkiStatusInfo(si, code) {
  ...
  if (!si || typeof si !== "object" || _isBuffer(si)) throw _err(code, "a PKIStatusInfo must be an object { status, statusString?, failInfo? }");
  if (typeof si.status !== "number" || !_isInteger(si.status) || si.status < 0 || si.status > 6) throw _err(code, "statusInfo.status must be a PKIStatus 0..6 (RFC 9810 sec. 5.2.3)");
  var children = [b.integer(_BigInt(si.status))];
  if (si.statusString != null) _push(children, _encodePkiFreeText(si.statusString, code, "statusInfo.statusString"));
  if (si.failInfo != null) _push(children, _encodeFailInfo(si.failInfo, code));   // strict order: status -> statusString -> failInfo (both optionals untagged)
  return b.sequence(children);
}
...
function _encodeCertStatus(cs, state) {
  if (!cs || typeof cs !== "object" || _isBuffer(cs)) throw _err("cmp/bad-cert-status", "each CertStatus must be an object");
  var children = [
  ...
  if (cs.statusInfo != null) _push(children, _encodePkiStatusInfo(cs.statusInfo, "cmp/bad-cert-status"));
  if (cs.hashAlg != null) {
    if (typeof cs.hashAlg !== "string" || !CERT_CONF_HASH_ALGS[cs.hashAlg]) throw _err("cmp/bad-name", "certConf hashAlg must be a hash algorithm (sha256 / sha384 / sha512 / sha3-256 / sha3-512 / sha1); got " + _stringify(cs.hashAlg));
    _push(children, b.explicit(0, _algIdNoParams(cs.hashAlg)));
...
function _encodeCertConfirmContent(list, state) {
  if (!_isArray(list)) throw _err("cmp/bad-cert-status", "certConf must be an array of CertStatus");
  return b.sequence(_map(list, function (cs) { return _encodeCertStatus(cs, state); }));
}

// PollReqContent ::= SEQUENCE OF SEQUENCE { certReqId INTEGER } (-1 legal).
function _encodePollReqContent(list) {
  if (!_isArray(list)) throw _err("cmp/bad-poll-req", "pollReq must be an array of { certReqId }");
  return b.sequence(_map(list, function (pr) {
    if (!pr || typeof pr !== "object" || _isBuffer(pr)) throw _err("cmp/bad-poll-req", "each pollReq entry must be { certReqId: <integer> }");
...
function _encodeGenMsgContent(list) {
  if (!_isArray(list)) throw _err("cmp/bad-info-type-and-value", "genm must be an array of InfoTypeAndValue");
  return b.sequence(_map(list, _encodeInfoTypeAndValue));
}
...
function _encodeRevDetails(rd) {
  if (!rd || typeof rd !== "object" || _isBuffer(rd)) throw _err("cmp/bad-rev-req", "each RevDetails must be an object { certDetails, crlEntryDetails? }");
  if (rd.certDetails == null) throw _err("cmp/bad-rev-req", "RevDetails.certDetails (a CertTemplate) is required");
  var certDetails;
  if (_isBuffer(rd.certDetails) || rd.certDetails instanceof Uint8Array) {
  ...
  if (rd.crlEntryDetails != null) _push(children, b.raw(_b.reqDer(rd.crlEntryDetails, "RevDetails.crlEntryDetails (a pre-encoded Extensions DER)")));
  return b.sequence(children);
}

function _encodeRevReqContent(list) {
  if (!_isArray(list) || !list.length) throw _err("cmp/bad-rev-req", "rr must be a non-empty array of RevDetails");
  return b.sequence(_map(list, _encodeRevDetails));
}
...
function _encodeCertOrEncCert(coec, state) {
  if (!coec || typeof coec !== "object" || _isBuffer(coec)) throw _err("cmp/bad-cert-response", "certifiedKeyPair must carry certificate or encryptedCert");
...
function _encodeCertifiedKeyPair(ckp, state) {
  if (!ckp || typeof ckp !== "object" || _isBuffer(ckp)) throw _err("cmp/bad-cert-response", "certifiedKeyPair must be an object");
  ...
  if (ckp.privateKey != null) {
    ...
    _push(children, b.explicit(0, b.raw(pk)));
  ...
  if (ckp.publicationInfo != null) _push(children, b.explicit(1, b.raw(_b.reqDer(ckp.publicationInfo, "certifiedKeyPair.publicationInfo (a pre-encoded DER)"))));
  return b.sequence(children);
}
...
function _encodeCertResponse(cr, state) {
  if (!cr || typeof cr !== "object" || _isBuffer(cr)) throw _err("cmp/bad-cert-response", "each CertResponse must be an object");
  ...
  if (hasCkp) _push(children, _encodeCertifiedKeyPair(cr.certifiedKeyPair, state));
  if (cr.rspInfo != null) _push(children, _reqOctets(cr.rspInfo, "CertResponse.rspInfo"));
  return b.sequence(children);
}
...
function _encodeCertRepMessage(spec, state, arm) {
  if (!spec || typeof spec !== "object" || _isBuffer(spec)) throw _err("cmp/bad-cert-rep", arm + " must be an object { caPubs?, response }");
  if (!_isArray(spec.response)) throw _err("cmp/bad-cert-rep", arm + ".response must be an array of CertResponse");
  ...
  if (spec.caPubs != null) {   // caPubs [1] PRECEDES response (the optional-first x509-version shape)
    if (!_isArray(spec.caPubs) || !spec.caPubs.length) throw _err("cmp/bad-cert-rep", arm + ".caPubs must be a non-empty array of certificate DERs");
    _push(children, b.explicit(1, b.sequence(_map(spec.caPubs, function (c) { return _certRaw(c, "cmp/bad-cert-rep", "a caPubs entry"); }))));
  }
  _push(children, b.sequence(_map(spec.response, function (cr) { return _encodeCertResponse(cr, state); })));
  return b.sequence(children);
}
...
function _encodeCertIdRr(cid) {
  if (!cid || typeof cid !== "object" || _isBuffer(cid) || cid.issuer == null || cid.serialNumber == null) throw _err("cmp/bad-rev-rep", "each revCerts CertId must be { issuer, serialNumber }");
...
function _encodeRevRepContent(spec) {
  if (!spec || typeof spec !== "object" || _isBuffer(spec)) throw _err("cmp/bad-rev-rep", "rp must be an object { status, revCerts?, crls? }");
  if (!_isArray(spec.status) || !spec.status.length) throw _err("cmp/bad-rev-rep", "rp.status must be a non-empty array of PKIStatusInfo");
  var children = [b.sequence(_map(spec.status, function (si) { return _encodePkiStatusInfo(si, "cmp/bad-rev-rep"); }))];
  if (spec.revCerts != null) {
    if (!_isArray(spec.revCerts) || !spec.revCerts.length) throw _err("cmp/bad-rev-rep", "rp.revCerts must be a non-empty array of CertId");
    _push(children, b.explicit(0, b.sequence(_map(spec.revCerts, _encodeCertIdRr))));
  }
  if (spec.crls != null) {
    if (!_isArray(spec.crls) || !spec.crls.length) throw _err("cmp/bad-rev-rep", "rp.crls must be a non-empty array of CertificateList DERs");
    _push(children, b.explicit(1, b.sequence(_map(spec.crls, function (c) {
...
function _encodeErrorMsgContent(spec) {
  if (!spec || typeof spec !== "object" || _isBuffer(spec)) throw _err("cmp/bad-error", "error must be an object { pKIStatusInfo, errorCode?, errorDetails? }");
  ...
  if (spec.errorCode != null) _push(children, b.integer(_reqIdInt(spec.errorCode, "cmp/bad-error", "error.errorCode")));
  if (spec.errorDetails != null) _push(children, _encodePkiFreeText(spec.errorDetails, "cmp/bad-error", "error.errorDetails"));
  return b.sequence(children);
}
...
function _encodePollRepContent(list) {
  if (!_isArray(list) || !list.length) throw _err("cmp/bad-poll-rep", "pollRep must be a non-empty array of { certReqId, checkAfter, reason? }");
  return b.sequence(_map(list, function (pr) {
    if (!pr || typeof pr !== "object" || _isBuffer(pr)) throw _err("cmp/bad-poll-rep", "each pollRep entry must be an object");
    if (typeof pr.checkAfter !== "number" || !_isInteger(pr.checkAfter) || pr.checkAfter < 0 || pr.checkAfter > 0x7fffffff) throw _err("cmp/bad-poll-rep", "pollRep checkAfter must be a non-negative uint31 delay in seconds (RFC 9810 sec. 5.3.22)");
    var kids = [b.integer(_reqIdInt(pr.certReqId, "cmp/bad-poll-rep", "pollRep certReqId")), b.integer(_BigInt(pr.checkAfter))];
    if (pr.reason != null) _push(kids, _encodePkiFreeText(pr.reason, "cmp/bad-poll-rep", "pollRep reason"));
...
function _encodeKeyRecRepContent(spec, state) {
  if (!spec || typeof spec !== "object" || _isBuffer(spec)) throw _err("cmp/bad-key-rec-rep", "krp must be an object { status, newSigCert?, caCerts?, keyPairHist? }");
  ...
  if (spec.newSigCert != null) _push(children, b.explicit(0, _certRaw(spec.newSigCert, "cmp/bad-key-rec-rep", "krp.newSigCert")));
  if (spec.caCerts != null) {
    if (!_isArray(spec.caCerts) || !spec.caCerts.length) throw _err("cmp/bad-key-rec-rep", "krp.caCerts must be a non-empty array of certificate DERs");
    _push(children, b.explicit(1, b.sequence(_map(spec.caCerts, function (c) { return _certRaw(c, "cmp/bad-key-rec-rep", "a krp.caCerts entry"); }))));
  }
  if (spec.keyPairHist != null) {
    if (!_isArray(spec.keyPairHist) || !spec.keyPairHist.length) throw _err("cmp/bad-key-rec-rep", "krp.keyPairHist must be a non-empty array of CertifiedKeyPair");
    _push(children, b.explicit(2, b.sequence(_map(spec.keyPairHist, function (ckp) { return _encodeCertifiedKeyPair(ckp, state); }))));
  }
  return b.sequence(children);
}
...
function _encodeBody(bodySpec, key, opts) {
  if (!bodySpec || typeof bodySpec !== "object" || _isBuffer(bodySpec)) throw _err("cmp/bad-input", "message.body must be a single-key object");
  var keys = _keys(bodySpec);
  if (keys.length !== 1) throw _err("cmp/bad-input", "message.body must have exactly one arm, got " + keys.length);
  var arm = keys[0];
  if (!KNOWN_BODY_KEYS[arm]) throw _err("cmp/bad-input", "unknown body arm " + _stringify(arm));
  ...
  if (CRMF_BODY[arm]) {
    ...
    if (reqSpec && typeof reqSpec === "object" && !_isBuffer(reqSpec) && "key" in reqSpec) {
      popKey = reqSpec.key;
      reqSpec = _assign({}, reqSpec);
      delete reqSpec.key;
    }
  ...
  return _promiseResolve(_Promise, { bodyTLV: b.explicit(tag, inner), usesCmp2021: state.usesCmp2021 });
}
...
function _resolveProtection(opts) {
  ...
  if (hasSig) {
    ...
    return {
      ...
      computeBits: function (protectedPartDer) {
        return _promiseResolve(_Promise, signScheme.signOverTbs(scheme, opts.key, protectedPartDer, _signE)).then(function (sig) {
          return _promiseResolve(_Promise, _b.assertSignatureVerifies(protectedPartDer, sig, senderSpki, scheme)).then(function () {
  ...
  // PBMAC1
  var m = opts.mac;
  if (!m || typeof m !== "object" || _isBuffer(m)) throw _err("cmp/bad-input", "opts.mac must be an object { secret, salt?, iterationCount?, prf?, keyLength? }");
  guard.identifier.assertKnownKeys(m, KNOWN_MAC_KEYS, _err, "cmp/bad-input", "unknown opts.mac field ");
  if (m.algorithm != null && m.algorithm !== "pbmac1") throw _err("cmp/unsupported-algorithm", "opts.mac.algorithm " + _stringify(m.algorithm) + " is not supported (v1 ships pbmac1; passwordBasedMac is deferred)");
  var secret = m.secret;
  if (typeof secret !== "string" || !secret) {
    if (!_isBuffer(secret) || !secret.length) throw _err("cmp/bad-input", "opts.mac.secret must be a non-empty string or Buffer");
  }
  var secretBuf = _isBuffer(secret) ? secret : _bufferFrom(secret, "utf8");
  ...
  if (typeof iterationCount !== "number" || !_isInteger(iterationCount) || iterationCount < PBMAC1_MIN_ITER) throw _err("cmp/bad-input", "opts.mac.iterationCount must be at least " + PBMAC1_MIN_ITER + " (RFC 8018 sec. 4.2)");
  ...
  if (typeof keyLength !== "number" || !_isInteger(keyLength) || keyLength < PBMAC1_MIN_KEYLEN) throw _err("cmp/bad-input", "opts.mac.keyLength must be an integer >= " + PBMAC1_MIN_KEYLEN + " bytes (RFC 9579 sec. 9)");
  ...
  var blocks = _ceil(keyLength / PBMAC1_PRF_HLEN[prf]);
  ...
  var salt = m.salt != null ? _b.reqDer(m.salt, "opts.mac.salt") : _bufferFrom(webcrypto.webcrypto.getRandomValues(new _Uint8Array(PBMAC1_DEFAULT_SALT_BYTES)));
...
function _build(message, opts) {
  opts = opts || {};
  if (!message || typeof message !== "object" || _isBuffer(message)) throw _err("cmp/bad-input", "the PKIMessage spec must be an object { header, body }");
  guard.identifier.assertKnownKeys(message, KNOWN_MESSAGE_KEYS, _err, "cmp/bad-input", function (k) {
    return "unknown message field " + _stringify(k) + " (a message carries only header + body)";
  });
  ...
  return _promiseResolve(_Promise, _encodeBody(message.body, senderKey, opts)).then(function (bodyResult) {
    var bodyTLV = bodyResult.bodyTLV;
    var pvno = message.header.pvno != null ? message.header.pvno : 2;
    if (typeof pvno !== "number" || !_isInteger(pvno)) throw _err("cmp/bad-input", "header.pvno must be an integer");
    ...
    return prot.computeBits(protectedPartDer).then(function (protectionBits) {
      ...
      if (extraCertsDers.length) _push(msgChildren, b.explicit(1, b.sequence(extraCertsDers)));
...
function _collectExtraCerts(opts, protCertDer) {
  var out = [];
  if (protCertDer != null) _push(out, b.raw(protCertDer));
  if (opts.extraCerts != null) {
    if (!_isArray(opts.extraCerts)) throw _err("cmp/bad-extra-certs", "opts.extraCerts must be an array of certificate DERs");
    _forEach(opts.extraCerts, function (c) {
...
function _cmpMessageDer(input) {
  var der;
  if (_isBuffer(input) || input instanceof Uint8Array) der = guard.bytes.view(input, CmpError, "cmp/bad-input", "the CMP message DER");
...
function _ctypeToken(headers) { return _toLowerCase(_trim(_strSplit(_String((headers || {})["content-type"] || ""), ";")[0])); }
...
function _classifyCmpResponse(status, headers, body, tls) {
  ...
  if (status === 200) {
    if (!_isPkixcmp(headers)) throw _err("cmp/bad-content-type", "a 200 CMP response must be application/pkixcmp (RFC 9811 sec. 3.2), got " + _stringify(ctype || null));
  ...
  var httpStatus = (typeof status === "number" && _isSafeInteger(status) && status >= 100 && status <= 599) ? status : null;
  ...
  throw _err("cmp/unexpected-status", "a CMP response must be HTTP 200 (or a 4xx/5xx carrying a CMP error), not " + _stringify(status) + " (RFC 9811 sec. 3.1)");
}
...
function _transfer(url, message, opts) {
  ...
  var parsedUrl;
  try { parsedUrl = new URL(_String(url)); }   // parse only -- NO client scheme gate (the transport owns socket security)
  catch (e) { throw _err("cmp/bad-url", "the CMP URL did not parse: " + _String(url), e); }
  var transport = opts.transport;
  if (!transport) {
    var t = opts.tls || {};
    var hasAnchors = t.anchors !== undefined && t.anchors !== null && !(_isArray(t.anchors) && t.anchors.length === 0);
  ...
  var headers = {};
  _forEach(_keys(opts.headers || {}), function (k) {
  ...
  return _promiseResolve(_Promise, transport({ method: "POST", url: parsedUrl.href, headers: headers, body: der, tls: tls, timeout: timeout, maxResponseBytes: maxResponseBytes })).then(function (res) {
    res = res || {};
    var h = {};
    ...
    var body = (_isBuffer(res.body) || res.body instanceof Uint8Array)
      ? guard.bytes.view(res.body, CmpError, "cmp/bad-response", "the CMP response body")
      : _bufferFrom(_String(res.body == null ? "" : res.body), "utf8");
...
function _wellKnownSeg(v, name) {
  var s = _String(v);
  if (s === "" || s === "." || s === ".." || _reTest(/[/?#\s\\]/, s)) throw _err("cmp/bad-url", "the CMP " + name + " must be a single safe path segment (no '/', dot-segment, or reserved char): " + _stringify(s));
  ...
  catch (e) { throw _err("cmp/bad-url", "the CMP " + name + " is not encodable (e.g. an unpaired surrogate): " + _stringify(s), e); }
}
...
function wellKnownUrl(base, opts) {
  ...
  guard.identifier.assertKnownKeys(opts, KNOWN_WELLKNOWN_OPTS, _err, "cmp/bad-input", function (k) {
    return "unknown wellKnownUrl option " + _stringify(k) + " (expected label / operation)";
  });
  var u;
  try { u = new URL(_String(base)); }
  catch (e) { throw _err("cmp/bad-url", "the CMP base URL did not parse: " + _String(base), e); }
  ...
  if (u.protocol !== "https:" && u.protocol !== "http:") throw _err("cmp/bad-url", "the CMP base URL must be http or https (a well-known URI is authority-rooted over HTTP), got " + u.protocol + ": " + _stringify(_String(base)));
  ...
  if (_reTest(/\\/, _String(base))) throw _err("cmp/bad-url", "the CMP base URL must not contain a backslash (WHATWG rewrites it to a path separator): " + _stringify(_String(base)));
  ...
  var rawBasePath = _strSplit(_strReplace(_String(base), /^[a-z]+:\/\/[^/?#]*/i, ""), /[?#]/)[0];
  if (rawBasePath !== "" && rawBasePath !== "/") throw _err("cmp/bad-url", "an RFC 8615 well-known URI is authority-rooted; the CMP base URL must have no path component (pass a full operation URL to transfer instead): " + _stringify(_String(base)));
  var segs = [".well-known", "cmp"];
  if (opts.label != null) { _push(segs, "p"); _push(segs, _wellKnownSeg(opts.label, "label")); }
  if (opts.operation != null) _push(segs, _wellKnownSeg(opts.operation, "operation"));
  return u.origin + "/" + _join(segs, "/");
}
