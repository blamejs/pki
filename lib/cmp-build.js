...
function _assertTransport(transport) {
  if (transport != null && typeof transport !== "function") {
    throw _err("cmp/bad-input", "opts.transport must be a function (request) => Promise<{ status, headers, body }>, got " + guard.text.showValue(transport));
  }
}
...
function _transfer(url, message, opts) {
  ...
  return guard.async.awaited(transport({ method: "POST", url: parsedUrl.href, headers: headers, body: der, tls: tls, proxy: proxy, timeout: timeout, maxResponseBytes: maxResponseBytes }), _err, "cmp/bad-input", "opts.transport").then(function (res) {
    res = res || {};
