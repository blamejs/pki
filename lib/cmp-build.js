...
function _cmpMessageDer(input) {
  if (Buffer.isBuffer(input) || input instanceof Uint8Array) return guard.bytes.view(input, CmpError, "cmp/bad-input", "the CMP message DER");
...
function _classifyCmpResponse(status, headers, body, tls) {
  ...
  if (_isPkixcmp(headers) && body.length > 0) {
    ...
    if (forwarded.body.arm !== "error") throw _err("cmp/http-error", "the CMP server returned HTTP " + status + " with a non-error '" + forwarded.body.arm + "' body; an HTTP failure forwards only a CMP error message (RFC 9811 sec. 1.2/3.1)");
    return { response: forwarded, responseBytes: body, status: status, contentType: ctype, tls: tls };
  }
  throw _err("cmp/http-error", "the CMP server returned HTTP " + status + " with no forwardable CMP error body");
}
