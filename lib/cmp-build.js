...
// The DER of a caller-supplied PKIMessage: a DER Buffer / Uint8Array as-is (sent VERBATIM -- protection
// covers these exact bytes), a PEM "CMP" string decoded, else a config-time cmp/bad-input before the wire.
function _cmpMessageDer(input) {
  if (Buffer.isBuffer(input)) return input;
  if (input instanceof Uint8Array) return Buffer.from(input);
  if (typeof input === "string") return cmp.pemDecode(input);
  throw _err("cmp/bad-input", "a CMP message must be a DER Buffer/Uint8Array or a PEM CMP string");
}
...
function _classifyCmpResponse(status, headers, body, tls) {
  ...
  if (status > 200 && status < 300) throw _err("cmp/unexpected-status", "a CMP response must be HTTP 200, not " + status + " (RFC 9811 sec. 3.1)");
  if (status >= 300 && status < 400) throw _err("cmp/redirect-not-followed", "a CMP " + status + " redirect is not followed (RFC 9811 sec. 3.1, sec. 5); reconfigure the endpoint URL");
  if (_isPkixcmp(headers) && body.length > 0) {
    var forwarded;
    try { forwarded = cmp.parse(body); }
    catch (e) { throw _err("cmp/http-error", "the CMP server returned HTTP " + status + " with an undecodable body", e); }
    return { response: forwarded, responseBytes: body, status: status, contentType: ctype, tls: tls };
  }
  throw _err("cmp/http-error", "the CMP server returned HTTP " + status + " with no CMP response body");
}
...
function _transfer(url, message, opts) {
  ...
  var headers = Object.assign({}, opts.headers, { "content-type": PKIXCMP });   // content-type set LAST -- an opts.headers override cannot change it
  ...
  return Promise.resolve(transport({ method: "POST", url: parsedUrl.href, headers: headers, body: der, tls: tls, timeout: timeout, maxResponseBytes: maxResponseBytes })).then(function (res) {
    ...
    // Normalize an injected string body to a Buffer (UTF-8, the width a real socket already counts) so the
    // size cap and the parser see the same bytes -- a non-ASCII body cannot undercount past the cap.
    var body = Buffer.isBuffer(res.body) ? res.body : Buffer.from(String(res.body == null ? "" : res.body), "utf8");
...
function wellKnownUrl(base, opts) {
  ...
  // An RFC 8615 well-known URI is AUTHORITY-ROOTED (/.well-known/...), so a base carrying a path prefix
  // (https://ca.example/tenant) would build a URL that never reaches a conforming discovery endpoint.
  // Reject it fail-closed rather than silently retarget -- an operator whose CMP endpoint sits under a
  // path passes that full operation URL to transfer() directly (the well-known form does not apply).
  // An RFC 8615 well-known URI is AUTHORITY-ROOTED (/.well-known/...), so a base carrying a path prefix
  // (https://ca.example/tenant) would build a URL that never reaches a conforming discovery endpoint.
  // Reject it fail-closed rather than silently retarget -- an operator whose CMP endpoint sits under a
  // path passes that full operation URL to transfer() directly (the well-known form does not apply).
  if (u.pathname !== "/") throw _err("cmp/bad-url", "an RFC 8615 well-known URI is authority-rooted; the CMP base URL must have no path component (pass a full operation URL to transfer instead): " + JSON.stringify(String(base)));
 ...
 * integrity-protected verdict (sec. 1.2/3.1) with the HTTP status surfaced as data, while a 4xx/5xx with no
 * CMP body is `cmp/http-error`. Protection is SURFACED, not verified -- the client confers no trust; the
 ...
 *   - `headers` -- extra request headers (the `content-type: application/pkixcmp` is always set and not overridable).
 ...
 * builder -- it does not enforce a scheme (RFC 9811 sec. 3.4 shows both http and https forms; the transport is
 * where https fail-closes). It rejects (`cmp/bad-url`) an unparseable `base`, a `base` carrying a query or
 * fragment (which would capture the well-known path), and a `label`/`operation` that is empty, a dot-segment,
 * or contains a `/` or other reserved char (never silently retarget the resource). The `operation` label is a
 * caller-supplied string, not validated against a profile vocabulary (RFC 9483 is out of scope).
 *
 * @opts
