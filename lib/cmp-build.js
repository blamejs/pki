...
function _encodeCertOrEncCert(coec) {
  ...
  if (coec.encryptedCert != null) {
    // encryptedCert [1] wraps an EncryptedKey (EnvelopedData) -- a pre-encoded DER hatch in v1 (the structured
    // private-key-transport / KEM form is the deferred protection cut).
    return b.explicit(1, b.raw(_b.reqDer(coec.encryptedCert, "certifiedKeyPair.encryptedCert (a pre-encoded EncryptedKey DER)")));
  }
...
function _encodeCertifiedKeyPair(ckp, state) {
  ...
  var children = [_encodeCertOrEncCert(ckp)];   // certificate / encryptedCert are direct keys (the parse-result shape)
  if (ckp.privateKey != null) { children.push(b.explicit(0, b.raw(_b.reqDer(ckp.privateKey, "certifiedKeyPair.privateKey (a pre-encoded EncryptedKey DER)")))); state.usesCmp2021 = true; }
...
function _encodeRevRepContent(spec) {
  ...
  if (spec.crls != null) {
    ...
    children.push(b.explicit(1, b.sequence(spec.crls.map(function (c) { return b.raw(_b.reqDer(c, "crls entry (a CertificateList DER)")); }))));
  }
  return b.sequence(children);
}
