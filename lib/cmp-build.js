...
function _certReqNeedsCmp2021(crmfDer) {
  var messages;
  ...
  for (var i = 0; i < messages.length; i++) {
    ...
    if (!popo && _templateRequestsCentralKeyGeneration(messages[i].certReq.certTemplate)) return true;
  }
  return false;
}
...
function _encodeBody(bodySpec, key, opts) {
  ...
  if (CRMF_BODY[arm]) {
    ...
    return crmf.build(reqSpec, popKey, {}).then(function (crmfDer) {
      return { bodyTLV: b.explicit(tag, b.raw(crmfDer)), usesCmp2021: _certReqNeedsCmp2021(crmfDer) };
    });
  }
  var inner;
  if (arm === "p10cr") {
