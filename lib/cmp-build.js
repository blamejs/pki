...
var KNOWN_BODY_KEYS = {
  ...
  nested: 1,   // an RA wrapper of complete, independently-protected PKIMessages (RFC 9810 sec. 5.1.3.4)
};
...
// 5.1.3.4): an RA forwards or batches complete, independently-protected PKIMessages, wrapping them under its
// OWN protection. Each entry is a full PKIMessage DER, validated via the real parser so a non-message fails
// closed here rather than as opaque bytes; the wrapper's protection covers this SEQUENCE, not the inner
// messages, and the parser never auto-recurses a nested body (the operator re-feeds each inner message).
function _encodeNestedContent(list) {
  if (!intrinsic.isArray(list) || list.length === 0) {
    throw _err("cmp/bad-input", "nested must be a non-empty array of PKIMessage DER (RFC 9810 sec. 5.1.3.4 PKIMessages is SIZE (1..MAX))");
  }
  return b.sequence(intrinsic.map(list, function (m, i) {
    ...
    try { cmp.parse(der); }   // validate each embedded message via the real parser (fail-closed on a non-PKIMessage)
    catch (e) { throw _err("cmp/bad-input", "nested[" + i + "] is not a valid PKIMessage", e); }
    return b.raw(der);
  }));
}
 ...
 * Registration-authority: `{ nested }` (an array of complete PKIMessage DER wrapped as `NestedMessageContent`,
 * RFC 9810 sec. 5.1.3.4; each entry is parsed as a PKIMessage before it is wrapped, and an empty array or an
 * invalid entry is refused). The
