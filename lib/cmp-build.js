...
function _encodePollReqContent(list) {
  ...
  return b.sequence(list.map(function (pr) {
    if (!pr || typeof pr !== "object" || Buffer.isBuffer(pr) || typeof pr.certReqId !== "number" || !Number.isInteger(pr.certReqId)) throw _err("cmp/bad-poll-req", "each pollReq entry must be { certReqId: <integer> }");
