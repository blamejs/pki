...
function _popNeedsCmp2021(crmfDer, isCcr) {
  var messages;
  ...
  // the caller's own input. A ccr template may carry signingAlg [2] and a version of v1, which the
  // generic CRMF parser rejects (RFC 4211 sec. 5); re-read it through the same App. D.6 profile
  // relaxations the CMP parser applies to this arm, so the self-verify accepts the bytes the CMP
  // parser will accept.
  try {
    messages = isCcr
      ? schemaCrmf.walkCertReqMessages(asn1.decode(crmfDer), { allowSigningAlg: true, allowV1Version: true }).messages
