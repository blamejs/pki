...
require("./path-validate");   // side-effect: path-validate injects its signature engine into csr-verify at load
var csrVerify = require("./csr-verify");   // verify an embedded p10cr request's proof-of-possession before sending it
...
var KNOWN_BODY_KEYS = {
  ir: 1, cr: 1, kur: 1, krr: 1, ccr: 1, p10cr: 1, certConf: 1, pollReq: 1, genm: 1, rr: 1,   // request-side
  ip: 1, cp: 1, kup: 1, ccp: 1, krp: 1, rp: 1, genp: 1, error: 1, pollRep: 1, pkiconf: 1,   // CA/responder-side
  nested: 1,   // an RA wrapper of complete, independently-protected PKIMessages (RFC 9810 sec. 5.1.3.5)
};
...
var BODY_TAG = {
  ...
  nested: 20,   // NestedMessageContent (schema-cmp.js BODY_ARMS [20])
};
var CRMF_BODY = { ir: 1, cr: 1, kur: 1, krr: 1, ccr: 1 };   // arms whose content is a CertReqMessages via pki.crmf.build (RFC 9810 sec. 5.3.7 krr / 5.3.11 ccr)
...
function _encodeRevDetails(rd) {
  ...
  if (guard.bytes.isByteSource(rd.certDetails)) {
    // The pre-encoded escape hatch: normalized through the byte guard so every BufferSource the guard
    // accepts (Buffer, any typed-array view, DataView, raw ArrayBuffer) is taken, not a narrowed subset
    // that would drop a caller's container into the { issuer, serialNumber } branch below.
...
function _encodeCrlEntryDetails(spec) {
  // A pre-encoded Extensions DER is classified through the byte guard and normalized through it, so
  // every BufferSource form is taken as bytes. A narrowed check would drop a raw ArrayBuffer / DataView
  // into the { reason } branch below, where an absent reason silently becomes a generated unspecified(0)
  // reasonCode -- a different revocation reason than the caller encoded.
  ...
  // After the byte guard, the only valid structured form is a plain { reason? } record. Anything else --
  // an array, a Date/Map/Set or other exotic, a boxed primitive -- carries no `reason`, so the fallback
  // below would read undefined and emit a generated unspecified(0) reasonCode: a real revocation reason
  // the caller never wrote, from a value outside the documented { reason } | DER union. assertKnownKeys
  // does not catch these (an empty array or a keyless exotic has no unknown key to reject), so the value
  // is held to being a plain record through guard.identifier.assertPlainRecord rather than a bare
  // typeof-object test.
  ...
  // Read `reason` ONCE: a caller getter or Proxy could otherwise answer the validation with one value
  // (a known reason) and the encode with another (null -> a generated unspecified(0)), so the request
  // would carry a different revocation reason than the one that was checked.
  // Read `reason` ONCE, consistent with the module's read-once discipline: build() already deep-copies
  // the caller's message at entry, so a getter or Proxy cannot vary between the reads here, but reading
  // one local keeps validation and encode on the same value with no dependence on that upstream copy.
...
function buildCrlStatusList(spec) {
  ...
  // Each caller field is read ONCE, here, and every use below is of the value that was read. A
  // second read of the same field can answer differently, and what is encoded then differs from
  ...
  // A distribution point name is a pointer the responder resolves internally, so on its own it
  // leaves nothing to hold the answer to: a CRL claiming to be its issuer's complete list states no
  // scope at all (RFC 5280 sec. 5.2.5 permits that), and the request named no issuer to compare it
  // with, so any well-formed CRL from any CA would answer. The issuer is therefore required
  // alongside a dpn, naming the CA whose CRL the caller will accept. sec. 4.3.4's own note says
  // where a distribution point name comes from, and both sources -- a certificate's
  // cRLDistributionPoints, a held CRL's issuingDistributionPoint -- carry the issuer with it.
  if (dpnSpec != null && issuerSpec == null) {
  ...
  // The issuer, when given, is kept whether or not it goes on the wire: a caller who names one is
  // saying which CA's CRL they will accept, and pki.cmp.session holds the answer to it. Taking the
  // encoded Name from here means the comparison reads the name the caller gave rather than
  // re-interpreting the spec at a later boundary.
  if (issuerSpec != null) issuerNameDer = _b.encodeName(issuerSpec);
  if (dpnSpec != null) {
    // CRLSource is a CHOICE, so exactly one alternative goes on the wire, and sec. 4.3.4 settles
    // which: the dpn "if the CRL distribution point name is available", otherwise the issuer. A
    // caller giving both is answered by the dpn, the more specific pointer, with the issuer held
    // back to bind the response.
    ...
    // The encoded GeneralNames are kept for the same reason as the issuer Name. RFC 5280 sec. 5.2.5
    // pins the comparison to the identical encoding, so what is kept is the bytes that went on the
    // wire rather than the spec they were built from.
    ...
    // GeneralName element". The directoryName [4] EXPLICIT wraps the SAME encoded Name saved above for
    // the response binding (issuerNameDer), not a second read of issuerSpec: a caller's issuer can be an
    // accessor, and re-encoding it here would put one name on the wire while the answer is held to
    // another, so a CRL from the name that was validated could answer a request that named a different one.
...
// The distributionPoint [0] of a CRL's issuingDistributionPoint extension (RFC 5280 sec. 5.2.5),
// decoded to the DistributionPointName form guard.name.dpnCorresponds compares, or null when the
// extension names no point -- a complete CRL, whose scope is its issuer's whole population rather
// than one point of it. A structural fault THROWS instead of reading as "names no point": a session
// binds a requested distribution point to this value, and an unreadable extension resolving to
// "nothing to compare" would skip the binding it is read for.
...
function decodeIdpDistributionPoint(valueBytes) {
  ...
  // distributionPoint [0] EXPLICIT-wraps the DistributionPointName CHOICE (a context tag on a
  // CHOICE-typed field is always EXPLICIT), so the name is its single child.
...
function _popNeedsCmp2021(crmfDer, allowSigningAlg) {
  var messages;
  ...
  // the caller's own input. A ccr template may carry signingAlg [2] (RFC 9810 App. D.6), which the
  // generic CRMF parser rejects (RFC 4211 sec. 5); re-read it through the same profile relaxation the
  // CMP parser applies to this arm, so the self-verify accepts the bytes the CMP parser will accept.
  try {
    messages = allowSigningAlg
      ? schemaCrmf.walkCertReqMessages(asn1.decode(crmfDer), { allowSigningAlg: true }).messages
      : schemaCrmf.parse(crmfDer).messages;
  }
  catch (e) {
...
// nested [20] NestedMessageContent ::= PKIMessages ::= SEQUENCE SIZE (1..MAX) OF PKIMessage (RFC 9810 sec.
// 5.1.3.5 "Multiple Protection"): an RA forwards or batches complete, independently-protected PKIMessages,
// wrapping them UNCHANGED under its OWN protection. Each entry is a full PKIMessage DER, checked by the real
// parser so a non-PKIMessage fails closed here rather than wrapping as opaque bytes. The check is the toolkit's
// PKIMessage parse: it validates the envelope and every arm the parser decodes and, exactly as a parse
// anywhere else does, surfaces the raw arms (p10cr, the announcements, a further nested) without re-validating
// their content -- which sec. 5.1.3.5 leaves to the recipient, since the RA forwards each message unchanged.
// The wrapper's protection covers this SEQUENCE, not the inner messages, and the parser never auto-recurses a
// nested body (the operator re-feeds each inner message).
function _encodeNestedContent(list) {
  ...
  // reqDenseArray rejects a sparse hole or a nullish entry up front: Array.prototype.map SKIPS a hole,
  // which would otherwise reach Buffer.concat as a native ERR_INVALID_ARG_TYPE and, on a huge sparse
  // length, traverse the empty slots. With the array proven dense, the map validates each entry in turn.
  return b.sequence(intrinsic.map(_b.reqDenseArray(list, "nested"), function (m, i) {
    // reqDerSequence requires a DER SEQUENCE up front: cmp.parse also unwraps PEM from a byte source, so a
    // PEM-armored entry (a Buffer straight from fs.readFileSync) would otherwise pass the parse yet be
    // embedded by b.raw as ASCII armor and rejected as cmp/bad-der by the outer message's own parse, only
    // after protection is computed. With DER assured, the bytes cmp.parse checks are the bytes embedded.
    var der = _b.reqDerSequence(m, "nested[" + i + "] (a PKIMessage DER)");
    // A bad entry is a build-time authoring error, so any parse failure is one uniform cmp/bad-input naming
    // the offending index (the underlying reason rides along as the cause), never the parser's internal code.
    try { cmp.parse(der); }   // check each embedded message parses as a PKIMessage (fail-closed on a non-PKIMessage)
...
function _encodeBody(bodySpec, key, opts) {
  ...
  if (CRMF_BODY[arm]) {
    ...
    // A cross-certification request (ccr) may profile its template with signingAlg [2] (RFC 9810 App. D.6),
    // which the parser accepts only for this arm; permit the CRMF builder to emit it here and nowhere else.
    return crmf.build(reqSpec, popKey, { crossCert: arm === "ccr" }).then(function (crmfDer) {
      ...
      return { bodyTLV: b.explicit(tag, b.raw(crmfDer)), usesCmp2021: _popNeedsCmp2021(crmfDer, arm === "ccr") };
    });
  }
  var inner;
  if (arm === "p10cr") {
    ...
    var parsedCsr;
    try { parsedCsr = csr.parse(csrDer); }   // validate structure via the real parser AND capture the parse for the PoP check
    catch (e) { if (e instanceof CmpError) throw e; throw _err("cmp/bad-input", "p10cr is not a valid PKCS#10 CertificationRequest", e); }
    // Verify the embedded request's proof-of-possession before it is protected and sent: a p10cr carries
    // a PKCS#10 CertificationRequest (RFC 9810 sec. 5.3.3, whose structure is RFC 2986), and its
    // self-signature under its own subject public key is that request's proof-of-possession. A CSR whose
    // self-signature does not verify is one the CA will reject, so refuse it rather than send it -- through
    // the shared inbound-PoP seam pki.csr.verify uses.
...
// `await`, not a `.then` chain: the PoP verdict is read through the native promise job, not the
// replaceable Promise.prototype.then a co-resident could swap to force `ok` true and slip an
// unverified request past. The caller has already validated csrDer's structure synchronously.
...
function _resolveProtection(opts) {
  ...
  // Default only when prf is absent (null/undefined), the same test the iterationCount and keyLength
  // siblings below use. `m.prf || "SHA-256"` would default on any falsy value, so a supplied "", false, or
  // 0 -- none a valid PRF name -- would silently encode HMAC-SHA-256 instead of being refused.
...
async function _build(message, opts) {
  ...
  // `await`, not a `.then` chain: an embedded p10cr/crm proof-of-possession is enforced by _encodeBody
  // REJECTING, so the verdict must reach here through the native promise job, not a Promise.prototype.then
  // a co-resident could swap to fire this callback with a forged body and slip an unverified request past.
 ...
 * RFC 9810 sec. 5.3.7) / `{ ccr }` (cross-certification, sec. 5.3.11 -- the requesting CA generates and keeps
 * the key, so a private-key-transport `encryptedKey` proof of possession is refused, and its template may carry
 * a `signingAlg` (a pre-encoded AlgorithmIdentifier DER) naming the algorithm the responding CA is asked to
 * sign with, App. D.6) (a `CertReqMessages` spec
...
// /[/?#\s\\]/ : does `s` contain a path separator, query/fragment marker, whitespace, or a backslash?
...
// ---- URL raw-path helpers by character scan (no regex) --------------------------------------------
function _isAsciiLetter(ch) { return (ch >= "A" && ch <= "Z") || (ch >= "a" && ch <= "z"); }
// The "scheme://authority" prefix removed (matching /^[a-z]+:\/\/[^/?#]*/i: a leading letter run,
// "://", then a run of characters other than "/", "?" or "#"), else the string unchanged.
...
// Everything before the first "?" or "#" (matching split(/[?#]/)[0]).
