...
function _encodeInfoValue(itav) {
  ...
  if (name === "confirmWaitTime") {
    if (!(itav.infoValue instanceof Date)) throw _err("cmp/bad-info-value", "confirmWaitTime infoValue must be a Date");
...
function _encodeHeader(headerSpec, protectionAlgDer, pvno) {
  ...
  if (headerSpec.messageTime != null) {
    if (!(headerSpec.messageTime instanceof Date)) throw _err("cmp/bad-input", "header.messageTime must be a Date");
...
function _encodePkiStatusInfo(si) {
  if (typeof si.status !== "number") throw _err("cmp/bad-cert-status", "statusInfo.status must be a PKIStatus integer");
...
function _encodeCertStatus(cs, state) {
  ...
  var children = [
    ...
    b.integer(BigInt(cs.certReqId)),   // signed, -1 legal
  ];
  if (cs.statusInfo != null) children.push(_encodePkiStatusInfo(cs.statusInfo));
  if (cs.hashAlg != null) {
    if (typeof cs.hashAlg !== "string" || O(cs.hashAlg) == null) throw _err("cmp/bad-name", "unknown certConf hashAlg " + JSON.stringify(cs.hashAlg));
...
function _encodePollReqContent(list) {
  ...
  return b.sequence(list.map(function (pr) {
    if (!pr || typeof pr !== "object" || Buffer.isBuffer(pr) || typeof pr.certReqId !== "number") throw _err("cmp/bad-poll-req", "each pollReq entry must be { certReqId: <integer> }");
    return b.sequence([b.integer(BigInt(pr.certReqId))]);
  }));
}
...
function _encodeRevDetails(rd) {
  ...
  var certDetails = Buffer.isBuffer(rd.certDetails) || rd.certDetails instanceof Uint8Array
    ? b.raw(_b.reqDer(rd.certDetails, "RevDetails.certDetails (a pre-encoded CertTemplate DER)"))
    : b.raw(crmf.buildCertTemplate(rd.certDetails));
...
function _encodeBody(bodySpec, key, opts) {
  ...
  if (CRMF_BODY[arm]) {
    return crmf.build(bodySpec[arm], key, {}).then(function (crmfDer) {
...
function _encodePbmac1AlgId(mac) {
  var pbkdf2Params = [b.octetString(mac.salt), b.integer(BigInt(mac.iterationCount)), b.integer(BigInt(mac.keyLength))];
  // prf: omit iff the DEFAULT algid-hmacWithSHA1; else the prf AlgorithmIdentifier with ABSENT params.
  if (mac.prfName !== "hmacWithSHA1") pbkdf2Params.push(_algIdNoParams(mac.prfName));
  var pbkdf2AlgId = b.sequence([b.oid(O("pbkdf2")), b.sequence(pbkdf2Params)]);
  var pbmac1Params = b.sequence([pbkdf2AlgId, _algIdNoParams(mac.macName)]);
...
function _resolveProtection(opts) {
  ...
  if (typeof iterationCount !== "number" || !Number.isInteger(iterationCount) || iterationCount < 1) throw _err("cmp/bad-input", "opts.mac.iterationCount must be a positive integer");
...
function _collectExtraCerts(opts, protCertDer) {
  ...
  if (opts.extraCerts != null) {
    ...
    opts.extraCerts.forEach(function (c) { out.push(b.raw(_b.reqDer(c, "extraCerts entry (a Certificate DER)"))); });
  }
  return out;
}
