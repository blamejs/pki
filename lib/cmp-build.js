 ...
 *   registry algorithm), a PBMAC1 shared-secret MAC, or a KEM-based MAC keyed by a secret the peer
 *   encapsulated to the sender's ML-KEM key. The emitted message round-trips through
 ...
 * @spec RFC 9810
 * @card Build a CMP PKIMessage with signature, PBMAC1 or KEM-based protection.
 */
...
var KNOWN_OPTS_KEYS = intrinsic.assign(intrinsic.create(null), { key: 1, cert: 1, mac: 1, kem: 1, extraCerts: 1, pem: 1, pss: 1, digestAlgorithm: 1 });
...
var KNOWN_OPTS_KEM_KEYS = intrinsic.assign(intrinsic.create(null), { kem: 1, extraCerts: 1, pem: 1 });
...
function _encodeInfoValue(itav) {
  ...
  if (name === "confirmWaitTime") {
    guard.time.assertEncodable(itav.infoValue, _err, "cmp/bad-info-value", "confirmWaitTime infoValue");
...
/** @internal The header as this verb read it, with the transaction fields a receiver requires filled
 * when the caller omitted them. RFC 9483 sec. 3.5 has a receiver refuse a PKIMessage without a
 * transactionID or with a senderNonce under 128 bits, and RFC 9810 sec. 5.1.1 recommends 128 random
 * bits for each, so a message built without them carries fresh 16-byte values rather than being sent
 * to be refused; a senderNonce the caller states under 128 bits is refused here for the same reason. A
 * recipNonce is never filled, since it echoes the other side's value or is absent. The header's known
 * fields are read once into a record, so the protection (a KEM MAC binds to the transactionID) and
 * the encoding see one value. */
function _headerWithTransactionFields(headerSpec) {
  if (!headerSpec || typeof headerSpec !== "object" || _isBuffer(headerSpec)) throw _err("cmp/bad-input", "message.header must be an object");
  guard.identifier.assertKnownKeys(headerSpec, KNOWN_HEADER_KEYS, _err, "cmp/bad-input", "unknown header field ");
  var header = guard.identifier.snapshotOptions(headerSpec, KNOWN_HEADER_KEYS);
  if (header.transactionID == null) header.transactionID = _freshTransactionField();
  if (header.senderNonce == null) header.senderNonce = _freshTransactionField();
  else if (_b.reqDer(header.senderNonce, "header.senderNonce").length < TRANSACTION_FIELD_BYTES) {
    throw _err("cmp/bad-input", "header.senderNonce must carry at least 128 bits, since a receiver refuses a shorter one (RFC 9483 sec. 3.5)");
  }
  return header;
}

var TRANSACTION_FIELD_BYTES = 16;

/** @internal A signature-protected message names its signer, so a `sender` the caller omits is filled
 * with the signer certificate's subject (RFC 9483 sec. 3.1), the value a receiver holds it to, the
 * same way the transaction fields are filled. This is the fill alone: a `sender` the caller states,
 * whether or not it names the signer, is left as given. An empty-subject signer names itself by a
 * subjectAltName entry, which cannot be chosen for the caller, so an omitted sender there is refused.
 * The header record is this build's own (from `_headerWithTransactionFields`), so the field is set on
 * the object the encoder reads. */
function _fillSenderFromSigner(header, certDer) {
  if (header.sender != null) return;
  var subject;
  try { subject = x509.parse(certDer).subject; }
  catch (e) { throw _err("cmp/bad-input", "opts.cert is not a valid X.509 certificate", e); }
  if (!subject.rdns.length) {
    throw _err("cmp/bad-input", "message.header.sender is required: the signer certificate's subject is empty, so name one of its subjectAltName entries (RFC 9483 sec. 3.1)");
  }
  header.sender = { directoryName: subject.bytes };
}

/** @internal 128 fresh random bits, the width RFC 9810 sec. 5.1.1 recommends for a transactionID and
 * a senderNonce, drawn the way pki.cmp.session draws its nonces. */
function _freshTransactionField() {
  return intrinsic.bufferFrom(webcrypto.webcrypto.getRandomValues(new intrinsic.Uint8Array(TRANSACTION_FIELD_BYTES)));
}
...
function _encodeHeader(headerSpec, protectionAlgDer, pvno) {
  ...
  if (headerSpec.messageTime != null) {
    guard.time.assertEncodable(headerSpec.messageTime, _err, "cmp/bad-input", "header.messageTime");
...
function buildCrlStatusList(spec) {
  ...
  if (asked != null) {
    guard.time.assertEncodable(asked, _err, "cmp/bad-input", "crlUpdate.thisUpdate");
...
/** @internal RFC 9480 sec. 2.20: cmp2021 is needed when a message exchange uses EnvelopedData,
 * "for the request being sent or for the expected response". A request that omits the certTemplate
 * publicKey, or fills its subjectPublicKey with a zero-length BIT STRING, asks the authority to
 * generate the key pair, and the response then carries it in an EnvelopedData, so RFC 9483 sec. 4.1.6
 * requires the version in the header "already for the initial request message". */
function _templateRequestsCentralKeyGeneration(certTemplate) {
  if (!certTemplate) return true;
  var spki = certTemplate.publicKey;
  if (spki == null) return true;
  return !!spki.publicKey && spki.publicKey.bytes.length === 0;
}

/** @internal The enrollment arms, and the only ones a keyless certTemplate speaks for. RFC 9483
 * sec. 4.1.6 asks the authority to generate the key pair by omitting the public key from an
 * enrollment request; a krr names a key to recover and a ccr names one to cross-certify, so the same
 * omission there says nothing about who generates a key and does not decide the version. */
var CENTRAL_KEYGEN_ARMS = intrinsic.assign(intrinsic.create(null), { ir: 1, cr: 1, kur: 1 });

function _certReqNeedsCmp2021(crmfDer, arm) {
  var messages;
  ...
  for (var i = 0; i < messages.length; i++) {
    ...
    if (!popo && _hasOwn(CENTRAL_KEYGEN_ARMS, arm)
      && _templateRequestsCentralKeyGeneration(messages[i].certReq.certTemplate)) return true;
  }
  return false;
}
...
function _encodeBody(bodySpec, key, opts) {
  ...
  var keys = guard.identifier.optionNames(bodySpec);
  ...
  if (CRMF_BODY[arm]) {
    ...
    return crmf.build(reqSpec, popKey, {}).then(function (crmfDer) {
      return { bodyTLV: b.explicit(tag, b.raw(crmfDer)), usesCmp2021: _certReqNeedsCmp2021(crmfDer, arm) };
    });
  }
  var inner;
  if (arm === "p10cr") {
...
/** @internal A byte field of the KEM protection spec, taken through the toolkit's byte door so a
 * caller object cannot answer differently on a second read. */
function _kemBytes(v, what) {
  if (v == null) throw _err("cmp/bad-input", what + " is required for KEM-based protection");
  if (!guard.bytes.isByteSource(v)) throw _err("cmp/bad-input", what + " must be DER bytes");
  return guard.bytes.snapshotSource(v, CmpError, "cmp/bad-input", what);
}

var KEM_STATIC_STRING = "CMP-KEM";
var KNOWN_KEM_KEYS = intrinsic.assign(intrinsic.create(null),
  { key: 1, ciphertext: 1, kemAlgorithm: 1, kemContext: 1, transactionID: 1, len: 1 });
/** @internal The KEM algorithms a ciphertext can be decapsulated under, and the MAC the derived key
 * drives. RFC 9810 sec. 5.1.3.4 leaves both to the profile and points at RFC 9481; the shipped set is
 * the ML-KEM parameter sets this toolkit already establishes secrets with, under HMAC-SHA256. */
/** @internal Keyed by the DOTTED OID, not by a display name: `pki.oid.register` can re-point a name,
 * and an algorithm this client supports must not become unsupported because a caller renamed it. */
var KEM_DECAPSULATE = intrinsic.create(null);
KEM_DECAPSULATE[O("id-ml-kem-512")] = "ML-KEM-512";
KEM_DECAPSULATE[O("id-ml-kem-768")] = "ML-KEM-768";
KEM_DECAPSULATE[O("id-ml-kem-1024")] = "ML-KEM-1024";
var KEM_MAC_KEY_BYTES = 32;

/** @internal The context a KEM-derived protection key is bound to (RFC 9810 sec. 5.1.3.4). Written
 * ONCE and read by both the protecting and the verifying half, because two copies of one derivation
 * drift: the static string, the transaction identifier and any kemContext all feed the KDF, so a
 * single byte of disagreement yields a key the peer cannot reproduce. The CMP module is EXPLICIT TAGS,
 * so `[0]` wraps the OCTET STRING. */
function kemOtherInfo(transactionID, kemContext) {
  var kids = [b.sequence([b.utf8(KEM_STATIC_STRING)]), b.octetString(transactionID)];
  if (kemContext != null) guard.list.append(kids, b.explicit(0, b.octetString(kemContext)));
  return b.sequence(kids);
}

/** @internal The shared secret key: HKDF over the KEM shared secret, with the DER KemOtherInfo as
 * info and `len` as the output size the MAC algorithm needs. */
async function kemSharedKey(sharedSecret, len, transactionID, kemContext) {
  var info = kemOtherInfo(transactionID, kemContext);
  var ikm = await subtle.importKey("raw", sharedSecret, { name: "HKDF" }, false, ["deriveBits"]);
  var bits = await subtle.deriveBits(
    { name: "HKDF", hash: "SHA-256", salt: intrinsic.bufferAlloc(0), info: info }, ikm, len * 8);
  return intrinsic.bufferFrom(bits);
}

/** @internal The protecting half of RFC 9810 sec. 5.1.3.4: the holder of the KEM private key
 * decapsulates the ciphertext the peer sent, derives the shared secret key, and MACs the message with
 * it. Decapsulation failure terminates the operation with the failInfo the section names, rather than
 * falling through to another flavor. */
/** @internal The decapsulation key, from the shapes every other key option in this module accepts: a
 * WebCrypto CryptoKey is adopted as it is, which is what `pki.key.generate` hands back, and an encoded
 * key is decoded and imported. Any copy the decode makes is collected for the caller to wipe. */
async function _kemPrivateKey(rawKey, wcName, ownedKeyCopies) {
  if (rawKey && typeof rawKey === "object" && !guard.bytes.isByteSource(rawKey) &&
      typeof rawKey.type === "string" && rawKey.algorithm != null) {
    if (rawKey.type !== "private") {
      throw _err("cmp/bad-input", "opts.kem.key is a CryptoKey of type " + guard.text.showValue(rawKey.type) +
        "; decapsulation needs the private key (type \"private\")");
    }
    return webcrypto.adoptKey(rawKey, { name: wcName }, ["decapsulateBits"], _signE, "bad-input");
  }
  var keyDer = signScheme.normalizePkcs8(rawKey, "opts.kem.key", _signE, ownedKeyCopies);
  return subtle.importKey("pkcs8", keyDer, { name: wcName }, false, ["decapsulateBits"]);
}

/** @internal The KemBMParameter fields in the order sec. 5.1.3.4 gives them, with the optional
 * kemContext between the KDF and the length. */
function _kemBmParameterKids(len, kemContext) {
  var kids = [b.sequence([b.oid(oid.byName("hkdfWithSha256"))])];
  if (kemContext != null) guard.list.append(kids, b.explicit(0, b.octetString(kemContext)));
  guard.list.append(kids, b.integer(_bigInt(len)));
  guard.list.append(kids, b.sequence([b.oid(oid.byName("hmacWithSHA256"))]));
  return kids;
}

/** @internal The MAC the derived key drives, computed the same way on both halves. */
async function kemMac(sharedKey, protectedPartDer) {
  var k = await subtle.importKey("raw", sharedKey, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
  return intrinsic.bufferFrom(await subtle.sign({ name: "HMAC" }, k, protectedPartDer));
}

function _resolveKemProtection(kemSpec, message) {
  if (!kemSpec || typeof kemSpec !== "object" || _isBuffer(kemSpec)) {
    throw _err("cmp/bad-input", "opts.kem must be an object { key, ciphertext, kemAlgorithm, kemContext?, transactionID?, len? }");
  }
  guard.identifier.assertKnownKeys(kemSpec, KNOWN_KEM_KEYS, _err, "cmp/bad-input", "unknown opts.kem field ");
  if (kemSpec.key == null) throw _err("cmp/bad-input", "opts.kem.key is the KEM private key the ciphertext was encapsulated to");
  /** @internal Taken once, and decoded LATER: normalizing a PEM or a Uint8Array allocates a buffer
   * holding the private key, and anything that throws between here and the MAC would leave it behind.
   * The copy is therefore made inside the protected region below, where its cleanup runs on every
   * exit; a caller-owned Buffer is only viewed and is never wiped. */
  var rawKey = kemSpec.key;
  var ct = _kemBytes(kemSpec.ciphertext, "opts.kem.ciphertext (the KEM ciphertext the peer sent)");
  var algName = kemSpec.kemAlgorithm;
  if (typeof algName !== "string") throw _err("cmp/bad-input", "opts.kem.kemAlgorithm names the KEM the ciphertext was produced under");
  var algOid = oid.isDottedDecimal(algName) ? algName : oid.byName(algName);
  var wcName = KEM_DECAPSULATE[guard.text.keyOf(algOid)];
  if (!wcName) {
    throw _err("cmp/unsupported-algorithm",
      "unsupported KEM " + algName + " for KEM-based message protection (ML-KEM-512/768/1024)");
  }
  /** @internal The ASN.1 admits len 1, and a one-byte key is one an observer of a single message can
   * search exhaustively, so the floor is the one this module already applies to a MAC key rather than
   * the syntax's. */
  var len = kemSpec.len == null ? KEM_MAC_KEY_BYTES : kemSpec.len;
  if (typeof len !== "number" || !Number.isInteger(len) ||
      len < constants.LIMITS.CMP_KEM_MIN_KEY_BYTES || len > constants.LIMITS.CMP_KEM_MAX_KEY_BYTES) {
    throw _err("cmp/bad-input", "opts.kem.len is the derived key size in bytes, " +
      constants.LIMITS.CMP_KEM_MIN_KEY_BYTES + ".." + constants.LIMITS.CMP_KEM_MAX_KEY_BYTES +
      " (a shorter key would not carry the strength the MAC is meant to have)");
  }
  var kemContext = kemSpec.kemContext == null ? null
    : _kemBytes(kemSpec.kemContext, "opts.kem.kemContext");
  /** @internal "transactionID MUST be the value from the message containing the ciphertext", which is
   * this message's own id in the ordinary flow and the peer's when it sent the ciphertext first, so
   * the caller can name it. */
  var txid = kemSpec.transactionID != null
    ? _kemBytes(kemSpec.transactionID, "opts.kem.transactionID")
    /** @internal `_build` has already refused a spec with no header, so this reads the header it
     * verified rather than re-testing for one. */
    : _kemBytes(message.header.transactionID,
      "message.header.transactionID (a KEM-derived key is bound to it)");
  return {
    protectionAlgDer: b.sequence([b.oid(oid.byName("kemBasedMac")), b.sequence(_kemBmParameterKids(len, kemContext))]),
    certDer: null,
    computeBits: async function (protectedPartDer) {
      var ss = null, ssk = null, ownedKeyCopies = [];
      try {
        try {
          var priv = await _kemPrivateKey(rawKey, wcName, ownedKeyCopies);
          ss = intrinsic.bufferFrom(await subtle.decapsulateBits({ name: wcName }, priv, ct));
        } catch (e) {
          if (e instanceof CmpError) throw e;
          throw _err("cmp/bad-message-check",
            "the KEM ciphertext did not decapsulate under opts.kem.key, so no protection key can be " +
            "derived and the operation is terminated (RFC 9810 sec. 5.1.3.4 step 2)", e);
        }
        ssk = await kemSharedKey(ss, len, txid, kemContext);
        return b.bitString(await kemMac(ssk, protectedPartDer), 0);
      } finally {
        var owned = [];
        if (ss) guard.list.append(owned, ss);
        if (ssk) guard.list.append(owned, ssk);
        for (var ci = 0; ci < ownedKeyCopies.length; ci++) guard.list.append(owned, ownedKeyCopies[ci]);
        ownedKeyCopies.length = 0;
        if (owned.length) guard.secret.zeroizeAll(owned, CmpError, "cmp/bad-input", "the KEM shared secret");
      }
    },
  };
}

function _resolveProtection(opts, message) {
  var hasSig = opts.key != null || opts.cert != null;
  var hasMac = opts.mac != null;
  var hasKem = opts.kem != null;
  if ((hasSig ? 1 : 0) + (hasMac ? 1 : 0) + (hasKem ? 1 : 0) > 1) {
    throw _err("cmp/bad-input", "supply exactly one of { key, cert } (signature), { mac } (PBMAC1) or { kem } (KEM-based MAC), not more than one");
  }
  if (hasKem) return _resolveKemProtection(opts.kem, message);
  if (!hasSig && !hasMac) throw _err("cmp/bad-input", "a PKIMessage requires protection: supply { key, cert } for a signature, { mac } for PBMAC1, or { kem } for a KEM-based MAC");
  ...
  if (m.algorithm != null && m.algorithm !== "pbmac1") throw _err("cmp/unsupported-algorithm", "opts.mac.algorithm " + guard.text.showValue(m.algorithm) + " is not supported; MAC protection is computed under pbmac1 (RFC 9481 sec. 6.1.2), the one password-based MAC in the RFC 9481 profile");
  var secret = m.secret;
...
async function _build(message, opts) {
  ...
  var kemForm = opts.kem != null;
  var formCount = (sigForm ? 1 : 0) + (macForm ? 1 : 0) + (kemForm ? 1 : 0);
  guard.identifier.assertKnownKeys(opts,
    formCount !== 1 ? KNOWN_OPTS_KEYS
      : sigForm ? KNOWN_OPTS_SIG_KEYS : macForm ? KNOWN_OPTS_MAC_KEYS : KNOWN_OPTS_KEM_KEYS,
  ...
  message = { header: _headerWithTransactionFields(message.header), body: message.body };

  var prot = _resolveProtection(opts, message);
  if (prot.certDer != null) _fillSenderFromSigner(message.header, prot.certDer);
 ...
 * metadata (`transactionID`, `senderNonce`, `messageTime`, `freeText`, `generalInfo`, ...). A `transactionID`
 * or `senderNonce` the caller omits is filled with 16 fresh random bytes, the width RFC 9810 sec. 5.1.1
 * recommends and RFC 9483 sec. 3.5 has a receiver require, so every built message is receivable; a
 * `senderNonce` given under 128 bits is refused (`cmp/bad-input`), and a `recipNonce` is never filled. `body` is a
 ...
 * A signature-protected `header.sender` the caller omits is filled with the signer certificate's subject
 * (RFC 9483 sec. 3.1, the value a receiver holds it to), as the transaction fields are filled; an
 * empty-subject signer, which names itself by a subjectAltName entry, requires an explicit sender. The
 * message is protected: `opts` carries exactly
...
function _transfer(url, message, opts) {
  ...
  guard.identifier.optionNames(opts.headers).forEach(function (k) {
  ...
  return guard.async.invoked(transport, [{ method: "POST", url: parsedUrl.href, headers: headers, body: der, tls: tls, proxy: proxy, timeout: timeout, maxResponseBytes: maxResponseBytes }], _err, "cmp/bad-input", "opts.transport").then(function (res) {
    res = res || {};
    var h = guard.header.lowerCased(res.headers);
...
function wellKnownUrl(base, opts) {
  ...
  if (opts.label != null) {
    guard.list.append(segs, "p");
    guard.list.append(segs, _wellKnownSeg(opts.label, "label"));
  }
  if (opts.operation != null) guard.list.append(segs, _wellKnownSeg(opts.operation, "operation"));
  return u.origin + "/" + intrinsic.join(segs, "/");
}
...
module.exports = {
  ...
  /** @internal The KEM protection derivation, read by pki.cmp.verify so the two halves of one
   * key agreement cannot drift apart. */
