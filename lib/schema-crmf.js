...
// (0xA0) rather than the SEQUENCE tag (0x30) a verifier must hash. IMPLICIT tagging
// changes only the identifier octet (both tag numbers are < 31, a single octet)
// and preserves the length + content verbatim, so the SEQUENCE-tagged signed region
// is recovered by retagging that one octet — no re-serialization of length/content.
// `signedBytes` is that recovered region; `bytes` keeps the raw wire [0] TLV, so a
// verifier can use either encoding its peer produced.
function poposkSignedBytes(node) { return Buffer.concat([Buffer.from([0x30]), node.bytes.slice(1)]); }
...
var POPO_SIGNING_KEY = schema.seq([
  schema.optional("poposkInput", schema.decode(function (n, ctx) {
    // poposkInput [0] POPOSigningKeyInput is an IMPLICIT [0] on a SEQUENCE, so a
    // primitive [0] is malformed. Validate the constructed shell before deferring
    // the body RAW (its authInfo / PKMACValue semantics are the verifier's).
    if (!n.children) throw ctx.E("crmf/bad-popo", "poposkInput [0] POPOSigningKeyInput must be constructed (RFC 4211 §4.1)");
    return n;
  }), { tag: 0 }),
  ...
  build: function (m) {
    ...
    return {
      type: "signature",
      poposkInput: pin.present ? { bytes: pin.node.bytes, signedBytes: poposkSignedBytes(pin.node) } : null,
...
function popoPrivKey(type) {
  return schema.decode(function (n, ctx) {
    ...
    return { type: type, bytes: n.bytes };
  });
}
 ...
 * "keyAgreement", bytes }`. When present, `poposkInput` is `{ bytes, signedBytes }`
