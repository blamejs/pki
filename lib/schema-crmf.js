...
function popoPrivKey(type) {
  return schema.decode(function (n, ctx) {
    ...
    // The alternative's FORM: thisMessage [0] / subsequentMessage [1] / dhMAC [2] are
    // primitive (BIT STRING / INTEGER / BIT STRING); agreeMAC [3] PKMACValue and
    // encryptedKey [4] EnvelopedData are constructed (SEQUENCE). The inner VALUE
    // stays deferred raw (the EnvelopedData decode is a verify-layer concern), but a
    // wrong-form node is malformed and fails closed here.
