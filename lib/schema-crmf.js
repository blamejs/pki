...
var CERT_TEMPLATE = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    return {
      ...
      serialNumberHex: f.serialNumber.present ? f.serialNumber.node.content.toString("hex") : null,
...
var CERT_REQ_MSG = schema.seq([
  ...
  build: function (m) {
    return {
      certReq: m.fields.certReq.value.result,
      popo:    m.fields.popo.present ? m.fields.popo.value : null,
 ...
 * `certTemplate` carries the requested certificate fields (`version`,
 * `serialNumber`, `signingAlg`, `issuer`, `validity`, `subject`, `publicKey`,
 * `issuerUID`, `subjectUID`, `extensions` — each `null` when absent). `popo` is
