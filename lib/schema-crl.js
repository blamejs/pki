...
function decodeExt(ext, scope, ctx) {
  ...
  try {
    if (ext.oid === OID_CRL_NUMBER && atCrl) {
      value = asn1.read.integer(pkix.decodeNested(ext.value, ctx, "a cRLNumber extension value"));
      if (value < 0n) throw new Error("cRLNumber must be non-negative (INTEGER 0..MAX)");
    } else if (ext.oid === OID_REASON_CODE && atEntry) {
      var reason = asn1.read.enumerated(pkix.decodeNested(ext.value, ctx, "a reasonCode extension value"));
      ...
      var n = pkix.decodeNested(ext.value, ctx, "an invalidityDate extension value");
