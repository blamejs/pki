...
function decodeExt(ext, scope) {
  var value = ext.value;
  var atCrl = scope !== "entry";
  var atEntry = scope !== "crl";
  try {
    if (ext.oid === OID_CRL_NUMBER && atCrl) {
      value = asn1.read.integer(asn1.decode(ext.value));
      if (value < 0n) throw new Error("cRLNumber must be non-negative (INTEGER 0..MAX)");
    } else if (ext.oid === OID_REASON_CODE && atEntry) {
      ...
      value = Number(reason);
    } else if (ext.oid === OID_INVALIDITY_DATE && atEntry) {
  ...
  build: function (m) {
    return {
      ...
      crlEntryExtensions: m.fields.crlEntryExtensions.present
        ? m.fields.crlEntryExtensions.value.result.map(function (e) { return decodeExt(e, "entry"); })
        : [],
    };
  },
});
  ...
  build: function (m) {
    return {
      ...
      crlExtensions: m.fields.crlExtensions.present
        ? m.fields.crlExtensions.value.result.map(function (e) { return decodeExt(e, "crl"); })
        : [],
...
/** @internal The criticality RFC 5280 sec. 5.3 fixes for each CRL entry extension, keyed by dotted
 *  OID through the registry the caller passes: the CRL signer, the OCSP signer (sec. 4.4.5 carries
 *  the same extensions into a SingleResponse) and the linter all read this one table. */
var ENTRY_EXTENSION_CRITICALITY_ROWS = [
...
function entryExtensionCriticality(oidRegistry) {
  ...
  intrinsic.forEach(ENTRY_EXTENSION_CRITICALITY_ROWS, function (r) {
...
function readEntryExtension(ext) {
  if (ext.oid !== OID_CERTIFICATE_ISSUER) return decodeExt(ext);
  var names;
  try { names = schema.walk(CERTIFICATE_ISSUER_NAMES, asn1.decode(ext.value), NS).result; }
  catch (e) {
    throw NS.E("crl/bad-extension-value", "malformed certificateIssuer extension value: " + ((e && e.message) || intrinsic.String(e)), e);
  }
  return { oid: ext.oid, name: ext.name, critical: ext.critical, value: names };
}
