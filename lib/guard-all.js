...
// guard-intrinsic is deliberately NOT re-exported here. Re-exporting it gave taking the captures a
// second spelling, and the check that holds a module to the discipline could then be satisfied
// around: a module reached them as `guard.intrinsic.<x>` and never entered scope, so its remaining
// live reads were never named. With one spelling, a module's own require IS the fact, and the check
// reads it off the loaded module graph rather than guessing at the text.

// Frozen, like every module it gathers. A boundary reaches its guard as `guard.<family>.<fn>(...)`
// at the call, and the registry hands every module this same object, so one property assignment
// would replace a fail-closed check everywhere at once -- which is the substitution the family
// exists to refuse. Nothing writes to it after load.
