...
function rawToEcdsaDer(raw, coordLen) {
  if (!_isBuffer(raw) || typeof coordLen !== "number" || coordLen <= 0 || raw.length !== coordLen * 2) {
    throw new TypeError("rawToEcdsaDer: raw signature must be a Buffer of exactly 2*coordLen bytes");
  }
  var r = _BigInt("0x" + raw.subarray(0, coordLen).toString("hex"));
  var s = _BigInt("0x" + raw.subarray(coordLen).toString("hex"));
...
var CURVE_ORDER = {
  "P-256": _BigInt("0xFFFFFFFF00000000FFFFFFFFFFFFFFFFBCE6FAADA7179E84F3B9CAC2FC632551"),
  "P-384": _BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFC7634D81F4372DDF581A0DB248B0A77AECEC196ACCC52973"),
  "P-521": _BigInt("0x01FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFA51868783BF2F966B7FCC0148F709A5D03BB5C9B8899C47AEBB6FB71E91386409"),
};
...
function ecdsaDerToP1363(der, curve, E, code) {
  ...
  function pad(v) {
    ...
    var out = _bufferAlloc(width);
    buf.copy(out, width - buf.length);
    return out;
  }
  return _bufferConcat([pad(r), pad(s)]);
}
