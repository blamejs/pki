...
// set once, so the packed and AIK profiles cannot drift apart or lose a gate. The version
// check belongs to the profile, not to whichever verifier reaches it first.
//
...
// Does this extended-key-usage list assert the given purpose? The answer decides whether a
// certificate qualifies as the kind the rule names, so it goes through the membership guard
// instead of a prototype method. See guard-list.
...
function aikCert(cert, exts, E, code) {
  ...
  // The purpose test IS the rule that makes this an AIK certificate, so it is decided by comparison
  // rather than by a prototype method: a replaced `indexOf` reports the purpose present on any
  // certificate and an ordinary leaf is accepted as a TPM attestation key.
