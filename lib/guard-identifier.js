...
//
// Two sources, and the split is about what a CALLER placed there rather than how deep it sits:
//
//   - every OWN name, enumerable or not, because the caller put all of them on this object;
//   - every INHERITED ENUMERABLE name, which is how a value assigned to a prototype arrives.
//
// Inherited NON-enumerable members are left alone: `constructor`, and the methods class syntax
// defines, are machinery no caller means as an option. This repository already treats a class
// instance as a legitimate options bag -- guard.bytes.snapshotDeep copies one and keeps its
// prototype so its methods still resolve -- so a rule that refused non-plain objects, or that
// flagged everything up the chain, would contradict a decision already made and reject working
// calls. A `for...in` supplies the inherited enumerable half without reaching the machinery.
