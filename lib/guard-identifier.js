...
//
// assertPlainRecord(v, E, code, label): v MUST be a plain data record -- a non-null object whose own
// prototype is Object.prototype or null, and not an array, a Buffer, or an exotic (a Date, Map, Set,
// boxed primitive, or Proxy). A caller's `typeof v === "object"` gate admits an array or a Date, which
// then passes assertKnownKeys with no own key to reject and reads as a keyless record -- every field
// `undefined` and silently defaulted. This door refuses that shape, so a malformed container is a
// refusal rather than a request assembled from defaults. Composes BEFORE assertKnownKeys, which only
...
// tree-wide detector; the RED vectors (an array, a Date, and a Proxy at each routed entry point) are
// the guard.
function assertPlainRecord(v, E, code, label) {
  ...
  if (proto !== _objectProto && proto !== null) throw E(code, label + " must be a plain object (an exotic such as a Date or Map is refused)");
}
