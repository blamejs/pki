...
/** @internal Does this function's source text begin with the `class` keyword? Scanned by character
 * code so no regular expression is involved: `c`, `l`, `a`, `s`, `s`, then the one character that
 * can follow the keyword in a class expression, which is whitespace, the opening brace of a body
 * with no name, or the slash that opens a comment. */
function _isClassSource(src) {
  if (typeof src !== "string" || src.length < 5) return false;
  if (_charCodeAt(src, 0) !== 0x63 || _charCodeAt(src, 1) !== 0x6c || _charCodeAt(src, 2) !== 0x61 ||
      _charCodeAt(src, 3) !== 0x73 || _charCodeAt(src, 4) !== 0x73) return false;
  if (src.length === 5) return true;
  var c = _charCodeAt(src, 5);
  return c === 0x20 || c === 0x09 || c === 0x0a || c === 0x0d || c === 0x7b || c === 0x2f;
}
...
function assertCallable(value, E, code, label, shape) {
  ...
  /** @internal A class constructor answers "function" to typeof and throws when called without new,
   * so the fault would otherwise land on the first call rather than here. The descriptor of its
   * prototype cannot tell it apart, because freezing an ordinary function leaves that non-writable
   * too, so the test is the source text the captured Function.prototype.toString returns. That
   * ignores any toString the caller put on the value, and a class expression's source begins with
   * the keyword. The character after it is checked so a name that merely starts with those letters
   * is not read as the keyword. */
  if (_isClassSource(_fnToString(value))) {
    throw E(code, wanted + ", got a class constructor, which cannot be called without new");
  }
  return value;
}
