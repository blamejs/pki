...
// Everything this module reads from the runtime, taken at load. `util.types` above is the slot
// predicate set, which answers whether a value is a Proxy -- the question that decides whether an
// options bag can be asked honestly what it holds at all.
//
// The array test decides whether a value is walked as a list, so one answering `false` makes the
// index scan report nothing and every index rule keyed on that scan applies to nothing, which is
// how an accessor under index 0 reaches a copy loop the refusal should have turned the call away
// for. The reflective operations are this module's honesty walk itself: `getOwnPropertyDescriptor`
// learns whether a field is an accessor, `getPrototypeOf` and `ownKeys` walk a chain, and a
// replacement for either lets an options bag report one shape while answering as another. The array
// and collection operations run on lists and sets this module owns, where `Array.prototype` and
// `Set.prototype` are what a caller can still reach: a `forEach` replaced with a no-op makes the
// name scan report nothing over perfectly real keys, and a `has` answering `true` ends a chain walk
// at the first prototype. See guard-intrinsic for the whole captured set.
...
// This module's whole job is deciding whether a caller's name is in a known set, and that decision
// is one `hasOwnProperty` call. Reading it and its `call` off their prototypes leaves the answer
// with whoever replaced either, which classifies an unknown option as an allowed one.
var _hasOwn = intrinsic.hasOwn;
// The index test below IS `String(Number(k)) === k`, and the readable-index scan orders and bounds
// its results with the same two conversions. Both are writable properties of globalThis: one
// replacement decides which own names this module reports as elements, in what order, and where the
// scan stops -- which is what every consumer's density and accessor refusal is built on.
...
function assertCanonicalOid(str, E, code, label, boundsCode) {
  ...
  // The arc bound below is a bound on THESE parts, so the split that produces them settles it. One
  // returning different pieces has the check read a first and second arc the identifier does not
  // carry, and an out-of-range OID passes a range test performed on other numbers.
  var parts = _strSplit(str, ".");
  // Captured: these two arcs are what the bound below is a bound ON, and `BigInt` is a writable
  // property of globalThis, so a replacement returns whatever it likes for the arcs and the range
  // test is then performed on numbers the identifier does not carry.
...
// that enumeration nothing. A value on the prototype (`_create({ password: "pw" })`). A
// non-enumerable own value (`_defineProperty(o, "password", { enumerable: false })`). A
...
// established without asking a prototype who it is. `_create(Map.prototype)` holds no map
...
//
// A value with no collection slot answers with nothing. `_create(Map.prototype)` holds no
...
// `_create(Map.prototype)` keeps `size` while holding no map.
//
...
var _INTRINSIC_HOLDERS = (function () {
  ...
  //
  // Read the DESCRIPTOR, never the value through the runtime. `globalThis[n]` invokes a getter, and a
  // host may define one that warns on read -- Node exposes `localStorage` and `WASI` as lazy accessors
  // that emit an ExperimentalWarning the moment they are touched, so a plain read here printed that
  // warning on every import -- or one that throws. A typed-array constructor is a plain data property, so
  // taking `d.value` finds every concrete kind without ever invoking an accessor, which also means such
  // an accessor can neither warn nor fault this load over a name it does not need.
...
function _readableNames(obj) {
  ...
  // Boxed through the captured `Object`. This decides which names the scan below then examines, so
  // a replacement wraps a primitive as an object of its own choosing and the guard reads a name set
  // and a prototype chain the caller supplied rather than the value's.
  ...
  while (o && !_setHas(visited, o)) {
    ...
    //
    // The level has to be an inherited one. A caller's own `_create(null)` has no prototype
    ...
    _forEach(_ownKeys(o), function (k) {
      ...
      // members on. The second is what carries `_create(Map.prototype)`, which holds no map
...
// `_setPrototypeOf([,], {0: signer})` reads `[signer]` to a `for` loop, to `forEach`, and to
...
//
// A built-in exotic whose data lives in internal slots rather than own enumerable keys, recognized by
// identity so a cross-realm instance is caught too. `util.types` covers each such kind; an array is not
// among them and is checked separately by the caller.
...
// assertPlainRecord(v, E, code, label): v MUST be a plain data record -- a non-null object that is not an
// array, a Buffer, a Proxy, or a built-in exotic (a Date, Map, Set, boxed primitive, ...). A caller's
// `typeof v === "object"` gate admits an array or a Date, which then passes assertKnownKeys with no own
// key to reject and reads as a keyless record -- every field `undefined` and silently defaulted. This
// door refuses that shape, so a malformed container is a refusal rather than a request assembled from
// defaults. Composes BEFORE assertKnownKeys, which only
// judges the keys that are present, not whether the value is a record at all.
//
// The prototype is NOT consulted, so a record built by a caller's own class, or one from another realm
// (a vm context, a worker) whose prototype is a different Object.prototype, is accepted -- it holds its
// fields as own enumerable properties like any record. Only a built-in exotic that keeps its data in
// internal slots (a Date, Map, Set, boxed primitive, ...), an array (index-keyed), a Buffer, or a Proxy
// is refused, each recognized by identity through `util.types` / `Array.isArray` so a cross-realm
// instance is caught where an `instanceof` or a prototype check would miss it.
//
// @enforced-by behavioral -- a plain-record refusal has no single rename-proof code shape worth a
// tree-wide detector; the RED vectors (an array, a Date, and a Proxy at each routed entry point, and a
// cross-realm/subclass record that must still be ACCEPTED) are the guard.
function assertPlainRecord(v, E, code, label) {
  ...
  _refuseUnenumerable(v, E, code, label);   // a Proxy's reported keys need not match what it answers
