...
// The kinds whose own surface is structural rather than supplied. Kept beside the walk that uses
// it, and deliberately the same set guard-bytes names when it decides which names to copy.
function _isIndexedKind(v) {
  return Array.isArray(v) || ArrayBuffer.isView(v) ||
    v instanceof Map || v instanceof Set || v instanceof ArrayBuffer;
}
...
function _readableNames(obj) {
  ...
  // An array, a byte view, a Map, a Set or an ArrayBuffer reports only what the CALLER added to
  // it: its own names, less `length` and the indices. A verb reads an option by name whatever
  // the argument's type, and `opts = []; opts.pem = true` is an options bag as far as `opts.pem`
  // is concerned, so these kinds have to work. Their surface is not the caller's, though: the
  // prototypes are full of accessors describing the kind (`buffer`, `byteLength`, `size`,
  // `BYTES_PER_ELEMENT`, Node's `parent`), and `length` is intrinsic even on an empty array. A
  // walk of the whole chain reports all of it and the verb then refuses its own supported shape.
  //
  // This is the rule guard-bytes states for the same kinds when it decides which names to copy;
  // the two are one rule seen from the reading side and the copying side, and they have to agree
  // or a name is checked here and dropped there, or the reverse.
  if (o && _isIndexedKind(o)) {
    // `length` and the indices are intrinsic to an ARRAY or a byte view and to nothing else. A
    // Map, a Set and an ArrayBuffer have no own `length`, so skipping the name there would
    // discard one the caller had put on themselves.
    var indexed = Array.isArray(o) || ArrayBuffer.isView(o);
    Reflect.ownKeys(o).forEach(function (k) {
      if (indexed && typeof k === "string" &&
          (k === "length" || String(Number(k)) === k)) return;
      out.push(k);
    });
    return out;
  }
