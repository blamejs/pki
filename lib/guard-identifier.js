...
function _isArrayIndex(k) {
  var n = Number(k);
  return _isInteger(n) && n >= 0 && n < 4294967295 && String(n) === k;
}
...
function readableIndices(obj, E, code, label) {
  ...
  return _sort(out, function (a, b) { return Number(a) - Number(b); });
}
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    if (holder && (holder.get || holder.set)) {
      throw E(code, label + " supplies " +
        _stringify(typeof names[i] === "symbol" ? String(names[i]) : names[i]) +
    ...
    for (var i = 0; i < names.length; i++) {
      try { void opts[names[i]]; }
        ...
        throw E(code, label + ": reading " +
          _stringify(typeof names[i] === "symbol" ? String(names[i]) : names[i]) + " threw");
      }
    }
  }
