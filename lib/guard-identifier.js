...
var _INTRINSIC_HOLDERS = (function () {
  var m = new Map([[_TYPED_ARRAY_PROTO, _VIEW_MEMBERS]]);
  function pair(ctor, members) { if (ctor && ctor.prototype) m.set(ctor.prototype, members); }
  pair(Array, _ARRAY_MEMBERS);
  pair(RegExp, _REGEXP_MEMBERS);
  [Map, Set, WeakMap, WeakSet].forEach(function (c) { pair(c, _KEYED_MEMBERS); });
  pair(ArrayBuffer, _BUFFER_MEMBERS);
  pair(typeof SharedArrayBuffer === "function" ? SharedArrayBuffer : null, _BUFFER_MEMBERS);
  [DataView, Buffer].forEach(function (c) { pair(c, _VIEW_MEMBERS); });
  return m;
})();
...
function _holderMembers(o) {
  ...
  if (Object.getPrototypeOf(o) === _TYPED_ARRAY_PROTO) return _VIEW_MEMBERS;
  return null;
}

function _kindMembers(v) {
  var holder = _kindHolder(v);
  // No slot, so no kind of its own. Whatever a real intrinsic prototype in its chain supplies is
  // handled by the level test at the walk; a name that merely LOOKS like a member, on a class of
  // the caller's, is the option they wrote.
