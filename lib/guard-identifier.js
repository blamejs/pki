...
// The reflective operations this module's honesty walk is built out of, taken at load. They are
// the load-bearing ones here: `getOwnPropertyDescriptor` is how it learns whether a field is an
// accessor, `getPrototypeOf` and `ownKeys` are how it walks a chain, and a replacement for either
// lets an options bag report one shape while answering as another -- which is the single thing
// this module exists to detect. See guard-intrinsic for the whole captured set.
...
// The array operations, uncurried. They run on arrays this module owns (an `ownKeys` result, a
// local accumulator), so the receiver is never a caller's object; `Array.prototype` is what a
// caller can still reach. A `forEach` replaced with a no-op makes the name scan report nothing over
// perfectly real keys, and the accessor refusal keyed on what that scan found then passes
// vacuously -- the same fail-open shape as an index test answering `false`.
...
function _refuseUnenumerable(v, E, code, label) {
  ...
  var visited = new Set();
  for (var o = v; o && !visited.has(o); o = _getPrototypeOf(o)) {
    visited.add(o);
...
function _isMethodOf(from, k, fn) {
  var visited = new Set();
  for (var o = from; o && !visited.has(o); o = _getPrototypeOf(o)) {
    visited.add(o);
...
function _shadowsAnother(v, k, holder) {
  var visited = new Set();
  var past = false;
  for (var o = v; o !== null && o !== undefined && !visited.has(o); o = _getPrototypeOf(o)) {
    visited.add(o);
...
function readableIndices(obj, E, code, label) {
  ...
  var out = [];
  var visited = new Set();
  for (var o = obj; o && !visited.has(o); o = _getPrototypeOf(o)) {
    visited.add(o);
    _forEach(_ownKeys(o), function (k) {
