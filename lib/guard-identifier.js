...
/** @internal The option form: an option counts as supplied when the object carries the key at all,
 * so a merge that produced `{ transport: undefined }` is a value the caller set rather than an
 * option it left out. Only a key the object does not carry falls through to the boundary's default,
 * which is the one case where installing a real network client is what the caller asked for. */
...
function assertCallableOption(opts, key, E, code, label, shape) {
  if (opts == null || !_hasOwn(_Object(opts), key)) return undefined;
