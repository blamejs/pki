...
var _KIND_MEMBERS = (function () {
  var s = new Set(["length", "size", "buffer", "byteLength", "byteOffset", "BYTES_PER_ELEMENT",
    "maxByteLength", "resizable", "growable", "detached",
    // Node puts these two on Buffer.prototype beside the ECMAScript ones.
    "parent", "offset"]);
  s.add(Symbol.toStringTag);
  s.add(Symbol.unscopables);
  return s;
})();
...
function _readableNames(obj) {
  ...
  while (o && !visited.has(o)) {
    ...
    Reflect.ownKeys(o).forEach(function (k) {
      ...
      if (inherited && _KIND_MEMBERS.has(k) && _looksIntrinsic(d)) return;
...
function _settle(opts, E, code, label) {
  ...
  for (var i = 0; i < before.length; i++) {
    ...
    try { out[before[i]] = opts[before[i]]; }
    catch (_e) {
