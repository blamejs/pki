...
var _fnToString = intrinsic.uncurry(Function.prototype.toString);
...
function assertCanonicalOid(str, E, code, label, boundsCode, countCode) {
  var who = label || "OID";
  ...
  if (parts.length - 1 > constants.LIMITS.OID_MAX_SUBIDENTIFIERS) {
    throw E(countCode || bcode, who + " carries more than " + constants.LIMITS.OID_MAX_SUBIDENTIFIERS + " sub-identifiers");
  }
...
// @enforced-by behavioral -- reading a property twice has no rename-proof code shape; the RED
// @enforced-by behavioral -- vectors that drive an accessor-backed option are the guard
function snapshotOptions(obj, known) {
  var out = _create(null);
  if (obj === null || obj === undefined) return out;
  _forEach(_getOwnPropertyNames(known), function (k) { out[k] = obj[k]; });
  return out;
}

/** @internal The option names a bag carries, read the way assertKnownKeys read them: every own
 *  string key, whatever its descriptor says. Object.keys reports only the enumerable ones, so an
 *  option written with `enumerable: false` clears the door and then vanishes from the read.
 *  `known` narrows the result to the declared options. */
// @enforced-by option-bag-read-by-enumerable-key
function optionNames(obj, known) {
  if (obj === null || obj === undefined) return [];
  var out = [];
  _forEach(_ownKeys(_Object(obj)), function (k) {
    if (typeof k !== "string") return;
    if (known !== null && known !== undefined && !_hasOwn(known, k)) return;
    _append(out, k);
  });
  return out;
}

/** @internal A caller's options bag copied onto a prototype-less object, keyed by every own string
 *  property, not the enumerable ones alone. Each is read once, so an accessor-backed option
 *  answers the copy and not the later reads. Inherited properties are left behind: the copy is
 *  what the decisions read, and a name reaching it from a prototype is not one the caller set. */
// @enforced-by option-bag-read-by-enumerable-key
function ownOptions(obj) {
  var out = _create(null);
  if (obj === null || obj === undefined) return out;
  var o = _Object(obj);
...
function _settle(opts, E, code, label) {
  ...
  return _withoutPlantedFields(opts, before);
}

/** @internal A language prototype, as opposed to one the caller built. A defaults object the
 * caller put under their bag is theirs and sits below these in the chain. */
function _isLanguageProto(o) {
  return o === intrinsic.ObjectProto || o === intrinsic.ArrayProto ||
    o === intrinsic.BufferProto || o === intrinsic.FunctionProto;
}

function _holderOf(obj, k) {
  var visited = new _Set();
  for (var o = obj; o !== null && o !== undefined && !_setHas(visited, o); o = _getPrototypeOf(o)) {
    _setAdd(visited, o);
    if (_getOwnPropertyDescriptor(o, k)) return o;
  }
  return null;
}

/** @internal An option is something the caller supplied, and a name whose descriptor sits on a
 * language prototype was supplied by whoever wrote to that prototype. The verb reads its options
 * with `opts.name`, which walks the chain, so such a name answers as though it had been passed:
 * `historicalMode` treats a revocation as not yet in force and `softFail` waives an undetermined
 * one, each turning a revoked certificate into a valid path. The unknown-key check catches a
 * planted name that is not an option at all; one spelled like a real option passes it.
 *
 * What comes back reads `undefined` for those names and is otherwise the object that was passed:
 * the same prototype chain, the same own fields, and one own `undefined` masking each planted
 * name. So the caller's own field still wins, a defaults object under their bag still supplies its
 * options whatever their type, the reported set of names does not change, and a verb in a polluted
 * process still runs instead of failing on every call. With nothing planted the object is handed
 * back untouched, which is every call outside a process that has been written to.
 *
 * The own fields are carried as OWN fields rather than left to be inherited from the object that
 * was passed. A verb that forwards its options builds the forwarded set by copying own keys, and a
 * bag whose fields had all become inherited arrives at the inner call carrying none of them. */
function _withoutPlantedFields(obj, names) {
  var planted = null;
  for (var i = 0; i < names.length; i++) {
    if (!_isLanguageProto(_holderOf(obj, names[i]))) continue;
    if (planted === null) planted = [];
    _append(planted, names[i]);
  }
  if (planted === null) return obj;
  var shadow = _create(_getPrototypeOf(obj));
  var own = _ownKeys(obj);
  for (var k = 0; k < own.length; k++) {
    var d = _getOwnPropertyDescriptor(obj, own[k]);
    if (!d) continue;
    _defineProperty(shadow, own[k], d);
  }
  for (var j = 0; j < planted.length; j++) {
    if (_getOwnPropertyDescriptor(obj, planted[j])) continue;
    _defineProperty(shadow, planted[j], { value: undefined, writable: true, enumerable: false, configurable: true });
  }
  return shadow;
}

/** @internal An injectable callback option, refused unless it is callable. A value that is present
 * but not a function reaches the call site otherwise, where the operator gets a bare TypeError
 * naming no option; and one read as absent sends the caller to whatever default the boundary
 * installs, which for a transport is a real network client the caller was trying to replace. Take a
 * value only when it is known to be present; `assertCallableOption` decides presence.
 *
 * Callable is all this asks. A class constructor answers "function" to typeof and still throws when
 * called, but no test here separates one from an ordinary function: freezing a function leaves its
 * prototype non-writable exactly as a class's is, and reading the source text mistakes a method
 * named `class` for the keyword. The boundary that calls the value reports that throw as its own
 * typed error (`guard.async.invoked`), which covers a class however it is disguised, binding it
 * included, and rejects nothing a caller can legitimately pass. */
// @enforced-by behavioral -- the shape is a typeof test against a string literal, and the pattern
// @enforced-by behavioral -- scanner blanks every literal, so "function" cannot be told from
// @enforced-by behavioral -- "string" or "object"; a lexical rule would flag every type check in
// @enforced-by behavioral -- lib. The per-verb vectors across est, scep, acme, cmp and ct are the
// @enforced-by behavioral -- guard: each drives the shipped verb and asserts the typed refusal.
function assertCallable(value, E, code, label, shape) {
  var wanted = label + " must be a function" + (shape ? " " + shape : "");
  if (value === undefined) {
    throw E(code, wanted + ", got undefined");
  }
  if (typeof value !== "function") {
    throw E(code, wanted + ", got " + (value === null ? "null" : typeof value));
  }
  return value;
}

/** @internal The option form: an option counts as supplied when a name lookup finds the key, so a
 * merge that produced `{ transport: undefined }` is a value the caller set rather than an option it
 * left out. Presence is asked the way the boundary asks for the value, which is a plain lookup, so
 * an inherited option from a defaults bag is checked rather than skipped; testing own keys alone
 * would let an inherited null pass here and then be read as absent, selecting the default client.
 * Only a key no lookup finds falls through to that default. */
// @enforced-by behavioral -- the per-verb vectors across est, scep, acme, cmp, ct and path drive the
// @enforced-by behavioral -- shipped verb and assert the typed refusal for each unusable value
function assertCallableOption(opts, key, E, code, label, shape) {
  if (opts == null || !(key in _Object(opts))) return undefined;
  return assertCallable(opts[key], E, code, label, shape);
}

/** @internal Whether a caller's object says something about itself, as opposed to inheriting the
 * answer. A declaration is a promise the author of that object made, so it has to sit on the object
 * they wrote: reading the flag through the object finds a value on `Function.prototype` that every
 * function in the process carries, and a boundary reading it that way treats every caller as having
 * promised whatever was planted there. The value is read from the descriptor rather than through
 * the object, so an accessor further up the chain cannot answer either.
 *
 * @enforced-by guard-shape-reinlined
 * @guard-shape /\.\s*blocksPrivateAddresses\s*===\s*true/ */
function declaresOwn(obj, name) {
  if (obj === null || obj === undefined) return false;
  var d;
  try { d = _getOwnPropertyDescriptor(_Object(obj), name); }
  catch (_e) { return false; }
  return !!d && !d.get && !d.set && d.value === true;
}

module.exports = intrinsic.freeze({
  assertCallable: assertCallable,
  declaresOwn: declaresOwn,
  assertCallableOption: assertCallableOption,
