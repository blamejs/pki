...
//
// Asking for that tag reads `Symbol.toStringTag`, which a caller can back with a getter, and a
// getter can throw. This door's contract is a typed error, so the fault a question of its own
// provoked is typed here rather than let out raw. The three slot tests come first and ask the
// value nothing, and the caller's own value is refused as a Proxy before this runs at all, so the
// only reader left is a getter on an ordinary object.
function _notAnOptionsBag(v, E, code, label) {
  ...
  var tag;
  try { tag = Object.prototype.toString.call(v); }
  catch (e) { throw E(code, label + ": reading its type tag threw", e); }
  if (tag === "[object Arguments]") return "an arguments object";
  return null;
}
...
function optionsObject(opts, E, code, label) {
  ...
  // once the value has been refused as a Proxy, because each one runs a trap: `Buffer.isBuffer`
  // is an `instanceof`, which walks the prototype chain, so a `getPrototypeOf` trap that throws
  // took this door out through the caller's own exception rather than the typed one it promises.
  if (typeof opts !== "object") throw E(code, label + " must be an object");
  _refuseUnenumerable(opts, E, code, label);
  if (Buffer.isBuffer(opts)) throw E(code, label + " must be an object");
  var wrong = _notAnOptionsBag(opts, E, code, label);
