...
function _kindHolder(v) {
  ...
  for (var o = v; o !== null && o !== undefined && !visited.has(o); o = Object.getPrototypeOf(o)) {
    ...
    if (Array.isArray(o) || ArrayBuffer.isView(o) ||
        ...
        util.types.isWeakMap(o) || util.types.isWeakSet(o) ||
        util.types.isRegExp(o)) return o;
  }
  return null;
}
...
// A RegExp carries `lastIndex` on the instance and the rest on its prototype. Its own member is
// structure the same way an array's `length` is, so it is passed over wherever it is read from.
var _REGEXP_MEMBERS = ["lastIndex", "source", "flags", "global", "ignoreCase", "multiline",
  "dotAll", "unicode", "unicodeSets", "sticky", "hasIndices"];
var _ANY_KIND_MEMBERS = _ARRAY_MEMBERS
  .concat(_VIEW_MEMBERS).concat(_BUFFER_MEMBERS).concat(_KEYED_MEMBERS).concat(_REGEXP_MEMBERS);
...
function _kindMembers(v) {
  ...
  if (util.types.isRegExp(holder)) return _REGEXP_MEMBERS;
...
// a Map. Recognizing the descriptor the language installed tells that apart from a caller's, so
// the override is reported as the option it is.
//
// A value from another realm carries that realm's descriptors, which are different objects, so
// this set cannot answer for one. `_isForeignKind` below decides when to fall back to the name.
var _INTRINSIC_HOLDERS = (function () {
  var s = new Set();
  [Array, Map, Set, WeakMap, WeakSet, ArrayBuffer, DataView, Buffer,
...
// Does this value reach none of the prototypes above? Then its kind came from somewhere this
// realm cannot name a descriptor for, and the member NAME is all there is to go on.
function _isForeignKind(v) {
  var visited = new Set();
  for (var o = v; o !== null && o !== undefined && !visited.has(o); o = Object.getPrototypeOf(o)) {
    visited.add(o);
    if (_INTRINSIC_HOLDERS.has(o)) return false;
  }
  return true;
}
...
function _readableNames(obj) {
  ...
  // Where the kind came from decides how a member is recognized. In this realm the descriptor
  // the language installed is known, so a caller's override of one is told apart from it. From
  // another realm the descriptors are different objects and the name is what there is.
  var foreignKind = _isForeignKind(o);
  ...
  while (o && !visited.has(o)) {
    ...
    Reflect.ownKeys(o).forEach(function (k) {
      ...
      if (inherited && members.indexOf(k) !== -1 && _looksIntrinsic(d) &&
          (foreignKind || _INTRINSIC_HOLDERS.has(o))) return;
...
function _settle(opts, E, code, label) {
  ...
  for (var n = 0; n < before.length; n++) {
    var holder = _descriptorHolder(opts, before[n]);
    if (holder && (holder.get || holder.set)) {
      throw E(code, label + " supplies " +
        JSON.stringify(typeof before[n] === "symbol" ? String(before[n]) : before[n]) +
        " through an accessor, whose value can differ between the check and the read; pass an " +
        "object whose options are plain values");
    }
  }
  readAll(before);
