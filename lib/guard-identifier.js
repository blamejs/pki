...
// The prototypes whose members describe a KIND rather than anything a caller supplied: `buffer`,
// `byteLength`, `size`, `BYTES_PER_ELEMENT`, Node's `parent` and `offset`. A level of the chain
// that IS one of these contributes no option names; every other level does.
//
// Held by IDENTITY, which is what makes this safe where reading the runtime for prototype
// POLLUTION was not. Pollution changes what an object contains; it cannot change which object a
// prototype slot points at, so `o === Array.prototype` answers the same on a polluted runtime.
//
// The alternative was to stop reading the chain at all for these kinds, and that left a hole with
// no intrinsic surface anywhere in it: `Object.setPrototypeOf([], { password: "pw" })` is still an
// array, `opts.password` still resolves, and the chain holds one ordinary object. There was
// nothing to protect there and the option went unreported.
var _KIND_PROTOS = (function () {
  var s = new Set();
  [Object, Array, Map, Set, WeakMap, WeakSet, ArrayBuffer, DataView, Buffer,
  ...
  ].forEach(function (c) { if (c && c.prototype) s.add(c.prototype); });
  // The prototype every typed array shares, which carries most of their accessors and is not
  // reachable as a named global.
  s.add(Object.getPrototypeOf(Uint8Array.prototype));
  return s;
})();
...
// The members the language defines on a value of each kind, as ECMA-262 names them.
//
// The prototypes above hold the levels these live on, which covers a value whose realm is this
// one. A value built in a `vm` context or a worker inherits from that realm's prototypes, which
// are different objects, so the level is read and its members look like names a caller chose.
// Naming the members answers for a value from any realm, since the kind is read from the
// internal slot below and the names the language gives a kind do not vary between realms.
...
// Which of those a value holds, decided by its internal slot so no prototype can claim a kind it
// does not have and no realm can hide one it does.
function _intrinsicMembers(v) {
  if (v === null || v === undefined) return null;
  if (Array.isArray(v)) return _ARRAY_MEMBERS;
  if (ArrayBuffer.isView(v)) return _VIEW_MEMBERS;
  if (util.types.isArrayBuffer(v) || util.types.isSharedArrayBuffer(v)) return _BUFFER_MEMBERS;
  if (util.types.isMap(v) || util.types.isSet(v) ||
      util.types.isWeakMap(v) || util.types.isWeakSet(v)) return _KEYED_MEMBERS;
  return null;
}
...
function _readableNames(obj) {
  ...
  // The names the language itself puts on a value of this kind, for the levels above it. Locally
  // those levels are the intrinsic prototypes passed over whole further down, so this changes
  // nothing; a value built in another realm inherits from that realm's, which are not the objects
  // held below, and without this its `size` or `Symbol.unscopables` reads as a name its caller
  // chose. Inherited only, so an own `detached` set on a buffer stays the caller's option, which
  // pki.cms.sign is the verb that has one.
  var intrinsic = _intrinsicMembers(o);
  ...
  while (o && !visited.has(o)) {
    ...
    // A level that IS a kind's prototype describes the kind, so it contributes no option names.
    // Object.prototype is excluded from this skip because it has its own rule below, which is
    // what reports a planted name there. Skipping the level rather than declining to read the
    // chain is the difference that matters: a bag whose chain holds no kind prototype at all,
    // such as `Object.setPrototypeOf([], { password: "pw" })`, is still read.
    if (!atObjectProto && _KIND_PROTOS.has(o)) { o = above; continue; }
    ...
    Reflect.ownKeys(o).forEach(function (k) {
      ...
      // The kind's own members, on a level above the value. Own is deliberately excluded: an own
      // `detached` on a buffer shadows the intrinsic getter, so `opts.detached` answers with what
      // the caller wrote, and pki.cms.sign reads an option under exactly that name.
      if (inherited && intrinsic && intrinsic.indexOf(k) !== -1) return;
      var d = Object.getOwnPropertyDescriptor(o, k);
