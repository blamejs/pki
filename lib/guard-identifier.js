...
function _readableNames(obj) {
  ...
  // is an options bag as far as `opts.pem` is concerned, so these kinds have to work. What their
  // PROTOTYPES carry is not the caller's, and that is handled per level below rather than by
  // refusing to read the chain, which left a bag whose chain holds nothing intrinsic unread.
  ...
  while (o && !visited.has(o)) {
    ...
    Reflect.ownKeys(o).forEach(function (k) {
      if (seen[k]) return;
      // `length` and the indices of an array or a byte view are its structure, not names the
      // caller chose. Only on the value itself: an object further up the chain that happens to
      // carry `length` supplied it deliberately.
      if (!inherited && indexedSelf && typeof k === "string" &&
      ...
      // defines its methods on its OWN prototype; nothing legitimate puts one on the root.
      if (atObjectProto) {
