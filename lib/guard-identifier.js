...
// The value in this chain that holds a collection's internal slot, or null when none does.
//
// The chain, not the value alone, because a bag can sit over one: `Object.create(someArray)`
// inherits that array's indices and its `length`, and those are the array's structure wherever
// they are reached from. Reading the kind off the wrapper alone calls them names its caller
// chose, and the settled bag a verb receives is exactly such a wrapper.
function _kindHolder(v) {
  var visited = new Set();
  for (var o = v; o !== null && o !== undefined && !visited.has(o); o = Object.getPrototypeOf(o)) {
    visited.add(o);
    if (Array.isArray(o) || ArrayBuffer.isView(o) ||
        util.types.isArrayBuffer(o) || util.types.isSharedArrayBuffer(o) ||
        util.types.isMap(o) || util.types.isSet(o) ||
        util.types.isWeakMap(o) || util.types.isWeakSet(o)) return o;
  }
  return null;
}

// Is this value one whose `length` and indices are structure rather than supplied names?
function _isIndexedKind(v) {
  var holder = _kindHolder(v);
  return holder !== null && (Array.isArray(holder) || ArrayBuffer.isView(holder));
}
...
var _ARRAY_MEMBERS = ["length", Symbol.unscopables];
// Node puts `parent` and `offset` on Buffer.prototype beside the ECMAScript ones.
var _VIEW_MEMBERS = ["length", "buffer", "byteLength", "byteOffset", "BYTES_PER_ELEMENT",
  "parent", "offset", Symbol.toStringTag];
var _BUFFER_MEMBERS = ["byteLength", "maxByteLength", "resizable", "growable", "detached",
  Symbol.toStringTag];
var _KEYED_MEMBERS = ["size", Symbol.toStringTag];
var _ANY_KIND_MEMBERS = _ARRAY_MEMBERS
  .concat(_VIEW_MEMBERS).concat(_BUFFER_MEMBERS).concat(_KEYED_MEMBERS);
...
//
// A value with no collection slot gets the union instead. Such a value can still inherit a real
// kind prototype (`Object.create(Map.prototype)` holds no map and carries `size`), and those
// members are the language's wherever they are reached from.
function _kindMembers(v) {
  var holder = _kindHolder(v);
  if (holder === null) return _ANY_KIND_MEMBERS;
  if (Array.isArray(holder)) return _ARRAY_MEMBERS;
  if (ArrayBuffer.isView(holder)) return _VIEW_MEMBERS;
  if (util.types.isArrayBuffer(holder) || util.types.isSharedArrayBuffer(holder)) return _BUFFER_MEMBERS;
  return _KEYED_MEMBERS;
}
...
function _readableNames(obj) {
  ...
  while (o && !visited.has(o)) {
    ...
    Reflect.ownKeys(o).forEach(function (k) {
      ...
      if (inherited && members.indexOf(k) !== -1 && _looksIntrinsic(d)) return;
...
// The bag a verb goes on to read: the caller's option names, each read once, on an object with no
// prototype and nothing but its own data properties.
//
...
//
// Reading here settles that: the getters run once, before anything is checked, and what the check
// and the verb both see is this object, whose values cannot change again. A bag whose names still
// differ after that read is refused rather than re-read, since a bag that rewrites itself as it is
// read has no set of names that can be checked. Values are carried across as they are, so a nested
// object stays the caller's; guard.bytes.snapshotDeep is what copies one through.
function _settle(opts, E, code, label) {
  var before = _readableNames(opts);
  // Built over the caller's bag, not beside it. The walk passes over a method so a caller's class
  // keeps its own, and a documented option can hold a function: `pki.path.build` takes `transport`,
  // and a defaults-style bag like `Object.create({ transport: fn })` carries it as an inherited
  // function. Copying only the reported names onto a bare object drops it, and the verb then falls
  // back to the built-in HTTPS transport and reaches the network the caller meant to replace.
  // Keeping the original underneath leaves every such name resolving as it did.
  var out = Object.create(opts);
  for (var i = 0; i < before.length; i++) {
    ...
    try { read = opts[before[i]]; }
    catch (_e) {
      throw E(code, label + ": reading " +
        JSON.stringify(typeof before[i] === "symbol" ? String(before[i]) : before[i]) + " threw");
    }
    // Defined, never assigned: an option the caller exposes through a getter has no setter behind
    // it, and assigning through one throws instead of recording what was read.
    Object.defineProperty(out, before[i], { value: read, enumerable: true, configurable: false });
  }
  var after = _readableNames(opts);
  var same = after.length === before.length;
  for (var j = 0; same && j < before.length; j++) same = before.indexOf(after[j]) !== -1;
  if (!same) {
  ...
  // Frozen, so the set this returns is the set the unknown-option check reads and the set the
  // verb goes on to act on, by construction rather than by every caller in between happening not
  // to write to it. No verb does; the freeze is what keeps that from being a thing to re-check
  // each time one is added. The values are not frozen with it, since a nested object is still the
  // caller's and guard.bytes.snapshotDeep is what copies one through.
