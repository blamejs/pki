 ...
 *   (`macedBytes`), public-key mode the SignedData with its signers. RFC 7292
...
var PBKDF2_PARAMS = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    if (iterations < 1n) throw ctx.E("pkcs12/bad-mac-data", "PBKDF2 iterationCount must be positive");
    var prf = m.fields.prf.present ? m.fields.prf.value.result : null;
    return {
      salt: m.fields.salt.value,
      iterationCount: Number(iterations),
      keyLength: Number(m.fields.keyLength.value),
  ...
  build: function (m, ctx) {
    ...
    if (di.digestAlgorithm.oid === OID_PBMAC1) {
      ...
      pbmac1 = schema.embeddedDer(PBMAC1_PARAMS, di.digestAlgorithm.parameters, ctx,
        { code: "pkcs12/bad-mac-data", what: "PBMAC1-params" }).result;
    }
    return {
    ...
    if (m.fields.bagAttributes.present) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (a.type === OID_LOCAL_KEY_ID) {
          ...
          localKeyId = _octetContent(asn1.decode(a.values[0]), ctx, "a localKeyId value", "pkcs12/bad-local-key-id");
        }
        seen[a.type] = true;
      }
    }
...
var PFX = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    if (authSafe.contentType === OID_DATA) {
      // §4 integrity coherence: password integrity carries MacData.
      if (!macPresent) throw ctx.E("pkcs12/bad-integrity-mode", "an id-data authSafe (password integrity) requires macData (RFC 7292 §4)");
      ...
      return {
        version: version, integrityMode: "password", mac: mac, macedBytes: macedBytes,
...
function _buildBag(rec, state, ctx) {
  ...
  if (rec.bagId === OID_KEY_BAG) {
    bag.type = "keyBag";
    bag.key = pkcs8.parse(rec.valueNode.bytes);
    return bag;
  }
  if (rec.bagId === OID_SHROUDED_KEY_BAG) {
    bag.type = "pkcs8ShroudedKeyBag";
    bag.encrypted = pkcs8.parseEncrypted(rec.valueNode.bytes);
    return bag;
  }
 ...
 * bytes, and `macedBytes` is the exact byte range the HMAC covers) or `"public-key"`
 * (id-signedData authSafe — the CMS SignedData surfaced on `authSafeSigned`,
 * signature not verified here). Each of `safeBags` is
...
var parse = pkix.makeParser({
  ...
  prefix: "pkcs12", what: "PFX", topSchema: PFX, ns: NS, berFallback: true,
});
