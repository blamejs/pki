...
function walkSafeContents(der) {
  ...
  var state = { budget: { remaining: C.LIMITS.PKCS12_MAX_REDECODES }, bagDepth: 0 };
  var bags = [];
  for (var i = 0; i < walked.items.length; i++) _push(bags, _buildBag(walked.items[i].value.result, state, NS));
  return bags;
}
  ...
  for (var i = 0; i < asMatch.items.length; i++) {
    ...
    if (el.contentType === OID_DATA) {
      ...
      for (var j = 0; j < contents.items.length; j++) {
        _push(safeBags, _buildBag(contents.items[j].value.result, state, ctx));
      }
    } else if (el.contentType === OID_ENCRYPTED_DATA) {
      _push(encryptedSafes, { type: "encryptedData", content: _privacySafe(cms.walkEncryptedData(el.innerNode), ctx) });
      ...
      _push(encryptedSafes, { type: "envelopedData", content: _privacySafe(cms.walkEnvelopedData(el.innerNode), ctx), envelopedDataDer: el.innerNode.bytes });
    } else {
