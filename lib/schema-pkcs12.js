    ...
    if (m.fields.bagAttributes.present) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (a.type === OID_LOCAL_KEY_ID) {
          ...
          localKeyId = _octetContent(pkix.decodeNested(a.values[0], ctx, "a localKeyId value", { ber: true }), ctx,
            "a localKeyId value", "pkcs12/bad-local-key-id");
        }
        seen[a.type] = true;
      }
    }
...
var PFX = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    if (authSafe.contentType === OID_SIGNED_DATA) {
      ...
      var signed = cms.walkSignedData(authSafe.innerNode, ctx);
...
function _bmpValue(bytes, ctx) {
  var node = pkix.decodeNested(bytes, ctx, "a friendlyName value", { ber: true });
...
/** @internal Reads a decrypted SafeContents. `ctx` is the parse context of the door that produced
 *  the bytes when there is one, so its caps and budget reach the bags; the decrypt path in
 *  pki.pkcs12.open has none and takes the built-in ceilings. */
function walkSafeContents(der, ctx) {
  var ns = schema.withResources(NS, ctx);
  var walked = schema.walk(SAFE_CONTENTS, pkix.decodeNested(der, ctx, "a SafeContents", { ber: true }), ns);
  var state = { budget: { remaining: C.LIMITS.PKCS12_MAX_REDECODES }, bagDepth: 0 };
  var bags = [];
...
function _dispatchSafes(asMatch, state, ctx) {
  ...
  for (var i = 0; i < asMatch.items.length; i++) {
    ...
    if (el.contentType === OID_DATA) {
      ...
      encryptedSafes.push({ type: "encryptedData", content: _privacySafe(cms.walkEncryptedData(el.innerNode, ctx), ctx) });
    } else if (el.contentType === OID_ENVELOPED_DATA) {
      encryptedSafes.push({ type: "envelopedData", content: _privacySafe(cms.walkEnvelopedData(el.innerNode, ctx), ctx), envelopedDataDer: el.innerNode.bytes });
    } else {
...
function _buildBag(rec, state, ctx) {
  ...
  if (rec.bagId === OID_KEY_BAG) {
    bag.type = "keyBag";
    bag.key = pkcs8.walkPrivateKeyInfo(rec.valueNode, ctx);
    bag.keyDer = rec.valueNode.bytes;
    return bag;
  }
  if (rec.bagId === OID_SHROUDED_KEY_BAG) {
    bag.type = "pkcs8ShroudedKeyBag";
    bag.encrypted = pkcs8.walkEncryptedPrivateKeyInfo(rec.valueNode, ctx);
    return bag;
  }
 ...
 * @signature  pki.schema.pkcs12.parse(input, caps?) -> pfx
 * @since      0.1.18
 * @status     stable
 ...
 *   var der = await pki.pkcs12.build({ key: key, cert: cert }, { password: "changeit" });
 ...
 *   var cert = await pki.x509.sign({ subject: "Signer", subjectPublicKey: await pki.key.export(pair.publicKey),
 *     notBefore: new Date("2026-01-01T00:00:00Z"), notAfter: new Date("2036-01-01T00:00:00Z") }, { key: key });
 *   var p12 = await pki.pkcs12.build({ key: key, cert: cert }, { password: "changeit" });
 *   var pemText = pki.schema.pkcs12.pemEncode(p12);
