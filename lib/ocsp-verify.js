...
// Does this extended-key-usage list assert the given purpose? The answer IS an authorization
// decision, so it goes through the membership guard rather than a prototype method. See guard-list.
var _asserts = guard.list.contains;
// Captured at load. Each of these answers a binding question -- does this response concern the
// certificate being asked about, did this key sign it -- so a replacement made after this module
// loaded would decide the answer while every check around it still runs.
...
function makeOcspVerify(deps) {
  ...
  async function ocspCertIdMatches(certID, cert, issuerNameCandidates, issuerKeyBits) {
    ...
    var hashName = guard.intrinsic.hasOwn(OCSP_CERTID_HASHES, certID.hashAlgorithm.oid) ? OCSP_CERTID_HASHES[certID.hashAlgorithm.oid] : null;
  ...
  async function ocspAuthorizeResponder(basicResponse, cert, issuer, issuerKeyBits, time) {
    ...
    for (var i = 0; i < basicResponse.certs.length; i++) {
      ...
      // Instants, never the Date objects: comparing two Dates coerces each through `valueOf`,
      // which a caller's subclass answers, so the delegate would be checked at whatever moment
      // the caller's `time` chose to report rather than the one it holds.
      ...
      // The membership test IS the delegation rule, so it is decided by comparison rather than by a
      // prototype method: dispatched live, an `Array.prototype.indexOf` replaced after load reports
      // the purpose present on ANY certificate, and a cert an attacker put in the response's own
      // certs bag is accepted as the authorized responder.
