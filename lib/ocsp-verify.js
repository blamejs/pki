...
var OCSP_CERTID_HASHES = intrinsic.create(null);
...
function makeOcspVerify(deps) {
  ...
  async function ocspAuthorizeResponder(basicResponse, cert, issuer, issuerKeyBits, time) {
    ...
    for (var i = 0; i < basicResponse.certs.length; i++) {
      ...
      /** @internal RFC 6960 sec. 4.2.2.2.1 gives this extension a value that SHALL be NULL, and its
       *  presence is what lets a client stop asking whether the responder itself is revoked. A
       *  malformed value is not that marker, so this decodes the extension, as the key usage and
       *  extended key usage checks above do. */
      var noCheck;
      try { noCheck = decodeExt(rc, OID_OCSP_NOCHECK); }
      catch (_e) { continue; }
      if (!noCheck) continue;
