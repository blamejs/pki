...
function _isArc(a) {
  ...
  return typeof a === "number" && _isSafeInteger(a) && a >= 0;
}
...
_keys(FAMILIES).forEach(function (fam) {
...
function _assertEncodableArcs(arcs, who) {
  ...
  var root = typeof arcs[0] === "bigint" ? arcs[0] : _BigInt(arcs[0]);
  var second = typeof arcs[1] === "bigint" ? arcs[1] : _BigInt(arcs[1]);
...
function registerFamily(base, members) {
  if (!_isArray(base) || base.length < 1 || !base.every(_isArc)) {
  ...
  _keys(members).forEach(function (nm) {
    ...
    if (!arcs.every(_isArc)) {
      throw new OidError("oid/bad-arc", "registerFamily: member " + _stringify(nm) + " has a non-arc leaf");
    }
...
function toArcs(dotted) {
  ...
  return dotted.split(".").map(function (p) {
    var b = _BigInt(p);
    return b <= 9007199254740991n ? _Number(b) : b;
  });
}

function fromArcs(arcs) {
  if (!_isArray(arcs) || arcs.length < 2) throw new OidError("oid/bad-input", "fromArcs: expected an array of >= 2 arcs");
  return arcs.map(function (a) {
    if (typeof a === "bigint") {
      if (a < 0n) throw new OidError("oid/bad-arc", "fromArcs: arc " + _String(a) + " is not a non-negative integer");
      return a.toString();
    }
    if (typeof a === "number" && _isSafeInteger(a) && a >= 0) return _String(a);
    throw new OidError("oid/bad-arc", "fromArcs: arc " + _String(a) + " is not a non-negative integer");
  }).join(".");
}
...
[
  ...
  if (typeof d !== "string") throw new OidError("oid/unknown-name", "paramsMustBeAbsent seed: " + _stringify(nm) + " is not a registered name");
...
[["id-ml-kem-512", 800, 1632, 768, 32],
  ...
  var row = _freeze({ ek: r[1], dk: r[2], ct: r[3], ss: r[4] });
