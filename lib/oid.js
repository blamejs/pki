...
var FAMILIES = {
  ...
  pkixAlg: { base: [1, 3, 6, 1, 5, 5, 7, 6], of: {
    ...
    // Composite ML-KEM key-establishment algorithms (draft-ietf-lamps-pq-composite-kem
    // sec. 6): a PQ ML-KEM paired with a traditional RSA-OAEP / ECDH / X25519 / X448 so the
    // established secret stays safe if EITHER primitive is later broken.
  ...
  // Brainpool named curves (RFC 5639).
  ...
  // SCEP (RFC 8894) transaction attributes on the VeriSign arc 2.16.840.1.113733.1.9
  // (id-attributes). scep-prefixed because the CMC arc already registers senderNonce /
  // transactionId under different OIDs (RFC 8894 sec. 3.2.1, Tables 1-2).
  ...
  // id-scep on the PKIX arc 1.3.6.1.5.5.7.24; failInfoText is the only leaf SCEP uses
  // (RFC 8894 sec. 3.2.1.4).
...
// Does `s` look like a dotted-decimal OID -- one or more digit groups joined by single dots (at least two
// groups, no leading/trailing/doubled dot)? Distinguishes a numeric OID from a symbolic name so a caller may
// pass either. Decided by character scan (no regex); the arc-value bounds are the encoder's authority.
function isDottedDecimal(s) {
  ...
  for (i = 0; i < s.length; i += 1) {
    ...
    else if (c >= 48 && c <= 57) { atomEmpty = false; }                               // 0-9
    else return false;
  }
...
[
  ...
  // Composite ML-KEM (draft-ietf-lamps-pq-composite-kem sec. 5.3: the parameters
  // field MUST be absent for every composite KEM AlgorithmIdentifier).
