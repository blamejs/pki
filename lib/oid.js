...
var FAMILIES = {
  ...
  // OIW Secsig arc (1.3.14.3.2). id-SHA1 (.26) plus every legacy RSA signature OID standardized under
  // the arc: md5WithRSA (.3), rsaSignature (.11, the ISO 9796 RSA scheme), shaWithRSAEncryption (.15),
  // and the md2/md5/sha1-WithRSASignature set (.24/.25/.29) -- pre-PKCS#1 RSA algorithms still seen in
  // old certs. Registered so a consumer can name them -- e.g. the CMP keySpec RSA exclusion, which
  // recognizes RSA wherever it is standardized. The arc is shared with DES, DSA, and the hashes, so no
  // prefix isolates RSA; each RSA member is named individually. Points the standard tables leave
  // unassigned name no known algorithm and stay unlisted.
  oiwSecsig: { base: [1, 3, 14, 3, 2], of: {
    sha1: 26,
    "md5WithRSA": 3, "rsaSignature": 11, "shaWithRSAEncryption": 15,
