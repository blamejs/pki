...
var FAMILIES = {
  ...
  // OIW Secsig arc (1.3.14.3.2). id-SHA1 (.26) plus the legacy RSA-with-hash signature OIDs
  // (md2/md5/sha1 WithRSASignature): pre-PKCS#1 RSA signature algorithms still seen in old certs.
  // Registered so a consumer can name them -- e.g. the CMP keySpec RSA exclusion, which recognizes
  // RSA wherever it is standardized. The arc is shared with DSA and the hash, so no prefix isolates RSA.
