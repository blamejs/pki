...
var FAMILIES = Object.assign(Object.create(null), {
  attributeType: { base: [2, 5, 4], of: {
    ...
    organizationName: 10, organizationalUnitName: 11, title: 12,
    businessCategory: 15, postalCode: 17, name: 41, givenName: 42,
    initials: 43, generationQualifier: 44, distinguishedNameQualifier: 46,
    clearance: 55, pseudonym: 65, role: 72, organizationIdentifier: 97 } },

  cabfJurisdiction: { base: [1, 3, 6, 1, 4, 1, 311, 60, 2, 1], of: {
    jurisdictionLocalityName: 1, jurisdictionStateOrProvinceName: 2, jurisdictionCountryName: 3 } },
  ...
  /** @internal The semantics identifiers of ETSI EN 319 412-1 sec. 5.1.2, which a
   *  SemanticsInformation names to say how a subject serialNumber or organizationIdentifier is to
   *  be read. */
  etsiQcsSemantics: { base: [0, 4, 0, 194121, 1], of: {
    "semanticsId-Natural": 1, "semanticsId-Legal": 2,
    "semanticsId-eIDASNatural": 3, "semanticsId-eIDASLegal": 4 } },
  ...
  pkixOn: { base: [1, 3, 6, 1, 5, 5, 7, 8], of: { hardwareModuleName: 4, dnsSRV: 7, smtpUtf8Mailbox: 9, macAddress: 12 } },

  /** @internal The S/MIME BR reserved policy identifiers of sec. 7.1.6.1 are a 4x3 matrix of
   *  certificate type by generation, and a subscriber certificate asserts exactly one. Reading it
   *  is how a linter learns which generation's table to hold the certificate to, so all twelve are
   *  named rather than only the current ones. */
  cabfPolicy: { base: [2, 23, 140, 1], of: {
    "ev-guidelines": 1, "domain-validated": [2, 1], "organization-validated": [2, 2], "individual-validated": [2, 3],
    "smime-mailbox-legacy": [5, 1, 1], "smime-mailbox-multipurpose": [5, 1, 2], "smime-mailbox-strict": [5, 1, 3],
    "smime-organization-legacy": [5, 2, 1], "smime-organization-multipurpose": [5, 2, 2], "smime-organization-strict": [5, 2, 3],
    "smime-sponsor-legacy": [5, 3, 1], "smime-sponsor-multipurpose": [5, 3, 2], "smime-sponsor-strict": [5, 3, 3],
    "smime-individual-legacy": [5, 4, 1], "smime-individual-multipurpose": [5, 4, 2], "smime-individual-strict": [5, 4, 3],
    "code-signing-ev": 3, "code-signing": [4, 1], "code-signing-timestamping": [4, 2] } },

  idCp: { base: [1, 3, 6, 1, 5, 5, 7, 14], of: { "id-cp-ipAddr-asNumber": 2, "id-cp-ipAddr-asNumber-v2": 3 } },

  /** @internal The Legal Entity Identifier extensions of S/MIME BR sec. 7.1.2.3 (l), which the
   *  certificate type decides the admissibility of. */
  lei: { base: [1, 3, 6, 1, 4, 1, 52266], of: { "lei-identifier": 1, "lei-role": 2 } },

  /** @internal The Adobe extensions of S/MIME BR sec. 7.1.2.3 (m), prohibited under the Strict
   *  generation. */
  adobe: { base: [1, 2, 840, 113583, 1, 1, 9], of: { "adobe-timestamp": 1, "adobe-archive-rev-info": 2 } },

  /** @internal The extended key purposes Code Signing BR sec. 7.1.2.3 (f) names as MAY beside
   *  id-kp-codeSigning, so a row reporting any other purpose can exclude them by name. Document
   *  Signing is registered twice: Microsoft assigns it 1.3.6.1.4.1.311.10.3.12, the sibling of the
   *  Lifetime Signing arc printed two lines above it in that clause, and the clause itself prints
   *  1.3.6.1.4.1.311.3.10.3.12. Both resolve, so a certificate carrying either is read as the
   *  purpose the clause permits. */
  msEku: { base: [1, 3, 6, 1, 4, 1, 311], of: {
    "ms-lifetime-signing": [10, 3, 13],
    "ms-document-signing": [10, 3, 12],
    "ms-document-signing-as-printed": [3, 10, 3, 12] } },
  ...
  pkixKp: { base: [1, 3, 6, 1, 5, 5, 7, 3], of: {
    ...
    secureShellClient: 21, secureShellServer: 22, cmcCA: 27, cmcRA: 28, cmcArchive: 29, cmKGA: 32,
    brandIndicatorforMessageIdentification: 31, bundleSecurity: 35 } },
  ...
  /** @internal md5WithRSAEncryption and sha1WithRSAEncryption are registered so the toolkit can
   *  NAME them, not so it can use them. Nothing signs or verifies from this registry: the signing
   *  and verifying paths carry their own name-keyed tables, and neither has a row for these. An
   *  unregistered OID reads back as an undefined name, which left a linter unable to say what a
   *  request from another implementation was signed under. */
  rsa: { base: [1, 2, 840, 113549, 1, 1], of: {
    /** @internal PKCS#1's MD4 signature carries the arc-qualified name because the OIW arc already
     *  holds the plain `md4WithRSAEncryption` for 1.3.14.3.2.4. The registry is two-way and a name
     *  registered twice repoints every lookup of it, so two standards using one name for two OIDs
     *  are told apart here rather than silently sharing an entry. */
    rsaEncryption: 1, md2WithRSAEncryption: 2, "md4WithRSAEncryption-pkcs1": 3,
    md5WithRSAEncryption: 4, sha1WithRSAEncryption: 5,
    rsaesOaep: 7, mgf1: 8, pSpecified: 9, rsassaPss: 10,
  ...
  keyPackageContentTypes: { base: [2, 16, 840, 1, 101, 2, 1, 2, 78], of: { aKeyPackage: 5 } },
  ...
  /** @internal ANSI X9.42 (RFC 3279 sec. 2.3.3): the SubjectPublicKeyInfo algorithm an X.509
   * Diffie-Hellman certificate names, whose parameters are DomainParameters rather than the PKCS#3
   * DHParameter that pkcs3 below names. */
  ansiX942: { base: [1, 2, 840, 10046, 2], of: { dhpublicnumber: 1 } },

  pkcs3: { base: [1, 2, 840, 113549, 1, 3], of: { dhKeyAgreement: 1 } },
  ...
  smimeCt: { base: [1, 2, 840, 113549, 1, 9, 16, 1], of: { authData: 2, tSTInfo: 4, compressedData: 9, encKeyWithID: 21, authEnvelopedData: 23, trustAnchorList: 34 } },
  ...
  pkixAlg: { base: [1, 3, 6, 1, 5, 5, 7, 6], of: {
    ...
    "id-dhPop-static-HMAC-SHA1": 3, "id-alg-dhPOP": 4,
  ...
  ansiX962: { base: [1, 2, 840, 10045], of: {
    ecPublicKey: [2, 1], prime192v1: [3, 1, 1], prime256v1: [3, 1, 7],
    ecdsaWithSHA1: [4, 1],
    ecdsaWithSHA256: [4, 3, 2], ecdsaWithSHA384: [4, 3, 3], ecdsaWithSHA512: [4, 3, 4] } },

  /** @internal X9.57 DSA, registered for naming alone. The toolkit neither signs nor verifies DSA,
   *  and dsaWithSha1 is here so a request or certificate carrying it reads back with a name rather
   *  than a bare dotted string, which is what a linter needs to say what it refused. */
  x957: { base: [1, 2, 840, 10040, 4], of: { dsa: 1, dsaWithSha1: 3 } },

  secg: { base: [1, 3, 132, 0], of: {
    sect163k1: 1, sect163r1: 2, secp256k1: 10, sect163r2: 15, sect283k1: 16, sect283r1: 17,
    sect233k1: 26, sect233r1: 27, secp192k1: 31, secp224k1: 32, secp224r1: 33,
    secp384r1: 34, secp521r1: 35, sect409k1: 36, sect409r1: 37, sect571k1: 38, sect571r1: 39 } },

  brainpool: { base: [1, 3, 36, 3, 3, 2, 8, 1, 1], of: {
    brainpoolP160r1: 1, brainpoolP160t1: 2, brainpoolP192r1: 3, brainpoolP192t1: 4,
    brainpoolP224r1: 5, brainpoolP224t1: 6, brainpoolP256r1: 7, brainpoolP256t1: 8,
    brainpoolP320r1: 9, brainpoolP320t1: 10, brainpoolP384r1: 11, brainpoolP384t1: 12,
    brainpoolP512r1: 13, brainpoolP512t1: 14 } },
  ...
  nistAes: { base: [2, 16, 840, 1, 101, 3, 4, 1], of: {
    "aes128-CBC": 2, "aes128-wrap": 5, "aes128-GCM": 6, "aes128-CCM": 7, "aes128-GMAC": 9,
    "aes192-CBC": 22, "aes192-wrap": 25, "aes192-GCM": 26, "aes192-CCM": 27, "aes192-GMAC": 29,
    "aes256-CBC": 42, "aes256-wrap": 45, "aes256-GCM": 46, "aes256-CCM": 47, "aes256-GMAC": 49 } },

  nistHash: { base: [2, 16, 840, 1, 101, 3, 4, 2], of: {
    sha256: 1, sha384: 2, sha512: 3, "sha3-256": 8, "sha3-512": 10, shake128: 11, shake256: 12,
    kmacWithSHAKE128: 19, kmacWithSHAKE256: 20 } },
  ...
  datatype: { base: [0, 9, 2342, 19200300, 100, 1], of: { userId: 1, domainComponent: 25 } },
...
[
  ...
  /** @internal RFC 3279 sec. 2.2.3: "When the ecdsa-with-SHA1 algorithm identifier appears as the
   * algorithm field in an AlgorithmIdentifier, the encoding MUST omit the parameters field", and
   * sec. 2.2.2 says SHALL for id-dsa-with-sha1. RFC 5758 sec. 3.2 repeats it for the SHA-2 set: "the
   * encoding MUST omit the parameters field". Unlike the `*WithRSAEncryption` identifiers, which
   * RFC 4055 requires to CARRY a NULL, these omit the field entirely. */
  "ecdsaWithSHA1", "ecdsaWithSHA256", "ecdsaWithSHA384", "ecdsaWithSHA512", "dsaWithSha1",
  /** @internal RFC 8702 sec. 2: "when using the id-shake128 or id-shake256 algorithm identifiers, the
   * parameters MUST be absent", sec. 3.1 for the CMS digest fields, and sec. 3.2 for the two
   * RSASSA-PSS-SHAKE signature identifiers. */
