...
var ALG_BY_NAME = {};
_keys(ALG_BY_NUMBER).forEach(function (n) { ALG_BY_NAME[ALG_BY_NUMBER[n]] = _Number(n); });
...
function _resolveCap(opts) {
  ...
  if (opts.maxOutputBytes !== undefined) {
    ...
    if (typeof mo !== "number" || !_isFinite(mo) || mo <= 0 || _floor(mo) !== mo) {
...
function _resolveAllowed(opts) {
  ...
  if (!_isArray(list)) throw _err("tls/bad-input", "opts.allowedAlgorithms must be an array of algorithm names or numbers");
  return list.map(function (a) {
    var name = (typeof a === "number") ? ALG_BY_NUMBER[a] : a;
    if (typeof name !== "string" || SUPPORTED.indexOf(name) === -1) {
      throw _err("tls/bad-input", "opts.allowedAlgorithms names an algorithm this toolkit cannot decompress: " + _stringify(a));
    }
    return name;
  });
}
...
function decompressCertificate(bytes, opts) {
  ...
  var message = guard.compress.bounded(name, _bufferFrom(body), uncompressedLength, _err,
...
function compressCertificate(certificateMessage, opts) {
  ...
  if (typeof name !== "string" || SUPPORTED.indexOf(name) === -1) {
    throw _err("tls/unsupported-algorithm", "opts.algorithm names an algorithm this toolkit cannot compress: " + _stringify(sel));
  }
  ...
  if (opts.level !== undefined && (typeof opts.level !== "number" || !_isFinite(opts.level) || _floor(opts.level) !== opts.level)) {
