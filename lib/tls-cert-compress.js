 ...
 *   decompressor must fault on a frame it could not finish. At load each is
 *   handed a frame with its last two bytes cut, and one that returns anything at
 *   all rather than faulting is dropped, because a runtime that accepts a cut
 *   frame lets a peer strip a frame's tail and have the receiver read a prefix as
 *   the whole message. A dropped algorithm is then neither compressed to nor
 *   accepted. At the Node 24.21 engine floor all three qualify; on a runtime
 *   where one does not, it is dropped.
 *
 * @card
...
var ALG_BY_NUMBER = intrinsic.assign(intrinsic.create(null), { 1: "zlib", 2: "brotli", 3: "zstd" });
var ALG_BY_NAME = intrinsic.create(null);
Object.keys(ALG_BY_NUMBER).forEach(function (n) { ALG_BY_NAME[ALG_BY_NUMBER[n]] = Number(n); });

var COMPRESS = intrinsic.assign(intrinsic.create(null), { zlib: zlib.deflateSync, brotli: zlib.brotliCompressSync, zstd: zlib.zstdCompressSync });

var LEVEL_OPT = intrinsic.assign(intrinsic.create(null), {
...
var FRAMING_CODES = intrinsic.assign(intrinsic.create(null), { truncated: "tls/truncated", framing: "tls/bad-framing", trailing: "tls/trailing-data" });
