...
// tsa [0] EXPLICIT GeneralName — GeneralName is a CHOICE of context tags [0]..[8]
// (RFC 5280 §4.2.1.6). Surfaced RAW (the full GeneralName form/content decode is the
// planned shared pkix.generalName primitive — the re-open condition); validate at
// least that the outer tag is a real GeneralName alternative so a non-GeneralName
// inner (e.g. a bare INTEGER) fails closed rather than passing as a valid tsa.
var GENERAL_NAME_RAW = schema.decode(function (n, ctx) {
  if (n.tagClass !== "context" || n.tagNumber < 0 || n.tagNumber > 8) {
    throw ctx.E("tsp/bad-tsa", "tsa must be a GeneralName (context tag [0]..[8]) (RFC 3161 §2.4.2)");
  }
...
var ACCURACY = schema.seq([
  ...
  build: function (m) {
    ...
    return {
      seconds: m.fields.seconds.present ? Number(m.fields.seconds.value) : 0,
...
var TST_INFO = schema.seq([
  ...
  schema.trailing([
    ...
    { tag: 1, name: "extensions", schema: schema.implicitSeqOf(1, EXTENSION, { code: "tsp/bad-extensions", what: "extensions" }) },
...
var PKI_STATUS_INFO = schema.seq([
  ...
  schema.optional("statusString", schema.seqOf(UTF8_TEXT, { assert: "sequence", code: "tsp/bad-status-info", what: "statusString" }), { whenUniversal: [TAGS.SEQUENCE] }),
...
function _namedBits(bytes) {
  ...
  for (var i = 0; i < bytes.length * 8; i++) {
    if ((bytes[i >> 3] >> (7 - (i & 7))) & 1) out.push(FAILURE_BITS[i] || ("bit" + i));
  }
  return out;
}
...
var TIME_STAMP_RESP = schema.seq([
  ...
  build: function (m) {
    ...
    return {
      ...
      timeStampToken: present ? m.fields.timeStampToken.value.bytes : null,
    };
  },
});
 ...
 * RFC 3161 §2.4.2), then decodes the inner `TSTInfo`. Returns `{ tstInfo, signerInfo,
 * certificates }` — the decoded payload plus the CMS signer material a verifier needs.
 *
 * @example
...
function parseToken(input) {
  ...
  return { tstInfo: tstInfo.result, signerInfo: signed.signerInfos[0], certificates: signed.certificates };
}
