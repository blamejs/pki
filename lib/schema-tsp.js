...
var _TOKEN_PARSE_OPTS = pkix.parseOpts({ ErrorClass: TspError, prefix: "tsp", what: "the timeStampToken" });
...
var FAILURE_BITS = Object.assign(Object.create(null), {
...
var TIME_STAMP_RESP = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    return {
      ...
      /** @internal The token is a second parse of bytes carried inside this one, so it takes the
       *  caps this door was given rather than the built-in ceilings. */
      timeStampToken: present ? parseToken(m.fields.timeStampToken.value.bytes, ctx ? ctx.caps : undefined) : null,
    };
  },
});
 ...
 * @signature  pki.schema.tsp.parseRequest(input, caps?) -> timeStampReq
 * @since      0.2.19
 * @status     stable
 * @spec       RFC 3161
 ...
 * @signature  pki.schema.tsp.parseTstInfo(input, caps?) -> tstInfo
 * @since      0.1.13
 * @status     stable
 * @spec       RFC 3161
 ...
 * @signature  pki.schema.tsp.parse(input, caps?) -> timeStampResp
 * @since      0.1.13
 * @status     stable
 * @spec       RFC 3161
 ...
 * @signature  pki.schema.tsp.parseToken(input, caps?) -> tstInfo
 * @since      0.1.13
 * @status     stable
 ...
 *     refused with the `/too-large` code of this format.
 */
function parseToken(input, callerOpts) {
  /** @internal The caller's options are this door's to answer for, so they are read here rather
   *  than inside the CMS parser: a complaint about an option must not come back wearing the
   *  delegate's verdict about the content. The snapshot travels onward, because handing the
   *  caller's own object to the CMS parser would read each cap a second time. */
  var caps = pkix.parseCaps(callerOpts, _TOKEN_PARSE_OPTS);
  var der = pkix.coerceToDer(input, { pemLabel: null, PemError: PemError, ErrorClass: TspError, prefix: "tsp" });
  var signed;
  try { signed = cms.parse(der, caps); }
  catch (e) {
    if (e instanceof TspError) throw e;
    /** @internal A resource refusal says the cap was too small, not that the token is malformed,
     *  so it keeps that meaning rather than becoming a statement about the bytes. */
    if (e && typeof e.code === "string" && e.code.indexOf("/too-large") !== -1) {
      throw new TspError("tsp/too-large", "the timeStampToken exceeds the decode budget for this parse: " + (e.message || ""), e);
    }
  ...
  /** @internal The TSTInfo is decoded out of the eContent the CMS layer handed back, so it is a
   *  second decode and takes the caller's caps the way the carrier did. The latch is re-read
   *  below, because the catch here answers in this domain and would otherwise report a cap that
   *  was too small as a malformed TSTInfo. */
  var nested = pkix.nestedContext(NS, caps);
  var tstInfo;
  try { tstInfo = schema.embeddedDer(TST_INFO, encap.eContent, nested.ns, { code: "tsp/bad-der", what: "the encapsulated TSTInfo" }); }
  catch (e) {
    if (nested.budget.exhausted()) {
      throw new TspError("tsp/too-large", "the timeStampToken exceeds the decode budget for this parse", e);
    }
    if (e instanceof TspError) throw e;
    throw new TspError("tsp/bad-der", "the encapsulated TSTInfo did not decode: " + ((e && e.message) || String(e)), e);
  }
  if (nested.budget.exhausted()) {
    throw new TspError("tsp/too-large", "the timeStampToken exceeds the decode budget for this parse");
  }
 ...
 * a block of any label is accepted unless `label` is given). The text holds one
 * object: a file of several is refused with `pem/multiple-blocks` and read with
 * `pki.schema.pem.decodeBundle`. Throws `PemError` on a missing envelope or a
 * non-base64 body.
 *
 * @example
...
var ESS_DIGEST = Object.assign(Object.create(null), { sha256: "sha256", sha384: "sha384", sha512: "sha512" });

/** @internal Which of the certificates a token embeds is the one that signed it. RFC 3161 sec. 2.4.2
 *  requires the ESS SigningCertificate or SigningCertificateV2 signed attribute and `parseToken`
 *  refuses a token carrying neither, so the certHash is present, and it names exactly one
 *  certificate. The SignerInfo sid names no certificate uniquely: an issuer with a serial, and a
 *  subject key identifier, are both values an issuer wrote, two certificates can carry either, and
 *  the `certificates` SET is outside the signature, so a reader that takes the first sid match
 *  answers from an order the sender chose. Returns the signer's DER, or null when no embedded
 *  certificate hashes to the ESS value, which is the answer a token supplying its certificate out of
 *  band gets. The hash is a public value and is compared by content. */
function signerCertificateDer(token) {
  var attrs = (token.signerInfo && token.signerInfo.signedAttrs) || [];
  if (!Array.isArray(attrs)) return null;
  var sc = null, scV1 = null;
  for (var i = 0; i < attrs.length; i++) {
    if (attrs[i].type === OID_SIGNING_CERT_V2) { sc = attrs[i]; break; }
    if (attrs[i].type === OID_SIGNING_CERT && scV1 === null) scV1 = attrs[i];
  }
  if (sc === null) sc = scV1;
  if (sc === null) return null;
  var decoded;
  // allow:swallow-unverified no token reaches here with an unreadable ESS attribute, since parseToken refuses one as tsp/bad-token; this answers for a caller that resolves against a token it parsed elsewhere
  try { decoded = smime.decodeAttribute(sc); } catch (_e) { return null; }
  var essCert = decoded && decoded.certs && decoded.certs[0];
  if (!essCert || !essCert.hashAlgorithm || !essCert.certHash) return null;
  var nodeAlg = ESS_DIGEST[essCert.hashAlgorithm.name];
  if (!nodeAlg) return null;
  var want = Buffer.from(essCert.certHash);
  ...
  for (var j = 0; j < certs.length; j++) {
    ...
    var actual = nodeCrypto.createHash(nodeAlg).update(element.bytes).digest();
    if (actual.length === want.length && Buffer.compare(actual, want) === 0) return element.bytes;
  }
  return null;
}
...
module.exports = {
  ...
  signerCertificateDer: signerCertificateDer,   // @internal
  pemDecode: pemDecode,
  pemEncode: pemEncode,
  matches: matches,
};
