...
// The runtime operations these shared factories decide with, bound at load. A replaced
// RegExp.prototype.exec decides which bytes a PEM block is taken to hold, and a replaced
// String.prototype.charAt moves where a distinguished-name escape is read.
...
function pemDecode(text, label, PemError) {
  ...
  if (label && m[1] !== label) throw new PemError("pem/label-mismatch", "expected " + intrinsic.stringify(label) + " block, got " + intrinsic.stringify(m[1]));
  var b64 = m[2].replace(/[\r\n\t ]+/g, "");
...
function pemDecodeAll(text, label, PemError) {
  ...
  while ((m = _reExec(re, text)) !== null) {
    if (/\S/.test(_strSlice(text, lastEnd, m.index))) throw new PemError("pem/explanatory-text", "explanatory text is not permitted around PEM blocks (RFC 8555 sec. 9.1)");
    if (m[1] !== label) throw new PemError("pem/label-mismatch", "expected " + intrinsic.stringify(label) + " block, got " + intrinsic.stringify(m[1]));
    var b64 = m[2].replace(/[\r\n\t ]+/g, "");
  ...
  if (/\S/.test(_strSlice(text, lastEnd))) throw new PemError("pem/explanatory-text", "explanatory text is not permitted after the PEM chain (RFC 8555 sec. 9.1)");
  return blocks;
}
...
function pemEncode(der, label, PemError) {
  ...
  var b64 = _bufToString(buf, "base64").replace(/(.{64})/g, "$1\n").replace(/\n$/, "");
  return "-----BEGIN " + label + "-----\n" + b64 + "\n-----END " + label + "-----\n";
}
...
function coerceToDer(input, opts) {
  ...
  if (intrinsic.isBuffer(input) || input instanceof Uint8Array) {
...
function _isPemArmor(buf) {
  var head = _bufToString(_bufSlice(buf, 0, 4096), "latin1");
...
function decodeRoot(der, opts) {
  ...
  try { return asn1.decode(der, opts.ber ? { ber: true } : undefined); }
    ...
    throw new opts.ErrorClass(opts.prefix + "/bad-der", (opts.what || "input") + " DER did not decode: " + ((e && e.message) || intrinsic.String(e)), e);
  }
}
...
function versionReader(ns, accept) {
  return schema.decode(function (n) {
    ...
    if (intrinsic.hasOwn(accept, key)) return accept[key];
  ...
  return schema.decode(function (node) {
    ...
    try { s = asn1.read.string(node); }
      ...
      if (!e || (e.code !== "asn1/expected-string" && e.code !== "asn1/expected-primitive")) {
        throw ns.E(ns.prefix + "/bad-atv", "malformed string in attribute value: " + ((e && e.message) || intrinsic.String(e)));
      }
      ...
      if (node.tagClass === "universal" && node.tagNumber === asn1.TAGS.NUMERIC_STRING) {
        ...
        catch (e2) { throw ns.E(ns.prefix + "/bad-atv", "malformed NumericString in attribute value: " + ((e2 && e2.message) || intrinsic.String(e2))); }
      }
...
function name(ns, opts) {
  ...
  function nameBuild(m) {
    var rdns = [], parts = [];
    intrinsic.forEach(m.items, function (rdnItem) {
      var atvs = [], atvParts = [];
      intrinsic.forEach(rdnItem.value.items, function (atvItem) {
      ...
      _push(rdns, atvs);
      _push(parts, intrinsic.join(atvParts, "+"));
    });
    ...
    return { rdns: rdns, dn: intrinsic.join(parts, ", "), bytes: opts.implicitTag != null ? asn1.sequenceTlv(m.node) : m.node.bytes };
  }
  ...
  return schema.decode(function (n, ctx) {
    ...
    if (GN_CONSTRUCTED[t]) {
      ...
      if (GN_IA5[t]) {
        ...
        if (decodeValue) value = intrinsic.bufferConcat([n.content]);
      } else if (t === 8) {
        var regId;
...
function generalNames(ns, opts) {
  ...
  function build(m) { return { names: intrinsic.map(m.items, function (it) { return it.value; }), bytes: m.node.bytes }; }
  if (opts.implicitTag != null) {
...
function distributionPointName(ns, node, code) {
  if (node && node.tagClass === "context" && node.tagNumber === 0) {
    ...
    return { kind: "fullName", names: intrinsic.map(gns.names, function (n) { return n.bytes; }) };
  }
...
function assertPolicyQualifiers(qNode, fail) {
  ...
  intrinsic.forEach(qNode.children, function (pq) {
...
function _noticeNumbers(node) {
  ...
  var nums = [], ok = true;
  intrinsic.forEach(node.children, function (n) {
    // allow:swallow-unverified a non-INTEGER member makes the whole reference undecodable, and the caller falls back
    try { _push(nums, intrinsic.String(asn1.read.integer(n))); } catch (_e) { ok = false; }
  });
  return ok ? nums : null;
}
...
function _decodeHelpers(ns) {
  function decodeTop(buf, code, what) {
    ...
    catch (e) { throw ns.E(code, "malformed " + what + " value: " + ((e && e.message) || intrinsic.String(e)), e); }
    return n;
  }
  ...
  function O(nm) {
    ...
    if (typeof d !== "string") throw new TypeError("pki.schema.pkix: " + intrinsic.stringify(nm) + " is not a registered OID name");
    return d;
  }
...
function certExtensionDecoders(ns) {
  ...
  function keyUsage(buf) {
    ...
    var out = {};
    intrinsic.forEach(KU_BITS, function (nm, bit) {
  ...
  function subtreeList(node, C) {
    ...
    return intrinsic.map(node.children, function (st) {
  ...
  function nameConstraints(buf) {
    ...
    intrinsic.forEach(kids, function (k) {
  ...
  function certificatePolicies(buf) {
    ...
    var seen = {};
    return intrinsic.map(kids, function (pi) {
  ...
  function policyMappings(buf) {
    ...
    return intrinsic.map(kids, function (mp) {
  ...
  function authorityInfoAccess(buf) {
    ...
    return intrinsic.map(descs, function (d) {
  ...
  function extKeyUsage(buf) {
    ...
    return intrinsic.map(kids, function (k) {
  ...
  function authorityKeyIdentifier(buf) {
    ...
    intrinsic.forEach(kids, function (k) {
      ...
      if (k.tagNumber === 0) { try { out.keyIdentifier = intrinsic.bufferFrom(asn1.read.octetStringImplicit(k, 0)); } catch (e) { throw ns.E(C, "AuthorityKeyIdentifier keyIdentifier [0] must be an IMPLICIT OCTET STRING", e); } }
  ...
  function subjectKeyIdentifier(buf) {
    ...
    try { return intrinsic.bufferConcat([asn1.read.octetString(n)]); }
  ...
  function crlDistributionPoints(buf) {
    ...
    return intrinsic.map(kids, function (dp) {
      ...
      var lastTag = -1;
      intrinsic.forEach(dp.children, function (f) {
  ...
  function _qcOidSeq(node, C, what) {   // a non-empty SEQUENCE OF OBJECT IDENTIFIER
    ...
    return intrinsic.map(node.children, function (c) {
  ...
  qcInfoByOid[O("qcType")] = function (node, C) {
    ...
    return { types: oids, typeNames: intrinsic.map(oids, function (d) { return ns.oid.name(d) || null; }) };
  };
  ...
  qcInfoByOid[O("qcCClegislation")] = function (node, C) {   // SEQUENCE OF CountryName (PrintableString SIZE 2)
    ...
    return { countries: intrinsic.map(node.children, function (c) { var v = _qcStr(c, _T.PRINTABLE_STRING, C, "QcCClegislation CountryName must be a PrintableString"); if (v.length !== 2) throw ns.E(C, "QcCClegislation CountryName must be a 2-letter ISO 3166-1 code"); return v; }) };
  };
  qcInfoByOid[O("qcPDS")] = function (node, C) {   // PdsLocations ::= SEQUENCE OF PdsLocation { url IA5, language Printable(2) }
    ...
    return { locations: intrinsic.map(node.children, function (loc) {
  ...
  qcInfoByOid[O("qcIdentMethod")] = function (node, C) {
    ...
    return { methods: oids, methodNames: intrinsic.map(oids, function (d) { return ns.oid.name(d) || null; }) };
  };
  qcInfoByOid[O("qcQSCDlegislation")] = function (node, C) {
    ...
    return { countries: intrinsic.map(node.children, function (c) { var v = _qcStr(c, _T.PRINTABLE_STRING, C, "QcQSCDlegislation CountryName must be a PrintableString"); if (v.length !== 2) throw ns.E(C, "QcQSCDlegislation CountryName must be a 2-letter ISO 3166-1 code"); return v; }) };
  };
  ...
  function msPreviousCertHash(buf) {
    ...
    var hash;
    try { hash = intrinsic.bufferConcat([asn1.read.octetString(n)]); }
...
function extensions(ns, opts) {
  ...
  return schema.seqOf(EXT, intrinsic.assign({ assert: "sequence" }, extOpts));
}
  ...
  return schema.seq([
    ...
    build: function (m, ctx) {
      ...
      return {
        ...
        values: intrinsic.map(m.fields.values.value.items, function (it) { return it.node.bytes; }),
      };
    },
  });
}
