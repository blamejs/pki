...
function certExtensionDecoders(ns) {
  ...
  function _qcSemantics(node, C) {   // SemanticsInformation, WITH COMPONENTS at least one field present
    ...
    if (node.children[i]) {
      ...
      nras = nn.children.map(function (g) { return g.bytes; });   // raw GeneralName TLVs surfaced
      i++;
    }
  ...
  qcInfoByOid[O("qcLimitValue")] = function (node, C) {   // MonetaryValue ::= SEQUENCE { currency, amount, exponent }
    ...
    return {
      currency: currency,
      amount: guard.range.uint31(readInt(node.children[1], C, "QcLimitValue amount"), ns.E, C, "QcLimitValue amount"),
