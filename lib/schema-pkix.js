...
// replaceable properties to answer one thing, and here it decides which version numbers a format
// accepts -- shared by every format module, so one admitted name widens all of them at once.
var _hasOwn = intrinsic.hasOwn;
// The rest of the operations this module decides with, taken at load for the same reason. What
// these factories produce is what a consumer compares an identity against -- a certificate's
// subject alternative names against an enrollment's own, a distinguished name against an issuer's
// -- so a name that traversal, folding or a byte copy invents here is a name the encoded structure
// never carried, and the comparison downstream still answers true.
...
// The JavaScript regex `\s` class, decided by code point, so the whitespace tests below match
// exactly what `/\s/` and `/\S/` matched with no caller-replaceable match protocol standing in for
// them. Set = ECMAScript WhiteSpace + LineTerminator: TAB LF VT FF CR SP, U+00A0, U+1680,
...
// Charset validation by character scan (the shared home for the `/^[chars]+$/`-family regexes format
// modules used to carry): `charTable` builds a code-point membership table from a string of the allowed
// characters, and `allCharsIn` checks that a NON-EMPTY string is composed only of them (`+`, not `*`).
// One data-table loop, not a per-charset inlined character-class chain.
...
// PEM armor markers, matched literally so no regex -- with its live, replaceable match protocol and
// the backreference the END repeats -- stands between the parser and the wire.
var _PEM_BEGIN = "-----BEGIN ";
var _PEM_END = "-----END ";
var _PEM_DASHES = "-----";

// The next PEM block at or after `from`: "-----BEGIN <label>-----<body>-----END <label>-----" where
// the label is one or more of [A-Z0-9 ] (RFC 7468 label octets, spaces included) and the END label
// repeats the BEGIN label exactly (the backreference). Returns { index, label, body, end } -- `index`
// the BEGIN offset, `end` the offset past the closing dashes -- or null when no such block remains.
// A "-----BEGIN " with no valid label or no matching END is skipped, mirroring the regex trying each
// start position; the body is the shortest span to the first matching END (the lazy quantifier).
function _findPemBlock(text, from) {
  ...
  for (;;) {
    ...
    while (p < text.length) {
      ...
      if (!((c >= 65 && c <= 90) || (c >= 48 && c <= 57) || c === 32)) break; // A-Z 0-9 space
      p += 1;
    }
...
// All ASCII CR / LF / TAB / SPACE removed (the base64 body's permitted layout whitespace).
...
// A base64 string wrapped to 64-column lines joined by LF, no trailing LF (matching (.{64})->"$1\n"
// then a trailing-newline strip; base64 output carries no newline of its own to strip).
function _wrap64(s) {
  var out = "";
...
// The label must be re-readable by this module's own PEM label scan (uppercase
...
// The armor label grammar, decided character by character (no regex protocol on the emit path):
// one or more A-Z0-9 words joined by single spaces -- so no leading/trailing space and no doubled
// space, and every other character in [A-Z0-9].
...
// Only the base64 alphabet (A-Z a-z 0-9 + / =) kept, every other character dropped -- the lenient
// filter of the historic signer-certificate path, by character scan.
...
// The @internal LENIENT PEM decode of a fixed-`label` block (the historic signer-certificate path,
// distinct from the strict canonical `pemDecode` above): the body of the first
// "-----BEGIN <label>-----...-----END <label>-----" block with any non-base64 character stripped,
// decoded through Node's lenient base64. Returns the DER Buffer, or null when no such block exists so
// the caller keeps its own domain error. NOT for verified input -- pemDecode is the fail-closed door.
...
function coerceToDer(input, opts) {
  ...
  if (guard.bytes.isByteSource(input)) {
    // Route any DER BufferSource (Buffer, typed array, DataView, or ArrayBuffer) through the shared byte
    // guard so a detached backing store fails closed here for every format that composes this boundary
    // (x509/crl/csr/pkcs8/cms/pkcs12), not per-format, and its byte view is read from the slot -- a
    // cross-realm ArrayBuffer or an offset DataView reaches the decoder exactly as a Buffer does.
...
function name(ns, opts) {
  ...
  function nameBuild(m) {
    var rdns = [], parts = [];
    // Through the captured traversal: `rdns` is the parsed subject a consumer reads a common name
    // out of, so a replaced Array.prototype.forEach or push could drop the name the certificate
    // carries and seat one it does not.
...
// Which GeneralName alternatives are constructed, and which carry an IA5String. Both tables answer
// off a tag number the encoded value supplies, and both decide which validation an alternative
// gets, so neither may inherit: with an ordinary prototype a planted numeric property routes a tag
// down another alternative's arm, and a dNSName then reaches the IA5 arm's control-byte reject as
// a constructed value, or misses it altogether.
...
function generalNames(ns, opts) {
  ...
  // Through the captured traversal: the names this yields are what a consumer compares an identity
  // against -- a certificate's subject alternative names against an enrollment's own -- and a
  // replaced Array.prototype.map could present a name the encoded GeneralNames does not carry.
...
var _KU_DECODER = new _WeakMapI();     // one decoder table per namespace, built on first use
...
function _isDisplayTextNode(n) {
  if (!_DISPLAY_TEXT_TAGS) {
    // Indexed by a tag number the encoded value supplies, so it holds no prototype: an inherited
    // numeric name would make an unregistered universal tag read as a DisplayText alternative.
...
function certExtensionDecoders(ns) {
  ...
  function certificatePolicies(buf) {
    ...
    // The duplicate-policy set answers off a dotted OID the encoded value supplies, so it holds no
    // prototype: an inherited name would report a policy as already seen and reject a valid one.
  ...
  // The per-statement decoder table answers off a dotted OID the encoded value supplies, so it holds
  // no prototype: an inherited name would hand a statement a decoder nothing registered.
  ...
  // The extension-decoder registry every consumer indexes with an OID read off the wire
  // (path validation, inspection, the ACME certificate binding), so it holds no prototype: a
  // planted dotted name would otherwise hand an extension a decoder nothing registered.
...
// One LDH label (RFC 1034 preferred syntax), decided character by character: every character an
// ASCII letter/digit/hyphen, and a hyphen neither first nor last. The caller has already rejected
// any label carrying a dot, whitespace, or underscore, so only the LDH shape remains to check.
...
module.exports = {
  ...
  pemEncode: pemEncode,
  isJsWhitespace: _isJsWhitespace,   // the shared JS regex \s code-point predicate for char-scan parsers
  stripBase64Whitespace: _stripB64Whitespace,   // shared CR/LF/TAB/SP strip for base64 bodies (PEM, MIME)
  charTable: _charTable,   // build a code-point membership table from a string of allowed characters
  allCharsIn: _allCharsIn,   // is a NON-EMPTY string composed only of table members (the /^[chars]+$/ family)
  coerceToDer: coerceToDer,
  isPemArmor: _isPemArmor,   // does a byte source carry a PEM "-----BEGIN" armor (RFC 7468 leading text allowed)
