...
function certExtensionDecoders(ns) {
  ...
  function _qcSemantics(node, C) {   // SemanticsInformation, WITH COMPONENTS at least one field present
    ...
    if (node.children[i]) {
      ...
      if (nn.tagClass !== "universal" || nn.tagNumber !== _T.SEQUENCE || !nn.children || !nn.children.length) throw ns.E(C, "SemanticsInformation nameRegistrationAuthorities must be a non-empty SEQUENCE OF GeneralName");
      // A GeneralName is a CHOICE whose alternatives are all context-tagged [0]..[8]; validate each
      // element is a GeneralName rather than surfacing arbitrary bytes as a name-registration authority.
      nras = nn.children.map(function (g) {
        if (g.tagClass !== "context") throw ns.E(C, "a SemanticsInformation nameRegistrationAuthority must be a GeneralName (a context-tagged CHOICE alternative)");
        return g.bytes;
      });
      i++;
    }
  ...
  qcInfoByOid[O("qcLimitValue")] = function (node, C) {   // MonetaryValue ::= SEQUENCE { currency, amount, exponent }
    ...
    return {
      currency: currency,
      // A reliance-limit amount is non-negative but can far exceed 2^31 (a large limit in a minor currency
      // unit), so bound it to the whole safe-integer range rather than uint31 -- a wider value would round
      // when narrowed to Number and mis-state the limit; the exponent is a small signed base-10 scale.
      amount: guard.range.int(readInt(node.children[1], C, "QcLimitValue amount"), 0n, 9007199254740991n, ns.E, C, "QcLimitValue amount"),
      exponent: guard.range.int(readInt(node.children[2], C, "QcLimitValue exponent"), -128n, 127n, ns.E, C, "QcLimitValue exponent"),
    };
  };
  ...
  function qcStatements(buf) {
    ...
    var out = [], seen = Object.create(null);   // null-proto: a dotted-OID statementId can never collide with a prototype key
    for (var i = 0; i < kids.length; i++) {
      ...
      if (seen[id]) throw ns.E(ns.prefix + "/duplicate-qc-statement", "duplicate QCStatement statementId " + id);
      seen[id] = true;
