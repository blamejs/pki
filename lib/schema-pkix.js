...
// subjectPublicKey BIT STRING }. The historical cert guard checked only
// children.length===2 (not the SEQUENCE tag) -> assert "constructed". Shared by
// the certificate and CSR parsers (RFC 5280 §4.1.2.7, RFC 2986 §4.1).
function spki(ns) {
  return schema.seq([
    schema.field("algorithm", algorithmIdentifier(ns)),
    schema.field("subjectPublicKey", schema.bitString()),
  ], {
    assert: "constructed", arity: { exact: 2 }, code: ns.prefix + "/bad-spki", what: "SubjectPublicKeyInfo",
    build: function (m) {
      return {
