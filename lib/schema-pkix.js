...
function decodeRoot(der, opts) {
  try { return asn1.decode(der); }
    ...
    // A format whose wire encoding is normatively BER (RFC 7292 PKCS#12)
    // opts in to a second decode pass accepting exactly the two BER shapes
    // the codec's `ber` mode licenses. DER-first: a strict-DER input never
    // reaches this path, and any other strictness fault still rejects.
    if (opts.berFallback && e && (e.code === "asn1/indefinite-length" || e.code === "asn1/constructed-primitive-type")) {
      try { return asn1.decode(der, { ber: true }); }
      catch (e2) { throw new opts.ErrorClass(opts.prefix + "/bad-der", (opts.what || "input") + " BER did not decode: " + ((e2 && e2.message) || String(e2)), e2); }
    }
