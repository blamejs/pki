...
function certExtensionDecoders(ns) {
  ...
  function nameConstraints(buf) {
    ...
    var permitted = [], excluded = [], sawP = false, sawE = false;
  ...
  function certificatePolicies(buf) {
    ...
    return kids.map(function (pi) {
      if (pi.tagClass !== "universal" || pi.tagNumber !== _T.SEQUENCE || !pi.children || pi.children.length < 1) {
      ...
      seen[pid] = true;
      var qualifiers = pi.children.length > 1 ? pi.children[1].bytes : null;
  ...
  function policyConstraints(buf) {
    ...
    kids.forEach(function (k) {
      ...
      if (k.tagNumber === 0) { if (rep !== null) throw ns.E(C, "duplicate requireExplicitPolicy field"); rep = Number(v); }
      else if (k.tagNumber === 1) { if (ipm !== null) throw ns.E(C, "duplicate inhibitPolicyMapping field"); ipm = Number(v); }
