...
// The DER encoding of NULL, for the PBKDF2 prf DEFAULT (algid-hmacWithSHA1 with NULL parameters) check.
var _DER_NULL = asn1.build.nullValue();
...
function pbkdf2Params(ns) {
  return schema.seq([
    ...
    build: function (m, ctx) {
      ...
      // RFC 8018 sec. 5.2); an explicit prf byte-equal to that default is non-canonical and rejects.
      // hmacWithSHA1 with ABSENT parameters is a different value and decodes.
      if (prf && prf.oid === hmacSha1 && prf.parameters !== null && prf.parameters.equals(_DER_NULL)) {
