...
// BMPString is UCS-2 (big-endian, two octets per character), which Buffer cannot decode directly;
// the other three arms are UTF-8 compatible for display purposes.
function _displayTextString(n) {
  if (n.tagNumber !== _T.BMP_STRING) return n.content.toString("utf8");
  var out = "";
  for (var i = 0; i + 1 < n.content.length; i += 2) out += String.fromCharCode((n.content[i] << 8) | n.content[i + 1]);
  return out;
}
function _dtEntry(field, node) {
  var text = _displayTextString(node);
  return { field: field, tagNumber: node.tagNumber, text: text, chars: displayTextChars(text) };
}
...
// The numbers ride on the organization entry as decimal strings. Best-effort by design: this reads an
// already-decoded node for display, so a member that is not a readable INTEGER is skipped rather than
// throwing on a path whose callers report rather than adjudicate.
function _noticeNumbers(node) {
  if (!node || node.tagClass !== "universal" || node.tagNumber !== _T.SEQUENCE || !node.children) return [];
  var nums = [];
  node.children.forEach(function (n) {
    try { nums.push(String(asn1.read.integer(n))); }
    catch (_e) { /* allow:swallow-unverified display best-effort: a non-INTEGER member is omitted, never a throw on a report path */ }
  });
  return nums;
}
function userNoticeTexts(qualifier) {
  ...
  var out = [];
  qualifier.children.forEach(function (member) {
    if (member.tagClass === "universal" && member.tagNumber === _T.SEQUENCE) {
      if (member.children && _isDisplayTextNode(member.children[0])) {
        var org = _dtEntry("organization", member.children[0]);
        org.noticeNumbers = _noticeNumbers(member.children[1]);
        out.push(org);
      }
      return;
    }
    if (_isDisplayTextNode(member)) out.push(_dtEntry("explicitText", member));
  });
  return out;
}
