...
var RESOURCE_REFUSALS = intrinsic.assign(intrinsic.create(null), {
  "asn1/too-large": 1, "asn1/too-deep": 1, "asn1/too-many-items": 1,
});
...
function decodeRoot(der, opts, caps, budget) {
  try { return asn1.decode(der, _decodeOptions(opts, caps)); }
    ...
    if (opts.rawDecodeErrors && !(e && RESOURCE_REFUSALS[e.code])) throw e;
    if (e && RESOURCE_REFUSALS[e.code]) {
      if (budget) {
        try {
      ...
      throw new opts.ErrorClass(opts.prefix + "/too-large",
        (opts.what || "the input") + " exceeds the decode budget for this parse: " + ((e && e.message) || _String(e)), e);
    }
...
function runParseWith(input, opts, callerOpts, build) {
  ...
  var ns = _nsWithBudget(opts.ns, budget);
  var result;
  try {
 ...
 *  under the walk may absorb the exception, and this still refuses. */
...
function _nsWithBudget(ns, budget) {
  var out = intrinsic.create(null);
  _forEach(intrinsic.keys(ns), function (k) { out[k] = ns[k]; });
  out.budget = budget;
  return out;
}
...
function decodeIssuingDistributionPointValue(ns, bytes, code) {
  ...
  var m;
  try {
    m = schema.walk(issuingDistributionPoint(code), asn1.decode(bytes), ns);
