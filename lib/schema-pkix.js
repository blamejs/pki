...
function assertPolicyQualifiers(qNode, fail) {
  ...
  qNode.children.forEach(function (pq) {
    ...
    try { asn1.read.oid(pq.children[0]); } catch (_e) { fail("PolicyQualifierInfo must lead with a policyQualifierId OID"); }
  });
}
  ...
  function certificatePolicies(buf) {
    ...
    return kids.map(function (pi) {
      ...
      if (pi.children.length > 1) {
        ...
        assertPolicyQualifiers(q, function (msg) { throw ns.E(C, msg); });
        qualifiers = q.bytes;
      }
