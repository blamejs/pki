...
// Taken at load. `byteLength` is what the RFC 5322 line limit is measured with, so a replacement
// decides whether an over-long header line is refused. See guard-intrinsic for the whole set.
var _byteLength = require("./guard-intrinsic").byteLength;
// The character-code read the RFC 5322 ftext and CR/LF/NUL checks are built on. One that never
// reports a control byte is a header-injection defense that inspects nothing.
...
// The conversion that produces the text this guard then scans for CR and LF. A replacement returns
// a clean string for a value carrying a line break, so the injection scan runs on text that is not
// what gets emitted.
