...
function _checkTargeting(ac, want) {
  ...
  var uncomparable = null, sawTargetCert = false;
  for (var g = 0; g < groups.length; g++) {
    for (var t = 0; t < groups[g].length; t++) {
      var entry = groups[g][t];
      // sec. 4.3.2 defines the check exactly: it "passes if the current server (recipient) is one of
      // the targetName fields in the Targets SEQUENCE, or if the current server is a member of one of
      // the targetGroup fields". The third arm is excluded by the same section -- "The targetCert
      // CHOICE within the Target structure is only present to allow future compatibility with
      // [X.509-2000] and MUST NOT be used" -- and the names inside it identify a certificate, not a
      // target. Matching on it would implement a form the profile forbids, and would widen what this
      // verifier accepts. It is passed over and named in the reason, so an operator debugging a
      // refusal can see that the AC used it.
      if (entry.kind === "targetCert") { sawTargetCert = true; continue; }
  ...
  return { checked: true, ok: false,
    reason: "this attribute certificate does not name this verifier among its targets (RFC 5755 sec. 4.3.2)" +
      (sawTargetCert ? "; it carries targetCert entries, which that section says MUST NOT be used and which name a certificate" : "") };
}
...
function _fixVerifyOptions(opts) {
  ...
  if (o.revocationStatus !== undefined) {
    if (typeof o.revocationStatus !== "string" ||
      !Object.prototype.hasOwnProperty.call(REVOCATION_STATUS, o.revocationStatus)) {
...
function _revocationScheme(ac) {
  ...
  for (var i = 0; i < ac.extensions.length; i++) {
    ...
    if (p !== undefined && Object.prototype.hasOwnProperty.call(REVOCATION_POINTERS, e.oid)) out.pointer = p;
  }
  return out;
}
...
function _verify(ac, issuer, opts) {
  ...
  return Promise.resolve(attrcertVerify.verifyAcSignature(parsed, trust.spki)).then(function (sigOk) {
    ...
    // running the section's "pointer in AC" scheme passes the status they established as
    // opts.revocationStatus; the AC's own crlDistributionPoints / authorityInfoAccess pointers ride
    // out on the verdict's `extensions` for them to follow.
