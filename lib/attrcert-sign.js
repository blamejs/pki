...
// object key -> { oid name, critical } of the AC / AA extension it emits (RFC 5755 sec. 4.3).
var EXT_META = {
  ...
  aaControls:             { name: "aaControls",            critical: false },
  acProxying:             { name: "acProxying",            critical: false },
...
function _encodeAAControls(aac) {
  ...
  if (aac.pathLenConstraint != null) {
    if (typeof aac.pathLenConstraint !== "number" || !Number.isInteger(aac.pathLenConstraint) || aac.pathLenConstraint < 0) throw _err("attrcert/bad-input", "aaControls.pathLenConstraint must be a non-negative integer");
