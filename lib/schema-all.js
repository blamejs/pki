 ...
 * format's own `parse` returns. Throws `PkiError("schema/unknown-format")` when
...
function parse(input) {
  ...
  catch (e) { throw new PkiError("schema/bad-der", "input did not decode as DER: " + ((e && e.message) || String(e)), e); }
  for (var i = 0; i < FORMATS.length; i++) {
    if (FORMATS[i].detect(root)) return FORMATS[i].parse(input);
  }
  throw new PkiError("schema/unknown-format", "input does not match any registered PKI format (" + all().join(", ") + ")");
}

// PEM is unwrapped to DER by the matched format's parser, but detection needs
// bytes first: accept a DER Buffer directly, else pull the first PEM block's DER.
function _coerceDer(input) {
  if (Buffer.isBuffer(input)) return input;
  if (typeof input === "string") return x509.pemDecode(input);
  throw new PkiError("schema/bad-input", "parse expects a DER Buffer or a PEM string");
}
