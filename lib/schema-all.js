...
var FORMATS = [
  ...
  },
  {
    ...
    // child is an INTEGER status. Disjoint from the OID-first ContentInfo, the
    // INTEGER-first PKCS#8 key, and the exactly-3 signed-envelope trio (RFC 3161
    // §2.4.2). A bare timestamp token is a CMS ContentInfo and routes to cms; a bare
    // TSTInfo is an internal payload (reached via pki.schema.tsp.parseTstInfo).
    name: "tsp",
    module: tsp,
    detect: tsp.matches,
 ...
 *
 * @example
 *   pki.schema.all();  // → ["cms", "ocsp-request", "ocsp-response", "tsp", "pkcs8", "csr", "crl", "x509"]
 */
