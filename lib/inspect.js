...
// Render a printable value indented under `inner`. CR / CRLF line endings are
// normalized to LF FIRST so a standalone carriage return becomes a real line
// break the indent handles, rather than a raw byte that moves the terminal cursor
// back over the report (a bare \r is not matched by an \n-only indent).
function _indent(s, inner) { return inner + String(s).replace(/\r\n?/g, "\n").replace(/\n/g, "\n" + inner); }
function _fallback(value, inner) {
  if (!Buffer.isBuffer(value) || value.length === 0) return inner + "(empty)";
  if (_printable(value)) return _indent(value.toString("utf8"), inner);
  try {
    var n = asn1.decode(value);
    if (n.tagClass === "universal" && _STRING_TAGS[n.tagNumber]) {
      var s = asn1.read.string(n);
      if (_printable(Buffer.from(s, "utf8"))) return _indent(s, inner);
    }
