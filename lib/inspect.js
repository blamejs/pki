...
function _looksParsedCms(o) {
  return typeof o.contentType === "string" && typeof o.contentTypeName === "string";
}
...
function cmsReport(input) {
  ...
  try { m = _parseCms(input); }
    ...
    if (e && e.code === "inspect/bad-cms" && e.cause && e.cause.code === "cms/unsupported-content-type") return _cmsOuterSummary(input);
    throw e;
  }
  var L = ["CMS ContentInfo:"];
  L.push("    Content Type: " + (m.contentTypeName || oid.name(m.contentType) || m.contentType) + " (" + m.contentType + ")");
  if (m.contentTypeName === "signedData") {
    L.push("    SignedData:");
...
function any(input) {
  ...
  var fmt = schemaAll.detectFormat(der);
