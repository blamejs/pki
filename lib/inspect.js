...
// Report-safety escaping routes through the guard family: control-byte
// neutralization and RFC 4514 DN-value escaping are fail-closed output shapes the
// guard-shape-reinlined detector protects, so no renderer re-inlines them.
// @guard-via guard\.name\.escape
var _clean = guard.name.escapeControlBytes;
var _dnEscape = guard.name.escapeDnValue;

// OpenSSL RDN short names (pki.C.NAMES.DN_SHORT); fall back to the registered long name or the OID.
var DN_SHORT = NAMES.DN_SHORT;
function _attr(a) {
