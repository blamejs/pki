...
// RFC 4514 string escaping for a DN attribute value: the structural separators
// (`"` `+` `,` `;` `<` `>` `\`) plus a leading `#` / leading-or-trailing space are
// backslash-escaped, then control bytes are escaped by _clean. Without this a value
// like `foo, CN=admin` would render as `CN=foo, CN=admin` and read as two RDNs,
// letting an attacker-controlled name masquerade as extra attributes in the report.
function _dnEscape(s) {
  s = String(s).replace(/([\\"+,;<>])/g, "\\$1").replace(/^#/, "\\#").replace(/^ | $/g, "\\ ");
  return _clean(s);
}
function _attr(a) {
  var label = DN_SHORT[a.name] || a.name || a.type;
  return label + "=" + _dnEscape(a.value);
}
