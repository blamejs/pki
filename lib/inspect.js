...
function _coerce(input, spec) {
  ...
  if (Buffer.isBuffer(input)) der = input;
    ...
    // Label-agnostic: a PEM block of any label (CMS vs the common PKCS7, X509 CRL vs CRL, ...)
    // unwraps to its DER, which is then parsed AS the requested format -- a non-matching block
    // fails structurally as the format's typed inspect/bad-<fmt>, not a label mismatch.
    try { der = pkix.pemDecode(input, null, InspectError); }
    catch (e) { throw _err("inspect/bad-input", "input is not a DER Buffer or a PEM block", e); }
  } else {
...
function _cmsOuterSummary(input) {
  var ct = null;
  try {
    var der = Buffer.isBuffer(input) ? input : pkix.pemDecode(input, null, InspectError);
