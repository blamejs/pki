...
function _reportsTruncation(name) {
  ...
  if (!_isBuffer(frame) || frame.length < 4) return false;
  ...
  if (!_isBuffer(whole) || !whole.equals(sample)) return false;
...
var SAFE = {};
_objectKeys(DECOMPRESS).forEach(function (n) { if (_reportsTruncation(n)) SAFE[n] = true; });
