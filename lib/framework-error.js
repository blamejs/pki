 ...
 *   retry. A network failure that says nothing about the bytes is
 *   `permanent: false`, so a caller can retry on the error rather than on
 *   a code list of its own: `transport/timeout` and
 *   `transport/proxy-connect-failed`.
 *
 * @card
...
var _TRANSIENT_CODES = intrinsic.assign(intrinsic.create(null), {
  "transport/timeout": 1,
  "transport/proxy-connect-failed": 1,
});
...
class PkiError extends Error {
  constructor(message, code) {
    ...
    this.permanent = !_TRANSIENT_CODES[c];
  }
}
