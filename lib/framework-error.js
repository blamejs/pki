...
// The frozen shape of every error code: a `domain/reason` string -- one or more lowercase alphanumerics or
// dashes, a single "/", then one or more of the same. Decided by character scan (no regex).
function _isCodeShape(c) {
  var n = c.length, i, ch, slash = -1;
  if (n < 3) return false;                                             // shortest is "a/b"
  for (i = 0; i < n; i += 1) {
    ch = _charCodeAt(c, i);
    if (ch === 47) { if (slash !== -1) return false; slash = i; }       // "/" -- at most one
    else if (!((ch >= 97 && ch <= 122) || (ch >= 48 && ch <= 57) || ch === 45)) return false;   // a-z 0-9 "-"
  }
  return slash > 0 && slash < n - 1;                                   // one slash, both sides non-empty
}
...
// ScepError -- a byte sequence that is not a well-formed SCEP pkiMessage (RFC 8894): a
// malformed CMS layering, a missing or mis-encoded transaction attribute, an unknown
// messageType / pkiStatus / failInfo enumerant, a nonce mismatch, or a weak-algorithm
// profile violation. Fail-closed: a malformed or off-profile message is a typed verdict.
...
// KemError -- a malformed composite ML-KEM key or ciphertext, or an unsupported
// composite algorithm (draft-ietf-lamps-pq-composite-kem). Raised by pki.kem.
