 ...
 *   on and safe to log. A failure that is a deterministic verdict on the
 *   bytes in hand (a malformed length, an unknown OID shape, a truncated
 *   certificate) is `permanent: true`: the same input will never parse on
 *   retry. A failure whose reason is `timeout` is `permanent: false`, so a
 *   caller can retry on the error rather than on a code list of its own.
 *   The reason is read without its domain, so the protocol clients that
 *   raise the same failure under their own prefix (`acme/timeout`,
 *   `est/timeout`) carry the flag too.
 *
 * @card
...
var _TRANSIENT_REASONS = intrinsic.assign(intrinsic.create(null), { timeout: 1 });

function _reasonOf(c) {
  for (var i = 0; i < c.length; i++) if (_charCodeAt(c, i) === 47) return _strSlice(c, i + 1);
  return c;
}
...
class PkiError extends Error {
  constructor(message, code) {
    ...
    this.permanent = !_TRANSIENT_REASONS[_reasonOf(c)];
  }
}
...
var TrustAnchorError = defineClass("TrustAnchorError", { withCause: true });
...
var Pkcs11Error = defineClass("Pkcs11Error", { withCause: true });
var Pkcs1Error = defineClass("Pkcs1Error", { withCause: true });
var Sec1Error = defineClass("Sec1Error", { withCause: true });
var IdentityError = defineClass("IdentityError", { withCause: true });
