...
function _isCrlExtensionEntry(e) {
  return !!e && typeof e.oid === "string" && _isOptName(e.name) &&
    typeof e.critical === "boolean" && e.value !== undefined && _isBuffer(e.valueBytes);
}
...
function isCert(o) {
  return !!o && typeof o === "object" &&
    ...
    (o.issuerUniqueID === null || _isBitString(o.issuerUniqueID)) &&
    (o.subjectUniqueID === null || _isBitString(o.subjectUniqueID)) &&
...
/**
 * The bytes a recorded parse result came from, for a consumer that has to hand DER on to a verb taking
 * bytes rather than a parsed object. A copy, so the caller cannot reach the recorded source.
 *
 * Null for anything not recorded, and for a result whose fields no longer match what the parse
 * produced: the bytes are the truth, so handing them on for an object a caller has since edited would
 * silently discard the edit and forward something the caller did not mean.
 *
 * @enforced-by behavioral -- an accessor has no rename-proof shape to detect; its contract is pinned by
 * the vectors that drive it on an unrecorded object, a recorded one, and an edited one.
 */
function sourceOf(obj) {
  if (!isRecordedAsProduced(obj)) return null;
  return _bufferFrom(_weakGet(PROVENANCE, obj).source);
}
...
module.exports = _freezeExports({
  ...
  isRecordedAsProduced: isRecordedAsProduced, sourceOf: sourceOf,
});
