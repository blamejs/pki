...
// The array test, taken at load. Every shape check below asks it whether a field IS a list, and
// one answering `false` makes each of those checks reject a well-formed parsed structure -- the
// fail-closed direction, and still a caller deciding what this guard recognizes.
var _isArray = require("./guard-intrinsic").isArray;
// The copy that becomes the recorded provenance bytes. A replacement returning a buffer over the
// caller's own memory would let those bytes change after they were recorded, which is the one
// thing provenance exists to prevent.
var _bufferFrom = require("./guard-intrinsic").bufferFrom;
// The sentinel the chain walk below stops at. Reached as a property of a replaceable global, a
// replacement makes the walk run past the top and report inherited names as the value's own.
var _ObjectProto = require("./guard-intrinsic").ObjectProto;
// The rest of what this module reads from the runtime, taken at load. The shape checks below ask
// these what a value IS and how it is put together, so a replacement decides what this guard
// recognizes as parser output. See guard-intrinsic for the whole captured set.
...
// The array operations, uncurried. Each shape check below asks `every` whether a list's members
// are all well formed, so one replaced to return true makes every malformed list pass.
var _every = require("./guard-intrinsic").every;
// The provenance store's own operations. This WeakMap is what says a value came out of this
// toolkit's parser, so a `get` that answers for a key never stored hands the door a forged
// provenance record, and a `has` that answers true admits an object the parser never produced.
...
// ArrayBuffer. Testing `_isBuffer(x) || x instanceof Uint8Array` is narrower than what the
...
// Null-prototype: this table decides which kinds are recognized, and the lookup below is what the
// unknown-kind refusal rests on. Built on a `{}`, a planted `Object.prototype[kind]` supplies a
// predicate for a kind the table never carried, and an unrecognized structure is then authorized.
...
//   - `_create(parsed)` is likewise a different object, so it inherits no
//     record, while `_assign({}, parsed)` and `{...parsed}` are different
...
// about its type.
//
// This list is a TYPE DETECTOR, not the integrity boundary, and reads as the latter if skimmed.
// Nothing is trusted because it appears here, and no field is unprotected because it does not:
// fromTrustedSource re-derives the WHOLE structure from the bytes the provenance record names, so a
// field edited on a parse result is discarded whether or not it is listed. Adding every
// security-relevant field would therefore change no verdict. What the list decides is narrower: it
// separates "an object claiming to be a parse result" from "bytes", so the first gets this domain's
// typed refusal instead of being handed to a byte parser.
// Null-prototype, for the same reason as _SHAPES: an inherited entry standing in for an absent one
// makes an unrecognized kind look recognized, and the refusal keyed on its absence never fires.
