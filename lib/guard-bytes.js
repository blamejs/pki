...
//
// A kind this table does not name gets `null`, and the copy refuses rather than reaching for
// `v.constructor` as a fallback: a fallback is the hole, since the kinds a table can miss are
// exactly the ones nobody reviewed. `util.types` gains a predicate as the language gains a kind,
// and `Float16Array` is the live example of one arriving.
var _CONCRETE_KINDS = [
  ["isUint8Array", Uint8Array], ["isUint8ClampedArray", Uint8ClampedArray],
  ["isUint16Array", Uint16Array], ["isUint32Array", Uint32Array],
  ["isInt8Array", Int8Array], ["isInt16Array", Int16Array], ["isInt32Array", Int32Array],
  ["isFloat32Array", Float32Array], ["isFloat64Array", Float64Array],
  ["isBigInt64Array", BigInt64Array], ["isBigUint64Array", BigUint64Array]
].filter(function (row) { return typeof util.types[row[0]] === "function" && row[1]; });

function _concreteKindOf(v) {
  for (var i = 0; i < _CONCRETE_KINDS.length; i++) {
    if (util.types[_CONCRETE_KINDS[i][0]](v)) return _CONCRETE_KINDS[i][1];
  }
  return null;
}
...
module.exports = {
  ...
  isByteSource: isByteSource, lengthOf: lengthOf,
