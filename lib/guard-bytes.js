...
var _NAMED_SCAN_MAX_ELEMENTS = require("./constants").LIMITS.NAMED_SCAN_MAX_ELEMENTS;
...
function asyncStreamOf(x) {
  ...
  var stream = intrinsic.create(null);
...
function _deep(v, ErrorClass, code, label, cap, depth, collect) {
  ...
  if (_isBuffer(v) || _isView(v) || _isArrayBuffer(v)) {
    ...
    if (bytesCopy.byteLength <= _NAMED_SCAN_MAX_ELEMENTS) {
