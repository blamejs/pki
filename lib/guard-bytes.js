...
function _opaqueSurface(v) {
  ...
  if (typeof v.then === "function") return _THENABLE_SURFACE;    // a pending result, not a value
  return null;
}
...
function _opaqueIsSafeToPass(v, surface) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    // construction -- this engine's key handle, a platform object's slots -- and a method belongs
    // to the kind rather than to the caller. What a caller adds, they add by assignment, which is
    // enumerable; that is the case the refusal is for.
    if (!_wasEnumerable(v, keys[i])) continue;
    var value;
    try { value = v[keys[i]]; }
    catch (_e) { return false; }   // unreadable is not "nothing" -- refuse rather than assume
