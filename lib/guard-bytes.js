...
function _storeOf(v)  { return util.types.isDataView(v) ? _dvBuffer.call(v) : _taBuffer.call(v); }
function _offsetOf(v) { return util.types.isDataView(v) ? _dvByteOffset.call(v) : _taByteOffset.call(v); }
function _lengthOf(v) { return util.types.isDataView(v) ? _dvByteLength.call(v) : _taByteLength.call(v); }
...
function _opaqueFieldOutsideSurface(v, surface, atArgument, ErrorClass, code, label) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    // anything. A SYMBOL key is reported by the same walk and judged the same way.
    if (surface[keys[i]]) continue;
    // Below the argument itself, a non-enumerable name is the platform's. Node hangs `_handle` on
    // a KeyObject that way, and which internals a runtime keeps, under which names, is its own
    // business and changes between versions -- a list of them here would be complete on the day it
    // was written and short afterwards, which is how a real operator's key ends up refused.
    //
    // At the argument the rule has no exception, because there the value IS the options bag: every
    // name on it is a name some verb reads as an option, and the bag is handed on by reference, so
    // a field written with `Object.defineProperty` would be read once as an option and stay the
    // caller's to change afterwards. An options bag whose state cannot be copied cannot be one.
    if (!atArgument && !_wasEnumerable(v, keys[i])) continue;
    return keys[i];
  }
  return null;
}
...
function _deep(v, ErrorClass, code, label, cap, depth, collect) {
  ...
  if (surface) {
    ...
    var carried = _opaqueFieldOutsideSurface(v, surface, depth === 0, ErrorClass, code, label);
