...
function _canChangeAfterTheCheck(v, name) {
  ...
  for (var o = v; o && !visited.has(o); o = Object.getPrototypeOf(o)) {
    ...
    if (!d) continue;
    return !!(d.get || d.set || d.writable || d.configurable);
  }
  return false;
}

function _opaqueFieldOutsideSurface(v, surface, ErrorClass, code, label) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (!_canChangeAfterTheCheck(v, keys[i])) continue;
    return keys[i];
  }
  return null;
}
...
function _copyNamed(src, dst, ErrorClass, code, label, cap, depth, collect, behavior) {
  ...
  if (behavior) {
    keys = keys.filter(function (n) { return !behavior[n]; });
  }
