...
// asyncIteratorOf(x) -> the async iterator of a byte STREAM, or null when x is not one.
//
...
// what counts. It reads x's Symbol.asyncIterator EXACTLY ONCE and, when present, ACQUIRES the iterator
// from that single read, returning it for the consumer to drive. Reading the protocol property a
// second time downstream would fire a one-shot or stateful accessor twice and refuse a stream a direct
// `for await` accepts, so the acquired iterator is threaded through rather than the source re-classified.
// A byte source is bytes, never a stream, EVEN when a caller attached a Symbol.asyncIterator to it: the
// byte-source classification wins and is checked FIRST, so a Buffer / typed array / DataView goes down
// the byte path every time and its own accessors are never invoked here. A callable carrying the method
// is a valid async iterable that `for await` consumes, so it is acquired too.
// @enforced-by behavioral -- acquiring an iterator has no rename-proof code shape; the RED vector (a
// stream acquired where a byte source is not, and a one-shot-accessor stream still consumed) is the guard.
function asyncIteratorOf(x) {
  ...
  return _apply(acquire, x, _NO_ARGS);
}
...
module.exports = intrinsic.freeze({
  ...
  isByteSource: isByteSource, asyncIteratorOf: asyncIteratorOf, lengthOf: lengthOf, outputView: outputView,
