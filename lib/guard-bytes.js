...
function _isOpaque(v) {
  if (typeof v.then === "function") return true;                  // a pending result, not a value
  if (v instanceof WeakMap || v instanceof WeakSet) return true;   // deliberately not enumerable
  // A key handle only. isCryptoKeyLike is structural so it can recognize another implementation's
  // key, and a plain object literal can wear that shape -- but a plain object is data this module
  // can read and copy, so it is never opaque whatever it looks like. A real key of any
  // implementation is an instance of that implementation's class.
  var proto = Object.getPrototypeOf(v);
  if (proto === Object.prototype || proto === null) return false;
  return require("./webcrypto").isCryptoKeyLike(v);   // allow:inline-require -- circular load: webcrypto requires this module
}

// Does this opaque object carry data of its own, beyond the surface its kind is defined by? That
// is the case with no safe answer: the object cannot be copied, and passing it through leaves that
// data the caller's to change after the checks have read it.
function _opaqueCarriesData(v) {
  var surface = require("./webcrypto").isCryptoKeyLike(v) ? _CRYPTO_KEY_SURFACE : null;   // allow:inline-require -- circular load: webcrypto requires this module
  for (var key in v) {
    if (!surface || !surface[key]) return true;
  }
  return false;
}
...
function _deep(v, ErrorClass, code, label, cap, depth, collect) {
  ...
  if (Buffer.isBuffer(v) || ArrayBuffer.isView(v) || v instanceof ArrayBuffer) {
    ...
    // A byte view can carry named properties too, and a verb reads an option by name whatever the
    // argument's type: `opts = new Uint8Array(0); opts.pem = true` is an options object as far as
    // `opts.pem` is concerned. An ArrayBuffer takes no properties, so it is left as it is.
    if (!(bytesCopy instanceof ArrayBuffer)) {
  ...
  if (_isOpaque(v)) {
    if (!_opaqueCarriesData(v)) return v;
  ...
  if (v instanceof RegExp || v instanceof Error) return v;   // no caller data of its own to fix
...
function _namesToCopy(src, dst) {
  ...
  if (!indexed && !(dst instanceof Map) && !(dst instanceof Set)) return _reachableKeys(src);
