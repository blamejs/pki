...
function _namesToCopy(src, dst) {
  ...
  // Reflect.ownKeys here too, so a Symbol the caller added to an array or a byte view is carried
  // across like any other name they added. The index test only applies to a string key: Number()
  // on a Symbol throws, and no Symbol is ever an index.
  var own = Reflect.ownKeys(src);
  var out = [];
  for (var i = 0; i < own.length; i++) {
