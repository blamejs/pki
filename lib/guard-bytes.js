...
var _CONCRETE_KINDS = _filter(
  _map(
    _filter(_objectKeys(util.types), function (name) { return /^is[A-Za-z0-9]+Array$/.test(name); }),
...
function _copyBytesSameKind(v, ErrorClass, code, label, collect) {
  ...
  if (collect) _append(collect, _bufferFrom(owned, 0, src.length));
  if (_isBuffer(v)) return _bufferFrom(owned, 0, src.length);
