...
function _reachableKeys(v) {
  ...
  for (var o = v; o && o !== Object.prototype; o = Object.getPrototypeOf(o)) {
    ...
    // While its target stays extensible a `getPrototypeOf` trap may return the proxy itself, and
    // the walk never reaches an end. It therefore stops at an object already visited. A second
    // lap over the same objects yields no name the first did not, so the names stay complete.
    // Without the test a caller's argument hangs the process before any check reads it.
    if (visited.has(o)) break;
    visited.add(o);
    // Reflect.ownKeys, matching guard-identifier's walk: Object.getOwnPropertyNames never returns
    // a Symbol key, so the copy would drop one the caller supplied and the unknown-option check
    // running on the copy would then never see it.
    var own = Reflect.ownKeys(o);
    for (var i = 0; i < own.length; i++) {
      // `constructor` is skipped where it is INHERITED, which is where a class put it and the
      // copy resolves it through the prototype it keeps. An OWN one is a value the caller wrote,
      // so it is carried across like any other: dropping it left `opts.constructor` resolving on
      // the caller's object and absent from the copy the unknown-option check then read.
      if ((own[i] === "constructor" && o !== v) || seen[own[i]]) continue;
      seen[own[i]] = true;
      names.push(own[i]);
    }
  }
...
function _deep(v, ErrorClass, code, label, cap, depth, collect) {
  ...
  if (Array.isArray(v)) {
    var arr = [];
    // The indices that are PRESENT, not a count from zero to `length`. An array is sparse
...
// The prototypes whose members describe a KIND rather than anything a caller supplied. Held by
// identity, and the same set guard-identifier holds, because the two modules answer one question:
// which names on this value did its caller put there.
...
// Every name a lookup on a collection-kind value can reach that its caller put there: the chain
// walked, with the levels that describe a kind skipped. The cycle test is the same one the other
// walks carry, since a Proxy can make the chain endless.
function _reachableKindKeys(v) {
  var names = [];
  var seen = Object.create(null);
  var visited = new Set();
  for (var o = v; o && !visited.has(o); o = Object.getPrototypeOf(o)) {
    visited.add(o);
    if (_KIND_PROTOS.has(o)) continue;
    Reflect.ownKeys(o).forEach(function (k) {
      if (k === "constructor" && o !== v) return;
      if (seen[k]) return;
      seen[k] = true;
      names.push(k);
    });
...
function _namesToCopy(src, dst) {
  ...
  // The value's OWN names, plus any carried on a prototype that is NOT a kind's own prototype.
  //
  // `Object.setPrototypeOf([], { password: "pw" })` is still an array, and `opts.password` still
  // resolves, but there is no kind prototype anywhere in that chain to protect: taking own keys
  // alone dropped the field from the copy while guard-identifier passed over it, so it reached
  // neither the check nor the verb. `_reachableKindKeys` reads the chain and skips only the
  // levels that describe a kind, which is the same question guard-identifier asks, so the set
  // one checks and the set this one copies stay the same set.
  var own = _reachableKindKeys(src);
  var out = [];
  for (var i = 0; i < own.length; i++) {
    // `length` and the indices belong to an ARRAY or a byte view and to nothing else. A Map, a
    // Set and an ArrayBuffer have no own `length`, so skipping the name there would drop one the
    // caller had added, and the copy would hold less than they passed.
    //
    // An index is what the language calls one, which `String(Number(k)) === k` is not: that also
    // admits "-1", "1.5", "NaN", "Infinity" and "4294967295", each an ordinary named property on
    // an array. Dropping those from the copy loses a field the caller set, and guard-identifier
    // passes over the same names, so the option reaches neither the check nor the verb. The rule
    // has to be the same in both, so both ask this question the same way.
    if (indexed && typeof own[i] === "string" &&
        (own[i] === "length" || _isArrayIndex(own[i]))) continue;
    out.push(own[i]);
  }
  return out;
}
