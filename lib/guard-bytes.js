...
function _deep(v, ErrorClass, code, label, cap, depth, collect) {
  ...
  if (_isArray(v)) {
    ...
    // the same.
    //
    // OWN indices only. An array reads its holes through the prototype chain, so an inherited index --
    // a polluted `Array.prototype[i]`, or one a hostile prototype supplies -- would be materialized here
    // as an own element and then signed as though the caller had placed it. Copying own indices alone
    // leaves each hole a hole; the caller-array density check the verbs apply then refuses a sparse or
    // prototype-filled list rather than the copy importing a value the array never held.
