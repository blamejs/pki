...
function _isHandle(v) {
  ...
  // Anything else with a prototype of its own and nothing enumerable ANYWHERE on it: a URL, a
  // stream, a platform object. There is no data in it to copy, so a copy would produce a broken
  // shell. Enumerable INHERITED properties count -- `Object.create({ signedAttributes: true })` has
  // no own keys and is still entirely data, and reading only own keys would have passed it through.
  return _enumerableCount(v) === 0;
}

function _enumerableCount(v) {
  var n = 0;
  // for-in, not Object.keys: an inherited enumerable property is data this object carries too.
  for (var _key in v) n++;
  return n;
}
...
function _deep(v, ErrorClass, code, label, cap, depth, collect) {
  ...
  var keys = [];
  // Every ENUMERABLE property, inherited ones included. A verb reads `opts.signedAttributes`, and
  // that resolves through the prototype chain -- so copying only own keys and keeping the caller's
  // prototype leaves an inherited field live: `Object.create({ signedAttributes: true })` would
  // still see a later write to the prototype. Copied as an OWN property, the inherited value is
  // shadowed and fixed. The prototype is still kept, because a class instance's methods live there
  // and they are not enumerable, so this picks up its data without breaking its behaviour.
  for (var key in v) keys.push(key);
  for (var k = 0; k < keys.length; k++) {
    Object.defineProperty(out, keys[k], {
      value: _deep(v[keys[k]], ErrorClass, code, label, cap, depth + 1, collect),
