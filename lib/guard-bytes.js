...
// The rest of what this module reads from the runtime while it is deciding something. `isView` is
// how it recognizes the shape whose backing store must be refused when shared, and the reflective
// operations are how it inspects a value honestly; a replacement for any of them answers the
// question this guard asked on the caller's behalf. See guard-intrinsic for the whole set.
...
// The array operations, uncurried. They run on arrays this module owns, so `Array.prototype` is
// what a caller can still reach; a `forEach` replaced with a no-op makes a walk over real elements
// report nothing, and whatever is keyed on that walk then passes vacuously.
...
// The constructor the private copy of a caller's Date is built with. A replacement returns an
// object of its own choosing, and the snapshot that is supposed to be ours carries the caller's
// instant only if they let it.
...
// The collection operations. The cycle-safe chain walks are a `Set` this module owns, and the copy
// writes bytes through the typed-array `set`; a replacement for either decides what the walk sees
// or what the copy contains.
...
// The array's OWN element indices below `length`, the positions it actually holds rather than the ones
// a read resolves. A hole reads through the prototype chain, so enumerating what reads resolve would
// materialize an inherited index -- a polluted `Array.prototype[i]` among them -- as an own element on
// the copy, importing a value the array never held. Copying own indices alone leaves the hole a hole,
// so the caller-array density check the verbs apply sees the original sparsity and refuses it, rather
// than the copy silently answering with a prototype-supplied element.
...
// asyncStreamOf(x) -> a LAZY single-use async iterable of x's byte chunks, or null when x is not a
// byte stream.
//
// A door offering both a byte source and a streamed content (an async iterable of byte chunks) asks
// this ONE module which it has, instead of re-inlining the `for await` protocol test and drifting on
// what counts. It reads x's Symbol.asyncIterator EXACTLY ONCE, captures that method, and returns a
// thin iterable that CALLS it only when the consumer drives the stream. Deferring the call matters
// twice: the underlying iterator is acquired once, in the digest engine, so reading the protocol
// property a second time downstream cannot fire a one-shot or stateful accessor and refuse a stream a
// direct `for await` accepts; AND a caller whose iterator factory opens a file or socket has that
// resource opened only on a path that will actually consume it, never on a pre-hash rejection (a
// missing detached option, an empty signer list, a message needing no streamed digest) that would
// otherwise acquire and leak it. A byte source is bytes, never a stream, EVEN when a caller attached a
// Symbol.asyncIterator to it: the byte-source classification wins and is checked FIRST, so a Buffer /
// typed array / DataView goes down the byte path every time and its own accessors are never invoked
// here. A callable carrying the method is a valid async iterable that `for await` consumes, so it too
// yields a stream.
// @enforced-by behavioral -- a lazy stream wrapper has no rename-proof code shape; the RED vectors (a
// stream produced where a byte source is not, a one-shot-accessor stream still consumed, and the
// factory NOT invoked until the stream is driven) are the guard.
...
// A streamed content driven through the engine's digestStream / encryptStream reports a malformed chunk
// or iterator in the ENGINE's domain: a non-byte chunk is webcrypto/data, and a malformed async-iterator
// protocol (a non-callable next, or a factory returning a non-iterator) is webcrypto/syntax. Rethrow both
// in the CALLER's domain through its typed factory E, so the public verb keeps its own <ns>/bad-input
// contract for malformed streamed content rather than leaking the engine code; a caller's own next()
// fault (any other error) is rethrown unchanged. E(code, msg, cause) is the caller's error factory. Always
// throws. The digest / cipher algorithm names each verb passes are its own, so webcrypto/syntax here is
// never an algorithm fault.
...
// from what it holds. `_create(ArrayBuffer.prototype)` inherits everything and holds no
...
// `Function.prototype.call` and hand back whatever store, offset and length a caller chose. The
// capture and the refusal for a runtime that lacks the accessor both live in guard-intrinsic, so
// every guard measuring a view measures it the same way.
...
// The two string operations in this table run AT LOAD, in the same statement that reads
// `util.types` once and for the same reason. Nothing a caller can replace has run yet, so there is
// no live read to remove; capturing them would only move the question one line earlier.
// A util.types predicate that names a concrete typed-array kind: "is" + one-or-more alphanumerics +
// "Array" (e.g. "isUint8Array"), decided character by character so no regex match protocol -- which
// re-dispatches through a replaceable `exec` -- stands between this table and the runtime it reads.
function _isTypedArrayPredName(name) {
  var n = name.length;
  if (n < 8) return false; // "is" + >=1 middle char + "Array"
  if (_charCodeAt(name, 0) !== 105 || _charCodeAt(name, 1) !== 115) return false; // "is"
  var tail = "Array";
...
function _article(name) {
  ...
  var vowel = c === 65 || c === 69 || c === 73 || c === 79 || c === 85 ||
              c === 97 || c === 101 || c === 105 || c === 111 || c === 117; // A E I O U, either case
  return vowel ? "an" : "a";
}
...
// property on any instance, so `_defineProperty(buf, "length", { get: () => 0 })` answers
...
// An identity fast-path (`_isBuffer(x) return x`) that skips the re-view
...
// Whether E's instances would be Errors, decided by walking the prototype chain with the captured
// `getPrototypeOf` rather than by `instanceof`, which asks `Error[Symbol.hasInstance]` at call time.
// The walk carries the same visited set _protoChainIsCyclic uses, because a Proxy can report a
// cyclic chain and an unbounded walk would hang here rather than at the boundary that refuses one.
...
// admits the mirror shape, `_create(Uint8Array.prototype)`, which holds none; that one comes
...
function _opaqueSurface(v, ErrorClass, code, label) {
  ...
  // Deliberately empty, and null-prototype so it stays empty: this surface is the allowed-field set
  // for a value with no enumerable contents, and on a `{}` a planted `Object.prototype[name]` reads
  // back as an allowed field, so a mutable one the caller added would pass the refusal.
  if (util.types.isWeakMap(v) || util.types.isWeakSet(v)) return _create(null);
  // The slot, not the prototype. This picks which surface an opaque object is copied through, and
  // `instanceof` asks `Error[Symbol.hasInstance]`, which a caller can answer. It also admits a real
  // Error from another realm, whose prototype is that realm's and which the prototype test refuses.
  ...
  // to `_create(KeyObject.prototype)`.
  ...
  // without a key behind it answers with a getter that throws: `_create(KeyObject.prototype)`
...
function _copyBytesSameKind(v, ErrorClass, code, label, collect) {
  ...
  // Sized once, from the slot. `src.length` is a replaceable accessor, and it decides three things
  // here at once: how large the private store is, how much of it the returned view covers, and how
  // much of it goes on the wipe list. A number smaller than the truth puts a short Buffer on that
  // list, so `release` clears the front of a copy that still holds the rest of the secret.
...
//
// The object branch does this with `_create(_getPrototypeOf(v))`, so an instance of a
...
function _deep(v, ErrorClass, code, label, cap, depth, collect) {
  ...
  if (_isArray(v)) {
    var arr = [];
    // The indices a read on the array resolves, rather than a count from zero to `length`. An array is
    // sparse whenever a caller sets a high index or assigns `length`, and `[].length = 4e9` is one
    // statement: walking to `length` then does four billion iterations for an array holding nothing,
    // inside a guard whose job includes keeping a caller's argument from costing unbounded work. Copying
    // the indices it actually holds costs what it holds; `length` is carried across afterwards so trailing
    // holes survive and the copy measures the same.
    //
    // OWN indices only, and refuse an index a read resolves through the PROTOTYPE. An array reads its
    // holes through the prototype chain, so an inherited in-range index -- a polluted `Array.prototype[i]`,
    // or one a hostile prototype supplies -- is a value the caller never placed on the array. Materializing
    // it would import it; leaving it a hole would let a verb's map silently skip an element the array
    // appears to hold. Neither is safe, so the copy refuses it, once, for every argument the module copies,
    // rather than each verb having to. A GENUINE hole -- own-absent AND prototype-absent -- is preserved,
    // so an ordinary sparse array round-trips unchanged and the per-verb density check on a list that must
    // be dense still refuses that. `readableIndices` walks own + inherited in-range indices; when it holds
    // one the array does not own, an in-range index came from the prototype.
  ...
  //
  // Having NO prototype is different and is kept. `_create(null)` and what `JSON.parse`
...
// reads every level outside a collection gives `_create(Array.prototype)` an own `length`
...
function _copyEntries(src, dst, ErrorClass, code, label, cap, depth, collect) {
  ...
  // different entries filled the copy with something other than what the collection holds. Both are
  // taken at load and invoked uncurried, so neither the method nor the `call` that reaches it is
  // read off a prototype while a caller is in a position to have replaced it.
...
function fixedCall(ErrorClass, code, args, body) {
  var handle = null;
  // Both halves through captures. `body.apply` and `.finally` are prototype reads, and between them
  // they decide whether the verb runs and whether the copies it was handed are ever cleared.
