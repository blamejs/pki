...
function _isHandle(v) {
  ...
  // isCryptoKeyLike is structural by design -- it has to recognize a key another WebCrypto
  // implementation minted, which is no instance of any class here. Structural means forgeable, so
  // it is not enough on its own: an options bag with `type`, `algorithm`, `extractable` and
  // `usages` added to it satisfies the predicate, and passing that through by reference leaves
  // every other option on it the caller's to rewrite. A key is a key AND nothing else.
  return require("./webcrypto").isCryptoKeyLike(v) && _onlyKeySurface(v);   // allow:inline-require -- circular load: webcrypto requires this module
}

function _onlyKeySurface(v) {
  for (var key in v) {
    if (!_CRYPTO_KEY_SURFACE[key]) return false;
  }
  return true;
}
...
function _copyNamed(src, dst, ErrorClass, code, label, cap, depth, collect) {
  var keys = _reachableKeys(src);
  for (var k = 0; k < keys.length; k++) {
    if (Array.isArray(dst) && (keys[k] === "length" || String(Number(keys[k])) === keys[k])) continue;
    var value;
