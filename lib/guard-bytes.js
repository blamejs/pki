...
// isAsyncIterable(x) -> boolean: is this a byte STREAM rather than a byte source?
//
// The structural test `for await (... of x)` itself uses: a value exposing a Symbol.asyncIterator
// method. Exported so a verb that accepts a streamed content (an async iterable of byte chunks) asks
// the one module about it, instead of each door re-inlining the test and drifting on what counts. A
// byte source is treated as bytes, never as a stream, EVEN when a caller has attached a
// Symbol.asyncIterator to it: the byte-source classification wins and is checked first, so a door
// offering both forms sends a Buffer / typed array / DataView down the byte path every time. The
// exclusion also means a byte source's own accessors are never invoked here.
// A callable is accepted too: a function carrying a Symbol.asyncIterator method is a valid async
// iterable that `for await` consumes, and narrowing to `typeof x === "object"` would reject it at the
// door while the engine would have iterated it.
// @enforced-by behavioral -- a one-line structural test has no rename-proof code shape; the RED
// vector (a streamed content accepted where a byte source is, and refused where it is not) is the guard.
function isAsyncIterable(x) {
  return !isByteSource(x) && x != null && (typeof x === "object" || typeof x === "function") && typeof x[Symbol.asyncIterator] === "function";
}
...
module.exports = intrinsic.freeze({
  ...
  isByteSource: isByteSource, isAsyncIterable: isAsyncIterable, lengthOf: lengthOf, outputView: outputView,
