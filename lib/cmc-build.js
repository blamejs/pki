...
var OID_DECRYPTED_POP = O("id-cmc-decryptedPOP");
...
var KNOWN_SPEC_KEYS = intrinsic.assign(intrinsic.create(null), {
  ...
  identityProof: 1, popLink: 1, renewal: 1, popChallenge: 1,
...
async function _assertTcrPops(requests, popProvenRequest) {
  for (var i = 0; i < requests.length; i++) {
    if (requests[i] && requests[i].tcr != null) await _assertOneTcrPop(requests[i].tcr, i, popProvenRequest);
  }
}
/** @internal The parameters an algorithm identifier carries, which for these four are absent or NULL. */
function _assertNoAlgorithmParameters(algId, what) {
  var params = algId.parameters;
  if (guard.der.paramsAbsentOrNull(params)) return;
  throw E("cmc/bad-pop-challenge",
    "the challenge's " + what + " carries a parameter value, and the algorithms RFC 5274 sec. 4.2 " +
    "names take none: they are encoded with the parameters absent or NULL");
}

/** @internal Whether these bytes are one DER OCTET STRING and nothing else. */
function _isNoSignatureValue(bytes) {
  if (!bytes || !bytes.length) return false;
  var node;
  try { node = asn1.decode(bytes); }
  catch (_e) { return false; } // allow:swallow-unverified bytes that do not decode are not a NoSignatureValue
  return node.tagClass === "universal" && node.tagNumber === asn1.TAGS.OCTET_STRING && !node.constructed;
}

/** @internal A key that can only decrypt cannot sign the request that asks for its certificate, so RFC
 * 5272 App. C.1 gives it a signature algorithm that carries no proof and sends the proof through the
 * sec. 6.7 challenge instead. A request in that form is accepted here only when this message answers a
 * challenge for that same request; without one it carries no proof of possession at all. */
async function _assertOneTcrPop(tcr, i, popProvenRequest) {
  var der = _der(tcr, "a tcr certificationRequest");
  var parsed = csr.parse(der);
  if (parsed.signatureAlgorithm.oid === O("id-alg-noSignature")) {
    var params = parsed.signatureAlgorithm.parameters;
    if (!params || params.length !== 2 || params[0] !== 0x05 || params[1] !== 0x00) {
      throw E("cmc/bad-popo", "a tcr request [" + i + "] uses id-alg-noSignature, whose parameters must be " +
        "present and encoded as NULL (RFC 5272 App. C.1)");
    }
    /** @internal "NoSignatureValue ::= OCTET STRING" (App. C.1), carried in the signature BIT STRING.
     * The section says it holds the hash of the request but names no algorithm, so the encoding is what
     * can be checked here; a value that is not one is a request an authority would reject. */
    var sig = parsed.signatureValue;
    if (!sig || sig.unusedBits !== 0 || !_isNoSignatureValue(sig.bytes)) {
      throw E("cmc/bad-popo", "a tcr request [" + i + "] uses id-alg-noSignature, whose signature is a " +
        "NoSignatureValue OCTET STRING (RFC 5272 App. C.1)");
    }
    if (!popProvenRequest || popProvenRequest.length !== der.length ||
        !guard.crypto.constantTimeEqual(popProvenRequest, der)) {
      throw E("cmc/bad-popo", "a tcr request [" + i + "] is signed with id-alg-noSignature, which proves " +
        "nothing about the key; such a request carries its proof of possession in an answered " +
        "challenge, so pass the one the authority sent as popChallenge (RFC 5272 App. C.1, sec. 6.7)");
    }
    return;
  }
...
/** @internal Controls the builder computes for itself from a spec field. Their value is a proof, so a
 * second one arriving hand-encoded cannot be reconciled with the computed one the way an Identification
 * name can: the two would both be emitted, and a message carrying two proofs of the same thing is one an
 * authority has no rule for. Naming the spec field that produces it says which of the two to drop. */
var GENERATED_BY_OID = intrinsic.create(null);
GENERATED_BY_OID[O("id-cmc-identityProofV2")] = { field: "identityProof" };
GENERATED_BY_OID[O("id-cmc-popLinkWitnessV2")] = { field: "popLink" };
GENERATED_BY_OID[O("id-cmc-popLinkRandom")] = { field: "popLink" };
GENERATED_BY_OID[O("id-cmc-decryptedPOP")] = { field: "popChallenge", perTarget: true };

/** @internal The bodyPartID a Decrypted POP names, which is the request its proof covers. Several may
 * ride in one message, one per request, so it is the target that has to be unique rather than the
 * control. A value that will not read as one is treated as naming nothing, and a second such control is
 * refused because there is no target to tell the two apart by. */
function _decryptedPopTarget(controlValue) {
  try {
    var node = asn1.decode(controlValue);
    return intrinsic.bigIntToString(asn1.read.integer(node.children[0]), 10);
  } catch (_e) { return null; } // allow:swallow-unverified an unreadable control names no request
}

function _assertNotAlsoComputed(controlType, controlValue, seen, spec) {
  var generated = GENERATED_BY_OID[controlType];
  if (!generated) return;
  /** @internal Tested the way the producer tests it. A field this reads as present but the emitting
   * code skips would refuse a message that carries one control, not two. */
  if (spec[generated.field]) {
    throw E("cmc/bad-input",
      "spec." + generated.field + " computes this control, and spec.controls carries one as well; a " +
      "request carries one or the other, since a message with two leaves the authority no proof to " +
      "check (RFC 5272 sec. 6.2.1 / 6.3.1 / 6.7)");
  }
  var target = generated.perTarget ? _decryptedPopTarget(controlValue) : null;
  var slot = generated.perTarget ? controlType + ":" + (target === null ? "?" : target) : controlType;
  if (seen[guard.text.keyOf(slot)]) {
    throw E("cmc/bad-input",
      "spec.controls carries two " + (oid.name(controlType) || controlType) + " controls" +
      (generated.perTarget
        ? (target === null
          ? " that name no readable certification request, so nothing tells the two proofs apart"
          : " naming certification request " + target + ", so the authority has two proofs of one request")
        : "") +
      "; a message with two leaves the authority no proof to check (RFC 5272 sec. 6.2.1 / 6.3.1 / 6.7)");
  }
  seen[guard.text.keyOf(slot)] = true;
}

function _assertNoDuplicateBinding(callerControls, spec) {
  var seen = intrinsic.create(null);
  callerControls.forEach(function (c) {
    var controlOid = _oidOf(c.type);
    var field = BINDING_BY_OID[controlOid];
    if (!field) return;
...
/** @internal RFC 5274 sec. 4.2 names what a conforming client implements for this exchange: SHA-1 MUST
 * and SHA-256 SHOULD for the witness, HMAC-SHA1 MUST and HMAC-SHA256 SHOULD for the proof. Both MUSTs
 * are legacy algorithms and are accepted because the profile requires them of a client answering a
 * challenge, which is a server's choice to make rather than this side's. Anything else is refused: a
 * proof computed under an algorithm the server did not ask for is not an answer. */
var POP_WITNESS_HASH = intrinsic.create(null);
POP_WITNESS_HASH[O("sha1")] = "sha1";
POP_WITNESS_HASH[O("sha256")] = "sha256";
var POP_MAC_HASH = intrinsic.create(null);
POP_MAC_HASH[O("hmacWithSHA1")] = "sha1";
POP_MAC_HASH[O("hmacWithSHA256")] = "sha256";
var POP_MAC_KEY_MAX_BYTES = 64;
var KNOWN_POP_CHALLENGE_KEYS = intrinsic.assign(intrinsic.create(null),
  { challenge: 1, recipient: 1 });

/** @internal The key this challenge is addressed to is the one being certified, which by definition has
 * no certificate yet, so the recipient cannot be selected by matching one. Sec. 6.7 says the identifier
 * "SHOULD be the SKI" of the request, and that an issuerAndSerialNumber form carries a NULL issuer with
 * the request's bodyPartID as the serial: neither names a certificate this caller holds. So each
 * recipient is tried with the key, and the WITNESS decides which attempt recovered the right value,
 * which is the check the section already requires and the only one that can answer the question. A
 * caller who does hold the certificate can still pass it and select the ordinary way. */
var POP_MAX_RECIPIENTS = 8;

/** @internal The CMS recipient kinds that identify a key. A password or a key-encryption key opens the
 * envelope without saying whose it is, so a decryption through one proves possession of no key. */
var ASYMMETRIC_RECIPIENT = intrinsic.assign(intrinsic.create(null), { ktri: 1, kari: 1, ori: 1 });

/** @internal What identifies a request within its arm, which is everything a TaggedRequest carries
 * except the body part identifier the new message is entitled to renumber. Defined once, so the two
 * sides of the comparison below cannot read different things: the other-message arm is identified by
 * its type together with its value, since a value alone could belong to another type. */
function _requestIdentity(parsed) {
  if (!parsed) return null;
  if (parsed.arm === "tcr") return { arm: "tcr", bytes: parsed.certificationRequestBytes };
  if (parsed.arm === "crm") return { arm: "crm", bytes: parsed.certReqMsgBytes };
  if (parsed.arm === "orm" && parsed.requestMessageType) {
    return { arm: "orm",
      bytes: _bufferConcat([b.oid(parsed.requestMessageType), parsed.requestMessageValueBytes]) };
  }
  return null;
}

/** @internal Whether the key that opened the challenge is the key the request asks to have certified.
 * Only a tcr is answered this way: a crm request carries its own proof-of-possession field. */
function _provesRequestKey(opening, outgoing) {
  if (!opening || !outgoing || !outgoing.certificationRequestBytes) return false;
  var requestedSpki;
  try {
    requestedSpki = csr.parse(outgoing.certificationRequestBytes).subjectPublicKeyInfo.bytes;
  } catch (_e) { return false; } // allow:swallow-unverified a request this runtime cannot read proves nothing here
  /** @internal Compared as keys rather than as encodings: an elliptic-curve point written in compressed
   * form and the same point written uncompressed are different bytes and the same key, and it is the key
   * the section is about. */
  if (opening.key !== null) {
    var requested;
    try {
      requested = _createPublicKey({ key: requestedSpki, format: "der", type: "spki" });
    } catch (_e) { return false; } // allow:swallow-unverified a key this runtime cannot read proves nothing here
    /** @internal Keys of different types are never the same key, and comparing them through the
     * runtime's equality leaves a fault queued in the crypto library that surfaces on the next
     * unrelated key import in the process, so the type decides that case first. */
    if (requested.asymmetricKeyType !== opening.key.asymmetricKeyType) return false;
    return intrinsic.keyEquals(requested, opening.key);
  }
  /** @internal A key the runtime engine cannot read has no KeyObject to compare, and a composite
   * ML-KEM key is one. Its encoding answers instead, which is sound because that encoding is
   * canonical: fixed component widths under a single OID, with no compressed-point spelling for two
   * forms of one key to differ over, which is the ambiguity the comparison above exists to absorb. */
  if (!_isCompositeSpki(requestedSpki)) return false;
  return intrinsic.bufferEquals(requestedSpki, opening.spki);
}

/** @internal Whether a SubjectPublicKeyInfo names a composite ML-KEM algorithm, read from its
 * AlgorithmIdentifier alone. Anything this cannot read is not one. */
function _isCompositeSpki(spki) {
  try {
    var node = asn1.decode(spki);
    if (!node.children || node.children.length !== 2) return false;
    var alg = node.children[0];
    if (!alg.children || alg.children.length < 1) return false;
    return compositeKem.isCompositeKem(asn1.read.oid(alg.children[0]));
  } catch (_e) { return false; } // allow:swallow-unverified an unreadable algorithm is not a composite one
}

/** @internal The public half of the key that OPENED the challenge. The recipient kind comes from the
 * decryption that succeeded rather than from the caller's object, so key material the envelope never
 * exercised is not read as the key that answered it. A key whose public half this runtime cannot derive
 * yields null, and the request is then refused rather than accepted on an unproven key. */
async function _openingPublicKey(recipient, recipientType) {
  if (!ASYMMETRIC_RECIPIENT[recipientType]) return null;
  var spki;
  try {
    spki = await key.publicFromPrivate(recipient.key);
  } catch (_e) { return null; } // allow:swallow-unverified a key whose public half cannot be derived proves nothing here
  /** @internal Both spellings of the answer: the KeyObject the comparison prefers, and the encoding a
   * key the runtime engine cannot read has instead of one. A null `key` is not a failure; it says the
   * engine does not know this algorithm, and the comparison below decides what that permits. */
  var asKey;
  try {
    asKey = _createPublicKey({ key: spki, format: "der", type: "spki" });
  } catch (_e) { asKey = null; } // allow:swallow-unverified a composite key the runtime cannot read is compared by its canonical bytes
  return { spki: spki, key: asKey };
}

async function _recoverProofValue(decoded, recipient, witnessHash) {
  /** @internal The count is read off the parse rather than re-walked here, so the structure the parser
   * accepted and the structure this loop iterates cannot disagree. */
  var count = decoded.cmsRecipientCount;
  if (count > POP_MAX_RECIPIENTS) {
    throw E("cmc/bad-pop-challenge",
      "the challenge's enveloped proof value names " + count + " recipients; a challenge is addressed to " +
      "the key being certified, and at most " + POP_MAX_RECIPIENTS + " are tried (RFC 5272 sec. 6.7)");
  }
  for (var i = 0; i < count; i++) {
    var opened;
    try {
      opened = await cmsDecrypt.decrypt(decoded.cms, recipient, { recipientIndex: i });
    } catch (_e) {
      continue;   // allow:swallow-unverified this recipient is not addressed to the supplied key; the witness decides
    }
    var candidate = opened.content;
    var computed = nodeCrypto.createHash(witnessHash).update(candidate).digest();
    var matches = computed.length === decoded.witness.length &&
      guard.crypto.constantTimeEqual(computed, decoded.witness);
    guard.secret.zeroize(computed, CmcError, "cmc/bad-input", "the computed witness");
    /** @internal The kind of recipient that actually opened it travels with the value, so what proved
     * possession is decided by the decryption that happened rather than by the caller's object. */
    if (matches) return { value: candidate, recipientType: opened.recipientType };
    guard.secret.zeroize(candidate, CmcError, "cmc/bad-input", "a rejected proof value");
  }
  throw E("cmc/pop-failed",
    "no recipient of the challenge yielded a proof value hashing to the witness it carries, so this " +
    "client cannot prove possession and the enrollment is abandoned (RFC 5272 sec. 6.7 step 3)");
}

/** @internal The client half of RFC 5272 sec. 6.7: decrypt the challenge, confirm it is the value the
 * server hashed into the witness, and answer with a MAC over the request it carried. The witness check
 * is not separable from the answer, because a client that cannot confirm the value "MUST abort the
 * enrollment process" rather than prove possession of something it could not read. */
async function answerPopChallenge(popChallenge, ids) {
  if (!popChallenge || typeof popChallenge !== "object" || _isArray(popChallenge) || _isBuffer(popChallenge)) {
    throw E("cmc/bad-input", "popChallenge is { challenge, recipient } (RFC 5272 sec. 6.7)");
  }
  guard.identifier.assertKnownKeys(popChallenge, KNOWN_POP_CHALLENGE_KEYS, E, "cmc/bad-input",
    "unknown popChallenge field ");
  var recipient = popChallenge.recipient;
  if (!recipient || typeof recipient !== "object" || _isArray(recipient) || _isBuffer(recipient)) {
    throw E("cmc/bad-input",
      "popChallenge.recipient is the CMS key material that opens the challenge, in the shape " +
      "pki.cms.decrypt takes (RFC 5272 sec. 6.7)");
  }
  /** @internal The challenge arrives from the authority, so bytes that are not DER at all are a
   * malformed challenge like any other and carry the same code rather than the codec's. */
  var challengeBytes = _der(popChallenge.challenge, "the popChallenge challenge");
  var challengeNode;
  try { challengeNode = asn1.decode(challengeBytes); }
  catch (e) {
    throw E("cmc/bad-pop-challenge",
      "the popChallenge challenge is the DER of an EncryptedPOP control value: " +
      ((e && e.message) || "malformed") + " (RFC 5272 sec. 6.7)", e);
  }
  var decoded = cmcFmt.parseEncryptedPop(challengeNode);
  var witnessHash = POP_WITNESS_HASH[guard.text.keyOf(decoded.witnessAlgID.oid)];
  var macHash = POP_MAC_HASH[guard.text.keyOf(decoded.thePOPAlgID.oid)];
  if (!witnessHash || !macHash) {
    throw E("cmc/bad-pop-challenge",
      "the challenge names an algorithm this client does not answer under: RFC 5274 sec. 4.2 requires " +
      "SHA-1 or SHA-256 for the witness and HMAC-SHA1 or HMAC-SHA256 for the proof, got " +
      decoded.witnessAlgID.oid + " and " + decoded.thePOPAlgID.oid);
  }
  /** @internal These four algorithms take no parameters: a SHA-2 identifier carries them absent or as
   * NULL (RFC 5754 sec. 2), and the SHA-1 and HMAC identifiers carry NULL. A parameter value that is
   * neither is not one of the algorithms named, and copying it into the answer would send the authority
   * an identifier it cannot read. */
  _assertNoAlgorithmParameters(decoded.witnessAlgID, "witnessAlgID");
  _assertNoAlgorithmParameters(decoded.thePOPAlgID, "thePOPAlgID");

  var proofValue = null, macKey = null;
  try {
    var recovered = await _recoverProofValue(decoded, recipient, witnessHash);
    proofValue = recovered.value;
    /** @internal "The POP Proof Value is used as the secret value in the HMAC algorithm and the request
     * is used as the data. If the POP Proof Value is greater than 64 bytes, only the first 64 bytes of
     * the POP Proof Value is used as the secret." The data is the TaggedRequest exactly as the
     * challenge carried it, which is the copy the server holds and the reason sec. 6.7 puts the request
     * in the challenge at all: neither side re-encodes it. */
    macKey = _bufferFrom(proofValue.length > POP_MAC_KEY_MAX_BYTES
      ? _subarray(proofValue, 0, POP_MAC_KEY_MAX_BYTES) : proofValue);
    var thePOP = nodeCrypto.createHmac(macHash, macKey).update(decoded.requestBytes).digest();
    return { algBytes: decoded.thePOPAlgID.bytes, thePOP: thePOP, ids: ids,
      recipientSpki: await _openingPublicKey(recipient, recovered.recipientType),
      challengedRequest: _requestIdentity(decoded.request) };
  } finally {
    var owned = [];
    if (proofValue && _isBuffer(proofValue)) guard.list.append(owned, proofValue);
    if (macKey) guard.list.append(owned, macKey);
    if (owned.length) guard.secret.zeroizeAll(owned, CmcError, "cmc/bad-input", "the POP proof value");
  }
}
...
async function _build(spec, signer, opts) {
  ...
  /** @internal Which of the controls the builder computes have already arrived hand-encoded, checked
   * against the bytes being encoded rather than a second read of the caller's object. */
  var generatedSeen = intrinsic.create(null);
  callerControls.forEach(function (c) {
    ...
    _assertNotAlsoComputed(controlType, controlValue, generatedSeen, spec);
    ...
    guard.list.append(controls, encodeControl(controlType, [controlValue], ids));
  });
  ...
  /** @internal The answer to a challenge-response proof of possession (RFC 5272 sec. 6.7 step 4). Its
   * bodyPartID names the certification request in THIS request, not the one the challenge quoted, so
   * the server can tie the proof to what is being asked for now. With more than one request there is
   * no single answer to give, and guessing which one the proof covers is not this side's call. */
  var popProvenRequest = null;
  if (spec.popChallenge) {
    if (encodedRequests.length !== 1) {
      throw E("cmc/bad-input",
        "a challenge-response proof of possession answers for one certification request, and this " +
        "request carries " + encodedRequests.length + " (RFC 5272 sec. 6.7)");
    }
    var answered = await answerPopChallenge(spec.popChallenge, ids);
    /** @internal The proof covers the request the CHALLENGE carried, and the control names the request
     * in THIS message. If those are not the same certification request, the message proves possession
     * of a key for a request it does not contain, so the two are compared on everything but the body
     * part identifier, which the new message is entitled to renumber. */
    var outgoing = cmcFmt.parseTaggedRequest(asn1.decode(encodedRequests[0].der));
    var outgoingId = _requestIdentity(outgoing);
    if (!answered.challengedRequest || !outgoingId ||
        answered.challengedRequest.arm !== outgoingId.arm ||
        answered.challengedRequest.bytes.length !== outgoingId.bytes.length ||
        !guard.crypto.constantTimeEqual(answered.challengedRequest.bytes, outgoingId.bytes)) {
      throw E("cmc/bad-input",
        "the challenge answers for a different certification request than this message carries, so the " +
        "proof would not cover the request it names (RFC 5272 sec. 6.7 step 4)");
    }
    /** @internal What the challenge proved possession of. A request signed with id-alg-noSignature has
     * no signature to prove it, and App. C.1 sends exactly that case here, so this is the request that
     * is then allowed to carry one. Opening the challenge shows possession of the key it was encrypted
     * to, which sec. 6.7 says is the key in the request; a challenge opened by any other key says
     * nothing about the key being certified, so it does not stand in for the missing signature. */
    popProvenRequest = _provesRequestKey(answered.recipientSpki, outgoing) ? outgoingId.bytes : null;
    guard.list.append(controls, encodeControl(OID_DECRYPTED_POP, [b.sequence([
      b.integer(BigInt(encodedRequests[0].bodyPartID)),
      b.raw(answered.algBytes),
      b.octetString(answered.thePOP),
    ])], ids));
  }
  ...
  if (signer && typeof signer === "object" && !_isArray(signer) && signer.mac != null) {
    ...
    try {
      await _assertTcrPops(requests, popProvenRequest);
...
function _copySigners(signer, owned) {
  function one(s) {
    ...
    var sNames = guard.identifier.optionNames(s);
    for (var si = 0; si < sNames.length; si++) {
      out[sNames[si]] = copyValue(s[sNames[si]], sNames[si] === "key");
    }
    return out;
  }
  function copyValue(v, isSecret) {
    ...
    var cNames = guard.identifier.optionNames(v);
    for (var ci = 0; ci < cNames.length; ci++) {
      var cv = v[cNames[ci]];
      c[cNames[ci]] = (Buffer.isBuffer(cv) || cv instanceof Uint8Array) ? _own(Buffer.from(cv), true) : cv;   // allow:byte-source-narrow -- composite key component
    }
    return c;
  }
  ...
  return Array.isArray(signer) ? guard.list.copyMap(signer, one) : one(signer);
}
