...
function _assertKeyOnlySigner(signer, requests) {
  var so = Array.isArray(signer) ? signer[0] : signer;
  if (!so || so.cert != null || so.spki == null) return;   // not a key-only signer
  ...
  requests.forEach(function (req) {
    ...
    var ski;
    try {
      ski = req && req.tcr != null ? _csrSubjectKeyIdentifier(req.tcr)
        : (req && req.crm != null ? _crmSubjectKeyIdentifier(req.crm) : null);
    } catch (_e) { ski = null; }     // a request this layer cannot read declares nothing
    if (req && (req.tcr != null || req.crm != null)) sawRequest = true;
    if (ski) declared.push(ski);
  });
  for (var i = 0; i < declared.length; i++) { if (declared[i].equals(idBytes)) return; }
  if (!sawRequest) {
    throw E("cmc/bad-signer",
  ...
  throw E("cmc/bad-signer",
    "the key-only signer's keyIdentifier is not a Subject Key Identifier declared by any certification " +
    "request in this PKIData; sec. 3.2 requires the request to carry the extension and the SignerInfo to " +
    "name its value, so the CA can bind the signature to the key being enrolled (RFC 5272 sec. 3.2)");
}

// The Subject Key Identifier a CRMF certification request declares, or null. The
// CertTemplate carries extensions directly, so there is no attribute wrapper to
// unwrap -- but the extnValue is the same DER SubjectKeyIdentifier OCTET STRING.
function _crmSubjectKeyIdentifier(crmDer) {
...
// The Subject Key Identifier a PKCS#10 certification request declares, or null.
// Read through the shipped CSR parser and the extensionRequest attribute it
// already decodes, so the extension rules are not restated here.
function _csrSubjectKeyIdentifier(csrDer) {
