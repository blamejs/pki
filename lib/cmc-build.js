...
async function _build(spec, signer, opts) {
  ...
  /** @internal A caller may also write the control directly into `spec.controls`, which is the third
   * place one request can name its client from. Its value is read here so the names can be compared;
   * one that does not read as a UTF8String cannot be compared at all, which is why the pair is refused
   * rather than assumed to agree. */
  var controlIdentifiers = [], controlIdentificationCount = 0, controlIdentifierUnreadable = false;
  for (var ic = 0; ic < callerControls.length; ic++) {
    var cc = callerControls[ic];
    if (!cc || typeof cc !== "object" || _oidOf(cc.type) !== OID_IDENTIFICATION) continue;
    controlIdentificationCount += 1;
    var idNode;
    try { idNode = asn1.decode(_der(cc.value, "an Identification control value")); }
    catch (_e) { controlIdentifierUnreadable = true; continue; }
    /** @internal The control is a UTF8String (RFC 5272 sec. 6.2.3). Another string type carrying the
     * same characters is a different value on the wire, and treating it as the name would let it stand
     * in for the control this request needs while a strict authority refuses the message. */
    if (idNode.tagClass !== "universal" || idNode.tagNumber !== UTF8_STRING_TAG) {
