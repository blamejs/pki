...
var pkiBuild = require("./pki-build");   // reqDenseArray: reject a sparse/nullish caller array as a typed cmc/bad-input
...
var csrVerify = require("./csr-verify");   // verify an embedded tcr request's proof-of-possession before signing it
...
// replaceable properties to answer one thing, and here it decides which controls a request carries.
...
// Verify each embedded PKCS#10 (tcr) request's proof-of-possession before the message is signed. A CSR
// whose self-signature does not verify under its own subjectPKInfo is one the CA will reject (RFC 5272
// sec. 4.1), so refusing it here -- through the shared inbound-PoP seam pki.csr.verify uses -- keeps the
// producer from signing a request that cannot be acted on, the same "do not sign what the recipient
// refuses" discipline the assembled-message readback already applies. The crm / orm arms carry their own
// POP structures (CRMF POPOSigningKey etc.), validated by their own parse; only tcr is a bare CSR.
// `await`, not `.then`: the PoP verdict is read through the native promise job, not the replaceable
// Promise.prototype.then a co-resident could swap to force `ok` true and slip an unverified request past.
...
async function _assertOneTcrPop(tcr, i) {
  // encodeRequest already validated this tcr's structure synchronously (it runs before this async pass),
  // so the re-parse here only recovers the parse the PoP check verifies over; on the shipped path it
  // cannot fail, and a sync throw surfaces as a rejection through build's async boundary regardless.
...
async function _build(spec, signer, opts) {
  ...
  // Density up front: requests is iterated by both the forEach below and the map that encodes it, and a
  // hole would survive either (map/forEach skip it) to reach b.sequence as a native error.
  ...
  // Verify every embedded tcr proof-of-possession BEFORE signing: do not sign a request the CA will
  // reject. `await`, not a `.then` chain, so the PoP gate and the sign are sequenced through native
  // promise jobs a co-resident cannot swap; the finally clears the signer key copy on every path,
  // the rejecting one included.
  try {
...
function _copySigners(signer, owned) {
  ...
  function copyValue(v, isSecret) {
    if (!isSecret) {
      // A public signer field (cert / spki / keyIdentifier DER): any BufferSource is
      // snapshotted to a private Buffer so a caller cannot mutate it in a later turn;
      // an ArrayBuffer or DataView is accepted, not silently turned into an empty object.
      // A non-byte descriptor or scalar (a parsed certificate, a CryptoKey handle) passes
      // through untouched.
      if (guard.bytes.isByteSource(v)) return guard.bytes.snapshotSource(v, CmcError, "cmc/bad-input", "a signer field");
      return v;
    }
    // The private key stays on the ownership contract: a Buffer is copied, a Uint8Array is
    // copied, and every copy is collected so it can be zeroized once signing settles.
