...
async function answerPopChallenge(popChallenge, ids) {
  ...
  try {
    ...
    return { algBytes: decoded.thePOPAlgID.bytes, thePOP: thePOP, ids: ids,
      ...
      certificationRequest: decoded.request.certificationRequestBytes ||
        decoded.request.certReqMsgBytes || null };
  } finally {
    var owned = [];
...
async function _build(spec, signer, opts) {
  ...
  if (spec.popChallenge) {
    ...
    var outgoingBytes = outgoing.certificationRequestBytes || outgoing.certReqMsgBytes || null;
    if (!answered.certificationRequest || !outgoingBytes ||
        answered.certificationRequest.length !== outgoingBytes.length ||
        !guard.crypto.constantTimeEqual(answered.certificationRequest, outgoingBytes)) {
      throw E("cmc/bad-input",
    ...
    popProvenRequest = _provesRequestKey(answered.recipientSpki, outgoing) ? outgoingBytes : null;
