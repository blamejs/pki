...
function makeIdAllocator() {
  ...
  var pending = _create(null);
  var next = 1;
  function assertValid(requested, what) {
    if (typeof requested !== "number" || !_isInteger(requested) || requested < 0 || requested > 4294967295) {
...
function fixedRequestId(req, index) {
  ...
  guard.identifier.assertKnownKeys(req, KNOWN_REQUEST_KEYS, E, "cmc/bad-input", function (k) {
    return "unknown field " + _stringify(k) + " on request " + index +
  ...
  if (req.crm != null) {
    ...
    if (req.bodyPartID != null && _asBigInt(req.bodyPartID, "a crm request bodyPartID") !== _BigInt(crmId)) {
      throw E("cmc/bad-input",
...
function encodeRequest(req, ids, index) {
  if (req.tcr != null) {
    ...
    return { bodyPartID: id, der: b.contextConstructed(0, _bufferConcat([b.integer(_BigInt(id)), _der(req.tcr, "a tcr certificationRequest")])) };
  }
  if (req.crm != null) {
  ...
  return {
    bodyPartID: ormId,
    der: b.contextConstructed(2, _bufferConcat([
      b.integer(_BigInt(ormId)), b.oid(_oidOf(orm.type)), _der(orm.value, "an orm requestMessageValue")])),
  };
}
...
function _der(v, what) {
  if (_isBuffer(v) || v instanceof Uint8Array) return guard.bytes.snapshot(v, CmcError, "cmc/bad-input", what);
...
function encodeControl(attrType, values, ids) {
  ...
  return b.sequence([b.integer(_BigInt(id)), b.oid(attrType), b.set(values)]);
}
...
function identityProofV2(secret, reqSequenceBytes, identity) {
  ...
  try {
    var material;
    if (identity == null) {
      material = _ownSecret(_bufferFrom(secret, "utf8"), owned);
    } else {
      material = _ownSecret(_bufferConcat([
        _ownSecret(_bufferFrom(secret, "utf8"), owned),
        _ownSecret(_bufferFrom(identity, "utf8"), owned),
      ]), owned);
    }
...
function _build(spec, signer, opts) {
  opts = opts || {};
  if (!spec || typeof spec !== "object" || _isArray(spec) || _isBuffer(spec)) {
    throw E("cmc/bad-input", "the CMC request spec must be an object");
  }
  if (typeof opts !== "object" || _isBuffer(opts)) throw E("cmc/bad-input", "pki.cmc.build options must be an object");
  ...
  if (!_isArray(requests)) throw E("cmc/bad-input", "spec.requests must be an array");
  var callerControls = spec.controls || [];
  if (!_isArray(callerControls)) throw E("cmc/bad-input", "spec.controls must be an array");
  ...
  callerControls.forEach(function (c) {
    ...
    guard.identifier.assertKnownKeys(c, KNOWN_CONTROL_KEYS, E, "cmc/bad-input", function (k) {
      return "unknown field " + _stringify(k) + " on a control -- a control is { type, value }, " +
        "and its bodyPartID is allocated by the builder";
    });
    _push(controls, encodeControl(_oidOf(c.type), [_der(c.value, "a control value")], ids));
  });
  ...
  if (spec.identityProof) {
    guard.identifier.assertKnownKeys(spec.identityProof, KNOWN_IDENTITY_PROOF_KEYS, E, "cmc/bad-input",
      function (k) { return "unknown field " + _stringify(k) + " on identityProof -- it is { secret, identity? }"; });
    ...
    if (identity != null) _push(controls, encodeControl(OID_IDENTIFICATION, [b.utf8(identity)], ids));
    _push(controls, encodeControl(OID_IDENTITY_PROOF_V2,
  ...
  if (spec.popLink) {
    guard.identifier.assertKnownKeys(spec.popLink, KNOWN_POP_LINK_KEYS, E, "cmc/bad-input",
      function (k) { return "unknown field " + _stringify(k) + " on popLink -- it is { secret }"; });
    var rBytes = opts.popLinkRandomBytes == null ? POP_LINK_RANDOM_BYTES : opts.popLinkRandomBytes;
    if (typeof rBytes !== "number" || !_isInteger(rBytes) || rBytes < 1) {
    ...
    // the API offers.
    _push(controls, encodeControl(OID_POP_LINK_RANDOM, [b.octetString(R)], ids));
    _push(controls, encodeControl(O("id-cmc-popLinkWitnessV2"),
...
function _copySigners(signer, owned) {
  ...
  function copyValue(v, isSecret) {
    if (_isBuffer(v) || v instanceof Uint8Array) return _own(_bufferFrom(v), isSecret);
    if (!v || typeof v !== "object" || typeof v.type === "string") return v;   // CryptoKey / scalar
    var c = {}, ck;
    for (ck in v) {
      if (!Object.prototype.hasOwnProperty.call(v, ck)) continue;
      var cv = v[ck];
      c[ck] = (_isBuffer(cv) || cv instanceof Uint8Array) ? _own(_bufferFrom(cv), isSecret) : cv;
    }
    return c;
  }
  ...
  function _own(buf, isSecret) {
    if (isSecret && owned) _push(owned, buf);
    return buf;
  }
  return _isArray(signer) ? signer.map(one) : one(signer);
}
...
function _assertKeyOnlySigner(signer, requests) {
  var all = _isArray(signer) ? signer : [signer];
...
function _assertKeyOnlySignerBinding(so, requests) {
  ...
  var idBytes = (_isBuffer(id) || id instanceof Uint8Array) ? _bufferFrom(id) : null;
  if (!idBytes) {
    throw E("cmc/bad-signer",
      "a key-only signer must name the Subject Key Identifier its certification request declares (RFC 5272 sec. 3.2)");
  }
  var signerSpki = (_isBuffer(so.spki) || so.spki instanceof Uint8Array) ? _bufferFrom(so.spki) : null;
  if (!signerSpki) {
...
function _crmKeyIdentity(crmDer) {
  ...
  return { ski: found, spki: _isBuffer(pk) ? pk : (pk && pk.bytes) || null };
}
