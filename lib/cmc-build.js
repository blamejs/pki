...
function _provesRequestKey(openingKey, outgoing) {
  if (!openingKey || !outgoing || !outgoing.certificationRequestBytes) return false;
  var requested;
  ...
  try {
    requested = _createPublicKey({
      key: csr.parse(outgoing.certificationRequestBytes).subjectPublicKeyInfo.bytes,
      format: "der", type: "spki",
    });
  } catch (_e) { return false; } // allow:swallow-unverified a key this runtime cannot read proves nothing here
  return intrinsic.keyEquals(requested, openingKey);
}
...
async function _openingPublicKey(recipient, recipientType) {
  ...
  try {
    return _createPublicKey({
      key: await key.publicFromPrivate(recipient.key), format: "der", type: "spki",
    });
