...
function _akiKeyId(val, ctx) {
  if (_isBuffer(val)) return guard.bytes.snapshot(val, CertificateError, "x509/bad-input", "the authorityKeyIdentifier keyIdentifier");
  if (val === true) {
...
function _buildExtensions(extSpec, ctx) {
  ...
  guard.identifier.assertKnownKeys(extSpec, KNOWN_EXT_KEYS, _err, "x509/bad-input", function (k) {
    return "unknown extension " + _stringify(k) + " in the extensions spec; pass a pre-encoded Extension DER via the array form for a custom extension";
  });
  ...
  var out = [];
  if (extSpec.subjectKeyIdentifier != null) _push(out, _ext(O("subjectKeyIdentifier"), false, _extSki(_skiKeyId(extSpec.subjectKeyIdentifier, ctx.spki))));
  if (extSpec.authorityKeyIdentifier != null) _push(out, _ext(O("authorityKeyIdentifier"), false, _extAki(_akiKeyId(extSpec.authorityKeyIdentifier, ctx))));
  if (ku != null) _push(out, _ext(O("keyUsage"), extSpec.keyUsageCritical !== false, _extKeyUsage(ku)));
  if (extSpec.extendedKeyUsage != null) _push(out, _ext(O("extKeyUsage"), !!extSpec.extendedKeyUsageCritical, _extExtKeyUsage(extSpec.extendedKeyUsage)));
  if (bc != null) {
    ...
    _push(out, _ext(O("basicConstraints"), bc.critical !== false, _extBasicConstraints(bc)));
  }
  if (extSpec.subjectAltName != null) _push(out, _ext(O("subjectAltName"), ctx.subjectEmpty, _extSan(extSpec.subjectAltName)));
  if (extSpec.certificatePolicies != null) _push(out, _ext(O("certificatePolicies"), !!extSpec.certificatePoliciesCritical, _extCertPolicies(extSpec.certificatePolicies)));
  return out;
}
...
function _issuedCaInfo(extSpec) {
  ...
  if (!_isArray(extSpec)) {
    var bc = extSpec.basicConstraints;
    return { cA: !!(bc && bc.cA === true), pathLen: bc && bc.pathLen != null ? _Number(bc.pathLen) : null };
  }
  for (var i = 0; i < extSpec.length; i++) {
    ...
    return { cA: dec.cA === true, pathLen: dec.pathLenConstraint != null ? _Number(dec.pathLenConstraint) : null };
  }
...
function _sign(spec, issuer, opts) {
  opts = opts || {};
  if (!spec || typeof spec !== "object" || _isBuffer(spec)) throw _err("x509/bad-input", "the certificate spec must be an object");
  issuer = issuer || {};
  ...
  var tbsChildren = [];
  if (version !== 1) _push(tbsChildren, b.explicit(0, b.integer(_BigInt(version - 1))));   // v2->INTEGER 1, v3->INTEGER 2
  _push(tbsChildren, serialTlv);
  _push(tbsChildren, scheme.sigAlgId);   // signature == signatureAlgorithm (RFC 5280 sec. 4.1.1.2), single source
  _push(tbsChildren, issuerDer);
  _push(tbsChildren, validityDer);
  _push(tbsChildren, subjectDer);
  _push(tbsChildren, b.raw(spki));
  if (exts.length) _push(tbsChildren, b.explicit(3, b.sequence(exts)));
