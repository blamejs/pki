...
function _atvString(attrName, value) {
  if (attrName === "countryName") return b.printable(value);
...
function _assertValidNameDer(der) {
  ...
  if (n.tagNumber !== asn1.TAGS.SEQUENCE || n.tagClass !== "universal") throw _err("x509/bad-name", "a raw Name must be an RDNSequence SEQUENCE");
  (n.children || []).forEach(function (rdn) {
    if (rdn.tagNumber !== asn1.TAGS.SET || rdn.tagClass !== "universal" || !rdn.children || !rdn.children.length) throw _err("x509/bad-name", "each RelativeDistinguishedName must be a non-empty SET");
    rdn.children.forEach(function (atv) {
      if (atv.tagNumber !== asn1.TAGS.SEQUENCE || atv.tagClass !== "universal" || !atv.children || atv.children.length < 2) throw _err("x509/bad-name", "each AttributeTypeAndValue must be a SEQUENCE { type, value }");
      try { asn1.read.oid(atv.children[0]); }
      catch (e) { throw _err("x509/bad-name", "an AttributeTypeAndValue type is not an OBJECT IDENTIFIER", e); }
    });
  });
}
...
function _buildExtensions(extSpec, ctx) {
  ...
  if (Array.isArray(extSpec)) {
    // RFC 5280 sec. 4.2 -- at most one instance of a given extension; the object form dedups by key, so
    // the array form enforces it explicitly.
    var seenExt = {};
    return extSpec.map(function (e, i) {
...
function _assertValidExtension(der, idx) {
  ...
  if (n.children.length === 3) {
    try { asn1.read.boolean(n.children[1]); }
...
// A node private-KeyObject from the signing-key input, or null for a form whose public half is not
// derivable here (a WebCrypto CryptoKey); an unparseable DER/PEM re-throws typed (never swallowed).
function _asPrivateKeyObject(key) {
  if (key && typeof key === "object" && !Buffer.isBuffer(key) && !(key instanceof Uint8Array) && typeof key.export === "function") return key;
  try {
    if (Buffer.isBuffer(key) || key instanceof Uint8Array) return nodeCrypto.createPrivateKey({ key: Buffer.from(key), format: "der", type: "pkcs8" });
    if (typeof key === "string") return nodeCrypto.createPrivateKey(key);
  } catch (e) { throw _err("x509/bad-input", "the signing key could not be parsed as a PKCS#8 private key", e); }
  return null;   // a CryptoKey handle -- the signOverTbs path still fails closed on an incompatible key
}
// Verify the signing key's public half matches the issuer public key the scheme was resolved from, so
// a mismatched-but-same-algorithm key pair cannot silently produce a certificate whose signature does
// not verify under the issuer's public key (a non-chaining certificate). OID-agnostic: it compares the
// raw subjectPublicKey bytes. A composite {mldsa, trad} key is the caller's to pair (skipped here).
function _assertSigningKeyMatches(key, issuerSpki, scheme) {
  if (scheme.composite) return;
  var priv = _asPrivateKeyObject(key);
  if (!priv) return;
  var derived;
  try { derived = nodeCrypto.createPublicKey(priv).export({ type: "spki", format: "der" }); }
  catch (e) { throw _err("x509/bad-input", "could not derive the public key from the signing key", e); }
  var got = asn1.read.bitString(asn1.decode(derived).children[1]).bytes;
  var want = asn1.read.bitString(asn1.decode(issuerSpki).children[1]).bytes;
  if (Buffer.compare(got, want) !== 0) throw _err("x509/bad-input", "the signing key does not correspond to the issuer public key -- the certificate would not chain");
}
...
function _sign(spec, issuer, opts) {
  ...
  // The signing key MUST correspond to the issuer public key, or the certificate would not chain.
  _assertSigningKeyMatches(issuer.key, issuerSpki, scheme);
