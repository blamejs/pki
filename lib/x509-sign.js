...
// This test picks which FORM the caller's extension spec is in, so it decides which fields are read
// and therefore what is signed into the certificate. Read live, a replacement answering true sends
// an object down the list arm and the extensions it carries are never seen.
...
// The three caller-owned objects pki.x509.sign reads, each with its own door. A key nothing here
// names is a typo or a misplaced argument, and the failure it causes is invisible: the certificate
// builds, signs, and simply does not carry what was asked for. `spec.extension` (one letter short
// of `extensions`) emitted a certificate with NO extensions at all -- no basicConstraints on an
// intended CA, no keyUsage on an intended constrained leaf. `spec.issuer` is the other shape: the
// issuer is argument TWO, and nesting it in the spec left the certificate SELF-SIGNED with nothing
// to read as a failure.
//
// Every table is derived from what the function actually reads, not from the doc block, because a
// table naming a key nothing reads reopens the hole it was added to close.
var KNOWN_SPEC_KEYS = {
  subject: 1, subjectPublicKey: 1, notBefore: 1, notAfter: 1, serialNumber: 1, extensions: 1,
};
// The issuer names itself in one of two FORMS, and the cert form wins unconditionally: when
// `issuer.cert` is present the builder takes the DN and the public key from it and never reads
// `name` or `publicKey`. A union accepted `{ key, cert, name: "Different CA", publicKey }` and
// signed under the certificate's identity while discarding the identity the caller wrote out.
 ...
 * fail-closed into its form (`"host.example"`, `"a@b.example"`, `"10.0.0.1"`, `"https://h/"`).
...
function _sign(spec, issuer, opts) {
  ...
  issuer = issuer || {};
  // Before a single field is read, so a misspelling is a refusal rather than an omission carried
  // into a signed certificate. The issuer message names its own argument because the mistake this
  // catches most often is putting the issuer inside the spec, where it self-signed in silence.
  ...
  // Read ONCE. Three separate consumers test which form this spec is in -- the encoder, the
  // empty-subject rule below and the path-length rule under it -- and each answering for its own
  // read is three chances to answer differently about one certificate. The door deep-copies the
  // spec, so today they cannot disagree; taking the value once means they still cannot if the
  // shape of that door ever changes.
