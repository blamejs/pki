 ...
 * private key passed directly, without exporting), a PKCS#8 private key as DER (`Buffer`) or PEM
 * (`string`), or a signer `{ algorithm, publicKey, sign }` for a key this process cannot export,
 * such as one in a hardware security module, a cloud key management service, a PKCS#11 token or a
 * PIV slot. `sign(bytes)` returns the bytes WebCrypto would return for that algorithm, which for
 * ECDSA is the fixed-width `r || s` of RFC 9053 sec. 2.1, and holds its client in its own closure,
 * since this verb snapshots its options. A public or secret CryptoKey,
 * or a `node:crypto` KeyObject, is refused. A `subjectAltName`
