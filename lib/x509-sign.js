  ...
  if (Array.isArray(extSpec)) {
    ...
    var arr = extSpec.map(function (e, i) {
      ...
      if (dec) {
        ...
        if (extnId === oidBc) { arrCa = decoded.cA === true; arrPathLen = decoded.pathLenConstraint != null; }
...
// A supplied issuer certificate MUST be a CA that may sign certificates: basicConstraints cA=TRUE
// (RFC 5280 sec. 4.2.1.9), and -- when a keyUsage extension is present -- the keyCertSign bit (sec.
// 4.2.1.3). Refuse a non-CA issuer rather than mint a certificate that will not chain.
function _assertIssuerIsCa(issuerCert) {
  var exts = issuerCert.extensions || [];
  function extVal(name) { var e = exts.filter(function (x) { return x.oid === O(name); })[0]; return e ? e.value : null; }
  var bcVal = extVal("basicConstraints");
  if (!bcVal) throw _err("x509/bad-input", "the issuer certificate is not a CA -- it has no basicConstraints extension (RFC 5280 sec. 4.2.1.9)");
  var bc;
  try { bc = EXT_DECODERS[O("basicConstraints")](bcVal); }
  ...
  var kuVal = extVal("keyUsage");
  if (kuVal) {
    var ku;
    try { ku = EXT_DECODERS[O("keyUsage")](kuVal); }
