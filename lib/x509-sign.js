...
function _akiKeyId(val, ctx) {
  var issuerSki = ctx.issuerCert ? _skiValueOf(ctx.issuerCert) : null;
...
function _extension(name, value, opts) {
  ...
  if (row === null) {
    ...
    return _ext(name, opts.critical === true, guard.bytes.snapshotSource(value, CertificateError, "x509/bad-input", "the pre-encoded extnValue"));
  }
