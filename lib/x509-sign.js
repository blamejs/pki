...
var KNOWN_EXT_KEYS = intrinsic.assign(intrinsic.create(null), {
  ...
  precertificatePoison: 1, signedCertificateTimestampList: 1, signedCertificateTimestampListCritical: 1,
});
...
function _buildExtensions(extSpec, ctx) {
  ...
  if (extSpec.signedCertificateTimestampList != null) {
    guard.list.append(out, _ext(O("signedCertificateTimestampList"), !!extSpec.signedCertificateTimestampListCritical,
 ...
 * embeds them in the certificate issued after the log answers (sec. 3.3, at least one). It is emitted
 * non-critical unless `signedCertificateTimestampListCritical` is set. The two name opposite ends of
 * one exchange, so a spec naming both is refused. A client recovers the signed entry from the issued
 * certificate with `pki.ct.x509CertEntry`.
 *
 * @opts
