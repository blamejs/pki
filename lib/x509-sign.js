...
// RFC 5280 sec. 4.1.2.5 -- UTCTime through 2049, GeneralizedTime from 2050, chosen per-date.
function _timeDer(date, which) {
  guard.time.assertValid(date, _err, "x509/bad-input", "certificate " + which);
  return date.getUTCFullYear() <= 2049 ? b.utcTime(date) : b.generalizedTime(date);
}
...
function _sign(spec, issuer, opts) {
  ...
  // RFC 5280 sec. 4.1.2.6 -- an empty subject requires a critical subjectAltName.
  if (subjectEmpty && !(spec.extensions && spec.extensions.subjectAltName)) {
