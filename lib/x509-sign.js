...
function _akiFields(val, ctx) {
  ...
  return [val.keyIdentifier == null ? null : _akiKeyId(val.keyIdentifier, ctx),
    val.authorityCertIssuer == null ? null : val.authorityCertIssuer,
    val.authorityCertSerialNumber == null ? null : val.authorityCertSerialNumber];
}
...
function _buildExtensions(extSpec, ctx) {
  ...
  if (extSpec.authorityKeyIdentifier != null) {
    var akiF = _akiFields(extSpec.authorityKeyIdentifier, ctx);
 ...
 * both present or both absent, which is the rule the reader applies.
 *
