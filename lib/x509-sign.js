 ...
 * `{ semanticsIdentifier, nameRegistrationAuthorities }`, at least one of the two. Any other
 * statement id is reachable by supplying `info` as pre-encoded DER, since nothing here can know its
 * syntax. `pki.path.validate` does not process this extension, so it is emitted non-critical unless
 * `qcStatementsCritical` is set.
 *
 * @opts
