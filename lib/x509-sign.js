...
var _encodeName = _b.encodeName, _isEmptyName = _b.isEmptyName, _reqDer = _b.reqDer,
  ...
  _encodeGeneralNames = _b.encodeGeneralNames,
...
function _buildExtensions(extSpec, ctx) {
  ...
  if (extSpec.issuerAltName != null) guard.list.append(out, _ext(O("issuerAltName"), false, _encodeGeneralNames(extSpec.issuerAltName)));
 ...
 * 4.2.1.5 forbids. That clause states a SHOULD for marking it critical, and `pki.path.validate` does
 * not process the extension, so it is emitted non-critical unless `policyMappingsCritical` is set.
