...
var _dnCharCodeAt = intrinsic.uncurry(String.prototype.charCodeAt);
var _dnKeys = intrinsic.keys;
var _dnHasOwn = intrinsic.hasOwn;
...
function _buildExtensions(extSpec, ctx) {
  ...
  if (_isArray(extSpec)) {
    ...
    var oidBc = O("basicConstraints"), oidKu = O("keyUsage"), oidNc = O("nameConstraints"), oidPm = O("policyMappings");
    var arr = extSpec.map(function (e, i) {
      ...
      if (dec) {
        ...
        else if (extnId === oidPm) { _assertNoAnyPolicyMapping(decoded); }
      }
      return b.raw(der);
    });
...
/** @internal RFC 5280 sec. 4.2.1.5: "Policies MUST NOT be mapped either to or from the special
 *  value anyPolicy." The object form's encoder refuses it as it builds; the pre-encoded form is
 *  held to the same clause on the decoded value. */
function _assertNoAnyPolicyMapping(mappings) {
  var anyPolicy = O("anyPolicy");
  for (var i = 0; i < mappings.length; i++) {
    if (mappings[i].issuerDomainPolicy === anyPolicy || mappings[i].subjectDomainPolicy === anyPolicy) {
      throw _err("x509/bad-input", "pre-encoded policyMappings entry " + i + " maps to or from anyPolicy, which RFC 5280 sec. 4.2.1.5 forbids");
    }
  }
}
 ...
 * private key passed directly, without exporting), a `node:crypto` `KeyObject`, a PKCS#8 private
 * key as DER (`Buffer`) or PEM (`string`), or a signer `{ algorithm, publicKey, sign }` for a key
 * this process cannot export, such as one in a hardware security module, a cloud key management
 * service, a PKCS#11 token or a PIV slot. A `KeyObject` signs through `node:crypto`, so a key that
 * refuses to export its private half still signs. `sign(bytes)` returns the bytes WebCrypto would
 * return for that algorithm, which for ECDSA is the fixed-width `r || s` of RFC 9053 sec. 2.1, and
 * holds its client in its own closure, since this verb snapshots its options. A public or secret
 * key is refused whichever form carries it. A `subjectAltName`
 ...
 * 4.2.1.5 forbids, the same rule `pki.path.validate` applies at sec. 6.1.4(a); the pre-encoded
 * form is held to it too. That clause states a SHOULD for marking it critical. The validator
 * processes the extension on an intermediate certificate, where sec. 6.1.4 applies the mappings to
 * the policy tree, and treats a critical one on the TARGET certificate as unprocessed, rejecting it;
 * `pki.lint.certificate` grades that case `unknown-critical-extension`. It is emitted non-critical
 * unless `policyMappingsCritical` is set.
...
/**
 * @primitive  pki.x509.parseDn
 * @signature  pki.x509.parseDn(dn) -> { rdns, dn, bytes }
 * @since      0.8.2
 * @status     stable
 * @spec       RFC 4514
 * @related    pki.x509.sign, pki.schema.x509.parse
 *
 * Read a distinguished-name string into the `Name` a builder takes, using the attribute names
 * and value escaping of RFC 4514 sec. 2.4 and sec. 3. It is the inverse of the `dn` string
 * every parsed name carries: `bytes` is the encoded `Name`, ready to hand to `pki.x509.sign`
 * as a `subject` or `issuer`, and `dn` is the string re-emitted from what was read, which for
 * a string this toolkit produced is the string it was given.
 *
 * Components are read in the order they are written, so the first one written is the first
 * element of the encoded `Name`. That is the order every `dn` here is emitted in, and the
 * order `openssl x509 -subject` prints. RFC 4514 sec. 2.1 specifies the opposite order, so a
 * string from a directory tool that follows it names the components from last to first;
 * reverse them before parsing one.
 *
 * The string form does not carry the ASN.1 string type of each value, so a parsed value takes
 * the type this toolkit encodes that attribute with: `PrintableString` for `countryName`,
 * `IA5String` for `emailAddress`, `UTF8String` otherwise. A name written by something that
 * chose different types will re-encode to different bytes, which is why RFC 4514 sec. 2.4
 * defines the `#`-prefixed hex form: a value given that way is the exact DER of the
 * `AttributeValue` and round-trips byte for byte.
 *
 * Malformed input throws `x509/bad-dn`. A non-string throws `x509/bad-input`.
 *
 * `bytes` is raw `Name` DER, the form `spec.subject` and `spec.issuer` take alongside a string
 * or an array of RDNs, so a name read from a string can be signed without rebuilding it.
 *
 * @example
 ...
 */
function parseDn(dn) {
  if (typeof dn !== "string") throw _err("x509/bad-input", "pki.x509.parseDn takes a distinguished-name string");
  var components = guard.name.parseDnString(dn, _err, "x509/bad-dn", "the distinguished name");
  var spec = [];
  for (var i = 0; i < components.length; i++) {
    var rdn = components[i], out = intrinsic.create(null);
    for (var j = 0; j < rdn.length; j++) {
      var atv = rdn[j];
      var attrName;
      try { attrName = _dnAttrName(atv.type); }
      catch (e) {
        if (e instanceof CertificateError && e.code === "x509/bad-dn") throw e;
        throw _err("x509/bad-dn", "the distinguished name has an attribute type that does not resolve: " + guard.text.showValue(atv.type), e);
      }
      if (_dnHasOwn(out, attrName)) {
        throw _err("x509/bad-dn", "the distinguished name repeats " + attrName + " inside one relative distinguished name");
      }
      out[attrName] = atv.hex === null ? _dnLiteralValue(atv.value) : "#" + atv.hex;
    }
    guard.list.append(spec, out);
  }
  var bytes, read;
  try {
    bytes = _b.encodeName(spec);
    read = schema.walk(NAME_SCHEMA, asn1.decode(bytes), NS).result;
  } catch (e) {
    throw _err("x509/bad-dn", "the distinguished name does not encode: " + (e && e.message), e);
  }
  return { rdns: read.rdns, dn: read.dn, bytes: bytes };
}

function _dnAttrName(type) {
  if (oid.isDottedDecimal(type)) {
    var byOid = oid.name(type);
    if (!byOid) throw _err("x509/bad-dn", "the distinguished name names an unregistered attribute OID " + type);
    return byOid;
  }
  var wanted = guard.name.lowerAscii(type);
  var shortNames = pkix.DN_SHORT;
  var keys = _dnKeys(shortNames);
  for (var i = 0; i < keys.length; i++) {
    if (guard.name.lowerAscii(shortNames[keys[i]]) === wanted || guard.name.lowerAscii(keys[i]) === wanted) return keys[i];
  }
  var table = oid.all();
  var dotted = _dnKeys(table);
  for (var j = 0; j < dotted.length; j++) {
    var registered = table[dotted[j]];
    if (guard.name.lowerAscii(registered) === wanted) return registered;
  }
  throw _err("x509/bad-dn", "the distinguished name names an unknown attribute " + guard.text.showValue(type));
}

function _dnLiteralValue(value) {
  var first = _dnCharCodeAt(value, 0);
  return (first === 0x23 || first === 0x5c) ? "\\" + value : value;
}

module.exports = { sign: sign, randomSerial: randomSerial, extension: extension, parseDn: parseDn };
