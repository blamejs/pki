...
var TPM_POLICY_PROFILES = {
  "hardware-bound": { fixedTPM: true, fixedParent: true, sensitiveDataOrigin: true, sign: true, restricted: false, x509sign: false },
};
var _TPM_POLICY_KEYS = { profile: 1, objectAttributes: 1, reservedBitsClear: 1, consistency: 1, authPolicy: 1 };
...
function normalizeObjectAttributePolicy(policy, E, code) {
  ...
  if (ap !== undefined) {
    ...
    if (ap.allow !== undefined && !Array.isArray(ap.allow)) throw new E(code, "opts.tpmPolicy.authPolicy.allow must be an array");
  }
  return { objectAttributes: want, reservedBitsClear: policy.reservedBitsClear === true,
    consistency: policy.consistency === true, authPolicy: ap || null };
}
...
function assertObjectAttributePolicy(pub, policy, E, policyCode, structuralCode) {
  ...
  if (policy.consistency) {
    // Part 2 sec. 8.3.3: restricted signing keys and the sign/decrypt pair have defined
    // combinations; a TPM2B_DIGEST is at most 64 octets.
  ...
  if (Array.isArray(ap.allow)) {
    var got = pub.authPolicy;
    var ok = ap.allow.some(function (entry) {
      var want = Buffer.isBuffer(entry) ? entry : Buffer.from(String(entry), "hex");
