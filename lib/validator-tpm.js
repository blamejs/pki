...
// Every operation this validator decides a TPM structure with, captured at load. A packed decode is
// a chain of size reads and byte slices and the key binding is a magnitude comparison, so a replaced
// `subarray` moves where a field starts and a replaced `compare` makes any pubArea key equal any
// credential key. See guard-intrinsic.
var intrinsic = require("./guard-intrinsic");
var _subarray = intrinsic.uncurry(Uint8Array.prototype.subarray);
...
function _uintBytes(n) {
  var hex = _numToString(n, 16); if (hex.length % 2) hex = "0" + hex;
  var b = _bufferFrom(hex, "hex"); var i = 0; while (i < b.length - 1 && b[i] === 0) i++; return _subarray(b, i);
}
...
// it was noticed.
// allow:guard-reads-runtime-live -- module-init, the bit table is built before any caller runs
...
function _decodeAttributes(oa) {
  var out = {};
  _keys(TPMA_OBJECT_BITS).forEach(function (n) { out[n] = ((oa >>> TPMA_OBJECT_BITS[n]) & 1) === 1; });
  return out;
}
...
function parsePubArea(buf, E, code) {
  ...
  var pub = { type: type, nameAlg: nameAlg, nameAlgBytes: _subarray(buf, 2, 4),
  ...
  if (type === TPM_ALG.RSA) {
    ...
    throw new E(code, "unsupported TPMT_PUBLIC type 0x" + _numToString(type, 16));
  }
...
function pubKeyEqualsCose(pub, cose, E, mismatchCode, code) {
  if (pub.type === TPM_ALG.ECC) {
    if (cose.kty !== 2 || TPM_CURVE_TO_COSE[pub.curveId] !== cose.crv || !_ucmp(pub.x, cose.x || _bufferAlloc(0)) || !_ucmp(pub.y, cose.y || _bufferAlloc(0))) {
  ...
  if (pub.type === TPM_ALG.RSA) {
    ...
    if (cose.kty !== 3 || !_ucmp(pub.rsa, cose.n || _bufferAlloc(0)) || !_ucmp(e, cose.e || _bufferAlloc(0))) {
...
// allow:guard-reads-runtime-live -- module-init, the profile table is built before any caller runs
...
// allow:guard-reads-runtime-live -- module-init, both key tables are built before any caller runs
...
function normalizeObjectAttributePolicy(policy, E, code) {
  ...
  if (!policy || typeof policy !== "object" || _isArray(policy)) throw new E(code, "opts.tpmPolicy must be an object");
  ...
  if (policy.profile !== undefined) {
    ...
    if (!preset) throw new E(code, "opts.tpmPolicy.profile " + _stringify(policy.profile) + " is not a defined profile");
    _keys(preset).forEach(function (n) { want[n] = preset[n]; });
  }
  ...
  if (explicit !== undefined) {
    if (!explicit || typeof explicit !== "object" || _isArray(explicit)) throw new E(code, "opts.tpmPolicy.objectAttributes must be an object");
    _keys(explicit).forEach(function (n) {
      if (TPMA_OBJECT_BITS[n] === undefined) throw new E(code, "opts.tpmPolicy.objectAttributes has an unknown attribute " + _stringify(n));
  ...
  var allow = null;
  if (ap !== undefined) {
    if (!ap || typeof ap !== "object" || _isArray(ap)) throw new E(code, "opts.tpmPolicy.authPolicy must be an object");
    ...
    if (ap.allow !== undefined) {
      if (!_isArray(ap.allow)) throw new E(code, "opts.tpmPolicy.authPolicy.allow must be an array");
      ...
      // and decode another.
      allow = _map(ap.allow, function (entry, i) {
        if (_isBuffer(entry)) return guard.bytes.snapshot(entry, E, code, "opts.tpmPolicy.authPolicy.allow[" + i + "]");
...
function assertObjectAttributePolicy(pub, policy, E, policyCode, structuralCode) {
  ...
  _keys(policy.objectAttributes).forEach(function (name) {
  ...
  if (ap.allow) {
    ...
    var ok = _some(ap.allow, function (want) {
