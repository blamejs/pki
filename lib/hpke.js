 ...
 *   public key into an encapsulated key plus an AEAD-encrypting context. Two KEM
 *   families run the RFC 9180 key schedule: the DHKEM suites P-256, P-521, X25519,
 *   and X448 in all four modes (base / psk / auth / auth-psk), proven against the
 *   RFC 9180 Appendix A known-answer vectors, and the post-quantum ML-KEM-512 /
 *   -768 / -1024 KEMs (IANA code points 0x0040 to 0x0042, draft-ietf-hpke-pq) in
 *   the base and psk modes, proven against that draft's Appendix A vectors. The
 *   KDFs are HKDF-SHA256 / HKDF-SHA384 / HKDF-SHA512; the AEADs are AES-128-GCM,
 *   AES-256-GCM, ChaCha20Poly1305, and export-only. DHKEM(P-384) is RFC-registered
 *   but no known-answer vector pairs it with an HKDF key schedule, so a request
 *   for it fails closed. The PQ/T hybrid KEMs and the SHAKE-based KDFs of the HPKE
 *   revision are a data-row extension once their specifications stabilize. Pure
...
function _ephemeral(kem, eph) {
  if (eph != null) {
    ...
    var skE = _importPrivate(kem, eph.skm);
    return { skE: skE, enc: _boundPublic(kem, skE, eph.pkm) };
  }
...
function _mlKemEncap(kem, pkR, eph) {
  if (eph != null) {
    throw _err("hpke/bad-input", "eph (a fixed ephemeral key pair) applies to a DHKEM suite; " + kem.name + " encapsulation takes its randomness from the runtime");
  }
  var r;
...
function _recipPrivate(suite, sk) {
  var kem = suite.kem;
  if (sk && sk.skm != null) {
    var key = _importPrivate(kem, sk.skm);
    return { key: key, pkm: _boundPublic(kem, key, sk.pkm) };
  }
  if (_isBuffer(sk)) {
...
var suites = intrinsic.assign(intrinsic.create(null), {
  KEM: intrinsic.assign(intrinsic.create(null), {
    DHKEM_P256_HKDF_SHA256: 0x0010, DHKEM_P521_HKDF_SHA512: 0x0012, DHKEM_X25519_HKDF_SHA256: 0x0020, DHKEM_X448_HKDF_SHA512: 0x0021,
    ML_KEM_512: 0x0040, ML_KEM_768: 0x0041, ML_KEM_1024: 0x0042,
  }),
  KDF: intrinsic.assign(intrinsic.create(null), { HKDF_SHA256: 0x0001, HKDF_SHA384: 0x0002, HKDF_SHA512: 0x0003 }),
 ...
 * suites only: an ML-KEM suite throws `hpke/auth-unsupported`.
 *
 * @example
