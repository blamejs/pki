...
// The sender and the recipient read the same option object from opposite ends, so the recognised
// set is the UNION and one table serves both. Every one of these narrows or authenticates: a
// misspelled `psk` leaves a psk-mode setup with no pre-shared key, a misspelled `senderKey` leaves
// an auth-mode setup unauthenticated -- and both currently fail with a message about the field the
// caller believes they supplied. `eph` is the deterministic-KAT seam; supplied by mistake it fixes
// the ephemeral key, which is the one option here whose silent acceptance is catastrophic.
var _SETUP_KEYS = {
  mode: 1, info: 1, psk: 1, pskId: 1,
  senderKey: 1,          // setupS, auth modes: the sender's KEM private key
  senderPublicKey: 1,    // setupR, auth modes: the sender's KEM public key
  eph: 1,                // test-vector determinism
};
function _mode(opts) {
  guard.identifier.assertKnownKeys(opts, _SETUP_KEYS, _err, "hpke/bad-input", "unknown HPKE setup option ");
