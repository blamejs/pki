...
function _kem(id) { var s = KEMS[id]; if (!s) throw _err("hpke/unknown-suite", "unsupported KEM id 0x" + id.toString(16)); return s; }
function _kdf(id) { var s = KDFS[id]; if (!s) throw _err("hpke/unknown-suite", "unsupported KDF id 0x" + id.toString(16)); return s; }
function _aead(id) { var s = AEADS[id]; if (!s) throw _err("hpke/unknown-suite", "unsupported AEAD id 0x" + id.toString(16)); return s; }
...
function _keySchedule(suite, mode, sharedSecret, info, psk, pskId) {
  ...
  return new Context(suite, key, baseNonce, exporterSecret);
}

// ---- AEAD context (RFC 9180 sec. 5.2 / 5.3) ----------------------------------

function Context(suite, key, baseNonce, exporterSecret) {
  this._suite = suite; this._key = key; this._baseNonce = baseNonce;
  this._exporterSecret = exporterSecret; this._seq = 0n;
}
...
function _suite(kemId, kdfId, aeadId) {
...
// A recipient/sender KEM key argument is a node KeyObject or serialized bytes; a
// KAT drives the deterministic path by passing { skm, pkm } (raw serialized).
function _recipPublic(suite, pk) {
  ...
  var pkObj = pk;   // node KeyObject
  return { key: pkObj, pkm: _exportPublic(suite.kem, pkObj) };
}
...
function _setupS(ids, pkR, opts) {
  opts = opts || {};
  var suite = _suite(ids.kem, ids.kdf, ids.aead);
  var mode = opts.mode || MODE_BASE;
  ...
  var ctx = _keySchedule(suite, mode, k.sharedSecret, _buf(opts.info), _buf(opts.psk), _buf(opts.pskId));
 ...
 * and the recipient KEM private key (a node KeyObject or serialized bytes),
...
function _setupR(ids, enc, skR, opts) {
  ...
  return _keySchedule(suite, mode, ss, _buf(opts.info), _buf(opts.psk), _buf(opts.pskId));
}
