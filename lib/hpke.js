...
function i2osp(n, len) {
  var b = _bufferAlloc(len);
  var v = _BigInt(n);
  for (var i = len - 1; i >= 0; i--) { b[i] = _Number(v & 0xffn); v >>= 8n; }
  return b;
}
function concat(arr) { return _bufferConcat(arr); }
function xor(a, b) { var o = _bufferAlloc(a.length); for (var i = 0; i < a.length; i++) o[i] = a[i] ^ b[i]; return o; }

var HPKE_V1 = _bufferFrom("HPKE-v1", "ascii");
function L(s) { return _bufferFrom(s, "ascii"); }
...
function _idStr(id) { return typeof id === "number" ? "0x" + id.toString(16) : _String(id); }
...
function _extract(hash, Nh, salt, ikm) {
  var key = (salt && salt.length) ? salt : _bufferAlloc(Nh);
...
function _expand(hash, Nh, prk, info, len) {
  ...
  var out = [], t = _bufferAlloc(0), n = _ceil(len / Nh);
  for (var i = 1; i <= n; i++) {
    ...
    var fed = concat([t, info, _bufferFrom([i])]);
...
function _importPublic(kem, raw) {
  ...
  try {
    ...
    return nodeCrypto.createPublicKey({ key: _bufferFrom(OKP_SPKI[kem.curve] + raw.toString("hex"), "hex"), format: "der", type: "spki" });
  } catch (e) {
...
function _exportPublic(kem, keyObject) {
  ...
  if (kem.kind === "ec") return concat([_bufferFrom([0x04]), _b64u(jwk.x), _b64u(jwk.y)]);
  return _b64u(jwk.x);
}
...
function _importPrivate(kem, rawSk, rawPk) {
  try {
    ...
    return nodeCrypto.createPrivateKey({ key: _bufferFrom(OKP_PKCS8[kem.curve] + rawSk.toString("hex"), "hex"), format: "der", type: "pkcs8" });
  } catch (e) {
...
function _extractAndExpand(kem, dh, kemContext) {
  ...
  var eaePrk = _labeledExtract(kdf, sid, _bufferAlloc(0), "eae_prk", dh);
...
function _kemId(kem) { for (var k in KEMS) if (KEMS[k] === kem) return _Number(k); return 0; }
...
function _keySchedule(suite, mode, sharedSecret, info, psk, pskId, role) {
  ...
  var pskIdHash = _labeledExtract(kdf, sid, _bufferAlloc(0), "psk_id_hash", pskId);
  var infoHash = _labeledExtract(kdf, sid, _bufferAlloc(0), "info_hash", info);
...
Context.prototype._inc = function () {
  var aead = this._suite.aead;
  if (this._seq >= (1n << _BigInt(8 * aead.Nn)) - 1n) throw _err("hpke/message-limit", "AEAD sequence number would overflow (RFC 9180 sec. 5.2)");
  this._seq += 1n;
};
...
Context.prototype.export = function (exporterContext, len) {
  if (!_isInteger(len) || len < 0) throw _err("hpke/export-length", "export length must be a non-negative integer");
  return _labeledExpand(this._suite.kdf, this._suite.suiteId, this._exporterSecret, "sec", _buf(exporterContext), len);
};
function _buf(x) { return _isBuffer(x) ? x : _bufferFrom(x || []); }
...
function _recipPublic(suite, pk) {
  if (_isBuffer(pk)) return { key: _importPublic(suite.kem, pk), pkm: pk };
...
function _recipPrivate(suite, sk) {
  ...
  if (_isBuffer(sk)) throw _err("hpke/bad-key", "a serialized private key must be provided as { skm, pkm } (raw scalar + public point), not a bare buffer");
  try {
...
function _withCommon(extra) {
  var out = {};
  _keys(_COMMON_KEYS).forEach(function (k) { out[k] = 1; });
...
function _mode(opts, keys, who) {
  guard.identifier.assertKnownKeys(opts, keys, _err, "hpke/bad-input", function (k) {
    ...
    return "unknown HPKE " + who + " option " + _stringify(k) +
      (Object.prototype.hasOwnProperty.call(other, k)
        ...
        : "") + " -- accepted: " + _keys(keys).sort().join(", ");
  });
  var mode = (opts.mode == null) ? MODE_BASE : opts.mode;
  if (mode !== MODE_BASE && mode !== MODE_PSK && mode !== MODE_AUTH && mode !== MODE_AUTH_PSK) {
    throw _err("hpke/unknown-mode", "unsupported HPKE mode " + _stringify(mode) + " (RFC 9180 sec. 5.1 defines base / psk / auth / auth-psk)");
  }
  return mode;
}
