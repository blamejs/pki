...
function _encap(kem, pkR, pkRm, eph) {
  ...
  try { return { sharedSecret: _extractAndExpand(kem, dh, concat([e.enc, pkRm])), enc: e.enc }; }
  finally { _wipeDh(dh); }
}
function _decap(kem, enc, skR, pkRm) {
  var pkE = _importPublic(kem, enc);
  var dh = _dh(skR, pkE);
  try { return _extractAndExpand(kem, dh, concat([enc, pkRm])); }
  finally { _wipeDh(dh); }
}
function _authEncap(kem, pkR, pkRm, skS, pkSm, eph) {
  var e = _ephemeral(kem, eph);
  var dh1 = _dh(e.skE, pkR), dh2 = _dh(skS, pkR);
  var dh = concat([dh1, dh2]);
  try { return { sharedSecret: _extractAndExpand(kem, dh, concat([e.enc, pkRm, pkSm])), enc: e.enc }; }
  finally { _wipeDh(dh1); _wipeDh(dh2); _wipeDh(dh); }
}
function _authDecap(kem, enc, skR, pkRm, pkS, pkSm) {
  var pkE = _importPublic(kem, enc);
  var dh1 = _dh(skR, pkE), dh2 = _dh(skR, pkS);
  var dh = concat([dh1, dh2]);
  try { return _extractAndExpand(kem, dh, concat([enc, pkRm, pkSm])); }
  finally { _wipeDh(dh1); _wipeDh(dh2); _wipeDh(dh); }
}
