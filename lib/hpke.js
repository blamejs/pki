...
// replaceable properties to answer one thing, and here it decides which suite parameters are read.
...
Context.prototype.open = function (aad, ct) {
  ...
  var pt;
  // Through the shared guard so the recovered plaintext is destroyed when the AEAD tag does not
  // check out (RFC 9180 sec. 5.2 gives open one outcome on failure, and it is not a copy left in
  // the heap), and so the success path's abandoned intermediate is cleared too.
