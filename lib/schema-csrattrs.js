...
var ATTRIBUTE_KEYTYPE = pkix.attribute(NS, { minValues: 0 });
...
// The recognized public-key type OIDs whose CsrAttrs request MAY carry an empty
// values SET ("no further requirements on the key", RFC 9908 sec. 3.2 -- rsaEncryption
// and ecPublicKey are only the examples, so a no-parameter key like Ed25519 or an
// ML-DSA / SLH-DSA / ML-KEM type is equally valid). Built by registered NAME (a typo
// fails at load), never dotted literals; every OTHER attribute keeps SET SIZE(1..MAX).
...
var ATTR_OR_OID = schema.decode(function (n, ctx) {
  ...
  if (schema.isUniversal(n, T.SEQUENCE) && n.children) {
    // Peek the type OID to choose the values-arity rule: a recognized key-type
    // hint MAY have an empty values SET; every other attribute is SET SIZE(1..MAX).
    ...
    var attrSchema = KEY_TYPE_OIDS.has(typeOid) ? ATTRIBUTE_KEYTYPE : ATTRIBUTE;
