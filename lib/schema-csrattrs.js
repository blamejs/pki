...
var CERT_REQ_INFO_TEMPLATE = schema.seq([
  ...
  build: function (m, ctx) {
    ...
    var extReqTemplates = 0, hasExtReq = false, extensionTemplates = [], extensions = null;
...
var ATTR_OR_OID = schema.decode(function (n, ctx) {
  ...
  if (schema.isUniversal(n, T.SEQUENCE) && n.children) {
    // Peek the type OID to choose the values-arity rule: a key-type hint MAY have
    // an empty values SET; every other attribute is SET SIZE(1..MAX).
    ...
    var attrSchema = (typeOid === OID_EC_PUBLIC_KEY || typeOid === OID_RSA_ENCRYPTION) ? ATTRIBUTE_KEYTYPE : ATTRIBUTE;
