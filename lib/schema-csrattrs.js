  ...
  build: function (m, ctx) {
    ...
    for (var i = 0; i < attributes.length; i++) {
      ...
      if (a.type === OID_EXTENSION_REQUEST) {
        ...
        extensions = schema.walk(EXTENSIONS, pkix.decodeNested(a.values[0], ctx, "a template id-ExtensionReq value"), ctx).result;
      }
      if (a.type === OID_EXT_REQ_TEMPLATE) {
        ...
        extensionTemplates = extensionTemplates.concat(schema.walk(EXTENSION_REQ_TEMPLATE,
          pkix.decodeNested(a.values[0], ctx, "an id-aa-extensionReqTemplate value"), ctx).result);
      }
    }
...
function _enrichAttribute(item, ctx) {
  ...
  if (item.oid === OID_EXTENSION_REQUEST) {
    ...
    item.extensions = schema.walk(EXTENSIONS, pkix.decodeNested(item.values[0], ctx, "an id-ExtensionReq value"), ctx).result;
    ...
    if (item.values.length === 1) {
      try { item.curve = asn1.read.oid(pkix.decodeNested(item.values[0], ctx, "an ecPublicKey attribute value")); }
    ...
    if (item.values.length === 1) {
      var sz;
      try { sz = asn1.read.integer(pkix.decodeNested(item.values[0], ctx, "an rsaEncryption attribute value")); }
    ...
    item.template = schema.walk(CERT_REQ_INFO_TEMPLATE,
      pkix.decodeNested(item.values[0], ctx, "a certificationRequestInfoTemplate value"), ctx).result;
  }
}
 ...
 * @signature  pki.schema.csrattrs.parse(der, caps?) -> { items }
 * @since      0.1.24
 * @status     stable
