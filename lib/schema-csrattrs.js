  ...
  build: function (m, ctx) {
    ...
    for (var i = 0; i < attributes.length; i++) {
      ...
      if (a.type === OID_EXT_REQ_TEMPLATE) {
        extReqTemplates += 1;
        // values MUST be a SET of exactly one element (RFC 9908 sec. 3.4).
        if (a.values.length !== 1) throw NS.E("csrattrs/bad-template-attrs", "an id-aa-extensionReqTemplate values must be a SET of exactly one element (RFC 9908 sec. 3.4)");
        // the single element is one ExtensionTemplate (a single SEQUENCE, not
        // a SEQUENCE OF).
        extensionTemplates.push(schema.walk(EXTENSION_TEMPLATE, asn1.decode(a.values[0]), ctx));
      }
    }
...
function _enrichAttribute(item, ctx) {
  if (item.oid === OID_EXTENSION_REQUEST) {
    ...
    // A key-type hint names ONE requested curve; a multi-valued SET is ambiguous
    // (DER ordering would pick the advertised constraint), so fail closed like
    // the extension-request and template branches rather than reading values[0].
    if (item.values.length !== 1) throw ctx.E("csrattrs/bad-key-type-attr", "an ecPublicKey attribute values must be a SET of exactly one named-curve OBJECT IDENTIFIER (RFC 9908 sec. 3.2)");
    ...
    var sz;
    // A key-type hint names ONE requested key size; reject a multi-valued SET.
    if (item.values.length !== 1) throw ctx.E("csrattrs/bad-key-type-attr", "an rsaEncryption attribute values must be a SET of exactly one INTEGER key size (RFC 9908 sec. 3.2)");
