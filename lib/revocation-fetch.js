...
/** @internal A switch that turns a security control on is read as the boolean it documents, never
 * coerced. `nonce: "true"` written by an operator asking for replay binding would read as off
 * under a `=== true` test, and the requests would go out carrying no nonce with nothing said. An
 * absent value is the documented default; anything else is a configuration fault. */
function _booleanOption(value, label) {
  if (value === undefined || value === null) return false;
  if (value === true || value === false) return value;
  throw E("path/bad-input", label + " must be true or false, got " + guard.text.showValue(value));
}
...
function _contentType(headers) {
  var h = headers || {};
  var keys = guard.identifier.optionNames(h);
...
function _keep(state, url, parsed, fetchedBytes, issuerNameBytes, keyBytes) {
  if (!(fetchedBytes > 0) || !parsed.nextUpdate || !parsed.thisUpdate) return;
  /** @internal Keyed on the name the CERTIFICATE carries, which is what the lookup above forms,
   * and not on the name the CRL carries. The two are compared by the RFC 5280 sec. 7.1 canonical
   * rule, so the same DN in another string encoding is accepted and still has different bytes:
   * writing under the CRL's spelling would file the entry where nothing looks, and every
   * validation would refetch inside a window the CRL had already signed for. */
  state.crlCache.put(_crlCacheKey(url, issuerNameBytes, keyBytes), parsed, parsed.thisUpdate, parsed.nextUpdate, fetchedBytes);
}
...
async function _tryCrl(state, cert, issuer, ctx, candidates, certDeltas) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    ...
    if (interim && (interim.status === "good" || interim.status === "revoked")) {
      if (authentic) _keep(state, url, parsed, fetchedBytes, cert.issuer.bytes, issuer.workingPublicKey);
 ...
 * @opts  nonce             Whether an OCSP request carries a 32-octet nonce. `true` or `false`,
 *                          and anything else is `path/bad-input` rather than the default, since a
 *                          control read as off is one an operator asked for and did not get.
 *                          Default `false`, since a pre-produced response carries none and the
 *                          currency window is the replay bound. Set, the response must echo the
 *                          nonce or the status is undetermined and the route falls through to
 *                          the CRL.
...
function fetchingChecker(opts) {
  ...
  var cfg = {
    ...
    nonce: _booleanOption(opts.nonce, "fetchingChecker: opts.nonce"),
    tls: opts.tls,
  ...
  return {
    check: async function (cert, issuer, ctx) {
      ...
      /** @internal A certificate may name a delta location and no base one. `crlChecker` treats an
       * unmerged delta that lists the serial as authoritative, so such a certificate has a real
       * revoked answer available and returning undetermined without asking would lose it. The
       * delta is tried as a candidate of its own here, which is the only place it can be: below,
       * the deltas are fetched beside a base that answered. */
      if (crlCands.length === 0 && certDeltas.length > 0) crlCands = certDeltas;
