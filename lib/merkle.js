...
var SHA256_BYTES = 32;
var LEAF_PREFIX = _bufferFrom([0x00]);
var NODE_PREFIX = _bufferFrom([0x01]);
var EMPTY = _bufferAlloc(0);
...
function _proofNodes(proof) {
  if (!_isArray(proof)) throw new MerkleError("merkle/bad-proof", "proof must be an array");
  ...
  var out = [];
  for (var i = 0; i < proof.length; i++) _push(out, _node32(proof[i], "proof[" + i + "]"));
  return out;
}
...
function leafHash(entry) {
  ...
  return _sha256(_bufferConcat([LEAF_PREFIX, e]));
}
...
function nodeHash(left, right) {
  ...
  return _sha256(_bufferConcat([NODE_PREFIX, l, r]));
}
