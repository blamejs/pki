...
// Every array here is built and read as own elements only. Appending through a captured Object.defineProperty
// (not `arr[i] = v`) creates an own element, so a co-resident Array.prototype numeric setter is never invoked
// and the own element shadows any inherited getter on read; and no Array.prototype.map / concat runs, so
// Array[Symbol.species] is never consulted. Both are routes a hostile prototype could otherwise use to
// rewrite a packed octet and make a certificate carry an attacker-chosen address. The emitted octets are
// written straight into a Buffer, whose integer-indexed store bypasses the prototype entirely.
function _push(arr, v) { _defineProperty(arr, arr.length, { value: v, writable: true, enumerable: true, configurable: true }); }
// Split on a literal string separator without String.prototype.split, which consults the separator's
// `[Symbol.split]`; this scans with captured indexOf/slice only.
...
function expandIpv6Hex(ip) {
  ...
  var groups = left, i;   // `left` is a fresh own-element array; append through _push (no inherited setter)
...
function packIpLiteral(s) {
  if (isIPv4(s)) {
    ...
    for (i = 0; i < 4; i += 1) buf[i] = _Number(parts[i]);   // Buffer integer-index store; each octet range-checked by _isOctet
    return buf;
  }
