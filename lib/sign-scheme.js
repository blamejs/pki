...
// Captured at load. This module decides which algorithm a signature runs under and whether the
// caller's key matches the certificate it claims, so an operation read at call time is one the
// caller answers: a replaced case fold reports two different algorithm names equal, and a replaced
// `Buffer.isBuffer` admits a key that is not bytes.
...
// Asks the value's internal slot rather than `instanceof`, which reads the `Uint8Array` global at
// call time and answers about whichever constructor is bound then. It also settles the two cases
// `instanceof` gets wrong on their own terms: a real typed array from another realm is one, and an
// object merely built on the prototype is not.
...
// mldsaDigestSuitable(mlName, md) -- is `md` listed as meeting `mlName`'s security strength?
// Both levels are asked for their OWN entries. The gate rejects on a falsy lookup, so a digest name
// planted on Object.prototype answers for every name the row omits and a below-strength digest
// passes the RFC 9882 sec. 3.3 check. This is the single home for that question: the signer and the
// verifier both ask it, and a copy at either would let the two drift.
// @enforced-by behavioral -- an own-property table read has no rename-proof shape distinct from a
//   legitimate one; the RED vectors (a below-strength digest still refused with its name planted on
//   Object.prototype, on both the sign and verify paths) are the guard.
...
function _assertKeyMatchesScheme(key, imp, E) {
  ...
  if (_toUpperCase(_String(ka.name)) !== _toUpperCase(_String(imp.name))) {
    // A WebCrypto RSA-PSS CryptoKey is bound to PSS and cannot produce the default RSASSA-PKCS1-v1_5
    // signature (which a raw PKCS#8 RSA key can), so name the fix rather than only the mismatch.
...
function _normCompositeKeys(key, comp, E) {
  if (!key || typeof key !== "object" || _isBuffer(key) || _isU8(key) || key.mldsa == null || key.trad == null) {
...
function _importKey(key, imp, E) {
  // A WebCrypto CryptoKey is detected structurally (a `.type` string + an `.algorithm` object, and not
  // a byte view), never by instanceof (cross-realm / a second installed copy) or util.types.isCryptoKey
  // (false for this engine's own keys). The gate is broader than webcrypto.isCryptoKeyLike on purpose:
  // an incomplete CryptoKey-shaped object (e.g. one missing its `usages`) still routes here so adoptKey
  // reports the specific reason (usages / not-exportable) rather than the generic byte-form message. It
  // signs, so it must be a PRIVATE key: a public or secret CryptoKey cannot produce a signature and is
  // refused precisely.
  if (key && typeof key === "object" && !_isBuffer(key) && !_isU8(key) && typeof key.type === "string" && key.algorithm != null) {
    ...
    // A CryptoKey created outside this engine signs here too -- it is re-imported through this engine,
    // because it carries none of the key material this engine signs with. One whose material this
    // process cannot reach is refused with that as the reason.
  ...
  if (_isBuffer(key)) der = guard.bytes.view(key, E, "bad-input", "the signer private key");
    ...
    // A node:crypto KeyObject carries a `.type` but not the WebCrypto surface (algorithm / usages), so
    // it fails the CryptoKey gate above and would otherwise land on the generic message -- name it precisely.
