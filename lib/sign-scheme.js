...
function _assertKeyMatchesScheme(key, imp, E) {
  ...
  if (_String(ka.name).toUpperCase() !== _String(imp.name).toUpperCase()) throw E("bad-input", "the signer CryptoKey algorithm (" + ka.name + ") does not match the certificate's key algorithm (" + imp.name + ")");
...
function _normCompositeKeys(key, comp, E) {
  if (!key || typeof key !== "object" || _isBuffer(key) || key instanceof Uint8Array || key.mldsa == null || key.trad == null) {
...
function _importKey(key, imp, E) {
  if (key && typeof key === "object" && !_isBuffer(key) && !(key instanceof Uint8Array) && key.type === "private") {
