...
var SIG_ALGS = intrinsic.assign(intrinsic.create(null), {
  ...
  EdDSA: { kty: "OKP", subtle: "EdDSA", sigBytesByCrv: intrinsic.assign(intrinsic.create(null), { Ed25519: 64, Ed448: 114 }) },
...
var PRIVATE_JWK_MEMBERS = ["d", "p", "q", "dp", "dq", "qi", "oth", "k", "priv"];
 ...
 * private member (`d`, the RSA CRT parameters `p`/`q`/`dp`/`dq`/`qi` and the multi-prime `oth`, the
 * symmetric `k`, or the AKP `priv`) throws `jose/private-key-material`, so an accidentally
 ...
 * exactly-one-of jwk/kid, crit), an embedded `jwk` carrying private key material is refused
 * (`jose/private-key-material`, RFC 7515 sec. 4.1.3: the embedded key is the signer's public key),
 * and the signature byte length is pinned per alg.
 *
 ...
 *   v.header.alg;   // -> "ES256"
 */
var _VERIFY_KEYS = intrinsic.assign(intrinsic.create(null), { key: 1, profile: 1 });
async function verify(jws, opts) {
  ...
  /** @internal opts.key is examined several times: the type gate, the RFC 7638 thumbprint comparison
   *  against the embedded jwk, and the key the signature is verified under. An option reached through
   *  an accessor answers each read separately, so the key the comparison ran on need not be the key
   *  the verification used. Take each option once. */
  opts = guard.identifier.snapshotOptions(opts, _VERIFY_KEYS);
  ...
  /** @internal The embedded jwk is the signer's PUBLIC key (RFC 7515 sec. 4.1.3). One carrying
   *  private material is a malformed message, refused before it is thumbprinted or imported, since
   *  accepting it would return the private key in the verdict header. sign holds its own embedded
   *  jwk to the same rule. */
  if (header.jwk) assertPublicJwk(header.jwk);
 ...
 */
var _SIGN_KEYS = intrinsic.assign(intrinsic.create(null), { protected: 1, payload: 1, key: 1, jwk: 1, profile: 1 });
var _SIGN_KEYS_EMBEDDED_JWK = intrinsic.assign(intrinsic.create(null), { protected: 1, payload: 1, key: 1, profile: 1 });
