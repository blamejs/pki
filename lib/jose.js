...
// What this module reads from the runtime while it is deciding something, taken at load. A JWS is
// signed and verified against a header the caller owns, and the reads below are the ones that decide
// what that header IS: whether a value is a list, whether a name is the caller's own, which
// conversion turns a claim into the string compared against a profile. A caller who receives control
// afterwards -- through an accessor on the header, or a `toJSON` while it is being serialized -- can
// replace any of them, and every check around them still runs and still passes. See guard-intrinsic.
 ...
 */
// The public JWK has two sources and the embedded one wins, so opts.jwk is read only in kid mode.
// Supplied alongside a header that embeds its own, it selected nothing: the signature parameters
// came from the header's key while the caller had named a different one.
...
async function sign(opts) {
  opts = opts || {};
  // The options are checked BEFORE the header is touched. Serializing it runs the caller's own code
  // -- a toJSON, an accessor -- and that code can delete an option from the very bag being checked,
  // so a misspelling reached the door already removed and the message went out signed.
  guard.identifier.assertKnownKeys(opts, _SIGN_KEYS, E, "jose/bad-input", "unknown pki.jose.sign option ");
  // The header is then serialized ONCE, and every read below comes from a snapshot of those exact
  // bytes: the header validated, the header embedded in the JWS and the key confirmed against the
  // produced signature are then all the same values. Read live, a caller getter answers each of
  // those questions separately -- assertPublicJwk sees a public JWK while the bytes that ship carry
  // the private one, and the door sees a jwk-mode header while the signer sees a kid-mode one.
  var headerJson;
  try { headerJson = _stringify(opts.protected); }
  catch (e) { throw E("jose/bad-header", "the protected header must be JSON-serializable", e); }
  // Read back through the same bounded reader a received JWS goes through, so a header this side
  // produces is held to the caps and the duplicate-member rule the other side enforces.
  ...
  // Which of the two key sources is in force is known only once the header has been read, so the
  // narrower table runs here. It re-walks the bag, which also catches a name that code added.
  ...
  var protectedB64 = b64uEncode(headerBytes);   // the exact bytes every check above read
...
async function thumbprint(jwk) {
  ...
  for (var i = 0; i < members.length; i++) {
    ...
    // The canonical form IS the thumbprint preimage (RFC 7638 sec. 3.3), so the serializer that
    // builds it is the captured one: a replacement changes the identity every consumer compares.
