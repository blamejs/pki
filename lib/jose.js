...
async function sign(opts) {
  opts = opts || {};
  // The header is serialized ONCE, and every read below comes from a snapshot of those exact bytes:
  // the header validated, the header embedded in the JWS and the key confirmed against the produced
  // signature are then all the same values. Read live, a caller getter answers each of those
  // questions separately -- assertPublicJwk sees a public JWK while the bytes that ship carry the
  // private one, and the door sees a jwk-mode header while the signer sees a kid-mode one.
  var headerJson;
  ...
  guard.identifier.assertKnownKeys(opts, embedsJwk ? _SIGN_KEYS_EMBEDDED_JWK : _SIGN_KEYS, E, "jose/bad-input",
    function (k) {
      return "unknown pki.jose.sign option " + JSON.stringify(k) +
        (embedsJwk && k === "jwk" ? "; the protected header already embeds the public JWK" : "");
    });
