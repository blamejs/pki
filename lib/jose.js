...
async function sign(opts) {
  ...
  var headerJson;
  try { headerJson = JSON.stringify(opts.protected); }
  ...
  var headerBytes = headerJson === undefined ? null : Buffer.from(headerJson, "utf8");
  ...
  if (embedsJwk) {
    guard.identifier.assertKnownKeys(opts, _SIGN_KEYS_EMBEDDED_JWK, E, "jose/bad-input", function (k) {
      return "unknown pki.jose.sign option " + JSON.stringify(k) +
