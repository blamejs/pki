...
function _checkHeader(header, profileName) {
  ...
  if (profile.requireUrl && (!own("url") || typeof header.url !== "string")) throw E("jose/bad-header", "the protected header must carry a string url");
  // nonce discipline.
  var hasNonce = own("nonce");
  if (profile.nonce === "required") {
    if (!hasNonce || typeof header.nonce !== "string") throw E("jose/bad-header", "the protected header must carry a nonce");
  ...
  if (profile.keyId === "one-of") {
    ...
    if (hasKid && typeof header.kid !== "string") throw E("jose/bad-header", "kid must be a string");
