...
function _checkHeader(header, profileName) {
  ...
  if (Object.prototype.hasOwnProperty.call(header, "crit")) {
    ...
    for (var c = 0; c < crit.length; c++) {
      ...
      if (Object.prototype.hasOwnProperty.call(seen, name)) throw E("jose/bad-crit", "duplicate crit entry " + _stringify(name));
      seen[name] = 1;
...
async function sign(opts) {
  ...
  var protectedB64 = b64uEncode(_bufferFrom(_stringify(header), "utf8"));
 ...
 */
function sigAlgs() {
  return _keys(SIG_ALGS).map(function (alg) {
