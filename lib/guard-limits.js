...
// The integer test, taken at load. Every bound below is only a bound while this answers honestly:
// one returning true admits a NaN or fractional cap, which makes the comparison it guards always
// false. See guard-intrinsic for the whole captured set.
var _isInteger = require("./guard-intrinsic").isInteger;
// And the size measurement, for the same reason one step further in: a cap compares against a
// length, and on a byte view `length` is a replaceable accessor. One reporting a small number for a
// large buffer leaves every byteCap below always-false, which is the size defense silently dead.
