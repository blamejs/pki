...
// A key type absent from this map has no leaf this reader can import, so no alg names it.
...
//
// The derivation is total over the key families a certificate can carry: EC becomes ECDSA over the
// curve the alg fixes, RSA with a salt length becomes RSASSA-PSS (RFC 7518 sec. 3.5), RSA without
// one becomes RSASSA-PKCS1-v1_5. An alg on a key type no x5c leaf can hold (OKP, AKP) is skipped
// here and rejected at the leaf-family check either way, so skipping it narrows nothing.
//
...
function metadataAnchors(entry, opts) {
  ...
  if (opts.metadata !== undefined && !isVerifiedResult(opts.metadata)) {
...
module.exports = {
  ...
  // derivation is total: every algorithm pki.jose verifies under a certificate-carryable key type
  // has a row here, with the import and verify parameters that key type needs.
  BLOB_ALGS: BLOB_ALGS,
};
