...
function _chainToAnchor(chain, anchors, at, what) {
  ...
  return anchors.reduce(function (p, anchor) {
    return p.then(function (done) {
      if (done) return true;
      // An anchor that IS the chain's own root is the anchor, not a path element -- validating it
      // against itself would be a different assertion from the one being made.
      var path = ordered.slice();
      if (path.length > 1 && guard.name.dnEqual(path[0].subject, anchor.subject) &&
          guard.name.dnEqual(path[0].issuer, path[0].subject)) {
        path = path.slice(1);
      }
      // Nothing left to validate. Unreachable as the code stands -- the chain is non-empty by the
      // time it gets here (an x5c with no entries is refused at the header, and an attestation with
      // no trust path is refused before metadata is applied), and the strip above only runs when
      // there is more than one element, so it can never empty the path. It refuses rather than
      // trusts: a validator handed nothing has proved nothing, and the strip's premise -- that the
      // first element IS the anchor -- rests on a NAME comparison, never on a key. If some future
      // caller does reach this, the safe verdict is the one that makes it try the next anchor.
...
function _parsePayload(seg, at, opts) {
  ...
  var nextUpdate = rfc3339.parseDate(payload.nextUpdate, function (c, m) { return _err("webauthn/bad-metadata-blob", m); },
    "webauthn/bad-metadata-blob", "the metadata BLOB nextUpdate");
  // nextUpdate is a DATE, so the BLOB is current through the END of that UTC day.
  var staleAfter = nextUpdate.getTime() + constants.TIME.days(1);
  var atMs = at.getTime();
  ...
  var entries = payload.entries.map(function (e, i) {
    ...
    var keyIds = [];
    if (st && st.attestationCertificateKeyIdentifiers !== undefined) {
      if (!Array.isArray(st.attestationCertificateKeyIdentifiers)) {
        throw _err("webauthn/bad-metadata-blob", "metadata entry " + i + " attestationCertificateKeyIdentifiers is not an array");
      }
      if (st.attestationCertificateKeyIdentifiers.length > C.MDS_MAX_KEY_IDS_PER_ENTRY) {
        throw _err("webauthn/too-large", "metadata entry " + i + " declares " + st.attestationCertificateKeyIdentifiers.length +
          " attestation certificate key identifiers, above the " + C.MDS_MAX_KEY_IDS_PER_ENTRY + " ceiling");
      }
      st.attestationCertificateKeyIdentifiers.forEach(function (k) {
        ...
        // matches -- an entry that silently cannot be found.
  ...
  return { no: payload.no, legalHeader: payload.legalHeader, nextUpdate: payload.nextUpdate,
    stale: stale, entries: entries, byAaguid: byAaguid, byKeyIdentifier: byKeyIdentifier,
...
function statusDenied(entry, metadata) {
  ...
  if (policy === "latest-by-date") {
    var dated = reports.filter(function (r) { return r && typeof r.effectiveDate === "string" && rfc3339.isValidDate(r.effectiveDate); });
    if (dated.length) {
      considered = [dated.reduce(function (a, b) { return a.effectiveDate >= b.effectiveDate ? a : b; })];
    }
  }
...
// match nothing in the catalogue and silently turn every U2F lookup into a miss. SHA-1 here is the
// identifier the specification defines, not a signature or an integrity check; the value it names is
// re-verified by the chain check that follows.
