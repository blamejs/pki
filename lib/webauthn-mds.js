...
// Every operation this reader decides a catalogue with, captured at load. The BLOB governs which
// authenticator models are trusted, and each of these settles one step of that: whether a value is
// a list, whether a key is really in a table, whether two public keys are the same bytes, what a
// status name lower-cases to. Read at call time they are ordinary writable properties, so a caller
// holding the verified result answers them instead. See guard-intrinsic.
...
// Promise.resolve reads the constructor it is called on, so the captured constructor is passed as
// the receiver rather than left to whatever the call site's property lookup returns.
...
// allow:guard-reads-runtime-live -- module-init, the scheme table is built before any caller runs
...
function _deriveBlobAlgs() {
  // allow:guard-reads-runtime-live -- module-init, the sole call is the BLOB_ALGS binding below
...
// allow:guard-reads-runtime-live -- module-init, the status table is built before any caller runs
...
// allow:guard-reads-runtime-live -- module-init, the scope table is built before any caller runs
var CERT_SCOPED_STATUS = Object.assign(Object.create(null), { ATTESTATION_KEY_COMPROMISE: 1 });

// allow:guard-reads-runtime-live -- module-init, the option name set is built before any caller runs
...
function _deepFreeze(v, depth) {
  if (!v || typeof v !== "object" || Object.isFrozen(v) || _isView(v)) return v;
  if (depth > C.JSON_MAX_DEPTH) return v;
  _freeze(v);
  _keys(v).forEach(function (k) { _deepFreeze(v[k], depth + 1); });
  return v;
}
...
// allow:guard-reads-runtime-live -- module-init, the header name set is built before any caller runs
...
function _isAnchorItself(cert, anchor) {
  return guard.name.dnEqual(cert.subject.rdns, anchor.subject.rdns, _err, "webauthn/bad-att-cert", "the anchor subject") &&
    _bufEquals(cert.subjectPublicKeyInfo.bytes, anchor.subjectPublicKeyInfo.bytes);
}
...
function _verifyMetadataBlob(blob, opts) {
  ...
  var at = opts.time === undefined ? new _Date() : opts.time;
  if (opts.previousNo !== undefined && (!_isSafeInteger(opts.previousNo) || opts.previousNo < 0)) {
  ...
  var anchors = _map(roots, function (r, i) { return _asCert(r, "opts.rootCertificates[" + i + "]"); });
  ...
  var raw = (_isView(blob) || blob instanceof ArrayBuffer)
  ...
  var declaredLength = raw ? raw.length : (typeof blob === "string" ? _byteLength(blob, "utf8") : null);
  ...
  var bytes = raw || _bufferFrom(blob, "utf8");
  ...
  var segs = _bufToString(bytes, "utf8").split(".");
  ...
  if (segs[0].length > _ceil(C.MDS_BLOB_HEADER_MAX_BYTES / 3) * 4) {
  ...
  if (segs[2].length > _ceil(C.MDS_BLOB_SIG_MAX_BYTES / 3) * 4) {
  ...
  try {
    ...
    header = guard.json.parse(_bufferFrom(jose.base64url.decode(segs[0])), _err, {
    ...
    sig = _bufferFrom(jose.base64url.decode(segs[2]));
  } catch (e) {
  ...
  if (_hasOwn(header, "crit")) {
    ...
    for (var ci = 0; ci < crit.length; ci++) {
      ...
      if (critSeen[critName]) throw _err("webauthn/bad-metadata-blob", "the metadata BLOB header crit repeats " + _stringify(critName));
      critSeen[critName] = 1;
      if (UNDERSTOOD_HEADER[critName]) {
        throw _err("webauthn/bad-metadata-blob", "the metadata BLOB header crit must not name the standard header parameter " + _stringify(critName) + " (RFC 7515 sec. 4.1.11)");
      }
      throw _err("webauthn/bad-metadata-blob", "the metadata BLOB header names critical header parameter " + _stringify(critName) + ", which this reader does not process");
    }
  }
  var algRow = typeof header.alg === "string" ? BLOB_ALGS[header.alg] : undefined;
  if (!algRow) throw _err("webauthn/unsupported-algorithm", "the metadata BLOB alg " + _stringify(header.alg) + " is not a supported JWS signature algorithm");
  ...
  if (!leafSchemes || !leafSchemes[algRow.scheme]) {
    throw _err("webauthn/unsupported-algorithm", "the metadata BLOB alg " + header.alg + " does not match the x5c leaf key type " + _stringify(leafAlg));
  }
  ...
  var signingInput = _bufferFrom(segs[0] + "." + segs[1], "ascii");
...
function _chainToAnchor(chain, anchors, at, what, code) {
  ...
  var ordered = _arraySlice(chain).reverse();   // path.validate takes anchor-adjacent first
  var lastFault = null;
...
function assertFresh(metadata, at, label) {
  ...
  if (!_isFinite(atMs) || !_isFinite(limit)) return;
  if (atMs >= limit) {
...
function _parsePayload(seg, at, opts) {
  ...
  try {
    ...
    payload = guard.json.parse(_bufferFrom(jose.base64url.decode(seg)), _err, {
  ...
  if (!_isSafeInteger(payload.no) || payload.no < 0) throw _err("webauthn/bad-metadata-blob", "the metadata BLOB payload must carry a non-negative integer no");
  ...
  if (!_isFinite(atMs) || !_isFinite(staleAfter)) {
  ...
  if (!_isArray(payload.entries)) throw _err("webauthn/bad-metadata-blob", "the metadata BLOB payload must carry an entries array");
  ...
  var byAaguid = _create(null);
  var byKeyIdentifier = _create(null);
  var entries = _map(payload.entries, function (e, i) {
    if (!_isPlainObject(e)) throw _err("webauthn/bad-metadata-blob", "metadata entry " + i + " is not an object");
    if (!_isArray(e.statusReports) || e.statusReports.length === 0) {
    ...
    _forEach(e.statusReports, function (r, ri) {
    ...
    if (e.aaguid !== undefined) {
      if (typeof e.aaguid !== "string" || !_reTest(AAGUID_RE, _toLowerCase(e.aaguid))) {
        throw _err("webauthn/bad-metadata-blob", "metadata entry " + i + " has a malformed aaguid");
      }
      aaguid = _toLowerCase(e.aaguid);
    }
    ...
    [[e.attestationCertificateKeyIdentifiers, "entry"], [st && st.attestationCertificateKeyIdentifiers, "metadataStatement"]]
      .forEach(function (pair) {
        ...
        _forEach(list, function (k) {
          ...
          if (!seenKeyId[lower]) { seenKeyId[lower] = 1; _push(keyIds, lower); }
        });
      });
  ...
  _forEach(entries, function (e) { _entryOrigin.set(e, result); });
  return result;
}
...
function metadataFor(metadata, identifier) {
  ...
  var key = _toLowerCase(identifier);
  if (_reTest(AAGUID_RE, key)) {
...
function metadataAnchors(entry, opts) {
  ...
  opts = opts || {};
  if (typeof opts !== "object" || _isArray(opts)) throw _err("webauthn/bad-input", "metadataAnchors opts must be an object");
  ...
  var list = st && _isArray(st.attestationRootCertificates) ? st.attestationRootCertificates : [];
  ...
  return _map(list, function (b64, i) {
    var der;
...
function statusDenied(entry, metadata, leaf, at) {
  ...
  reports = _filter(reports, function (r) {
  ...
  var atMs = (guard.time.isDate(at) && _isFinite(guard.time.instantOf(at))) ? guard.time.instantOf(at) : null;
  if (atMs !== null) {
    reports = _filter(reports, function (r) { return !isDated(r) || _reportInForceAt(r, atMs); });
  }
  var considered = reports;
  if (policy === "latest-by-date") {
    var dated = _filter(reports, isDated);
    if (dated.length) {
      ...
      // report still counts.
      considered = _filter(dated, function (r) { return r.effectiveDate === newest; })
        .concat(_filter(reports, function (r) { return !isDated(r); }));
    }
  }
  var rejectUnknown = !!(metadata && metadata.rejectUnknownStatus);
  return _some(considered, function (r) {
...
function aaguidToString(buf) {
  if (!_isBuffer(buf) || buf.length !== 16) return null;
  var h = _bufToString(buf, "hex");
...
function metadataForKeyIdentifier(metadata, keyId) {
  ...
  return metadata.byKeyIdentifier[_toLowerCase(keyId)] || null;
}
