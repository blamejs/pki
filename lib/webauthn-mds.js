...
function statusDenied(entry, metadata, leaf) {
  ...
  // Reports that demonstrably concern a DIFFERENT certificate are removed first, before any policy
  // runs. Scope has to be settled before recency, or under latest-by-date such a report could be
  // selected as the newest, displace an older model-wide revocation, and then be discarded as
  // inapplicable -- clearing the entry using a report that was never about this authenticator.
