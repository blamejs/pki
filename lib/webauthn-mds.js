...
function _chainToAnchor(chain, anchors, at, what) {
  ...
  return anchors.reduce(function (p, anchor) {
    return p.then(function (done) {
      ...
      if (path.length > 1 && _isAnchorItself(path[0], anchor)) path = path.slice(1);
      ...
      // trusts: a validator handed nothing has proved nothing. If some future caller does reach
      // this, the safe verdict is the one that makes it try the next anchor.
...
function statusDenied(entry, metadata, leaf) {
  ...
  return considered.some(function (r) {
    ...
    if (DISQUALIFYING[s]) return !_reportNamesOtherCert(r, leaf);
