...
function _parsePayload(seg, at, opts) {
  ...
  // Recorded as verified HERE, at the only place a catalogue can have been through the signature,
  // chain, rollback and freshness gates.
...
function statusDenied(entry, metadata, leaf, at) {
  ...
  if (policy === "latest-by-date") {
    ...
    // A report dated after the instant being judged has NOT taken effect yet, so it cannot be "the
    // latest" -- letting it be selected would allow a future-dated clean report to displace a
    // revocation that is in force right now, and would give a deliberately historical verdict
    // (opts.time in the past) the benefit of reports that had not been filed at that time.
    var atMs = (at instanceof Date && isFinite(at.getTime())) ? at.getTime() : null;
    if (atMs !== null) {
      var effective = dated.filter(function (r) { return _reportInForceAt(r, atMs); });
      // If nothing has taken effect yet there is no "latest" to narrow to, so every report stays in
      // consideration rather than none of them -- an empty considered set denies nothing.
      if (effective.length) dated = effective;
    }
    if (dated.length) {
