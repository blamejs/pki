...
// The runtime operations the three stages decide with, captured at load through the guard family's
// home for them. Until it is captured each is an ordinary writable property of a global, and each
// answers a question this module asks on the operator's behalf: a `Buffer.compare` returning 0
// matches any recipient identifier against any certificate, an `equals` answering true accepts the
// originator's own claimed digest as the recomputed one, and a `Buffer.from` returning a view over
// other memory leaves the recovered key readable after the wipe.
...
function _toDer(input) {
  if (_isBuffer(input) || input instanceof Uint8Array) return guard.bytes.snapshot(input, CmsError, "cms/bad-input", "input");
...
function _selectCandidates(recips, km, opts) {
  ...
  if (opts.recipientIndex != null) {
    ...
    if (typeof i !== "number" || !_isFinite(i) || i < 0 || i >= recips.length || _floor(i) !== i) throw _err("cms/bad-input", "recipientIndex must be an in-range integer");
  ...
  for (var k = 0; k < recips.length; k++) {
    ...
    if (want === "asym" && (r.type === "ktri" || r.type === "kari" || r.type === "ori")) {
      if (cert && _ridMatches(r, cert)) _push(out, { ri: r, index: k });   // a cert may match several recipients
    } else if (want === "pwri" && r.type === "pwri") { _push(out, { ri: r, index: k }); }
    else if (want === "kekri" && r.type === "kekri") {
      if (km.kekId == null || _kekIdMatches(r, km.kekId)) _push(out, { ri: r, index: k });
    }
  }
...
function _assertSupported(ri, km) {
  if (ri.type === "kari" && ri.keyEncryptionAlgorithm && _reTest(/mqv/i, ri.keyEncryptionAlgorithm.name || "")) throw _err("cms/unsupported-algorithm", "ECMQV kari is not supported");
  if (ri.type === "ori") {
...
function _ridEq(rid, cert) {
  ...
  if (rid.subjectKeyIdentifier) { var ski = _skiOf(cert); return !!ski && _compare(ski, rid.subjectKeyIdentifier) === 0; }
...
function _kekIdMatches(ri, kekId) {
  ...
  if (!id) return false;
  return _compare(id, guard.bytes.view(kekId, CmsError, "cms/bad-input", "kekId")) === 0;
}
...
async function _ktriCek(ri, km) {
  ...
  try {
    if (kea.oid === O("rsaesOaep")) {
      ...
      return _bufferFrom(await subtle.decrypt({ name: "RSA-OAEP" }, pub, ri.encryptedKey));
    }
...
async function _kariCek(ri, km) {
  ...
  try {
    ...
    if (_isMont(origSpki)) {
      ...
      mz = _bufferFrom(await subtle.deriveBits({ name: mont.name, public: origPub }, recipPriv, null));
      if (_taEvery(mz, function (x) { return x === 0; })) throw _fail();
      ...
      kek = _bufferFrom(await subtle.deriveBits({ name: "HKDF", hash: mont.hkdf, salt: ukm || _bufferAlloc(0), info: _eccSharedInfo(wrapAlg.name, ukm, kekBytes) }, mzKey, kekBytes * 8));
    } else {
      ...
      z = _bufferFrom(await subtle.deriveBits({ name: "ECDH", public: origEc }, recipEc, null));
      var zKey = await subtle.importKey("raw", z, { name: "X963KDF" }, false, ["deriveBits"]);
      kek = _bufferFrom(await subtle.deriveBits({ name: "X963KDF", hash: _x963Hash(scheme), info: _eccSharedInfo(wrapAlg.name, ukm, kekBytes) }, zKey, kekBytes * 8));
    }
...
async function _kemriCek(ri, km) {
  ...
  var kekBytes = _Number(k.kekLength);
  var wrapAlg = k.wrap;
  ...
  try {
    ...
    kekAb = await subtle.deriveBits({ name: "HKDF", hash: "SHA-256", salt: _bufferAlloc(0), info: _kemOtherInfo(wrapAlg.name, kekBytes, k.ukm || null) }, ssKey, kekBytes * 8);
    ...
    guard.secret.zeroizeAll([ss, kek, ssAb ? new _Uint8Array(ssAb) : null, kekAb ? new _Uint8Array(kekAb) : null],
...
function _assertContentCipherMode(eci, ct) {
  ...
  if (CONTENT_MODE[oidStr] !== wantMode) {
    throw _err("cms/unsupported-algorithm", "contentEncryptionAlgorithm " + oidStr + " is not a " + _toUpperCase(wantMode) + " cipher, which " + ct + " requires (RFC 5083 sec. 2.1 / RFC 5084 sec. 3)");
  }
}
...
async function _openContent(parsed, eci, cek, ct) {
  ...
  try {
    if (ct === "authEnvelopedData") {
      var aad = parsed.authAttrsBytes != null ? _explicitSetOf(parsed.authAttrsBytes) : _bufferAlloc(0);
...
function _gcmOpen(cek, nonce, ct, tag, aad, keyBits, icvLen) {
  ...
  return _bufferConcat([d.update(ct), d.final()]); // final() throws on tag mismatch -> uniform
}
...
async function _aesKwUnwrap(kek, wrapped) {
  ...
  try {
    ...
    var view = new _Uint8Array(rawAb);
    var out = _bufferAlloc(view.length);
    out.set(view);
    return out;
  } finally {
    guard.secret.zeroize(new _Uint8Array(rawAb), CmsError, "cms/bad-input", "the unwrapped content key");
  }
}
...
function _kemOtherInfo(wrapName, kekBytes, ukm) {
  var kids = [b.sequence([b.oid(O(wrapName))]), b.integer(_BigInt(kekBytes))];
  if (ukm) _push(kids, b.explicit(0, b.octetString(ukm)));
  return b.sequence(kids);
}
...
function _pwriUnwrap(kek, wrapped, iv, keyBits) {
  ...
  var ecb = nodeCrypto.createDecipheriv("aes-" + keyBits + "-ecb", kek, _bufferAlloc(0)); ecb.setAutoPadding(false);
  var lastDec = _bufferConcat([ecb.update(_subarray(wrapped, n - blk)), ecb.final()]);
  var iv2 = _bufferAlloc(blk);
  ...
  var pass1 = _bufferConcat([d1.update(wrapped), d1.final()]);
...
function _oaepHashFromParams(paramsBytes) {
  ...
  _forEach(node.children || [], function (ch) {
...
function _originatorSpki(originator) {
  ...
  if (v.algorithm.parameters != null) _push(algKids, b.raw(v.algorithm.parameters));
...
function _explicitSetOf(implicitBytes) {
  ...
  var out = _bufferFrom(implicitBytes);
  out[0] = 0x31;
  return out;
}
...
function _isDerNull(p) { return _isBuffer(p) && p.length === 2 && p[0] === 0x05 && p[1] === 0x00; }
...
async function _verifyAuthenticatedData(parsed, km, opts) {
  ...
  if (!hash) throw _err("cms/unsupported-algorithm", "unsupported macAlgorithm " + _stringify(macAlg.name || macAlg.oid) + " (HMAC-SHA-256/384/512 only)");
  ...
  if (parsed.authAttrsBytes) {
    ...
    if (!dHash) throw _err("cms/unsupported-algorithm", "unsupported digestAlgorithm " + _stringify(parsed.digestAlgorithm && parsed.digestAlgorithm.name));
    ...
    var mdAttr = _filter(parsed.authAttrs || [], function (a) { return a.type === O("messageDigest"); })[0];
    mdCheck = { hash: dHash, declared: asn1.read.octetString(asn1.decode(mdAttr.values[0])) };
  } else {
    preimage = _bufferFrom(content);
  }
  ...
  for (var ci = 0; ci < candidates.length; ci++) {
    try {
      ...
      try {
        ...
        if (!(await subtle.verify({ name: "HMAC" }, key, _bufferFrom(parsed.mac), preimage))) throw _fail();
        if (mdCheck) {
          var actual = _bufferFrom(await subtle.digest(mdCheck.hash, content));
          if (!_equals(actual, mdCheck.declared)) throw _fail();   // sec. 9.3: recompute, never trust the originator's digest
        }
        return _originFields({
          content: _bufferFrom(content),
...
function _normKeyDer(key) {
  if (_isBuffer(key)) return { der: key, owned: false };
  if (key instanceof Uint8Array) return { der: _bufferFrom(key), owned: true };
...
function _normCertDer(cert) {
  if (_isBuffer(cert) || cert instanceof Uint8Array) return guard.bytes.snapshot(cert, CmsError, "cms/bad-input", "the recipient certificate");
