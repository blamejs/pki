...
var _passThrough = intrinsic.assign(intrinsic.create(null), { "cms/unsupported-algorithm": 1, "cms/unsupported-recipient-type": 1, "cms/bad-input": 1, "cms/iteration-limit": 1, "cms/missing-key-derivation": 1, "cms/no-encrypted-content": 1, "cms/too-many-recipient-keys": 1 });
...
/** @internal One KeyAgreeRecipientInfo can carry an encrypted key for several recipients under one
 * originator and one key-derivation, and a caller decrypting with a private key alone has no identifier
 * to select its own entry by. The key-encryption key is derived once and every entry is offered to it;
 * AES key wrap's integrity check settles which entry belongs to this key, so an entry that opens is one
 * this key was addressed by. The attempt count is bounded because the list is attacker-supplied. */
async function _kariUnwrapAny(kek, reks) {
  var list = reks || [];
  /** @internal The list arrives with the message, so the number of unwraps it can demand is bounded.
   * The bound caps the attempts rather than refusing the list, so an entry within it still opens the
   * message it always opened. */
  var tried = list.length < C.LIMITS.CMS_MAX_KARI_ENCRYPTED_KEYS
    ? list.length : C.LIMITS.CMS_MAX_KARI_ENCRYPTED_KEYS;
  for (var i = 0; i < tried; i++) {
    try { return await _aesKwUnwrap(kek, list[i].encryptedKey); }
    catch (_e) { continue; } // allow:swallow-unverified this entry is not addressed to this key; a later one may be
  }
  if (list.length > tried) {
    throw _err("cms/too-many-recipient-keys", "a KeyAgreeRecipientInfo names " + list.length +
      " encrypted keys and none of the first " + tried + " opened; at most " + tried + " are tried");
  }
  throw _fail();
}
...
async function _kariCek(ri, km) {
  ...
  try {
    ...
    var rek = (km.cert != null && _kariRekFor(ri, x509.parse(_normCertDer(km.cert)))) || null;
    ...
    if (_isMont(origSpki)) {
      ...
      var curveOid = (km.cert != null && _ecCurveFromCert(km.cert)) ||
        (origHasParams ? asn1.read.oid(origAlg.children[1]) : _ecCurveFromPrivateKey(keyDer));
    ...
    /** @internal A caller that supplied a certificate selects its entry by recipient identifier, which
     * is what that certificate is for. Offering every entry is for the caller that has only a key, and
     * so has no identifier to select by. */
    if (km.cert != null) {
      var chosen = rek || ri.recipientEncryptedKeys[0];
      if (!chosen) throw _fail();
      return await _aesKwUnwrap(kek, chosen.encryptedKey);
    }
    return await _kariUnwrapAny(kek, ri.recipientEncryptedKeys);
  } finally {
...
async function _kemriCek(ri, km) {
  var k = ri.kemri;
  var isComposite = compositeKem.isCompositeKem(k.kem.oid);
  var wcName = isComposite ? null : _mlkemName(k.kem.oid);
  if (!isComposite && !wcName) throw _err("cms/unsupported-algorithm", "unsupported KEM " + k.kem.oid);
  ...
  try {
    if (isComposite) {
      var ckey = _normKeyDer(km.key);
      try {
        if (pkcs8.parse(ckey.der).privateKeyAlgorithm.oid !== k.kem.oid) throw _fail();
        ss = await compositeKem.decapsulate(ckey.der, kemct);
      } finally { _releaseKeyDer(ckey); }
    } else {
...
/** @internal RFC 4055 sec. 4.1 gives every RSAES-OAEP-params field a default: `hashFunc` sha1,
 * `maskGenFunc` mgf1SHA1, `pSourceFunc` pSpecifiedEmpty. An omitted field therefore NAMES a value
 * rather than leaving the question open, so each default is resolved here before the fields are
 * compared. Reading an absent `maskGenFunc` as "whatever hashFunc said" would enforce the equality
 * below on a parameter set that spells the mask generator out and skip it on the same set written with
 * the field omitted. The engine takes one hash for both, so a set naming two is one it cannot perform. */
function _oaepHashFromParams(paramsBytes) {
  if (paramsBytes == null) return "SHA-1";
  var node = asn1.decode(paramsBytes);
  var hashName = "SHA-1", mgfHash = "SHA-1", label = null;
  ...
  if (mgfHash !== hashName) throw _err("cms/unsupported-algorithm", "the OAEP MGF1 hash must equal the OAEP hash");
  if (label != null && label.length > 0) throw _err("cms/unsupported-algorithm", "a non-empty OAEP label is not supported");
  return hashName;
}
var HASH_W3C = intrinsic.create(null); HASH_W3C[O("sha1")] = "SHA-1"; HASH_W3C[O("sha256")] = "SHA-256"; HASH_W3C[O("sha384")] = "SHA-384"; HASH_W3C[O("sha512")] = "SHA-512";
function _hashW3c(o) { if (!HASH_W3C[o]) throw _err("cms/unsupported-algorithm", "unsupported OAEP hash " + o); return HASH_W3C[o]; }

var X963_HASH = intrinsic.create(null);
...
var MONT = intrinsic.create(null); MONT[O("X25519")] = { name: "X25519", hkdf: "SHA-256" }; MONT[O("X448")] = { name: "X448", hkdf: "SHA-512" };
...
var CURVE = intrinsic.create(null); CURVE[O("prime256v1")] = { curve: "P-256" }; CURVE[O("secp384r1")] = { curve: "P-384" }; CURVE[O("secp521r1")] = { curve: "P-521" };
...
/** @internal The curve of the recipient's own private key. RFC 5753 sec. 7.1 lets an originator omit its
 * curve parameters because the recipient already knows the curve; a recipient decrypting with a key and
 * no certificate has that knowledge in the key itself, and nowhere else. */
function _ecCurveFromPrivateKey(keyDer) {
  var spki;
  try {
    spki = nodeCrypto.createPublicKey({ key: keyDer, format: "der", type: "pkcs8" })
      .export({ format: "der", type: "spki" });
  } catch (_e) { return null; } // allow:swallow-unverified a key this runtime cannot read names no curve
  var alg = asn1.decode(spki).children[0];
  if (asn1.read.oid(alg.children[0]) !== O("ecPublicKey") || alg.children.length < 2) return null;
  return asn1.read.oid(alg.children[1]);
}
...
var MLKEM = intrinsic.create(null); MLKEM[O("id-ml-kem-512")] = "ML-KEM-512"; MLKEM[O("id-ml-kem-768")] = "ML-KEM-768"; MLKEM[O("id-ml-kem-1024")] = "ML-KEM-1024";
...
var MAC_HASH = intrinsic.assign(intrinsic.create(null), { hmacWithSHA256: "SHA-256", hmacWithSHA384: "SHA-384", hmacWithSHA512: "SHA-512" });
var DIGEST_WC = intrinsic.assign(intrinsic.create(null), { sha256: "SHA-256", sha384: "SHA-384", sha512: "SHA-512" });
...
async function _verifyAuthenticatedData(parsed, km, opts) {
  ...
  if (parsed.authAttrsBytes) {
    ...
    guard.der.assertParams(parsed.digestAlgorithm.parameters, "absentOrNull", _err,
      "cms/unsupported-algorithm", parsed.digestAlgorithm.name + " digestAlgorithm (RFC 5754 sec. 2)");
