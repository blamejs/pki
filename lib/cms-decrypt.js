...
function _ridMatches(ri, cert) {
  var rid = ri.rid || (ri.recipientEncryptedKeys && ri.recipientEncryptedKeys[0] && ri.recipientEncryptedKeys[0].rid) || (ri.kemri && ri.kemri.rid);
  if (!rid) return false;
...
async function _kariCek(ri, km) {
  ...
  var rek = ri.recipientEncryptedKeys[0];
  ...
  if (_isMont(origSpki)) {
    ...
    var curve = _curveName(origSpki);
    var recipEc = await subtle.importKey("pkcs8", keyDer, { name: "ECDH", namedCurve: curve.curve }, false, ["deriveBits"]);
    var origEc = await subtle.importKey("spki", origSpki, { name: "ECDH", namedCurve: curve.curve }, false, []);
...
function _pbkdf2Params(paramsDer, opts) {
  ...
  var cap = opts.maxIterations != null ? Math.min(opts.maxIterations, C.LIMITS.PBKDF2_MAX_ITERATIONS) : C.LIMITS.PBKDF2_MAX_ITERATIONS;
...
// _x963Hash, and _originatorSpki / _curveName that follow): reachable only from a fully well-formed
// recipient that names an inner algorithm we do not implement (a non-registry prf / OAEP hash / kari
// scheme / originator curve). The producer never emits one; the fuzz harness (fuzz/cms-decrypt.fuzz.js)
// drives these arms behaviorally under the PkiError-only contract.
...
function _oaepHashFromParams(paramsBytes) {
  if (paramsBytes == null) return "SHA-1"; // absent = the RFC 3560 defaults (accept floor)
  var node = asn1.decode(paramsBytes);
  var hashName = "SHA-1";
  (node.children || []).forEach(function (ch) {
    if (ch.tagClass === "context" && ch.tagNumber === 0) { var h = asn1.read.oid(ch.children[0].children[0]); hashName = _hashW3c(h); }
  });
  return hashName;
}
...
function _curveName(spki) {
  var alg = asn1.decode(spki).children[0];
  var curveOid = asn1.read.oid(alg.children[1]);
  var c = CURVE[curveOid];
  if (!c) throw _err("cms/unsupported-algorithm", "unsupported originator EC curve");
  return c;
}
