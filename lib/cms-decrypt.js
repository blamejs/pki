...
async function _kemriCek(ri, km) {
  ...
  var ss = null, kek = null;
  try {
    ss = Buffer.from(await subtle.decapsulateBits({ name: wcName }, priv, kemct));
    var ssKey = await subtle.importKey("raw", ss, { name: "HKDF" }, false, ["deriveBits"]);
    kek = Buffer.from(await subtle.deriveBits({ name: "HKDF", hash: "SHA-256", salt: Buffer.alloc(0), info: _kemOtherInfo(wrapAlg.name, kekBytes, k.ukm || null) }, ssKey, kekBytes * 8));
    ...
    guard.secret.zeroizeAll([ss, kek], CmsError, "cms/bad-input", "the KEM shared secret");
  }
}
