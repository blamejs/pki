...
function verifyCrlSignature(crl, spkiBytes) {
  // pki.crl.sign requires path-validate for the injection side-effect, so the engine is set on every
  ...
  if (_engine == null) throw new CrlError("crl/bad-input", "the crl-verify signature engine is not initialized (require pki before use)");
