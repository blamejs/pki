...
var NULL_PARAMS_DER = _bufferFrom([0x05, 0x00]);
...
async function compress(content, opts) {
  ...
  if (level !== undefined && (typeof level !== "number" || !_isFinite(level) || _floor(level) !== level)) throw _err("cms/bad-input", "opts.level must be an integer");
...
async function decompress(input, opts) {
  ...
  if (opts.maxOutputBytes !== undefined) {
    ...
    if (typeof mo !== "number" || !_isFinite(mo) || mo <= 0 || _floor(mo) !== mo) throw _err("cms/bad-input", "opts.maxOutputBytes must be a positive integer");
  ...
  if (alg.parameters != null && _bufferCompare(alg.parameters, NULL_PARAMS_DER) !== 0) throw _err("cms/bad-algorithm-parameters", "id-alg-zlibCompress parameters must be absent or NULL (RFC 3274 sec. 2)");
