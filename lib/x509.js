...
function _attrValueToString(node) {
  try { return asn1.read.string(node); }
    ...
    // Surface it as a CertificateError so parse fails closed on the documented
    // x509/* contract rather than leaking a raw asn1/* code to the caller.
    // Only a value whose tag is not a recognized string type (an ANY-typed
    // attribute) falls back to a hex #-string so the DN stays representable.
    if (!e || e.code !== "asn1/expected-string") {
