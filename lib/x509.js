...
function algorithmIdentifier(ns) {
  return schema.seq([schema.field("algorithm", schema.oidLeaf())], {
    ...
    build: function (m, ctx) {
      ...
      return { oid: dotted, name: ctx.oid.name(dotted) || null, parameters: m.node.children.length > 1 ? m.node.children[1].bytes : null };
    },
  });
}
