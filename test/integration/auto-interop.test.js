...
var NO_FIXTURE_YET = {
  ...
  "pki.schema.trustanchor": "no toolchain in the interop image READS RFC 5914: openssl has no trust-anchor verb and NSS ships no TrustAnchorList reader, so there is no independent implementation to disagree with the envelope. Two things ARE independently checkable and are not checked today, and this row stands for both. The `certificate` arm embeds a whole Certificate and the `tbsCert` arm a whole TBSCertificate, both of which openssl's decoder reads: `asn1parse -strparse` into a built list would have an independent implementation confirm the embedded certificate survives the embedding byte for byte. And the `taInfo` arm's `certPath` carries NameConstraints and CertificatePolicies in their RFC 5280 encodings, which openssl reads with its own compiled templates. A fixtures row doing either would cover more than the round-trip through this toolkit's own reader, which is all the schema-trustanchor vectors establish",
