...
async function run() {
  ...
  try {
    ...
    check("pki.pkcs12.verifyMac accepts the OpenSSL classic PKCS#12", (await pki.pkcs12.verifyMac(oClassicDer, PW)).valid === true);
    check("pki.pkcs12.verifyMac rejects a wrong password on the OpenSSL store", (await pki.pkcs12.verifyMac(oClassicDer, "wrong")).valid === false);
    ...
    if (ep.code === 0) {
      ...
      check("pki.pkcs12.verifyMac accepts the OpenSSL PBMAC1 PKCS#12", (await pki.pkcs12.verifyMac(pbmac1Der, PW)).valid === true);
    } else {
