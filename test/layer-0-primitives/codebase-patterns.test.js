...
function testNoDuplicateCodeBlocks() {
  ...
  var KNOWN_CLUSTERS = [
    ...
    },
    {
      ...
      files: ["lib/acme.js:client", "lib/acme.js:_sendFollowing", "lib/acme.js:_tlsFor", "lib/est.js:_client", "lib/est.js:step", "lib/est.js:_tlsFor", "lib/scep.js:_client", "lib/scep.js:step", "lib/scep.js:_hopTls", "lib/cmp-build.js:_transfer", "lib/ct.js:fetchLogList", "lib/ct.js:_fetchBody", "lib/path-validate.js:build", "lib/path-validate.js:_aiaFetchOne"],
      mode: "family-subset",
      reason: "network-client entry + response glue: URL parse + default-transport trust-anchor gate (explicit anchor|useSystemStore else */no-trust-anchors, then pki.transport.https) + guard.limits.cap timeout/maxResponseBytes budgets + opts.tls->request.tls mapping + the response-body re-view (guard.bytes.view) and size recheck, plus the redirect-follow step and the per-hop mTLS scoping (_tlsFor/_hopTls: strip cert/key/servername on a cross-origin hop so the client identity never leaves the initial origin). The transport/guard/budget primitives are shared in http-transport.js / guard-limits.js; each client binds a different domain and protocol shape (acme stateful session, est/scep functional verbs, cmp stateless transfer, ct fetch-then-verify, path-validate's opt-in AIA caIssuers fetch), so the glue recurs without being further extractable.",
    },
    {
