...
async function run() {
  ...
  check("DG-space-boundary. membership is by LITERAL URI prefix (RFC 7616 sec. 3.5 / RFC 2617 sec. 1.2): /a covers /ab", httpDigest.inProtectionSpace(chDom, ORIG, "/ab") === true && httpDigest.inProtectionSpace(chDom, ORIG, "/b") === false);
  ...
  // DG-realm-utf8: the realm arrives from the wire. A Latin-1-decoding transport (the Node default) presents a
  // UTF-8 realm's octets one-per-char, so answer() must hash those octets RAW -- re-encoding as UTF-8 would
  // double-encode and disagree with the server. realmOctets is the UTF-8 encoding of "re-acute" as Latin-1 chars.
  var realmOctets = "r" + String.fromCharCode(0xc3) + String.fromCharCode(0xa9);
  var chRealm = httpDigest.parseChallenge('Digest realm="' + realmOctets + '", nonce="n", qop="auth", algorithm=SHA-256, charset=UTF-8', E, "bad");
  ...
  check("DG-p-badcharset. a charset other than UTF-8 is rejected (RFC 7616 sec. 3.3 permits only UTF-8)", codeOf(function () { httpDigest.parseChallenge('Digest realm="r", nonce="n", qop="auth", algorithm=SHA-256, charset=ISO-8859-1', E, "est/digest-bad-challenge"); }) === "est/digest-bad-challenge");
  check("DG-p-quotedcharset. a QUOTED charset (must be the unquoted token UTF-8) is rejected", codeOf(function () { httpDigest.parseChallenge('Digest realm="r", nonce="n", qop="auth", algorithm=SHA-256, charset="UTF-8"', E, "est/digest-bad-challenge"); }) === "est/digest-bad-challenge");
  check("DG-p-quotedstale. a QUOTED stale directive (must be an unquoted token) is rejected", codeOf(function () { httpDigest.parseChallenge('Digest realm="r", nonce="n", qop="auth", algorithm=SHA-256, stale="true"', E, "est/digest-bad-challenge"); }) === "est/digest-bad-challenge");
  check("DG-p-badstale. a stale value other than true/false is rejected (not parsed as false)", codeOf(function () { httpDigest.parseChallenge('Digest realm="r", nonce="n", qop="auth", algorithm=SHA-256, stale=maybe', E, "est/digest-bad-challenge"); }) === "est/digest-bad-challenge");
  check("DG-p-quoteduserhash. a QUOTED userhash directive is rejected (must be an unquoted token)", codeOf(function () { httpDigest.parseChallenge('Digest realm="r", nonce="n", qop="auth", algorithm=SHA-256, userhash="true"', E, "est/digest-bad-challenge"); }) === "est/digest-bad-challenge");
  check("DG-p-baduserhash. a userhash value other than true/false is rejected", codeOf(function () { httpDigest.parseChallenge('Digest realm="r", nonce="n", qop="auth", algorithm=SHA-256, userhash=maybe', E, "est/digest-bad-challenge"); }) === "est/digest-bad-challenge");
  var chUhf = httpDigest.parseChallenge('Digest realm="r", nonce="n", qop="auth", algorithm=SHA-256, userhash=false', E, "bad");
