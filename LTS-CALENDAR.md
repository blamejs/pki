...
|------------------|---------------|----------------------------|--------------|-----------------------------------------|
| `v0.x` (pre-1.0) | 2026          | until v1.0 ships           | 24.21        | ML-DSA / SLH-DSA + classical (RSA/ECDSA/EdDSA) signatures via the WebCrypto engine |
...
- Critical and high-severity vulnerabilities in the toolkit's own code: parser denial-of-service, verification bypass, or a structure that mis-encodes in a way that misrepresents a certificate or message.
- Vendored-dependency CVE refreshes. The SECURITY.md commitment applies on the LTS line too.
- Algorithm-registry updates required by NIST or IETF deprecations of an algorithm the toolkit resolves.
...
The toolkit is post-quantum-first: ML-DSA and SLH-DSA signatures run in the WebCrypto engine alongside the classical RSA, ECDSA, and EdDSA set, and every algorithm is named in the OID registry (`pki.oid`), so a new algorithm is a registry entry rather than a special case. Signature verification resolves the algorithm from the OID carried in the certificate or message, so a signature verifies under the identifier in the bytes rather than under a single active default. When an algorithm is added or deprecated, the change ships as a minor on the current major and this row is updated in the same commit.

## Node minimum policy

The "Node minimum" column is the lowest Node major the toolkit supports for that line. It tracks Node's own active-LTS schedule: a new major adopts whichever Node major is the active LTS at the time. Once on the LTS line, the Node minimum is frozen for that major's security-patch window, so consumers on the LTS line are not forced onto a newer Node mid-window. Nothing is transpiled, so the supported Node version is exactly the version the source runs on.

## Pre-1.0 caveat

`v0.x` carries no LTS commitment. Every release may change something consumers depend on, and the algorithm posture and the surface are deliberately still evolving. Read [CHANGELOG.md](CHANGELOG.md) before upgrading across more than a few patches at a time. The LTS calendar takes effect at v1.0.

## Experimental primitives are exempt

Primitives documented `@status experimental`, shown as "experimental" on each wiki page, are not covered by the stability contract or the LTS window. They may change signature, behavior, or wire format, or be removed, in any minor, without the deprecation cycle stable primitives get. This applies on the LTS line too. The exemption exists so the toolkit can ship primitives that track in-flight standards — draft RFCs, pre-IANA codepoints, newly published algorithm identifiers — without freezing an unsettled format for a major's full support window. A primitive graduates to stable by dropping the `@status experimental` marker in a release whose notes call out the graduation.
...
- **Deprecation is bounded.** A `deprecated` primitive must declare `@deprecated <remove-by-version>`, and the gate fails once that version ships, so a deprecation is actually removed rather than lingering. Deprecation warnings ship at least one minor before removal.
