...
var OUT_OF_BAND_BREAKS = [
  {
    release: "v0.6.12",
    surface: "pki.crl.verify, pki.pkcs12.verifyMac, and pki.ct.verifySctWithLogList",
    summary: "Each returned a bare boolean and now returns a verdict object; read `res.valid` where you read the boolean.",
    migration: [
  ...
  {
    release: "v0.6.11",
    surface: "pki.path.validate(path, opts) and pki.tsp.verify(token, data, opts)",
    summary: "The trust anchor option is now `trustAnchors` (a single anchor or an array); the former singular `trustAnchor` is removed and refused by name.",
    migration: [
  ...
  {
    release: "v0.5.24",
    surface: "pki.acme.client(...).downloadCertificate(url, opts)",
    summary: "The verb needs to be told which certificate the download is allowed to be, and refuses with acme/binding-required when it is not.",
    migration: [
  ...
  {
    release: "v0.5.8",
    surface: "pki.smime.verify(...).headerProtection.fromMismatch",
    summary: "Now null when there was no protected From to compare against, where it used to be false.",
    migration: [
  ...
  {
    release: "v0.5.8",
    surface: "pki.merkle.verifyConsistency({ oldSize: 0, newSize: n }) where n > 0",
    summary: "Refused as merkle/no-consistency-claim rather than answered `true`.",
    migration: [
