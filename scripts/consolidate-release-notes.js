...
"use strict";
/**
 * scripts/consolidate-release-notes.js
 *
 * Roll up per-patch `release-notes/v<X>.<Y>.<Z>.json` files for any MINOR
 * line OTHER than the current one into a single consolidated
 * `release-notes/v<X>.<Y>.x.json` file. The current minor (derived from
 * `package.json#version`) stays as per-patch files so the release flow
 ...
 *   }
 *
 * The `generate-changelog-entry.js` lookup falls back to the consolidated
 * file when the per-patch file is absent, so deleting per-patch files after
 * rollup (via `--prune`) keeps the generator working for the historical
 * line.
 *
 * Usage:
 *   node scripts/consolidate-release-notes.js            # preview — writes consolidated files, keeps per-patch
 ...
 *
 * Idempotent: re-running is safe. If a previous run consolidated with
 * `--prune`, the next run sees the consolidated file in place and the
 * per-patch files absent, and reports "already consolidated".
 */
...
function _scan() {
  // Returns {
  //   perPatchByMinor: { "0.1": [ { version, file, payload }, ... ], ... },
  //   consolidatedByMinor: { "0.1": file, ... },
  // }
  var perPatchByMinor     = {};
...
function _stripTopSchema(obj) {
  // The inner release objects don't carry their own $schema — the wrapper
  // carries it for the consolidated file.
...
function main() {
  ...
  if (minors.length === 0 && Object.keys(scan.consolidatedByMinor).length === 0) {
    _exit("no per-version OR consolidated release notes found in " + path.relative(ROOT, NOTES_DIR));
  }
  ...
  // Sanity-check existing consolidated files for the current minor — if the
  // current minor accidentally has a v<minor>.x.json, that's a mistake
  // (consolidation should NEVER apply to the active line); the operator
  // should hand-fix.
