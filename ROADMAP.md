...
- **Certificate Transparency SCTs** — *Parsing + signature verification shipped.* `pki.ct.parseSctList` decodes the SCT-list extension to per-SCT log id, exact timestamp, named algorithm, and raw signature (unknown versions preserved opaque); `pki.ct.reconstructSignedData` rebuilds the exact digitally-signed preimage; `pki.ct.verifySct(entry, sct, logKey)` verifies an SCT's signature against a log's public key per RFC 6962 §3.2 — reconstructing the signed data, routing an ECDSA signature through the shared strict DER-conformance gate before conversion to the raw form the crypto engine expects, and verifying through `pki.webcrypto`. *Planned next:* SCT-list encode/sign (the producing side) and the CT log-list trust surface. RFC 6962.
