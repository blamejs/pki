...
- **WebAuthn / passkey attestation** — *Shipped (attestation-statement verification).* `pki.webauthn.parseAttestationObject` decodes the attestation object + authenticatorData + COSE credential key over the strict `pki.cbor` codec, and `pki.webauthn.verify` cryptographically verifies each defined attestation-statement format — packed, tpm, android-key, apple, fido-u2f, none — checking the signature and each format's structural bindings (the x5c leaf key, the apple nonce, the tpm `certInfo` Name/`extraData` over the `pubArea`, the android `KeyDescription`, the fido-u2f `verificationData`), binding every attestation certificate key to the credential key, and enforcing each leaf's certificate requirements. Verified against real attestations from all five formats. Chaining the returned trust path to a pinned root — and resolving a root from an authenticator's aaguid via the FIDO Metadata Service — remains *Planned*, re-openable behind a caller-supplied root set or an `opts.mdsBlob`; the verifier surfaces the x5c chain for `pki.path.validate`. W3C WebAuthn Level 3.
