...
- **SignedData** — *Verification shipped; signing Targeted.* `pki.cms.verify(input, opts)` verifies a CMS SignedData signature (RFC 5652 §5) — attached and detached content, single- and multiple-signer, RSA / RSASSA-PSS / ECDSA / EdDSA. It locates each signer certificate by its issuerAndSerialNumber or subjectKeyIdentifier (embedded or caller-supplied) and checks the signature over the exact §5.4 preimage: with signed attributes present, the message-digest attribute must equal the content digest and the signature covers the DER re-encoding of the SignedAttributes (the on-wire `[0]` implicit tag replaced by a universal SET OF); otherwise the signature covers the content directly. Each parameter comes from the correct structure — the content digest, the signer's digest algorithm, and the key's own algorithm identifier are kept distinct. It reports a per-signer verdict and does not chain the signer certificate to a trust anchor — that is the caller's step through `pki.path.validate`. *Signing* (the producing side), unsigned attributes, and composite-signature `SignerInfo` verification remain *Targeted*.
