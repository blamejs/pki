...
- **CMP** — *Parse and build shipped.* `pki.schema.cmp.parse` decodes an RFC 9810 PKIMessage — the protected transport envelope CMP enrollment, revocation, confirmation, and support exchanges ride — into its header, its 27-arm body (certificate requests via the CRMF parser, encrypted certificates via CMS, response / revocation / confirmation / error / support / polling arms structural, the rest raw), protection inputs, and extra certificates, surfacing the exact byte slices an external verifier reconstructs the protected part from. `pki.cmp.build` assembles a protected PKIMessage on the request side — a header (sender / recipient, including the anonymous NULL-DN, plus transaction metadata) and a single-arm body: an `ir` / `cr` / `kur` request (a CertReqMessages via `pki.crmf.build`), a `p10cr` (a PKCS#10 CertificationRequest), or a `certConf` / `pollReq` / `genm` / `rr` transaction — protected by a sender-key signature (any registry algorithm, resolved from the signer certificate) or a PBMAC1 shared-secret HMAC (RFC 9481 / 9483). The protection is computed over the exact ProtectedPart DER, the protectionAlg is derived rather than caller-set, and the protection self-verifies before the message is returned, so every message round-trips through the parser. HTTP transfer (RFC 9811), the CA-response arms, the lightweight-profile (RFC 9483) validation rules, and incoming MAC / signature verification are *Planned*. RFC 9810 / RFC 9483 / RFC 9481.
