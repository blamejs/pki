...
- **Time-Stamp Protocol** — *Token creation + parsing shipped.* `pki.tsp.sign(messageImprint, tsa, opts)` creates an RFC 3161 TimeStampToken — a CMS SignedData (over `pki.cms.sign`) whose content is a `TSTInfo` carrying the message imprint, TSA policy, serial number, and `genTime` (with optional accuracy / nonce / ordering), plus the RFC 3161 §2.4.2 / RFC 5816 signing-certificate (ESSCertIDv2) attribute binding the token to the TSA certificate. `pki.schema.tsp.parseToken` decodes one. *Planned next:* the TimeStampReq / TimeStampResp request-response wrappers and token verification helper.
