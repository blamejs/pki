...
- **Human-readable inspection (`openssl x509 -text` in pure JS)** — *Under design.* A text renderer that turns a parsed certificate — and, on the same surface, a CRL / CSR / CMS structure — into the familiar OpenSSL-style report: version, serial, signature algorithm, the issuer and subject distinguished names, the validity window, the public-key details (curve or modulus size, the raw point/modulus), and every decoded extension printed with its critical flag and value; the same call also returns a structured object for programmatic use. So an operator can ingest a PEM or DER blob and read exactly what it contains without shelling out to OpenSSL. Built over the strict parser and the two-way OID registry, so the algorithm, curve, and extension names it prints are the same vocabulary the verifier resolves — no separate, drift-prone naming table.
