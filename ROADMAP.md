...
- **Certificate Transparency SCTs** — *Parse, verify, encode, and sign shipped.* `pki.ct.parseSctList` decodes the SCT-list extension to per-SCT log id, exact timestamp, named algorithm, and raw signature (unknown versions preserved opaque); `pki.ct.reconstructSignedData` rebuilds the exact digitally-signed preimage; `pki.ct.verifySct(entry, sct, logKey)` verifies an SCT's signature against a log's public key per RFC 6962 §3.2 — reconstructing the signed data, routing an ECDSA signature through the shared strict DER-conformance gate before conversion to the raw form the crypto engine expects, and verifying through `pki.webcrypto`. The producing side: `pki.ct.encodeSctList(scts)` builds the SCT-list extension value byte-for-byte (the exact inverse of `parseSctList`), and `pki.ct.signSct(entry, logKey)` performs a log's signing step — rebuilding the same signed-data preimage the verifier hashes and signing it with the log's ECDSA-P-256 / RSA key to yield a v1 SCT `verifySct` accepts. *Planned next:* the CT log-list trust surface. RFC 6962.
