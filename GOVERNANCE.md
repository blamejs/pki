...
`@blamejs/pki` is a pure-JavaScript PKI toolkit with a published LTS calendar and a documented threat model. This document captures how decisions get made, who makes them, and what happens if the maintainer becomes unavailable.
...
- **Technical direction.** Maintainer-final. Consumer input arrives via GitHub Issues + Discussions; the maintainer weighs it but the final call rests with them. There is no formal vote. The capability plan is public in [ROADMAP.md](ROADMAP.md).
- **Security-vulnerability triage.** Per [SECURITY.md](SECURITY.md) — coordinated disclosure via GitHub Security Advisories, a fix target for High / Critical vulnerabilities in the toolkit's own code and in vendored dependencies, and a public advisory on remediation.
- **Consumer-impacting changes.** Pre-1.0 the toolkit reserves the right to break consumer-facing surface in any minor version; major versions ship deprecation warnings at least one minor before removal. Post-1.0 the same contract applies across majors with a 24-month LTS window per [LTS-CALENDAR.md](LTS-CALENDAR.md).
...
Bus-factor-1 is the largest non-technical risk the project carries. This section documents the recovery path so a consumer depending on the toolkit has a defensible plan if the maintainer becomes unavailable.
...
- An aligned contributor with sustained commit cadence to a core area (the codec, the OID registry, the certificate/message structures, the crypto engine, or the release workflow).
- Demonstrated familiarity with the architectural decisions recorded in [ARCHITECTURE.md](ARCHITECTURE.md), the release notes, and `memory/specs/` (the latter is maintainer-local, but the public architecture and roadmap docs plus the source itself should let a successor reconstruct the decision context).
- A documented commitment to the project's stated discipline: zero npm runtime deps, post-quantum-first defaults, fail-closed parsing, and standards-as-contract.
...
The GitHub Organization (`blamejs`) is currently single-owner. The maintainer commits to adding a second organization owner within 30 days of naming a designated successor. Until then, the maintainer-incapacitation path goes through GitHub Support's account-recovery flow (DNS + 2FA recovery codes).
...
3. Update the consumer-side verification files + the SECURITY.md fingerprint.
...
Identity-based; rotation is automatic when GitHub Organization ownership rotates. No standalone key material to manage.
...
Maintainer-local notes under `memory/specs/` are NOT consumer-facing and are not durable — a successor inheriting the project relies on the public artifacts above plus the source code itself.
...
- **Contact channel:** `security@pkijs.com` per SECURITY.md. The mailbox is dual-controlled at the email + DNS level (DNS held by the registrar with 2FA + recovery email; mail routed through the registrar's MX records).
...
- bus-factor risk class for solo-maintainer popular projects.
